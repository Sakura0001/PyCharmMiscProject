...
>
> 本文的 P0/P1 项已全部闭环，P2/P3 也已落地，请以 `FIX_LOG.md` 为准：
> - G1 QPS 采集 ✅；G5 DDL Fuzz 只做 ADD COLUMN 且不校验返回值 → ✅ 已加 MODIFY 轮次并**校验每条 DDL**
>   （实测 4000 条全部成功）
> - per-DML 延迟分位 ✅（p50/p95/p99/max + 尖刺倍数）；外键表 / 分区表并发 ✅（外键矩阵 + S7 场景）
> - §4.3 列为"不在本次范围"的**崩溃恢复**与**主备复制**已实现：
>   `scenarios/replication_and_crash.py`（R1 binlog 7/7、C1/C2 崩溃恢复各 14/14）
> - 大表非 INT 类型 / 5000 万行矩阵仍需在内网或更长窗口执行（`--rows-scale` 已就绪）
>
