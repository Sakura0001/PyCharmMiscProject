...
>
> 本文的主要过期点（**结论错误，务必不要引用**）：
> - VC-08 `VARCHAR(16382)→VARCHAR(16383)` utf8mb4、VC-09 `VARCHAR(65528)→VARCHAR(65529)` latin1、
>   VBIN-03 `VARBINARY(65528)→VARBINARY(65529)` 标注 "✅SUCCESS / minimal_table 是" —— **实测建不出表**
>   （errno 1118 行宽超 65535；含 `id INT` 时 16383×4+2+4=65538）
> - 二分实测真实上界：`VARCHAR(16382)` utf8mb4 / `VARCHAR(65528)` latin1 / `VARBINARY(65528)`，
>   故上限用例已改为 **16381→16382 / 65527→65528 / 65527→65528**
> - §3.1/§3.2 声称"插入超上限值(预期FAIL)"：这些 INSERT 在生成的 SQL 里**整条被注释**（共 2,623 条），
>   从未执行；现已改为真执行 + errno 级校验（5,228 条探针）
> - §6.1 声称的 "emoji × 4095 / 你好 × 5460 / emoji × 8000" 数据多样性在生成器里**并不存在**
>   （只有单个 `'你好'` 与 `'🎉'`）
>
