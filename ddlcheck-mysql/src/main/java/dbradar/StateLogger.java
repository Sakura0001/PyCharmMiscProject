...
public final class StateLogger {
    ...
    private final List<Loggable> pendingCurrentLoggables = new ArrayList<>();
    private boolean currentStateWritten;
    ...
    public synchronized void writeCurrent(StateToReproduce state) {
        if (!logEachSelect) {
            throw new UnsupportedOperationException();
        }
        printState(getCurrentFileWriter(), state);
        currentStateWritten = true;
        flushPendingCurrentLoggables();
        try {
    ...
    public synchronized boolean isCurrentStateWritten() {
        return currentStateWritten;
    }

    public boolean logsCurrentStatements() {
        return logEachSelect;
    }

    public synchronized void writeCurrentIfStateWritten(String input) {
        if (!logEachSelect || !currentStateWritten) {
            return;
        }
        write(databaseProvider.getLoggableFactory().createLoggable(input));
    }

    private synchronized void write(Loggable loggable) {
        if (!logEachSelect) {
            throw new UnsupportedOperationException();
        }
        if (!currentStateWritten) {
            pendingCurrentLoggables.add(loggable);
            return;
        }
        try {
    ...
    private void flushPendingCurrentLoggables() {
        if (pendingCurrentLoggables.isEmpty()) {
            return;
        }
        try {
            for (Loggable pending : pendingCurrentLoggables) {
                getCurrentFileWriter().write(pending.getLogString());
            }
            pendingCurrentLoggables.clear();
        } catch (IOException e) {
            throw new AssertionError(e);
        }
    }
