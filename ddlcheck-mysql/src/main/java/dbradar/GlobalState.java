...
//        readSchema();
    ...
    private ExecutionTimer executePrologue(Query q) throws Exception {
        ...
        if (getOptions().logEachSelect() && !isConnectionLoggingEnabled()) {
            if (logExecutionTime) {
    ...
    protected boolean isConnectionLoggingEnabled() {
        return databaseConnection instanceof SQLConnection
                && ((SQLConnection) databaseConnection).isLoggingEnabled();
    }

    private void attachLoggerToConnection() {
        if (databaseConnection instanceof SQLConnection && logger != null) {
            ((SQLConnection) databaseConnection).setStateLogger(logger);
        }
    }
