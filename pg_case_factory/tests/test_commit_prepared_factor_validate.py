...
from pg_case_factory.commit_prepared_factor_validate import (
    CommitPreparedFactorProgramValidation,
...
class CommitPreparedFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="cprep_validate_"))
        generate_commit_prepared_factor_programs(
    ...
    def test_validation_passes_for_all_12284_files(self) -> None:
        result = validate_commit_prepared_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CommitPreparedFactorProgramValidation
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_commit_prepared_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_commit_prepared_factor_case(case, ROOT)
        tampered = sql.replace("COMMIT PREPARED", "COMMITPREPAREDBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"COMMITPREPARED00001", "COMMITPREPARED00002"}
        result = validate_commit_prepared_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
