...
from pg_case_factory.alter_text_search_template_factor_runtime import (
...
class AlterTextSearchTemplateRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_atst", PG18_DATABASE)
...
class AlterTextSearchTemplateRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_183_cases(self) -> None:
        sql_dir = Path(
            "/tmp/alter_text_search_template_probe_leaf"
        )
        ...
        self.assertEqual(183, len(cases))
        self.assertEqual(183, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        self.assertEqual(183, ordinals[-1])


class AlterTextSearchTemplateRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = AlterTextSearchTemplatePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(
            ...
            AlterTextSearchTemplatePg18Runner(timeout_seconds=0)
        with self.assertRaises(
            AlterTextSearchTemplateRuntimeError
        ):
            AlterTextSearchTemplatePg18Runner(timeout_seconds=301)
...
class AlterTextSearchTemplateComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ALTERTEXTSEARCHTEMPLATE00001"
    ) -> AlterTextSearchTemplateCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = AlterTextSearchTemplateSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = AlterTextSearchTemplateSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_alter_text_search_template_runs(
            run1, run2
        )
    ...
    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = AlterTextSearchTemplateCaseRuntimeResult(
            case_id="ALTERTEXTSEARCHTEMPLATE00001",
            sql_filename="ALTERTEXTSEARCHTEMPLATE00001.sql",
            expected_sqlstate="00000",
