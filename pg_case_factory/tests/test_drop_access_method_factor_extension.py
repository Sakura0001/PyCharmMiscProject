"""Tests for the bounded DROP ACCESS METHOD post-coverage extension expander.

The marginal factor-value-loop (``drop_access_method_factor_loop.py``) is the
...
from pg_case_factory.drop_access_method_factor_extension import (
...
from pg_case_factory.drop_access_method_factor_loop import (
...
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 367
_SUCCESS_EXTENSIONS = 120
_FAILURE_EXTENSIONS = 216
_EXTENSION_MULTISET_SHA256 = (
    "a40b7e4d34f9fd5eba185c8203e2088d33110c8cd0cf4700e682c0afa95ee825"
)
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_access_method"}
...
class DropAccessMethodFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_access_method_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropAccessMethodFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPACCESSMETHOD00032", first.case_id)
        self.assertEqual(f"DROPACCESSMETHOD{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPACCESSMETHOD00032.sql", first.sql_filename)
        self.assertEqual(
            f"DROPACCESSMETHOD{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropaccessmethod_00032_", first.object_prefix)
        self.assertEqual(
            f"dropaccessmethod_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropAccessMethodFactorExtensionCase)
    ...
    def test_privilege_negative_fires_for_non_superuser(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_level"] == "non_superuser":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_am_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["DROP_ACCESS_METHOD_CASCADE"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_access_method_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_access_method_factor_loop_plan(ROOT)
