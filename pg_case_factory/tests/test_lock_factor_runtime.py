...
from pg_case_factory.lock_factor_runtime import (
...
class LockRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_cluster_and_asub(self) -> None:
        # LOCK runner must not collide with the CLUSTER or ALTER
        # SUBSCRIPTION runners' isolated server endpoints.
        self.assertNotIn(PG18_PORT, (55498, 55499))
        self.assertNotIn(PG18_DATABASE, ("pgcf_asub", "pgcf_cluster"))
        self.assertNotIn(
            PG18_SOCKET.name,
            (
                "pgcf-pg18-asub-sock-20260820",
                "pgcf-pg18-cluster-sock-20260820",
            ),
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55500, PG18_PORT)
        self.assertEqual("pgcf_lock", PG18_DATABASE)
        self.assertEqual("pgcf_lock_superuser", PG18_SUPERUSER)
        self.assertEqual(1, MAX_PARALLELISM)
        self.assertEqual(30, PER_FILE_TIMEOUT_SECONDS)


class LockRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_2314_cases(self) -> None:
        sql_dir = Path("/tmp/lock_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_lock_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(2314, len(cases))
        self.assertEqual(2314, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_lock_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(2314, ordinals[-1])


class LockRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = LockPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(LockRuntimeError):
            LockPg18Runner(timeout_seconds=0)
        with self.assertRaises(LockRuntimeError):
            LockPg18Runner(timeout_seconds=301)
...
class LockComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "LOCK00001"
    ) -> LockCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = LockSuiteRun(run_ordinal=1, cases=(case,))
        run2 = LockSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_lock_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = LockCaseRuntimeResult(
            case_id="LOCK00001",
            sql_filename="LOCK00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "LOCK00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(LockRuntimeError):
            LockSuiteRun(run_ordinal=3, cases=())
