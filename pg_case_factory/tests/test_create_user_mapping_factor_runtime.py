"""Tests for create_user_mapping factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_user_mapping_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateUserMappingFactorRuntime(unittest.TestCase):
    def test_constants_distinct_from_create_text_search_parser(
        self,
    ) -> None:
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_cum")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-cum-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        from src.pg_case_factory.create_user_mapping_factor_extension import (
        ...
        from src.pg_case_factory.create_user_mapping_factor_render import (
            generate_create_user_mapping_factor_programs,
        )
        bp = build_create_user_mapping_factor_loop_plan(
            _REPO
        )
        ep = build_create_user_mapping_factor_extension_plan(
            _REPO
        )
        generate_create_user_mapping_factor_programs(
            bp, ep, sql_dir
        ...
        self.assertEqual(1966, len(cases))
        self.assertEqual(1966, len(paths))
    ...
    def test_runner_construction(self) -> None:
        runner = CreateUserMappingPg18Runner()
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(
            ...
            CreateUserMappingPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            CreateUserMappingRuntimeError
        ):
            CreateUserMappingPg18Runner(timeout_seconds=301)
