...
from pg_case_factory.refresh_materialized_view_factor_render import (
...
class RefreshMaterializedViewFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_refresh_materialized_view_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="refresh_materialized_view_render_")
        )
        cls.count = generate_refresh_materialized_view_factor_programs(
    ...
    def test_renders_exactly_1694_files(self) -> None:
        self.assertEqual(1694, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1694, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"REFRESHMATERIALIZEDVIEW{i:06d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_refresh_materialized_view(text)
    ...
    def test_matview_probe_uses_relkind_m(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        seen_relkind = False
        for f in files:
            text = f.read_text()
            if "relkind = 'm'" in text:
                seen_relkind = True
                self.assertIn("pg_catalog.pg_class", text)
        self.assertTrue(seen_relkind)

    def test_no_quoted_create_table_in_fixture(self) -> None:
        # class-2: fixture CREATE TABLE must never be quoted.
        render_src = (
            ROOT
            / "src"
            / "pg_case_factory"
            / "refresh_materialized_view_factor_render.py"
        )
        text = render_src.read_text()
        self.assertEqual(
            0, len(re.findall(r'CREATE TABLE "', text))
        )

    def test_primary_target_is_refresh_materialized_view(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*REFRESH\s+MATERIALIZED\s+VIEW\b",
                    region,
                ),
                f"no REFRESH MATERIALIZED VIEW in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_refresh_materialized_view_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, RefreshMaterializedViewFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_refresh_materialized_view_factor_case(case, root)
        b = render_refresh_materialized_view_factor_case(case, root)
