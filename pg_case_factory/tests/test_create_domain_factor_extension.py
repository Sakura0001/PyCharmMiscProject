...
class CreateDomainFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_domain_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateDomainFactorExtensionPlan
        )
        self.assertEqual(2880, len(plan.cases))
        self.assertEqual(2880, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(98, 2978)), ordinals
        )
        self.assertEqual(
            "CREATEDOMAIN00098", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEDOMAIN02977", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEDOMAIN00098.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEDOMAIN02977.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(2880, outcomes["success"])
        self.assertEqual(0, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_domain_factor_extension_plan(ROOT)
        plan_b = build_create_domain_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_domain_factor_extension_plan(ROOT)
        self.assertEqual(
            "acbe360c4d967271307e689e8ffb2fdd897c9267de71b73f7fc166b5b6b95500",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(97, _BASELINE_COUNT)
        ...
        self.assertEqual(
            ("drop_domain", "cascade_drop_domain"),
            _CLEANUP_MODES,
        )
