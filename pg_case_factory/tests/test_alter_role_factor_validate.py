...
from pg_case_factory.alter_role_factor_validate import (
    AlterRoleFactorProgramValidation,
...
# Module-level cache so the 1,743-program render happens once across tests.
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_role_factor_extension_plan(ROOT)
        programs: dict[str, str] = {}
        for case in baseline_plan.cases:
            programs[case.sql_filename] = render_alter_role_factor_case(
                case, ROOT
            )
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_role_factor_case(c, ROOT) for c in chosen
    }
    selected = {c.case_id for c in chosen}
    return programs, selected


class AlterRoleFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_role_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(report, AlterRoleFactorProgramValidation)
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 9999 is outside the 0001-1743 case range, so it is unknown.
        extra = "ALTERROLE9999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERROLE9999\n"
            "-- primary_obligation_id: AR-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER ROLE bogus LOGIN;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_role_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_role_factor_case(target_case, ROOT)
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(rec["primary_obligation_id"].startswith("ALTERROLE-EXT|"))
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn("alter_role_actual_factor_witness_report", blob)
    ...
    def test_byte_mismatch_is_detected(self) -> None:
        ...
        canonical = render_alter_role_factor_case(target_case, ROOT)
        tampered = canonical.replace("ALTER ROLE", "ALTER  ROLE", 1)
