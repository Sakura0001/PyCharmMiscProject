...
from pg_case_factory.drop_table_factor_render import (
    generate_drop_table_factor_programs,
    render_drop_table_factor_case,
)
from pg_case_factory.validate_drop_table import (
    DropTableFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_table_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropTableFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_table_factor_loop_plan(ROOT)
        cls.extension = build_drop_table_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_table_validate_"))
        generate_drop_table_factor_programs(
        ...
        cls.result = validate_drop_table_factor_programs(
    ...
    def test_validation_passes_for_all_7382_files(self) -> None:
        self.assertIsInstance(
            self.result, DropTableFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(50, self.result.baseline_case_count)
        self.assertEqual(7332, self.result.extension_case_count)
        self.assertEqual(7382, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(7382, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(7382, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_table_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_table_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPTABLE00001", "DROPTABLE00002"}
        result = validate_drop_table_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
