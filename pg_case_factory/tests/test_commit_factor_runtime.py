...
from pg_case_factory.commit_factor_runtime import (
...
class CommitRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_commit", PG18_DATABASE)
...
class CommitRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1844_cases(self) -> None:
        sql_dir = Path("/tmp/commit_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_commit_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_commit_runtime_case_set(ROOT, sql_dir)
...
class CommitRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CommitPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CommitRuntimeError):
            CommitPg18Runner(timeout_seconds=0)
        with self.assertRaises(CommitRuntimeError):
            CommitPg18Runner(timeout_seconds=301)
...
class CommitComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "COMMIT00001"
    ) -> CommitCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CommitSuiteRun(run_ordinal=1, cases=(case,))
        run2 = CommitSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_commit_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CommitCaseRuntimeResult(
            case_id="COMMIT00001",
            sql_filename="COMMIT00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("COMMIT00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CommitRuntimeError):
            CommitSuiteRun(run_ordinal=3, cases=())
