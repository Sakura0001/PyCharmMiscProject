...
class CheckpointFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_checkpoint_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CheckpointFactorExtensionPlan
        )
        self.assertEqual(9216, len(plan.cases))
        self.assertEqual(9216, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(47, 9263)), ordinals
        )
        self.assertEqual(
            "CHECKPOINT00047", plan.cases[0].case_id
        )
        self.assertEqual(
            "CHECKPOINT09262", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CHECKPOINT00047.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(6144, outcomes["success"])
        self.assertEqual(3072, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_checkpoint_factor_extension_plan(ROOT)
        plan_b = build_checkpoint_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_checkpoint_factor_extension_plan(ROOT)
        self.assertEqual(
            "d9a4d619078591ebd004ddb35d0108d67dee0aa1e77ab487c1ef8c79b2335bb1",
