"""Tests for create_publication factor extension plan."""
...
class TestCreatePublicationFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_publication_factor_extension_plan(_REPO)

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(4230, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 1230, "expected_failure": 3000}, dict(counts)
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "ab939818c88d189c4651472b6635a22bf0da863da1580292e8eda181ccf32c8d",
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("CPUB-EXT|"))
