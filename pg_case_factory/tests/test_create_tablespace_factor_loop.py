...
class CreateTablespaceFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_tablespace_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(66) = 67
        ...
        self.assertEqual(67, len({row.obligation_id for row in rows}))
...
class CreateTablespaceFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_tablespace_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateTablespaceFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(67, len(plan.cases))
        self.assertEqual(
            list(range(1, 68)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATETABLESPACE00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATETABLESPACE00067", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATETABLESPACE00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATETABLESPACE00067.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createtablespace_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createtablespace_00067_",
        ...
        self.assertEqual(
            67, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_tablespace_factor_loop_plan(ROOT)
        obligations = (
            compile_create_tablespace_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_tablespace_factor_loop_plan(ROOT)
        again = build_create_tablespace_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_tablespace_factor_loop_plan(ROOT)
        self.assertEqual(
            "6eddabc442a6306ae51174a072f7fa9e895887a5bbf78350af7cb6669ec3f9d0",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(25, outcomes["expected_failure"])
