...
from pg_case_factory.call_factor_validate import (
    CallFactorProgramValidation,
...
class CallFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_17713_files(self) -> None:
        result = validate_call_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, CallFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(49, result.baseline_case_count)
        self.assertEqual(17664, result.extension_case_count)
        self.assertEqual(17713, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(17713, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_call_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(17713, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_call_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_call_factor_case(case, ROOT)
        tampered = sql.replace("CALL", "CALLBOGUS")
