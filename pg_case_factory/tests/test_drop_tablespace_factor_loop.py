...
_OBLIGATION_MULTISET_SHA256 = (
    "82ab12ef2e2dc4f028e96868e1c1c3ccfa4215d5dfef52eb550e80e2dd1c73a1"
)


class DropTablespaceFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_tablespace_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_tablespace_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTablespaceFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_tablespace_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTablespaceFactorLoopPlan)
        ...
        self.assertEqual("DROPTABLESPACE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTABLESPACE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTABLESPACE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTABLESPACE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptablespace_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptablespace_{_OBLIGATION_COUNT:05d}_",
