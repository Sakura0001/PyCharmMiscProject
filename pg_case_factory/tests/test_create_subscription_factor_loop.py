"""Tests for create_subscription factor_loop obligation ledger."""
...
class TestCreateSubscriptionFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_subscription_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(55, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(55, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 54}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "a457c871e092fce81fa4e97a023bc4088ec35549f5a26f6cd63c43654fe29e82",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_subscription_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 42, "expected_failure": 13}, dict(counts)
        )
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATESUBSCRIPTION"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_subscription_factor_loop_obligations(_REPO)
