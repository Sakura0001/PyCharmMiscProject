...
class AlterSubscriptionFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="asub_render_"))
        cls.count = generate_alter_subscription_factor_programs(
    ...
    def test_renders_exactly_1119_files(self) -> None:
        self.assertEqual(1119, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1119, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"ALTERSUBSCRIPTION{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_alter_subscription(text)
    ...
    def test_primary_target_is_alter_subscription(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*ALTER\s+SUBSCRIPTION\b", region
                ),
                f"no ALTER SUBSCRIPTION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_alter_subscription_factor_witness(case, root)
            self.assertIsInstance(
                w, AlterSubscriptionFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_alter_subscription_factor_case(case, root)
        b = render_alter_subscription_factor_case(case, root)
