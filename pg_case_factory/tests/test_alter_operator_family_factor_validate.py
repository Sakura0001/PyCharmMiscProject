"""Byte-witness and conservation validation tests for ALTER OPERATOR FAMILY."""
...
from pg_case_factory.alter_operator_family_factor_render import (
    generate_alter_operator_family_factor_programs,
    render_alter_operator_family_factor_case,
)
from pg_case_factory.alter_operator_family_factor_validate import (
...
class AlterOperatorFamilyFactorValidateTests(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_alter_operator_family_factor_loop_plan(ROOT)
        self.extension = build_alter_operator_family_factor_extension_plan(ROOT)
    ...
    def test_validation_passes(self) -> None:
        result = validate_alter_operator_family_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertTrue(
            result.passed, msg=f"issues: {result.issues[:5]}"
        )
        self.assertEqual(result.baseline_case_count, 59)
        self.assertEqual(result.extension_case_count, 4464)
        self.assertEqual(result.sql_file_count, 4523)
    ...
    def test_validation_to_dict_has_required_fields(self) -> None:
        ...
        self.assertEqual(
            payload["kind"],
            "alter_operator_family_actual_factor_witness_report",
        )
        self.assertEqual(payload["baseline_case_count"], 59)
        self.assertEqual(payload["extension_case_count"], 4464)
    ...
    def test_selected_subset_validation(self) -> None:
        ...
        subset = {
            ...
            if f.startswith("ALTEROPERATORFAMILY0000")
            or f.startswith("ALTEROPERATORFAMILY0006")
        }
        result = validate_alter_operator_family_factor_programs(
    ...
    def test_tampered_program_detected(self) -> None:
        ...
        tampered[filename] = tampered[filename].replace(
            "ALTER OPERATOR FAMILY", "SELECT true AS noop"
        )
        result = validate_alter_operator_family_factor_programs(
