"""Tests for create_collation factor render module."""
...
class TestCreateCollationFactorRender(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_create_collation(sql))
    ...
    def test_primary_target_is_create_collation(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split("-- primary-target-begin", 1)[1]
            region = region.split("-- primary-target-end", 1)[0]
            self.assertRegex(region, r"(?im)CREATE\s+COLLATION\b")
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_collation_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_collation_factor_case(case, _REPO)
            b = render_create_collation_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "CREATECOLLATION00001.sql").read_text()
        self.assertIn("-- case_id: CREATECOLLATION00001", sql)
