"""Tests for create_operator_family factor validate module."""
...
from src.pg_case_factory.create_operator_family_factor_validate import (
...
class TestCreateOperatorFamilyFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_operator_family_factor_loop_plan(
            _REPO
        )
        self.extension = (
            build_create_operator_family_factor_extension_plan(_REPO)
        )
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_operator_family_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(31, report.baseline_case_count)
        self.assertEqual(1188, report.extension_case_count)
        self.assertEqual(1219, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_operator_family_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(1219, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_operator_family_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(1219, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATEOPERATORFAMILY00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE OPERATOR FAMILY", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "CREATEOPERATORFAMILY00001",
            "CREATEOPERATORFAMILY00032",
        }
        report = validate_create_operator_family_factor_programs(
            self.baseline,
            self.extension,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_operator_family_actual_factor_witness_report",
            d["kind"],
        )
