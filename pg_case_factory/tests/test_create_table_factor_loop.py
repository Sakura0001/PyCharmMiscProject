"""Tests for create_table factor_loop obligation ledger."""
...
class TestCreateTableFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_table_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(188, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(188, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 3, "SFV": 185}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "e1e519d50812e5a7cd757718c72035db887645dd82cb0bb6472fe82e40ef6dff",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_table_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 158, "expected_failure": 30}, dict(counts)
        )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_table_factor_loop_obligations(_REPO)
