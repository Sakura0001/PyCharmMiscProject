...
class SecurityLabelFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="security_label_render_")
        )
        cls.count = generate_security_label_factor_programs(
    ...
    def test_renders_exactly_182_files(self) -> None:
        self.assertEqual(182, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(182, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"SECURITYLABEL{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_security_label(text)
    ...
    def test_no_quoted_create_table(self) -> None:
        """Class-2 invariant: unquoted CREATE TABLE only; no quoted form."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_primary_target_is_security_label(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*SECURITY\s+LABEL\b", region
                ),
                f"no SECURITY LABEL in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_security_label_factor_witness(case, root)
            self.assertIsInstance(w, SecurityLabelFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_security_label_factor_case(case, root)
        b = render_security_label_factor_case(case, root)
