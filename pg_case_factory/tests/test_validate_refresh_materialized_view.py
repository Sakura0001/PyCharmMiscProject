...
from pg_case_factory.validate_refresh_materialized_view import (
    RefreshMaterializedViewFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_refresh_materialized_view_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class RefreshMaterializedViewFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="refresh_materialized_view_validate_")
        )
        generate_refresh_materialized_view_factor_programs(
    ...
    def test_validation_passes_for_all_1694_files(self) -> None:
        result = validate_refresh_materialized_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, RefreshMaterializedViewFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(38, result.baseline_case_count)
        self.assertEqual(1656, result.extension_case_count)
        self.assertEqual(1694, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1694, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_refresh_materialized_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(1694, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_refresh_materialized_view_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "REFRESH MATERIALIZED VIEW", "REFRESH BOGUS", 1
        )
