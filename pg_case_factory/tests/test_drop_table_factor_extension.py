"""Tests for the bounded DROP TABLE post-coverage extension expander.

The marginal factor-value-loop (``drop_table_factor_loop.py``) is the
...
from pg_case_factory.drop_table_factor_extension import (
...
from pg_case_factory.drop_table_factor_loop import (
...
_BASELINE_COUNT = 50
_EXTENSION_COUNT = 7332
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 7382
_SUCCESS_EXTENSIONS = 4284
_FAILURE_EXTENSIONS = 3048
_EXTENSION_MULTISET_SHA256 = (
    "79e8ccbeb9ea94478b143750304fcb5afe67c1f84182bc5d9e0055e8b517a9a8"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("owner", "superuser", "schema_owner", "non_owner"),
    "dependency_state": (
        "no_dependents",
        "has_views",
        "has_fk_references",
...
_BRANCH_GRAMMAR = {"branch_drop_table": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_table": "drop_table"}
...
class DropTableFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_table_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropTableFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTABLE00051", first.case_id)
        self.assertEqual(f"DROPTABLE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPTABLE00051.sql", first.sql_filename)
        self.assertEqual(
            f"DROPTABLE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droptable_00051_", first.object_prefix)
        self.assertEqual(
            f"droptable_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropTableFactorExtensionCase)
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if ("dependency_state", dep) not in _DEPENDENCY_NEGATIVES:
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_table_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_table_factor_loop_plan(ROOT)
