...
class InsertFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_insert_factor_loop_obligations(ROOT)
...
class InsertFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_insert_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, InsertFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(50, len(plan.cases))
        self.assertEqual(
            list(range(1, 51)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("INSERT00001", plan.cases[0].case_id)
        self.assertEqual("INSERT00050", plan.cases[-1].case_id)
        self.assertEqual(
            "INSERT00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "INSERT00050.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual(
            "insert_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            "insert_00050_", plan.cases[-1].object_prefix
        )
        self.assertEqual(
            50,
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_insert_factor_loop_plan(ROOT)
        obligations = compile_insert_factor_loop_obligations(ROOT)
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_insert_factor_loop_plan(ROOT)
        again = build_insert_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_insert_factor_loop_plan(ROOT)
        self.assertEqual(
            "d12e7e908731070924faf1c5aca37d06a2e33e39e398290c501376b98f119fd7",
            plan.obligation_multiset_sha256,
        )
