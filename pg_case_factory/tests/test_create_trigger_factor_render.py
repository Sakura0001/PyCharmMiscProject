...
from pg_case_factory.create_trigger_factor_extension import (
...
from pg_case_factory.create_trigger_factor_render import (
...
class CreateTriggerFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_trigger_factor_loop_plan(ROOT)
        cls.extension = build_create_trigger_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="ctrg_render_"))
        cls.count = generate_create_trigger_factor_programs(
    ...
    def test_renders_exactly_6571_files(self) -> None:
        self.assertEqual(6571, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(6571, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATETRIGGER{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_trigger(text)
    ...
    def _executable_statements(text: str) -> list[str]:
        ...
        Strips ``--`` line comments and ``\set``/\echo`` meta-commands so
        the cleanup-bookend bracket markers do not shadow the first/last
        executable ``DROP TABLE`` the bookend gate expects.
        """
        text = re.sub(r"--[^\n]*", "", text)
    ...
    def test_primary_target_is_create_trigger(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_create_trigger_factor_witness(case, root)
            self.assertIsInstance(
                w, CreateTriggerFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_trigger_factor_case(case, root)
        b = render_create_trigger_factor_case(case, root)
