...
from pg_case_factory.cluster_factor_validate import (
...
class ClusterFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_4919_files(self) -> None:
        result = validate_cluster_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, ClusterFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(55, result.baseline_case_count)
        self.assertEqual(4864, result.extension_case_count)
        self.assertEqual(4919, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(4919, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_cluster_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(4919, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_cluster_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_cluster_factor_case(case, ROOT)
        tampered = sql.replace("CLUSTER", "CLUSTERBOGUS", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CLUSTER00001", "CLUSTER00002"}
        result = validate_cluster_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
