...
_OBLIGATION_MULTISET_SHA256 = (
    "bfc60515286a6cc282ae6cf70ea06ae96d7bfd6489c72b39aaed824f48aac974"
)


class DropTextSearchConfigurationFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_text_search_configuration_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_text_search_configuration_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTextSearchConfigurationFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_text_search_configuration_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTextSearchConfigurationFactorLoopPlan)
        ...
        self.assertEqual(
            "DROPTEXTSEARCHCONFIGURATION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            f"DROPTEXTSEARCHCONFIGURATION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTEXTSEARCHCONFIGURATION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHCONFIGURATION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptextsearchconfiguration_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptextsearchconfiguration_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_drop_text_search_configuration_factor_loop_plan(ROOT)
        obligations = (
            compile_drop_text_search_configuration_factor_loop_obligations(ROOT)
        )
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        for case in sfv_cases:
            self.assertRegex(
                ...
                r"^DROPTEXTSEARCHCONFIGURATION-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
