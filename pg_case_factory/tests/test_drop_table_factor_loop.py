...
_OBLIGATION_MULTISET_SHA256 = (
    "38f3d965c3f29d3fb2f0a22edf8262071064cedc757a06288e37aed696cf93ed"
)


class DropTableFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_table_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_table_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTableFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_table_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTableFactorLoopPlan)
        ...
        self.assertEqual("DROPTABLE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTABLE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTABLE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTABLE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptable_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptable_{_OBLIGATION_COUNT:05d}_",
