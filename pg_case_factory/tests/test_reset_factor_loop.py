"""Tests for reset factor_loop obligation ledger."""
...
class TestResetFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_reset_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "bf8683aa1f46107819b44595e09c0295f683d9a0705c3d4f"
            "0257ccfae48fb2b8",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_reset_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("reset_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("RESET"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_reset_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
