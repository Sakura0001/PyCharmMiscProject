...
from pg_case_factory.validate_drop_role import (
    DropRoleFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_role_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropRoleFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_role_factor_loop_plan(ROOT)
        cls.extension = build_drop_role_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_role_validate_"))
        generate_drop_role_factor_programs(
    ...
    def test_validation_passes_for_all_7803_files(self) -> None:
        result = validate_drop_role_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropRoleFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(59, result.baseline_case_count)
        self.assertEqual(7744, result.extension_case_count)
        self.assertEqual(7803, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(7803, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_role_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(7803, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_role_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPROLE00001", "DROPROLE00002"}
        result = validate_drop_role_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
