...
_OBLIGATION_MULTISET_SHA256 = (
    "16ca259c57ad615c2e68afdcc7bccaec72ccfeb7b8a0e23c518060f978bc8089"
)


class DropRuleFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_rule_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_rule_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropRuleFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_rule_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropRuleFactorLoopPlan)
        ...
        self.assertEqual("DROPRULE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPRULE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPRULE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPRULE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droprule_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droprule_{_OBLIGATION_COUNT:05d}_",
