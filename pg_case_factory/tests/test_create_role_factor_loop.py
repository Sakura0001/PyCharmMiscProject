"""Tests for create_role factor_loop obligation ledger."""
...
class TestCreateRoleFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_role_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(102, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(102, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 101}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "0210e217744dd2380c155b9491dba5818287ab57da699c524cdf15ecf1e3237d",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_role_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 79, "expected_failure": 23}, dict(counts)
        )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_role_factor_loop_obligations(_REPO)
