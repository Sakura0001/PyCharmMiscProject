...
class CreateDatabaseFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_database_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(67) = 68.
        ...
        self.assertEqual(68, len({row.obligation_id for row in rows}))
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(67, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(67, sum(seen.values()))
        self.assertEqual(67, len(seen))
...
class CreateDatabaseFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_database_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateDatabaseFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(68, len(plan.cases))
        self.assertEqual(
            list(range(1, 69)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATEDATABASE00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEDATABASE00068", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEDATABASE00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEDATABASE00068.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createdatabase_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createdatabase_00068_",
        ...
        self.assertEqual(
            68, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_database_factor_loop_plan(ROOT)
        obligations = compile_create_database_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_database_factor_loop_plan(ROOT)
        again = build_create_database_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_database_factor_loop_plan(ROOT)
        self.assertEqual(
            "52b43606bd936396a06efe84dadf937cb444096b0820dda41cc86b9979317ab2",
            plan.obligation_multiset_sha256,
        )
