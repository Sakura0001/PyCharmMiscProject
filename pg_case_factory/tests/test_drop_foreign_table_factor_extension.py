"""Tests for the bounded DROP FOREIGN TABLE post-coverage extension expander.

The marginal factor-value-loop (``drop_foreign_table_factor_loop.py``) is the
...
from pg_case_factory.drop_foreign_table_factor_extension import (
...
from pg_case_factory.drop_foreign_table_factor_loop import (
...
_BASELINE_COUNT = 44
_EXTENSION_COUNT = 468
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 512
_SUCCESS_EXTENSIONS = 216
_FAILURE_EXTENSIONS = 252
_EXTENSION_MULTISET_SHA256 = (
    "d173518ee75acf56ed83a2482d301bcbc657f2a00456a8a2121a3fa94bbf0b0e"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("non_owner", "superuser", "table_owner"),
    "nonexistent_table": ("table_exists", "table_missing_no_if_exists"),
    "if_exists_clause": ("absent", "present"),
    "cascade_restrict": ("cascade", "restrict_default"),
...
_BRANCH_GRAMMAR = {"branch_drop_foreign_table": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_foreign_table": "drop_foreign_table"}
...
class DropForeignTableFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_foreign_table_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropForeignTableFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPFOREIGNTABLE00045", first.case_id)
        self.assertEqual(f"DROPFOREIGNTABLE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPFOREIGNTABLE00045.sql", first.sql_filename)
        self.assertEqual(
            f"DROPFOREIGNTABLE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropforeigntable_00045_", first.object_prefix)
        self.assertEqual(
            f"dropforeigntable_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropForeignTableFactorExtensionCase)
    ...
    def test_not_exist_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_level"] == "non_owner":
                continue
            if (
                a["nonexistent_table"] == "table_missing_no_if_exists"
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["dependency_state"] != "has_view_dependency":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_class_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["recreate_foreign_table"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_foreign_table_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_foreign_table_factor_loop_plan(ROOT)
