"""Tests for show factor validate module."""
...
from src.pg_case_factory.validate_show import (
...
class TestShowFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_show_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "SHOW00001.sql"
        original = self.programs[filename]
        tampered = original.replace("SHOW work_mem;", "DROP TABLE")
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "show_actual_factor_witness_report",
            d["kind"],
        )
