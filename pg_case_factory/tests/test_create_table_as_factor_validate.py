"""Tests for create_table_as factor validate module."""
...
from src.pg_case_factory.create_table_as_factor_validate import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_table_as_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestCreateTableAsFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_table_as_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(86, report.baseline_case_count)
        self.assertEqual(3975, report.extension_case_count)
        self.assertEqual(4061, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_table_as_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4061, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_table_as_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4061, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATETABLEAS00001.sql"
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATETABLEAS00001", "CREATETABLEAS00087"}
        report = validate_create_table_as_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_table_as_actual_factor_witness_report",
            d["kind"],
        )
