"""Tests for the bounded DROP RULE post-coverage extension expander.

The marginal factor-value-loop (``drop_rule_factor_loop.py``) is the
...
_BASELINE_COUNT = 50
_EXTENSION_COUNT = 9216
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 9266
_SUCCESS_EXTENSIONS = 2688
_FAILURE_EXTENSIONS = 6528
_EXTENSION_MULTISET_SHA256 = (
    "b754918d2e9044836580d6f616c54549ccd13bb65c76884e991ebeff5f1e4382"
)
...
_BRANCH_GRAMMAR = {"branch_drop_rule": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_rule": "drop_rule"}
...
class DropRuleFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_rule_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropRuleFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPRULE00051", first.case_id)
        self.assertEqual(f"DROPRULE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPRULE00051.sql", first.sql_filename)
        self.assertEqual(
            f"DROPRULE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droprule_00051_", first.object_prefix)
        self.assertEqual(
            f"droprule_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropRuleFactorExtensionCase)
    ...
    def test_table_missing_negative_fires(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_level"] == "non_owner":
                continue
            if a["table_existence"] == "table_not_exists":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_rule_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner":
                continue
            ...
            if (
                (
                    ...
                    or a["rule_name_shape"] == "nonexistent_name"
                )
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["catalog_query_pg_rewrite"],
        )
        self.assertEqual(
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_rule_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_rule_factor_loop_plan(ROOT)
