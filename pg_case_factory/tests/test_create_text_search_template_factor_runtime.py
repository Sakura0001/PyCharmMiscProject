"""Tests for create_text_search_template factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_text_search_template_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateTextSearchTemplateFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_ctst")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-ctst-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_text_search_template_factor_loop_plan(
            _REPO
        )
        ep = build_create_text_search_template_factor_extension_plan(
            _REPO
        )
        generate_create_text_search_template_factor_programs(
            bp, ep, sql_dir
    ...
    def test_runner_construction(self) -> None:
        runner = CreateTextSearchTemplatePg18Runner()
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(
            ...
            CreateTextSearchTemplatePg18Runner(timeout_seconds=0)
        with self.assertRaises(
            CreateTextSearchTemplateRuntimeError
        ):
            CreateTextSearchTemplatePg18Runner(timeout_seconds=301)
