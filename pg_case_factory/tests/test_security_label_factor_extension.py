...
class SecurityLabelFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_security_label_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, SecurityLabelFactorExtensionPlan
        )
        self.assertEqual(96, len(plan.cases))
        self.assertEqual(96, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(
        ...
        self.assertEqual(list(range(87, 183)), ordinals)
        self.assertEqual(
            "SECURITYLABEL00087", plan.cases[0].case_id
        )
        self.assertEqual(
            "SECURITYLABEL00182", plan.cases[-1].case_id
        )
        self.assertEqual(
            "SECURITYLABEL00087.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "SECURITYLABEL00182.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(48, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_security_label_factor_extension_plan(ROOT)
        plan_b = build_security_label_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_security_label_factor_extension_plan(ROOT)
        self.assertEqual(
            "8f10f703567383e9e3969c51433c7a0c675c927e93310538efd9022fada6a82f",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("security_label_is_null", "drop_object"),
            _CLEANUP_MODES,
        )
