...
class RollbackPreparedFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="rprep_render_"))
        cls.count = generate_rollback_prepared_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"ROLLBACKPREPARED{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_rollback_prepared(text)
    ...
    def test_primary_target_is_rollback_prepared(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*ROLLBACK\s+PREPARED\b", region),
                f"no ROLLBACK PREPARED in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_rollback_prepared_factor_witness(case, root)
            self.assertIsInstance(
                w, RollbackPreparedFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_rollback_prepared_factor_case(case, root)
        b = render_rollback_prepared_factor_case(case, root)
