"""Tests for deallocate factor_loop obligation ledger."""
...
class TestDeallocateFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_deallocate_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "6068a55a26b3c122d485ae913a73c76bc99dd2a8"
            "8d25f7a2dca11ba8cc2997d6",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_deallocate_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("deallocate_")
            )
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.case_id.startswith("DEALLOCATE")
            )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = (
            compile_deallocate_factor_loop_obligations(_REPO)
        )
        self.assertEqual(
