"""Tests for unlisten factor_loop obligation ledger."""
...
class TestUnlistenFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_unlisten_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "e7db9e2a76899f1e70a13640132a45d72db23fa3de0cbd29"
            "1557009dd4fba58b",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_unlisten_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("unlisten_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("UNLISTEN"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_unlisten_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
