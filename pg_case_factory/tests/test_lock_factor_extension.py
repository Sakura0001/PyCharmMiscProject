...
class LockFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_lock_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, LockFactorExtensionPlan)
        self.assertEqual(2268, len(plan.cases))
        self.assertEqual(2268, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(47, 2315)), ordinals)
        self.assertEqual("LOCK00047", plan.cases[0].case_id)
        self.assertEqual("LOCK02314", plan.cases[-1].case_id)
        self.assertEqual(
            "LOCK00047.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "LOCK02314.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_lock_factor_extension_plan(ROOT)
        plan_b = build_lock_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_lock_factor_extension_plan(ROOT)
        self.assertEqual(
            "a4b59dab4a78a4586cd978512eb8c95b08f0e6e44af345b555bacf0659838e36",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("drop_objects", "reset_state", "rollback"),
            _CLEANUP_MODES,
        )
