"""Tests for unlisten factor validate module."""
...
from src.pg_case_factory.validate_unlisten import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_unlisten_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestUnlistenFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_unlisten_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "UNLISTEN00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "UNLISTEN unlisten_00001_ch;", "DROP TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"UNLISTEN00001", "UNLISTEN00043"}
        report = validate_unlisten_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "unlisten_actual_factor_witness_report",
            d["kind"],
        )
