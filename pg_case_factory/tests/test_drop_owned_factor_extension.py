"""Tests for the bounded DROP OWNED post-coverage extension expander.

The marginal factor-value-loop (``drop_owned_factor_loop.py``) is the frozen
44-case baseline (GRM 1 + SFV 41 + RISK 2).  This module's expander adds the
...
from pg_case_factory.drop_owned_factor_extension import (
...
from pg_case_factory.drop_owned_factor_loop import (
...
_BASELINE_COUNT = 44
_EXTENSION_COUNT = 4608
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 4652
_SUCCESS_EXTENSIONS = 2016
_FAILURE_EXTENSIONS = 2592
_EXTENSION_MULTISET_SHA256 = (
    "6b49177fa88a6cda9271c8c4236e59110546c46e5188056ec9ceb088e85f8399"
)
...
_BRANCH_GRAMMAR = {"branch_drop_owned_by": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_owned_by": "drop_owned"}
...
class DropOwnedFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_owned_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropOwnedFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPOWNED00045", first.case_id)
        self.assertEqual(f"DROPOWNED{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPOWNED00045.sql", first.sql_filename)
        self.assertEqual(
            f"DROPOWNED{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropowned_00045_", first.object_prefix)
        self.assertEqual(
            f"dropowned_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropOwnedFactorExtensionCase)
    ...
    def test_role_not_exist_negative_fires(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["executor_privilege"] == "normal_user_no_privilege":
                continue
            if a["role_existence"] == "role_not_exists":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if a["role_existence"] == "role_not_exists":
                continue
            if a["dependent_objects"] != "has_dependent_objects_restrict_fails":
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_owned_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_owned_factor_loop_plan(ROOT)
