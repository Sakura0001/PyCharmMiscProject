"""Tests for the bounded DROP CONVERSION post-coverage extension expander.

The marginal factor-value-loop (``drop_conversion_factor_loop.py``) is the
...
of the positive behaviour axes across the single official synopsis
...
_BASELINE_COUNT = 38
_EXTENSION_COUNT = 486
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 524
_SUCCESS_EXTENSIONS = 228
_FAILURE_EXTENSIONS = 258
_EXTENSION_MULTISET_SHA256 = (
    "d7267eefd9ef054c0dc0def69ee14e605188af83e87792c26f3e415734b2e367"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("conversion_owner", "non_owner", "superuser"),
    "conversion_not_exist": ("conversion_exists", "conversion_not_exists"),
    ...
    "cleanup_mode": ("drop_conversion", "drop_schema_cascade"),
}
...
_BRANCH_GRAMMAR = {"branch_drop_conversion": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_conversion": "drop_conversion"}
...
class DropConversionFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_conversion_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropConversionFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPCONVERSION00039", first.case_id)
        self.assertEqual(f"DROPCONVERSION{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPCONVERSION00039.sql", first.sql_filename)
        self.assertEqual(
            f"DROPCONVERSION{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropconversion_00039_", first.object_prefix)
        self.assertEqual(
            f"dropconversion_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropConversionFactorExtensionCase)
    ...
    def test_schema_negative_fires_for_schema_qualified(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["schema_existence"] != "schema_not_exists":
                continue
            if a["conversion_name_shape"] != "schema_qualified":
                continue
            if a["privilege_level"] == "non_owner":
                continue
            self.assertEqual(
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["catalog_query_pg_conversion"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["drop_conversion"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_conversion_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_conversion_factor_loop_plan(ROOT)
