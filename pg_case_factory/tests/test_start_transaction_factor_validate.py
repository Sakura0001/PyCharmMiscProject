...
from pg_case_factory.start_transaction_factor_extension import (
...
from pg_case_factory.start_transaction_factor_render import (
    generate_start_transaction_factor_programs,
    render_start_transaction_factor_case,
)
from pg_case_factory.validate_start_transaction import (
    StartTransactionFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_start_transaction_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class StartTransactionFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_start_transaction_factor_loop_plan(ROOT)
        cls.extension = (
            build_start_transaction_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="start_transaction_validate_")
        )
        generate_start_transaction_factor_programs(
    ...
    def test_validation_passes_for_all_1364_files(self) -> None:
        result = validate_start_transaction_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, StartTransactionFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(44, result.baseline_case_count)
        self.assertEqual(1320, result.extension_case_count)
        self.assertEqual(1364, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1364, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_start_transaction_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(1364, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_start_transaction_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "START TRANSACTION", "START TRANSACTIONBOGUS", 1
        )
