...
# Frozen spec: GRM(1) + SFV(43) + RISK(2) = 46 ; INV is not applicable.
_OBLIGATION_COUNT = 46
_KIND_COUNTS = {"GRM": 1, "SFV": 43, "RISK": 2}
_FAILURE_COUNT = 4
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 42
_OBLIGATION_MULTISET_SHA256 = (
    "e2335d76cd807ea4aee9ce411b50808f13b07d8ad9a943454abe5d70098f2ccd"
)


class DropOperatorClassFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_operator_class_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_operator_class_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropOperatorClassFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_operator_class_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropOperatorClassFactorLoopPlan)
        ...
        self.assertEqual("DROPOPERATORCLASS00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPOPERATORCLASS{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPOPERATORCLASS00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPOPERATORCLASS{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropoperatorclass_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropoperatorclass_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(43, len(sfv_cases))
        for case in sfv_cases:
            self.assertRegex(
                case.primary_obligation_id,
                r"^DROPOPERATORCLASS-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
