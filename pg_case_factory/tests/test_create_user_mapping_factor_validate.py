"""Tests for create_user_mapping factor validate module."""
...
from src.pg_case_factory.create_user_mapping_factor_validate import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_user_mapping_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestCreateUserMappingFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_create_user_mapping_factor_loop_plan(
                _REPO
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_user_mapping_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(46, report.baseline_case_count)
        self.assertEqual(1920, report.extension_case_count)
        self.assertEqual(1966, report.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1966, len(report.records))
    ...
    def test_sha256_dictionary_complete(self) -> None:
        ...
        self.assertEqual(1966, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATEUSERMAPPING00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE USER MAPPING", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "CREATEUSERMAPPING00001",
            "CREATEUSERMAPPING00047",
        }
        report = validate_create_user_mapping_factor_programs(
            self.baseline,
            self.extension,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_user_mapping_actual_factor_witness_report",
            d["kind"],
        )
