...
from pg_case_factory.release_savepoint_factor_runtime import (
    MAX_PARALLELISM,
    PG18_BIN,
    PG18_DATABASE,
    PG18_PORT,
...
class ReleaseSavepointRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_rsp", PG18_DATABASE)
...
class ReleaseSavepointRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1844_cases(self) -> None:
        sql_dir = Path("/tmp/release_savepoint_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_release_savepoint_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_release_savepoint_runtime_case_set(
            ROOT, sql_dir
        )
...
class ReleaseSavepointRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = ReleaseSavepointPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(ReleaseSavepointRuntimeError):
            ReleaseSavepointPg18Runner(timeout_seconds=0)
        with self.assertRaises(ReleaseSavepointRuntimeError):
            ReleaseSavepointPg18Runner(timeout_seconds=301)
...
class ReleaseSavepointComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "RELEASESAVEPOINT00001"
    ) -> ReleaseSavepointCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = ReleaseSavepointSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = ReleaseSavepointSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_release_savepoint_runs(run1, run2)
    ...
    def test_comparison_fails_on_sqlstate_mismatch(
        ...
        case = ReleaseSavepointCaseRuntimeResult(
            case_id="RELEASESAVEPOINT00001",
            sql_filename="RELEASESAVEPOINT00001.sql",
            expected_sqlstate="00000",
