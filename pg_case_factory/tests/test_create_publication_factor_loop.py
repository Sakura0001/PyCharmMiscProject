"""Tests for create_publication factor_loop obligation ledger."""
...
class TestCreatePublicationFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_publication_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(72, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(72, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 4, "SFV": 68}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "eb9fa3831f09207ea604c6257482194a5ecce61a018302a7da575925d5a8ca46",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_publication_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 57, "expected_failure": 15}, dict(counts)
        )
    ...
    def test_contiguous_ordinals(self) -> None:
        ...
        self.assertEqual(
            list(range(1, 73)), ordinals
        )

    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("createpublication_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATEPUBLICATION"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_publication_factor_loop_obligations(_REPO)
