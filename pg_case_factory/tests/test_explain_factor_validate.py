...
from pg_case_factory.validate_explain import (
...
class ExplainFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_6672_files(self) -> None:
        result = validate_explain_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, ExplainFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(48, result.baseline_case_count)
        self.assertEqual(6624, result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_explain_factor_case(case, ROOT)
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_explain_factor_case(case, ROOT)
        tampered = sql.replace("EXPLAIN", "VACUUMBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"EXPLAIN00001", "EXPLAIN00002"}
        result = validate_explain_factor_programs(
            self.baseline, self.extension, self.programs, ROOT,
