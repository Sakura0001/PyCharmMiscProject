...
from pg_case_factory.create_domain_factor_validate import (
...
class CreateDomainFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="cdom_validate_"))
        generate_create_domain_factor_programs(
    ...
    def test_validation_passes_for_all_2977_files(self) -> None:
        result = validate_create_domain_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CreateDomainFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(97, result.baseline_case_count)
        self.assertEqual(2880, result.extension_case_count)
        self.assertEqual(2977, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(2977, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_domain_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(2977, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_domain_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_create_domain_factor_case(case, ROOT)
        tampered = sql.replace("CREATEDOMAIN", "CREATEDOMAINBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATEDOMAIN00001", "CREATEDOMAIN00002"}
        result = validate_create_domain_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
