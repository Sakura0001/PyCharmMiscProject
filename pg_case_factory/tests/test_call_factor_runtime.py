...
from pg_case_factory.call_factor_runtime import (
...
class CallRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_call", PG18_DATABASE)
...
class CallRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_17713_cases(self) -> None:
        sql_dir = Path("/tmp/call_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_call_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_call_runtime_case_set(ROOT, sql_dir)
...
class CallRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CallPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CallRuntimeError):
            CallPg18Runner(timeout_seconds=0)
        with self.assertRaises(CallRuntimeError):
            CallPg18Runner(timeout_seconds=301)
...
class CallComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "CALL00001"
    ) -> CallCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CallSuiteRun(run_ordinal=1, cases=(case,))
        run2 = CallSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_call_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CallCaseRuntimeResult(
            case_id="CALL00001",
            sql_filename="CALL00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="42883",
            exit_code=0,
            timed_out=False,
            normalized_stdout=b"PGCF_TARGET_SQLSTATE=42883\n",
            normalized_stderr=b"",
            boolean_oracle_failure_count=0,
            pre_clean=True,
            post_clean=True,
        ...
        self.assertIn("CALL00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CallRuntimeError):
            CallSuiteRun(run_ordinal=3, cases=())
