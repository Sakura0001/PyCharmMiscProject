...
class AlterTypeFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_type_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTypeFactorExtensionPlan
        )
        self.assertEqual(10428, len(plan.cases))
        self.assertEqual(10428, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(95, 10523)), ordinals
        )
        self.assertEqual(
            "ALTERTYPE00095", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTYPE10522", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTYPE00095.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "ALTERTYPE10522.sql", plan.cases[-1].sql_filename
        )

    def test_outcome_counts_are_frozen(self) -> None:
        plan = build_alter_type_factor_extension_plan(ROOT)
        outcomes = Counter(c.outcome for c in plan.cases)
        self.assertEqual(3672, outcomes["success"])
        self.assertEqual(6756, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_type_factor_extension_plan(ROOT)
        plan_b = build_alter_type_factor_extension_plan(ROOT)
        self.assertEqual(
