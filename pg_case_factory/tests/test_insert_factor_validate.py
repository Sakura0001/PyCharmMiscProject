...
from pg_case_factory.validate_insert import (
    InsertFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_insert_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class InsertFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_insert_factor_loop_plan(ROOT)
        cls.extension = build_insert_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="insert_validate_"))
        generate_insert_factor_programs(
    ...
    def test_validation_passes_for_all_17714_files(self) -> None:
        result = validate_insert_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, InsertFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(50, result.baseline_case_count)
        self.assertEqual(17664, result.extension_case_count)
        self.assertEqual(17714, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(17714, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_insert_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(17714, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_insert_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_insert_factor_case(case, ROOT)
        tampered = sql.replace("INSERT", "INSERTBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"INSERT00001", "INSERT00002"}
        result = validate_insert_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
