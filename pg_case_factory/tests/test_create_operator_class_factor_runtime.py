"""Tests for create_operator_class factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_operator_class_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateOperatorClassFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_copc")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-copc-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_operator_class_factor_loop_plan(
            _REPO
        )
        ep = build_create_operator_class_factor_extension_plan(
            _REPO
        )
        generate_create_operator_class_factor_programs(
            bp, ep, sql_dir
        ...
        self.assertEqual(5622, len(cases))
        self.assertEqual(5622, len(paths))
    ...
    def test_runner_construction(self) -> None:
        runner = CreateOperatorClassPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreateOperatorClassRuntimeError):
            CreateOperatorClassPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateOperatorClassRuntimeError):
            CreateOperatorClassPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(
        self,
    ) -> None:
        with self.assertRaises(CreateOperatorClassRuntimeError):
            CreateOperatorClassSuiteRun(
