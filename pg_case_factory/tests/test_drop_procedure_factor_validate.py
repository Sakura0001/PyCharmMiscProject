...
from pg_case_factory.validate_drop_procedure import (
    DropProcedureFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_procedure_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]
_BASELINE = 43
_EXTENSION = 1896
_TOTAL = _BASELINE + _EXTENSION  # 1939


class DropProcedureFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_procedure_factor_loop_plan(ROOT)
        cls.extension = build_drop_procedure_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_procedure_validate_"))
        generate_drop_procedure_factor_programs(
    ...
    def test_validation_passes_for_all_files(self) -> None:
        result = validate_drop_procedure_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropProcedureFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(_BASELINE, result.baseline_case_count)
        self.assertEqual(_EXTENSION, result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_procedure_factor_case(case, ROOT)
    ...
    def test_mutation_helper_rejects_missing_case_trace(self) -> None:
        with self.assertRaises(ValueError):
            remove_primary_semantic_locus_but_keep_comments(
                "SELECT 1;", self.baseline.cases[0]
            )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPPROCEDURE00001", "DROPPROCEDURE00002"}
        result = validate_drop_procedure_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
