...
_FAILURE_COUNT = 3
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 35
_OBLIGATION_MULTISET_SHA256 = (
    "a478126f795e2f9b2b06ce6ef5c293b7e1111631736ba20c62cd7aae48e82a8c"
)


class DropEventTriggerFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_event_trigger_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_event_trigger_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropEventTriggerFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_event_trigger_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropEventTriggerFactorLoopPlan)
        ...
        self.assertEqual("DROPEVENTTRIGGER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPEVENTTRIGGER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPEVENTTRIGGER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPEVENTTRIGGER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropeventtrigger_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropeventtrigger_{_OBLIGATION_COUNT:05d}_",
