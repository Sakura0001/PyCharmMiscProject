"""Tests for prepare factor render module."""
...
class TestPrepareFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_prepare_factor_loop_plan(_REPO)
        self.extension = (
            build_prepare_factor_extension_plan(_REPO)
        )
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_prepare_factor_programs(
    ...
    def test_primary_target_is_prepare(
        self,
    ) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_prepare_factor_witness(
                case, _REPO
            )
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_prepare_factor_case(case, _REPO)
            b = render_prepare_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (
            self.out / "PREPARE00001.sql"
        ).read_text()
        self.assertIn(
            "-- case_id: PREPARE00001", sql
        )
