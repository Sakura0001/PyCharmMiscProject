"""Tests for create_index factor_loop obligation ledger."""
...
class TestCreateIndexFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_index_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(101, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(101, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 12, "SFV": 89}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "30382c3559c26c895aa94059c3c5d5e2631965b5747a02cfc7890763cb8a0746",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_index_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 93, "expected_failure": 8}, dict(counts)
        )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_index_factor_loop_obligations(_REPO)
