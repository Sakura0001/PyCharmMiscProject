...
class AlterPublicationFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_publication_factor_witness(case, ROOT)
                self.assertEqual(
                ...
                self.assertTrue(
                    witness.target_sql_fragment.startswith("ALTER PUBLICATION")
                )
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_publication_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_publication(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_publication_factor_loop_plan(ROOT)
        first = [
            render_alter_publication_factor_case(row, ROOT)
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(75, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(
        ...
        commit_sql = render_alter_publication_factor_case(
            rows["commit"], ROOT
        )
        rollback_sql = render_alter_publication_factor_case(
            rows["rollback"], ROOT
        )
    ...
    def test_header_carries_huawei_and_factor_metadata(self) -> None:
        ...
        self.assertIn("-- description  : ALTER PUBLICATION", sql)
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                ...
                if "pg_publication" in sql:
                    self.assertIn("pg_catalog.pg_publication", sql)
                if "pg_publication_tables" in sql:
                    self.assertIn(
                        "pg_catalog.pg_publication_tables", sql
                    )


class AlterPublicationExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = build_alter_publication_factor_extension_plan(
            ROOT
        )

    def _representative_subset(self) -> list[AlterPublicationFactorExtensionCase]:
        seen: set[tuple[str, str]] = set()
        subset: list[AlterPublicationFactorExtensionCase] = []
    ...
    def test_extension_witness_fragments_present_in_rendered_bytes(
        ...
        for case in self._representative_subset():
            with self.subTest(case_id=case.case_id):
                sql = render_alter_publication_factor_case(case, ROOT)
                witness = resolve_alter_publication_factor_witness(
                    case, ROOT
                )
                self.assertEqual(
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [
            render_alter_publication_factor_case(c, ROOT) for c in subset
        ]
        second = [
            render_alter_publication_factor_case(c, ROOT) for c in subset
        ]
    ...
    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_publication_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_publication"
            count = generate_alter_publication_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_BASELINE_COUNT + _EXTENSION_COUNT, count)
            files = sorted(out.glob("ALTERPUBLICATION*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_publication(b0))
            e0 = (
                out / self.extension_plan.cases[0].sql_filename
            ).read_text(encoding="utf-8")
            self.assertEqual(1, count_primary_alter_publication(e0))
