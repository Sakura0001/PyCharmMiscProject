"""Tests for the bounded DROP TRIGGER post-coverage extension expander.

The marginal factor-value-loop (``drop_trigger_factor_loop.py``) is the
...
from pg_case_factory.drop_trigger_factor_extension import (
...
from pg_case_factory.drop_trigger_factor_loop import (
...
_BASELINE_COUNT = 36
_EXTENSION_COUNT = 4320
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 4356
_SUCCESS_EXTENSIONS = 1296
_FAILURE_EXTENSIONS = 3024
_EXTENSION_MULTISET_SHA256 = (
    "5c6eba1b2fd71e80c1cb1c402e63dec096721c0b5a1960654afeb7ac127b0105"
)
...
_CROSSED_AXES = {
    ...
    "trigger_name_shape": ("simple", "quoted", "reserved_word"),
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_trigger"}
...
class DropTriggerFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_trigger_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropTriggerFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTRIGGER00037", first.case_id)
        self.assertEqual(f"DROPTRIGGER{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPTRIGGER00037.sql", first.sql_filename)
        self.assertEqual(
            f"DROPTRIGGER{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droptrigger_00037_", first.object_prefix)
        self.assertEqual(
            f"droptrigger_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropTriggerFactorExtensionCase)
    ...
    def test_expected_status_is_derived_from_present_failure_pair(
        self,
    ) -> None:
        for case in self.plan.cases:
            assignment = dict(case.factor_assignment)
    ...
    def test_table_missing_negative_fires(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner_no_privilege":
                continue
            if a["table_dependency"] == "table_not_exists":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
                    "42P01", case.expected_sqlstate, case.case_id
                )

    def test_trigger_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner_no_privilege":
                continue
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["dependent_objects"] != "has_dependents_restrict_blocks":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_trigger_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["DROP_TRIGGER_cascade"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_trigger_factor_loop_plan(ROOT)
