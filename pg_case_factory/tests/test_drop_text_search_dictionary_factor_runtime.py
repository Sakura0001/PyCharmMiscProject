...
from pg_case_factory.drop_text_search_dictionary_factor_runtime import (
...
class DropTextSearchDictionaryRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_dtsd", PG18_DATABASE)
...
class DropTextSearchDictionaryRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1886_cases(self) -> None:
        sql_dir = Path("/tmp/drop_text_search_dictionary_probe_leaf")
        ...
        self.assertEqual(1886, len(cases))
        self.assertEqual(1886, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_text_search_dictionary_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1886, ordinals[-1])


class DropTextSearchDictionaryRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropTextSearchDictionaryPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropTextSearchDictionaryRuntimeError):
            DropTextSearchDictionaryPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropTextSearchDictionaryRuntimeError):
            DropTextSearchDictionaryPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_ts_dict(self) -> None:
        runner = DropTextSearchDictionaryPg18Runner()
...
class DropTextSearchDictionaryComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPTEXTSEARCHDICTIONARY00001"
    ) -> DropTextSearchDictionaryCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropTextSearchDictionarySuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = DropTextSearchDictionarySuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_drop_text_search_dictionary_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropTextSearchDictionaryCaseRuntimeResult(
            case_id="DROPTEXTSEARCHDICTIONARY00001",
            sql_filename="DROPTEXTSEARCHDICTIONARY00001.sql",
            expected_sqlstate="00000",
