...
_OBLIGATION_MULTISET_SHA256 = (
    "29140e7f8e4443566c6de3915c1b42c6dba4c17d6a3c4b6de35b5620046b4914"
)


class DropLanguageFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_language_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_language_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropLanguageFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_language_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropLanguageFactorLoopPlan)
        ...
        self.assertEqual("DROPLANGUAGE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPLANGUAGE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPLANGUAGE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPLANGUAGE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droplanguage_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droplanguage_{_OBLIGATION_COUNT:05d}_",
