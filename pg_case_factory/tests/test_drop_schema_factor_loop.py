...
_OBLIGATION_MULTISET_SHA256 = (
    "95f811cb30dc4cfc80189cbb51091b0bec9afd7ed38c1e62bde854aa8df47f73"
)


class DropSchemaFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_schema_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_schema_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropSchemaFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_schema_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropSchemaFactorLoopPlan)
        ...
        self.assertEqual("DROPSCHEMA00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPSCHEMA{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPSCHEMA00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPSCHEMA{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropschema_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropschema_{_OBLIGATION_COUNT:05d}_",
