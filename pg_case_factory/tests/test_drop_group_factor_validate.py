...
from pg_case_factory.drop_group_factor_render import (
    generate_drop_group_factor_programs,
    render_drop_group_factor_case,
)
from pg_case_factory.drop_group_factor_validate import (
    DropGroupFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_group_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropGroupFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_group_factor_loop_plan(ROOT)
        cls.extension = build_drop_group_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_group_validate_"))
        generate_drop_group_factor_programs(
    ...
    def test_validation_passes_for_all_197_files(self) -> None:
        result = validate_drop_group_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropGroupFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(44, result.baseline_case_count)
        self.assertEqual(153, result.extension_case_count)
        self.assertEqual(197, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(197, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_group_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(197, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_group_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPGROUP00001", "DROPGROUP00002"}
        result = validate_drop_group_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
