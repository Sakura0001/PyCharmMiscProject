...
class AlterUserFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_user_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, AlterUserFactorExtensionPlan)
        self.assertEqual(3456, len(plan.cases))
        self.assertEqual(3456, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(76, 3532)), ordinals
        )
        self.assertEqual(
            "ALTERUSER00076", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERUSER03531", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERUSER00076.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(1512, outcomes["success"])
        self.assertEqual(1944, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_user_factor_extension_plan(ROOT)
        plan_b = build_alter_user_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_user_factor_extension_plan(ROOT)
        self.assertEqual(
            "870f3ff9b6f4421611938ff80d8637444e12bfaa00f7922629b1f2fe86098f71",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(75, _BASELINE_COUNT)
