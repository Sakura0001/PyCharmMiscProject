...
from pg_case_factory.end_factor_runtime import (
    EndCaseRuntimeResult,
    EndPg18Runner,
    EndRuntimeError,
    EndSuiteRun,
    EndTwoRunComparison,
...
class EndRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_commit_begin_and_alter_subscription(
        self,
    ) -> None:
        # END must not collide with commit (55500/pgcf_commit),
        # begin (55499/pgcf_begin), or alter_subscription (55498/pgcf_asub).
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_end", PG18_DATABASE)
    ...
    def test_socket_dir_is_end_scoped(self) -> None:
        self.assertIn("end", str(PG18_SOCKET))


class EndRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1844_cases(self) -> None:
        sql_dir = Path("/tmp/end_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_end_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_end_runtime_case_set(ROOT, sql_dir)
...
class EndRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = EndPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(EndRuntimeError):
            EndPg18Runner(timeout_seconds=0)
        with self.assertRaises(EndRuntimeError):
            EndPg18Runner(timeout_seconds=301)
...
class EndComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "END00001"
    ) -> EndCaseRuntimeResult:
        return EndCaseRuntimeResult(
            case_id=case_id,
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = EndSuiteRun(run_ordinal=1, cases=(case,))
        run2 = EndSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_end_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = EndCaseRuntimeResult(
            case_id="END00001",
            sql_filename="END00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("END00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(EndRuntimeError):
            EndSuiteRun(run_ordinal=3, cases=())
