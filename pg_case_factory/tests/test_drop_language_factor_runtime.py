...
from pg_case_factory.drop_language_factor_runtime import (
...
class DropLanguageRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_dl", PG18_DATABASE)
...
class DropLanguageRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_539_cases(self) -> None:
        sql_dir = Path("/tmp/drop_language_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_language_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_language_runtime_case_set(ROOT, sql_dir)
...
class DropLanguageRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropLanguagePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropLanguageRuntimeError):
            DropLanguagePg18Runner(timeout_seconds=0)
        with self.assertRaises(DropLanguageRuntimeError):
            DropLanguagePg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_language(self) -> None:
        runner = DropLanguagePg18Runner()
        query = runner._clean_probe.__code__.co_consts
        joined = " ".join(str(c) for c in query if isinstance(c, str))
        self.assertIn("pg_language", joined)


class DropLanguageComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPLANGUAGE00001"
    ) -> DropLanguageCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropLanguageSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropLanguageSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_language_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropLanguageCaseRuntimeResult(
            case_id="DROPLANGUAGE00001",
            sql_filename="DROPLANGUAGE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPLANGUAGE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropLanguageRuntimeError):
            DropLanguageSuiteRun(run_ordinal=3, cases=())
