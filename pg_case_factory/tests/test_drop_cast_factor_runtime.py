...
from pg_case_factory.drop_cast_factor_runtime import (
...
class DropCastRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_dcast", PG18_DATABASE)
...
class DropCastRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_140_cases(self) -> None:
        sql_dir = Path("/tmp/drop_cast_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_cast_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(140, len(cases))
        self.assertEqual(140, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_cast_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(140, ordinals[-1])


class DropCastRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropCastPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropCastRuntimeError):
            DropCastPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropCastRuntimeError):
            DropCastPg18Runner(timeout_seconds=301)
...
class DropCastComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPCAST00001"
    ) -> DropCastCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropCastSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropCastSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_cast_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropCastCaseRuntimeResult(
            case_id="DROPCAST00001",
            sql_filename="DROPCAST00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPCAST00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropCastRuntimeError):
            DropCastSuiteRun(run_ordinal=3, cases=())
