...
from pg_case_factory.alter_sequence_factor_runtime import (
...
def _sample_case(stdout: bytes) -> AlterSequenceCaseRuntimeResult:
    return AlterSequenceCaseRuntimeResult(
        case_id="ALTERSEQUENCE00001",
        sql_filename="ALTERSEQUENCE00001.sql",
...
class AlterSequenceFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(
        self,
    ) -> None:
        identical = compare_alter_sequence_runs(
            AlterSequenceSuiteRun(
        ...
        changed = compare_alter_sequence_runs(
            AlterSequenceSuiteRun(
                run_ordinal=1, cases=(_sample_case(b"ok\n"),)
            ),
            AlterSequenceSuiteRun(
    ...
    def test_sqlstate_mismatch_is_detected(self) -> None:
        ...
        wrong_sqlstate = AlterSequenceCaseRuntimeResult(
            case_id="ALTERSEQUENCE00001",
            sql_filename="ALTERSEQUENCE00001.sql",
            expected_sqlstate="00000",
        ...
        comparison = compare_alter_sequence_runs(
            AlterSequenceSuiteRun(
    ...
    def test_cleanup_failure_is_detected(self) -> None:
        ...
        dirty = AlterSequenceCaseRuntimeResult(
            case_id="ALTERSEQUENCE00001",
            sql_filename="ALTERSEQUENCE00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="00000",
    ...
    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_alter_sequence_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..28591 sequence",
            )
    ...
    def test_runner_timeout_bounds_are_enforced(self) -> None:
        with self.assertRaises(Exception):
            AlterSequencePg18Runner(timeout_seconds=0)
        with self.assertRaises(Exception):
            AlterSequencePg18Runner(timeout_seconds=301)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(Exception):
            AlterSequenceSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(Exception):
            AlterSequenceSuiteRun(run_ordinal=3, cases=())
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        selected = (
            ...
            one(
                ...
                and case.factor_value == "non_existing_role"
            ),
            one(
                lambda case: case.factor_key == "new_schema_name"
                and case.factor_value == "non_existing_schema"
            ),
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for case in selected:
                ...
                path.write_text(
                    render_alter_sequence_factor_case(case, ROOT),
                    encoding="utf-8",
                )
                paths[case.case_id] = path

            runner = AlterSequencePg18Runner()
            ...
            comparison = compare_alter_sequence_runs(run_01, run_02)
