"""Tests for notify factor render module."""
...
class TestNotifyFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.count = generate_notify_factor_programs(
    ...
    def test_filename_numbering(self) -> None:
        ...
        self.assertEqual(
            [f"NOTIFY{i:05d}.sql" for i in range(1, 4267)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_notify(sql))
    ...
    def test_primary_target_is_notify(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
            ...
            self.assertRegex(region, r"(?im)^\s*NOTIFY\b")

    def test_col0_keyword_regex(self) -> None:
        rx = re.compile(r"(?m)^NOTIFY(?:\s|;|$)")
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
                "-- primary-target-begin", 1
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_notify_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_notify_factor_case(case, _REPO)
            b = render_notify_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "NOTIFY00001.sql").read_text()
        self.assertIn("-- case_id: NOTIFY00001", sql)
