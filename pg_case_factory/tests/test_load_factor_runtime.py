...
from pg_case_factory.load_factor_runtime import (
...
class LoadRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_prior_statements(self) -> None:
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_load", PG18_DATABASE)
...
class LoadRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_7726_cases(self) -> None:
        sql_dir = Path("/tmp/load_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_load_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(7726, len(cases))
        self.assertEqual(7726, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_load_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(7726, ordinals[-1])


class LoadRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = LoadPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(LoadRuntimeError):
            LoadPg18Runner(timeout_seconds=0)
        with self.assertRaises(LoadRuntimeError):
            LoadPg18Runner(timeout_seconds=301)
...
class LoadComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "LOAD00001"
    ) -> LoadCaseRuntimeResult:
    ...
    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        ...
        self.assertIn(
            "LOAD00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(LoadRuntimeError):
            LoadSuiteRun(run_ordinal=3, cases=())
