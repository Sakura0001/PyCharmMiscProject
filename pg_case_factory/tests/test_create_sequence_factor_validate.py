"""Tests for create_sequence factor validate module."""
...
from src.pg_case_factory.create_sequence_factor_validate import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_sequence_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestCreateSequenceFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_sequence_factor_loop_plan(_REPO)
        self.extension = build_create_sequence_factor_extension_plan(_REPO)
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_sequence_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(71, report.baseline_case_count)
        self.assertEqual(6912, report.extension_case_count)
        self.assertEqual(6983, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_sequence_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(6983, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_sequence_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(6983, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATESEQUENCE00001.sql"
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATESEQUENCE00001", "CREATESEQUENCE00072"}
        report = validate_create_sequence_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_sequence_actual_factor_witness_report", d["kind"]
        )
