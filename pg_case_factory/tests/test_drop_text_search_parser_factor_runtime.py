...
from pg_case_factory.drop_text_search_parser_factor_runtime import (
...
class DropTsParserRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_dtsp", PG18_DATABASE)
...
class DropTsParserRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1020_cases(self) -> None:
        sql_dir = Path("/tmp/drop_text_search_parser_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_text_search_parser_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(1020, len(cases))
        self.assertEqual(1020, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_text_search_parser_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1020, ordinals[-1])


class DropTsParserRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropTsParserPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropTsParserRuntimeError):
            DropTsParserPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropTsParserRuntimeError):
            DropTsParserPg18Runner(timeout_seconds=301)
...
class DropTsParserComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPTEXTSEARCHPARSER00001"
    ) -> DropTsParserCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropTsParserSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropTsParserSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_text_search_parser_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropTsParserCaseRuntimeResult(
            case_id="DROPTEXTSEARCHPARSER00001",
            sql_filename="DROPTEXTSEARCHPARSER00001.sql",
            expected_sqlstate="00000",
