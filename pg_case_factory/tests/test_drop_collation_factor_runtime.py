...
from pg_case_factory.drop_collation_factor_runtime import (
...
def _sample_case(stdout: bytes) -> DropCollationCaseRuntimeResult:
    return DropCollationCaseRuntimeResult(
        case_id="DROPCOLLATION00001",
        sql_filename="DROPCOLLATION00001.sql",
...
class DropCollationFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(self) -> None:
        identical = compare_drop_collation_runs(
            DropCollationSuiteRun(
        ...
        changed = compare_drop_collation_runs(
            DropCollationSuiteRun(
                run_ordinal=1, cases=(_sample_case(b"ok\n"),)
            ),
            DropCollationSuiteRun(
    ...
    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        # The case-set combiner must merge the frozen 30-case baseline with
        # the bounded 336-case extension into a single ordinal-sorted
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_drop_collation_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..366 sequence",
            )
            self.assertEqual(1, ordinals[0])
            self.assertEqual(_TOTAL, ordinals[-1])
            # baseline (ordinal 1) is not an extension; last (366) is.
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        selected = (
            ...
            one(
                ...
                and case.factor_value == "not_exists"
            ),
            one(
            ...
            one(
                lambda case: case.factor_key == "dependency_state"
                and case.factor_value == "referenced_by_table_column"
            ),
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for case in selected:
                ...
                path.write_text(
                    render_drop_collation_factor_case(case, ROOT),
                    encoding="utf-8",
                )
                paths[case.case_id] = path

            runner = DropCollationPg18Runner()
            ...
            comparison = compare_drop_collation_runs(run_01, run_02)
