...
class ExplainFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_explain_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, ExplainFactorExtensionPlan)
        self.assertEqual(6624, len(plan.cases))
        self.assertEqual(6624, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(49, 6673)), ordinals)
        self.assertEqual("EXPLAIN00049", plan.cases[0].case_id)
        self.assertEqual("EXPLAIN06672", plan.cases[-1].case_id)
        self.assertEqual(
            "EXPLAIN00049.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "EXPLAIN06672.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_explain_factor_extension_plan(ROOT)
        plan_b = build_explain_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_explain_factor_extension_plan(ROOT)
        self.assertEqual(
            "55f9972b4373646019dbcbfcc9026ad905ac39e845b9ed65d4391538ad9f49a4",
