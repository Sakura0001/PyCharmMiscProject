...
class AlterTablespaceFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_tablespace_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTablespaceFactorExtensionPlan
        )
        self.assertEqual(6600, len(plan.cases))
        self.assertEqual(6600, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(73, 6673)), ordinals
        )
        self.assertEqual(
            "ALTERTABLESPACE00073", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTABLESPACE06672", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTABLESPACE00073.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTABLESPACE06672.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(1800, outcomes["success"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_tablespace_factor_extension_plan(ROOT)
        plan_b = build_alter_tablespace_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_tablespace_factor_extension_plan(ROOT)
        self.assertEqual(
            "c1e1e0001a92b91cf4dfb493474108cfb3cc8501ef4e2acbd22ce89a5b5bba28",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("catalog_query_pg_tablespace", "effect_query", "error_assertion"),
            _VERIFICATION_MODES,
        )
        self.assertEqual(
            ("revert_rename", "revert_owner", "reset_option", "drop_tablespace", "role_cleanup"),
            _CLEANUP_MODES,
        )
