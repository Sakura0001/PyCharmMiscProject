...
# Frozen spec: GRM(1) + SFV(53) + RISK(2) = 56 ; INV is not applicable.
_OBLIGATION_COUNT = 56
_KIND_COUNTS = {"GRM": 1, "SFV": 53, "RISK": 2}
_FAILURE_COUNT = 13
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 43
_OBLIGATION_MULTISET_SHA256 = (
    "97aef9658f45b7f1c429dc2c93c3645392ebeb3ec680645793644d00d6ccc87c"
)


class DropIndexFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_index_factor_loop_obligations(ROOT)
    ...
    def test_statement_branch_is_not_skipped(self) -> None:
        rows = compile_drop_index_factor_loop_obligations(ROOT)
        sfv = [row for row in rows if row.kind == "SFV"]
        branch = [row for row in sfv if row.factor_key == "statement_branch"]
        self.assertEqual(4, len(branch))
        branch_values = {row.value for row in branch}
        self.assertEqual(
            {"drop_single", "drop_multiple", "drop_concurrently", "drop_cascade"},
            branch_values,
        )
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_index_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropIndexFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_index_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropIndexFactorLoopPlan)
        ...
        self.assertEqual("DROPINDEX00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPINDEX{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPINDEX00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPINDEX{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropindex_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropindex_{_OBLIGATION_COUNT:05d}_",
