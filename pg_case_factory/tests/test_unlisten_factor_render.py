"""Tests for unlisten factor render module."""
...
class TestUnlistenFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_unlisten_factor_loop_plan(_REPO)
        self.extension = build_unlisten_factor_extension_plan(
            _REPO
        )
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_unlisten_factor_programs(
    ...
    def test_filename_numbering(self) -> None:
        ...
        self.assertEqual(
            [f"UNLISTEN{i:05d}.sql" for i in range(1, 4267)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_unlisten(sql))
    ...
    def test_primary_target_is_unlisten(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
            ...
            self.assertRegex(region, r"(?im)^\s*UNLISTEN\b")

    def test_col0_keyword_regex(self) -> None:
        rx = re.compile(r"(?m)^UNLISTEN(?:\s|;|$)")
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
                "-- primary-target-begin", 1
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_unlisten_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_unlisten_factor_case(case, _REPO)
            b = render_unlisten_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "UNLISTEN00001.sql").read_text()
        self.assertIn("-- case_id: UNLISTEN00001", sql)
    ...
    def test_success_case_setup_listens(self) -> None:
        sql = (self.out / "UNLISTEN00001.sql").read_text()
        self.assertIn("LISTEN unlisten_00001_ch;", sql)

    def test_probe_asserts_absence(self) -> None:
        sql = (self.out / "UNLISTEN00001.sql").read_text()
        self.assertIn("AS not_listening", sql)
