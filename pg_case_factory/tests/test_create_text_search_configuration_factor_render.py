...
from pg_case_factory.create_text_search_configuration_factor_extension import (
    build_create_text_search_configuration_factor_extension_plan,
)
from pg_case_factory.create_text_search_configuration_factor_loop import (
    build_create_text_search_configuration_factor_loop_plan,
)
from pg_case_factory.create_text_search_configuration_factor_render import (
...
class CreateTextSearchConfigurationFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="ctscfg_render_")
        )
        cls.count = (
            generate_create_text_search_configuration_factor_programs(
    ...
    def test_renders_exactly_1302_files(self) -> None:
        self.assertEqual(1302, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1302, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATETEXTSEARCHCONFIGURATION{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_text_search_configuration(text)
    ...
    def test_primary_target_is_create_text_search_configuration(
        self,
    ) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*CREATE\s+TEXT\s+SEARCH\s+CONFIGURATION\b",
                    region,
                ),
                f"no CREATE TEXT SEARCH CONFIGURATION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_create_text_search_configuration_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, CreateTextSearchConfigurationFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_text_search_configuration_factor_case(
            case, root
        )
        b = render_create_text_search_configuration_factor_case(
            case, root
        )
    ...
    def test_object_prefix_naming_convention(self) -> None:
        """NON-alter convention: no underscores between words."""
        for case in self.baseline.cases[:3]:
            self.assertTrue(
                case.object_prefix.startswith(
                    "createtextsearchconfiguration_"
                ),
                f"non-alter prefix expected: {case.object_prefix}",
            )
