...
from pg_case_factory.create_text_search_configuration_factor_validate import (
    CreateTextSearchConfigurationFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_text_search_configuration_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class CreateTextSearchConfigurationFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="ctscfg_validate_")
        )
        generate_create_text_search_configuration_factor_programs(
    ...
    def test_validation_passes_for_all_1302_files(self) -> None:
        result = (
            validate_create_text_search_configuration_factor_programs(
        ...
        self.assertEqual(54, result.baseline_case_count)
        self.assertEqual(1248, result.extension_case_count)
        self.assertEqual(1302, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1302, len(result.records))
    ...
    def test_sha256_dictionary_is_complete(self) -> None:
        ...
        self.assertEqual(1302, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_text_search_configuration_factor_case(
            case, ROOT
        )
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            ...
            "CREATETEXTSEARCHCONFIGURATION00002",
        }
        result = (
            validate_create_text_search_configuration_factor_programs(
                self.baseline,
