...
class CreateServerFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.loop = build_create_server_factor_loop_plan(ROOT)
        cls.ext = build_create_server_factor_extension_plan(ROOT)
        cls.all_cases = list(cls.loop.cases) + list(cls.ext.cases)

    def test_renders_all_cases_without_error(self) -> None:
        for case in self.all_cases:
            sql = render_create_server_factor_case(case, ROOT)
    ...
    def test_primary_target_cardinality_is_one(self) -> None:
        for case in self.all_cases:
            ...
            self.assertEqual(
                1,
                count_primary_create_server(sql),
    ...
    def test_re_render_is_deterministic(self) -> None:
        for case in self.all_cases[:50]:
            first = render_create_server_factor_case(case, ROOT)
            second = render_create_server_factor_case(case, ROOT)
    ...
    def test_cleanup_bookend_invariants(self) -> None:
        ...
        for case in self.all_cases:
            text = render_create_server_factor_case(case, ROOT)
    ...
    def test_witness_resolution_succeeds(self) -> None:
        case = self.loop.cases[0]
        witness = resolve_create_server_factor_witness(case, ROOT)
