...
# Frozen spec: GRM(1) + SFV(36) + RISK(2) = 39 ; INV is not applicable.
_OBLIGATION_COUNT = 39
_KIND_COUNTS = {"GRM": 1, "SFV": 36, "RISK": 2}
_FAILURE_COUNT = 12
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 27
_OBLIGATION_MULTISET_SHA256 = (
    "c8dcc367730c3aa673e903597662cceb1e690442207eedb13f40b3c32bef9737"
)


class DropUserFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_user_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(36, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(36, sum(seen.values()))
        self.assertEqual(36, len(seen))
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_user_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropUserFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_user_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropUserFactorLoopPlan)
        ...
        self.assertEqual("DROPUSER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPUSER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPUSER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPUSER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropuser_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropuser_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(36, len(sfv_cases))
