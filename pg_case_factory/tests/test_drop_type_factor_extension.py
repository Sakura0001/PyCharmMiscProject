"""Tests for the bounded DROP TYPE post-coverage extension expander.

The marginal factor-value-loop (``drop_type_factor_loop.py``) is the
...
_SUCCESS_EXTENSIONS = 1632
_FAILURE_EXTENSIONS = 2640
_EXTENSION_MULTISET_SHA256 = (
    "cfb125726ecf5b8172bccd011993644b71ec0c29e13770fc9c1512874cf241d1"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("non_owner", "superuser", "owner"),
...
_BRANCH_GRAMMAR = {"branch_drop_type": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_type": "drop_type"}
...
class DropTypeFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_type_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropTypeFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTYPE00047", first.case_id)
        self.assertEqual(f"DROPTYPE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPTYPE00047.sql", first.sql_filename)
        self.assertEqual(
            f"DROPTYPE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droptype_00047_", first.object_prefix)
        self.assertEqual(
            f"droptype_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropTypeFactorExtensionCase)
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_type_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_type_factor_loop_plan(ROOT)
    ...
    def test_no_dependency_with_absent_type(self) -> None:
        """A dependent fixture requires the type to exist."""

        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["dependency_state"] in _DEPENDENCY_VALUES:
                self.assertNotEqual(
                    "not_exists",
                    a["object_state"],
                    case.case_id,
