...
class AlterUserFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_user_factor_loop_obligations(ROOT)
        # GRM(6) + SFV(69) = 75 ; RISK is not applicable.
        self.assertEqual(75, len(rows))
        self.assertEqual(
            {"GRM": 6, "SFV": 69},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(75, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 54 SFV + 6 GRM = 60 are covered.
        self.assertEqual(15, len(failures))
        self.assertEqual(60, covered)
        self.assertEqual(len(failures), len(set(failures)))
        for pair in failures:
            self.assertIn(pair, _SFV_FAILURE_SQLSTATE)


class AlterUserFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_user_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterUserFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(75, len(plan.cases))
        self.assertEqual(
            list(range(1, 76)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "ALTERUSER00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERUSER00075", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERUSER00001.sql",
        ...
        self.assertEqual(
            75, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_alter_user_factor_loop_plan(ROOT)
        obligations = compile_alter_user_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_user_factor_loop_plan(ROOT)
        again = build_alter_user_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_user_factor_loop_plan(ROOT)
        self.assertEqual(
            "8fe8d77e69c4b675aeb30987c5885ef39adf7adeabe28820d950a924f0eb687d",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(60, outcomes["success"])
