...
from pg_case_factory.validate_reindex import (
    ReindexFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_reindex_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class ReindexFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_reindex_factor_loop_plan(ROOT)
        cls.extension = build_reindex_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="reindex_validate_"))
        generate_reindex_factor_programs(
    ...
    def test_validation_passes_for_all_3575_files(self) -> None:
        result = validate_reindex_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, ReindexFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(83, result.baseline_case_count)
        self.assertEqual(3492, result.extension_case_count)
        self.assertEqual(3575, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(3575, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_reindex_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(3575, len(result.sql_sha256))
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_reindex_factor_case(case, ROOT)
        tampered = sql.replace("REINDEX", "REINDEXBOGUS", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"REINDEX00001", "REINDEX00002"}
        result = validate_reindex_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
