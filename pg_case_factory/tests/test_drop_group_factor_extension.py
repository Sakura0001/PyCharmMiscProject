"""Tests for the bounded DROP GROUP post-coverage extension expander.

The marginal factor-value-loop (``drop_group_factor_loop.py``) is the
...
from pg_case_factory.drop_group_factor_extension import (
...
from pg_case_factory.drop_group_factor_loop import (
...
_BASELINE_COUNT = 44
_EXTENSION_COUNT = 153
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 197
_SUCCESS_EXTENSIONS = 54
_FAILURE_EXTENSIONS = 99
_EXTENSION_MULTISET_SHA256 = (
    "bf1626252c55efc6b870cfcdbd9be6b1565da34fc90e544cf2be6d88eb40c1cb"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("non_privilege", "superuser", "createrole_privilege"),
...
_BRANCH_GRAMMAR = {"branch_drop_group": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_group": "drop_group"}
...
class DropGroupFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_group_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropGroupFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPGROUP00045", first.case_id)
        self.assertEqual(f"DROPGROUP{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPGROUP00045.sql", first.sql_filename)
        self.assertEqual(
            f"DROPGROUP{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropgroup_00045_", first.object_prefix)
        self.assertEqual(
            f"dropgroup_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropGroupFactorExtensionCase)
            self.assertTrue(case.is_extension, case.case_id)
            self.assertEqual(21, len(case.factor_assignment), case.case_id)
    ...
    def test_privilege_negative_fires_for_non_privilege(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_privilege":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_dependency_negative_fires_always(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_privilege":
                continue
            if (
                ...
                continue
            if a["role_dependency"] != "has_dependencies":
                continue
            self.assertEqual(
                "expected_failure", case.outcome, case.case_id
            )
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_authid_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["recreate_role"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_group_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_group_factor_loop_plan(ROOT)
