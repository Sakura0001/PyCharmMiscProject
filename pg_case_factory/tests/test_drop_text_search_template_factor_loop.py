...
_OBLIGATION_MULTISET_SHA256 = (
    "edd8c95aae41c5d997771710fe9a2acbdcbf516589bb8c42d432f617cd902f0e"
)


class DropTextSearchTemplateFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_text_search_template_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_text_search_template_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTextSearchTemplateFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_text_search_template_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTextSearchTemplateFactorLoopPlan)
        ...
        self.assertEqual("DROPTEXTSEARCHTEMPLATE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTEXTSEARCHTEMPLATE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTEXTSEARCHTEMPLATE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHTEMPLATE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptextsearchtemplate_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptextsearchtemplate_{_OBLIGATION_COUNT:05d}_",
