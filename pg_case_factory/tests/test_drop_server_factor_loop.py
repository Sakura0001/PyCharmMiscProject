...
_FAILURE_COUNT = 5
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 34
_OBLIGATION_MULTISET_SHA256 = (
    "6e4ad3c4ba49e1c0f248b2ed71a41685c76118c12c4658c272d3f52e741647bd"
)


class DropServerFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_server_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_server_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropServerFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_server_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropServerFactorLoopPlan)
        ...
        self.assertEqual("DROPSERVER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPSERVER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPSERVER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPSERVER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropserver_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropserver_{_OBLIGATION_COUNT:05d}_",
