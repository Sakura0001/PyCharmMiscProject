"""Tests for create_language factor extension plan."""
...
class TestCreateLanguageFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_language_factor_extension_plan(_REPO)

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(2496, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 768, "expected_failure": 1728}, dict(counts)
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "8127c41eaedd8cd89d999572ab32a17effa6e0a5313dcd26aa6d02710ca3c0db",
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("CLANG-EXT|"))
