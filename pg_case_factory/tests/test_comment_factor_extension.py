...
class CommentFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_comment_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, CommentFactorExtensionPlan)
        self.assertEqual(13545, len(plan.cases))
        self.assertEqual(13545, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(118, 13663)), ordinals
        )
        self.assertEqual("COMMENT00118", plan.cases[0].case_id)
        self.assertEqual("COMMENT13662", plan.cases[-1].case_id)
        self.assertEqual(
            "COMMENT00118.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "COMMENT13662.sql", plan.cases[-1].sql_filename
        )

    def test_outcome_counts_are_frozen(self) -> None:
        plan = build_comment_factor_extension_plan(ROOT)
        outcomes = Counter(c.outcome for c in plan.cases)
        self.assertEqual(5805, outcomes["success"])
        self.assertEqual(7740, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_comment_factor_extension_plan(ROOT)
        plan_b = build_comment_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_comment_factor_extension_plan(ROOT)
        self.assertEqual(
            "ed79fe7fad2d69c1aec3e9c06f5b8178433f5a1c4cb4c91dbd759a9f6341eced",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(117, _BASELINE_COUNT)
        self.assertEqual(20000, _CAP)
        self.assertEqual(5, len(_VERIFICATION_MODES))
