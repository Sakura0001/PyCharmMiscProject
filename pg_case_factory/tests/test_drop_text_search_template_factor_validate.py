...
from pg_case_factory.validate_drop_text_search_template import (
    DropTextSearchTemplateFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_text_search_template_factor_programs,
)
...
class DropTextSearchTemplateFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_ts_template_validate_"))
        generate_drop_text_search_template_factor_programs(
        ...
        cls.result = validate_drop_text_search_template_factor_programs(
    ...
    def test_validation_passes_for_all_3564_files(self) -> None:
        self.assertIsInstance(
            self.result, DropTextSearchTemplateFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(36, self.result.baseline_case_count)
        self.assertEqual(3528, self.result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_text_search_template_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_text_search_template_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPTEXTSEARCHTEMPLATE00001", "DROPTEXTSEARCHTEMPLATE00002"}
        result = validate_drop_text_search_template_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
