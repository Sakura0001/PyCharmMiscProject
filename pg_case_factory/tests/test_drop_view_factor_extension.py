"""Tests for the bounded DROP VIEW post-coverage extension expander.

The marginal factor-value-loop (``drop_view_factor_loop.py``) is the
...
_BASELINE_COUNT = 40
_EXTENSION_COUNT = 3924
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 3964
_SUCCESS_EXTENSIONS = 2268
_FAILURE_EXTENSIONS = 1656
_EXTENSION_MULTISET_SHA256 = (
    "499cb1d7388c304715d1b812bf1b072167c0d81a690b49b5ae70919d51c63997"
)
...
_BRANCH_GRAMMAR = {"branch_drop_view": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_view": "drop_view"}
...
class DropViewFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_view_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropViewFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPVIEW00041", first.case_id)
        self.assertEqual(f"DROPVIEW{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPVIEW00041.sql", first.sql_filename)
        self.assertEqual(
            f"DROPVIEW{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropview_00041_", first.object_prefix)
        self.assertEqual(
            f"dropview_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropViewFactorExtensionCase)
            self.assertTrue(case.is_extension, case.case_id)
            self.assertEqual(14, len(case.factor_assignment), case.case_id)
    ...
    def test_view_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner":
                continue
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["dependency_state"] != "has_dependent_views":
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_view_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_view_factor_loop_plan(ROOT)
