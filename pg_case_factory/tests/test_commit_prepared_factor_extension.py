...
class CommitPreparedFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_commit_prepared_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CommitPreparedFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            "COMMITPREPARED00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "COMMITPREPARED12284", plan.cases[-1].case_id
        )
        self.assertEqual(
            "COMMITPREPARED00045.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "COMMITPREPARED12284.sql",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_commit_prepared_factor_extension_plan(ROOT)
        plan_b = build_commit_prepared_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_commit_prepared_factor_extension_plan(ROOT)
        self.assertEqual(
            "4eef13f05ed668df194b85dfb136787b9b8fcbd9762d4f2e82139017965fe656",
