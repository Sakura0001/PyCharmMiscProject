...
# Frozen spec: GRM(2) + SFV(68) + RISK(2) = 72 ; INV is not applicable.
_OBLIGATION_COUNT = 72
_KIND_COUNTS = {"GRM": 2, "SFV": 68, "RISK": 2}
_FAILURE_COUNT = 16
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 56
_OBLIGATION_MULTISET_SHA256 = (
    "e1c953a67207fd2ce18c50eebe34a78028bb060f803773126f17f3f2a3238455"
)


class AlterPolicyFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_policy_factor_loop_obligations(ROOT)
...
class AlterPolicyFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_alter_policy_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterPolicyFactorLoopPlan)
        # delegated is empty for ALTER POLICY
        ...
        self.assertEqual("ALTERPOLICY00001", plan.cases[0].case_id)
        self.assertEqual(
            f"ALTERPOLICY{_OBLIGATION_COUNT:05d}", plan.cases[-1].case_id
        )
        self.assertEqual("ALTERPOLICY00001.sql", plan.cases[0].sql_filename)
        self.assertEqual(
            f"ALTERPOLICY{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual("alterpolicy_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            f"alterpolicy_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_expected_status_failure_sentinel_carries_missing_target(self) -> None:
        ...
        # (42704); its baseline assignment models the fixture as
        # object_state=not_exists so the renderer creates no policy and the
        # alter surfaces the not-found error.
        plan = build_alter_policy_factor_loop_plan(ROOT)
        sentinel = next(
            c
            for c in plan.cases
        ...
        self.assertEqual("not_exists", assignments["object_state"])
        self.assertEqual("42704", sentinel.expected_sqlstate)
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        # row_id (AP-SFV|{row_id}|{consumer}), proving the canonical ledger
        ...
        self.assertEqual(68, len(sfv_cases))
