"""Tests for the bounded DROP POLICY post-coverage extension expander.

The marginal factor-value-loop (``drop_policy_factor_loop.py``) is the
...
from pg_case_factory.drop_policy_factor_extension import (
...
from pg_case_factory.drop_policy_factor_loop import (
...
_BASELINE_COUNT = 48
_EXTENSION_COUNT = 1836
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 1884
_SUCCESS_EXTENSIONS = 648
_FAILURE_EXTENSIONS = 1188
_EXTENSION_MULTISET_SHA256 = (
    "5ac81271ea7ddabe10329b24d3d52f7ffa6152af02428de574d8c8f9abd93121"
)
...
_BRANCH_GRAMMAR = {"branch_drop_policy": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_policy": "drop_policy"}
...
class DropPolicyFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_policy_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropPolicyFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPPOLICY00049", first.case_id)
        self.assertEqual(f"DROPPOLICY{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPPOLICY00049.sql", first.sql_filename)
        self.assertEqual(
            f"DROPPOLICY{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droppolicy_00049_", first.object_prefix)
        self.assertEqual(
            f"droppolicy_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropPolicyFactorExtensionCase)
    ...
    def test_table_negative_fires_when_table_missing(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner":
                continue
    ...
    def test_policy_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_level"] == "non_owner":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            cm_counts["disable_rls_drop_policy"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_policy_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_policy_factor_loop_plan(ROOT)
