...
from pg_case_factory.alter_tablespace_factor_runtime import (
...
class AlterTablespaceRuntimeConstantsTest(unittest.TestCase):
...
class AlterTablespaceRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_6672_cases(self) -> None:
        sql_dir = Path("/tmp/alter_tablespace_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_alter_tablespace_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_alter_tablespace_runtime_case_set(
            ROOT, sql_dir
        )
...
class AlterTablespaceRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = AlterTablespacePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(AlterTablespaceRuntimeError):
            AlterTablespacePg18Runner(timeout_seconds=0)
        with self.assertRaises(AlterTablespaceRuntimeError):
            AlterTablespacePg18Runner(timeout_seconds=301)
...
class AlterTablespaceComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ALTERTABLESPACE00001"
    ) -> AlterTablespaceCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = AlterTablespaceSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = AlterTablespaceSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_alter_tablespace_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = AlterTablespaceCaseRuntimeResult(
            case_id="ALTERTABLESPACE00001",
            sql_filename="ALTERTABLESPACE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("ALTERTABLESPACE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(AlterTablespaceRuntimeError):
            AlterTablespaceSuiteRun(run_ordinal=3, cases=())
