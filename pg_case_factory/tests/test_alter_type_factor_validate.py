...
from pg_case_factory.alter_type_factor_validate import (
...
class AlterTypeFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_10522_files(self) -> None:
        result = validate_alter_type_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterTypeFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(94, result.baseline_case_count)
        self.assertEqual(10428, result.extension_case_count)
        self.assertEqual(10522, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(10522, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_type_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(10522, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_type_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_alter_type_factor_case(case, ROOT)
        tampered = sql.replace("ALTER TYPE", "ALTER SCHEMABOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ALTERTYPE00001", "ALTERTYPE00002"}
        result = validate_alter_type_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
