"""Tests for the bounded DROP EVENT TRIGGER post-coverage extension expander.

The marginal factor-value-loop (``drop_event_trigger_factor_loop.py``) is the
frozen 38-case baseline (GRM 1 + SFV 35 + RISK 2).  This module's expander
...
from pg_case_factory.drop_event_trigger_factor_extension import (
...
from pg_case_factory.drop_event_trigger_factor_loop import (
...
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 542
_SUCCESS_EXTENSIONS = 216
_FAILURE_EXTENSIONS = 288
_EXTENSION_MULTISET_SHA256 = (
    "c358ce468c0cf8baf2249b79099e98a950ad40aa179199e59d4f66f9774932f1"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("non_owner", "superuser", "trigger_owner"),
    "trigger_not_exist_no_if_exists": (
        "trigger_exists",
        "trigger_not_exists_no_if_exists",
    ),
    "if_exists_clause": ("omitted", "specified_if_exists"),
    "cascade_restrict": ("cascade", "omitted_default_restrict", "restrict"),
    ...
    "cleanup_mode": ("cascade_cleanup", "drop_event_trigger", "drop_function"),
}
...
_BRANCH_GRAMMAR = {"branch_drop_event_trigger": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_event_trigger": "drop_event_trigger"}
...
class DropEventTriggerFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_event_trigger_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropEventTriggerFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPEVENTTRIGGER00039", first.case_id)
        self.assertEqual(f"DROPEVENTTRIGGER{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPEVENTTRIGGER00039.sql", first.sql_filename)
        self.assertEqual(
            f"DROPEVENTTRIGGER{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropeventtrigger_00039_", first.object_prefix)
        self.assertEqual(
            f"dropeventtrigger_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropEventTriggerFactorExtensionCase)
    ...
    def test_not_exist_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_level"] == "non_owner":
                continue
            if (
                a["trigger_not_exist_no_if_exists"]
                == "trigger_not_exists_no_if_exists"
                and a["if_exists_clause"] == "omitted"
            ):
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["trigger_dependency"] != "has_dependencies":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["catalog_query_pg_event_trigger_absence"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["drop_event_trigger"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_event_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_event_trigger_factor_loop_plan(ROOT)
