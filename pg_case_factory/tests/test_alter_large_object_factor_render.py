...
from pg_case_factory.alter_large_object_factor_render import (
...
class AlterLargeObjectFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_large_object_factor_witness(case, ROOT)
                self.assertEqual(
                ...
                self.assertTrue(
                    witness.target_sql_fragment.startswith("ALTER LARGE OBJECT")
                )
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_large_object_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_large_object(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_large_object_factor_loop_plan(ROOT)
        first = [render_alter_large_object_factor_case(row, ROOT) for row in plan.cases]
        second = [render_alter_large_object_factor_case(row, ROOT) for row in plan.cases]
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(23, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(self) -> None:
        ...
        commit_sql = render_alter_large_object_factor_case(rows["commit"], ROOT)
        rollback_sql = render_alter_large_object_factor_case(rows["rollback"], ROOT)
    ...
    def test_oracle_is_catalog_audit_compliant(self) -> None:
        ...
        # never a ``SELECT EXISTS(SELECT 1 FROM ...)`` subquery.
        plan = build_alter_large_object_factor_loop_plan(ROOT)
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_large_object_factor_case(case, ROOT)
                ...
                self.assertRegex(
                    ...
                    r"FROM pg_catalog\.pg_largeobject_metadata",
                )
...
class AlterLargeObjectExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = build_alter_large_object_factor_extension_plan(ROOT)

    def _representative_subset(self) -> list[AlterLargeObjectFactorExtensionCase]:
        # One case per (branch, outcome) pair so every branch x outcome
        # combination is exercised without rendering all 204 programs.
        # ALTER LARGE OBJECT has a single branch, so this yields at most two.
        seen: set[tuple[str, str]] = set()
        subset: list[AlterLargeObjectFactorExtensionCase] = []
    ...
    def test_extension_cases_render_complete_5_phase_program(self) -> None:
        for case in self._representative_subset():
            with self.subTest(case_id=case.case_id):
                ...
                self.assertIn("-- description  : ALTER LARGE OBJECT", sql)
                self.assertNotRegex(sql, r"\{[A-Za-z_]\w*\}")
                self.assertTrue(sql.endswith(";\n"))
                self.assertFalse(sql.endswith("\n\n"))
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [render_alter_large_object_factor_case(c, ROOT) for c in subset]
        second = [render_alter_large_object_factor_case(c, ROOT) for c in subset]
        self.assertEqual(first, second)

    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_large_object_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_large_object"
            count = generate_alter_large_object_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_TOTAL, count)
            files = sorted(out.glob("ALTERLARGEOBJECT*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_large_object(b0))
            e0 = (out / self.extension_plan.cases[0].sql_filename).read_text(
                encoding="utf-8"
            )
            self.assertEqual(1, count_primary_alter_large_object(e0))
