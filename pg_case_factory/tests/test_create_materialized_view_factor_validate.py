...
from pg_case_factory.create_materialized_view_factor_validate import (
...
class CreateMaterializedViewFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="cmatview_validate_")
        )
        generate_create_materialized_view_factor_programs(
    ...
    def test_validation_passes_for_all_12912_files(self) -> None:
        result = validate_create_materialized_view_factor_programs(
        ...
        self.assertEqual(12864, result.extension_case_count)
        self.assertEqual(12912, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(12912, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_materialized_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(12912, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_materialized_view_factor_case(case, ROOT)
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
                sql, case
            )
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE MATERIALIZED VIEW", "CREATE SCHEMABOGUS"
        )
