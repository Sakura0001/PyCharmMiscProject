"""Tests for create_text_search_parser factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_text_search_parser_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateTextSearchParserFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55614)
        self.assertEqual(PG18_DATABASE, "pgcf_ctsp")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-ctsp-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        from src.pg_case_factory.create_text_search_parser_factor_extension import (
            build_create_text_search_parser_factor_extension_plan,
        )
        from src.pg_case_factory.create_text_search_parser_factor_loop import (
            build_create_text_search_parser_factor_loop_plan,
        )
        from src.pg_case_factory.create_text_search_parser_factor_render import (
            generate_create_text_search_parser_factor_programs,
        )
        bp = build_create_text_search_parser_factor_loop_plan(
            _REPO
        )
        ep = build_create_text_search_parser_factor_extension_plan(
            _REPO
        )
        generate_create_text_search_parser_factor_programs(
            bp, ep, sql_dir
        ...
        self.assertEqual(1014, len(cases))
        self.assertEqual(1014, len(paths))
    ...
    def test_runner_construction(self) -> None:
        runner = CreateTextSearchParserPg18Runner()
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(
            ...
            CreateTextSearchParserPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            CreateTextSearchParserRuntimeError
        ):
            CreateTextSearchParserPg18Runner(timeout_seconds=301)
