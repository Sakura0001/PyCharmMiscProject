...
class StartTransactionFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="start_transaction_render_")
        )
        cls.count = generate_start_transaction_factor_programs(
    ...
    def test_renders_exactly_1364_files(self) -> None:
        self.assertEqual(1364, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1364, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"STARTTRANSACTION{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_start_transaction(text)
    ...
    def test_primary_target_is_start_transaction(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*START\s+TRANSACTION\b", region
                ),
                f"no START TRANSACTION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_start_transaction_factor_witness(case, root)
            self.assertIsInstance(w, StartTransactionFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_start_transaction_factor_case(case, root)
        b = render_start_transaction_factor_case(case, root)
