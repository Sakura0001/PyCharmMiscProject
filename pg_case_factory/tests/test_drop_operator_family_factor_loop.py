...
_FAILURE_COUNT = 6
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 39
_OBLIGATION_MULTISET_SHA256 = (
    "c3c80da08bb132c453414c3c7c7bd35ea34fc6e8ab6a13404c4f8275b5325abf"
)


class DropOperatorFamilyFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_operator_family_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_operator_family_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropOperatorFamilyFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_operator_family_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropOperatorFamilyFactorLoopPlan)
        ...
        self.assertEqual("DROPOPERATORFAMILY00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPOPERATORFAMILY{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPOPERATORFAMILY00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPOPERATORFAMILY{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropoperatorfamily_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropoperatorfamily_{_OBLIGATION_COUNT:05d}_",
