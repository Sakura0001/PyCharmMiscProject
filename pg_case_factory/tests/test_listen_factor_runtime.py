"""Tests for listen factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.listen_factor_runtime import (
    MAX_PARALLELISM,
...
class TestListenFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55619)
        self.assertEqual(PG18_DATABASE, "pgcf_listen")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-listen-sock-20260821"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        from src.pg_case_factory.listen_factor_extension import (
            build_listen_factor_extension_plan,
        )
        from src.pg_case_factory.listen_factor_loop import (
            build_listen_factor_loop_plan,
        )
        from src.pg_case_factory.listen_factor_render import (
            generate_listen_factor_programs,
        )
        bp = build_listen_factor_loop_plan(_REPO)
        ep = build_listen_factor_extension_plan(_REPO)
        generate_listen_factor_programs(bp, ep, sql_dir)
        cases, paths = build_listen_runtime_case_set(
            _REPO, sql_dir
        )
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        ep = build_listen_factor_extension_plan(_REPO)
        generate_listen_factor_programs(bp, ep, sql_dir)
        cases, _ = build_listen_runtime_case_set(
            _REPO, sql_dir
        )
    ...
    def test_runner_construction(self) -> None:
        runner = ListenPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(ListenRuntimeError):
            ListenPg18Runner(timeout_seconds=0)
        with self.assertRaises(ListenRuntimeError):
            ListenPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(ListenRuntimeError):
            ListenSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(ListenRuntimeError):
            ListenSuiteRun(run_ordinal=3, cases=())

    def test_compare_runs_passes_for_identical(self) -> None:
        result = ListenSuiteRun(
            run_ordinal=1, cases=()
        )
        other = ListenSuiteRun(run_ordinal=2, cases=())
        comparison = compare_listen_runs(result, other)
