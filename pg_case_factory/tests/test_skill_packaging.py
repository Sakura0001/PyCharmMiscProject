...
class SkillPackagingTest(unittest.TestCase):
    def test_shipped_skill_packages_its_pg18_evidence_ledger(self) -> None:
        root = Path(__file__).resolve().parents[1]
        skill_root = root / "skills" / "pg-sql-generation"
        ledger_entry = (
            "pg-sql-generation/references/common/"
            "postgresql_18_4_factor_audit.tsv"
        )
        with TemporaryDirectory() as raw_dir:
            archive_path = Path(raw_dir) / "skill.zip"
            package_skill(skill_root, archive_path)

            with zipfile.ZipFile(archive_path) as archive:
                self.assertIn(ledger_entry, archive.namelist())
                self.assertEqual(
                    9_979,
                    len(archive.read(ledger_entry).decode("utf-8").splitlines()),
                )
    ...
    def test_package_is_reproducible_complete_and_clean(self) -> None:
        with TemporaryDirectory() as raw_dir:
            ...
            with zipfile.ZipFile(first) as archive:
                ...
                self.assertIn(
                    "pg-sql-generation/references/combinations/ddl/index/create_index.yaml",
                    names,
                )
                self.assertIn("pg-sql-generation/MANIFEST.sha256", names)
                self.assertTrue(
    ...
    def test_existing_manifest_is_reserved_and_repackaging_is_idempotent(self) -> None:
        with TemporaryDirectory() as raw_dir:
            ...
            with zipfile.ZipFile(archive_path) as archive:
                manifest_name = "pg-sql-generation/MANIFEST.sha256"
    ...
    def test_verify_rejects_missing_matrix_and_macos_metadata(self) -> None:
        with TemporaryDirectory() as raw_dir:
            ...
            with zipfile.ZipFile(archive_path, "w") as archive:
                archive.writestr("pg-sql-generation/SKILL.md", "# skill\n")
