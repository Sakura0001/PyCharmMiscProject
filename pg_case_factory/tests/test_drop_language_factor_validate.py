...
from pg_case_factory.drop_language_factor_validate import (
    DropLanguageFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_language_factor_programs,
)
...
class DropLanguageFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_539_files(self) -> None:
        result = validate_drop_language_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropLanguageFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_language_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPLANGUAGE00001", "DROPLANGUAGE00002"}
        result = validate_drop_language_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
