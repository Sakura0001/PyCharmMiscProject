...
class CopyFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_copy_factor_loop_plan(ROOT)
        cls.extension = build_copy_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="copy_render_"))
        cls.count = generate_copy_factor_programs(
    ...
    def test_renders_exactly_5619_files(self) -> None:
        self.assertEqual(5619, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(5619, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"COPY{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_copy(text)
    ...
    def test_primary_target_is_copy(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*COPY\b", region),
                f"no COPY in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_copy_factor_witness(case, root)
            self.assertIsInstance(w, CopyFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_copy_factor_case(case, root)
        b = render_copy_factor_case(case, root)
    ...
    def test_never_emits_from_stdin_or_program(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            region = self._extract_target_region(text)
            ...
            self.assertNotIn(
                "FROM STDIN",
                region.upper(),
                f"FROM STDIN leaked in {f.name}",
            )
            self.assertNotIn(
                "FROM PROGRAM",
                region.upper(),
                f"FROM PROGRAM leaked in {f.name}",
            )
