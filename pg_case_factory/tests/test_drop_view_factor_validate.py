...
from pg_case_factory.validate_drop_view import (
    DropViewFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_view_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropViewFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_view_factor_loop_plan(ROOT)
        cls.extension = build_drop_view_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_view_validate_"))
        generate_drop_view_factor_programs(
        ...
        cls.result = validate_drop_view_factor_programs(
    ...
    def test_validation_passes_for_all_3964_files(self) -> None:
        self.assertIsInstance(
            self.result, DropViewFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(40, self.result.baseline_case_count)
        self.assertEqual(3924, self.result.extension_case_count)
        self.assertEqual(3964, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(3964, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(3964, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_view_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_view_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPVIEW00001", "DROPVIEW00002"}
        result = validate_drop_view_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
