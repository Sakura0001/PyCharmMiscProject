"""Tests for create_text_search_dictionary factor validate module."""
...
from src.pg_case_factory.create_text_search_dictionary_factor_validate import (
...
class TestCreateTextSearchDictionaryFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_text_search_dictionary_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(45, report.baseline_case_count)
        self.assertEqual(4608, report.extension_case_count)
        self.assertEqual(4653, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_text_search_dictionary_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4653, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_text_search_dictionary_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4653, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATETEXTSEARCHDICTIONARY00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE TEXT SEARCH DICTIONARY", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "CREATETEXTSEARCHDICTIONARY00001",
            "CREATETEXTSEARCHDICTIONARY00046",
        }
        report = validate_create_text_search_dictionary_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_text_search_dictionary_actual_factor_witness_report", d["kind"]
        )
