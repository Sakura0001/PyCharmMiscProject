"""Tests for set_constraints factor_loop obligation ledger."""
...
class TestSetConstraintsFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_set_constraints_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "c15510eb153d50260695fee77ad99fe01d71bcbd7283329f"
            "53d4982b4f1fbd06",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_set_constraints_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("setconstraints_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("SETCONSTRAINTS"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_set_constraints_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
