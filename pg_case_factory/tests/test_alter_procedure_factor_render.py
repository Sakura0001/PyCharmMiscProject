...
from pg_case_factory.alter_procedure_factor_render import (
...
class AlterProcedureFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_procedure_factor_witness(case, ROOT)
                self.assertEqual(
                ...
                self.assertTrue(
                    witness.target_sql_fragment.startswith("ALTER PROCEDURE")
                )
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_procedure_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_procedure(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_procedure_factor_loop_plan(ROOT)
        first = [render_alter_procedure_factor_case(row, ROOT) for row in plan.cases]
        second = [render_alter_procedure_factor_case(row, ROOT) for row in plan.cases]
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(70, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(self) -> None:
        ...
        commit_sql = render_alter_procedure_factor_case(rows["commit"], ROOT)
        rollback_sql = render_alter_procedure_factor_case(rows["rollback"], ROOT)
    ...
    def test_header_carries_huawei_and_factor_metadata(self) -> None:
        ...
        self.assertIn("-- description  : ALTER PROCEDURE", sql)
        self.assertIn("-- FE           : PG18-STATEMENT-FACTOR-LOOP", sql)

    def test_no_return_type_in_procedure_fixture(self) -> None:
        # ALTER PROCEDURE targets procedures, not functions; the fixture
        # must use CREATE PROCEDURE ... AS $$ ... $$ without RETURNS.
        plan = build_alter_procedure_factor_loop_plan(ROOT)
        proc_case = next(
            c for c in plan.cases
            if c.factor_key == "object_state" and c.factor_value == "exists"
        )
        sql = render_alter_procedure_factor_case(proc_case, ROOT)
        # The fixture must contain CREATE PROCEDURE, not CREATE FUNCTION.
        self.assertIn("CREATE PROCEDURE", sql)
        self.assertNotIn("RETURNS", sql)

    def test_wrong_object_fixture_creates_function_for_42809(self) -> None:
        plan = build_alter_procedure_factor_loop_plan(ROOT)
        case = next(
            c for c in plan.cases
            if c.factor_key == "target_procedure_different_type"
            and c.factor_value == "same_name_is_function"
        )
        sql = render_alter_procedure_factor_case(case, ROOT)
        self.assertIn("CREATE FUNCTION", sql)
        self.assertIn("(integer)", sql)


class AlterProcedureExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = build_alter_procedure_factor_extension_plan(ROOT)

    def _representative_subset(self) -> list[AlterProcedureFactorExtensionCase]:
        # One case per (branch, outcome) pair so every branch x outcome
        # combination is exercised without rendering all 13,104 programs.
        seen: set[tuple[str, str]] = set()
        subset: list[AlterProcedureFactorExtensionCase] = []
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [render_alter_procedure_factor_case(c, ROOT) for c in subset]
        second = [render_alter_procedure_factor_case(c, ROOT) for c in subset]
        self.assertEqual(first, second)

    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_procedure_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_procedure"
            count = generate_alter_procedure_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_BASELINE_COUNT + _EXTENSION_COUNT, count)
            files = sorted(out.glob("ALTERPROCEDURE*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_procedure(b0))
            e0 = (out / self.extension_plan.cases[0].sql_filename).read_text(
                encoding="utf-8"
            )
            self.assertEqual(1, count_primary_alter_procedure(e0))
