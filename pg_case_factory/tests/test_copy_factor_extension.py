...
class CopyFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_copy_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, CopyFactorExtensionPlan)
        self.assertEqual(5568, len(plan.cases))
        self.assertEqual(5568, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(52, 5620)), ordinals)
        self.assertEqual("COPY00052", plan.cases[0].case_id)
        self.assertEqual("COPY05619", plan.cases[-1].case_id)
        self.assertEqual(
            "COPY00052.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "COPY05619.sql", plan.cases[-1].sql_filename
        )

    def test_outcome_counts_are_frozen(self) -> None:
        plan = build_copy_factor_extension_plan(ROOT)
        outcomes = Counter(c.outcome for c in plan.cases)
        self.assertEqual(1152, outcomes["success"])
        self.assertEqual(4416, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_copy_factor_extension_plan(ROOT)
        plan_b = build_copy_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_copy_factor_extension_plan(ROOT)
        self.assertEqual(
            "b34e89e52c65ede2c622b04e86344ece72b0befe7f9da201b95d809befbec12c",
