"""Tests for create_transform factor validate module."""
...
from src.pg_case_factory.create_transform_factor_validate import (
...
class TestCreateTransformFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_create_transform_factor_loop_plan(_REPO)
        )
        self.extension = (
            build_create_transform_factor_extension_plan(
                _REPO
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_transform_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(59, report.baseline_case_count)
        self.assertEqual(1218, report.extension_case_count)
        self.assertEqual(1277, report.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1277, len(report.records))
    ...
    def test_sha256_dictionary_complete(self) -> None:
        ...
        self.assertEqual(1277, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATETRANSFORM00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE TRANSFORM", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "CREATETRANSFORM00001",
            "CREATETRANSFORM00060",
        }
        report = validate_create_transform_factor_programs(
            self.baseline,
            self.extension,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_transform_actual_factor_witness_report",
            d["kind"],
        )
