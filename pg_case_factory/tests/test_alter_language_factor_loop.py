...
# Frozen spec: GRM(2) + SFV(38) + RISK(2) = 42 ; INV is not applicable.
_OBLIGATION_COUNT = 42
_KIND_COUNTS = {"GRM": 2, "SFV": 38, "RISK": 2}
_FAILURE_COUNT = 11
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 31
_OBLIGATION_MULTISET_SHA256 = (
    "d2980c6d51497e73153c5ccedbef5906edf6ec9cf8f30b99b96e30b656de6498"
)


class AlterLanguageFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_language_factor_loop_obligations(ROOT)
...
class AlterLanguageFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_alter_language_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterLanguageFactorLoopPlan)
        # delegated is empty for ALTER LANGUAGE (boundaries are expected_failure)
        ...
        self.assertEqual("ALTERLANGUAGE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"ALTERLANGUAGE{_OBLIGATION_COUNT:05d}", plan.cases[-1].case_id
        )
        self.assertEqual("ALTERLANGUAGE00001.sql", plan.cases[0].sql_filename)
        self.assertEqual(
            f"ALTERLANGUAGE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual("alterlanguage_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            f"alterlanguage_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        # row_id (AL-SFV|{row_id}|{consumer}), proving the canonical ledger
        ...
        self.assertEqual(38, len(sfv_cases))
