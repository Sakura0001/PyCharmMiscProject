...
from pg_case_factory.alter_language_factor_extension import (
...
class AlterLanguageFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_language_factor_witness(case, ROOT)
                self.assertEqual(
                ...
                self.assertTrue(
                    witness.target_sql_fragment.startswith("ALTER LANGUAGE")
                )
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_language_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_language(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_language_factor_loop_plan(ROOT)
        first = [render_alter_language_factor_case(row, ROOT) for row in plan.cases]
        second = [render_alter_language_factor_case(row, ROOT) for row in plan.cases]
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(31, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(self) -> None:
        ...
        commit_sql = render_alter_language_factor_case(rows["commit"], ROOT)
        rollback_sql = render_alter_language_factor_case(rows["rollback"], ROOT)
    ...
    def test_oracle_is_catalog_audit_compliant(self) -> None:
        ...
        # ``SELECT count(*) <op> AS alias FROM pg_catalog... WHERE... ORDER BY count(*)``
...
class AlterLanguageExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = build_alter_language_factor_extension_plan(ROOT)

    def _representative_subset(self) -> list[AlterLanguageFactorExtensionCase]:
        # One case per (branch, outcome) pair so every branch x outcome
        # combination is exercised without rendering all 600 programs.
        seen: set[tuple[str, str]] = set()
        subset: list[AlterLanguageFactorExtensionCase] = []
    ...
    def test_extension_cases_render_complete_5_phase_program(self) -> None:
        for case in self._representative_subset():
            with self.subTest(case_id=case.case_id):
                ...
                self.assertIn("-- description  : ALTER LANGUAGE", sql)
                self.assertNotRegex(sql, r"\{[A-Za-z_]\w*\}")
                self.assertTrue(sql.endswith(";\n"))
                self.assertFalse(sql.endswith("\n\n"))
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [render_alter_language_factor_case(c, ROOT) for c in subset]
        second = [render_alter_language_factor_case(c, ROOT) for c in subset]
        self.assertEqual(first, second)

    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_language_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_language"
            count = generate_alter_language_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_TOTAL, count)
            files = sorted(out.glob("ALTERLANGUAGE*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_language(b0))
            e0 = (out / self.extension_plan.cases[0].sql_filename).read_text(
                encoding="utf-8"
            )
            self.assertEqual(1, count_primary_alter_language(e0))
