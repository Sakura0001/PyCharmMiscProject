...
from pg_case_factory.insert_factor_runtime import (
...
class InsertRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_insert", PG18_DATABASE)
...
class InsertRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_17714_cases(self) -> None:
        sql_dir = Path("/tmp/insert_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_insert_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_insert_runtime_case_set(ROOT, sql_dir)
...
class InsertRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = InsertPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(InsertRuntimeError):
            InsertPg18Runner(timeout_seconds=0)
        with self.assertRaises(InsertRuntimeError):
            InsertPg18Runner(timeout_seconds=301)
...
class InsertComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "INSERT00001"
    ) -> InsertCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = InsertSuiteRun(run_ordinal=1, cases=(case,))
        run2 = InsertSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_insert_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = InsertCaseRuntimeResult(
            case_id="INSERT00001",
            sql_filename="INSERT00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("INSERT00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(InsertRuntimeError):
            InsertSuiteRun(run_ordinal=3, cases=())
