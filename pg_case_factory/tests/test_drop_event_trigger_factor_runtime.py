...
from pg_case_factory.drop_event_trigger_factor_runtime import (
...
class DropEventTriggerRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_det", PG18_DATABASE)
...
class DropEventTriggerRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_542_cases(self) -> None:
        sql_dir = Path("/tmp/drop_event_trigger_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_event_trigger_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(542, len(cases))
        self.assertEqual(542, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_event_trigger_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(542, ordinals[-1])


class DropEventTriggerRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropEventTriggerPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropEventTriggerRuntimeError):
            DropEventTriggerPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropEventTriggerRuntimeError):
            DropEventTriggerPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_event_trigger(self) -> None:
        runner = DropEventTriggerPg18Runner()
...
class DropEventTriggerComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPEVENTTRIGGER00001"
    ) -> DropEventTriggerCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropEventTriggerSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropEventTriggerSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_event_trigger_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropEventTriggerCaseRuntimeResult(
            case_id="DROPEVENTTRIGGER00001",
            sql_filename="DROPEVENTTRIGGER00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPEVENTTRIGGER00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropEventTriggerRuntimeError):
            DropEventTriggerSuiteRun(run_ordinal=3, cases=())
