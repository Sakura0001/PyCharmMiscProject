"""Tests for create_operator factor extension plan."""
...
class TestCreateOperatorFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_operator_factor_extension_plan(_REPO)

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(1224, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 300, "expected_failure": 924}, dict(counts)
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "c52696874ede351c428713a3c9f2b278a26a08568e5c66d8bd5f61c58c1e0ddc",
            self.plan.extension_multiset_sha256,
        )

    def test_constants(self) -> None:
        self.assertEqual(56, _BASELINE_COUNT)
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("COP-EXT|"))
