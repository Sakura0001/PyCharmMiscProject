...
from pg_case_factory.create_event_trigger_factor_extension import (
...
from pg_case_factory.create_event_trigger_factor_render import (
...
class CreateEventTriggerFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_event_trigger_factor_loop_plan(
            ROOT
        )
        cls.extension = (
            build_create_event_trigger_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="cet_render_")
        )
        cls.count = generate_create_event_trigger_factor_programs(
    ...
    def test_renders_exactly_6542_files(self) -> None:
        self.assertEqual(6542, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(6542, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATEEVENTTRIGGER{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_event_trigger(text)
        ...
        for f in files:
            ...
            for m in re.finditer(
                ...
                if target.startswith("information_schema."):
                    continue
                self.assertNotIn(
                    '"', target, f"quoted FROM in {f.name}"
                )
                self.assertNotIn(
                    ".", target, f"dotted FROM in {f.name}"
                )

    def test_primary_target_is_create_event_trigger(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^CREATE\s+EVENT\s+TRIGGER\b",
                    region,
                ),
                f"no CREATE EVENT TRIGGER in {f.name}",
            )
    ...
    def test_no_or_replace_clause(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
                re.search(
                    r"(?i)CREATE\s+OR\s+REPLACE\s+EVENT",
                    text,
                ),
                f"OR REPLACE found in {f.name}",
            )

    def test_witness_resolves_for_every_case(self) -> None:
        root = ROOT
        for case in self.baseline.cases[:5]:
            w = resolve_create_event_trigger_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, CreateEventTriggerFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_event_trigger_factor_case(case, root)
        b = render_create_event_trigger_factor_case(case, root)
