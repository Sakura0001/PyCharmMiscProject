...
from pg_case_factory.commit_factor_loop import (
    CommitFactorCase,
...
class CommitFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_commit_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(43) = 44 ; COMMIT has a single synopsis branch.
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # COMMIT: 6 canonical values reach the PG target check and are
        # rejected (and_chain / and_no_chain are VALID for COMMIT, so they
    ...
    def test_chain_behavior_values_are_success_for_commit(self) -> None:
        # COMMIT supports AND [NO] CHAIN -- these are success, not failure.
        self.assertNotIn(
    ...
    def test_outside_transaction_is_failure_for_commit(self) -> None:
        # COMMIT outside an active transaction warns 25P01.
        self.assertIn(
...
class CommitFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_commit_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CommitFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(44, len(plan.cases))
        self.assertEqual(
            list(range(1, 45)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("COMMIT00001", plan.cases[0].case_id)
        self.assertEqual("COMMIT00044", plan.cases[-1].case_id)
        self.assertEqual(
            "COMMIT00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "COMMIT00044.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual(
            "commit_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            "commit_00044_", plan.cases[-1].object_prefix
        )
        self.assertEqual(
            44,
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_commit_factor_loop_plan(ROOT)
        again = build_commit_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_commit_factor_loop_plan(ROOT)
        self.assertEqual(
            "fdc671e60f988e2077c14add19b593fdf2c69db004845f0bd0c1ca576c454bc8",
            plan.obligation_multiset_sha256,
        )
