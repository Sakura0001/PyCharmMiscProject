...
from pg_case_factory.delete_factor_extension import (
    build_delete_factor_extension_plan,
)
from pg_case_factory.delete_factor_loop import (
    build_delete_factor_loop_plan,
)
from pg_case_factory.delete_factor_render import (
    generate_delete_factor_programs,
    render_delete_factor_case,
)
from pg_case_factory.delete_factor_validate import (
    DeleteFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_delete_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DeleteFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_delete_factor_loop_plan(ROOT)
        cls.extension = build_delete_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="delete_validate_"))
        generate_delete_factor_programs(
    ...
    def test_validation_passes_for_all_17714_files(self) -> None:
        result = validate_delete_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DeleteFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_delete_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_delete_factor_case(case, ROOT)
        tampered = sql.replace("DELETE", "DELETEBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DELETE00001", "DELETE00002"}
        result = validate_delete_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
