...
class StartTransactionFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_start_transaction_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, StartTransactionFactorExtensionPlan)
        self.assertEqual(1320, len(plan.cases))
        self.assertEqual(1320, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(45, 1365)), ordinals)
        self.assertEqual(
            "STARTTRANSACTION00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "STARTTRANSACTION01364", plan.cases[-1].case_id
        )
        self.assertEqual(
            "STARTTRANSACTION00045.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "STARTTRANSACTION01364.sql", plan.cases[-1].sql_filename
        )

    def test_outcome_counts_are_frozen(self) -> None:
        plan = build_start_transaction_factor_extension_plan(ROOT)
        outcomes = Counter(c.outcome for c in plan.cases)
        self.assertEqual(360, outcomes["success"])
        self.assertEqual(960, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_start_transaction_factor_extension_plan(ROOT)
        plan_b = build_start_transaction_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_start_transaction_factor_extension_plan(ROOT)
        self.assertEqual(
            "a8dc0d6bd0811d9c46c6e6a6f0c551f2ceb27495c4858643167cef14beb91deb",
