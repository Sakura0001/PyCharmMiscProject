"""Tests for create_function factor extension plan."""
...
class TestCreateFunctionFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_function_factor_extension_plan(_REPO)

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(9600, len(self.plan.cases))
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "25d0c6646a4b18288706e8ea531b4de7de1811126ce47b93147cee7d1731baeb",
            self.plan.extension_multiset_sha256,
        )

    def test_constants(self) -> None:
        self.assertEqual(121, _BASELINE_COUNT)
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("CFUNC-EXT|"))
