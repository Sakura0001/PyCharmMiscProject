...
_OBLIGATION_MULTISET_SHA256 = (
    "d1ca3d69f180eaa741b49a779b895a8fe3d298c3c0c8b6f08a57592db5994fd4"
)


class DropTriggerFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_trigger_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_trigger_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTriggerFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_trigger_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTriggerFactorLoopPlan)
        ...
        self.assertEqual("DROPTRIGGER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTRIGGER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTRIGGER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTRIGGER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptrigger_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptrigger_{_OBLIGATION_COUNT:05d}_",
