...
from pg_case_factory.create_database_factor_validate import (
...
class CreateDatabaseFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="crdb_validate_"))
        generate_create_database_factor_programs(
    ...
    def test_validation_passes_for_all_16268_files(self) -> None:
        result = validate_create_database_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CreateDatabaseFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(68, result.baseline_case_count)
        self.assertEqual(16200, result.extension_case_count)
        self.assertEqual(16268, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(16268, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_database_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(16268, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_database_factor_case(case, ROOT)
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
                sql, case
            )
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_create_database_factor_case(case, ROOT)
        tampered = sql.replace("CREATE DATABASE", "CREATE BOGUSDB")
