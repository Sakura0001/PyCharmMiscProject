...
from pg_case_factory.alter_text_search_dictionary_factor_extension import (
...
from pg_case_factory.alter_text_search_dictionary_factor_render import (
    generate_alter_text_search_dictionary_factor_programs,
    render_alter_text_search_dictionary_factor_case,
)
from pg_case_factory.alter_text_search_dictionary_factor_validate import (
    AlterTextSearchDictionaryFactorProgramValidation,
...
class AlterTextSearchDictionaryFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="atsd_validate_")
        )
        generate_alter_text_search_dictionary_factor_programs(
    ...
    def test_validation_passes_for_all_1384_files(self) -> None:
        result = validate_alter_text_search_dictionary_factor_programs(
        ...
        self.assertEqual(64, result.baseline_case_count)
        self.assertEqual(1320, result.extension_case_count)
        self.assertEqual(1384, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1384, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_text_search_dictionary_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(1384, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_text_search_dictionary_factor_case(
            case, ROOT
        )
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            ...
            "ALTERTEXTSEARCHDICTIONARY00002",
        }
        result = validate_alter_text_search_dictionary_factor_programs(
            self.baseline,
            self.extension,
