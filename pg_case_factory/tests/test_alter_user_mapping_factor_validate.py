...
from pg_case_factory.alter_user_mapping_factor_extension import (
...
from pg_case_factory.alter_user_mapping_factor_render import (
    generate_alter_user_mapping_factor_programs,
    render_alter_user_mapping_factor_case,
)
from pg_case_factory.alter_user_mapping_factor_validate import (
    AlterUserMappingFactorProgramValidation,
...
class AlterUserMappingFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_alter_user_mapping_factor_loop_plan(ROOT)
        cls.extension = build_alter_user_mapping_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="aum_validate_"))
        generate_alter_user_mapping_factor_programs(
    ...
    def test_validation_passes_for_all_6103_files(self) -> None:
        result = validate_alter_user_mapping_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterUserMappingFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(55, result.baseline_case_count)
        self.assertEqual(6048, result.extension_case_count)
        self.assertEqual(6103, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(6103, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_user_mapping_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(6103, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_user_mapping_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "ALTER USER MAPPING", "ALTER SCHEMABOGUS"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ALTERUSERMAPPING00001", "ALTERUSERMAPPING00002"}
        result = validate_alter_user_mapping_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
