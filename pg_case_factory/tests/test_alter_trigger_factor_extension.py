...
class AlterTriggerFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_trigger_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTriggerFactorExtensionPlan
        )
        self.assertEqual(3024, len(plan.cases))
        self.assertEqual(3024, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(44, 3068)), ordinals
        )
        self.assertEqual(
            "ALTERTRIGGER00044", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTRIGGER03067", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTRIGGER00044.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTRIGGER03067.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(1056, outcomes["success"])
        self.assertEqual(1968, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_trigger_factor_extension_plan(ROOT)
        plan_b = build_alter_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
            "caade568e3d2ece23eabc1f709e57d555e56afc5b02c7eba9a5892eecc48e7af",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("DROP_TRIGGER", "DROP_TRIGGER_IF_EXISTS"),
            _CLEANUP_MODES,
        )
