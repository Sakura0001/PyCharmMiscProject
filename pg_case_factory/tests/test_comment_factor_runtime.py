...
from pg_case_factory.comment_factor_runtime import (
...
class CommentRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_comment", PG18_DATABASE)
...
class CommentRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_13662_cases(self) -> None:
        sql_dir = Path("/tmp/COMMENT_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_comment_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(13662, len(cases))
        self.assertEqual(13662, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_comment_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(13662, ordinals[-1])


class CommentRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CommentPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CommentRuntimeError):
            CommentPg18Runner(timeout_seconds=0)
        with self.assertRaises(CommentRuntimeError):
            CommentPg18Runner(timeout_seconds=301)
...
class CommentComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "COMMENT00001"
    ) -> CommentCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CommentSuiteRun(run_ordinal=1, cases=(case,))
        run2 = CommentSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_comment_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CommentCaseRuntimeResult(
            case_id="COMMENT00001",
            sql_filename="COMMENT00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "COMMENT00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CommentRuntimeError):
            CommentSuiteRun(run_ordinal=3, cases=())
