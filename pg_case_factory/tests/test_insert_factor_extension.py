...
class InsertFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_insert_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, InsertFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual("INSERT00051", plan.cases[0].case_id)
        self.assertEqual("INSERT17714", plan.cases[-1].case_id)
        self.assertEqual(
            "INSERT00051.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "INSERT17714.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_insert_factor_extension_plan(ROOT)
        plan_b = build_insert_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_insert_factor_extension_plan(ROOT)
        self.assertEqual(
            "b9548c8c39f882b3b90f6440010ead979c4cbd3381e61fa631f2e90c05a8b082",
