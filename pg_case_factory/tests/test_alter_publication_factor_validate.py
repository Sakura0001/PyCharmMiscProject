...
from pg_case_factory.alter_publication_factor_validate import (
    AlterPublicationFactorProgramValidation,
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_publication_factor_extension_plan(ROOT)
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_publication_factor_case(c, ROOT)
        for c in chosen
    }
...
class AlterPublicationFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_publication_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(
            report, AlterPublicationFactorProgramValidation
        )
    ...
    def test_missing_program_is_reported(self) -> None:
        baseline_plan = build_alter_publication_factor_loop_plan(ROOT)
        extension_plan = (
            build_alter_publication_factor_extension_plan(ROOT)
        )
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        extra = "ALTERPUBLICATION99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERPUBLICATION99999\n"
            "-- primary_obligation_id: ALTPUB-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER PUBLICATION bogus ADD TABLE t;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_publication_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_publication_factor_case(
            target_case, ROOT
        )
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(
            rec["primary_obligation_id"].startswith("ALTPUB-EXT|")
        )
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn(
            "alter_publication_actual_factor_witness_report", blob
        )
        self.assertIn(
    ...
    def test_byte_mismatch_is_detected(self) -> None:
        ...
        canonical = render_alter_publication_factor_case(
            target_case, ROOT
        )
        tampered = canonical.replace(
            "ALTER PUBLICATION", "ALTER  PUBLICATION", 1
        )
