"""Tests for show factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.show_factor_runtime import (
    MAX_PARALLELISM,
...
class TestShowFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_show")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-show-sock-20260821"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_show_factor_loop_plan(_REPO)
        ep = build_show_factor_extension_plan(_REPO)
        generate_show_factor_programs(bp, ep, sql_dir)
        cases, paths = build_show_runtime_case_set(
            _REPO, sql_dir
        )
        self.assertEqual(5036, len(cases))
        self.assertEqual(5036, len(paths))
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        ep = build_show_factor_extension_plan(_REPO)
        generate_show_factor_programs(bp, ep, sql_dir)
        cases, _ = build_show_runtime_case_set(
            _REPO, sql_dir
        )
    ...
    def test_runner_construction(self) -> None:
        runner = ShowPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(ShowRuntimeError):
            ShowPg18Runner(timeout_seconds=0)
        with self.assertRaises(ShowRuntimeError):
            ShowPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(ShowRuntimeError):
            ShowSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(ShowRuntimeError):
            ShowSuiteRun(run_ordinal=3, cases=())

    def test_compare_runs_passes_for_identical(self) -> None:
        result = ShowSuiteRun(
            run_ordinal=1, cases=()
        )
        other = ShowSuiteRun(run_ordinal=2, cases=())
        comparison = compare_show_runs(result, other)
