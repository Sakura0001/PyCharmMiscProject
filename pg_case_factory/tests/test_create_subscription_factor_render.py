"""Tests for create_subscription factor render module."""
...
class TestCreateSubscriptionFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_subscription_factor_loop_plan(_REPO)
        self.extension = build_create_subscription_factor_extension_plan(_REPO)
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_create_subscription_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(1111, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [f"CREATESUBSCRIPTION{i:05d}.sql" for i in range(1, 1112)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_create_subscription(sql))
    ...
    def test_primary_target_is_create_subscription(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split("-- primary-target-begin", 1)[1]
            region = region.split("-- primary-target-end", 1)[0]
            self.assertRegex(region, r"(?im)CREATE\s+SUBSCRIPTION\b")
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_subscription_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_subscription_factor_case(case, _REPO)
            b = render_create_subscription_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "CREATESUBSCRIPTION00001.sql").read_text()
        self.assertIn("-- case_id: CREATESUBSCRIPTION00001", sql)
