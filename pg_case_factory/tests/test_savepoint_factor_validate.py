...
from pg_case_factory.savepoint_factor_render import (
    generate_savepoint_factor_programs,
    render_savepoint_factor_case,
)
from pg_case_factory.validate_savepoint import (
    SavepointFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_savepoint_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class SavepointFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_savepoint_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="savepoint_validate_")
        )
        generate_savepoint_factor_programs(
    ...
    def test_validation_passes_for_all_1844_files(self) -> None:
        result = validate_savepoint_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, SavepointFactorProgramValidation
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_savepoint_factor_case(case, ROOT)
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
                sql, case
            )
