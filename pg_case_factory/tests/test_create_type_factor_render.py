"""Tests for create_type factor render module."""
...
class TestCreateTypeFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_type_factor_loop_plan(_REPO)
        self.extension = build_create_type_factor_extension_plan(_REPO)
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_create_type_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(3471, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [f"CREATETYPE{i:05d}.sql" for i in range(1, 3472)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_create_type(sql))
    ...
    def test_primary_target_is_create_type(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split("-- primary-target-begin", 1)[1]
            region = region.split("-- primary-target-end", 1)[0]
            self.assertRegex(region, r"(?im)CREATE\s+TYPE\b")
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_type_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_type_factor_case(case, _REPO)
            b = render_create_type_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "CREATETYPE00001.sql").read_text()
        self.assertIn("-- case_id: CREATETYPE00001", sql)
    ...
    def test_all_five_branches_present(self) -> None:
        ...
        for f in sorted(self.out.iterdir()):
            ...
            region = region.split("-- primary-target-end", 1)[0]
            for m in re.finditer(
                r"CREATE TYPE \S+ (AS|AS ENUM|AS RANGE|\()", region
            ):
                seen.add(m.group(1))
        # composite (AS), enum (AS ENUM), range (AS RANGE), base (()
        self.assertIn("AS", seen)
