"""Tests for create_procedure factor render module."""
...
class TestCreateProcedureFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_procedure_factor_loop_plan(_REPO)
        self.extension = build_create_procedure_factor_extension_plan(_REPO)
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_create_procedure_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(4412, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [f"CREATEPROCEDURE{i:05d}.sql" for i in range(1, 4413)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_create_procedure(sql))
    ...
    def test_primary_target_is_create_procedure(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            ...
            self.assertRegex(
                region,
                r"(?im)CREATE\s+(?:OR\s+REPLACE\s+)?PROCEDURE\b",
            )
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_procedure_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_procedure_factor_case(case, _REPO)
            b = render_create_procedure_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "CREATEPROCEDURE00001.sql").read_text()
        self.assertIn("-- case_id: CREATEPROCEDURE00001", sql)
