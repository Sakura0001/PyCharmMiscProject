"""Tests for create_operator_family factor extension plan."""
...
class TestCreateOperatorFamilyFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_operator_family_factor_extension_plan(
            _REPO
        )

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(1188, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 216, "expected_failure": 972}, dict(counts)
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "8dce2808d48bb4b6ad09b5f2800b7ce07c269d3707681a4c36a2d9a3da6c3304",
            self.plan.extension_multiset_sha256,
        )

    def test_constants(self) -> None:
        self.assertEqual(31, _BASELINE_COUNT)
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("COF-EXT|"))
