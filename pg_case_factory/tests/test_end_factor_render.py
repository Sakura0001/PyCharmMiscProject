...
class EndFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_end_factor_loop_plan(ROOT)
        cls.extension = build_end_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="end_render_"))
        cls.count = generate_end_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"END{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_end(text)
    ...
    def test_primary_target_is_end(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_end_factor_witness(case, root)
            self.assertIsInstance(w, EndFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_end_factor_case(case, root)
        b = render_end_factor_case(case, root)
