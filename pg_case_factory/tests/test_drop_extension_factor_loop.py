...
# Frozen spec: GRM(1) + SFV(42) + RISK(2) = 45 ; INV is not applicable.
_OBLIGATION_COUNT = 45
_KIND_COUNTS = {"GRM": 1, "SFV": 42, "RISK": 2}
_FAILURE_COUNT = 4
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 41
_OBLIGATION_MULTISET_SHA256 = (
    "0461ae65e9b7bbf5d68d85e5ff1a9bdb4e94f85d52942608acac11cb53133fec"
)


class DropExtensionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_extension_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(42, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(42, sum(seen.values()))
        self.assertEqual(42, len(seen))
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_extension_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropExtensionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_extension_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropExtensionFactorLoopPlan)
        ...
        self.assertEqual("DROPEXTENSION00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPEXTENSION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPEXTENSION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPEXTENSION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropextension_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropextension_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(42, len(sfv_cases))
