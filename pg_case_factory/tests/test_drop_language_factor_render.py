...
from pg_case_factory.drop_language_factor_render import (
...
class DropLanguageFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_language_factor_loop_plan(ROOT)
        cls.extension = build_drop_language_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_language_render_"))
        cls.count = generate_drop_language_factor_programs(
    ...
    def test_renders_exactly_539_files(self) -> None:
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPLANGUAGE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_language(text)
    ...
    def test_no_file_creates_a_table(self) -> None:
        """drop_language fixtures are LANGUAGE + FUNCTION only; no tables."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_primary_target_is_drop_language(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+LANGUAGE\b", region),
                f"no col-0 DROP LANGUAGE in {f.name}",
            )

    def test_no_procedural_keyword_in_target(self) -> None:
        """All cases use the modern DROP LANGUAGE form (no PROCEDURAL)."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            region = self._extract_target_region(text)
            self.assertIsNotNone(region, f.name)
            self.assertFalse(
                re.search(r"(?im)^\s*DROP\s+PROCEDURAL\b", region),
                f"unexpected DROP PROCEDURAL in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_language_factor_witness(case, root)
            self.assertIsInstance(w, DropLanguageFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_language_factor_case(case, root)
        b = render_drop_language_factor_case(case, root)
