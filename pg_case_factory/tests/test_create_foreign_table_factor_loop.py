...
class CreateForeignTableFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_foreign_table_factor_loop_obligations(ROOT)
        self.assertEqual(96, len(rows))
        self.assertEqual(
            {"GRM": 5, "SFV": 91},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(96, len({row.obligation_id for row in rows}))
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(91, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(91, sum(seen.values()))
        self.assertEqual(91, len(seen))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        self.assertEqual(19, len(failures))
        self.assertEqual(77, covered)
        self.assertEqual(len(failures), len(set(failures)))
        for pair in failures:
            self.assertIn(pair, _SFV_FAILURE_SQLSTATE)


class CreateForeignTableFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_foreign_table_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateForeignTableFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(96, len(plan.cases))
        self.assertEqual(
            list(range(1, 97)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATEFOREIGNTABLE00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEFOREIGNTABLE00096", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEFOREIGNTABLE00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEFOREIGNTABLE00096.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createforeigntable_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createforeigntable_00096_",
        ...
        self.assertEqual(
            96, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_foreign_table_factor_loop_plan(ROOT)
        obligations = (
            compile_create_foreign_table_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_foreign_table_factor_loop_plan(ROOT)
        again = build_create_foreign_table_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_foreign_table_factor_loop_plan(ROOT)
        self.assertEqual(
            "18cffd1fb519043c799b523cf59e035debdeaf8a1267330019d3c4167c58d4f4",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(77, outcomes["success"])
        self.assertEqual(19, outcomes["expected_failure"])
