...
class AlterViewFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_alter_view_factor_loop_plan(ROOT)
        cls.extension = build_alter_view_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="aview_render_"))
        cls.count = generate_alter_view_factor_programs(
    ...
    def test_renders_exactly_8321_files(self) -> None:
        self.assertEqual(8321, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(8321, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"ALTERVIEW{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_alter_view(text)
    ...
    def test_primary_target_is_alter_view(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*ALTER\s+VIEW\b", region),
                f"no ALTER VIEW in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_alter_view_factor_witness(case, root)
            self.assertIsInstance(w, AlterViewFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_alter_view_factor_case(case, root)
        b = render_alter_view_factor_case(case, root)
