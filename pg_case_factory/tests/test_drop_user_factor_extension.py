"""Tests for the bounded DROP USER post-coverage extension expander.

The marginal factor-value-loop (``drop_user_factor_loop.py``) is the
...
_BASELINE_COUNT = 39
_EXTENSION_COUNT = 1566
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 1605
_SUCCESS_EXTENSIONS = 396
_FAILURE_EXTENSIONS = 1170
_EXTENSION_MULTISET_SHA256 = (
    "9c1d9651338f3afb0fb71544525b61935edc67d2c6fac501eb194634a5d730a7"
)
...
_BRANCH_GRAMMAR = {"branch_drop_user": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_user": "drop_user"}
...
class DropUserFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_user_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropUserFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPUSER00040", first.case_id)
        self.assertEqual(f"DROPUSER{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPUSER00040.sql", first.sql_filename)
        self.assertEqual(
            f"DROPUSER{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropuser_00040_", first.object_prefix)
        self.assertEqual(
            f"dropuser_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropUserFactorExtensionCase)
    ...
    def test_privilege_negative_fires_for_non_createrole(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["authorization_path"] == "non_createrole":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
        ...
        for case in self.plan.cases:
            ...
            if (
                (
                    ...
                    or a["role_name_shape"] == "non_existent_name"
                )
    ...
    def test_dependency_negative_fires(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["authorization_path"] == "non_createrole":
                continue
            if (
                ...
                continue
            if a["dependency_context"] == "no_dependencies":
                continue
            self.assertEqual(
                "expected_failure", case.outcome, case.case_id
            )
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            cm_counts["reassign_objects_then_drop"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_user_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_user_factor_loop_plan(ROOT)
