"""Tests for listen factor_loop obligation ledger."""
...
class TestListenFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_listen_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "bb3692887f337c08b072b51ea9fe9a5d8db605eaf4d17e3d"
            "92734c7cea843349",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_listen_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("listen_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("LISTEN"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_listen_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
