...
_OBLIGATION_MULTISET_SHA256 = (
    "fe3d67148150231d29b347e07856dd930afa516e135ba8dc84047f9ca304a8b0"
)


class DropDomainFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_domain_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_domain_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropDomainFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_domain_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropDomainFactorLoopPlan)
        ...
        self.assertEqual("DROPDOMAIN00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPDOMAIN{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPDOMAIN00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPDOMAIN{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropdomain_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropdomain_{_OBLIGATION_COUNT:05d}_",
