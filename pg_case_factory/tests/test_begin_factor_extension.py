...
class BeginFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_begin_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, BeginFactorExtensionPlan)
        self.assertEqual(1560, len(plan.cases))
        self.assertEqual(1560, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(45, 1605)), ordinals)
        self.assertEqual("BEGIN00045", plan.cases[0].case_id)
        self.assertEqual("BEGIN01604", plan.cases[-1].case_id)
        self.assertEqual(
            "BEGIN00045.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "BEGIN01604.sql", plan.cases[-1].sql_filename
        )

    def test_outcome_counts_are_frozen(self) -> None:
        plan = build_begin_factor_extension_plan(ROOT)
        outcomes = Counter(c.outcome for c in plan.cases)
        self.assertEqual(480, outcomes["success"])
        self.assertEqual(1080, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_begin_factor_extension_plan(ROOT)
        plan_b = build_begin_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_begin_factor_extension_plan(ROOT)
        self.assertEqual(
            "67530fa4bba0c0484ebe75a7c1f2c39d94efef34f52fff48384634d65336a931",
