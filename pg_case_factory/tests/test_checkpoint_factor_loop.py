...
class CheckpointFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_checkpoint_factor_loop_obligations(ROOT)
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # 2 canonical values reach the PG target check and are rejected;
...
class CheckpointFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_checkpoint_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CheckpointFactorLoopPlan)
        ...
        self.assertEqual(
            "CHECKPOINT00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CHECKPOINT00046", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CHECKPOINT00001.sql",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_checkpoint_factor_loop_plan(ROOT)
        obligations = compile_checkpoint_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_checkpoint_factor_loop_plan(ROOT)
        again = build_checkpoint_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_checkpoint_factor_loop_plan(ROOT)
        self.assertEqual(
            "bccb5bce8217cdf38ce19daa45cf18be1ed1b4d995a4e3547a3db48928212e13",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(2, outcomes["expected_failure"])
