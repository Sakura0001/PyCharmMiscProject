...
from pg_case_factory.validate_drop_user import (
    DropUserFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_user_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropUserFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_user_factor_loop_plan(ROOT)
        cls.extension = build_drop_user_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_user_validate_"))
        generate_drop_user_factor_programs(
        ...
        cls.result = validate_drop_user_factor_programs(
    ...
    def test_validation_passes_for_all_1605_files(self) -> None:
        self.assertIsInstance(
            self.result, DropUserFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(39, self.result.baseline_case_count)
        self.assertEqual(1566, self.result.extension_case_count)
        self.assertEqual(1605, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(1605, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(1605, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_user_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_user_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPUSER00001", "DROPUSER00002"}
        result = validate_drop_user_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
