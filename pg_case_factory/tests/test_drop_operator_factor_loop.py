...
# Frozen spec: GRM(1) + SFV(40) + RISK(2) = 43 ; INV is not applicable.
_OBLIGATION_COUNT = 43
_KIND_COUNTS = {"GRM": 1, "SFV": 40, "RISK": 2}
_FAILURE_COUNT = 7
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 36
_OBLIGATION_MULTISET_SHA256 = (
    "a1a5ca96351754b7a939eb8fb313254dfde78296361a87f12b5ca2d26f967d0d"
)


class DropOperatorFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_operator_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_operator_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropOperatorFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_operator_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropOperatorFactorLoopPlan)
        ...
        self.assertEqual("DROPOPERATOR00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPOPERATOR{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPOPERATOR00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPOPERATOR{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropoperator_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropoperator_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(40, len(sfv_cases))
