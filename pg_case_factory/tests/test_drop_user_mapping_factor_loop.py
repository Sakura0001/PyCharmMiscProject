...
_OBLIGATION_MULTISET_SHA256 = (
    "9ac6157271d5bbe07c901fd5f55f828cc0316a657c475ae699c8d85524da0820"
)


class DropUserMappingFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_user_mapping_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_user_mapping_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropUserMappingFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_user_mapping_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropUserMappingFactorLoopPlan)
        ...
        self.assertEqual("DROPUSERMAPPING00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPUSERMAPPING{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPUSERMAPPING00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPUSERMAPPING{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropusermapping_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropusermapping_{_OBLIGATION_COUNT:05d}_",
