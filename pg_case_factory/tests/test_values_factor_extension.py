...
class ValuesFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_values_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, ValuesFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(
        ...
        self.assertEqual(list(range(50, 17714)), ordinals)
        self.assertEqual(
            "VALUES00050", plan.cases[0].case_id
        )
        self.assertEqual(
            "VALUES17713", plan.cases[-1].case_id
        )
        self.assertEqual(
            "VALUES00050.sql",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_values_factor_extension_plan(ROOT)
        plan_b = build_values_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_values_factor_extension_plan(ROOT)
        self.assertEqual(
            "d628da4c875d0ed2d86680eba2899e671612e8a89bf310dfd960b8260ef71c1b",
