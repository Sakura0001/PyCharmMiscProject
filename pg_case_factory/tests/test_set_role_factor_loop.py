"""Tests for set_role factor_loop obligation ledger."""
...
class TestSetRoleFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_set_role_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(43, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(43, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 42}, dict(counts))

    def test_factor_value_count(self) -> None:
        from src.pg_case_factory.set_role_factor_loop import (
            _FACTOR_VALUE_COUNT,
        )
        self.assertEqual(42, _FACTOR_VALUE_COUNT)

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "9d503d2e7e485332f742fb6d34e1b59b35546eb3aa7b95a"
            "55770cd1abd30a736",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_set_role_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_contiguous_ordinals(self) -> None:
        ...
        self.assertEqual(list(range(1, 44)), ordinals)
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("SETROLE"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_set_role_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
