"""Tests for do factor validate module."""
...
from src.pg_case_factory.do_factor_validate import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_do_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestDoFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_do_factor_loop_plan(_REPO)
        )
        self.extension = (
            build_do_factor_extension_plan(_REPO)
        )
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_do_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(10740, report.extension_case_count)
        self.assertEqual(10786, report.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(10786, len(report.records))
    ...
    def test_sha256_dictionary_complete(self) -> None:
        ...
        self.assertEqual(10786, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "DO00001.sql"
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "do_actual_factor_witness_report",
            d["kind"],
        )
