...
from pg_case_factory.alter_large_object_factor_validate import (
    AlterLargeObjectFactorProgramValidation,
...
# Module-level cache so the 235-program render happens once across tests.
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_large_object_factor_extension_plan(ROOT)
        programs: dict[str, str] = {}
        for case in baseline_plan.cases:
            programs[case.sql_filename] = render_alter_large_object_factor_case(
                case, ROOT
            )
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_large_object_factor_case(c, ROOT) for c in chosen
    }
    selected = {c.case_id for c in chosen}
    return programs, selected


class AlterLargeObjectFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_large_object_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(report, AlterLargeObjectFactorProgramValidation)
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 99999 is outside the 0001-00235 case range, so it is genuinely unknown.
        extra = "ALTERLARGEOBJECT99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERLARGEOBJECT99999\n"
            "-- primary_obligation_id: ALO-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER LARGE OBJECT 99999 OWNER TO bogus_owner;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_large_object_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_large_object_factor_case(target_case, ROOT)
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(rec["primary_obligation_id"].startswith("ALO-EXT|"))
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn("alter_large_object_actual_factor_witness_report", blob)
