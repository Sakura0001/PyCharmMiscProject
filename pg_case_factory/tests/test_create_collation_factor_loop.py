"""Tests for create_collation factor_loop obligation ledger."""
...
class TestCreateCollationFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_collation_factor_loop_plan(_REPO)
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 5, "SFV": 50}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "23ffa8311877569b67805bbeacb385bdca499000378c9ac4f0e13de0bb43a50e",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_collation_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 40, "expected_failure": 15}, dict(counts)
        )
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("createcollation_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATECOLLATION"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_collation_factor_loop_obligations(_REPO)
