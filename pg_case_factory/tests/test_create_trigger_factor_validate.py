...
from pg_case_factory.create_trigger_factor_validate import (
    CreateTriggerFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_trigger_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class CreateTriggerFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="ctrg_validate_"))
        generate_create_trigger_factor_programs(
    ...
    def test_validation_passes_for_all_6571_files(self) -> None:
        result = validate_create_trigger_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CreateTriggerFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(91, result.baseline_case_count)
        self.assertEqual(6480, result.extension_case_count)
        self.assertEqual(6571, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(6571, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_trigger_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(6571, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_trigger_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE TRIGGER", "CREATE SCHEMABOGUS"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATETRIGGER00001", "CREATETRIGGER00002"}
        result = validate_create_trigger_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
