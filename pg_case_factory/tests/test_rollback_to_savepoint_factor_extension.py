...
class RollbackToSavepointFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_rollback_to_savepoint_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, RollbackToSavepointFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(
        ...
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT01844", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00045.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT01844.sql",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_rollback_to_savepoint_factor_extension_plan(ROOT)
        plan_b = build_rollback_to_savepoint_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_rollback_to_savepoint_factor_extension_plan(ROOT)
        self.assertEqual(
            "98d8262007dbc0b47036225990e4f17c858ac753fae5d9b49cf182f30a5cf6fc",
