...
class CreateDatabaseFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_database_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateDatabaseFactorExtensionPlan
        )
        self.assertEqual(16200, len(plan.cases))
        self.assertEqual(16200, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(69, 16269)), ordinals
        )
        self.assertEqual(
            "CREATEDATABASE00069", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEDATABASE16268", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEDATABASE00069.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEDATABASE16268.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(6480, outcomes["success"])
        self.assertEqual(9720, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_database_factor_extension_plan(ROOT)
        plan_b = build_create_database_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_database_factor_extension_plan(ROOT)
        self.assertEqual(
            "358c2b33717f55d739eb74e0d0fb90b9f4170dd08a55c8e7bcbae4d70f467b1d",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(68, _BASELINE_COUNT)
        ...
        self.assertEqual(
            ("drop_database", "force_drop_database"),
            _CLEANUP_MODES,
        )
