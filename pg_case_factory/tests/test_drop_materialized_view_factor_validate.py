...
from pg_case_factory.drop_materialized_view_factor_validate import (
    DropMaterializedViewFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_materialized_view_factor_programs,
)
...
class DropMaterializedViewFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="drop_materialized_view_validate_")
        )
        generate_drop_materialized_view_factor_programs(
    ...
    def test_validation_passes_for_all_877_files(self) -> None:
        result = validate_drop_materialized_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropMaterializedViewFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(37, result.baseline_case_count)
        self.assertEqual(840, result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_materialized_view_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "DROPMATERIALIZEDVIEW00001",
            "DROPMATERIALIZEDVIEW00002",
        }
        result = validate_drop_materialized_view_factor_programs(
            self.baseline,
            self.extension,
