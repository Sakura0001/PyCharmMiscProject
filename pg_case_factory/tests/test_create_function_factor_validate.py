"""Tests for create_function factor validate module."""
...
from src.pg_case_factory.create_function_factor_validate import (
...
class TestCreateFunctionFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_function_factor_loop_plan(_REPO)
        self.extension = build_create_function_factor_extension_plan(_REPO)
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_function_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(121, report.baseline_case_count)
        self.assertEqual(9600, report.extension_case_count)
        self.assertEqual(9721, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_function_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(9721, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_function_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(9721, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATEFUNCTION00001.sql"
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATEFUNCTION00001", "CREATEFUNCTION00122"}
        report = validate_create_function_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_function_actual_factor_witness_report", d["kind"]
        )
