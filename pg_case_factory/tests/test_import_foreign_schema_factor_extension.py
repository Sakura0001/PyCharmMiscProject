"""Tests for import_foreign_schema factor extension plan."""
...
class TestImportForeignSchemaFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = (
    ...
    def test_frozen_extension_count(self) -> None:
        self.assertEqual(576, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 192, "expected_failure": 384},
            dict(counts),
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "dbff947f8a9780bb37a806d5054e3f7c5d736cbd"
            "f163e5071e4e2dbbaf004e85",
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.derivation_id.startswith(
                    "IMPORTFOREIGNSCHEMA-EXT|"
                )
            )
