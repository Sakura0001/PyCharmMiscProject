...
# Frozen spec: GRM(1) + SFV(27) + RISK(2) = 30 ; INV is not applicable.
_OBLIGATION_COUNT = 30
_KIND_COUNTS = {"GRM": 1, "SFV": 27, "RISK": 2}
_FAILURE_COUNT = 8
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 22
_OBLIGATION_MULTISET_SHA256 = (
    "2787fddccc95535953660591f8d76b90c4287e53740b99196b0d8315848feeae"
)


class DropCollationFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_collation_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(27, sum(seen.values()))
...
class DropCollationFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_collation_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropCollationFactorLoopPlan)
        # delegated is empty for DROP COLLATION (boundaries are
        # expected_failure)
        ...
        self.assertEqual("DROPCOLLATION00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPCOLLATION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPCOLLATION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPCOLLATION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropcollation_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropcollation_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        # row_id (DC-SFV|{row_id}|{consumer}), proving the canonical ledger
        ...
        self.assertEqual(27, len(sfv_cases))
