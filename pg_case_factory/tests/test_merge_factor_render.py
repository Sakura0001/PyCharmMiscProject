...
class MergeFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_merge_factor_loop_plan(ROOT)
        cls.extension = build_merge_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="merge_render_"))
        cls.count = generate_merge_factor_programs(
    ...
    def test_renders_exactly_17716_files(self) -> None:
        self.assertEqual(17716, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(17716, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"MERGE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_merge(text)
    ...
    def test_primary_target_is_merge(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^MERGE\b", region),
                f"no col-0 MERGE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_merge_factor_witness(case, root)
            self.assertIsInstance(w, MergeFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_merge_factor_case(case, root)
        b = render_merge_factor_case(case, root)
