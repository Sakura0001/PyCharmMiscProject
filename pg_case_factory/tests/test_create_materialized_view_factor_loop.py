...
class CreateMaterializedViewFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_materialized_view_factor_loop_obligations(
            ROOT
        )
        # GRM(1) + SFV(47) = 48 ; RISK is not applicable.
...
class CreateMaterializedViewFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_materialized_view_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateMaterializedViewFactorLoopPlan)
        ...
        self.assertEqual(
            "CREATEMATERIALIZEDVIEW00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEMATERIALIZEDVIEW00048", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEMATERIALIZEDVIEW00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEMATERIALIZEDVIEW00048.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "creatematerializedview_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "creatematerializedview_00048_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_materialized_view_factor_loop_plan(ROOT)
        obligations = (
            compile_create_materialized_view_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_materialized_view_factor_loop_plan(ROOT)
        again = build_create_materialized_view_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_materialized_view_factor_loop_plan(ROOT)
        self.assertEqual(
            "7924708eeeade9ebfa3ab77f2767211544b97b7392edb323cd8934fd25f172bd",
            plan.obligation_multiset_sha256,
        )
