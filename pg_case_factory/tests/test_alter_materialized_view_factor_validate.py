...
from pg_case_factory.alter_materialized_view_factor_validate import (
...
# Module-level cache so the 8485-program render happens once across tests.
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_materialized_view_factor_case(c, ROOT)
        for c in chosen
    }
...
class AlterMaterializedViewFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_materialized_view_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(
            report, AlterMaterializedViewFactorProgramValidation
        )
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 99999 is outside the 0001-08485 case range, so it is genuinely
        # unknown.
        extra = "ALTERMATERIALIZEDVIEW99999.sql"
        programs = {
            **programs,
            extra: (
                "-- --------------------------------------------------------\n"
                "-- case_id: ALTERMATERIALIZEDVIEW99999\n"
                "-- primary_obligation_id: AMV-BOGUS|unknown\n"
                "-- expected_outcome: success\n"
                "-- expected_sqlstate: 00000\n"
                "-- primary-target-begin\n"
                "ALTER MATERIALIZED VIEW 99999 OWNER TO bogus_owner;\n"
                "-- primary-target-end\n"
                "SELECT 1;\n"
            ),
        }
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_materialized_view_factor_case(
            target_case, ROOT
        )
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(rec["primary_obligation_id"].startswith("AMV-EXT|"))
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn(
            "alter_materialized_view_actual_factor_witness_report", blob
        )
