"""Tests for prepare factor_loop obligation ledger."""
...
class TestPrepareFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_prepare_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "ab9fc355c3cef5f2eecff78a95792647"
            "def986c282528d629c751dabd438f998",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_prepare_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.case_id.startswith("PREPARE")
            )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = (
            compile_prepare_factor_loop_obligations(_REPO)
        )
        self.assertEqual(
