"""Tests for create_type factor validate module."""
...
from src.pg_case_factory.create_type_factor_validate import (
    remove_primary_semantic_locus_but_keep_comments,
    validate_create_type_factor_programs,
)

_REPO = Path(__file__).resolve().parents[1]


class TestCreateTypeFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_type_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(96, report.baseline_case_count)
        self.assertEqual(3375, report.extension_case_count)
        self.assertEqual(3471, report.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(3471, len(report.records))
    ...
    def test_sha256_dictionary_complete(self) -> None:
        ...
        self.assertEqual(3471, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATETYPE00001.sql"
        original = self.programs[filename]
        tampered = original.replace("CREATE TYPE", "CREATE TABLE", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATETYPE00001", "CREATETYPE00097"}
        report = validate_create_type_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_type_actual_factor_witness_report",
            d["kind"],
        )
