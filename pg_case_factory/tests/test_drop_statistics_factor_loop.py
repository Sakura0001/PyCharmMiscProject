...
# Frozen spec: GRM(1) + SFV(32) + RISK(2) = 35 ; INV is not applicable.
_OBLIGATION_COUNT = 35
_KIND_COUNTS = {"GRM": 1, "SFV": 32, "RISK": 2}
_FAILURE_COUNT = 8
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 27
_OBLIGATION_MULTISET_SHA256 = (
    "dec9266331d5957f6696e2090b74354d78b5756075f8b91ffb19db7e1296b495"
)


class DropStatisticsFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_statistics_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_statistics_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropStatisticsFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_statistics_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropStatisticsFactorLoopPlan)
        ...
        self.assertEqual("DROPSTATISTICS00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPSTATISTICS{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPSTATISTICS00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPSTATISTICS{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropstatistics_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropstatistics_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(32, len(sfv_cases))
        for case in sfv_cases:
            self.assertRegex(
                case.primary_obligation_id,
                r"^DROPSTATISTICS-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
