...
_FAILURE_COUNT = 3
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 42
_OBLIGATION_MULTISET_SHA256 = (
    "31d767a9aab1ab924c02f3e4546894788b3f9621fdca93c0c86456086d0dc15e"
)


class DropForeignDataWrapperFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_foreign_data_wrapper_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_foreign_data_wrapper_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropForeignDataWrapperFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_foreign_data_wrapper_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropForeignDataWrapperFactorLoopPlan)
        ...
        self.assertEqual("DROPFOREIGNDATAWRAPPER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPFOREIGNDATAWRAPPER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPFOREIGNDATAWRAPPER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPFOREIGNDATAWRAPPER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropforeigndatawrapper_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropforeigndatawrapper_{_OBLIGATION_COUNT:05d}_",
