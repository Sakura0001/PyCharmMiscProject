"""Tests for set_session_authorization factor render module."""
...
class TestSetSessionAuthorizationFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
    ...
    def test_filename_numbering(self) -> None:
        ...
        self.assertEqual(
            [
                f"SETSESSIONAUTHORIZATION{i:05d}.sql"
                for i in range(1, 4267)
            ],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(
                1, count_primary_set_session_authorization(sql)
            )
    ...
    def test_primary_target_is_set_session_authorization(
        self,
    ) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
    ...
    def test_col0_keyword_regex(self) -> None:
        rx = re.compile(
            r"(?m)^SET\s+SESSION\s+AUTHORIZATION"
            r"(?:\s|;|$)"
        )
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_set_session_authorization_factor_case(
                case, _REPO
            )
            b = render_set_session_authorization_factor_case(
                case, _REPO
            )
    ...
    def test_header_fields(self) -> None:
        sql = (
            self.out / "SETSESSIONAUTHORIZATION00001.sql"
        ).read_text()
        self.assertIn(
            "-- case_id: SETSESSIONAUTHORIZATION00001", sql
        )
