...
class CallFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_call_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, CallFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(50, 17714)), ordinals
        )
        self.assertEqual("CALL00050", plan.cases[0].case_id)
        self.assertEqual("CALL17713", plan.cases[-1].case_id)
        self.assertEqual(
            "CALL00050.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "CALL17713.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_call_factor_extension_plan(ROOT)
        plan_b = build_call_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_call_factor_extension_plan(ROOT)
        self.assertEqual(
            "3cde2d5cb391b0c9d69068c7117de1b6a1cc82c605f1c081e14dc0d27a8a16b5",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(49, _BASELINE_COUNT)
