...
from pg_case_factory.drop_text_search_parser_factor_render import (
    generate_drop_text_search_parser_factor_programs,
    render_drop_text_search_parser_factor_case,
)
from pg_case_factory.validate_drop_text_search_parser import (
    DropTsParserFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_text_search_parser_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropTsParserFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_drop_text_search_parser_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="drop_ts_parser_validate_")
        )
        generate_drop_text_search_parser_factor_programs(
        ...
        cls.result = validate_drop_text_search_parser_factor_programs(
    ...
    def test_validation_passes_for_all_1020_files(self) -> None:
        self.assertIsInstance(
            self.result, DropTsParserFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(36, self.result.baseline_case_count)
        self.assertEqual(984, self.result.extension_case_count)
        self.assertEqual(1020, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(1020, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(1020, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_text_search_parser_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_text_search_parser_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
