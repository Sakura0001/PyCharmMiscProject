"""Tests for the bounded DROP OPERATOR post-coverage extension expander.

The marginal factor-value-loop (``drop_operator_factor_loop.py``) is the
frozen 43-case baseline (GRM 1 + SFV 40 + RISK 2).  This module's expander
...
from pg_case_factory.drop_operator_factor_extension import (
...
from pg_case_factory.drop_operator_factor_loop import (
...
_BASELINE_COUNT = 43
_EXTENSION_COUNT = 1920
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 1963
_SUCCESS_EXTENSIONS = 528
_FAILURE_EXTENSIONS = 1392
_EXTENSION_MULTISET_SHA256 = (
    "8251889f8b5a5e715e7f53d56548726577eafee29f7ad08ced7662eb058ca1c8"
)
...
_CROSSED_AXES = {
    ...
    "ownership_boundary": ("non_owner", "superuser", "owner"),
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_operator"}
...
class DropOperatorFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_operator_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropOperatorFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPOPERATOR00044", first.case_id)
        self.assertEqual(f"DROPOPERATOR{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPOPERATOR00044.sql", first.sql_filename)
        self.assertEqual(
            f"DROPOPERATOR{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropoperator_00044_", first.object_prefix)
        self.assertEqual(
            f"dropoperator_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropOperatorFactorExtensionCase)
    ...
    def test_privilege_negative_fires_for_non_owner(self) -> None:
        for case in self.plan.cases:
            ...
            if (a["privilege_context"] in ("non_owner", "insufficient_privilege")
                or a["ownership_boundary"] == "non_owner"):
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_operator_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_operator_factor_loop_plan(ROOT)
