...
_FAILURE_COUNT = 11
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 32
_OBLIGATION_MULTISET_SHA256 = (
    "3251a1ea617725679e37f8756da08a0a2c1b290f8eba14fe884abdaf9c257240"
)


class DropProcedureFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_procedure_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_procedure_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropProcedureFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_procedure_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropProcedureFactorLoopPlan)
        ...
        self.assertEqual("DROPPROCEDURE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPPROCEDURE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual("dropprocedure_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            _OBLIGATION_COUNT,
