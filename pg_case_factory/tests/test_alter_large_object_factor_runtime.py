...
from pg_case_factory.alter_large_object_factor_runtime import (
...
def _sample_case(stdout: bytes) -> AlterLargeObjectCaseRuntimeResult:
    return AlterLargeObjectCaseRuntimeResult(
        case_id="ALTERLARGEOBJECT00001",
        sql_filename="ALTERLARGEOBJECT00001.sql",
...
class AlterLargeObjectFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(self) -> None:
        identical = compare_alter_large_object_runs(
            AlterLargeObjectSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterLargeObjectSuiteRun(run_ordinal=2, cases=(_sample_case(b"ok\n"),)),
        )
        self.assertTrue(identical.passed, identical.issues)

        changed = compare_alter_large_object_runs(
            AlterLargeObjectSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterLargeObjectSuiteRun(
                run_ordinal=2,
        ...
        self.assertEqual(
            ("ALTERLARGEOBJECT00001",),
            changed.transcript_mismatch_case_ids,
        )

    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        # The case-set combiner must merge the frozen 31-case baseline with
        # the bounded 204-case extension into a single ordinal-sorted
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_alter_large_object_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..235 sequence",
            )
            self.assertEqual(1, ordinals[0])
            self.assertEqual(_TOTAL, ordinals[-1])
            # baseline (ordinal 1) is not an extension; last (235) is.
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        selected = (
            ...
            one(
                lambda case: case.factor_key == "oid_shape"
                and case.factor_value == "nonexistent_oid"
            ),
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for case in selected:
                ...
                path.write_text(
                    render_alter_large_object_factor_case(case, ROOT),
                    encoding="utf-8",
                )
                paths[case.case_id] = path

            runner = AlterLargeObjectPg18Runner()
            ...
            comparison = compare_alter_large_object_runs(run_01, run_02)
