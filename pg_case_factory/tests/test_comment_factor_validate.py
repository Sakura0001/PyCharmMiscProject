...
from pg_case_factory.comment_factor_validate import (
    CommentFactorProgramValidation,
...
class CommentFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_13662_files(self) -> None:
        result = validate_comment_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CommentFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(117, result.baseline_case_count)
        self.assertEqual(13545, result.extension_case_count)
        self.assertEqual(13662, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(13662, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_comment_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(13662, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_comment_factor_case(case, ROOT)
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_comment_factor_case(case, ROOT)
        tampered = sql.replace("COMMENT ON", "COMMENTBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"COMMENT00001", "COMMENT00002"}
        result = validate_comment_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
