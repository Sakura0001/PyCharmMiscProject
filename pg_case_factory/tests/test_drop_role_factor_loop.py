...
# Frozen spec: GRM(1) + SFV(56) + RISK(2) = 59 ; INV is not applicable.
_OBLIGATION_COUNT = 59
_KIND_COUNTS = {"GRM": 1, "SFV": 56, "RISK": 2}
_FAILURE_COUNT = 3
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 56
_OBLIGATION_MULTISET_SHA256 = (
    "a6a77516ee7f38805cfbec8f0a84a3fc9f3bb513091ddc26e4f8db0174d36042"
)


class DropRoleFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_role_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_role_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropRoleFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_role_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropRoleFactorLoopPlan)
        ...
        self.assertEqual("DROPROLE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPROLE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual("DROPROLE00001.sql", plan.cases[0].sql_filename)
        self.assertEqual(
            f"DROPROLE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual("droprole_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            f"droprole_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(56, len(sfv_cases))
