...
from pg_case_factory.drop_schema_factor_runtime import (
...
class DropSchemaRuntimeConstantsTest(unittest.TestCase):
...
class DropSchemaRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1111_cases(self) -> None:
        sql_dir = Path("/tmp/drop_schema_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_schema_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(1111, len(cases))
        self.assertEqual(1111, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_schema_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1111, ordinals[-1])


class DropSchemaRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropSchemaPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropSchemaRuntimeError):
            DropSchemaPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropSchemaRuntimeError):
            DropSchemaPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_namespace(self) -> None:
        ...
        self.assertIn("nspname", joined)


class DropSchemaComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPSCHEMA00001"
    ) -> DropSchemaCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropSchemaSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropSchemaSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_schema_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropSchemaCaseRuntimeResult(
            case_id="DROPSCHEMA00001",
            sql_filename="DROPSCHEMA00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="2BP01",
            exit_code=0,
            timed_out=False,
            normalized_stdout=b"PGCF_TARGET_SQLSTATE=2BP01\n",
            normalized_stderr=b"",
            boolean_oracle_failure_count=0,
            pre_clean=True,
            post_clean=True,
        ...
        self.assertIn("DROPSCHEMA00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropSchemaRuntimeError):
            DropSchemaSuiteRun(run_ordinal=3, cases=())
