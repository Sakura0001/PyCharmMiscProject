...
from pg_case_factory.drop_index_factor_runtime import (
...
class DropIndexRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_di", PG18_DATABASE)
    ...
    def test_socket_and_bin_are_frozen(self) -> None:
        self.assertEqual(
            "/tmp/pgcf-pg18-di-sock-20260820", str(PG18_SOCKET)
        )
        self.assertEqual(
            "/tmp/pgcf-postgresql-18.4-install/bin", str(PG18_BIN)
        )


class DropIndexRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_407_cases(self) -> None:
        sql_dir = Path("/tmp/drop_index_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_index_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(407, len(cases))
        self.assertEqual(407, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_index_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(407, ordinals[-1])


class DropIndexRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropIndexPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropIndexRuntimeError):
            DropIndexPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropIndexRuntimeError):
            DropIndexPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_class(self) -> None:
        ...
        self.assertIn("pg_constraint", joined)


class DropIndexComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPINDEX00001"
    ) -> DropIndexCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropIndexSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropIndexSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_index_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropIndexCaseRuntimeResult(
            case_id="DROPINDEX00001",
            sql_filename="DROPINDEX00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPINDEX00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropIndexRuntimeError):
            DropIndexSuiteRun(run_ordinal=3, cases=())
