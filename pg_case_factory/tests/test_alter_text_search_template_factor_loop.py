...
from pg_case_factory.alter_text_search_template_factor_loop import (
...
class AlterTextSearchTemplateFactorLoopLedgerTest(unittest.TestCase):
...
class AlterTextSearchTemplateFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_text_search_template_factor_loop_plan(ROOT)
        ...
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00035", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00035.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "altertextsearchtemplate_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "altertextsearchtemplate_00035_",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_text_search_template_factor_loop_plan(ROOT)
        again = build_alter_text_search_template_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_text_search_template_factor_loop_plan(ROOT)
        self.assertEqual(
            "11e7dccacc51ff8f1747bb20dc106b6f08ec9da222926f174ed03569ea6a9800",
            plan.obligation_multiset_sha256,
        )
