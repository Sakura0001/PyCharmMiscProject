...
class DropGroupFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_group_factor_loop_plan(ROOT)
        cls.extension = build_drop_group_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_group_render_"))
        cls.count = generate_drop_group_factor_programs(
    ...
    def test_renders_exactly_197_files(self) -> None:
        self.assertEqual(197, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(197, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPGROUP{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_group(text)
    ...
    def test_primary_target_is_drop_group(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+GROUP\b", region),
                f"no col-0 DROP GROUP in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_group_factor_witness(case, root)
            self.assertIsInstance(w, DropGroupFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_group_factor_case(case, root)
        b = render_drop_group_factor_case(case, root)
