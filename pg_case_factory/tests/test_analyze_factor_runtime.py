...
from pg_case_factory.analyze_factor_runtime import (
...
class AnalyzeRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_analyze", PG18_DATABASE)
...
class AnalyzeRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_7152_cases(self) -> None:
        sql_dir = Path("/tmp/analyze_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_analyze_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(7152, len(cases))
        self.assertEqual(7152, len(sql_paths))

    def test_case_set_is_sorted_by_ordinal(self) -> None:
        sql_dir = Path("/tmp/analyze_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_analyze_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(7152, ordinals[-1])


class AnalyzeRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = AnalyzePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(AnalyzeRuntimeError):
            AnalyzePg18Runner(timeout_seconds=0)
        with self.assertRaises(AnalyzeRuntimeError):
            AnalyzePg18Runner(timeout_seconds=301)
...
class AnalyzeComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ANALYZE00001"
    ) -> AnalyzeCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = AnalyzeSuiteRun(run_ordinal=1, cases=(case,))
        run2 = AnalyzeSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_analyze_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = AnalyzeCaseRuntimeResult(
            case_id="ANALYZE00001",
            sql_filename="ANALYZE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("ANALYZE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(AnalyzeRuntimeError):
            AnalyzeSuiteRun(run_ordinal=3, cases=())
