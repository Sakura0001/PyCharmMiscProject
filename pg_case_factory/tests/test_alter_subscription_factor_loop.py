...
from pg_case_factory.alter_subscription_factor_loop import (
...
class AlterSubscriptionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_subscription_factor_loop_obligations(ROOT)
        # GRM(11) + SFV(70) = 81 ; RISK is not applicable
        # (ALTER SUBSCRIPTION is non-transactional).
        self.assertEqual(81, len(rows))
        self.assertEqual(
            {"GRM": 11, "SFV": 70},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(81, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 56 SFV + 11 GRM = 67 are covered.
        self.assertEqual(14, len(failures))
        self.assertEqual(67, covered)
        self.assertEqual(len(failures), len(set(failures)))
        for pair in failures:
            self.assertIn(pair, _SFV_FAILURE_SQLSTATE)


class AlterSubscriptionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_subscription_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterSubscriptionFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(81, len(plan.cases))
        self.assertEqual(
            list(range(1, 82)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "ALTERSUBSCRIPTION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERSUBSCRIPTION00081", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERSUBSCRIPTION00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERSUBSCRIPTION00081.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "alter_subscription_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "alter_subscription_00081_",
        ...
        self.assertEqual(
            81, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_alter_subscription_factor_loop_plan(ROOT)
        obligations = compile_alter_subscription_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_subscription_factor_loop_plan(ROOT)
        again = build_alter_subscription_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_subscription_factor_loop_plan(ROOT)
        self.assertEqual(
            "87adfb9da81a3bdf96a848d976a8ebff2801afa84efb12e0a22c0493ff3bd3b4",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(67, outcomes["success"])
