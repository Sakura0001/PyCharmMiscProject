...
from pg_case_factory.select_factor_render import (
    SelectFactorWitness,
...
class SelectFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_select_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="select_render_")
        )
        cls.count = generate_select_factor_programs(
    ...
    def test_renders_exactly_17715_files(self) -> None:
        self.assertEqual(17715, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(17715, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"SELECT{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_select(text)
            self.assertEqual(1, n, f.name)

    def test_no_file_uses_quoted_create_table(self) -> None:
        # CLASS-2 bookend gate: fixture CREATE TABLE must use UNQUOTED
        # base names; the quoted/shaped form appears only in the
        # credited FROM target, never in CREATE TABLE.
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_primary_target_is_select(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*SELECT\b", region),
                f"no SELECT in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_select_factor_witness(case, root)
            self.assertIsInstance(w, SelectFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_select_factor_case(case, root)
        b = render_select_factor_case(case, root)
