...
from pg_case_factory.savepoint_factor_loop import (
    SavepointFactorCase,
...
class SavepointFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_savepoint_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(43) = 44 ; SAVEPOINT has a single synopsis.
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # SAVEPOINT: 6 canonical values reach the PG target check
    ...
    def test_chain_behavior_values_are_success_for_savepoint(
        self,
    ) -> None:
        # SAVEPOINT mirrors COMMIT: and_chain / and_no_chain are
    ...
    def test_outside_transaction_is_failure_for_savepoint(
        self,
    ) -> None:
        # SAVEPOINT outside an active transaction warns 25P01.
        self.assertIn(
...
class SavepointFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_savepoint_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, SavepointFactorLoopPlan)
        ...
        self.assertEqual(
            "SAVEPOINT00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "SAVEPOINT00044", plan.cases[-1].case_id
        )
        self.assertEqual(
            "SAVEPOINT00001.sql",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_savepoint_factor_loop_plan(ROOT)
        obligations = (
            compile_savepoint_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_savepoint_factor_loop_plan(ROOT)
        again = build_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
            "6ae15b02a79909b5041d58be37f0c7ff938829436eaaf90061eff191340da94e",
            plan.obligation_multiset_sha256,
        )
