...
# Frozen spec: GRM(1) + SFV(33) + RISK(2) = 36 ; INV is not applicable.
_OBLIGATION_COUNT = 36
_KIND_COUNTS = {"GRM": 1, "SFV": 33, "RISK": 2}
_FAILURE_COUNT = 10
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 26
_OBLIGATION_MULTISET_SHA256 = (
    "b5aad9e7cf04b3517ec346cd595b9d0a1a87732a47379c62e34f42c4979808c6"
)


class DropTsParserFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_text_search_parser_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_text_search_parser_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTsParserFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_text_search_parser_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTsParserFactorLoopPlan)
        ...
        self.assertEqual("DROPTEXTSEARCHPARSER00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTEXTSEARCHPARSER{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTEXTSEARCHPARSER00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHPARSER{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptextsearchparser_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptextsearchparser_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_drop_text_search_parser_factor_loop_plan(ROOT)
        obligations = (
            compile_drop_text_search_parser_factor_loop_obligations(ROOT)
        )
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(33, len(sfv_cases))
        for case in sfv_cases:
            self.assertRegex(
                case.primary_obligation_id,
                r"^DROPTEXTSEARCHPARSER-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
