...
class SavepointFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="savepoint_render_")
        )
        cls.count = generate_savepoint_factor_programs(
    ...
    def test_every_file_has_exactly_one_primary_target(
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_savepoint(text)
    ...
    def test_primary_target_is_savepoint(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*SAVEPOINT\b", region
                ),
                f"no SAVEPOINT in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_savepoint_factor_witness(case, root)
            self.assertIsInstance(w, SavepointFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_savepoint_factor_case(case, root)
        b = render_savepoint_factor_case(case, root)
