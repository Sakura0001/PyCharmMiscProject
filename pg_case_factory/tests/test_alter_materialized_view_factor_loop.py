...
# Frozen spec: GRM(6) + SFV(53) + RISK(2) = 61 ; INV is not applicable.
_OBLIGATION_COUNT = 61
_KIND_COUNTS = {"GRM": 6, "SFV": 53, "RISK": 2}
_FAILURE_COUNT = 10
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 51
_OBLIGATION_MULTISET_SHA256 = (
    "ae781fe39e4a89ab990976b6905ba71def7ce4ab23dccd5399b2671eaa25b238"
)


class AlterMaterializedViewFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_materialized_view_factor_loop_obligations(ROOT)
...
class AlterMaterializedViewFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_alter_materialized_view_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterMaterializedViewFactorLoopPlan)
        # delegated is empty for ALTER MATERIALIZED VIEW
        ...
        self.assertEqual("ALTERMATERIALIZEDVIEW00001", plan.cases[0].case_id)
        self.assertEqual(
            f"ALTERMATERIALIZEDVIEW{_OBLIGATION_COUNT:05d}", plan.cases[-1].case_id
        )
        self.assertEqual("ALTERMATERIALIZEDVIEW00001.sql", plan.cases[0].sql_filename)
        self.assertEqual(
            f"ALTERMATERIALIZEDVIEW{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual("altermaterializedview_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            f"altermaterializedview_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_expected_status_failure_sentinel_carries_missing_target(self) -> None:
        # expected_status=failure is the "materialized view does not exist"
        # sentinel (42P01); its baseline assignment must model the fixture as
        # target_object_state=missing so the renderer creates no mview.
        plan = build_alter_materialized_view_factor_loop_plan(ROOT)
        sentinel = next(
            c
            for c in plan.cases
            if c.factor_key == "expected_status" and c.factor_value == "failure"
        )
        assignments = dict(sentinel.baseline_assignments)
        self.assertEqual("missing", assignments["target_object_state"])
        self.assertEqual("42P01", sentinel.expected_sqlstate)
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        # row_id (AMV-SFV|{row_id}|{consumer}), proving the canonical ledger
        ...
        self.assertEqual(53, len(sfv_cases))
