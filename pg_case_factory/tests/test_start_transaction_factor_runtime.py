...
from pg_case_factory.start_transaction_factor_runtime import (
    MAX_PARALLELISM,
    PG18_BIN,
    PG18_DATABASE,
    PG18_PORT,
...
class StartTransactionRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_start_transaction", PG18_DATABASE)
...
class StartTransactionRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1364_cases(self) -> None:
        sql_dir = Path("/tmp/start_transaction_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_start_transaction_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(1364, len(cases))
        self.assertEqual(1364, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_start_transaction_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1364, ordinals[-1])


class StartTransactionRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = StartTransactionPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(StartTransactionRuntimeError):
            StartTransactionPg18Runner(timeout_seconds=0)
        with self.assertRaises(StartTransactionRuntimeError):
            StartTransactionPg18Runner(timeout_seconds=301)
...
class StartTransactionComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "STARTTRANSACTION00001"
    ) -> StartTransactionCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = StartTransactionSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = StartTransactionSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_start_transaction_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = StartTransactionCaseRuntimeResult(
            case_id="STARTTRANSACTION00001",
            sql_filename="STARTTRANSACTION00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="25001",
            exit_code=0,
            timed_out=False,
            normalized_stdout=b"PGCF_TARGET_SQLSTATE=25001\n",
            normalized_stderr=b"",
            boolean_oracle_failure_count=0,
            pre_clean=True,
            post_clean=True,
