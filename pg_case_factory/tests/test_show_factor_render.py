"""Tests for show factor render module."""
...
class TestShowFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_show_factor_loop_plan(_REPO)
        self.extension = build_show_factor_extension_plan(
            _REPO
        )
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_show_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(5036, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [f"SHOW{i:05d}.sql" for i in range(1, 5037)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_show(sql))
    ...
    def test_primary_target_is_show(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
            ...
            self.assertRegex(region, r"(?im)^\s*SHOW\b")

    def test_col0_keyword_regex(self) -> None:
        rx = re.compile(r"(?m)^SHOW(?:\s|;|$)")
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
                "-- primary-target-begin", 1
    ...
    def test_lc_collate_renders_locale_target(self) -> None:
        case = next(
            c for c in self.baseline.cases
            if c.factor_key == "name_shape"
            and c.factor_value == "lc_collate"
        )
        sql = render_show_factor_case(case, _REPO)
        self.assertIn("SHOW LC_COLLATE;", sql)

    def test_lc_ctype_renders_locale_target(self) -> None:
        case = next(
            c for c in self.baseline.cases
            if c.factor_key == "name_shape"
            and c.factor_value == "lc_ctype"
        )
        sql = render_show_factor_case(case, _REPO)
        self.assertIn("SHOW LC_CTYPE;", sql)

    def test_pg_settings_probe_uses_name_setting(self) -> None:
        sql = (self.out / "SHOW00001.sql").read_text()
        self.assertIn("pg_catalog.pg_settings", sql)
        self.assertIn("name = 'work_mem'", sql)
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_show_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_show_factor_case(case, _REPO)
            b = render_show_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "SHOW00001.sql").read_text()
        self.assertIn("-- case_id: SHOW00001", sql)
