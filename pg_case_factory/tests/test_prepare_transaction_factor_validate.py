...
from pg_case_factory.prepare_transaction_factor_extension import (
    build_prepare_transaction_factor_extension_plan,
)
from pg_case_factory.prepare_transaction_factor_loop import (
    build_prepare_transaction_factor_loop_plan,
)
from pg_case_factory.prepare_transaction_factor_render import (
    generate_prepare_transaction_factor_programs,
    render_prepare_transaction_factor_case,
)
from pg_case_factory.validate_prepare_transaction import (
    PrepareTransactionFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_prepare_transaction_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class PrepareTransactionFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_prepare_transaction_factor_loop_plan(ROOT)
        cls.extension = build_prepare_transaction_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="preptran_validate_"))
        generate_prepare_transaction_factor_programs(
    ...
    def test_validation_passes_for_all_12284_files(self) -> None:
        result = validate_prepare_transaction_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, PrepareTransactionFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(44, result.baseline_case_count)
        self.assertEqual(12240, result.extension_case_count)
        self.assertEqual(12284, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(12284, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_prepare_transaction_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(12284, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_prepare_transaction_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_prepare_transaction_factor_case(case, ROOT)
        tampered = sql.replace("PREPARE TRANSACTION", "PREPARETRANSACTIONBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"PREPARETRANSACTION00001", "PREPARETRANSACTION00002"}
        result = validate_prepare_transaction_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
