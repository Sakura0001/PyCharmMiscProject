...
class CreateConversionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_conversion_factor_loop_obligations(ROOT)
        # GRM(2) + SFV(64) = 66 ; INV is not applicable
        # (CREATE CONVERSION targets pg_conversion, no table columns).
        self.assertEqual(66, len(rows))
        self.assertEqual(
            {"GRM": 2, "SFV": 64},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(66, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 42 SFV + 2 GRM = 44 are covered.
...
class CreateConversionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_conversion_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateConversionFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(66, len(plan.cases))
        self.assertEqual(
            list(range(1, 67)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATECONVERSION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATECONVERSION00066", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATECONVERSION00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATECONVERSION00066.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createconversion_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createconversion_00066_",
        ...
        self.assertEqual(
            66, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_conversion_factor_loop_plan(ROOT)
        again = build_create_conversion_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_conversion_factor_loop_plan(ROOT)
        self.assertEqual(
            "15b37346756fddcac5a99a23585dc79bd0b7a152d23600add1e21c8df12ed1a6",
            plan.obligation_multiset_sha256,
        )
