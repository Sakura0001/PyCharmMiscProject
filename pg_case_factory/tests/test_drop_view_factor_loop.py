...
_OBLIGATION_MULTISET_SHA256 = (
    "16c2732defafe595a6e5a72b3dd3020738cf06ee492e6da5b1f3289aef21ceb0"
)


class DropViewFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_view_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_view_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropViewFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_view_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropViewFactorLoopPlan)
        ...
        self.assertEqual("DROPVIEW00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPVIEW{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPVIEW00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPVIEW{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropview_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropview_{_OBLIGATION_COUNT:05d}_",
