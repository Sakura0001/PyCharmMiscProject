"""Tests for ALTER SYSTEM factor-loop actual-byte coverage validation."""
...
from pg_case_factory.alter_system_factor_validate import (
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        baseline_plan = build_alter_system_factor_loop_plan(ROOT)
        extension_plan = build_alter_system_factor_extension_plan(ROOT)
        programs: dict[str, str] = {}
        for case in baseline_plan.cases:
            programs[case.sql_filename] = (
                render_alter_system_factor_case(case, ROOT)
            )
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_system_factor_case(c, ROOT)
        for c in chosen
    }
...
class AlterSystemFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_system_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(report, AlterSystemFactorProgramValidation)
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_system_factor_case(target_case, ROOT)
    ...
    def test_byte_mismatch_is_detected(self) -> None:
        ...
        canonical = render_alter_system_factor_case(target_case, ROOT)
        tampered = canonical.replace(
            "ALTER SYSTEM", "ALTER  SYSTEM", 1
        )
