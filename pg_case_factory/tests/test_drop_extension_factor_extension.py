"""Tests for the bounded DROP EXTENSION post-coverage extension expander.

The marginal factor-value-loop (``drop_extension_factor_loop.py``) is the
...
from pg_case_factory.drop_extension_factor_extension import (
...
from pg_case_factory.drop_extension_factor_loop import (
...
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 396
_SUCCESS_EXTENSIONS = 162
_FAILURE_EXTENSIONS = 189
_EXTENSION_MULTISET_SHA256 = (
    "eb5bd383d397e6bcec37f91989e223f3cc08fd2f20edfa629635ae7bb5d5dc69"
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("extension_owner", "non_owner", "superuser"),
    "nonexistent_extension": (
        "extension_exists",
        "extension_missing_no_if_exists",
    ),
    ...
    "external_dependency_state": ("no_external_deps", "has_external_deps"),
    "verification_mode": (
...
_BRANCH_GRAMMAR = {"branch_drop_extension": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_extension": "drop_extension"}
...
class DropExtensionFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_extension_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropExtensionFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPEXTENSION00046", first.case_id)
        self.assertEqual(f"DROPEXTENSION{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPEXTENSION00046.sql", first.sql_filename)
        self.assertEqual(
            f"DROPEXTENSION{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropextension_00046_", first.object_prefix)
        self.assertEqual(
            f"dropextension_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropExtensionFactorExtensionCase)
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["external_dependency_state"] != "has_external_deps":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_extension_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["drop_dependent_objects"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_extension_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_extension_factor_loop_plan(ROOT)
