...
from pg_case_factory.drop_conversion_factor_validate import (
    DropConversionFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_conversion_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropConversionFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_conversion_validate_"))
        generate_drop_conversion_factor_programs(
    ...
    def test_validation_passes_for_all_524_files(self) -> None:
        result = validate_drop_conversion_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropConversionFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(38, result.baseline_case_count)
        self.assertEqual(486, result.extension_case_count)
        self.assertEqual(524, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(524, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_conversion_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(524, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_conversion_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPCONVERSION00001", "DROPCONVERSION00002"}
        result = validate_drop_conversion_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
