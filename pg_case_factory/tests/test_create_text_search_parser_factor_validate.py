"""Tests for create_text_search_parser factor validate module."""
...
from src.pg_case_factory.create_text_search_parser_factor_validate import (
...
class TestCreateTextSearchParserFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_create_text_search_parser_factor_loop_plan(
                _REPO
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_text_search_parser_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATETEXTSEARCHPARSER00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE TEXT SEARCH PARSER", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "CREATETEXTSEARCHPARSER00001",
            "CREATETEXTSEARCHPARSER00039",
        }
        report = validate_create_text_search_parser_factor_programs(
            self.baseline,
            self.extension,
