...
class CommitPreparedFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_commit_prepared_factor_loop_obligations(ROOT)
...
class CommitPreparedFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_commit_prepared_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CommitPreparedFactorLoopPlan)
        ...
        self.assertEqual(
            "COMMITPREPARED00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "COMMITPREPARED00044", plan.cases[-1].case_id
        )
        self.assertEqual(
            "COMMITPREPARED00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "COMMITPREPARED00044.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "commitprepared_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "commitprepared_00044_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_commit_prepared_factor_loop_plan(ROOT)
        obligations = compile_commit_prepared_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_commit_prepared_factor_loop_plan(ROOT)
        again = build_commit_prepared_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_commit_prepared_factor_loop_plan(ROOT)
        self.assertEqual(
            "ca7736b2a31b34fc3079a5c298ec8f6609b49a912d52febae2882342ca7905dc",
            plan.obligation_multiset_sha256,
        )
