"""Tests for the bounded DROP OPERATOR FAMILY post-coverage extension expander.

The marginal factor-value-loop (``drop_operator_family_factor_loop.py``) is the
...
failure-causing boundary per case, so attribution stays clean), with
...
_BASELINE_COUNT = 45
_EXTENSION_COUNT = 2184
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 2229
_SUCCESS_EXTENSIONS = 576
_FAILURE_EXTENSIONS = 1608
_EXTENSION_MULTISET_SHA256 = (
    "19877db90992875069c641155ca4b50c17e9153be41e3b5c64c9552ada7cc51f"
)

_PRIVILEGE_FAILURE_VALUES = frozenset(
    {"non_owner", "insufficient_privilege"}
)
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_operator_family"}
...
class DropOperatorFamilyFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_operator_family_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropOperatorFamilyFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPOPERATORFAMILY00046", first.case_id)
        self.assertEqual(f"DROPOPERATORFAMILY{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPOPERATORFAMILY00046.sql", first.sql_filename)
        self.assertEqual(
            f"DROPOPERATORFAMILY{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropoperatorfamily_00046_", first.object_prefix)
        self.assertEqual(
            f"dropoperatorfamily_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropOperatorFamilyFactorExtensionCase)
    ...
    def test_privilege_negative_fires_for_non_owner(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_context"] in _PRIVILEGE_FAILURE_VALUES:
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_object_lookup_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["privilege_context"] in _PRIVILEGE_FAILURE_VALUES:
                continue
            if (
                a["target_object_state"] == _MISSING
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if not _has_dependents_present(a):
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_operator_family_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_operator_family_factor_loop_plan(ROOT)
