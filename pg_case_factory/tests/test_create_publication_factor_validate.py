"""Tests for create_publication factor validate module."""
...
from src.pg_case_factory.create_publication_factor_validate import (
...
class TestCreatePublicationFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_publication_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(72, report.baseline_case_count)
        self.assertEqual(4230, report.extension_case_count)
        self.assertEqual(4302, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_publication_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4302, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_publication_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(4302, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATEPUBLICATION00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE PUBLICATION", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATEPUBLICATION00001", "CREATEPUBLICATION00073"}
        report = validate_create_publication_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_publication_actual_factor_witness_report", d["kind"]
        )
