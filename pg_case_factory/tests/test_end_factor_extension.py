...
class EndFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_end_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, EndFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual("END00045", plan.cases[0].case_id)
        self.assertEqual("END01844", plan.cases[-1].case_id)
        self.assertEqual(
            "END00045.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "END01844.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_end_factor_extension_plan(ROOT)
        plan_b = build_end_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_end_factor_extension_plan(ROOT)
        self.assertEqual(
            "0806859180b2790fb8e65562e8baf4462eaf9a4b752bcd5b60c205889cec08a4",
