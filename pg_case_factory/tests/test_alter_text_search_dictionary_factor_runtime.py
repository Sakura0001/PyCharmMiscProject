...
from pg_case_factory.alter_text_search_dictionary_factor_runtime import (
...
class AlterTextSearchDictionaryRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_alter_schema(self) -> None:
        self.assertNotEqual(PG18_PORT, 55497)
        self.assertNotEqual(
            PG18_DATABASE, "pgcf_aschema"
        )
        self.assertNotEqual(
            PG18_SOCKET.name,
            "pgcf-pg18-aschema-sock-20260820",
        )
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_atsd", PG18_DATABASE)
...
class AlterTextSearchDictionaryRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1384_cases(self) -> None:
        sql_dir = Path(
            "/tmp/alter_text_search_dictionary_probe_leaf"
        )
        ...
        self.assertEqual(1384, len(cases))
        self.assertEqual(1384, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        self.assertEqual(1384, ordinals[-1])


class AlterTextSearchDictionaryRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = AlterTextSearchDictionaryPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(
            ...
            AlterTextSearchDictionaryPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            AlterTextSearchDictionaryRuntimeError
        ):
            AlterTextSearchDictionaryPg18Runner(timeout_seconds=301)
...
class AlterTextSearchDictionaryComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ALTERTEXTSEARCHDICTIONARY00001"
    ) -> AlterTextSearchDictionaryCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = AlterTextSearchDictionarySuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = AlterTextSearchDictionarySuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_alter_text_search_dictionary_runs(
            run1, run2
        )
    ...
    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = AlterTextSearchDictionaryCaseRuntimeResult(
            case_id="ALTERTEXTSEARCHDICTIONARY00001",
            sql_filename="ALTERTEXTSEARCHDICTIONARY00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "ALTERTEXTSEARCHDICTIONARY00001",
