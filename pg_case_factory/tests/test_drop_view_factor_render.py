...
from pg_case_factory.drop_view_factor_render import (
...
class DropViewFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_view_factor_loop_plan(ROOT)
        cls.extension = build_drop_view_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_view_render_"))
        cls.count = generate_drop_view_factor_programs(
    ...
    def test_renders_exactly_3964_files(self) -> None:
        self.assertEqual(3964, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(3964, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPVIEW{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_view(text)
    ...
    def test_all_files_are_table_creating(self) -> None:
        """Every file must create a base table (bookend gate applies to all)."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertTrue(
                re.search(r"(?im)^\s*CREATE\s+TABLE\b", text),
                f"no CREATE TABLE in {f.name}",
            )
    ...
    def test_primary_target_is_drop_view(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+VIEW\b", region),
                f"no col-0 DROP VIEW in {f.name}",
            )
    ...
    def test_no_quoted_create_table(self) -> None:
        """CLASS-2 bug guard: zero CREATE TABLE with quoted names."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertEqual(
                0,
                len(re.findall(r'(?im)^\s*CREATE\s+TABLE\s+"', text)),
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_view_factor_witness(case, root)
            self.assertIsInstance(w, DropViewFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_view_factor_case(case, root)
        b = render_drop_view_factor_case(case, root)
