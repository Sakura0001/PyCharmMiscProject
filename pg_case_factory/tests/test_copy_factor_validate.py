...
from pg_case_factory.copy_factor_extension import (
    build_copy_factor_extension_plan,
)
from pg_case_factory.copy_factor_loop import (
    build_copy_factor_loop_plan,
)
from pg_case_factory.copy_factor_render import (
    generate_copy_factor_programs,
    render_copy_factor_case,
)
from pg_case_factory.copy_factor_validate import (
    CopyFactorProgramValidation,
...
class CopyFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_copy_factor_loop_plan(ROOT)
        cls.extension = build_copy_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="copy_validate_"))
        generate_copy_factor_programs(
    ...
    def test_validation_passes_for_all_5619_files(self) -> None:
        result = validate_copy_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, CopyFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(51, result.baseline_case_count)
        self.assertEqual(5568, result.extension_case_count)
        self.assertEqual(5619, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(5619, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_copy_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(5619, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_copy_factor_case(case, ROOT)
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_copy_factor_case(case, ROOT)
        tampered = sql.replace("COPY", "VACUUMBOGUS")
