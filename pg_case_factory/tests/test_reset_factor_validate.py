"""Tests for reset factor validate module."""
...
from src.pg_case_factory.validate_reset import (
...
class TestResetFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_reset_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "RESET00001.sql"
        original = self.programs[filename]
        tampered = original.replace("RESET work_mem;", "DROP TABLE")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"RESET00001", "RESET00043"}
        report = validate_reset_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "reset_actual_factor_witness_report",
            d["kind"],
        )
