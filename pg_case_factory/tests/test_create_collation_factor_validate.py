"""Tests for create_collation factor validate module."""
...
from src.pg_case_factory.create_collation_factor_validate import (
...
class TestCreateCollationFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_collation_factor_loop_plan(_REPO)
        self.extension = build_create_collation_factor_extension_plan(_REPO)
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_collation_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(1080, report.extension_case_count)
        self.assertEqual(1135, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_collation_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(1135, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_collation_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(1135, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATECOLLATION00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "CREATE COLLATION", "CREATE TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATECOLLATION00001", "CREATECOLLATION00056"}
        report = validate_create_collation_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_collation_actual_factor_witness_report", d["kind"]
        )
