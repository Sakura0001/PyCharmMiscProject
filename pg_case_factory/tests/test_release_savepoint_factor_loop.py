...
from pg_case_factory.release_savepoint_factor_loop import (
...
class ReleaseSavepointFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_release_savepoint_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(43) = 44 ; RELEASE SAVEPOINT has a single synopsis.
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # RELEASE SAVEPOINT: 6 canonical values reach the PG target check
    ...
    def test_chain_behavior_values_are_success_for_release_savepoint(
        self,
    ) -> None:
        # RELEASE SAVEPOINT mirrors COMMIT: and_chain / and_no_chain are
    ...
    def test_outside_transaction_is_failure_for_release_savepoint(
        self,
    ) -> None:
        # RELEASE SAVEPOINT outside an active transaction warns 25P01.
        self.assertIn(
...
class ReleaseSavepointFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_release_savepoint_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, ReleaseSavepointFactorLoopPlan)
        ...
        self.assertEqual(
            "RELEASESAVEPOINT00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "RELEASESAVEPOINT00044", plan.cases[-1].case_id
        )
        self.assertEqual(
            "RELEASESAVEPOINT00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "RELEASESAVEPOINT00044.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "releasesavepoint_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "releasesavepoint_00044_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_release_savepoint_factor_loop_plan(ROOT)
        obligations = (
            compile_release_savepoint_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_release_savepoint_factor_loop_plan(ROOT)
        again = build_release_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_release_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
            "d98022a29e18a39bb36ce7cf43929dee34ff67a6e97d06eab4969c548873503d",
            plan.obligation_multiset_sha256,
        )
