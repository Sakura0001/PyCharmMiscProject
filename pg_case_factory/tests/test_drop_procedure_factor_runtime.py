...
from pg_case_factory.drop_procedure_factor_runtime import (
...
class DropProcedureRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_call(self) -> None:
        ...
        self.assertNotEqual(PG18_SOCKET.name, "pgcf-pg18-call-sock-20260820")

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55502, PG18_PORT)
        self.assertEqual("pgcf_dp", PG18_DATABASE)
...
class DropProcedureRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_total_cases(self) -> None:
        sql_dir = Path("/tmp/drop_procedure_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_procedure_runtime_case_set(ROOT, sql_dir)
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_procedure_runtime_case_set(ROOT, sql_dir)
...
class DropProcedureRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropProcedurePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropProcedureRuntimeError):
            DropProcedurePg18Runner(timeout_seconds=0)
        with self.assertRaises(DropProcedureRuntimeError):
            DropProcedurePg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_proc(self) -> None:
        runner = DropProcedurePg18Runner()
        ...
        self.assertIn("proname", joined)


class DropProcedureComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPPROCEDURE00001"
    ) -> DropProcedureCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropProcedureSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropProcedureSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_procedure_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropProcedureCaseRuntimeResult(
            case_id="DROPPROCEDURE00001",
            sql_filename="DROPPROCEDURE00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="42809",
            exit_code=0,
            timed_out=False,
            normalized_stdout=b"PGCF_TARGET_SQLSTATE=42809\n",
            normalized_stderr=b"",
            boolean_oracle_failure_count=0,
            pre_clean=True,
            post_clean=True,
        ...
        self.assertIn("DROPPROCEDURE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropProcedureRuntimeError):
            DropProcedureSuiteRun(run_ordinal=3, cases=())
