"""Tests for discard factor_loop obligation ledger."""
...
class TestDiscardFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_discard_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "d52be9f01c4d2c4565e7927f8c9985e0a39f035f1454d4f6"
            "6cfbba8881e7615d",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_discard_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("discard_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("DISCARD"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_discard_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
