...
class CreateAggregateFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_aggregate_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateAggregateFactorExtensionPlan
        )
        self.assertEqual(18720, len(plan.cases))
        self.assertEqual(18720, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(70, 18790)), ordinals
        )
        self.assertEqual(
            "CREATEAGGREGATE00070", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEAGGREGATE18789", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEAGGREGATE00070.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEAGGREGATE18789.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(7020, outcomes["success"])
        self.assertEqual(11700, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_aggregate_factor_extension_plan(ROOT)
        plan_b = build_create_aggregate_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_aggregate_factor_extension_plan(ROOT)
        self.assertEqual(
            "24fd19da07a0ef1a92602f81f2da6a60363b4548ac8b5d18b1103b0b3bc43c68",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(69, _BASELINE_COUNT)
