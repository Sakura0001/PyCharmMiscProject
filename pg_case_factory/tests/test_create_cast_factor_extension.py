...
class CreateCastFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_cast_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateCastFactorExtensionPlan
        )
        self.assertEqual(19152, len(plan.cases))
        self.assertEqual(19152, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(64, 19216)), ordinals
        )
        self.assertEqual(
            "CREATECAST00064", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATECAST19215", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATECAST00064.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(8208, outcomes["success"])
        self.assertEqual(10944, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_cast_factor_extension_plan(ROOT)
        plan_b = build_create_cast_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_cast_factor_extension_plan(ROOT)
        self.assertEqual(
            "b8ca46a6d4e4c0655eb555dbeb15a40b8f2679da0948903d472cacbe3301eb0d",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(63, _BASELINE_COUNT)
        self.assertEqual(20000, _CAP)
        self.assertEqual(
            ("pg_cast_catalog_query", "actual_cast_execution"),
            _VERIFICATION_MODES,
        )
        self.assertEqual(
            ("DROP_CAST", "DROP_CAST_IF_EXISTS"), _CLEANUP_MODES
        )
