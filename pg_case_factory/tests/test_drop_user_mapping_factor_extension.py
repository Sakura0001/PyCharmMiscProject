"""Tests for the bounded DROP USER MAPPING post-coverage extension expander.

The marginal factor-value-loop (``drop_user_mapping_factor_loop.py``) is the
...
_BASELINE_COUNT = 46
_EXTENSION_COUNT = 1035
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 1081
_SUCCESS_EXTENSIONS = 270
_FAILURE_EXTENSIONS = 765
_EXTENSION_MULTISET_SHA256 = (
    "cc4a9943611b59ecc5b2081ad90763c8bdd7451762070d2dfa12dadaab428fcc"
)
...
_CROSSED_AXES = {
    ...
    "authorization_path": ("non_privileged", "server_owner", "user_with_usage"),
...
_BRANCH_GRAMMAR = {"branch_drop_user_mapping": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_user_mapping": "drop_user_mapping"}
...
class DropUserMappingFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_user_mapping_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropUserMappingFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPUSERMAPPING00047", first.case_id)
        self.assertEqual(f"DROPUSERMAPPING{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPUSERMAPPING00047.sql", first.sql_filename)
        self.assertEqual(
            f"DROPUSERMAPPING{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropusermapping_00047_", first.object_prefix)
        self.assertEqual(
            f"dropusermapping_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropUserMappingFactorExtensionCase)
    ...
    def test_privilege_negative_fires_for_non_privileged(self) -> None:
        for case in self.plan.cases:
    ...
    def test_server_missing_negative_fires(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["authorization_path"] == "non_privileged":
                continue
    ...
    def test_mapping_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["authorization_path"] == "non_privileged":
                continue
            if a["server_existence"] == "server_not_exists":
                continue
            if a["server_name_shape"] == "nonexistent_name":
                continue
            if (
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_user_mapping_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_user_mapping_factor_loop_plan(ROOT)
