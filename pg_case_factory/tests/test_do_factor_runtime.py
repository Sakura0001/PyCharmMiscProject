"""Tests for do factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.do_factor_runtime import (
    MAX_PARALLELISM,
...
class TestDoFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_do")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-do-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        from src.pg_case_factory.do_factor_extension import (
            build_do_factor_extension_plan,
        )
        from src.pg_case_factory.do_factor_loop import (
            build_do_factor_loop_plan,
        )
        from src.pg_case_factory.do_factor_render import (
            generate_do_factor_programs,
        )
        bp = build_do_factor_loop_plan(_REPO)
        ep = build_do_factor_extension_plan(_REPO)
        generate_do_factor_programs(bp, ep, sql_dir)
        cases, paths = (
            build_do_runtime_case_set(_REPO, sql_dir)
        )
        self.assertEqual(10786, len(cases))
        self.assertEqual(10786, len(paths))
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(DoRuntimeError):
            DoPg18Runner(timeout_seconds=0)
        with self.assertRaises(DoRuntimeError):
            DoPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DoRuntimeError):
            DoSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(DoRuntimeError):
            DoSuiteRun(run_ordinal=3, cases=())
