...
_OBLIGATION_MULTISET_SHA256 = (
    "07a30d4d178c0e1a816f30106b3cbc63b342526ec644c17d78e880509adea0c2"
)


class DropAccessMethodFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_access_method_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_access_method_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropAccessMethodFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_access_method_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropAccessMethodFactorLoopPlan)
        ...
        self.assertEqual("DROPACCESSMETHOD00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPACCESSMETHOD{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPACCESSMETHOD00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPACCESSMETHOD{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropaccessmethod_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropaccessmethod_{_OBLIGATION_COUNT:05d}_",
