...
class PrepareTransactionFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_prepare_transaction_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, PrepareTransactionFactorExtensionPlan
        )
        self.assertEqual(12240, len(plan.cases))
        self.assertEqual(12240, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(45, 12285)), ordinals
        )
        self.assertEqual(
            "PREPARETRANSACTION00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "PREPARETRANSACTION12284", plan.cases[-1].case_id
        )
        self.assertEqual(
            "PREPARETRANSACTION00045.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "PREPARETRANSACTION12284.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(4320, outcomes["success"])
        self.assertEqual(7920, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_prepare_transaction_factor_extension_plan(ROOT)
        plan_b = build_prepare_transaction_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_prepare_transaction_factor_extension_plan(ROOT)
        self.assertEqual(
            "1e177b20e4afd4808d15eb953936493b45a1f6d3fa323dd9c82fae7d648276fe",
