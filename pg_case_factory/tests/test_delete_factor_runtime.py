...
from pg_case_factory.delete_factor_runtime import (
...
class DeleteRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_delete", PG18_DATABASE)
...
class DeleteRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_17714_cases(self) -> None:
        sql_dir = Path("/tmp/delete_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_delete_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_delete_runtime_case_set(ROOT, sql_dir)
...
class DeleteRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DeletePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DeleteRuntimeError):
            DeletePg18Runner(timeout_seconds=0)
        with self.assertRaises(DeleteRuntimeError):
            DeletePg18Runner(timeout_seconds=301)
...
class DeleteComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DELETE00001"
    ) -> DeleteCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DeleteSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DeleteSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_delete_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DeleteCaseRuntimeResult(
            case_id="DELETE00001",
            sql_filename="DELETE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DELETE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DeleteRuntimeError):
            DeleteSuiteRun(run_ordinal=3, cases=())
