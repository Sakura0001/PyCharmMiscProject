"""Tests for the bounded DROP OPERATOR CLASS post-coverage extension expander.

The marginal factor-value-loop (``drop_operator_class_factor_loop.py``) is the
...
from pg_case_factory.drop_operator_class_factor_extension import (
...
from pg_case_factory.drop_operator_class_factor_loop import (
...
_BASELINE_COUNT = 46
_EXTENSION_COUNT = 888
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 934
_SUCCESS_EXTENSIONS = 336
_FAILURE_EXTENSIONS = 552
_EXTENSION_MULTISET_SHA256 = (
    "b4b36756b2f021849ed64f262bdc2604d0a209c0558628c559a76610c0759c4e"
)
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_operator_class"}
...
class DropOperatorClassFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_operator_class_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropOperatorClassFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPOPERATORCLASS00047", first.case_id)
        self.assertEqual(f"DROPOPERATORCLASS{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPOPERATORCLASS00047.sql", first.sql_filename)
        self.assertEqual(
            f"DROPOPERATORCLASS{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropoperatorclass_00047_", first.object_prefix)
        self.assertEqual(
            f"dropoperatorclass_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropOperatorClassFactorExtensionCase)
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["dependency_state"] != "has_dependents":
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_operator_class_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_operator_class_factor_loop_plan(ROOT)
