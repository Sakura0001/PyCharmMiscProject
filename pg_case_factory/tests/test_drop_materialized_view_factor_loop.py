...
# Frozen spec: GRM(1) + SFV(34) + RISK(2) = 37 ; INV is not applicable.
_OBLIGATION_COUNT = 37
_KIND_COUNTS = {"GRM": 1, "SFV": 34, "RISK": 2}
_FAILURE_COUNT = 7
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 30
_OBLIGATION_MULTISET_SHA256 = (
    "cdb2d41b989a93aeb7f7501b2990f61281e46ba0dd26d75dec94f46a19a0ec5d"
)


class DropMaterializedViewFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_materialized_view_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(34, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(34, sum(seen.values()))
        self.assertEqual(34, len(seen))
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_materialized_view_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropMaterializedViewFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_materialized_view_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropMaterializedViewFactorLoopPlan)
        ...
        self.assertEqual("DROPMATERIALIZEDVIEW00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPMATERIALIZEDVIEW{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPMATERIALIZEDVIEW00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPMATERIALIZEDVIEW{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropmaterializedview_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropmaterializedview_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(34, len(sfv_cases))
        for case in sfv_cases:
            self.assertRegex(
                case.primary_obligation_id,
                r"^DROPMATERIALIZEDVIEW-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
