...
from pg_case_factory.alter_user_factor_validate import (
    AlterUserFactorProgramValidation,
...
class AlterUserFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="ausr_validate_"))
        generate_alter_user_factor_programs(
    ...
    def test_validation_passes_for_all_3531_files(self) -> None:
        result = validate_alter_user_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterUserFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(75, result.baseline_case_count)
        self.assertEqual(3456, result.extension_case_count)
        self.assertEqual(3531, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(3531, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_user_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(3531, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_user_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ALTERUSER00001", "ALTERUSER00002"}
        result = validate_alter_user_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
