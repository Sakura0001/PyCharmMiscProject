...
from pg_case_factory.drop_function_factor_render import (
...
class DropFunctionFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_function_factor_loop_plan(ROOT)
        cls.extension = build_drop_function_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_function_render_"))
        cls.count = generate_drop_function_factor_programs(
    ...
    def test_renders_exactly_1389_files(self) -> None:
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPFUNCTION{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_function(text)
    ...
    def test_bookend_gate_table_creating_cases(self) -> None:
        ...
        for f in files:
            ...
            self.assertTrue(
                re.match(
                    r"(?i)^DROP\s+(TABLE|VIEW)\s+IF\s+EXISTS\b",
                    stmts[0],
                ),
                f"first stmt not DROP TABLE/VIEW in {f.name}: {stmts[0][:60]}",
            )
            self.assertTrue(
                re.match(
                    r"(?i)^DROP\s+(TABLE|VIEW)\s+IF\s+EXISTS\b",
                ...
                f"last stmt not DROP TABLE/VIEW in {f.name}: {stmts[-1][:60]}",
            )
    ...
    def test_primary_target_is_drop_function(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+FUNCTION\b", region),
                f"no col-0 DROP FUNCTION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_function_factor_witness(case, root)
            self.assertIsInstance(w, DropFunctionFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_function_factor_case(case, root)
        b = render_drop_function_factor_case(case, root)
