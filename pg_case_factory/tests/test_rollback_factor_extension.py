...
class RollbackFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_rollback_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, RollbackFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual("ROLLBACK00045", plan.cases[0].case_id)
        self.assertEqual("ROLLBACK01844", plan.cases[-1].case_id)
        self.assertEqual(
            "ROLLBACK00045.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "ROLLBACK01844.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_rollback_factor_extension_plan(ROOT)
        plan_b = build_rollback_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_rollback_factor_extension_plan(ROOT)
        self.assertEqual(
            "95ef98bc33f00c1e4cc18062654fc34d64a07e69a15ffa4edc0c575550995034",
