"""Tests for create_transform factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_transform_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateTransformFactorRuntime(unittest.TestCase):
    ...
    def test_constants_distinct_from_text_search_parser(self) -> None:
        self.assertNotEqual(PG18_PORT, 55614)
        self.assertNotEqual(PG18_DATABASE, "pgcf_ctsp")

    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55616)
        self.assertEqual(PG18_DATABASE, "pgcf_ctr")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-ctr-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        from src.pg_case_factory.create_transform_factor_extension import (
        ...
        from src.pg_case_factory.create_transform_factor_render import (
            generate_create_transform_factor_programs,
        )
        bp = build_create_transform_factor_loop_plan(
            _REPO
        )
        ep = build_create_transform_factor_extension_plan(
            _REPO
        )
        generate_create_transform_factor_programs(
            bp, ep, sql_dir
        ...
        self.assertEqual(1277, len(cases))
        self.assertEqual(1277, len(paths))
    ...
    def test_runner_construction(self) -> None:
        runner = CreateTransformPg18Runner()
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(
            ...
            CreateTransformPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            CreateTransformRuntimeError
        ):
            CreateTransformPg18Runner(timeout_seconds=301)
