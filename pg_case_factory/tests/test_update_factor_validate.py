...
from pg_case_factory.update_factor_extension import (
    build_update_factor_extension_plan,
)
from pg_case_factory.update_factor_loop import (
    build_update_factor_loop_plan,
)
from pg_case_factory.update_factor_render import (
    generate_update_factor_programs,
    render_update_factor_case,
)
from pg_case_factory.validate_update import (
    UpdateFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_update_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class UpdateFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_update_factor_loop_plan(ROOT)
        cls.extension = build_update_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="update_validate_"))
        generate_update_factor_programs(
    ...
    def test_validation_passes_for_all_17714_files(self) -> None:
        result = validate_update_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, UpdateFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_update_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_update_factor_case(case, ROOT)
        tampered = sql.replace("UPDATE", "UPDATEBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"UPDATE00001", "UPDATE00002"}
        result = validate_update_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
