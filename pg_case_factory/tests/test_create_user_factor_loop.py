"""Tests for create_user factor_loop obligation ledger."""
...
class TestCreateUserFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_user_factor_loop_plan(_REPO)
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 53}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "d7b4eb14e806b20128562d9d9ae1b6a8a7f0a3bab005de2fb5af873338ec5353",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_user_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 43, "expected_failure": 11}, dict(counts)
        )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_user_factor_loop_obligations(_REPO)
