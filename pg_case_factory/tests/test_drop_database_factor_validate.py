...
from pg_case_factory.drop_database_factor_render import (
    generate_drop_database_factor_programs,
    render_drop_database_factor_case,
)
from pg_case_factory.drop_database_factor_validate import (
    DropDatabaseFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_database_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropDatabaseFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_database_factor_loop_plan(ROOT)
        cls.extension = build_drop_database_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_database_validate_"))
        generate_drop_database_factor_programs(
    ...
    def test_validation_passes_for_all_482_files(self) -> None:
        result = validate_drop_database_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropDatabaseFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(50, result.baseline_case_count)
        self.assertEqual(432, result.extension_case_count)
        self.assertEqual(482, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(482, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_database_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(482, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_database_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPDATABASE00001", "DROPDATABASE00002"}
        result = validate_drop_database_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
