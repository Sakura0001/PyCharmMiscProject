...
from pg_case_factory.values_factor_runtime import (
    MAX_PARALLELISM,
    PG18_BIN,
    PG18_DATABASE,
    PG18_PORT,
...
class ValuesRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_select(self) -> None:
        self.assertNotEqual(PG18_PORT, 55622)
        self.assertNotEqual(PG18_DATABASE, "pgcf_select")

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55623, PG18_PORT)
        self.assertEqual("pgcf_values", PG18_DATABASE)
    ...
    def test_socket_dir_is_distinct(self) -> None:
        self.assertEqual(
...
class ValuesRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_17713_cases(self) -> None:
        sql_dir = Path("/tmp/values_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_values_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(17713, len(cases))
        self.assertEqual(17713, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_values_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(17713, ordinals[-1])


class ValuesRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = ValuesPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(ValuesRuntimeError):
            ValuesPg18Runner(timeout_seconds=0)
        with self.assertRaises(ValuesRuntimeError):
            ValuesPg18Runner(timeout_seconds=301)
...
class ValuesComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "VALUES00001"
    ) -> ValuesCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        result = compare_values_runs(run1, run2)
    ...
    def test_comparison_fails_on_sqlstate_mismatch(
        ...
        case = ValuesCaseRuntimeResult(
            case_id="VALUES00001",
            sql_filename="VALUES00001.sql",
            expected_sqlstate="00000",
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(ValuesRuntimeError):
            ValuesSuiteRun(run_ordinal=3, cases=())
