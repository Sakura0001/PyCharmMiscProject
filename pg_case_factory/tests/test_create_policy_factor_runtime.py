"""Tests for create_policy factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_policy_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreatePolicyFactorRuntime(unittest.TestCase):
    def test_constants_distinct_from_others(self) -> None:
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_cpol")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-cpol-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_policy_factor_loop_plan(_REPO)
        ep = build_create_policy_factor_extension_plan(_REPO)
        generate_create_policy_factor_programs(
            bp, ep, sql_dir
        )
        cases, paths = build_create_policy_runtime_case_set(
            _REPO, sql_dir
        )
        self.assertEqual(1017, len(cases))
        self.assertEqual(1017, len(paths))
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        cases, _ = build_create_policy_runtime_case_set(
            _REPO, sql_dir
        )
    ...
    def test_runner_construction(self) -> None:
        runner = CreatePolicyPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreatePolicyRuntimeError):
            CreatePolicyPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreatePolicyRuntimeError):
            CreatePolicyPg18Runner(timeout_seconds=301)
