...
class AlterTriggerFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_trigger_factor_loop_obligations(ROOT)
        # GRM(2) + SFV(41) = 43.
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 29 SFV + 2 GRM = 31 are covered.
        self.assertEqual(12, len(failures))
        self.assertEqual(31, covered)
        self.assertEqual(len(failures), len(set(failures)))
        for pair in failures:
            self.assertIn(pair, _SFV_FAILURE_SQLSTATE)


class AlterTriggerFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_trigger_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterTriggerFactorLoopPlan)
        ...
        self.assertEqual(
            "ALTERTRIGGER00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTRIGGER00043", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTRIGGER00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTRIGGER00043.sql",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_alter_trigger_factor_loop_plan(ROOT)
        obligations = compile_alter_trigger_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_trigger_factor_loop_plan(ROOT)
        again = build_alter_trigger_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_trigger_factor_loop_plan(ROOT)
        self.assertEqual(
            "b8e13df1da27d72a42c89106e7339573f894b3398a654116fdeefa889f41f5c2",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(31, outcomes["success"])
