...
_OBLIGATION_MULTISET_SHA256 = (
    "d2942cc625882bf317b72868d235bcbea9970d9ba86a473c33bfcc2712f68e63"
)


class DropOwnedFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_owned_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_owned_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropOwnedFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_owned_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropOwnedFactorLoopPlan)
        ...
        self.assertEqual("DROPOWNED00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPOWNED{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPOWNED00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPOWNED{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropowned_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropowned_{_OBLIGATION_COUNT:05d}_",
