...
class DeleteFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_delete_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, DeleteFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual("DELETE00051", plan.cases[0].case_id)
        self.assertEqual("DELETE17714", plan.cases[-1].case_id)
        self.assertEqual(
            "DELETE00051.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "DELETE17714.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_delete_factor_extension_plan(ROOT)
        plan_b = build_delete_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_delete_factor_extension_plan(ROOT)
        self.assertEqual(
            "e1d33038ec0595b67caef5ca93ff88d19d0e0154a94352ae3a181ab17b45cf49",
