...
_FAILURE_COUNT = 12
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 33
_OBLIGATION_MULTISET_SHA256 = (
    "a45b2c5a8efb5d869912cc03510bda90b634b6c80281a021401a7429902d2b99"
)


class DropFunctionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_function_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_function_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropFunctionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_function_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropFunctionFactorLoopPlan)
        ...
        self.assertEqual("DROPFUNCTION00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPFUNCTION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPFUNCTION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPFUNCTION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropfunction_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropfunction_{_OBLIGATION_COUNT:05d}_",
