...
class CreateTextSearchConfigurationFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_text_search_configuration_factor_loop_obligations(ROOT)
        # GRM(2) + SFV(52) = 54 ; RISK is not applicable
        # (CREATE TEXT SEARCH CONFIGURATION is non-transactional).
        self.assertEqual(54, len(rows))
        self.assertEqual(
            {"GRM": 2, "SFV": 52},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(54, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 35 SFV + 2 GRM = 37 are covered.
...
class CreateTextSearchConfigurationFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_text_search_configuration_factor_loop_plan(ROOT)
        self.assertIsInstance(
            plan, CreateTextSearchConfigurationFactorLoopPlan
        )
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(54, len(plan.cases))
        self.assertEqual(
            list(range(1, 55)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION00054", plan.cases[-1].case_id
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION00054.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createtextsearchconfiguration_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createtextsearchconfiguration_00054_",
        ...
        self.assertEqual(
            54, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_text_search_configuration_factor_loop_plan(ROOT)
        obligations = (
            compile_create_text_search_configuration_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_text_search_configuration_factor_loop_plan(ROOT)
        again = build_create_text_search_configuration_factor_loop_plan(ROOT)
        self.assertEqual(
