...
from pg_case_factory.alter_policy_factor_runtime import (
...
def _sample_case(stdout: bytes) -> AlterPolicyCaseRuntimeResult:
    return AlterPolicyCaseRuntimeResult(
        case_id="ALTERPOLICY00001",
        sql_filename="ALTERPOLICY00001.sql",
...
class AlterPolicyFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(
        self,
    ) -> None:
        identical = compare_alter_policy_runs(
            AlterPolicySuiteRun(
        ...
        changed = compare_alter_policy_runs(
            AlterPolicySuiteRun(
                run_ordinal=1, cases=(_sample_case(b"ok\n"),)
            ),
            AlterPolicySuiteRun(
    ...
    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        # The case-set combiner must merge the frozen 72-case baseline with
        # the bounded 13032-case extension into a single ordinal-sorted
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_alter_policy_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..13104 sequence",
            )
            self.assertEqual(1, ordinals[0])
            self.assertEqual(_TOTAL, ordinals[-1])
            # baseline (ordinal 1) is not an extension; last (13104) is.
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            comparison = compare_alter_policy_runs(run_01, run_02)
