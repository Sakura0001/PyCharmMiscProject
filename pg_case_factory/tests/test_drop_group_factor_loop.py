...
_OBLIGATION_MULTISET_SHA256 = (
    "4d28270f2170cfa4d50f1e1d98046ccb2bfe9dda6d129157489f1c4754df6c2d"
)


class DropGroupFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_group_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_group_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropGroupFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_group_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropGroupFactorLoopPlan)
        ...
        self.assertEqual("DROPGROUP00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPGROUP{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPGROUP00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPGROUP{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropgroup_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropgroup_{_OBLIGATION_COUNT:05d}_",
