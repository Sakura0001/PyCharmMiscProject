...
class PrepareTransactionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_prepare_transaction_factor_loop_obligations(ROOT)
...
class PrepareTransactionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_prepare_transaction_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, PrepareTransactionFactorLoopPlan)
        ...
        self.assertEqual(
            "PREPARETRANSACTION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "PREPARETRANSACTION00044", plan.cases[-1].case_id
        )
        self.assertEqual(
            "PREPARETRANSACTION00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "PREPARETRANSACTION00044.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "preparetransaction_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "preparetransaction_00044_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_prepare_transaction_factor_loop_plan(ROOT)
        obligations = compile_prepare_transaction_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_prepare_transaction_factor_loop_plan(ROOT)
        again = build_prepare_transaction_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_prepare_transaction_factor_loop_plan(ROOT)
        self.assertEqual(
            "7499373f36b4e5fce35df6add6f9a91a8b69629dd59f3d73d5946d47178a5838",
            plan.obligation_multiset_sha256,
        )
