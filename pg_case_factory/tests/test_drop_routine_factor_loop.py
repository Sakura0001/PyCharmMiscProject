...
# Frozen spec: GRM(1) + SFV(60) + RISK(2) = 63 ; INV is not applicable.
_OBLIGATION_COUNT = 63
_KIND_COUNTS = {"GRM": 1, "SFV": 60, "RISK": 2}
_FAILURE_COUNT = 14
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 49
_OBLIGATION_MULTISET_SHA256 = (
    "478e9c3aeb3f4946e9d0119a8946ffae81c81802f6558d4372550f8b28a02c3c"
)


class DropRoutineFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_routine_factor_loop_obligations(ROOT)
        self.assertEqual(_OBLIGATION_COUNT, len(rows))
        self.assertEqual(_KIND_COUNTS, Counter(row.kind for row in rows))
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        for row in sfv:
            seen[(row.factor_key, row.value)] = seen.get((row.factor_key, row.value), 0) + 1
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_routine_factor_loop_plan(ROOT)
        self.assertEqual(_OBLIGATION_MULTISET_SHA256, plan.obligation_multiset_sha256)


class DropRoutineFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_routine_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropRoutineFactorLoopPlan)
        ...
        self.assertEqual("DROPROUTINE00001", plan.cases[0].case_id)
        self.assertEqual(f"DROPROUTINE{_OBLIGATION_COUNT:05d}", plan.cases[-1].case_id)
        self.assertEqual("DROPROUTINE00001.sql", plan.cases[0].sql_filename)
        self.assertEqual(
            f"DROPROUTINE{_OBLIGATION_COUNT:05d}.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual("droproutine_00001_", plan.cases[0].object_prefix)
        self.assertEqual(
            f"droproutine_{_OBLIGATION_COUNT:05d}_", plan.cases[-1].object_prefix
        )
        self.assertEqual(_OBLIGATION_COUNT, len({row.primary_obligation_id for row in plan.cases}))
        self.assertEqual(_OBLIGATION_COUNT, len({row.sql_filename for row in plan.cases}))
        self.assertTrue(all(row.execution_profile == "serial_sql" for row in plan.cases))
    ...
    def test_baseline_has_one_primary_value_no_duplicates(self) -> None:
        ...
        for case in plan.cases:
            ...
            self.assertEqual(case.baseline_assignments, tuple(sorted(case.baseline_assignments)))

    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        plan = build_drop_routine_factor_loop_plan(ROOT)
        sfv_cases = [c for c in plan.cases if c.kind == "SFV"]
        self.assertEqual(60, len(sfv_cases))
