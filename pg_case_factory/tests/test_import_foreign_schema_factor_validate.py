"""Tests for import_foreign_schema factor validate module."""
...
from src.pg_case_factory.validate_import_foreign_schema import (
...
class TestImportForeignSchemaFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_import_foreign_schema_factor_loop_plan(_REPO)
        )
        self.extension = (
            build_import_foreign_schema_factor_extension_plan(
                _REPO
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_import_foreign_schema_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(57, report.baseline_case_count)
        self.assertEqual(576, report.extension_case_count)
        self.assertEqual(633, report.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(633, len(report.records))
    ...
    def test_sha256_dictionary_complete(self) -> None:
        ...
        self.assertEqual(633, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "IMPORTFOREIGNSCHEMA00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "IMPORT FOREIGN SCHEMA importforeignschema_00001_remote",
            "DROP TABLE importforeignschema_00001_remote",
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            "IMPORTFOREIGNSCHEMA00001",
            "IMPORTFOREIGNSCHEMA00058",
        }
        report = validate_import_foreign_schema_factor_programs(
            self.baseline,
            self.extension,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "import_foreign_schema_actual_factor_witness_report",
            d["kind"],
        )
