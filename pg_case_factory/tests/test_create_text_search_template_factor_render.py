"""Tests for create_text_search_template factor render module."""
...
class TestCreateTextSearchTemplateFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_create_text_search_template_factor_loop_plan(_REPO)
        )
        self.extension = (
            build_create_text_search_template_factor_extension_plan(
                _REPO
    ...
    def test_filename_numbering(self) -> None:
        ...
        self.assertEqual(
            [
                f"CREATETEXTSEARCHTEMPLATE{i:05d}.sql"
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(
                1, count_primary_create_text_search_template(sql)
            )
    ...
    def test_primary_target_is_create_text_search_template(
        self,
    ) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            ...
            self.assertRegex(
                region, r"(?im)CREATE\s+TEXT\s+SEARCH\s+TEMPLATE\b"
            )
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_text_search_template_factor_witness(
                case, _REPO
            )
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_text_search_template_factor_case(
                case, _REPO
            )
            b = render_create_text_search_template_factor_case(
                case, _REPO
            )
    ...
    def test_header_fields(self) -> None:
        sql = (
            self.out / "CREATETEXTSEARCHTEMPLATE00001.sql"
        ).read_text()
        self.assertIn(
            "-- case_id: CREATETEXTSEARCHTEMPLATE00001", sql
        )
