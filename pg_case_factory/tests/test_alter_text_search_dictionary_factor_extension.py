...
class AlterTextSearchDictionaryFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_text_search_dictionary_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTextSearchDictionaryFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(65, 1385)), ordinals
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00065", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY01384", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00065.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY01384.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(600, outcomes["success"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_text_search_dictionary_factor_extension_plan(ROOT)
        plan_b = build_alter_text_search_dictionary_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_text_search_dictionary_factor_extension_plan(ROOT)
        self.assertEqual(
            "bb1b6c87afca26408e4ddf15174417a8eb9e0f002964be0de9412f74d9292456",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(64, _BASELINE_COUNT)
