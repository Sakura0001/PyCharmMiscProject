"""Tests for listen factor validate module."""
...
from src.pg_case_factory.validate_listen import (
...
class TestListenFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_listen_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "LISTEN00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "LISTEN listen_00001_ch;", "DROP TABLE"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"LISTEN00001", "LISTEN00043"}
        report = validate_listen_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "listen_actual_factor_witness_report",
            d["kind"],
        )
