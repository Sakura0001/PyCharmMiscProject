...
from pg_case_factory.drop_procedure_factor_render import (
...
class DropProcedureFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_procedure_factor_loop_plan(ROOT)
        cls.extension = build_drop_procedure_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_procedure_render_"))
        cls.count = generate_drop_procedure_factor_programs(
    ...
    def test_renders_exactly_total_files(self) -> None:
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPPROCEDURE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        for f in sorted(self.tmp.glob("*.sql")):
            text = f.read_text()
            self.assertEqual(1, count_primary_drop_procedure(text), f.name)
    ...
    def test_primary_target_is_drop_procedure(self) -> None:
        for f in sorted(self.tmp.glob("*.sql")):
            text = f.read_text()
            region = self._extract_target_region(text)
            self.assertIsNotNone(region, f.name)
            self.assertTrue(
                re.search(r"(?m)^DROP\s+PROCEDURE\b", region),
                f"no col-0 DROP PROCEDURE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        for case in self.baseline.cases[:5]:
            w = resolve_drop_procedure_factor_witness(case, ROOT)
            self.assertIsInstance(w, DropProcedureFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        case = self.baseline.cases[0]
        self.assertEqual(
            render_drop_procedure_factor_case(case, ROOT),
            render_drop_procedure_factor_case(case, ROOT),
        )
