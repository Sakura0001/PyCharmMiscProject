...
from pg_case_factory.drop_sequence_factor_render import (
...
class DropSequenceFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_sequence_factor_loop_plan(ROOT)
        cls.extension = build_drop_sequence_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_sequence_render_"))
        cls.count = generate_drop_sequence_factor_programs(
    ...
    def test_renders_exactly_4414_files(self) -> None:
        self.assertEqual(4414, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(4414, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPSEQUENCE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_sequence(text)
    ...
    def test_primary_target_is_drop_sequence(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+SEQUENCE\b", region),
                f"no col-0 DROP SEQUENCE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_sequence_factor_witness(case, root)
            self.assertIsInstance(w, DropSequenceFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_sequence_factor_case(case, root)
        b = render_drop_sequence_factor_case(case, root)
    ...
    def test_byte_witness_round_trip_is_identical(self) -> None:
        """Re-render via witness resolver and assert byte-identical."""
        root = ROOT
        for case in self.baseline.cases[:10]:
            expected = render_drop_sequence_factor_case(case, root)
            witness = resolve_drop_sequence_factor_witness(case, root)
            self.assertIn(
    ...
    def test_probe_uses_pg_class_with_relkind_s(self) -> None:
        """Every catalog probe queries pg_class with relkind = 'S'."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            for m in re.finditer(
                r"FROM\s+pg_catalog\.pg_class\s+WHERE[^;]*",
                text,
            ):
                self.assertIn("relkind = 'S'", m.group(0), f.name)
