...
from pg_case_factory.alter_text_search_template_factor_extension import (
...
from pg_case_factory.alter_text_search_template_factor_render import (
    generate_alter_text_search_template_factor_programs,
    render_alter_text_search_template_factor_case,
)
from pg_case_factory.alter_text_search_template_factor_validate import (
...
class AlterTextSearchTemplateFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="atst_validate_")
        )
        generate_alter_text_search_template_factor_programs(
    ...
    def test_validation_passes_for_all_183_files(self) -> None:
        result = validate_alter_text_search_template_factor_programs(
        ...
        self.assertEqual(148, result.extension_case_count)
        self.assertEqual(183, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(183, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_text_search_template_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(183, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_text_search_template_factor_case(
            case, ROOT
        )
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
