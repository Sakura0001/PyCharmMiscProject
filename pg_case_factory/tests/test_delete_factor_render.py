...
class DeleteFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_delete_factor_loop_plan(ROOT)
        cls.extension = build_delete_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="delete_render_"))
        cls.count = generate_delete_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DELETE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_delete(text)
    ...
    def test_primary_target_is_delete(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DELETE\b", region),
                f"no col-0 DELETE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_delete_factor_witness(case, root)
            self.assertIsInstance(w, DeleteFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_delete_factor_case(case, root)
        b = render_delete_factor_case(case, root)
