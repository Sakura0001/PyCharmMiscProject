"""Tests for create_role factor extension plan."""
...
class TestCreateRoleFactorExtension(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_role_factor_extension_plan(_REPO)

    def test_frozen_extension_count(self) -> None:
        self.assertEqual(2160, len(self.plan.cases))
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 864, "expected_failure": 1296}, dict(counts)
        )
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        self.assertEqual(
            "7cf03dd0deaeb60ee3a0f048b79884c3ec640cd9e75e87c047ec490326f1c75e",
            self.plan.extension_multiset_sha256,
        )

    def test_constants(self) -> None:
        self.assertEqual(102, _BASELINE_COUNT)
    ...
    def test_derivation_id_prefix(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.derivation_id.startswith("CRP-EXT|"))
