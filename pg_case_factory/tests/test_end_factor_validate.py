...
from pg_case_factory.end_factor_extension import (
    build_end_factor_extension_plan,
)
from pg_case_factory.end_factor_loop import (
    build_end_factor_loop_plan,
)
from pg_case_factory.end_factor_render import (
    generate_end_factor_programs,
    render_end_factor_case,
)
from pg_case_factory.validate_end import (
    EndFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_end_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class EndFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_end_factor_loop_plan(ROOT)
        cls.extension = build_end_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="end_validate_"))
        generate_end_factor_programs(
    ...
    def test_validation_passes_for_all_1844_files(self) -> None:
        result = validate_end_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, EndFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_end_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_end_factor_case(case, ROOT)
        tampered = sql.replace("END", "ENDBOGUS", 1)
