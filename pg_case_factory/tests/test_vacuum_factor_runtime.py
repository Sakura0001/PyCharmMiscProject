...
from pg_case_factory.vacuum_factor_runtime import (
    MAX_PARALLELISM,
...
class VacuumRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_vacuum", PG18_DATABASE)
        self.assertEqual("pgcf_vacuum_superuser", PG18_SUPERUSER)
        self.assertEqual(1, MAX_PARALLELISM)
        self.assertEqual(30, PER_FILE_TIMEOUT_SECONDS)


class VacuumRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1415_cases(self) -> None:
        sql_dir = Path("/tmp/vacuum_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_vacuum_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(1415, len(cases))
        self.assertEqual(1415, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_vacuum_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1415, ordinals[-1])


class VacuumRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = VacuumPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(VacuumRuntimeError):
            VacuumPg18Runner(timeout_seconds=0)
        with self.assertRaises(VacuumRuntimeError):
            VacuumPg18Runner(timeout_seconds=301)
...
class VacuumComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "VACUUM00001"
    ) -> VacuumCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = VacuumSuiteRun(run_ordinal=1, cases=(case,))
        run2 = VacuumSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_vacuum_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = VacuumCaseRuntimeResult(
            case_id="VACUUM00001",
            sql_filename="VACUUM00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "VACUUM00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(VacuumRuntimeError):
            VacuumSuiteRun(run_ordinal=3, cases=())
