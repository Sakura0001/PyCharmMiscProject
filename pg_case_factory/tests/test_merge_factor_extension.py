...
class MergeFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_merge_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, MergeFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(53, 17717)), ordinals
        )
        self.assertEqual("MERGE00053", plan.cases[0].case_id)
        self.assertEqual("MERGE17716", plan.cases[-1].case_id)
        self.assertEqual(
            "MERGE00053.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "MERGE17716.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_merge_factor_extension_plan(ROOT)
        plan_b = build_merge_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_merge_factor_extension_plan(ROOT)
        self.assertEqual(
            "4c0f4e580bb65767e85c1346eff74d37e0d4e5028d56b482ae92f0b7f7e86b5a",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(52, _BASELINE_COUNT)
