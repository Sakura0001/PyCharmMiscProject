...
class ExplainFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_explain_factor_loop_obligations(ROOT)
...
class ExplainFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_explain_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, ExplainFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(48, len(plan.cases))
        self.assertEqual(
            list(range(1, 49)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("EXPLAIN00001", plan.cases[0].case_id)
        self.assertEqual("EXPLAIN00048", plan.cases[-1].case_id)
        self.assertEqual(
            "EXPLAIN00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "EXPLAIN00048.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual(
            "explain_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            48,
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_explain_factor_loop_plan(ROOT)
        again = build_explain_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_explain_factor_loop_plan(ROOT)
        self.assertEqual(
            "e0d95b2f044b48f2463728561896a68049938d7b32cb9442ca7d817c90f82dee",
            plan.obligation_multiset_sha256,
        )
