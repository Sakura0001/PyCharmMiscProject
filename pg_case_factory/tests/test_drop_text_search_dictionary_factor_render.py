...
from pg_case_factory.drop_text_search_dictionary_factor_render import (
...
class DropTextSearchDictionaryFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_drop_text_search_dictionary_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="drop_ts_dict_render_")
        )
        cls.count = generate_drop_text_search_dictionary_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"DROPTEXTSEARCHDICTIONARY{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_drop_text_search_dictionary(text)
    ...
    def test_primary_target_is_drop_ts_dict(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?m)^DROP\s+TEXT\s+SEARCH\s+DICTIONARY\b",
                    region,
                ),
                f"no col-0 DROP TEXT SEARCH DICTIONARY in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_text_search_dictionary_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, DropTextSearchDictionaryFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_drop_text_search_dictionary_factor_case(case, root)
        b = render_drop_text_search_dictionary_factor_case(case, root)
    ...
    def test_no_create_table_in_table_less_statement(self) -> None:
        """DROP TEXT SEARCH DICTIONARY is table-less: no CREATE TABLE."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
                bool(re.search(r"(?im)^\s*CREATE\s+TABLE\b", text)),
