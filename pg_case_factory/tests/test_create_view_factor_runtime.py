"""Tests for create_view factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_view_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateViewFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_cv")
        self.assertEqual(
            PG18_SOCKET,
            Path("/tmp/pgcf-pg18-cv-sock-20260820"),
        )
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_view_factor_loop_plan(_REPO)
        ep = build_create_view_factor_extension_plan(_REPO)
        generate_create_view_factor_programs(
            bp, ep, sql_dir
        )
        cases, paths = (
        ...
        self.assertEqual(5543, len(cases))
        self.assertEqual(5543, len(paths))
    ...
    def test_runner_construction(self) -> None:
        runner = CreateViewPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreateViewRuntimeError):
            CreateViewPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateViewRuntimeError):
            CreateViewPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateViewRuntimeError):
            CreateViewSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(CreateViewRuntimeError):
            CreateViewSuiteRun(run_ordinal=3, cases=())
