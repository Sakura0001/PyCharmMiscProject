...
from pg_case_factory.create_domain_factor_runtime import (
...
class CreateDomainRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55540, PG18_PORT)
        self.assertEqual("pgcf_cdom", PG18_DATABASE)
        ...
        self.assertEqual(
            "pgcf-pg18-cdom-sock-20260820",
            PG18_SOCKET.name,
        )


class CreateDomainRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_2977_cases(self) -> None:
        sql_dir = Path("/tmp/create_domain_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_create_domain_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(2977, len(cases))
        self.assertEqual(2977, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_create_domain_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(2977, ordinals[-1])


class CreateDomainRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CreateDomainPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CreateDomainRuntimeError):
            CreateDomainPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateDomainRuntimeError):
            CreateDomainPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_type_and_roles(self) -> None:
        runner = CreateDomainPg18Runner()
        ...
        self.assertIn("pg_collation", joined)


class CreateDomainComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "CREATEDOMAIN00001"
    ) -> CreateDomainCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CreateDomainSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = CreateDomainSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_create_domain_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CreateDomainCaseRuntimeResult(
            case_id="CREATEDOMAIN00001",
            sql_filename="CREATEDOMAIN00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "CREATEDOMAIN00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateDomainRuntimeError):
            CreateDomainSuiteRun(run_ordinal=3, cases=())
