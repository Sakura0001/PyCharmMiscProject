...
from pg_case_factory.drop_tablespace_factor_render import (
...
class DropTablespaceFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_tablespace_factor_loop_plan(ROOT)
        cls.extension = build_drop_tablespace_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_tablespace_render_"))
        cls.count = generate_drop_tablespace_factor_programs(
    ...
    def test_renders_exactly_8595_files(self) -> None:
        self.assertEqual(8595, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(8595, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPTABLESPACE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_tablespace(text)
            self.assertEqual(1, n, f.name)

    def test_statement_is_table_less(self) -> None:
        """DROP TABLESPACE never creates a TABLE (bookend gate N/A)."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_primary_target_is_drop_tablespace(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+TABLESPACE\b", region),
                f"no col-0 DROP TABLESPACE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_tablespace_factor_witness(case, root)
            self.assertIsInstance(w, DropTablespaceFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_tablespace_factor_case(case, root)
        b = render_drop_tablespace_factor_case(case, root)
    ...
    def test_probe_uses_real_spcname_column(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertIn("pg_catalog.pg_tablespace", text)
            self.assertIn("spcname", text)
