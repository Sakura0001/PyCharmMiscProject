...
from pg_case_factory.merge_factor_extension import (
    build_merge_factor_extension_plan,
)
from pg_case_factory.merge_factor_loop import (
    build_merge_factor_loop_plan,
)
from pg_case_factory.merge_factor_render import (
    generate_merge_factor_programs,
    render_merge_factor_case,
)
from pg_case_factory.validate_merge import (
    MergeFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_merge_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class MergeFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_merge_factor_loop_plan(ROOT)
        cls.extension = build_merge_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="merge_validate_"))
        generate_merge_factor_programs(
    ...
    def test_validation_passes_for_all_17716_files(self) -> None:
        result = validate_merge_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, MergeFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(52, result.baseline_case_count)
        self.assertEqual(17664, result.extension_case_count)
        self.assertEqual(17716, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(17716, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_merge_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(17716, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_merge_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_merge_factor_case(case, ROOT)
        tampered = sql.replace("MERGE", "MERGEBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"MERGE00001", "MERGE00002"}
        result = validate_merge_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
