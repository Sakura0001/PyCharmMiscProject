...
from pg_case_factory.alter_routine_factor_render import (
...
_TOTAL = _BASELINE_COUNT + _EXTENSION_COUNT  # 5555


class AlterRoutineFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_routine_factor_witness(case, ROOT)
                self.assertEqual(
                ...
                self.assertTrue(
                    witness.target_sql_fragment.startswith("ALTER ROUTINE")
                )
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_routine_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_routine(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_routine_factor_loop_plan(ROOT)
        first = [render_alter_routine_factor_case(row, ROOT) for row in plan.cases]
        second = [render_alter_routine_factor_case(row, ROOT) for row in plan.cases]
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(95, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(self) -> None:
        ...
        commit_sql = render_alter_routine_factor_case(rows["commit"], ROOT)
        rollback_sql = render_alter_routine_factor_case(rows["rollback"], ROOT)
    ...
    def test_header_carries_huawei_and_factor_metadata(self) -> None:
        ...
        self.assertIn("-- description  : ALTER ROUTINE", sql)
        self.assertIn("-- FE           : PG18-STATEMENT-FACTOR-LOOP", sql)

    def test_function_fixture_uses_create_function_with_returns(self) -> None:
        plan = build_alter_routine_factor_loop_plan(ROOT)
        func_case = next(
            c for c in plan.cases
            if c.factor_key == "routine_state"
            and c.factor_value == "exists_as_function"
        )
        sql = render_alter_routine_factor_case(func_case, ROOT)
        self.assertIn("CREATE FUNCTION", sql)
        self.assertIn("RETURNS", sql)

    def test_aggregate_fixture_creates_aggregate_for_42809(self) -> None:
        plan = build_alter_routine_factor_loop_plan(ROOT)
        agg_case = next(
            c for c in plan.cases
            if c.factor_key == "routine_state"
            and c.factor_value == "exists_as_aggregate"
        )
        sql = render_alter_routine_factor_case(agg_case, ROOT)
        self.assertIn("CREATE AGGREGATE", sql)

    def test_procedure_fixture_has_no_returns(self) -> None:
        plan = build_alter_routine_factor_loop_plan(ROOT)
        proc_case = next(
            c for c in plan.cases
            if c.factor_key == "routine_state"
            and c.factor_value == "exists_as_procedure"
        )
        sql = render_alter_routine_factor_case(proc_case, ROOT)
        self.assertIn("CREATE PROCEDURE", sql)
        self.assertNotIn("RETURNS", sql)


class AlterRoutineExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = build_alter_routine_factor_extension_plan(ROOT)

    def _representative_subset(self) -> list[AlterRoutineFactorExtensionCase]:
        seen: set[tuple[str, str]] = set()
        subset: list[AlterRoutineFactorExtensionCase] = []
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [render_alter_routine_factor_case(c, ROOT) for c in subset]
        second = [render_alter_routine_factor_case(c, ROOT) for c in subset]
        self.assertEqual(first, second)

    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_routine_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_routine"
            count = generate_alter_routine_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_BASELINE_COUNT + _EXTENSION_COUNT, count)
            files = sorted(out.glob("ALTERROUTINE*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_routine(b0))
            e0 = (out / self.extension_plan.cases[0].sql_filename).read_text(
                encoding="utf-8"
            )
            self.assertEqual(1, count_primary_alter_routine(e0))
