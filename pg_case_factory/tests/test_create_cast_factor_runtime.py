...
from pg_case_factory.create_cast_factor_runtime import (
...
class CreateCastRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_ccast", PG18_DATABASE)
...
class CreateCastRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_19215_cases(self) -> None:
        sql_dir = Path("/tmp/create_cast_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_create_cast_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(19215, len(cases))
        self.assertEqual(19215, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_create_cast_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(19215, ordinals[-1])


class CreateCastRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CreateCastPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CreateCastRuntimeError):
            CreateCastPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateCastRuntimeError):
            CreateCastPg18Runner(timeout_seconds=301)
...
class CreateCastComparisonTest(unittest.TestCase):
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CreateCastSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = CreateCastSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_create_cast_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CreateCastCaseRuntimeResult(
            case_id="CREATECAST00001",
            sql_filename="CREATECAST00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "CREATECAST00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateCastRuntimeError):
            CreateCastSuiteRun(run_ordinal=3, cases=())
