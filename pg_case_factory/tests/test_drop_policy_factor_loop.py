...
# Frozen spec: GRM(1) + SFV(45) + RISK(2) = 48 ; INV is not applicable.
_OBLIGATION_COUNT = 48
_KIND_COUNTS = {"GRM": 1, "SFV": 45, "RISK": 2}
_FAILURE_COUNT = 8
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 40
_OBLIGATION_MULTISET_SHA256 = (
    "d233fa4ccd963d6456a220034ea8c624daa91832db7b6cee637d0df404aa32e3"
)


class DropPolicyFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_policy_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_policy_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropPolicyFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_policy_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropPolicyFactorLoopPlan)
        ...
        self.assertEqual("DROPPOLICY00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPPOLICY{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPPOLICY00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPPOLICY{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droppolicy_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droppolicy_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(45, len(sfv_cases))
