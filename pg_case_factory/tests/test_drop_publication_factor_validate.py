...
from pg_case_factory.validate_drop_publication import (
    DropPublicationFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_publication_factor_programs,
)
...
class DropPublicationFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_publication_validate_"))
        generate_drop_publication_factor_programs(
    ...
    def test_validation_passes_for_all_2312_files(self) -> None:
        result = validate_drop_publication_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropPublicationFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(40, result.baseline_case_count)
        self.assertEqual(2272, result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_publication_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPPUBLICATION00001", "DROPPUBLICATION00002"}
        result = validate_drop_publication_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
