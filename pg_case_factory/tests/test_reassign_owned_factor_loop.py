...
# Frozen spec: GRM(1) + SFV(46) + RISK(2) = 49 ; INV is not applicable.
_OBLIGATION_COUNT = 49
_KIND_COUNTS = {"GRM": 1, "SFV": 46, "RISK": 2}
_FAILURE_COUNT = 4
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 45
_OBLIGATION_MULTISET_SHA256 = (
    "f9a72e9f00383b6fe22da3bd2a5e0351801853b67f620529b804fef445e09e3d"
)


class ReassignOwnedFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_reassign_owned_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(46, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(46, sum(seen.values()))
        self.assertEqual(46, len(seen))
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_reassign_owned_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class ReassignOwnedFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_reassign_owned_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, ReassignOwnedFactorLoopPlan)
        ...
        self.assertEqual("REASSIGNOWNED00001", plan.cases[0].case_id)
        self.assertEqual(
            f"REASSIGNOWNED{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "REASSIGNOWNED00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"REASSIGNOWNED{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "reassignowned_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"reassignowned_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(46, len(sfv_cases))
