"""Tests for the bounded DROP TEXT SEARCH DICTIONARY post-coverage extension expander.

The marginal factor-value-loop is the frozen 38-case baseline (GRM 1 +
SFV 35 + RISK 2).  This module's expander adds the bounded post-coverage
...
from pg_case_factory.drop_text_search_dictionary_factor_extension import (
...
from pg_case_factory.drop_text_search_dictionary_factor_loop import (
...
_EXTENSION_MULTISET_SHA256 = (
    "fc07fb733901cbfe77dc57e5d901949bbe7de45e8f9c67b208878ec65abd76d1"
)
...
_CROSSED_AXES = {
    ...
    "dependency_context": ("no_dependencies", "config_using_dict"),
    "dict_name_shape": (
        "simple_id",
        "schema_qualified_id",
        "quoted_id",
...
_BRANCH_GRAMMAR = {"branch_drop_ts_dict": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_ts_dict": "drop_text_search_dictionary"}
...
class DropTextSearchDictionaryFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_text_search_dictionary_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(
            self.plan, DropTextSearchDictionaryFactorExtensionPlan
        )
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTEXTSEARCHDICTIONARY00039", first.case_id)
        self.assertEqual(
            f"DROPTEXTSEARCHDICTIONARY{_TOTAL_COUNT:05d}", last.case_id
        )
        self.assertEqual(
            "DROPTEXTSEARCHDICTIONARY00039.sql", first.sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHDICTIONARY{_TOTAL_COUNT:05d}.sql",
            last.sql_filename,
        )
        self.assertEqual("droptextsearchdictionary_00039_", first.object_prefix)
        self.assertEqual(
            f"droptextsearchdictionary_{_TOTAL_COUNT:05d}_",
            last.object_prefix,
        )
    ...
    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(
                case, DropTextSearchDictionaryFactorExtensionCase
            )
        ...
        for case in self.plan.cases:
            ...
            if (
                (
                    ...
                    or a["dict_name_shape"] == "non_existent_name"
                )
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if a["authorization_path"] == "non_owner":
                continue
            if a["object_state"] == "absent":
                continue
            if a["dict_name_shape"] == "non_existent_name":
                continue
            if a["dependency_context"] != "config_using_dict":
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_text_search_dictionary_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_text_search_dictionary_factor_loop_plan(ROOT)
