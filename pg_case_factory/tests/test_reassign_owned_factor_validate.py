...
from pg_case_factory.validate_reassign_owned import (
    ReassignOwnedFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_reassign_owned_factor_programs,
)
...
class ReassignOwnedFactorValidateTest(unittest.TestCase):
    ...
    def test_validation_passes_for_all_4081_files(self) -> None:
        result = validate_reassign_owned_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, ReassignOwnedFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(49, result.baseline_case_count)
        self.assertEqual(4032, result.extension_case_count)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_reassign_owned_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_reassign_owned_factor_case(case, ROOT)
        tampered = sql.replace("REASSIGN", "REASSIGNBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"REASSIGNOWNED00001", "REASSIGNOWNED00002"}
        result = validate_reassign_owned_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
