...
from pg_case_factory.drop_text_search_template_factor_render import (
...
class DropTextSearchTemplateFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_text_search_template_factor_loop_plan(ROOT)
        cls.extension = build_drop_text_search_template_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_ts_template_render_"))
        cls.count = generate_drop_text_search_template_factor_programs(
    ...
    def test_renders_exactly_3564_files(self) -> None:
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPTEXTSEARCHTEMPLATE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_text_search_template(text)
            self.assertEqual(1, n, f.name)

    def test_bookend_gate_not_applicable_for_table_less_statement(self) -> None:
        """DROP TEXT SEARCH TEMPLATE has no tables; bookend gate is N/A."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertFalse(has_create_table, f.name)
    ...
    def test_primary_target_is_drop_text_search_template(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*DROP\s+TEXT\s+SEARCH\s+TEMPLATE\b", region),
                f"no col-0 DROP TEXT SEARCH TEMPLATE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_text_search_template_factor_witness(case, root)
            self.assertIsInstance(w, DropTextSearchTemplateFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_text_search_template_factor_case(case, root)
        b = render_drop_text_search_template_factor_case(case, root)
