...
from pg_case_factory.alter_routine_factor_runtime import (
...
def _sample_case(stdout: bytes) -> AlterRoutineCaseRuntimeResult:
    return AlterRoutineCaseRuntimeResult(
        case_id="ALTERROUTINE00001",
        sql_filename="ALTERROUTINE00001.sql",
...
class AlterRoutineFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(self) -> None:
        identical = compare_alter_routine_runs(
            AlterRoutineSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterRoutineSuiteRun(run_ordinal=2, cases=(_sample_case(b"ok\n"),)),
        )
        self.assertTrue(identical.passed, identical.issues)

        changed = compare_alter_routine_runs(
            AlterRoutineSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterRoutineSuiteRun(
                run_ordinal=2,
    ...
    def test_sqlstate_mismatch_is_detected(self) -> None:
        ...
        wrong_sqlstate = AlterRoutineCaseRuntimeResult(
            case_id="ALTERROUTINE00001",
            sql_filename="ALTERROUTINE00001.sql",
            expected_sqlstate="00000",
        ...
        comparison = compare_alter_routine_runs(
            AlterRoutineSuiteRun(
    ...
    def test_cleanup_failure_is_detected(self) -> None:
        ...
        dirty = AlterRoutineCaseRuntimeResult(
            case_id="ALTERROUTINE00001",
            sql_filename="ALTERROUTINE00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="00000",
        ...
        comparison = compare_alter_routine_runs(
            AlterRoutineSuiteRun(run_ordinal=1, cases=(clean,)),
            AlterRoutineSuiteRun(run_ordinal=2, cases=(dirty,)),
        )
        self.assertFalse(comparison.passed)
        self.assertEqual(
            ("ALTERROUTINE00001",),
    ...
    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_alter_routine_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..5555 sequence",
            )
    ...
    def test_runner_timeout_bounds_are_enforced(self) -> None:
        with self.assertRaises(Exception):
            AlterRoutinePg18Runner(timeout_seconds=0)
        with self.assertRaises(Exception):
            AlterRoutinePg18Runner(timeout_seconds=301)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(Exception):
            AlterRoutineSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(Exception):
            AlterRoutineSuiteRun(run_ordinal=3, cases=())
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        selected = (
            ...
            one(
                lambda case: case.factor_key == "routine_state"
                and case.factor_value == "non_existent"
            ),
            one(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for case in selected:
                ...
                path.write_text(
                    render_alter_routine_factor_case(case, ROOT),
                    encoding="utf-8",
                )
                paths[case.case_id] = path

            runner = AlterRoutinePg18Runner()
            ...
            comparison = compare_alter_routine_runs(run_01, run_02)
