...
from pg_case_factory.create_foreign_table_factor_validate import (
...
class CreateForeignTableFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="cft_validate_"))
        generate_create_foreign_table_factor_programs(
    ...
    def test_validation_passes_for_all_2556_files(self) -> None:
        result = validate_create_foreign_table_factor_programs(
        ...
        self.assertEqual(96, result.baseline_case_count)
        self.assertEqual(2460, result.extension_case_count)
        self.assertEqual(2556, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(2556, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_foreign_table_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(2556, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_foreign_table_factor_case(
            case, ROOT
        )
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE FOREIGN TABLE", "CREATE BOGUS TABLE"
        )
