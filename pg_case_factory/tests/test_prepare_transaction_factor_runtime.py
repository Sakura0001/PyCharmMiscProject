...
from pg_case_factory.prepare_transaction_factor_runtime import (
...
class PrepareTransactionRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_distinct_from_commit_prepared(self) -> None:
        # No port/socket/db collision with the commit_prepared ledger.
        self.assertNotEqual(PG18_PORT, 55500)
        self.assertNotEqual(PG18_DATABASE, "pgcf_cprep")
        self.assertNotEqual(
            PG18_SOCKET.name, "pgcf-pg18-cprep-sock-20260820"
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55501, PG18_PORT)
        self.assertEqual("pgcf_preptran", PG18_DATABASE)
...
class PrepareTransactionRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_12284_cases(self) -> None:
        sql_dir = Path("/tmp/prepare_transaction_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_prepare_transaction_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_prepare_transaction_runtime_case_set(
            ROOT, sql_dir
        )
...
class PrepareTransactionRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = PrepareTransactionPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(PrepareTransactionRuntimeError):
            PrepareTransactionPg18Runner(timeout_seconds=0)
        with self.assertRaises(PrepareTransactionRuntimeError):
            PrepareTransactionPg18Runner(timeout_seconds=301)
...
class PrepareTransactionComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "PREPARETRANSACTION00001"
    ) -> PrepareTransactionCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = PrepareTransactionSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = PrepareTransactionSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_prepare_transaction_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = PrepareTransactionCaseRuntimeResult(
            case_id="PREPARETRANSACTION00001",
            sql_filename="PREPARETRANSACTION00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "PREPARETRANSACTION00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(PrepareTransactionRuntimeError):
            PrepareTransactionSuiteRun(run_ordinal=3, cases=())
