...
from pg_case_factory.release_savepoint_factor_extension import (
...
from pg_case_factory.release_savepoint_factor_render import (
...
class ReleaseSavepointFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_release_savepoint_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="release_savepoint_render_")
        )
        cls.count = generate_release_savepoint_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"RELEASESAVEPOINT{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_release_savepoint(text)
    ...
    def test_primary_target_is_release_savepoint(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*RELEASE\s+SAVEPOINT\b", region
                ),
                f"no RELEASE SAVEPOINT in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_release_savepoint_factor_witness(case, root)
            self.assertIsInstance(w, ReleaseSavepointFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_release_savepoint_factor_case(case, root)
        b = render_release_savepoint_factor_case(case, root)
