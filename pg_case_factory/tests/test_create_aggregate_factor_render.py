...
from pg_case_factory.create_aggregate_factor_extension import (
...
from pg_case_factory.create_aggregate_factor_render import (
...
class CreateAggregateFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_aggregate_factor_loop_plan(ROOT)
        cls.extension = build_create_aggregate_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="cagg_render_"))
        cls.count = generate_create_aggregate_factor_programs(
    ...
    def test_renders_exactly_18789_files(self) -> None:
        self.assertEqual(18789, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(18789, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATEAGGREGATE{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_aggregate(text)
    ...
    def test_no_quoted_table_identifiers_in_from_join(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
                re.search(r'(?im)^\s*FROM\s+"', text),
                f"quoted FROM in {f.name}",
            )
            self.assertFalse(
                re.search(r'(?im)\bJOIN\s+"', text),
                f"quoted JOIN in {f.name}",
            )

    def test_primary_target_is_create_aggregate(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*CREATE\s+(?:OR\s+REPLACE\s+)?AGGREGATE\b",
                    region,
                ),
                f"no CREATE AGGREGATE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_create_aggregate_factor_witness(case, root)
            self.assertIsInstance(
                w, CreateAggregateFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_aggregate_factor_case(case, root)
        b = render_create_aggregate_factor_case(case, root)
