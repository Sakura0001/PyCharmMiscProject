...
from pg_case_factory.alter_text_search_parser_factor_loop import (
...
class AlterTextSearchParserFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_text_search_parser_factor_loop_obligations(ROOT)
        # GRM(2) + SFV(33) = 35 ; RISK is not applicable
        # (ALTER TEXT SEARCH PARSER has no transaction_outcome factor).
        self.assertEqual(35, len(rows))
        self.assertEqual(
            {"GRM": 2, "SFV": 33},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(35, len({row.obligation_id for row in rows}))
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(33, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(33, sum(seen.values()))
        self.assertEqual(33, len(seen))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 22 SFV + 2 GRM = 24 are covered.
...
class AlterTextSearchParserFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_text_search_parser_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, AlterTextSearchParserFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(35, len(plan.cases))
        self.assertEqual(
            list(range(1, 36)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00035", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00035.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "alter_text_search_parser_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "alter_text_search_parser_00035_",
        ...
        self.assertEqual(
            35, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_alter_text_search_parser_factor_loop_plan(ROOT)
        obligations = (
            compile_alter_text_search_parser_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_text_search_parser_factor_loop_plan(ROOT)
        again = build_alter_text_search_parser_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_text_search_parser_factor_loop_plan(ROOT)
        self.assertEqual(
            "f19b91ea3f0e9efeb35434dfe7775add557a4ee561aaf7c479bf185c7c3be034",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(24, outcomes["success"])
