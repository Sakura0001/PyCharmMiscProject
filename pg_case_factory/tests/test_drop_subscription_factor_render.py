...
from pg_case_factory.drop_subscription_factor_render import (
...
class DropSubscriptionFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_subscription_factor_loop_plan(ROOT)
        cls.extension = build_drop_subscription_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_sub_render_"))
        cls.count = generate_drop_subscription_factor_programs(
    ...
    def test_renders_exactly_1188_files(self) -> None:
        self.assertEqual(1188, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1188, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPSUBSCRIPTION{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_subscription(text)
    ...
    def test_bookend_gate_table_creating_cases(self) -> None:
        ...
        for f in files:
            ...
            self.assertTrue(
                re.match(r"(?i)^DROP\s+TABLE\s+IF\s+EXISTS\b", stmts[0]),
                f"first stmt not DROP TABLE in {f.name}",
            )
            self.assertTrue(
                re.match(r"(?i)^DROP\s+TABLE\s+IF\s+EXISTS\b", stmts[-1]),
    ...
    def test_primary_target_is_drop_subscription(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+SUBSCRIPTION\b", region),
                f"no col-0 DROP SUBSCRIPTION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_subscription_factor_witness(case, root)
            self.assertIsInstance(w, DropSubscriptionFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_subscription_factor_case(case, root)
        b = render_drop_subscription_factor_case(case, root)
    ...
    def test_probe_uses_subname_column(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            if "pg_catalog.pg_subscription" in text:
                self.assertIn("subname", text, f.name)
