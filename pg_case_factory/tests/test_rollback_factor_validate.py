...
from pg_case_factory.rollback_factor_extension import (
    build_rollback_factor_extension_plan,
)
from pg_case_factory.rollback_factor_loop import (
    build_rollback_factor_loop_plan,
)
from pg_case_factory.rollback_factor_render import (
    generate_rollback_factor_programs,
    render_rollback_factor_case,
)
from pg_case_factory.validate_rollback import (
    RollbackFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_rollback_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class RollbackFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_rollback_factor_loop_plan(ROOT)
        cls.extension = build_rollback_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="rollback_validate_"))
        generate_rollback_factor_programs(
    ...
    def test_validation_passes_for_all_1844_files(self) -> None:
        result = validate_rollback_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, RollbackFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_rollback_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_rollback_factor_case(case, ROOT)
        tampered = sql.replace("ROLLBACK", "ROLLBACKBOGUS", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ROLLBACK00001", "ROLLBACK00002"}
        result = validate_rollback_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
