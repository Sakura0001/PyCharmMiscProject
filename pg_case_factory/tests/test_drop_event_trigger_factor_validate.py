...
from pg_case_factory.drop_event_trigger_factor_validate import (
    DropEventTriggerFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_event_trigger_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropEventTriggerFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_event_trigger_validate_"))
        generate_drop_event_trigger_factor_programs(
    ...
    def test_validation_passes_for_all_542_files(self) -> None:
        result = validate_drop_event_trigger_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropEventTriggerFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(38, result.baseline_case_count)
        self.assertEqual(504, result.extension_case_count)
        self.assertEqual(542, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(542, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_event_trigger_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(542, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_event_trigger_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPEVENTTRIGGER00001", "DROPEVENTTRIGGER00002"}
        result = validate_drop_event_trigger_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
