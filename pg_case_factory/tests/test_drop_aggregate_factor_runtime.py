...
from pg_case_factory.drop_aggregate_factor_runtime import (
...
class DropAggregateRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_da", PG18_DATABASE)
...
class DropAggregateRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_376_cases(self) -> None:
        sql_dir = Path("/tmp/drop_aggregate_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_aggregate_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(376, len(cases))
        self.assertEqual(376, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_aggregate_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(376, ordinals[-1])


class DropAggregateRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropAggregatePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropAggregateRuntimeError):
            DropAggregatePg18Runner(timeout_seconds=0)
        with self.assertRaises(DropAggregateRuntimeError):
            DropAggregatePg18Runner(timeout_seconds=301)
...
class DropAggregateComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPAGGREGATE00001"
    ) -> DropAggregateCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropAggregateSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropAggregateSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_aggregate_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropAggregateCaseRuntimeResult(
            case_id="DROPAGGREGATE00001",
            sql_filename="DROPAGGREGATE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPAGGREGATE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropAggregateRuntimeError):
            DropAggregateSuiteRun(run_ordinal=3, cases=())
