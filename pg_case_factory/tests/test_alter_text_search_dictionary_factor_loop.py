...
from pg_case_factory.alter_text_search_dictionary_factor_loop import (
...
class AlterTextSearchDictionaryFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_alter_text_search_dictionary_factor_loop_obligations(ROOT)
        # GRM(4) + SFV(60) = 64 ; RISK is not applicable.
        self.assertEqual(64, len(rows))
        self.assertEqual(
            {"GRM": 4, "SFV": 60},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(64, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 44 SFV + 4 GRM = 48 are covered.
...
class AlterTextSearchDictionaryFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_alter_text_search_dictionary_factor_loop_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTextSearchDictionaryFactorLoopPlan
        )
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(64, len(plan.cases))
        self.assertEqual(
            list(range(1, 65)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00064", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHDICTIONARY00064.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "alter_text_search_dictionary_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "alter_text_search_dictionary_00064_",
        ...
        self.assertEqual(
            64, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_alter_text_search_dictionary_factor_loop_plan(ROOT)
        again = build_alter_text_search_dictionary_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_alter_text_search_dictionary_factor_loop_plan(ROOT)
        self.assertEqual(
            "13a3aed3f97e54419b9b4975329bc78a6dfebe3800617956874e252d418fc820",
            plan.obligation_multiset_sha256,
        )
