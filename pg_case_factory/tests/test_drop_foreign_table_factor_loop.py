...
_OBLIGATION_MULTISET_SHA256 = (
    "7730916f366e5fe72a6fdab8fe63bfd485284aaf2c932478d28a2e3faa67fe5e"
)


class DropForeignTableFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_foreign_table_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_foreign_table_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropForeignTableFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_foreign_table_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropForeignTableFactorLoopPlan)
        ...
        self.assertEqual("DROPFOREIGNTABLE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPFOREIGNTABLE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPFOREIGNTABLE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPFOREIGNTABLE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropforeigntable_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropforeigntable_{_OBLIGATION_COUNT:05d}_",
