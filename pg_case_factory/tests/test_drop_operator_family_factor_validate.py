...
from pg_case_factory.validate_drop_operator_family import (
    DropOperatorFamilyFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_operator_family_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropOperatorFamilyFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="drop_operator_family_validate_")
        )
        generate_drop_operator_family_factor_programs(
    ...
    def test_validation_passes_for_all_2229_files(self) -> None:
        result = validate_drop_operator_family_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, DropOperatorFamilyFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(45, result.baseline_case_count)
        self.assertEqual(2184, result.extension_case_count)
        self.assertEqual(2229, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(2229, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_drop_operator_family_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(2229, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_operator_family_factor_case(case, ROOT)
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPOPERATORFAMILY00001", "DROPOPERATORFAMILY00002"}
        result = validate_drop_operator_family_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
