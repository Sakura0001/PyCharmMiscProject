...
from pg_case_factory.create_aggregate_factor_validate import (
    CreateAggregateFactorProgramValidation,
...
class CreateAggregateFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="cagg_validate_"))
        generate_create_aggregate_factor_programs(
    ...
    def test_validation_passes_for_all_files(self) -> None:
        ...
        self.assertIsInstance(
            result, CreateAggregateFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(69, result.baseline_case_count)
        self.assertEqual(18720, result.extension_case_count)
        self.assertEqual(18789, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        result = self.full_result
        self.assertEqual(18789, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = self.full_result
        self.assertEqual(18789, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_aggregate_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE AGGREGATE", "CREATX AGGREGATE"
        )
        programs = {case.sql_filename: tampered}
        result = validate_create_aggregate_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATEAGGREGATE00001", "CREATEAGGREGATE00002"}
        result = validate_create_aggregate_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
