...
from pg_case_factory.alter_procedure_factor_runtime import (
...
def _sample_case(stdout: bytes) -> AlterProcedureCaseRuntimeResult:
    return AlterProcedureCaseRuntimeResult(
        case_id="ALTERPROCEDURE00001",
        sql_filename="ALTERPROCEDURE00001.sql",
...
class AlterProcedureFactorRuntimeTest(unittest.TestCase):
    def test_two_run_result_requires_identical_normalized_transcripts(self) -> None:
        identical = compare_alter_procedure_runs(
            AlterProcedureSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterProcedureSuiteRun(run_ordinal=2, cases=(_sample_case(b"ok\n"),)),
        )
        self.assertTrue(identical.passed, identical.issues)

        changed = compare_alter_procedure_runs(
            AlterProcedureSuiteRun(run_ordinal=1, cases=(_sample_case(b"ok\n"),)),
            AlterProcedureSuiteRun(
                run_ordinal=2,
    ...
    def test_sqlstate_mismatch_is_detected(self) -> None:
        ...
        wrong_sqlstate = AlterProcedureCaseRuntimeResult(
            case_id="ALTERPROCEDURE00001",
            sql_filename="ALTERPROCEDURE00001.sql",
            expected_sqlstate="00000",
        ...
        comparison = compare_alter_procedure_runs(
            AlterProcedureSuiteRun(
    ...
    def test_cleanup_failure_is_detected(self) -> None:
        ...
        dirty = AlterProcedureCaseRuntimeResult(
            case_id="ALTERPROCEDURE00001",
            sql_filename="ALTERPROCEDURE00001.sql",
            expected_sqlstate="00000",
            target_sqlstate="00000",
        ...
        comparison = compare_alter_procedure_runs(
            AlterProcedureSuiteRun(run_ordinal=1, cases=(clean,)),
            AlterProcedureSuiteRun(run_ordinal=2, cases=(dirty,)),
        )
        self.assertFalse(comparison.passed)
        self.assertEqual(
            ("ALTERPROCEDURE00001",),
    ...
    def test_runtime_case_set_combines_baseline_and_extension(self) -> None:
        # The case-set combiner must merge the frozen 94-case baseline with
        # the bounded 13,104-case extension into a single ordinal-sorted
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cases, sql_paths = build_alter_procedure_runtime_case_set(
                ROOT, sql_dir
            )
            ...
            self.assertEqual(
                ...
                ordinals,
                "ordinals must form a contiguous 1..13198 sequence",
            )
    ...
    def test_runner_timeout_bounds_are_enforced(self) -> None:
        with self.assertRaises(Exception):
            AlterProcedurePg18Runner(timeout_seconds=0)
        with self.assertRaises(Exception):
            AlterProcedurePg18Runner(timeout_seconds=301)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(Exception):
            AlterProcedureSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(Exception):
            AlterProcedureSuiteRun(run_ordinal=3, cases=())
    ...
    def test_six_representative_cases_execute_twice_on_pg18_4(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for case in selected:
                ...
                path.write_text(
                    render_alter_procedure_factor_case(case, ROOT),
                    encoding="utf-8",
                )
                paths[case.case_id] = path

            runner = AlterProcedurePg18Runner()
            ...
            comparison = compare_alter_procedure_runs(run_01, run_02)
