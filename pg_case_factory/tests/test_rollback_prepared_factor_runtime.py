...
from pg_case_factory.rollback_prepared_factor_runtime import (
...
class RollbackPreparedRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_distinct_from_commit_prepared(self) -> None:
        # No collision with the commit_prepared ledger.
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_rprep", PG18_DATABASE)
...
class RollbackPreparedRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_12284_cases(self) -> None:
        sql_dir = Path("/tmp/rollback_prepared_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_rollback_prepared_runtime_case_set(
            ROOT, sql_dir
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_rollback_prepared_runtime_case_set(
            ROOT, sql_dir
        )
...
class RollbackPreparedRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = RollbackPreparedPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(RollbackPreparedRuntimeError):
            RollbackPreparedPg18Runner(timeout_seconds=0)
        with self.assertRaises(RollbackPreparedRuntimeError):
            RollbackPreparedPg18Runner(timeout_seconds=301)
...
class RollbackPreparedComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ROLLBACKPREPARED00001"
    ) -> RollbackPreparedCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = RollbackPreparedSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = RollbackPreparedSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_rollback_prepared_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = RollbackPreparedCaseRuntimeResult(
            case_id="ROLLBACKPREPARED00001",
            sql_filename="ROLLBACKPREPARED00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "ROLLBACKPREPARED00001", result.sqlstate_mismatch_case_ids
        )

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(RollbackPreparedRuntimeError):
            RollbackPreparedSuiteRun(run_ordinal=3, cases=())
