...
# Frozen spec: GRM(1) + SFV(35) + RISK(2) = 38 ; INV is not applicable.
_OBLIGATION_COUNT = 38
_KIND_COUNTS = {"GRM": 1, "SFV": 35, "RISK": 2}
_FAILURE_COUNT = 10
_COVERED_COUNT = _OBLIGATION_COUNT - _FAILURE_COUNT  # 28
_OBLIGATION_MULTISET_SHA256 = (
    "2ae7f25deed5a94cc92c74b1d6a156c51c3b967df1ec20b48c77a252d01a6e0d"
)


class DropTextSearchDictionaryFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_text_search_dictionary_factor_loop_obligations(ROOT)
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(35, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(35, sum(seen.values()))
        self.assertEqual(35, len(seen))
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_text_search_dictionary_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTextSearchDictionaryFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_text_search_dictionary_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTextSearchDictionaryFactorLoopPlan)
        ...
        self.assertEqual(
            "DROPTEXTSEARCHDICTIONARY00001", plan.cases[0].case_id
        )
        self.assertEqual(
            f"DROPTEXTSEARCHDICTIONARY{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTEXTSEARCHDICTIONARY00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHDICTIONARY{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptextsearchdictionary_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptextsearchdictionary_{_OBLIGATION_COUNT:05d}_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_drop_text_search_dictionary_factor_loop_plan(ROOT)
        obligations = (
            compile_drop_text_search_dictionary_factor_loop_obligations(ROOT)
        )
    ...
    def test_sfv_obligation_ids_carry_row_id_one_to_one(self) -> None:
        ...
        self.assertEqual(35, len(sfv_cases))
        for case in sfv_cases:
            self.assertRegex(
                case.primary_obligation_id,
                r"^DROPTEXTSEARCHDICTIONARY-SFV\|[^|]+\|[^|]+$",
                case.case_id,
            )
