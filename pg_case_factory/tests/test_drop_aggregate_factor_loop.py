...
_OBLIGATION_MULTISET_SHA256 = (
    "f4872448b35b1709f2b0c240a13d063858d94dee16bb53bbc03157fd2e14c0be"
)


class DropAggregateFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_aggregate_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_aggregate_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropAggregateFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_aggregate_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropAggregateFactorLoopPlan)
        ...
        self.assertEqual("DROPAGGREGATE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPAGGREGATE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPAGGREGATE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPAGGREGATE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropaggregate_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropaggregate_{_OBLIGATION_COUNT:05d}_",
