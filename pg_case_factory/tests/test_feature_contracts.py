...
def _risk_decisions() -> dict:
    ...
    return decisions
from pg_case_factory.feature_plan import topological_test_points
...
class FeatureContractTest(unittest.TestCase):
    ...
    def coverage_document(self) -> dict:
        ...
        return {
            ...
            "scope_decisions": {
                ...
                "column_type": {
                    "status": "not_applicable",
                    "reason": "The compact fixture does not claim the complete PG18.4 type universe.",
                },
            },
    ...
    def test_case_execution_profile_is_fail_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(
                ContractValidationError,
                "basic_psql route must not declare execution_harness",
            ):
    ...
    def test_feature_source_hash_target_and_requirement_locators_are_mandatory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ContractValidationError, "postgresql-18.4"):
                load_feature_manifest(
    ...
    def test_inline_inventory_requires_feature_and_pg18_source_locators(self):
        document = self.coverage_document()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(
                ContractValidationError,
                "requires both feature:<requirement-id> and pg18:<official-topic>",
            ):
                load_coverage_plan(self.write_yaml(Path(tmp), "inline.yaml", document))
    ...
    def test_declared_execution_harness_must_route_at_least_one_obligation(self):
        ...
        compression_point = next(
            point
            for point in document["test_points"]
            if point["id"] == "TP-TOAST-STORAGE-COMPRESSION"
        )
        compression_point.pop("execution_rules")

        with tempfile.TemporaryDirectory() as tmp:
            plan_path = self.write_yaml(Path(tmp), "unrouted-lz4.yaml", document)
            with self.assertRaisesRegex(
                ContractValidationError,
                r"risk decision lz4_provisioning declares execution harness "
                r"external-toast-lz4.*none of its declared test points routes",
            ):
                load_coverage_plan(plan_path, inventory_root=repository_root)

    def test_shipped_feature_and_plan_templates_trace_pg18_copy_requirements(self):
        repository_root = Path(__file__).resolve().parents[1]
        template_root = (
            repository_root
            / "skills"
        ...
        feature_document = yaml.safe_load(
            (template_root / "feature_manifest_template.yaml").read_text(
                encoding="utf-8"
            )
        )
        feature_document["source"]["sha256"] = "a" * 64
        with tempfile.TemporaryDirectory() as tmp:
            manifest = load_feature_manifest(
                self.write_yaml(Path(tmp), "feature.yaml", feature_document)
            )
            plan = load_coverage_plan(
                template_root / "coverage_plan_template.yaml",
                inventory_root=repository_root,
                manifest=manifest,
            )
        ...
        copy_point = next(
            ...
            if point.test_point_id == "TP-COPY-PG18-OPTIONS"
        )
        self.assertIn("REQ-008", copy_point.requirement_ids)
        self.assertIn(
            "feature:REQ-008",
            plan.axes["copy_pg18_option_case"].source_locators,
        )
