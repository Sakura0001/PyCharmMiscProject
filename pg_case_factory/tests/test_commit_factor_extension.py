...
class CommitFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_commit_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, CommitFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual("COMMIT00045", plan.cases[0].case_id)
        self.assertEqual("COMMIT01844", plan.cases[-1].case_id)
        self.assertEqual(
            "COMMIT00045.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "COMMIT01844.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_commit_factor_extension_plan(ROOT)
        plan_b = build_commit_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_commit_factor_extension_plan(ROOT)
        self.assertEqual(
            "71f7931ef1daa9b9f8e32e102d7eb9381fe8a87a988b6e2d1bbb1a11793ed9d5",
