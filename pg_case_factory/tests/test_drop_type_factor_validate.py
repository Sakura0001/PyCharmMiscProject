...
from pg_case_factory.validate_drop_type import (
    DropTypeFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_type_factor_programs,
)
...
class DropTypeFactorValidateTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.result = validate_drop_type_factor_programs(
    ...
    def test_validation_passes_for_all_4318_files(self) -> None:
        self.assertIsInstance(
            self.result, DropTypeFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(_BASELINE_COUNT, self.result.baseline_case_count)
        self.assertEqual(
            _EXTENSION_COUNT, self.result.extension_case_count
        )
        self.assertEqual(_TOTAL_COUNT, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(_TOTAL_COUNT, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(_TOTAL_COUNT, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_type_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_type_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPTYPE00001", "DROPTYPE00002"}
        result = validate_drop_type_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
