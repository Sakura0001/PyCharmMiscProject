"""Tests for execute factor validate module."""
...
from src.pg_case_factory.validate_execute import (
...
class TestExecuteFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_execute_factor_loop_plan(
            _REPO
        )
        self.extension = (
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_execute_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            _REPO,
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "EXECUTE00001.sql"
        original = self.programs[filename]
        tampered = original.replace(
            "EXECUTE execute_00001_stmt;",
            "DROP TABLE execute_00001_stmt;",
        )
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "execute_actual_factor_witness_report",
            d["kind"],
        )
