...
from pg_case_factory.create_conversion_factor_runtime import (
...
class CreateConversionRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55520, PG18_PORT)
        self.assertEqual("pgcf_cconv", PG18_DATABASE)
    ...
    def test_socket_is_distinct(self) -> None:
        self.assertEqual(
            "pgcf-pg18-cconv-sock-20260820",
            PG18_SOCKET.name,
        )


class CreateConversionRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1002_cases(self) -> None:
        sql_dir = Path("/tmp/create_conversion_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_create_conversion_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(1002, len(cases))
        self.assertEqual(1002, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_create_conversion_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(1002, ordinals[-1])


class CreateConversionRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = CreateConversionPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(CreateConversionRuntimeError):
            CreateConversionPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateConversionRuntimeError):
            CreateConversionPg18Runner(timeout_seconds=301)
...
class CreateConversionComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "CREATECONVERSION00001"
    ) -> CreateConversionCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = CreateConversionSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = CreateConversionSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_create_conversion_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = CreateConversionCaseRuntimeResult(
            case_id="CREATECONVERSION00001",
            sql_filename="CREATECONVERSION00001.sql",
            expected_sqlstate="00000",
