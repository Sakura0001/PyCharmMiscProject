"""Tests for create_text_search_dictionary factor_loop obligation ledger."""
...
class TestCreateTextSearchDictionaryFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_text_search_dictionary_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(45, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(45, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 44}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "659228e75179e452f1db8c76cf66d1a4563cff7d72c9c935f0f207090574b17c",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_text_search_dictionary_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 31, "expected_failure": 14}, dict(counts)
        )
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATETEXTSEARCHDICTIONARY"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_text_search_dictionary_factor_loop_obligations(_REPO)
