...
from pg_case_factory.drop_type_factor_runtime import (
...
class DropTypeRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_socket_dir_is_frozen(self) -> None:
        self.assertEqual(
            "pgcf-pg18-dt-sock-20260821",
            PG18_SOCKET.name,
        )


class DropTypeRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_4318_cases(self) -> None:
        sql_dir = Path("/tmp/droptype_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_type_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(_TOTAL_COUNT, len(cases))
        self.assertEqual(_TOTAL_COUNT, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_type_runtime_case_set(ROOT, sql_dir)
...
class DropTypeRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropTypePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropTypeRuntimeError):
            DropTypePg18Runner(timeout_seconds=0)
        with self.assertRaises(DropTypeRuntimeError):
            DropTypePg18Runner(timeout_seconds=301)
...
class DropTypeComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPTYPE00001"
    ) -> DropTypeCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropTypeSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropTypeSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_type_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropTypeCaseRuntimeResult(
            case_id="DROPTYPE00001",
            sql_filename="DROPTYPE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPTYPE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropTypeRuntimeError):
            DropTypeSuiteRun(run_ordinal=3, cases=())
