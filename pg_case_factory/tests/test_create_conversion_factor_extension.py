...
class CreateConversionFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_conversion_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateConversionFactorExtensionPlan
        )
        self.assertEqual(936, len(plan.cases))
        self.assertEqual(936, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(67, 1003)), ordinals
        )
        self.assertEqual(
            "CREATECONVERSION00067", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATECONVERSION01002", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATECONVERSION00067.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATECONVERSION01002.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(192, outcomes["success"])
        self.assertEqual(744, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_conversion_factor_extension_plan(ROOT)
        plan_b = build_create_conversion_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_conversion_factor_extension_plan(ROOT)
        self.assertEqual(
            "3a35f5e6dc4f3baa99a87d0bced9306ab03eb7116244ae895e53744a9dab53d2",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(66, _BASELINE_COUNT)
        self.assertEqual(20000, _CAP)
        self.assertEqual(
            ("catalog_query_pg_conversion", "error_assertion"),
            _VERIFICATION_MODES,
        )
        self.assertEqual(
            ("drop_conversion", "cascade_drop"), _CLEANUP_MODES
        )
    ...
    def test_total_case_count_is_within_cap(self) -> None:
        ...
        self.assertEqual(1002, total)

    def test_total_case_count_exceeds_threshold(self) -> None:
        plan = build_create_conversion_factor_extension_plan(ROOT)
        total = _BASELINE_COUNT + len(plan.cases)
