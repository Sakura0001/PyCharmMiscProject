...
class CreateCastFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="ccast_render_"))
        cls.count = generate_create_cast_factor_programs(
    ...
    def test_renders_exactly_19215_files(self) -> None:
        self.assertEqual(19215, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(19215, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATECAST{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_cast(text)
    ...
    def test_primary_target_is_create_cast(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*CREATE\s+CAST\b", region
                ),
                f"no CREATE CAST in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_create_cast_factor_witness(case, root)
            self.assertIsInstance(
                w, CreateCastFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_cast_factor_case(case, root)
        b = render_create_cast_factor_case(case, root)
