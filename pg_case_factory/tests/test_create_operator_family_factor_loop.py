"""Tests for create_operator_family factor_loop obligation ledger."""
...
class TestCreateOperatorFamilyFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_operator_family_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(31, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(31, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 30}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "cb5fae42d43e3ce8c4908cbd7b704e0e378264ea9eb8bfddcabb0b82bf17945f",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_operator_family_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 24, "expected_failure": 7}, dict(counts)
        )
    ...
    def test_contiguous_ordinals(self) -> None:
        ...
        self.assertEqual(
            list(range(1, 32)), ordinals
        )

    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("createoperatorfamily_")
            )
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.case_id.startswith("CREATEOPERATORFAMILY")
            )
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = (
            compile_create_operator_family_factor_loop_obligations(_REPO)
        )
