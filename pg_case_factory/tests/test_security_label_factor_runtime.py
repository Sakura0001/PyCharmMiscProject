...
from pg_case_factory.security_label_factor_runtime import (
    MAX_PARALLELISM,
    PG18_BIN,
    PG18_DATABASE,
    PG18_PORT,
...
class SecurityLabelRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_security_label", PG18_DATABASE)
...
class SecurityLabelRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_182_cases(self) -> None:
        sql_dir = Path("/tmp/security_label_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_security_label_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(182, len(cases))
        self.assertEqual(182, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_security_label_runtime_case_set(
            ROOT, sql_dir
        )
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(182, ordinals[-1])


class SecurityLabelRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = SecurityLabelPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(SecurityLabelRuntimeError):
            SecurityLabelPg18Runner(timeout_seconds=0)
        with self.assertRaises(SecurityLabelRuntimeError):
            SecurityLabelPg18Runner(timeout_seconds=301)
...
class SecurityLabelComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "SECURITYLABEL00001"
    ) -> SecurityLabelCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = SecurityLabelSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = SecurityLabelSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_security_label_runs(run1, run2)
    ...
    def test_comparison_fails_on_sqlstate_mismatch(
        ...
        case = SecurityLabelCaseRuntimeResult(
            case_id="SECURITYLABEL00001",
            sql_filename="SECURITYLABEL00001.sql",
            expected_sqlstate="00000",
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(SecurityLabelRuntimeError):
            SecurityLabelSuiteRun(run_ordinal=3, cases=())
