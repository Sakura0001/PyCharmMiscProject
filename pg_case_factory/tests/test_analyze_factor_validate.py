...
from pg_case_factory.analyze_factor_render import (
    generate_analyze_factor_programs,
    render_analyze_factor_case,
)
from pg_case_factory.analyze_factor_validate import (
...
class AnalyzeFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_analyze_factor_loop_plan(ROOT)
        cls.extension = build_analyze_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="analyze_validate_"))
        generate_analyze_factor_programs(
    ...
    def test_validation_passes_for_all_7152_files(self) -> None:
        result = validate_analyze_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, AnalyzeFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(48, result.baseline_case_count)
        self.assertEqual(7104, result.extension_case_count)
        self.assertEqual(7152, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(7152, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_analyze_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(7152, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_analyze_factor_case(case, ROOT)
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_analyze_factor_case(case, ROOT)
        tampered = sql.replace("ANALYZE", "VACUUMBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ANALYZE00001", "ANALYZE00002"}
        result = validate_analyze_factor_programs(
            self.baseline, self.extension, self.programs, ROOT,
