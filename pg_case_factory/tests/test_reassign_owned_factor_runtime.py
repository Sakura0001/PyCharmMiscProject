...
from pg_case_factory.reassign_owned_factor_runtime import (
...
class ReassignOwnedRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55514, PG18_PORT)
        self.assertEqual("pgcf_ro", PG18_DATABASE)
...
class ReassignOwnedRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_4081_cases(self) -> None:
        sql_dir = Path("/tmp/reassign_owned_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_reassign_owned_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(4081, len(cases))
        self.assertEqual(4081, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_reassign_owned_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(4081, ordinals[-1])


class ReassignOwnedRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = ReassignOwnedPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(ReassignOwnedRuntimeError):
            ReassignOwnedPg18Runner(timeout_seconds=0)
        with self.assertRaises(ReassignOwnedRuntimeError):
            ReassignOwnedPg18Runner(timeout_seconds=301)
...
class ReassignOwnedComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "REASSIGNOWNED00001"
    ) -> ReassignOwnedCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = ReassignOwnedSuiteRun(run_ordinal=1, cases=(case,))
        run2 = ReassignOwnedSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_reassign_owned_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = ReassignOwnedCaseRuntimeResult(
            case_id="REASSIGNOWNED00001",
            sql_filename="REASSIGNOWNED00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("REASSIGNOWNED00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(ReassignOwnedRuntimeError):
            ReassignOwnedSuiteRun(run_ordinal=3, cases=())
