...
from pg_case_factory.alter_text_search_parser_factor_runtime import (
...
class AlterTextSearchParserRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_alter_schema_and_subscription(
        self,
    ) -> None:
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_atsp", PG18_DATABASE)
...
class AlterTextSearchParserRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_251_cases(self) -> None:
        sql_dir = Path("/tmp/alter_text_search_parser_probe_leaf")
        ...
        self.assertEqual(251, len(cases))
        self.assertEqual(251, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        self.assertEqual(251, ordinals[-1])


class AlterTextSearchParserRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = AlterTextSearchParserPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(AlterTextSearchParserRuntimeError):
            AlterTextSearchParserPg18Runner(timeout_seconds=0)
        with self.assertRaises(AlterTextSearchParserRuntimeError):
            AlterTextSearchParserPg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_ts_parser(self) -> None:
        runner = AlterTextSearchParserPg18Runner()
        query = runner._clean_probe.__code__.co_consts
        joined = " ".join(str(c) for c in query if isinstance(c, str))
        self.assertIn("pg_ts_parser", joined)


class AlterTextSearchParserComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "ALTERTEXTSEARCHPARSER00001"
    ) -> AlterTextSearchParserCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = AlterTextSearchParserSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = AlterTextSearchParserSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_alter_text_search_parser_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = AlterTextSearchParserCaseRuntimeResult(
            case_id="ALTERTEXTSEARCHPARSER00001",
            sql_filename="ALTERTEXTSEARCHPARSER00001.sql",
            expected_sqlstate="00000",
