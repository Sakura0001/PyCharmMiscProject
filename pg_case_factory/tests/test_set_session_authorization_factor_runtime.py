"""Tests for set_session_authorization factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.set_session_authorization_factor_runtime import (
    MAX_PARALLELISM,
...
class TestSetSessionAuthorizationFactorRuntime(unittest.TestCase):
    ...
    def test_constants_distinct_from_set_role(self) -> None:
        self.assertNotEqual(PG18_PORT, 55622)
        self.assertNotEqual(PG18_DATABASE, "pgcf_set_role")

    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55623)
        self.assertEqual(
        ...
        self.assertEqual(
            PG18_SOCKET,
            Path(
                "/tmp/pgcf-pg18-set-session-authorization-"
                "sock-20260821"
            ),
        )
    ...
    def test_runner_construction(self) -> None:
        runner = SetSessionAuthorizationPg18Runner()
    ...
    def test_timeout_validation(self) -> None:
        with self.assertRaises(
            ...
            SetSessionAuthorizationPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            SetSessionAuthorizationRuntimeError
        ):
            SetSessionAuthorizationPg18Runner(timeout_seconds=301)
    ...
    def test_compare_runs_passes_for_identical(self) -> None:
        result = SetSessionAuthorizationSuiteRun(
            run_ordinal=1, cases=()
        )
        other = SetSessionAuthorizationSuiteRun(
            run_ordinal=2, cases=()
        )
        comparison = compare_set_session_authorization_runs(
            result, other
        )
