...
class AlterMaterializedViewFactorRenderTest(unittest.TestCase):
    def test_every_case_resolves_a_concrete_witness(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                witness = resolve_alter_materialized_view_factor_witness(
                    case, ROOT
                )
                self.assertEqual(
                ...
                # section rather than the primary fence) is an ALTER
    ...
    def test_all_programs_are_complete_and_have_one_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                sql = render_alter_materialized_view_factor_case(case, ROOT)
                self.assertTrue(
                    sql.startswith("-- --------------------------------------------------------\n")
                )
                self.assertEqual(1, count_primary_alter_materialized_view(sql))
                self.assertIn(
    ...
    def test_all_programs_are_deterministic_in_memory(self) -> None:
        plan = build_alter_materialized_view_factor_loop_plan(ROOT)
        first = [
            render_alter_materialized_view_factor_case(row, ROOT)
    ...
    def test_success_programs_keep_error_stop_armed(self) -> None:
        ...
        self.assertEqual(51, len(successes))
    ...
    def test_transaction_witnesses_have_distinct_commit_and_rollback(
        ...
        commit_sql = render_alter_materialized_view_factor_case(
            rows["commit"], ROOT
        )
        rollback_sql = render_alter_materialized_view_factor_case(
            rows["rollback"], ROOT
        )
    ...
    def test_oracle_is_catalog_audit_compliant(self) -> None:
        # Every catalog_query / effect_query baseline oracle SELECT is a
        # top-level
        # ``SELECT count(*) <op> AS alias FROM pg_catalog.pg_class ... ORDER BY count(*)``
        # never a ``SELECT EXISTS(SELECT 1 FROM ...)`` subquery.  The single
        # error_assertion baseline case carries only the sqlstate oracle (no
        # count probe) and is intentionally excluded from the count-regex
        # assertion.
        plan = build_alter_materialized_view_factor_loop_plan(ROOT)
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
    ...
    def test_primary_fence_is_unique_and_brackets_target(self) -> None:
        ...
        for case in plan.cases:
            with self.subTest(case_id=case.case_id):
                ...
                self.assertEqual(
                    1, count_primary_alter_materialized_view(sql)
                )


class AlterMaterializedViewExtensionRenderTest(unittest.TestCase):
    """Extension cases render through a byte-safe synthetic baseline case."""

    def setUp(self) -> None:
        self.extension_plan = (
            build_alter_materialized_view_factor_extension_plan(ROOT)
        )

    def _representative_subset(
        self,
    ) -> list[AlterMaterializedViewFactorExtensionCase]:
        # One case per (branch, outcome) pair so every branch x outcome
        # combination is exercised without rendering all 8424 programs.
        # ALTER MATERIALIZED VIEW has a single branch, so this yields two.
        seen: set[tuple[str, str]] = set()
        subset: list[AlterMaterializedViewFactorExtensionCase] = []
    ...
    def test_extension_cases_render_complete_5_phase_program(self) -> None:
        for case in self._representative_subset():
            with self.subTest(case_id=case.case_id):
                ...
                self.assertIn("-- description  : ALTER MATERIALIZED VIEW", sql)
                self.assertNotRegex(sql, r"\{[A-Za-z_]\w*\}")
                self.assertTrue(sql.endswith(";\n"))
                self.assertFalse(sql.endswith("\n\n"))
    ...
    def test_extension_renders_are_deterministic(self) -> None:
        ...
        first = [
            render_alter_materialized_view_factor_case(c, ROOT) for c in subset
        ]
        second = [
            render_alter_materialized_view_factor_case(c, ROOT) for c in subset
        ]
    ...
    def test_generator_writes_baseline_and_extension_files(self) -> None:
        baseline_plan = build_alter_materialized_view_factor_loop_plan(ROOT)
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "alter_materialized_view"
            count = generate_alter_materialized_view_factor_programs(
                baseline_plan, self.extension_plan, out
            )
            self.assertEqual(_TOTAL, count)
            files = sorted(out.glob("ALTERMATERIALIZEDVIEW*.sql"))
            ...
            self.assertEqual(1, count_primary_alter_materialized_view(b0))
            e0 = (
                out / self.extension_plan.cases[0].sql_filename
            ).read_text(encoding="utf-8")
            self.assertEqual(1, count_primary_alter_materialized_view(e0))
