...
class PrepareTransactionFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="preptran_render_"))
        cls.count = generate_prepare_transaction_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"PREPARETRANSACTION{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_prepare_transaction(text)
    ...
    def test_primary_target_is_prepare_transaction(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?im)^\s*PREPARE\s+TRANSACTION\b", region),
                f"no PREPARE TRANSACTION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_prepare_transaction_factor_witness(case, root)
            self.assertIsInstance(
                w, PrepareTransactionFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_prepare_transaction_factor_case(case, root)
        b = render_prepare_transaction_factor_case(case, root)
