...
class ReleaseSavepointFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_release_savepoint_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, ReleaseSavepointFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(
        ...
        self.assertEqual(
            "RELEASESAVEPOINT00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "RELEASESAVEPOINT01844", plan.cases[-1].case_id
        )
        self.assertEqual(
            "RELEASESAVEPOINT00045.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "RELEASESAVEPOINT01844.sql",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_release_savepoint_factor_extension_plan(ROOT)
        plan_b = build_release_savepoint_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_release_savepoint_factor_extension_plan(ROOT)
        self.assertEqual(
            "629be4b235f1bfc8aade4c824bee08baef3c512896234467efc385bde4c205e3",
