"""Tests for create_policy factor_loop obligation ledger."""
...
class TestCreatePolicyFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_policy_factor_loop_plan(_REPO)

    def test_obligation_count(self) -> None:
        self.assertEqual(81, len(self.plan.obligations))

    def test_case_count(self) -> None:
        self.assertEqual(81, len(self.plan.cases))
    ...
    def test_kind_counts(self) -> None:
        ...
        self.assertEqual({"GRM": 1, "SFV": 80}, dict(counts))

    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "2f16773031dd50e2c30df4327309469f987f637303aed90323b54a7891222de8",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_policy_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 63, "expected_failure": 18}, dict(counts)
        )
    ...
    def test_contiguous_ordinals(self) -> None:
        ...
        self.assertEqual(list(range(1, 82)), ordinals)

    def test_object_prefix_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.object_prefix.startswith("createpolicy_")
            )
            self.assertTrue(case.object_prefix.endswith("_"))

    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATEPOLICY"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_policy_factor_loop_obligations(
            _REPO
        )
        self.assertEqual(
