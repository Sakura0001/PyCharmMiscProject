...
from pg_case_factory.alter_sequence_factor_validate import (
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_sequence_factor_extension_plan(ROOT)
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_sequence_factor_case(c, ROOT)
        for c in chosen
    }
...
class AlterSequenceFactorValidateTest(unittest.TestCase):
    ...
    def test_missing_program_is_reported(self) -> None:
        baseline_plan = build_alter_sequence_factor_loop_plan(ROOT)
        extension_plan = (
            build_alter_sequence_factor_extension_plan(ROOT)
        )
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        extra = "ALTERSEQUENCE99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERSEQUENCE99999\n"
            "-- primary_obligation_id: ALTSEQ-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER SEQUENCE bogus RESTART;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_sequence_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_sequence_factor_case(
            target_case, ROOT
        )
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(
            rec["primary_obligation_id"].startswith("ALTSEQ-EXT|")
        )
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn(
            "alter_sequence_actual_factor_witness_report", blob
        )
        self.assertIn(
    ...
    def test_byte_mismatch_is_detected(self) -> None:
        ...
        canonical = render_alter_sequence_factor_case(
            target_case, ROOT
        )
        tampered = canonical.replace(
            "ALTER SEQUENCE", "ALTER  SEQUENCE", 1
        )
