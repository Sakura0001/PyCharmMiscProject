...
class LockFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_lock_factor_loop_obligations(ROOT)
        # LOCK TABLE has no GRM block: the single statement_branch value
        # IS the syntax branch and is itself an SFV row.  All 46 declared
        # factor values are SFV obligations (0 GRM, 0 INV).
...
class LockFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_lock_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, LockFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(46, len(plan.cases))
        self.assertEqual(
            list(range(1, 47)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("LOCK00001", plan.cases[0].case_id)
        self.assertEqual("LOCK00046", plan.cases[-1].case_id)
        self.assertEqual("LOCK00001.sql", plan.cases[0].sql_filename)
        self.assertEqual("LOCK00046.sql", plan.cases[-1].sql_filename)
        self.assertEqual("lock_00001_", plan.cases[0].object_prefix)
        self.assertEqual("lock_00046_", plan.cases[-1].object_prefix)
        self.assertEqual(
            46,
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_lock_factor_loop_plan(ROOT)
        again = build_lock_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_lock_factor_loop_plan(ROOT)
        self.assertEqual(
            "6f2ba4935662cc68e3db245987632ac9ab70d8bacac04939ae413619631804ad",
            plan.obligation_multiset_sha256,
        )
