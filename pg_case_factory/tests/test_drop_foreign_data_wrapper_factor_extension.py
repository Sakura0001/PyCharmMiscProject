"""Tests for the bounded DROP FOREIGN DATA WRAPPER post-coverage extension expander.

The marginal factor-value-loop (``drop_foreign_data_wrapper_factor_loop.py``) is
the frozen 45-case baseline (GRM 1 + SFV 42 + RISK 2).  This module's expander
...
from pg_case_factory.drop_foreign_data_wrapper_factor_extension import (
    DropForeignDataWrapperFactorExtensionCase,
    DropForeignDataWrapperFactorExtensionError,
    DropForeignDataWrapperFactorExtensionPlan,
    build_drop_foreign_data_wrapper_factor_extension_plan,
)
from pg_case_factory.drop_foreign_data_wrapper_factor_loop import (
...
_BASELINE_COUNT = 45
_EXTENSION_COUNT = 657
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 702
_SUCCESS_EXTENSIONS = 270
_FAILURE_EXTENSIONS = 387
_EXTENSION_MULTISET_SHA256 = (
    "bfe1cf0abd80c3ba35fdac431332e5a8e96a5ccfd914adb9e2932485cd40a98d"
)

_PRIVILEGE_NEGATIVE = ("privilege_level", "non_owner")
_NOT_EXIST_NEGATIVE = (
    "nonexistent_fdw",
    "fdw_missing_no_if_exists",
)
...
_CROSSED_AXES = {
    ...
    "privilege_level": ("fdw_owner", "non_owner", "superuser"),
    "nonexistent_fdw": ("fdw_exists", "fdw_missing_no_if_exists"),
    "if_exists_clause": ("absent", "present"),
    "cascade_restrict": ("restrict_default", "cascade"),
    "dependent_objects": (
        "no_dependencies",
        "server_dependencies",
...
_BRANCH_GRAMMAR = {"branch_drop_fdw": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_fdw": "drop_foreign_data_wrapper"}
...
def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("nonexistent_fdw") == "fdw_missing_no_if_exists"
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("dependent_objects", "no_dependencies") != "no_dependencies"
...
def _present_failure_pair(
    ...
    if _dependency_failure_fires(assignment):
        return ("dependent_objects", assignment.get("dependent_objects", "no_dependencies"))
    return None


class DropForeignDataWrapperFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_foreign_data_wrapper_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropForeignDataWrapperFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPFOREIGNDATAWRAPPER00046", first.case_id)
        self.assertEqual(f"DROPFOREIGNDATAWRAPPER{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPFOREIGNDATAWRAPPER00046.sql", first.sql_filename)
        self.assertEqual(
            f"DROPFOREIGNDATAWRAPPER{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropforeigndatawrapper_00046_", first.object_prefix)
        self.assertEqual(
            f"dropforeigndatawrapper_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropForeignDataWrapperFactorExtensionCase)
    ...
    def test_not_exist_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_level"] == "non_owner":
                continue
            if (
                a["nonexistent_fdw"] == "fdw_missing_no_if_exists"
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if a["dependent_objects"] == "no_dependencies":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_foreign_data_wrapper_catalog"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["recreate_fdw"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_foreign_data_wrapper_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_foreign_data_wrapper_factor_loop_plan(ROOT)
