...
from pg_case_factory.alter_language_factor_validate import (
    AlterLanguageFactorProgramValidation,
...
# Module-level cache so the 642-program render happens once across tests.
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_language_factor_extension_plan(ROOT)
        programs: dict[str, str] = {}
        for case in baseline_plan.cases:
            programs[case.sql_filename] = render_alter_language_factor_case(
                case, ROOT
            )
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_language_factor_case(c, ROOT) for c in chosen
    }
    selected = {c.case_id for c in chosen}
    return programs, selected


class AlterLanguageFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_language_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(report, AlterLanguageFactorProgramValidation)
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 99999 is outside the 0001-00642 case range, so it is genuinely unknown.
        extra = "ALTERLANGUAGE99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERLANGUAGE99999\n"
            "-- primary_obligation_id: AL-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER LANGUAGE bogus RENAME TO bogus_renamed;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_language_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_language_factor_case(target_case, ROOT)
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(rec["primary_obligation_id"].startswith("AL-EXT|"))
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn("alter_language_actual_factor_witness_report", blob)
