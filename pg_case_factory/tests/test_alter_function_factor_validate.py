...
# Module-level cache so the 13,263-program render happens once across tests.
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_function_factor_extension_plan(ROOT)
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_function_factor_case(c, ROOT) for c in chosen
    }
...
class AlterFunctionFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        ...
        self.assertIsInstance(report, AlterFunctionFactorProgramValidation)
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 99999 is outside the 0001-13263 case range, so it is genuinely unknown.
        extra = "ALTERFUNCTION99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERFUNCTION99999\n"
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_function_factor_case(target_case, ROOT)
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(rec["primary_obligation_id"].startswith("AF-EXT|"))
