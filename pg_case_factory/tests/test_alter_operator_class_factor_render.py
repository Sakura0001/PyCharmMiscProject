"""Byte-contract render tests for ALTER OPERATOR CLASS factor-loop SQL."""
...
class AlterOperatorClassFactorRenderTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _render(self, case) -> str:
        return render_alter_operator_class_factor_case(case)

    def test_baseline_case_has_single_primary_target(self) -> None:
        sql = self._render(self.baseline.cases[0])
        self.assertEqual(count_primary_alter_operator_class(sql), 1)
    ...
    def test_header_fields_present(self) -> None:
        sql = self._render(self.baseline.cases[0])
        self.assertIn("-- case_id: ALTEROPERATORCLASS00001", sql)
    ...
    def test_witness_loci_nonempty(self) -> None:
        ...
        witness = resolve_alter_operator_class_factor_witness(case)
    ...
    def test_generate_programs_to_disk(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            count = generate_alter_operator_class_factor_programs(
                self.baseline, self.extension, Path(td)
            )
            self.assertEqual(count, 51 + 9072)

    def test_escape_sqlstate_rejects_invalid(self) -> None:
        with self.assertRaises(AlterOperatorClassFactorRenderError):
            from pg_case_factory.alter_operator_class_factor_render import (
                _escape_sqlstate,
            )
