...
class RollbackPreparedFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_rollback_prepared_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, RollbackPreparedFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            "ROLLBACKPREPARED00045", plan.cases[0].case_id
        )
        self.assertEqual(
            "ROLLBACKPREPARED12284", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ROLLBACKPREPARED00045.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ROLLBACKPREPARED12284.sql",
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_rollback_prepared_factor_extension_plan(ROOT)
        plan_b = build_rollback_prepared_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_rollback_prepared_factor_extension_plan(ROOT)
        self.assertEqual(
            "90c40f01f3fdee36e82f6d2547e2ae568b879c24bae0a8f7e74ed346d373829c",
