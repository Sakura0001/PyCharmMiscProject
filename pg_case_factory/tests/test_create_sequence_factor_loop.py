"""Tests for create_sequence factor_loop obligation ledger."""
...
class TestCreateSequenceFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_sequence_factor_loop_plan(_REPO)
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "99438f6c30e51debf0b2943cd5a897c74b9f2f3b66534044077bee1e875d2512",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_sequence_factor_loop_plan(_REPO)
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_outcome_counts(self) -> None:
        ...
        self.assertEqual(
            {"success": 50, "expected_failure": 21}, dict(counts)
        )
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(case.case_id.startswith("CREATESEQUENCE"))
    ...
    def test_compile_obligations_matches_plan(self) -> None:
        obligations = compile_create_sequence_factor_loop_obligations(_REPO)
