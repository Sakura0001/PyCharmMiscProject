...
from pg_case_factory.drop_text_search_configuration_factor_runtime import (
...
class DropTextSearchConfigurationRuntimeConstantsTest(unittest.TestCase):
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual("pgcf_dtsc", PG18_DATABASE)
        ...
        self.assertEqual(
            "pgcf-pg18-dtsc-sock-20260821", PG18_SOCKET.name
        )


class DropTextSearchConfigurationRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_1886_cases(self) -> None:
        sql_dir = Path(
            "/tmp/drop_text_search_configuration_probe_leaf"
        )
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_text_search_configuration_runtime_case_set(
            ROOT, sql_dir
        )
...
class DropTextSearchConfigurationRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropTextSearchConfigurationPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(
            ...
            DropTextSearchConfigurationPg18Runner(timeout_seconds=0)
        with self.assertRaises(
            DropTextSearchConfigurationRuntimeError
        ):
            DropTextSearchConfigurationPg18Runner(timeout_seconds=301)
...
class DropTextSearchConfigurationComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPTEXTSEARCHCONFIGURATION00001"
    ) -> DropTextSearchConfigurationCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropTextSearchConfigurationSuiteRun(
            run_ordinal=1, cases=(case,)
        )
        run2 = DropTextSearchConfigurationSuiteRun(
            run_ordinal=2, cases=(case,)
        )
        result = compare_drop_text_search_configuration_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropTextSearchConfigurationCaseRuntimeResult(
            case_id="DROPTEXTSEARCHCONFIGURATION00001",
            sql_filename="DROPTEXTSEARCHCONFIGURATION00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn(
            "DROPTEXTSEARCHCONFIGURATION00001",
