...
from pg_case_factory.create_conversion_factor_validate import (
    CreateConversionFactorProgramValidation,
...
class CreateConversionFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="cconv_validate_"))
        generate_create_conversion_factor_programs(
    ...
    def test_validation_passes_for_all_1002_files(self) -> None:
        result = validate_create_conversion_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CreateConversionFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(66, result.baseline_case_count)
        self.assertEqual(936, result.extension_case_count)
        self.assertEqual(1002, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1002, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_conversion_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(1002, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_conversion_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE CONVERSION", "CREATE SCHEMABOGUS"
        )
