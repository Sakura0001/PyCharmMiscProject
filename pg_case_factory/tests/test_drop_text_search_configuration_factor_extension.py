"""Tests for the bounded DROP TEXT SEARCH CONFIGURATION post-coverage extension expander.

The marginal factor-value-loop (``drop_text_search_configuration_factor_loop.py``)
is the frozen 38-case baseline (GRM 1 + SFV 35 + RISK 2).  This module's
...
from pg_case_factory.drop_text_search_configuration_factor_extension import (
...
from pg_case_factory.drop_text_search_configuration_factor_loop import (
...
_BASELINE_COUNT = 38
_EXTENSION_COUNT = 1848
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 1886
_SUCCESS_EXTENSIONS = 816
_FAILURE_EXTENSIONS = 1032
_EXTENSION_MULTISET_SHA256 = (
    "bbb96f890b4242500086a0415b645f7a36627c5d01bf379983a56a12d934d414"
)
...
class DropTextSearchConfigurationFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_text_search_configuration_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(
            self.plan, DropTextSearchConfigurationFactorExtensionPlan
        )
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTEXTSEARCHCONFIGURATION00039", first.case_id)
        self.assertEqual(
            f"DROPTEXTSEARCHCONFIGURATION{_TOTAL_COUNT:05d}", last.case_id
        )
        self.assertEqual(
            "DROPTEXTSEARCHCONFIGURATION00039.sql", first.sql_filename
        )
        self.assertEqual(
            f"DROPTEXTSEARCHCONFIGURATION{_TOTAL_COUNT:05d}.sql",
            last.sql_filename,
        )
        self.assertEqual(
            "droptextsearchconfiguration_00039_", first.object_prefix
        )
        self.assertEqual(
            f"droptextsearchconfiguration_{_TOTAL_COUNT:05d}_",
            last.object_prefix,
        )
    ...
    def test_privilege_negative_fires_for_non_owner(self) -> None:
        for case in self.plan.cases:
            ...
            if a["authorization_path"] == "non_owner":
                self.assertEqual(
                    "expected_failure", case.outcome, case.case_id
                )
                self.assertEqual(
    ...
    def test_config_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if a["authorization_path"] == "non_owner":
                continue
            if (
                (
                    ...
                    or a["config_name_shape"] == "non_existent_name"
                )
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if not _config_exists(a):
                continue
            if a["dependency_status"] != "has_dependencies":
                continue
    ...
    def test_derived_factors_are_coherent_with_their_drivers(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            self.assertEqual(
                _PRIVILEGE_CONTEXT_FOR[a["authorization_path"]],
                a["privilege_context"],
                case.case_id,
            )
            self.assertEqual(
                _DEPENDENCY_CONTEXT_FOR[a["dependency_status"]],
                a["dependency_context"],
                case.case_id,
            )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_text_search_configuration_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_text_search_configuration_factor_loop_plan(ROOT)
