...
class CreateExtensionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_extension_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(62) = 63 ; no INV (CREATE EXTENSION
        # loads into pg_extension, not pg_class).
    ...
    def test_canonical_obligation_count_matches_inventory(self) -> None:
        ...
        self.assertEqual(62, len(sfv))
        seen: dict[tuple[str, str], int] = {}
        for row in sfv:
            seen[(row.factor_key, row.value)] = (
                seen.get((row.factor_key, row.value), 0) + 1
            )
        self.assertEqual(62, sum(seen.values()))
        self.assertEqual(62, len(seen))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # 19 canonical values reach the PG target check and are rejected;
...
class CreateExtensionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_extension_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateExtensionFactorLoopPlan)
        ...
        self.assertEqual(
            "CREATEEXTENSION00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEEXTENSION00063", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEEXTENSION00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEEXTENSION00063.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createextension_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createextension_00063_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_extension_factor_loop_plan(ROOT)
        obligations = (
            compile_create_extension_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_extension_factor_loop_plan(ROOT)
        again = build_create_extension_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_extension_factor_loop_plan(ROOT)
        self.assertEqual(
            "f4990c83940102a9ddf74dd74ad1e7d643edf8a2507716be81ce4744322b70bf",
            plan.obligation_multiset_sha256,
        )
