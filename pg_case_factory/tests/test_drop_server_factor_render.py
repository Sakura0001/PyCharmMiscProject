...
from pg_case_factory.drop_server_factor_render import (
...
class DropServerFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_server_factor_loop_plan(ROOT)
        cls.extension = build_drop_server_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_server_render_"))
        cls.count = generate_drop_server_factor_programs(
    ...
    def test_renders_exactly_2967_files(self) -> None:
        self.assertEqual(2967, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(2967, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPSERVER{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_server(text)
    ...
    def test_bookend_gate_table_creating_cases(self) -> None:
        ...
        self.assertEqual(0, table_cases)
    ...
    def test_primary_target_is_drop_server(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+SERVER\b", region),
                f"no col-0 DROP SERVER in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_server_factor_witness(case, root)
            self.assertIsInstance(w, DropServerFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_server_factor_case(case, root)
        b = render_drop_server_factor_case(case, root)
    ...
    def test_catalog_probe_uses_srvname_column(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            if "pg_foreign_server" not in text:
                continue
            self.assertIn("srvname", text)
            self.assertNotIn("server_name", text.replace("server_name_shape", ""))
            self.assertNotRegex(
                text, r"\bsrv_name\b|\bname\b\s*="
            )
