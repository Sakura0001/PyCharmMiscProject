...
from pg_case_factory.create_server_factor_render import (
    render_create_server_factor_case,
)
from pg_case_factory.create_server_factor_validate import (
...
class CreateServerFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_server_factor_loop_plan(ROOT)
        cls.extension = build_create_server_factor_extension_plan(ROOT)
        cls.all_cases = list(cls.baseline.cases) + list(
            cls.extension.cases
        )
        cls.programs = {
            case.sql_filename: (
                render_create_server_factor_case(case, ROOT)
            )
            for case in cls.all_cases
        }

    def test_validation_passes_for_all_programs(self) -> None:
        report = validate_create_server_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            ROOT,
        ...
        self.assertEqual(43, report.baseline_case_count)
        self.assertEqual(4800, report.extension_case_count)
        self.assertEqual(4843, report.sql_file_count)
