...
_OBLIGATION_MULTISET_SHA256 = (
    "3d4c7cee20466dd2d4f044cc59afe1e39b082e7c2678d7afd5e78cce9f11c267"
)


class DropTransformFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_transform_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_transform_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTransformFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_transform_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTransformFactorLoopPlan)
        ...
        self.assertEqual("DROPTRANSFORM00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTRANSFORM{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTRANSFORM00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTRANSFORM{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptransform_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptransform_{_OBLIGATION_COUNT:05d}_",
