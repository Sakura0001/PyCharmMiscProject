...
_OBLIGATION_MULTISET_SHA256 = (
    "3b6efcb5a8530843c03e1e8cc18bca4745f2990eb055de1511ab875881916903"
)


class DropSequenceFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_sequence_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_sequence_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropSequenceFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_sequence_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropSequenceFactorLoopPlan)
        ...
        self.assertEqual("DROPSEQUENCE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPSEQUENCE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPSEQUENCE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPSEQUENCE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropsequence_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropsequence_{_OBLIGATION_COUNT:05d}_",
