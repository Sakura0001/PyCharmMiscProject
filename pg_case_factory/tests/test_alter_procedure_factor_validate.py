...
from pg_case_factory.alter_procedure_factor_validate import (
    AlterProcedureFactorProgramValidation,
...
# Module-level cache so the 13,198-program render happens once across tests.
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_procedure_factor_extension_plan(ROOT)
        programs: dict[str, str] = {}
        for case in baseline_plan.cases:
            programs[case.sql_filename] = render_alter_procedure_factor_case(
                case, ROOT
            )
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_procedure_factor_case(c, ROOT) for c in chosen
    }
    selected = {c.case_id for c in chosen}
    return programs, selected


class AlterProcedureFactorValidateTest(unittest.TestCase):
    def test_full_program_set_is_conserved(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_procedure_factor_programs(
            baseline_plan, extension_plan, programs, ROOT
        )
        self.assertIsInstance(report, AlterProcedureFactorProgramValidation)
    ...
    def test_missing_program_is_reported(self) -> None:
        baseline_plan = build_alter_procedure_factor_loop_plan(ROOT)
        extension_plan = build_alter_procedure_factor_extension_plan(ROOT)
        programs, selected = _subset_programs(baseline_plan, extension_plan)
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        # 99999 is outside the 0001-13198 case range, so it is genuinely unknown.
        extra = "ALTERPROCEDURE99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERPROCEDURE99999\n"
            "-- primary_obligation_id: AP-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER PROCEDURE bogus() SECURITY INVOKER;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_procedure_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_program = render_alter_procedure_factor_case(target_case, ROOT)
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        # Every extension case must carry a derivation record or the
        # byte witness fails closed on extension_derivation_record_missing.
        programs, baseline_plan, extension_plan = _full_programs()
        report = validate_alter_procedure_factor_programs(
            baseline_plan, extension_plan, programs, ROOT,
        ...
        self.assertTrue(report.passed, report.issues)
        rec = next(
            r for r in report.records if r["is_extension"]
        )
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn("alter_procedure_actual_factor_witness_report", blob)
        self.assertIn("marginal_baseline_plus_bounded_extension_v1", blob)

    def test_byte_mismatch_is_detected(self) -> None:
        baseline_plan = build_alter_procedure_factor_loop_plan(ROOT)
        extension_plan = build_alter_procedure_factor_extension_plan(ROOT)
        programs, selected = _subset_programs(baseline_plan, extension_plan)
        target_case = baseline_plan.cases[0]
        canonical = render_alter_procedure_factor_case(target_case, ROOT)
        tampered = canonical.replace(
            "ALTER PROCEDURE", "ALTER  PROCEDURE", 1
        )
        programs[target_case.sql_filename] = tampered
        selected = selected | {target_case.case_id}
        report = validate_alter_procedure_factor_programs(
            baseline_plan, extension_plan, programs, ROOT,
            selected_case_ids=selected,
