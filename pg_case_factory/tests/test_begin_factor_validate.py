...
from pg_case_factory.begin_factor_extension import (
    build_begin_factor_extension_plan,
)
from pg_case_factory.begin_factor_loop import (
    build_begin_factor_loop_plan,
)
from pg_case_factory.begin_factor_render import (
    generate_begin_factor_programs,
    render_begin_factor_case,
)
from pg_case_factory.begin_factor_validate import (
    BeginFactorProgramValidation,
...
class BeginFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_begin_factor_loop_plan(ROOT)
        cls.extension = build_begin_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="begin_validate_"))
        generate_begin_factor_programs(
    ...
    def test_validation_passes_for_all_1604_files(self) -> None:
        result = validate_begin_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, BeginFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(44, result.baseline_case_count)
        self.assertEqual(1560, result.extension_case_count)
        self.assertEqual(1604, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(1604, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_begin_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(1604, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_begin_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_begin_factor_case(case, ROOT)
        tampered = sql.replace("BEGIN", "BEGINBOGUS", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"BEGIN00001", "BEGIN00002"}
        result = validate_begin_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
