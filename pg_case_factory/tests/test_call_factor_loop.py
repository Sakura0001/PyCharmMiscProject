...
class CallFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_call_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(48) = 49 ; RISK is not applicable.
        self.assertEqual(49, len(rows))
        self.assertEqual(
            {"GRM": 1, "SFV": 48},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(49, len({row.obligation_id for row in rows}))
...
class CallFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_call_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CallFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(49, len(plan.cases))
        self.assertEqual(
            list(range(1, 50)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("CALL00001", plan.cases[0].case_id)
        self.assertEqual("CALL00049", plan.cases[-1].case_id)
        self.assertEqual(
            "CALL00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "CALL00049.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual(
            "call_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            "call_00049_", plan.cases[-1].object_prefix
        )
        self.assertEqual(
            49,
        ...
        self.assertEqual(
            49, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_call_factor_loop_plan(ROOT)
        again = build_call_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_call_factor_loop_plan(ROOT)
        self.assertEqual(
            "4c24c5818a1a80bcd41d391d4ff17d59c367fa182893cd80d76c4304e2dad44e",
            plan.obligation_multiset_sha256,
        )
