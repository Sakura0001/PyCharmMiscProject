...
class CreateTriggerFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_trigger_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, CreateTriggerFactorExtensionPlan
        )
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(92, 6572)), ordinals
        )
        self.assertEqual(
            "CREATETRIGGER00092", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATETRIGGER06571", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATETRIGGER00092.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATETRIGGER06571.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(4860, outcomes["success"])
        self.assertEqual(1620, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_trigger_factor_extension_plan(ROOT)
        plan_b = build_create_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_create_trigger_factor_extension_plan(ROOT)
        self.assertEqual(
            "aecde8d17d798371c96d0ea3fba68d00c42ce00526bf5ad9310e3d64d684373f",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(91, _BASELINE_COUNT)
