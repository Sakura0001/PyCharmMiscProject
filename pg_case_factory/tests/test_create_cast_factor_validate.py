...
from pg_case_factory.create_cast_factor_extension import (
...
from pg_case_factory.create_cast_factor_render import (
    generate_create_cast_factor_programs,
    render_create_cast_factor_case,
)
from pg_case_factory.create_cast_factor_validate import (
...
class CreateCastFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_cast_factor_loop_plan(ROOT)
        cls.extension = build_create_cast_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="ccast_validate_"))
        generate_create_cast_factor_programs(
    ...
    def test_validation_passes_for_all_19215_files(self) -> None:
        result = validate_create_cast_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CreateCastFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(63, result.baseline_case_count)
        self.assertEqual(19152, result.extension_case_count)
        self.assertEqual(19215, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(19215, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_create_cast_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(19215, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_create_cast_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "CREATE CAST", "CREATE SCHEMABOGUS"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATECAST00001", "CREATECAST00002"}
        result = validate_create_cast_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
