...
class AlterTextSearchTemplateFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_text_search_template_factor_extension_plan(
            ROOT
        )
        self.assertIsInstance(
            plan, AlterTextSearchTemplateFactorExtensionPlan
        )
        self.assertEqual(148, len(plan.cases))
        self.assertEqual(148, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(36, 184)), ordinals)
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00036", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00183", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00036.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHTEMPLATE00183.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(100, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_text_search_template_factor_extension_plan(
            ROOT
        )
        plan_b = build_alter_text_search_template_factor_extension_plan(
            ROOT
        )
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        ...
        self.assertEqual(
            "ce982ed730582b663d1debfc1147ebdf2786eb70488c941dde5eddf2ba939e0d",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("revert_rename", "drop_text_search_template"),
            _CLEANUP_MODES,
        )
