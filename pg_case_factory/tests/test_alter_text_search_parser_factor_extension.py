...
class AlterTextSearchParserFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_text_search_parser_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterTextSearchParserFactorExtensionPlan
        )
        self.assertEqual(216, len(plan.cases))
        self.assertEqual(216, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(36, 252)), ordinals
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00036", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00251", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00036.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERTEXTSEARCHPARSER00251.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(36, outcomes["success"])
        self.assertEqual(180, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_text_search_parser_factor_extension_plan(ROOT)
        plan_b = build_alter_text_search_parser_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_text_search_parser_factor_extension_plan(ROOT)
        self.assertEqual(
            "0584a48109904d5e71cbd657cfa165755b77a0ddf9ba4922a45cca62e38dec43",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(35, _BASELINE_COUNT)
        self.assertEqual(20000, _CAP)
        self.assertEqual(
            ("catalog_query_pg_ts_parser", "error_assertion"),
            _VERIFICATION_MODES,
        )
        self.assertEqual(
            ("revert_rename", "drop_text_search_parser"),
            _CLEANUP_MODES,
        )
