...
from pg_case_factory.commit_factor_extension import (
    build_commit_factor_extension_plan,
)
from pg_case_factory.commit_factor_loop import (
    build_commit_factor_loop_plan,
)
from pg_case_factory.commit_factor_render import (
    generate_commit_factor_programs,
    render_commit_factor_case,
)
from pg_case_factory.commit_factor_validate import (
    CommitFactorProgramValidation,
...
class CommitFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_commit_factor_loop_plan(ROOT)
        cls.extension = build_commit_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="commit_validate_"))
        generate_commit_factor_programs(
    ...
    def test_validation_passes_for_all_1844_files(self) -> None:
        result = validate_commit_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, CommitFactorProgramValidation)
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_commit_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_commit_factor_case(case, ROOT)
        tampered = sql.replace("COMMIT", "COMMITBOGUS", 1)
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"COMMIT00001", "COMMIT00002"}
        result = validate_commit_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
