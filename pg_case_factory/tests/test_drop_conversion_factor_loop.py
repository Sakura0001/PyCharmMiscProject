...
_OBLIGATION_MULTISET_SHA256 = (
    "686c5620243e04346de05ee03d44c990bed6566c6779df4b55cfd413f8e13c7d"
)


class DropConversionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_conversion_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_conversion_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropConversionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_conversion_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropConversionFactorLoopPlan)
        ...
        self.assertEqual("DROPCONVERSION00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPCONVERSION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPCONVERSION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPCONVERSION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropconversion_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropconversion_{_OBLIGATION_COUNT:05d}_",
