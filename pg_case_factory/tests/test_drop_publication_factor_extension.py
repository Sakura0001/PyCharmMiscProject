"""Tests for the bounded DROP PUBLICATION post-coverage extension expander.

The marginal factor-value-loop (``drop_publication_factor_loop.py``) is the
...
from pg_case_factory.drop_publication_factor_extension import (
...
from pg_case_factory.drop_publication_factor_loop import (
...
_BASELINE_COUNT = 40
_EXTENSION_COUNT = 2272
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 2312
_SUCCESS_EXTENSIONS = 960
_FAILURE_EXTENSIONS = 1312
_EXTENSION_MULTISET_SHA256 = (
    "cebb2c35afbeaa93725f76263ce3948aceb68e90003c4894c6bde0d1b14f1517"
)
...
_CROSSED_AXES = {
    ...
    "verification_mode": ("pg_publication_catalog", "error_assertion"),
    "cleanup_mode": (
...
_BRANCH_GRAMMAR = {"branch_drop_publication": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_publication": "drop_publication"}
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("subscription_dependency")
        == "has_subscription_dependency"
...
class DropPublicationFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_publication_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropPublicationFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPPUBLICATION00041", first.case_id)
        self.assertEqual(f"DROPPUBLICATION{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPPUBLICATION00041.sql", first.sql_filename)
        self.assertEqual(
            f"DROPPUBLICATION{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droppublication_00041_", first.object_prefix)
        self.assertEqual(
            f"droppublication_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropPublicationFactorExtensionCase)
    ...
    def test_existence_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            ...
            if a["privilege_context"] == "non_owner_no_privilege":
                continue
            if (
                a["publication_existence"] == "publication_not_exists"
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if (
                a["publication_name_shape"] == "non_existing_name"
                and a["if_exists_clause"] == "without_if_exists"
            ):
                continue
            if (
                a["multi_target"] == "multi_target_some_not_exist"
                and a["if_exists_clause"] == "without_if_exists"
            ):
                continue
            if a["subscription_dependency"] != "has_subscription_dependency":
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_publication_catalog"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["drop_publication_cascade"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_publication_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_publication_factor_loop_plan(ROOT)
