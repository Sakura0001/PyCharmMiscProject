...
class UpdateFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_update_factor_extension_plan(ROOT)
        self.assertIsInstance(plan, UpdateFactorExtensionPlan)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(51, 17715)), ordinals
        )
        self.assertEqual("UPDATE00051", plan.cases[0].case_id)
        self.assertEqual("UPDATE17714", plan.cases[-1].case_id)
        self.assertEqual(
            "UPDATE00051.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "UPDATE17714.sql", plan.cases[-1].sql_filename
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_update_factor_extension_plan(ROOT)
        plan_b = build_update_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_update_factor_extension_plan(ROOT)
        self.assertEqual(
            "6584c749c8afd921d4ace71f628bfcfc1788cd7c295fdf314ca98e8af3c137c7",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(50, _BASELINE_COUNT)
