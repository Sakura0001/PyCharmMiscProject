...
from pg_case_factory.create_access_method_factor_loop import (
...
class CreateAccessMethodFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_access_method_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(30) = 31 ; no INV (CREATE ACCESS METHOD
        # registers in pg_am, not pg_class).
        self.assertEqual(31, len(rows))
        self.assertEqual(
            {"GRM": 1, "SFV": 30},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(31, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 18 SFV + 1 GRM = 19 are covered.
...
class CreateAccessMethodFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_access_method_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateAccessMethodFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(31, len(plan.cases))
        self.assertEqual(
            list(range(1, 32)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATEACCESSMETHOD00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEACCESSMETHOD00031", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEACCESSMETHOD00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEACCESSMETHOD00031.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "createaccessmethod_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "createaccessmethod_00031_",
        ...
        self.assertEqual(
            31, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_access_method_factor_loop_plan(ROOT)
        obligations = (
            compile_create_access_method_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_access_method_factor_loop_plan(ROOT)
        again = build_create_access_method_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_access_method_factor_loop_plan(ROOT)
        self.assertEqual(
            "f325f9aac983097641ec854f2fb7bb778ae4e71680955a9ee68e17fbfa49d932",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(19, outcomes["success"])
        self.assertEqual(12, outcomes["expected_failure"])
