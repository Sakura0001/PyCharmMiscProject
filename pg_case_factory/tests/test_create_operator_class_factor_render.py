"""Tests for create_operator_class factor render module."""
...
class TestCreateOperatorClassFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
            build_create_operator_class_factor_loop_plan(_REPO)
        )
        self.extension = (
            build_create_operator_class_factor_extension_plan(_REPO)
        )
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_create_operator_class_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(5622, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [
                f"CREATEOPERATORCLASS{i:05d}.sql"
                for i in range(1, 5623)
            ],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(
                1, count_primary_create_operator_class(sql)
            )
    ...
    def test_no_create_drop_table(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertNotRegex(
                sql, r"(?im)CREATE\s+TABLE"
            )
            self.assertNotRegex(
                sql, r"(?im)DROP\s+TABLE"
            )
    ...
    def test_primary_target_is_create_opclass(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
            ...
            self.assertRegex(
                region,
                r"(?im)CREATE\s+OPERATOR\s+CLASS\b",
            )
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_operator_class_factor_witness(
                case, _REPO
            )
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_operator_class_factor_case(
                case, _REPO
            )
            b = render_create_operator_class_factor_case(
                case, _REPO
            )
    ...
    def test_header_fields(self) -> None:
        sql = (
            self.out / "CREATEOPERATORCLASS00001.sql"
        ).read_text()
        self.assertIn(
            "-- case_id: CREATEOPERATORCLASS00001", sql
        )
