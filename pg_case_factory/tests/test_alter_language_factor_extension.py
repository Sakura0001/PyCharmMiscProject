"""Tests for the bounded ALTER LANGUAGE post-coverage extension expander.

The marginal factor-value-loop (``alter_language_factor_loop.py``) is the
frozen 42-case baseline (GRM 2 + SFV 38 + RISK 2).  This module's expander
adds the bounded post-coverage extension phase: cross-factor combinations of
the positive T1-T4 behaviour axes across the two official synopsis branches
...
_CROSSED_AXES = {
    "statement_branch": ("branch_rename", "branch_owner"),
    "target_action": ("rename", "owner_change"),
    "new_name_shape": (
        "plain_identifier",
        "quoted_identifier",
...
class AlterLanguageFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_alter_language_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, AlterLanguageFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("ALTERLANGUAGE00043", first.case_id)
        self.assertEqual(f"ALTERLANGUAGE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("ALTERLANGUAGE00043.sql", first.sql_filename)
        self.assertEqual(
            f"ALTERLANGUAGE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("alterlanguage_00043_", first.object_prefix)
        self.assertEqual(
            f"alterlanguage_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, AlterLanguageFactorExtensionCase)
            self.assertTrue(case.is_extension, case.case_id)
            # 15 keys: 12 _BASELINE_DEFAULTS (excl. verification/cleanup
            # crossed) + verification_mode + cleanup_mode + the branch's own
            # new_name/new_owner shape keys both present at baseline.
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_alter_language_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_alter_language_factor_loop_plan(ROOT)
