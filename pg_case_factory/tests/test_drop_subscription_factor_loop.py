...
_OBLIGATION_MULTISET_SHA256 = (
    "c12604d15969b47cf765605eb7c53d5dc9c0e88359efb7aece18d0286537440d"
)


class DropSubscriptionFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_subscription_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_subscription_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropSubscriptionFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_subscription_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropSubscriptionFactorLoopPlan)
        ...
        self.assertEqual("DROPSUBSCRIPTION00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPSUBSCRIPTION{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPSUBSCRIPTION00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPSUBSCRIPTION{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "dropsubscription_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"dropsubscription_{_OBLIGATION_COUNT:05d}_",
