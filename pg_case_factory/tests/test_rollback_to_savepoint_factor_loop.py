...
from pg_case_factory.rollback_to_savepoint_factor_loop import (
...
class RollbackToSavepointFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_rollback_to_savepoint_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(43) = 44 ; ROLLBACK TO SAVEPOINT has a single synopsis.
        ...
        self.assertEqual(
            44, len({row.obligation_id for row in rows})
        )
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # ROLLBACK TO SAVEPOINT: 6 canonical values reach the PG target check
        # and are rejected; the remaining 37 SFV + 1 GRM = 38 are covered.
    ...
    def test_chain_behavior_values_are_success_for_rollback_to_savepoint(
        self,
    ) -> None:
        # ROLLBACK TO SAVEPOINT mirrors COMMIT: and_chain / and_no_chain are
        # treated as covered (provisional), not failures.
        self.assertNotIn(
    ...
    def test_outside_transaction_is_failure_for_rollback_to_savepoint(
        self,
    ) -> None:
        # ROLLBACK TO SAVEPOINT outside an active transaction warns 25P01.
        self.assertIn(
...
class RollbackToSavepointFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_rollback_to_savepoint_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, RollbackToSavepointFactorLoopPlan)
        ...
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00044", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ROLLBACKTOSAVEPOINT00044.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "rollbacktosavepoint_00001_",
            plan.cases[0].object_prefix,
        )
        self.assertEqual(
            "rollbacktosavepoint_00044_",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_rollback_to_savepoint_factor_loop_plan(ROOT)
        obligations = (
            compile_rollback_to_savepoint_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_rollback_to_savepoint_factor_loop_plan(ROOT)
        again = build_rollback_to_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_rollback_to_savepoint_factor_loop_plan(ROOT)
        self.assertEqual(
            "82fb2a688a2e8e8bfcea1f026cca36e238c1788608acc4b665de0ac5825d4dd9",
            plan.obligation_multiset_sha256,
        )
