...
from pg_case_factory.validate_drop_tablespace import (
    DropTablespaceFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_tablespace_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropTablespaceFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_tablespace_validate_"))
        generate_drop_tablespace_factor_programs(
        ...
        cls.result = validate_drop_tablespace_factor_programs(
    ...
    def test_validation_passes_for_all_8595_files(self) -> None:
        self.assertIsInstance(
            self.result, DropTablespaceFactorProgramValidation
        )
        self.assertTrue(self.result.passed)
        self.assertEqual(45, self.result.baseline_case_count)
        self.assertEqual(8550, self.result.extension_case_count)
        self.assertEqual(8595, self.result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        self.assertEqual(8595, len(self.result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        self.assertEqual(8595, len(self.result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_tablespace_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        result = validate_drop_tablespace_factor_programs(
            self.baseline,
            self.extension,
            programs,
            ROOT,
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"DROPTABLESPACE00001", "DROPTABLESPACE00002"}
        result = validate_drop_tablespace_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
