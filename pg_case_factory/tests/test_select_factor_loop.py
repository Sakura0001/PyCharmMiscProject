...
from pg_case_factory.select_factor_loop import (
    SelectFactorCase,
...
class SelectFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_select_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(50) = 51 ; SELECT has a single synopsis.
        ...
        self.assertEqual(
            51, len({row.obligation_id for row in rows})
        )
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # SELECT mirrors DELETE: 8 canonical values reach the PG target
        # check and are rejected; the remaining 42 SFV + 1 GRM = 43 are
        # covered.
    ...
    def test_target_relation_state_missing_is_failure_for_select(
        self,
    ) -> None:
        # SELECT FROM a missing relation yields 42P01 (undefined_table).
        self.assertIn(
    ...
    def test_privilege_context_insufficient_is_failure_for_select(
        self,
    ) -> None:
        # SELECT without privilege yields 42501.
        self.assertIn(
...
class SelectFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_select_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, SelectFactorLoopPlan)
        ...
        self.assertEqual(
            "SELECT00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "SELECT00051", plan.cases[-1].case_id
        )
        self.assertEqual(
            "SELECT00001.sql",
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_select_factor_loop_plan(ROOT)
        obligations = (
            compile_select_factor_loop_obligations(ROOT)
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_select_factor_loop_plan(ROOT)
        again = build_select_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_select_factor_loop_plan(ROOT)
        self.assertEqual(
            "9c820f14282e9d3610c8bd91971b791b692a43c11540c492d4747ac2ddb10469",
            plan.obligation_multiset_sha256,
        )
