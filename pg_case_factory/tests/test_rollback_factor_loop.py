...
from pg_case_factory.rollback_factor_loop import (
    RollbackFactorCase,
...
class RollbackFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_rollback_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(43) = 44 ; ROLLBACK has a single synopsis branch.
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # ROLLBACK: 6 canonical values reach the PG target check and are
        # rejected (and_chain / and_no_chain are VALID for ROLLBACK, so they
        # are NOT failures here; outside_transaction IS a failure 25P01);
    ...
    def test_chain_behavior_values_are_success_for_rollback(self) -> None:
        # ROLLBACK supports AND [NO] CHAIN -- these are success, not failure.
        self.assertNotIn(
    ...
    def test_outside_transaction_is_failure_for_rollback(self) -> None:
        # ROLLBACK outside an active transaction warns 25P01.
        self.assertIn(
...
class RollbackFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_rollback_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, RollbackFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(44, len(plan.cases))
        self.assertEqual(
            list(range(1, 45)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual("ROLLBACK00001", plan.cases[0].case_id)
        self.assertEqual("ROLLBACK00044", plan.cases[-1].case_id)
        self.assertEqual(
            "ROLLBACK00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            "ROLLBACK00044.sql", plan.cases[-1].sql_filename
        )
        self.assertEqual(
            "rollback_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            "rollback_00044_", plan.cases[-1].object_prefix
        )
        self.assertEqual(
            44,
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_rollback_factor_loop_plan(ROOT)
        again = build_rollback_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_rollback_factor_loop_plan(ROOT)
        self.assertEqual(
            "c86abaa97268837f274b3105dfdbb885fe2f551188f9b9a6ee157e81c67065ac",
            plan.obligation_multiset_sha256,
        )
