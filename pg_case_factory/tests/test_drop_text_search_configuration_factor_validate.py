...
from pg_case_factory.validate_drop_text_search_configuration import (
    DropTextSearchConfigurationFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_drop_text_search_configuration_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class DropTextSearchConfigurationFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(
                prefix="drop_text_search_configuration_validate_"
            )
        )
        generate_drop_text_search_configuration_factor_programs(
    ...
    def test_no_semantic_witness_mismatches(self) -> None:
        self.assertEqual(
            0, self.result.semantic_witness_mismatch_count
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_drop_text_search_configuration_factor_case(
            case, ROOT
        )
        mutated = (
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {
            ...
            "DROPTEXTSEARCHCONFIGURATION00002",
        }
        result = (
            validate_drop_text_search_configuration_factor_programs(
                self.baseline,
