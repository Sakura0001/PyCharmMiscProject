...
class AlterUserMappingFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="aum_render_"))
        cls.count = generate_alter_user_mapping_factor_programs(
    ...
    def test_renders_exactly_6103_files(self) -> None:
        self.assertEqual(6103, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(6103, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"ALTERUSERMAPPING{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_alter_user_mapping(text)
    ...
    def test_primary_target_is_alter_user_mapping(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*ALTER\s+USER\s+MAPPING\b", region
                ),
                f"no ALTER USER MAPPING in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_alter_user_mapping_factor_witness(case, root)
            self.assertIsInstance(
                w, AlterUserMappingFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_alter_user_mapping_factor_case(case, root)
        b = render_alter_user_mapping_factor_case(case, root)
