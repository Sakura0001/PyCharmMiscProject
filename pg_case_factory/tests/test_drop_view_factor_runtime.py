...
from pg_case_factory.drop_view_factor_runtime import (
...
class DropViewRuntimeConstantsTest(unittest.TestCase):
    def test_constants_are_distinct_from_call(self) -> None:
        ...
        self.assertNotEqual(
            PG18_SOCKET.name, "pgcf-pg18-call-sock-20260821"
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(55504, PG18_PORT)
        self.assertEqual("pgcf_dv", PG18_DATABASE)
...
class DropViewRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_3964_cases(self) -> None:
        sql_dir = Path("/tmp/dropview_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_view_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(3964, len(cases))
        self.assertEqual(3964, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_view_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(3964, ordinals[-1])


class DropViewRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropViewPg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropViewRuntimeError):
            DropViewPg18Runner(timeout_seconds=0)
        with self.assertRaises(DropViewRuntimeError):
            DropViewPg18Runner(timeout_seconds=301)
...
class DropViewComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPVIEW00001"
    ) -> DropViewCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropViewSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropViewSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_view_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropViewCaseRuntimeResult(
            case_id="DROPVIEW00001",
            sql_filename="DROPVIEW00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPVIEW00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropViewRuntimeError):
            DropViewSuiteRun(run_ordinal=3, cases=())
