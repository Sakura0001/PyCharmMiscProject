...
from pg_case_factory.validate_load import (
    LoadFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_load_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class LoadFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="load_validate_"))
        generate_load_factor_programs(
    ...
    def test_validation_passes_for_all_7726_files(self) -> None:
        result = validate_load_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, LoadFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(46, result.baseline_case_count)
        self.assertEqual(7680, result.extension_case_count)
        self.assertEqual(7726, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(7726, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_load_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(7726, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_load_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_load_factor_case(case, ROOT)
        tampered = sql.replace("LOAD", "LOADBOGUS")
