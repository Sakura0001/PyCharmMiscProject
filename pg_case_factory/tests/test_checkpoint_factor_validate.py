...
from pg_case_factory.checkpoint_factor_validate import (
    CheckpointFactorProgramValidation,
...
class CheckpointFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(tempfile.mkdtemp(prefix="ckpt_validate_"))
        generate_checkpoint_factor_programs(
    ...
    def test_validation_passes_for_all_9262_files(self) -> None:
        result = validate_checkpoint_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, CheckpointFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(46, result.baseline_case_count)
        self.assertEqual(9216, result.extension_case_count)
        self.assertEqual(9262, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(9262, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_checkpoint_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(9262, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_checkpoint_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_checkpoint_factor_case(case, ROOT)
        tampered = sql.replace("CHECKPOINT", "CHECKPOINTBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CHECKPOINT00001", "CHECKPOINT00002"}
        result = validate_checkpoint_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
