...
class CreateTextSearchConfigurationFactorExtensionPlanTest(
    unittest.TestCase
):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_text_search_configuration_factor_extension_plan(
            ROOT
        )
        ...
        self.assertEqual(1248, len(plan.cases))
        self.assertEqual(1248, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(55, 1303)), ordinals)
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION00055", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION01302", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION00055.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATETEXTSEARCHCONFIGURATION01302.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(1056, outcomes["expected_failure"])
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        ...
        self.assertEqual(
            "66b56b85d09591d6bec2c377e08298e62cf825e499ab28abd90dfbf2a4680294",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(54, _BASELINE_COUNT)
        self.assertEqual(20000, _CAP)
        self.assertEqual(
            ("catalog_query_pg_ts_config", "error_assertion"),
            _VERIFICATION_MODES,
        )
        self.assertEqual(
            (
