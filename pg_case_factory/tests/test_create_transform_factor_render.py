"""Tests for create_transform factor render module."""
...
class TestCreateTransformFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = (
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(
                1, count_primary_create_transform(sql)
            )
    ...
    def test_primary_target_is_create_transform(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_transform_factor_witness(
                case, _REPO
            )
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_transform_factor_case(
                case, _REPO
            )
            b = render_create_transform_factor_case(
                case, _REPO
            )
    ...
    def test_header_fields(self) -> None:
        sql = (
            self.out / "CREATETRANSFORM00001.sql"
        ).read_text()
        self.assertIn(
            "-- case_id: CREATETRANSFORM00001", sql
        )
    ...
    def test_or_replace_case_present(self) -> None:
        or_replace_found = False
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            region = sql.split(
                "-- primary-target-begin", 1
            ...
            if re.search(
                ...
                or_replace_found = True
                break
        self.assertTrue(
            or_replace_found,
            "at least one OR REPLACE TRANSFORM case expected",
        )
