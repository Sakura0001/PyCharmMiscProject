...
class AlterTablespaceFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.count = generate_alter_tablespace_factor_programs(
    ...
    def test_filename_numbering_is_contiguous(self) -> None:
        ...
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"ALTERTABLESPACE{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_alter_tablespace(text)
    ...
    def test_primary_target_is_alter_tablespace(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*ALTER\s+TABLESPACE\b", region
                ),
                f"no ALTER TABLESPACE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_alter_tablespace_factor_witness(case, root)
            self.assertIsInstance(
                w, AlterTablespaceFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_alter_tablespace_factor_case(case, root)
        b = render_alter_tablespace_factor_case(case, root)
