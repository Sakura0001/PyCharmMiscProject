...
from pg_case_factory.alter_view_factor_render import (
    generate_alter_view_factor_programs,
    render_alter_view_factor_case,
)
from pg_case_factory.alter_view_factor_validate import (
...
class AlterViewFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_alter_view_factor_loop_plan(ROOT)
        cls.extension = build_alter_view_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="aview_validate_"))
        generate_alter_view_factor_programs(
    ...
    def test_validation_passes_for_all_8321_files(self) -> None:
        result = validate_alter_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterViewFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(71, result.baseline_case_count)
        self.assertEqual(8250, result.extension_case_count)
        self.assertEqual(8321, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(8321, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_view_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(8321, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_view_factor_case(case, ROOT)
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_alter_view_factor_case(case, ROOT)
        tampered = sql.replace("ALTER VIEW", "ALTER SCHEMABOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ALTERVIEW00001", "ALTERVIEW00002"}
        result = validate_alter_view_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
