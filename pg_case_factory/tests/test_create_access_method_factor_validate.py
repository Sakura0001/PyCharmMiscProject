...
from pg_case_factory.create_access_method_factor_render import (
    render_create_access_method_factor_case,
)
from pg_case_factory.create_access_method_factor_validate import (
...
class CreateAccessMethodFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_access_method_factor_loop_plan(ROOT)
        cls.extension = (
            build_create_access_method_factor_extension_plan(ROOT)
        )
        cls.all_cases = list(cls.baseline.cases) + list(
            cls.extension.cases
        )
        cls.programs = {
            case.sql_filename: render_create_access_method_factor_case(
                case, ROOT
            )
    ...
    def test_validation_passes_for_all_programs(self) -> None:
        report = validate_create_access_method_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
            ROOT,
        ...
        self.assertEqual(1008, report.extension_case_count)
        self.assertEqual(1039, report.sql_file_count)
