"""Tests for the bounded DROP TEXT SEARCH TEMPLATE post-coverage extension expander.

The marginal factor-value-loop (``drop_text_search_template_factor_loop.py``)
...
from pg_case_factory.drop_text_search_template_factor_extension import (
...
from pg_case_factory.drop_text_search_template_factor_loop import (
...
_BASELINE_COUNT = 36
_EXTENSION_COUNT = 3528
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 3564
_SUCCESS_EXTENSIONS = 720
_FAILURE_EXTENSIONS = 2808
_EXTENSION_MULTISET_SHA256 = (
    "50488c330e60b4a388f8daa3bf02a79675266e6882fe27fcdea7cffa3b87758a"
)
...
_CROSSED_AXES = {
    ...
    "privilege_context": ("non_superuser_session", "superuser_session"),
    ...
    "dependency_status": ("has_dict_dependencies", "no_dependencies"),
    "template_name_shape": (
        "non_existent_name",
        "quoted_id",
        "reserved_word_id",
...
_BRANCH_GRAMMAR = {"branch_drop_ts_template": "branch_1"}
_BRANCH_FIXED_ACTION = {"branch_drop_ts_template": "drop_text_search_template"}
...
class DropTextSearchTemplateFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_text_search_template_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropTextSearchTemplateFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPTEXTSEARCHTEMPLATE00037", first.case_id)
        self.assertEqual(f"DROPTEXTSEARCHTEMPLATE{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPTEXTSEARCHTEMPLATE00037.sql", first.sql_filename)
        self.assertEqual(
            f"DROPTEXTSEARCHTEMPLATE{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("droptextsearchtemplate_00037_", first.object_prefix)
        self.assertEqual(
            f"droptextsearchtemplate_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropTextSearchTemplateFactorExtensionCase)
    ...
    def test_object_missing_negative_fires(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
    ...
    def test_name_missing_negative_fires_without_if_exists(self) -> None:
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            if _privilege_failure_fires(a):
                continue
            if (
                ...
                continue
            if (
                a["template_name_shape"] == "non_existent_name"
                and a["if_exists_clause"] == _IF_EXISTS_OMITTED
            ):
                self.assertEqual("expected_failure", case.outcome, case.case_id)
                self.assertEqual("42704", case.expected_sqlstate, case.case_id)
    ...
    def test_dependency_negative_fires_under_restrict(self) -> None:
        for case in self.plan.cases:
            ...
            if (
                ...
                continue
            if not _template_exists(a):
                continue
            dep = (
                a["dependency_context"] == "dict_using_template"
                or a["dependency_status"] == "has_dict_dependencies"
            )
            if not dep:
                continue
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_text_search_template_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_text_search_template_factor_loop_plan(ROOT)
