...
from pg_case_factory.drop_text_search_configuration_factor_render import (
...
class DropTextSearchConfigurationFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_drop_text_search_configuration_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(
                prefix="drop_text_search_configuration_render_"
            )
        )
        cls.count = generate_drop_text_search_configuration_factor_programs(
    ...
    def test_renders_exactly_1886_files(self) -> None:
        self.assertEqual(1886, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(1886, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"DROPTEXTSEARCHCONFIGURATION{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_drop_text_search_configuration(text)
            self.assertEqual(1, n, f.name)

    def test_table_less_statement_has_no_create_table(self) -> None:
        """DROP TEXT SEARCH CONFIGURATION is table-less: no CREATE TABLE."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_bookend_gate_is_na_for_table_less_statement(self) -> None:
        """Bookend (DROP TABLE IF EXISTS first+last) is N/A: no tables."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            has_create = bool(
            ...
            self.assertTrue(stmts, f"no executable statements in {f.name}")
            self.assertTrue(
                re.match(
    ...
    def test_primary_target_is_drop_text_search_configuration(
        self,
    ) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^DROP\s+TEXT\s+SEARCH\s+CONFIGURATION\b",
                    region,
                ),
                f"no col-0 DROP TEXT SEARCH CONFIGURATION in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_text_search_configuration_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, DropTextSearchConfigurationFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_drop_text_search_configuration_factor_case(case, root)
        b = render_drop_text_search_configuration_factor_case(case, root)
