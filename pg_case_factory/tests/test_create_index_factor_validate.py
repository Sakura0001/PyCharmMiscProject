"""Tests for create_index factor validate module."""
...
from src.pg_case_factory.create_index_factor_validate import (
...
class TestCreateIndexFactorValidate(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_validation_passes_for_all_files(self) -> None:
        report = validate_create_index_factor_programs(
    ...
    def test_baseline_case_count(self) -> None:
        ...
        self.assertEqual(101, report.baseline_case_count)
        self.assertEqual(1908, report.extension_case_count)
        self.assertEqual(2009, report.sql_file_count)

    def test_records_cover_all_cases(self) -> None:
        report = validate_create_index_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(2009, len(report.records))

    def test_sha256_dictionary_complete(self) -> None:
        report = validate_create_index_factor_programs(
            self.baseline, self.extension, self.programs, _REPO
        )
        self.assertEqual(2009, len(report.sql_sha256))

    def test_validation_fails_on_tampered_file(self) -> None:
        filename = "CREATEINDEX00001.sql"
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"CREATEINDEX00001", "CREATEINDEX00102"}
        report = validate_create_index_factor_programs(
            self.baseline, self.extension, self.programs, _REPO,
    ...
    def test_to_dict_structure(self) -> None:
        ...
        self.assertEqual(
            "create_index_actual_factor_witness_report", d["kind"]
        )
