"""Tests for create_group factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_group_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateGroupFactorRuntime(unittest.TestCase):
    ...
    def test_frozen_constants(self) -> None:
        ...
        self.assertEqual(PG18_DATABASE, "pgcf_cgrp")
        self.assertEqual(PG18_SOCKET, Path("/tmp/pgcf-pg18-cgrp-sock-20260820"))
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_group_factor_loop_plan(_REPO)
        ep = build_create_group_factor_extension_plan(_REPO)
        generate_create_group_factor_programs(bp, ep, sql_dir)
        cases, paths = build_create_group_runtime_case_set(_REPO, sql_dir)
        self.assertEqual(426, len(cases))
        self.assertEqual(426, len(paths))
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        ep = build_create_group_factor_extension_plan(_REPO)
        generate_create_group_factor_programs(bp, ep, sql_dir)
        cases, _ = build_create_group_runtime_case_set(_REPO, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))

    def test_runner_construction(self) -> None:
        runner = CreateGroupPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreateGroupRuntimeError):
            CreateGroupPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateGroupRuntimeError):
            CreateGroupPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateGroupRuntimeError):
            CreateGroupSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(CreateGroupRuntimeError):
            CreateGroupSuiteRun(run_ordinal=3, cases=())
