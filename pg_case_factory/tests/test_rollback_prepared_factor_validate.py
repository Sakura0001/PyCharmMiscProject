...
from pg_case_factory.rollback_prepared_factor_extension import (
...
from pg_case_factory.rollback_prepared_factor_render import (
    generate_rollback_prepared_factor_programs,
    render_rollback_prepared_factor_case,
)
from pg_case_factory.validate_rollback_prepared import (
    RollbackPreparedFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_rollback_prepared_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class RollbackPreparedFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_rollback_prepared_factor_loop_plan(ROOT)
        cls.extension = build_rollback_prepared_factor_extension_plan(
            ROOT
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="rprep_validate_"))
        generate_rollback_prepared_factor_programs(
    ...
    def test_validation_passes_for_all_12284_files(self) -> None:
        result = validate_rollback_prepared_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, RollbackPreparedFactorProgramValidation
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_rollback_prepared_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_rollback_prepared_factor_case(case, ROOT)
        tampered = sql.replace("ROLLBACK PREPARED", "ROLLBACKPREPAREDBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ROLLBACKPREPARED00001", "ROLLBACKPREPARED00002"}
        result = validate_rollback_prepared_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
