"""Tests for create_language factor render module."""
...
class TestCreateLanguageFactorRender(unittest.TestCase):
    def setUp(self) -> None:
        self.baseline = build_create_language_factor_loop_plan(_REPO)
        self.extension = build_create_language_factor_extension_plan(_REPO)
        self.out = Path(tempfile.mkdtemp())
        self.count = generate_create_language_factor_programs(
            self.baseline, self.extension, self.out
        )

    def test_file_count(self) -> None:
        self.assertEqual(2540, self.count)

    def test_filename_numbering(self) -> None:
        files = sorted(f.name for f in self.out.iterdir())
        self.assertEqual(
            [f"CREATELANGUAGE{i:05d}.sql" for i in range(1, 2541)],
            files,
        )
    ...
    def test_one_primary_target_per_file(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            self.assertEqual(1, count_primary_create_language(sql))
    ...
    def test_primary_target_is_create_language(self) -> None:
        for f in sorted(self.out.iterdir()):
            sql = f.read_text()
            ...
            self.assertRegex(
                region,
                r"(?im)CREATE\s+(?:OR\s+REPLACE\s+)?(?:TRUSTED\s+)?LANGUAGE\b",
            )
    ...
    def test_or_replace_coverage(self) -> None:
        ...
        for f in sorted(self.out.iterdir()):
            ...
            if re.search(
                ...
                or_replace_count += 1
            if re.search(r"(?im)^\s*CREATE\s+LANGUAGE\b", region):
                plain_count += 1
        self.assertGreater(or_replace_count, 0)
        self.assertGreater(plain_count, 0)
    ...
    def test_witness_resolves(self) -> None:
        ...
        for case in self.baseline.cases[:3]:
            w = resolve_create_language_factor_witness(case, _REPO)
    ...
    def test_deterministic_render(self) -> None:
        for case in self.baseline.cases[:5]:
            a = render_create_language_factor_case(case, _REPO)
            b = render_create_language_factor_case(case, _REPO)
            self.assertEqual(a, b)

    def test_header_fields(self) -> None:
        sql = (self.out / "CREATELANGUAGE00001.sql").read_text()
        self.assertIn("-- case_id: CREATELANGUAGE00001", sql)
