"""Tests for the bounded DROP COLLATION post-coverage extension expander.

The marginal factor-value-loop (``drop_collation_factor_loop.py``) is the
frozen 30-case baseline (GRM 1 + SFV 27 + RISK 2).  This module's expander
...
from pg_case_factory.drop_collation_factor_extension import (
    ...
    DropCollationFactorExtensionError,
    DropCollationFactorExtensionPlan,
...
_TOTAL_COUNT = _BASELINE_COUNT + _EXTENSION_COUNT  # 366
...
# The privilege cluster (privilege_level=non_owner /
# insufficient_privilege) is counted as a single unit, so neither member is
# duplicated here.  The dependency negatives only fire under a non-CASCADE
# drop policy, so they are conditional rather than unconditional value
# matches.
...
# Crossed axes (the union of all per-branch crossed axes + T6).  Every value
# here must be witnessed somewhere in the extension plan.
_CROSSED_AXES = {
    "statement_branch": ("branch_1",),
    "target_action": ("drop_collation",),
    ...
    "dependency_state": (
        "no_dependencies",
        "referenced_by_table_column",
        "referenced_by_index",
    ),
    "privilege_level": ("collation_owner", "non_owner"),
    "verification_mode": (
        "pg_collation_catalog_query",
        "pg_collation_removed_assertion",
    ),
    "cleanup_mode": (
...
_BRANCH_FIXED_ACTION = {"branch_1": "drop_collation"}
...
class DropCollationFactorExtensionPlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_drop_collation_factor_extension_plan(ROOT)

    def test_builds_frozen_extension_count_with_no_truncation(self) -> None:
        self.assertIsInstance(self.plan, DropCollationFactorExtensionPlan)
    ...
    def test_ordinals_continue_after_baseline_with_five_digit_scheme(
        ...
        self.assertEqual("DROPCOLLATION00031", first.case_id)
        self.assertEqual(f"DROPCOLLATION{_TOTAL_COUNT:05d}", last.case_id)
        self.assertEqual("DROPCOLLATION00031.sql", first.sql_filename)
        self.assertEqual(
            f"DROPCOLLATION{_TOTAL_COUNT:05d}.sql", last.sql_filename
        )
        self.assertEqual("dropcollation_00031_", first.object_prefix)
        self.assertEqual(
            f"dropcollation_{_TOTAL_COUNT:05d}_", last.object_prefix
        )

    def test_every_case_is_marked_extension_with_full_assignment(self) -> None:
        for case in self.plan.cases:
            self.assertIsInstance(case, DropCollationFactorExtensionCase)
            self.assertTrue(case.is_extension, case.case_id)
            # 15 keys: 13 _BASELINE_DEFAULTS + verification_mode +
            # cleanup_mode (both crossed over their baseline values).
    ...
    def test_privilege_negative_fires_for_non_owner(self) -> None:
        # privilege_level=non_owner models the must_be_owner (42501)
        # boundary and must never co-occur with another failure unit.
        for case in self.plan.cases:
    ...
    def test_dependency_negative_only_fires_under_restrict(self) -> None:
        # dependency_state=referenced_by_* surfaces 2BPQ1 only when
        # cascade_restrict != CASCADE; under CASCADE the dependent is
        # removed with the collation so the drop succeeds.
        for case in self.plan.cases:
            a = dict(case.factor_assignment)
            dep = a["dependency_state"]
            if dep == "no_dependencies":
                continue
            owner = a["privilege_level"] == "collation_owner"
            if not owner:
                continue
    ...
    def test_verification_and_cleanup_modes_are_both_crossed(self) -> None:
        ...
        self.assertEqual(
            ...
            vm_counts["pg_collation_catalog_query"],
        )
        self.assertEqual(
            _EXTENSION_COUNT // len(_CROSSED_AXES["cleanup_mode"]),
            cm_counts["DROP_COLLATION_CASCADE"],
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        again = build_drop_collation_factor_extension_plan(ROOT)
        self.assertEqual(
            self.plan.extension_multiset_sha256,
    ...
    def test_extensions_do_not_collide_with_baseline_numbering(self) -> None:
        baseline = build_drop_collation_factor_loop_plan(ROOT)
