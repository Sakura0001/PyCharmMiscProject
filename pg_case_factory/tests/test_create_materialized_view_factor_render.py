...
from pg_case_factory.create_materialized_view_factor_loop import (
    build_create_materialized_view_factor_loop_plan,
)
from pg_case_factory.create_materialized_view_factor_render import (
...
class CreateMaterializedViewFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_create_materialized_view_factor_loop_plan(
            ROOT
        )
        cls.extension = (
            build_create_materialized_view_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="cmatview_render_")
        )
        cls.count = generate_create_materialized_view_factor_programs(
    ...
    def test_renders_exactly_12912_files(self) -> None:
        self.assertEqual(12912, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(12912, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(
                f"CREATEMATERIALIZEDVIEW{i:05d}.sql", f.name
            )
    ...
    def test_every_file_has_exactly_one_primary_target(self) -> None:
        ...
        for f in files:
            text = f.read_text()
            n = count_primary_create_materialized_view(text)
            self.assertEqual(1, n, f.name)

    def test_no_file_creates_or_drops_tables(self) -> None:
        # Table-less-exempt: CREATE MATERIALIZED VIEW never emits
        # CREATE TABLE/DROP TABLE (fixture sources are CREATE VIEW), so the
        # shared bookend gate never audits these scripts.
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
    ...
    def test_primary_target_is_create_materialized_view(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(
                    r"(?im)^\s*CREATE\s+MATERIALIZED\s+VIEW\b",
                    region,
                ),
                f"no CREATE MATERIALIZED VIEW in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_create_materialized_view_factor_witness(
                case, root
            )
            self.assertIsInstance(
                w, CreateMaterializedViewFactorWitness
            )
    ...
    def test_render_is_deterministic(self) -> None:
        ...
        a = render_create_materialized_view_factor_case(case, root)
        b = render_create_materialized_view_factor_case(case, root)
    ...
    def test_boundary_select_present(self) -> None:
        # The \set ON_ERROR_STOP on meta-command has no terminator, so a
        # boundary SELECT 1 AS setup_boundary; ensures the CREATE
        # MATERIALIZED VIEW starts its own ';'-segment at column 0.
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertIn("SELECT 1 AS setup_boundary;", text)
