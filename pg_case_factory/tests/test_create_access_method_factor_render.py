...
class CreateAccessMethodFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.loop = build_create_access_method_factor_loop_plan(ROOT)
        cls.ext = build_create_access_method_factor_extension_plan(ROOT)
    ...
    def test_primary_target_cardinality_is_one(self) -> None:
        for case in self.all_cases:
            ...
            self.assertEqual(
                1,
                count_primary_create_access_method(sql),
    ...
    def test_re_render_is_deterministic(self) -> None:
        for case in self.all_cases[:50]:
            first = render_create_access_method_factor_case(case, ROOT)
            second = render_create_access_method_factor_case(case, ROOT)
