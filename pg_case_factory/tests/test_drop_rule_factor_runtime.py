...
from pg_case_factory.drop_rule_factor_runtime import (
...
class DropRuleRuntimeConstantsTest(unittest.TestCase):
...
class DropRuleRuntimeCaseSetTest(unittest.TestCase):
    def test_case_set_has_9266_cases(self) -> None:
        sql_dir = Path("/tmp/drop_rule_probe_leaf")
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, sql_paths = build_drop_rule_runtime_case_set(
            ROOT, sql_dir
        )
        self.assertEqual(9266, len(cases))
        self.assertEqual(9266, len(sql_paths))
        for case in cases:
    ...
    def test_case_set_is_sorted_by_ordinal(self) -> None:
        ...
        if not sql_dir.is_dir():
            self.skipTest("probe leaf directory not found")
        cases, _ = build_drop_rule_runtime_case_set(ROOT, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))
        self.assertEqual(1, ordinals[0])
        self.assertEqual(9266, ordinals[-1])


class DropRuleRunnerConstructionTest(unittest.TestCase):
    def test_runner_constructs_with_defaults(self) -> None:
        runner = DropRulePg18Runner()
    ...
    def test_runner_rejects_invalid_timeout(self) -> None:
        with self.assertRaises(DropRuleRuntimeError):
            DropRulePg18Runner(timeout_seconds=0)
        with self.assertRaises(DropRuleRuntimeError):
            DropRulePg18Runner(timeout_seconds=301)
    ...
    def test_clean_probe_checks_pg_rewrite(self) -> None:
        runner = DropRulePg18Runner()
        query = runner._clean_probe.__code__.co_consts
        joined = " ".join(str(c) for c in query if isinstance(c, str))
        self.assertIn("pg_rewrite", joined)


class DropRuleComparisonTest(unittest.TestCase):
    def _make_result(
        self, case_id: str = "DROPRULE00001"
    ) -> DropRuleCaseRuntimeResult:
    ...
    def test_comparison_passes_on_identical_runs(self) -> None:
        ...
        run1 = DropRuleSuiteRun(run_ordinal=1, cases=(case,))
        run2 = DropRuleSuiteRun(run_ordinal=2, cases=(case,))
        result = compare_drop_rule_runs(run1, run2)
        self.assertTrue(result.passed)
        self.assertEqual(2, result.execution_count)

    def test_comparison_fails_on_sqlstate_mismatch(self) -> None:
        case = DropRuleCaseRuntimeResult(
            case_id="DROPRULE00001",
            sql_filename="DROPRULE00001.sql",
            expected_sqlstate="00000",
        ...
        self.assertIn("DROPRULE00001", result.sqlstate_mismatch_case_ids)

    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(DropRuleRuntimeError):
            DropRuleSuiteRun(run_ordinal=3, cases=())
