"""Tests for create_index factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_index_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateIndexFactorRuntime(unittest.TestCase):
    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55603)
        self.assertEqual(PG18_DATABASE, "pgcf_cinx")
        self.assertEqual(
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_index_factor_loop_plan(_REPO)
        ep = build_create_index_factor_extension_plan(_REPO)
        generate_create_index_factor_programs(bp, ep, sql_dir)
        cases, paths = build_create_index_runtime_case_set(_REPO, sql_dir)
        self.assertEqual(2009, len(cases))
        self.assertEqual(2009, len(paths))
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        ep = build_create_index_factor_extension_plan(_REPO)
        generate_create_index_factor_programs(bp, ep, sql_dir)
        cases, _ = build_create_index_runtime_case_set(_REPO, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))

    def test_runner_construction(self) -> None:
        runner = CreateIndexPg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreateIndexRuntimeError):
            CreateIndexPg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateIndexRuntimeError):
            CreateIndexPg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateIndexRuntimeError):
            CreateIndexSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(CreateIndexRuntimeError):
            CreateIndexSuiteRun(run_ordinal=3, cases=())
