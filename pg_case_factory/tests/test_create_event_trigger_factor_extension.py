...
class CreateEventTriggerFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_create_event_trigger_factor_extension_plan(
            ROOT
        )
        self.assertIsInstance(
            plan, CreateEventTriggerFactorExtensionPlan
        )
        self.assertEqual(6480, len(plan.cases))
        self.assertEqual(6480, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(list(range(63, 6543)), ordinals)
        self.assertEqual(
            "CREATEEVENTTRIGGER00063", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEEVENTTRIGGER06542", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEEVENTTRIGGER00063.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEEVENTTRIGGER06542.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(5400, outcomes["expected_failure"])
    ...
    def test_failure_cases_have_sqlstate_and_reason(self) -> None:
        ...
        for case in plan.cases:
            if case.outcome == "expected_failure":
                ...
                self.assertEqual(
                    "00000", case.expected_sqlstate
                )

    def test_plan_is_deterministic(self) -> None:
        plan_a = build_create_event_trigger_factor_extension_plan(
            ROOT
        )
        plan_b = build_create_event_trigger_factor_extension_plan(
            ROOT
        )
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        ...
        self.assertEqual(
            "95cc2933c5f8264a351747d59f6f5d1d08043a94c10ed7b25946d5b556789706",
            plan.extension_multiset_sha256,
        )

    def test_constants_are_frozen(self) -> None:
        self.assertEqual(62, _BASELINE_COUNT)
