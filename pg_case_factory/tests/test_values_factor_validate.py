...
from pg_case_factory.validate_values import (
...
class ValuesFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="values_validate_")
        )
        generate_values_factor_programs(
    ...
    def test_validation_passes_for_all_17713_files(self) -> None:
        result = validate_values_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, ValuesFactorProgramValidation
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_values_factor_case(case, ROOT)
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
                sql, case
            )
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_values_factor_case(case, ROOT)
        tampered = sql.replace("VALUES", "VALUESBOGUS", 1)
