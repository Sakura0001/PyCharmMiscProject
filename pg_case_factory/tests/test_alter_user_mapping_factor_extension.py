...
class AlterUserMappingFactorExtensionPlanTest(unittest.TestCase):
    def test_extension_case_count_is_frozen(self) -> None:
        plan = build_alter_user_mapping_factor_extension_plan(ROOT)
        self.assertIsInstance(
            plan, AlterUserMappingFactorExtensionPlan
        )
        self.assertEqual(6048, len(plan.cases))
        self.assertEqual(6048, plan.raw_combination_count)
    ...
    def test_case_numbering_is_contiguous_after_baseline(self) -> None:
        ...
        self.assertEqual(
            list(range(56, 6104)), ordinals
        )
        self.assertEqual(
            "ALTERUSERMAPPING00056", plan.cases[0].case_id
        )
        self.assertEqual(
            "ALTERUSERMAPPING06103", plan.cases[-1].case_id
        )
        self.assertEqual(
            "ALTERUSERMAPPING00056.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "ALTERUSERMAPPING06103.sql",
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(4320, outcomes["expected_failure"])
    ...
    def test_plan_is_deterministic(self) -> None:
        plan_a = build_alter_user_mapping_factor_extension_plan(ROOT)
        plan_b = build_alter_user_mapping_factor_extension_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_extension_multiset_sha256(self) -> None:
        plan = build_alter_user_mapping_factor_extension_plan(ROOT)
        self.assertEqual(
            "92aee6582484288e98d4193e84781a7fec630d3ff04a765446ce9eeb7a2c0ceb",
    ...
    def test_constants_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ("revert_option", "drop_user_mapping", "drop_server"),
            _CLEANUP_MODES,
        )
