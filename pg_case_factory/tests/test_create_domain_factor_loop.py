...
class CreateDomainFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_create_domain_factor_loop_obligations(ROOT)
        # GRM(1) + SFV(96) = 97 ; RISK is not applicable.
        self.assertEqual(97, len(rows))
        self.assertEqual(
            {"GRM": 1, "SFV": 96},
            Counter(row.kind for row in rows),
        )
        self.assertEqual(97, len({row.obligation_id for row in rows}))
    ...
    def test_expected_failure_dispositions_are_frozen(self) -> None:
        ...
        # the remaining 72 SFV + 1 GRM = 73 are covered.
...
class CreateDomainFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(
        self,
    ) -> None:
        plan = build_create_domain_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, CreateDomainFactorLoopPlan)
        self.assertEqual(0, len(plan.delegated))
        self.assertEqual(97, len(plan.cases))
        self.assertEqual(
            list(range(1, 98)), [row.ordinal for row in plan.cases]
        )
        self.assertEqual(
            "CREATEDOMAIN00001", plan.cases[0].case_id
        )
        self.assertEqual(
            "CREATEDOMAIN00097", plan.cases[-1].case_id
        )
        self.assertEqual(
            "CREATEDOMAIN00001.sql",
            plan.cases[0].sql_filename,
        )
        self.assertEqual(
            "CREATEDOMAIN00097.sql",
        ...
        self.assertEqual(
            97, len({row.sql_filename for row in plan.cases})
        )
        self.assertTrue(
            all(
    ...
    def test_outcome_driven_by_disposition(self) -> None:
        plan = build_create_domain_factor_loop_plan(ROOT)
        obligations = compile_create_domain_factor_loop_obligations(
            ROOT
        )
    ...
    def test_plan_is_deterministic(self) -> None:
        plan = build_create_domain_factor_loop_plan(ROOT)
        again = build_create_domain_factor_loop_plan(ROOT)
        self.assertEqual(
    ...
    def test_frozen_multiset_sha256(self) -> None:
        plan = build_create_domain_factor_loop_plan(ROOT)
        self.assertEqual(
            "8f35160666b722c669e979edfc1b8d99a309704b459e75bb7b652d566adcac9a",
            plan.obligation_multiset_sha256,
        )
    ...
    def test_outcome_counts_are_frozen(self) -> None:
        ...
        self.assertEqual(24, outcomes["expected_failure"])
