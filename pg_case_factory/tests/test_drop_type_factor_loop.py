...
_OBLIGATION_MULTISET_SHA256 = (
    "633bcb3921b77ccdd874554c2e8145c0533dcdcae75790caadad0544dc125e75"
)


class DropTypeFactorLoopLedgerTest(unittest.TestCase):
    def test_compiles_exact_required_obligation_bag(self) -> None:
        rows = compile_drop_type_factor_loop_obligations(ROOT)
    ...
    def test_obligation_multiset_sha256_is_frozen(self) -> None:
        plan = build_drop_type_factor_loop_plan(ROOT)
        self.assertEqual(
            _OBLIGATION_MULTISET_SHA256,
...
class DropTypeFactorLoopPlanTest(unittest.TestCase):
    def test_one_case_per_local_obligation_with_stable_numbering(self) -> None:
        plan = build_drop_type_factor_loop_plan(ROOT)
        self.assertIsInstance(plan, DropTypeFactorLoopPlan)
        ...
        self.assertEqual("DROPTYPE00001", plan.cases[0].case_id)
        self.assertEqual(
            f"DROPTYPE{_OBLIGATION_COUNT:05d}",
            plan.cases[-1].case_id,
        )
        self.assertEqual(
            "DROPTYPE00001.sql", plan.cases[0].sql_filename
        )
        self.assertEqual(
            f"DROPTYPE{_OBLIGATION_COUNT:05d}.sql",
            plan.cases[-1].sql_filename,
        )
        self.assertEqual(
            "droptype_00001_", plan.cases[0].object_prefix
        )
        self.assertEqual(
            f"droptype_{_OBLIGATION_COUNT:05d}_",
