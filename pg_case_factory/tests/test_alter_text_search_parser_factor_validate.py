...
from pg_case_factory.alter_text_search_parser_factor_extension import (
...
from pg_case_factory.alter_text_search_parser_factor_render import (
    generate_alter_text_search_parser_factor_programs,
    render_alter_text_search_parser_factor_case,
)
from pg_case_factory.alter_text_search_parser_factor_validate import (
    AlterTextSearchParserFactorProgramValidation,
...
class AlterTextSearchParserFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.extension = (
            build_alter_text_search_parser_factor_extension_plan(ROOT)
        )
        cls.tmp = Path(tempfile.mkdtemp(prefix="atsp_validate_"))
        generate_alter_text_search_parser_factor_programs(
    ...
    def test_validation_passes_for_all_251_files(self) -> None:
        result = validate_alter_text_search_parser_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterTextSearchParserFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(35, result.baseline_case_count)
        self.assertEqual(216, result.extension_case_count)
        self.assertEqual(251, result.sql_file_count)
    ...
    def test_sha256_dictionary_is_complete(self) -> None:
        ...
        self.assertEqual(251, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_text_search_parser_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "ALTER TEXT SEARCH PARSER", "ALTER SCHEMABOGUS"
        )
