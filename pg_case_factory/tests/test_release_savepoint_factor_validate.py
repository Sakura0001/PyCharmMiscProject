...
from pg_case_factory.validate_release_savepoint import (
    ReleaseSavepointFactorProgramValidation,
    remove_primary_semantic_locus_but_keep_comments,
    validate_release_savepoint_factor_programs,
)

ROOT = Path(__file__).resolve().parents[1]


class ReleaseSavepointFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.tmp = Path(
            tempfile.mkdtemp(prefix="release_savepoint_validate_")
        )
        generate_release_savepoint_factor_programs(
    ...
    def test_validation_passes_for_all_1844_files(self) -> None:
        result = validate_release_savepoint_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, ReleaseSavepointFactorProgramValidation
        )
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_release_savepoint_factor_case(case, ROOT)
        mutated = (
            remove_primary_semantic_locus_but_keep_comments(
                sql, case
            )
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "RELEASE SAVEPOINT", "RELEASESAVEPOINTBOGUS", 1
        )
