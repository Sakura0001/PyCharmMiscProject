...
from pg_case_factory.insert_factor_extension import (
    build_insert_factor_extension_plan,
)
from pg_case_factory.insert_factor_loop import (
    build_insert_factor_loop_plan,
)
from pg_case_factory.insert_factor_render import (
    InsertFactorWitness,
...
class InsertFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_insert_factor_loop_plan(ROOT)
        cls.extension = build_insert_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="insert_render_"))
        cls.count = generate_insert_factor_programs(
    ...
    def test_renders_exactly_17714_files(self) -> None:
        self.assertEqual(17714, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(17714, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"INSERT{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_insert(text)
    ...
    def test_every_catalog_query_has_order_by_limit_1(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            for m in re.finditer(
                ...
                frag = m.group(0).upper()
                self.assertIn(
                    "ORDER BY", frag, f"missing ORDER BY in {f.name}"
                )
                self.assertIn(
                    "LIMIT 1", frag, f"missing LIMIT 1 in {f.name}"
                )

    def test_primary_target_is_insert(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^INSERT\b", region),
                f"no col-0 INSERT in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_insert_factor_witness(case, root)
            self.assertIsInstance(w, InsertFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_insert_factor_case(case, root)
        b = render_insert_factor_case(case, root)
    ...
    def test_no_quoted_create_table(self) -> None:
        """Gate #7: fixtures never use quoted CREATE TABLE identifiers."""
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            self.assertFalse(
