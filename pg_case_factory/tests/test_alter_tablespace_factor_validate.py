...
from pg_case_factory.alter_tablespace_factor_extension import (
...
from pg_case_factory.alter_tablespace_factor_render import (
    generate_alter_tablespace_factor_programs,
    render_alter_tablespace_factor_case,
)
from pg_case_factory.alter_tablespace_factor_validate import (
    AlterTablespaceFactorProgramValidation,
...
class AlterTablespaceFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_alter_tablespace_factor_loop_plan(ROOT)
        cls.extension = build_alter_tablespace_factor_extension_plan(
            ROOT
        )
    ...
    def test_validation_passes_for_all_6672_files(self) -> None:
        result = validate_alter_tablespace_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(
            result, AlterTablespaceFactorProgramValidation
        )
        self.assertTrue(result.passed)
        self.assertEqual(72, result.baseline_case_count)
        self.assertEqual(6600, result.extension_case_count)
        self.assertEqual(6672, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(6672, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_alter_tablespace_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(6672, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_alter_tablespace_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        ...
        tampered = sql.replace(
            "ALTER TABLESPACE", "ALTER SCHEMABOGUS"
        )
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"ALTERTABLESPACE00001", "ALTERTABLESPACE00002"}
        result = validate_alter_tablespace_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
