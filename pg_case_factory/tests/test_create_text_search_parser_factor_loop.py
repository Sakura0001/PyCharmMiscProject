"""Tests for create_text_search_parser factor_loop obligation ledger."""
...
class TestCreateTextSearchParserFactorLoop(unittest.TestCase):
    def setUp(self) -> None:
        self.plan = build_create_text_search_parser_factor_loop_plan(
            _REPO
        )
    ...
    def test_frozen_multiset_sha256(self) -> None:
        self.assertEqual(
            "ef7dd1caca6b9b30dd7b3c99a2fa6f3ef0dbfbd41991af0734"
            "dcc838d1c71b0f",
            self.plan.obligation_multiset_sha256,
        )

    def test_deterministic(self) -> None:
        other = build_create_text_search_parser_factor_loop_plan(
            _REPO
        )
        self.assertEqual(
            self.plan.obligation_multiset_sha256,
    ...
    def test_case_id_format(self) -> None:
        for case in self.plan.cases:
            self.assertTrue(
                case.case_id.startswith("CREATETEXTSEARCHPARSER")
            )
    ...
    def test_headline_consistency(self) -> None:
        for case in self.plan.cases:
            assign = dict(case.baseline_assignments)
            sb = assign.get(
                "statement_branch", "branch_without_headline"
            )
            hc = assign.get("headline_clause", "omitted")
            if sb == "branch_with_headline":
                self.assertEqual("specified", hc)
            else:
                self.assertEqual("omitted", hc)
