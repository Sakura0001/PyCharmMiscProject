"""Tests for create_table factor runtime module."""

import unittest
from pathlib import Path

from src.pg_case_factory.create_table_factor_runtime import (
    MAX_PARALLELISM,
...
class TestCreateTableFactorRuntime(unittest.TestCase):
    def test_frozen_constants(self) -> None:
        self.assertEqual(PG18_PORT, 55610)
        self.assertEqual(PG18_DATABASE, "pgcf_ctable")
        self.assertEqual(
    ...
    def test_case_set_count(self) -> None:
        ...
        bp = build_create_table_factor_loop_plan(_REPO)
        ep = build_create_table_factor_extension_plan(_REPO)
        generate_create_table_factor_programs(bp, ep, sql_dir)
        cases, paths = build_create_table_runtime_case_set(_REPO, sql_dir)
        self.assertEqual(5948, len(cases))
        self.assertEqual(5948, len(paths))
    ...
    def test_sorted_by_ordinal(self) -> None:
        ...
        ep = build_create_table_factor_extension_plan(_REPO)
        generate_create_table_factor_programs(bp, ep, sql_dir)
        cases, _ = build_create_table_runtime_case_set(_REPO, sql_dir)
        ordinals = [c.ordinal for c in cases]
        self.assertEqual(ordinals, sorted(ordinals))

    def test_runner_construction(self) -> None:
        runner = CreateTablePg18Runner()
        self.assertEqual(runner.port, PG18_PORT)
        self.assertEqual(runner.database, PG18_DATABASE)

    def test_timeout_validation(self) -> None:
        with self.assertRaises(CreateTableRuntimeError):
            CreateTablePg18Runner(timeout_seconds=0)
        with self.assertRaises(CreateTableRuntimeError):
            CreateTablePg18Runner(timeout_seconds=301)
    ...
    def test_suite_run_rejects_invalid_ordinal(self) -> None:
        with self.assertRaises(CreateTableRuntimeError):
            CreateTableSuiteRun(run_ordinal=0, cases=())
        with self.assertRaises(CreateTableRuntimeError):
            CreateTableSuiteRun(run_ordinal=3, cases=())
