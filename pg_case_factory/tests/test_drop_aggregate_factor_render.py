...
from pg_case_factory.drop_aggregate_factor_render import (
...
class DropAggregateFactorRenderTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_drop_aggregate_factor_loop_plan(ROOT)
        cls.extension = build_drop_aggregate_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="drop_aggregate_render_"))
        cls.count = generate_drop_aggregate_factor_programs(
    ...
    def test_renders_exactly_376_files(self) -> None:
        self.assertEqual(376, self.count)
        files = sorted(self.tmp.glob("*.sql"))
        self.assertEqual(376, len(files))

    def test_filename_numbering_is_contiguous(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for i, f in enumerate(files, start=1):
            self.assertEqual(f"DROPAGGREGATE{i:05d}.sql", f.name)

    def test_every_file_has_exactly_one_primary_target(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            n = count_primary_drop_aggregate(text)
            self.assertEqual(1, n, f.name)

    def test_bookend_gate_table_creating_cases(self) -> None:
        """First + last ; -stmts are DROP TABLE IF EXISTS when CREATE TABLE present.

        DROP AGGREGATE is a table-less statement; cases that create a dependent
        fixture still bookend with DROP TABLE IF EXISTS.  Table-less cases
        (no CREATE TABLE) are skipped here -- that is a valid pass.
        """
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
    ...
    def test_primary_target_is_drop_aggregate(self) -> None:
        files = sorted(self.tmp.glob("*.sql"))
        for f in files:
            text = f.read_text()
            ...
            self.assertTrue(
                re.search(r"(?m)^DROP\s+AGGREGATE\b", region),
                f"no col-0 DROP AGGREGATE in {f.name}",
            )
    ...
    def test_witness_resolves_for_every_case(self) -> None:
        ...
        for case in self.baseline.cases[:5]:
            w = resolve_drop_aggregate_factor_witness(case, root)
            self.assertIsInstance(w, DropAggregateFactorWitness)
            self.assertTrue(w.target_sql_fragment)

    def test_render_is_deterministic(self) -> None:
        root = ROOT
        case = self.baseline.cases[0]
        a = render_drop_aggregate_factor_case(case, root)
        b = render_drop_aggregate_factor_case(case, root)
