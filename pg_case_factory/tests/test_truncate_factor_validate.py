...
from pg_case_factory.truncate_factor_render import (
    generate_truncate_factor_programs,
    render_truncate_factor_case,
)
from pg_case_factory.validate_truncate import (
...
class TruncateFactorValidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline = build_truncate_factor_loop_plan(ROOT)
        cls.extension = build_truncate_factor_extension_plan(ROOT)
        cls.tmp = Path(tempfile.mkdtemp(prefix="truncate_validate_"))
        generate_truncate_factor_programs(
    ...
    def test_validation_passes_for_all_5813_files(self) -> None:
        result = validate_truncate_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertIsInstance(result, TruncateFactorProgramValidation)
        self.assertTrue(result.passed)
        self.assertEqual(53, result.baseline_case_count)
        self.assertEqual(5760, result.extension_case_count)
        self.assertEqual(5813, result.sql_file_count)
    ...
    def test_records_cover_all_cases(self) -> None:
        ...
        self.assertEqual(5813, len(result.records))

    def test_sha256_dictionary_is_complete(self) -> None:
        result = validate_truncate_factor_programs(
            self.baseline, self.extension, self.programs, ROOT
        )
        self.assertEqual(5813, len(result.sql_sha256))
    ...
    def test_mutation_helper_replaces_target(self) -> None:
        ...
        sql = render_truncate_factor_case(case, ROOT)
        mutated = (
    ...
    def test_validation_fails_on_tampered_file(self) -> None:
        case = self.baseline.cases[0]
        sql = render_truncate_factor_case(case, ROOT)
        tampered = sql.replace("TRUNCATE", "TRUNCATEBOGUS")
    ...
    def test_selected_case_ids_validates_subset(self) -> None:
        selected = {"TRUNCATE00001", "TRUNCATE00002"}
        result = validate_truncate_factor_programs(
            self.baseline,
            self.extension,
            self.programs,
