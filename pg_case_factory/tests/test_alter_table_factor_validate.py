...
from pg_case_factory.alter_table_factor_validate import (
    AlterTableFactorProgramValidation,
...
def _full_programs() -> tuple[dict[str, str], object, object]:
    ...
    if _FULL_CACHE is None:
        ...
        extension_plan = build_alter_table_factor_extension_plan(ROOT)
...
def _subset_programs(
    ...
    programs = {
        c.sql_filename: render_alter_table_factor_case(c, ROOT)
        for c in chosen
    }
...
class AlterTableFactorValidateTest(unittest.TestCase):
    ...
    def test_missing_program_is_reported(self) -> None:
        baseline_plan = build_alter_table_factor_loop_plan(ROOT)
        extension_plan = (
            build_alter_table_factor_extension_plan(ROOT)
        )
    ...
    def test_unknown_program_is_reported(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        extra = "ALTERTABLE99999.sql"
        programs = {**programs, extra: (
            "-- --------------------------------------------------------\n"
            "-- case_id: ALTERTABLE99999\n"
            "-- primary_obligation_id: ALT-TBL-BOGUS|unknown\n"
            "-- expected_outcome: success\n"
            "-- expected_sqlstate: 00000\n"
            "-- primary-target-begin\n"
            "ALTER TABLE bogus ADD COLUMN col integer;\n"
            "-- primary-target-end\n"
            "SELECT 1;\n"
        )}
        report = validate_alter_table_factor_programs(
    ...
    def test_trace_comments_alone_receive_no_credit(self) -> None:
        ...
        target_case = next(
            row for row in baseline_plan.cases
            if row.outcome == "success"
        )
        target_program = render_alter_table_factor_case(
            target_case, ROOT
        )
    ...
    def test_extension_records_carry_derivation_identity(self) -> None:
        ...
        self.assertTrue(
            rec["primary_obligation_id"].startswith("ALT-TBL-EXT|")
        )
    ...
    def test_validation_report_is_json_serialisable(self) -> None:
        ...
        self.assertIn(
            "alter_table_actual_factor_witness_report", blob
        )
        self.assertIn(
            "marginal_baseline_plus_bounded_extension_v1", blob
        )
    ...
    def test_byte_mismatch_is_detected(self) -> None:
        ...
        canonical = render_alter_table_factor_case(
            target_case, ROOT
        )
    ...
    def test_validation_is_deterministic(self) -> None:
        programs, baseline_plan, extension_plan = _full_programs()
        report_1 = validate_alter_table_factor_programs(
            baseline_plan, extension_plan, programs, ROOT,
            selected_case_ids={
                baseline_plan.cases[0].case_id,
                extension_plan.cases[0].case_id,
        ...
        report_2 = validate_alter_table_factor_programs(
            baseline_plan, extension_plan, programs, ROOT,
            selected_case_ids={
                baseline_plan.cases[0].case_id,
                extension_plan.cases[0].case_id,
        ...
        self.assertEqual(
            report_1.semantic_witness_mismatch_count,
            report_2.semantic_witness_mismatch_count,
        )
        self.assertEqual(
            tuple(r["case_id"] for r in report_1.records),
            tuple(r["case_id"] for r in report_2.records),
        )
