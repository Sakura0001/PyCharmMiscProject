"""Factor-value-loop obligation ledger for PostgreSQL 18.4 SELECT.

SELECT is a PostgreSQL DML read-only query statement with a single official
synopsis branch.  It references a ``pg_class`` relation of kind ``r`` (a base
table) but performs no row mutation: the target state (exists / missing /
wrong_object_type) refers to the queried relation.  Because SELECT is
read-only, no ``transaction_outcome`` factor is declared and there are no
``RISK`` obligations — the obligation structure is ``GRM`` + ``SFV`` only.

The 15 canonical factors and their values are frozen in the shipped
combination matrix ``select.yaml`` (the source of truth);
``factor_value_count`` (the sum of ``len(values)`` across all 15 factors)
equals 50.  The ``GRM`` target action is synthesised from the official
synopsis branch and is NOT counted in ``factor_value_count``.  All 50
...
Hence ``GRM`` 1 + ``SFV`` 50 = 51 local obligations, each materialised as
exactly one regress program.

SELECT mirrors DELETE's failure structure (both target a ``pg_class`` base
table and share the DML 15-factor schema).  The 8 canonical values that
reach the PostgreSQL target check and are rejected are provisionally
mapped to the same SQLSTATEs as DELETE; a later DB phase verifies them on
PG 18.4.  SQLSTATE expectations are provisional.
"""
...
class SelectFactorLoopError(ValueError):
    """Raised when a frozen SELECT obligation input drifts."""


@dataclass(frozen=True)
class SelectGrammarAction:
    """One official target action form of the SELECT synopsis."""
...
@dataclass(frozen=True)
class SelectFactorLoopPlan:
    obligations: tuple[SelectFactorObligation, ...]
    cases: tuple[SelectFactorCase, ...]
    delegated: tuple[SelectFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-select.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-select"
_STATEMENT_REF = "references/statements/dml/query/select.md"

# The single consumer action for all SELECT factors.  Since SELECT has
# only one branch, every factor's consumer is "select".
_REPRESENTATIVE_ACTION = "select"
...
# SELECT mirrors DELETE: both target a pg_class base-table relation, so a
# missing target yields 42P01 (undefined_table) rather than a routine
# sqlstate.  These 8 pairs map 1:1 to select.yaml's documented failure
...
# SELECT targets a ``pg_class`` relation, mirroring DELETE's provisional
# mapping; the DB phase calibrates these on PG 18.4.
...
# Canonical factor -> (tier, values) hardcoded verbatim from select.yaml
# ``factor_contract.factors``.  The sum of len(values) is exactly 50 (the
...
_CANONICAL_FACTORS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ...
    ("with_clause", "T2", ("absent", "non_recursive", "recursive", "merge_cte")),
    (
        "name_shape",
        "T3",
        (
...
def _load_grammar_actions() -> tuple[SelectGrammarAction, ...]:
    """Freeze every SELECT synopsis target action."""
    ...
    rows: tuple[tuple[str, str, str, str], ...] = (
        (
            ...
            (
                ...
                "SELECT [ ALL | DISTINCT [ ON ( ... ) ] ] select_list "
    ...
    if len(actions) != 1:
        raise SelectFactorLoopError("select action count drift")
...
def _canonical_consumer(row) -> str:
    try:
        ...
        raise SelectFactorLoopError(
            f"canonical factor has no consumer action: {row.factor}"
        ) from exc


def _compile_grammar_obligations() -> list[SelectFactorObligation]:
    rows: list[SelectFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            SelectFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SELECT-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise SelectFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations(
    repository_root,  # noqa: ANN001  (Path)
) -> list[SelectFactorObligation]:
    """Compile the 50 SFV obligations from the shipped applicability catalog."""

    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("select")
    if len(catalog_rows) != 50:
        raise SelectFactorLoopError("canonical obligation count drift")
    rows: list[SelectFactorObligation] = []
    for row in catalog_rows:
        ...
        rows.append(
            SelectFactorObligation(
                ordinal=0,
                obligation_id=f"SELECT-SFV|{row.row_id}|{consumer}",
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[SelectFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"select-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
# data_or_query_shape and expression_shape are observable in the WHERE
# predicate, mirroring how DELETE defaulted condition_shape so the
# argument shapes were observable.  SELECT is read-only, so the baseline
# result_shape is "none" (a constant select-list).
...
def _baseline_assignments(
    obligation: SelectFactorObligation,
) -> tuple[tuple[str, str], ...]:
    ...
    if len(assignments) != len(set(assignments)):
        raise SelectFactorLoopError("duplicate baseline factor key")
...
def build_select_factor_loop_plan(
    repository_root,  # noqa: ANN001  (Path; kept for API parity)
) -> SelectFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    obligations = compile_select_factor_loop_obligations(repository_root)
    cases: list[SelectFactorCase] = []
    delegated: list[SelectFactorObligation] = []
    ...
    if len(plan.cases) != 51 or len(plan.delegated) != 0:
        raise SelectFactorLoopError("factor loop plan count drift")
    if len({row.primary_obligation_id for row in plan.cases}) != 51:
        raise SelectFactorLoopError("local obligation mapping drift")
    if len({row.sql_filename for row in plan.cases}) != 51:
        raise SelectFactorLoopError("duplicate SQL filename")
    return plan


def compile_select_factor_loop_obligations(
    repository_root,  # noqa: ANN001  (Path; kept for API parity)
) -> tuple[SelectFactorObligation, ...]:
    ...
    if len(rows) != 51:
        raise SelectFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise SelectFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 50}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise SelectFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise SelectFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 51:
        raise SelectFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise SelectFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "SelectFactorLoopError",
    "SelectGrammarAction",
    "SelectFactorObligation",
    "SelectFactorCase",
    "SelectFactorLoopPlan",
    ...
    "_SFV_FAILURE_VALUES",
    "compile_select_factor_loop_obligations",
