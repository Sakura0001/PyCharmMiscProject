"""Bounded PostgreSQL 18.4 runtime for CREATE ACCESS METHOD factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cam-sock-20260820")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_cam"
...
class CreateAccessMethodRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateAccessMethodCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateAccessMethodSuiteRun:
    run_ordinal: int
    cases: tuple[CreateAccessMethodCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateAccessMethodRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_access_method_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateAccessMethodTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_access_method_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_access_method_runs(
    run_01: CreateAccessMethodSuiteRun,
    run_02: CreateAccessMethodSuiteRun,
    ...
    return CreateAccessMethodTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateAccessMethodRuntimeCase(Protocol):
...
def build_create_access_method_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateAccessMethodRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_create_access_method_factor_loop_plan(root).cases
    extensions = (
        build_create_access_method_factor_extension_plan(root).cases
    )
    cases: list[CreateAccessMethodRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateAccessMethodRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateAccessMethodPg18Runner:
    ...
    def _execute_case(
        self,
        case: CreateAccessMethodRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateAccessMethodRuntimeCase],
        ...
        *,
        ...
        results: list[CreateAccessMethodCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateAccessMethodSuiteRun(
