"""Actual-byte coverage validation for ALTER VIEW factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 71-case baseline +
bounded 8250-case extension) and compares them exactly against the on-disk
...
class AlterViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_view_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_view_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_view_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_view(sql) != 1:
...
def validate_alter_view_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterViewFactorProgramValidation:
    ...
    return AlterViewFactorProgramValidation(
        passed=not issues,
