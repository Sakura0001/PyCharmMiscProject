"""Bounded PostgreSQL 18.4 runtime for DROP TYPE factor programs."""
...
from .drop_type_factor_loop import DropTypeFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dt-sock-20260821")
PG18_PORT = 55503
...
class DropTypeRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTypeCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTypeSuiteRun:
    run_ordinal: int
    cases: tuple[DropTypeCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTypeRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_type_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTypeTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_type_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_type_runs(
    run_01: DropTypeSuiteRun,
    run_02: DropTypeSuiteRun,
) -> DropTypeTwoRunComparison:
    ...
    return DropTypeTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTypeRuntimeCase(Protocol):
...
def build_drop_type_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTypeRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_type_factor_loop_plan(root).cases
    extensions = build_drop_type_factor_extension_plan(root).cases
    cases: list[DropTypeRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTypeRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTypePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTypeRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_type WHERE typname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTypeRuntimeCase,
        sql_path: Path,
    ) -> DropTypeCaseRuntimeResult:
        ...
        return DropTypeCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropTypeRuntimeCase],
        ...
        *,
        ...
        results: list[DropTypeCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_type_runtime_case_set",
    "compare_drop_type_runs",
]
