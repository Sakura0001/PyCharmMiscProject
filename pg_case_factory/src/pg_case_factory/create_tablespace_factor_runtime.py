"""Bounded PostgreSQL 18.4 runtime for CREATE TABLESPACE factor programs."""
...
from .create_tablespace_factor_loop import CreateTablespaceFactorCase
...
class CreateTablespaceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTablespaceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTablespaceSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTablespaceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTablespaceRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_tablespace_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTablespaceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_tablespace_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_tablespace_runs(
    run_01: CreateTablespaceSuiteRun,
    run_02: CreateTablespaceSuiteRun,
    ...
    return CreateTablespaceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTablespaceRuntimeCase(Protocol):
...
def build_create_tablespace_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateTablespaceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_tablespace_factor_loop_plan(root).cases
    extensions = build_create_tablespace_factor_extension_plan(
        root
    ).cases
    cases: list[CreateTablespaceRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateTablespaceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTablespacePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateTablespaceRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateTablespaceRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTablespaceRuntimeCase],
        ...
        *,
        ...
        results: list[CreateTablespaceCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateTablespaceSuiteRun(
...
__all__ = [
    "CreateTablespaceCaseRuntimeResult",
    "CreateTablespacePg18Runner",
    "CreateTablespaceRuntimeCase",
