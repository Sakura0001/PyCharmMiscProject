"""Bounded PostgreSQL 18.4 runtime for DROP EVENT TRIGGER factor programs."""
...
from .drop_event_trigger_factor_loop import DropEventTriggerFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-det-sock-20260820")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_det"
...
class DropEventTriggerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropEventTriggerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropEventTriggerSuiteRun:
    run_ordinal: int
    cases: tuple[DropEventTriggerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropEventTriggerRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_event_trigger_pg18_suite_run",
...
@dataclass(frozen=True)
class DropEventTriggerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_event_trigger_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_event_trigger_runs(
    run_01: DropEventTriggerSuiteRun,
    run_02: DropEventTriggerSuiteRun,
) -> DropEventTriggerTwoRunComparison:
    ...
    return DropEventTriggerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropEventTriggerRuntimeCase(Protocol):
...
def build_drop_event_trigger_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropEventTriggerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_event_trigger_factor_loop_plan(root).cases
    extensions = build_drop_event_trigger_factor_extension_plan(root).cases
    cases: list[DropEventTriggerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropEventTriggerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropEventTriggerPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropEventTriggerRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropEventTriggerRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_event_trigger WHERE evtname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropEventTriggerRuntimeCase,
        sql_path: Path,
    ) -> DropEventTriggerCaseRuntimeResult:
        ...
        return DropEventTriggerCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropEventTriggerSuiteRun(
...
__all__ = [
    "DropEventTriggerCaseRuntimeResult",
    "DropEventTriggerPg18Runner",
    "DropEventTriggerRuntimeCase",
    "DropEventTriggerRuntimeError",
    "DropEventTriggerSuiteRun",
    "DropEventTriggerTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_event_trigger_runtime_case_set",
    "compare_drop_event_trigger_runs",
]
