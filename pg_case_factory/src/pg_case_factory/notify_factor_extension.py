"""Bounded post-coverage extension plan for NOTIFY factor regress.

The baseline :mod:`notify_factor_loop` assigns one local SQL program to
...
NOTIFY is a session-scoped statement that creates no tables, so the
...
target.  NOTIFY is permissive inside transactions, so the only crossed
...
class NotifyFactorExtensionError(ValueError):
    """Raised when NOTIFY extension input drifts."""


@dataclass(frozen=True)
class NotifyFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class NotifyFactorExtensionPlan:
    cases: tuple[NotifyFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "notify_required_factor_value_matrix"

_CONSUMER_ACTION = "notify"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"notify-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_notify_factor_extension_plan(
    repository_root: Path,
) -> NotifyFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_notify_factor_loop_plan(root)
    ...
    cases: list[NotifyFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise NotifyFactorExtensionError(
                    ...
                    cases.append(
                        NotifyFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"NOTIFY{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"notify_{ordinal:05d}_"
                            ),
    ...
    plan = NotifyFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if ordinals != expected_ordinals:
        raise NotifyFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise NotifyFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("NOTIFY-EXT|")
        for case in plan.cases
    ):
        raise NotifyFactorExtensionError(
