"""Actual-byte coverage validation for CREATE OPERATOR CLASS factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 54-case baseline +
...
class CreateOperatorClassFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_operator_class_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_operator_class_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_operator_class(sql) != 1
    ):
...
def validate_create_operator_class_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateOperatorClassFactorProgramValidation:
    ...
    return CreateOperatorClassFactorProgramValidation(
        passed=not issues,
