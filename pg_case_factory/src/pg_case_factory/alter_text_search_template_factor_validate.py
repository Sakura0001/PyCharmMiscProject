"""Actual-byte coverage validation for ALTER TEXT SEARCH TEMPLATE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 35-case baseline +
bounded 148-case extension) and compares them exactly against the
...
class AlterTextSearchTemplateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_template_actual_factor_"
                "witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_text_search_template_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_text_search_template_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_alter_text_search_template(sql) != 1
    ):
...
def validate_alter_text_search_template_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterTextSearchTemplateFactorProgramValidation:
    ...
    return AlterTextSearchTemplateFactorProgramValidation(
        passed=not issues,
