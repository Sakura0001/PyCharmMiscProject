"""Bounded PostgreSQL 18.4 runtime for CREATE OPERATOR factor programs."""
...
from .create_operator_factor_loop import CreateOperatorFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cop-sock-20260820")
PG18_PORT = 55606
PG18_DATABASE = "pgcf_cop"
...
class CreateOperatorRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateOperatorCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateOperatorSuiteRun:
    run_ordinal: int
    cases: tuple[CreateOperatorCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateOperatorRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateOperatorTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_operator_runs(
    run_01: CreateOperatorSuiteRun,
    run_02: CreateOperatorSuiteRun,
) -> CreateOperatorTwoRunComparison:
    ...
    return CreateOperatorTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateOperatorRuntimeCase(Protocol):
...
def build_create_operator_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateOperatorRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_operator_factor_loop_plan(root).cases
    extensions = build_create_operator_factor_extension_plan(root).cases
    cases: list[CreateOperatorRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateOperatorRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateOperatorPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateOperatorRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateOperatorRuntimeCase,
        sql_path: Path,
    ) -> CreateOperatorCaseRuntimeResult:
        ...
        return CreateOperatorCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateOperatorRuntimeCase],
        ...
        *,
        ...
        results: list[CreateOperatorCaseRuntimeResult] = []
        for case in cases:
