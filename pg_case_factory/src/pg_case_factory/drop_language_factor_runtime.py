"""Bounded PostgreSQL 18.4 runtime for DROP LANGUAGE factor programs."""
...
from .drop_language_factor_loop import DropLanguageFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dl-sock-20260820")
PG18_PORT = 55508
PG18_DATABASE = "pgcf_dl"
...
class DropLanguageRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropLanguageCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropLanguageSuiteRun:
    run_ordinal: int
    cases: tuple[DropLanguageCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropLanguageRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_language_pg18_suite_run",
...
@dataclass(frozen=True)
class DropLanguageTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_language_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_language_runs(
    run_01: DropLanguageSuiteRun,
    run_02: DropLanguageSuiteRun,
) -> DropLanguageTwoRunComparison:
    ...
    return DropLanguageTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropLanguageRuntimeCase(Protocol):
...
def build_drop_language_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropLanguageRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_language_factor_loop_plan(root).cases
    extensions = build_drop_language_factor_extension_plan(root).cases
    cases: list[DropLanguageRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropLanguageRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropLanguagePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropLanguageRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropLanguageRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropLanguageRuntimeCase,
        sql_path: Path,
    ) -> DropLanguageCaseRuntimeResult:
        ...
        return DropLanguageCaseRuntimeResult(
...
__all__ = [
    "DropLanguageCaseRuntimeResult",
    "DropLanguagePg18Runner",
    "DropLanguageRuntimeCase",
    "DropLanguageRuntimeError",
    "DropLanguageSuiteRun",
    "DropLanguageTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_language_runtime_case_set",
    "compare_drop_language_runs",
]
