"""Actual-byte coverage validation for REASSIGN OWNED factor-loop SQL.
...
required factor value is still witnessed across ``baseline u extension``.
"""
...
from .reassign_owned_factor_render import (
...
class ReassignOwnedFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reassign_owned_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_reassign_owned_factor_witness(case, repository_root)
    expected_sql = render_reassign_owned_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_reassign_owned(sql) != 1:
...
def validate_reassign_owned_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ReassignOwnedFactorProgramValidation(
        passed=not issues,
