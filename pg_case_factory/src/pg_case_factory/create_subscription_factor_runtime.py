"""Bounded PostgreSQL 18.4 runtime for CREATE SUBSCRIPTION factor programs."""
...
from .create_subscription_factor_loop import CreateSubscriptionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-csub-sock-20260820")
PG18_PORT = 55607
PG18_DATABASE = "pgcf_csub"
...
class CreateSubscriptionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateSubscriptionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateSubscriptionSuiteRun:
    run_ordinal: int
    cases: tuple[CreateSubscriptionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateSubscriptionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_subscription_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateSubscriptionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_subscription_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_subscription_runs(
    run_01: CreateSubscriptionSuiteRun,
    run_02: CreateSubscriptionSuiteRun,
) -> CreateSubscriptionTwoRunComparison:
    ...
    return CreateSubscriptionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateSubscriptionRuntimeCase(Protocol):
...
def build_create_subscription_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateSubscriptionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_subscription_factor_loop_plan(root).cases
    extensions = build_create_subscription_factor_extension_plan(root).cases
    cases: list[CreateSubscriptionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateSubscriptionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateSubscriptionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateSubscriptionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateSubscriptionRuntimeCase,
        sql_path: Path,
    ) -> CreateSubscriptionCaseRuntimeResult:
        ...
        return CreateSubscriptionCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return CreateSubscriptionSuiteRun(
...
__all__ = [
    "CreateSubscriptionCaseRuntimeResult",
    "CreateSubscriptionPg18Runner",
    "CreateSubscriptionRuntimeCase",
    "CreateSubscriptionRuntimeError",
    "CreateSubscriptionSuiteRun",
    "CreateSubscriptionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_subscription_runtime_case_set",
    "compare_create_subscription_runs",
]
