"""Factor-value-loop obligation ledger for PostgreSQL 18.4 DEALLOCATE.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``DEALLOCATE`` (deallocate a prepared statement).  The statement has a
single official synopsis branch: ``DEALLOCATE [ PREPARE ] { name | ALL }``
with two name forms (specific name, ALL) and the optional ``PREPARE``
keyword.  The statement operates on session-scoped prepared statements
 catalogued in ``pg_catalog.pg_prepared_statements`` (not a
``pg_class`` relation), so column/table/relation coverage is
``not_applicable`` and there is no ``INV`` block.

DEALLOCATE fixtures may ``CREATE TABLE`` (to ``PREPARE`` a
...
class DeallocateFactorLoopError(ValueError):
    """Raised when a frozen DEALLOCATE obligation input drifts."""


@dataclass(frozen=True)
class DeallocateGrammarAction:
    """One official target action form of the DEALLOCATE synopsis."""
...
@dataclass(frozen=True)
class DeallocateFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class DeallocateFactorLoopPlan:
    obligations: tuple[DeallocateFactorObligation, ...]
    cases: tuple[DeallocateFactorCase, ...]
    delegated: tuple[DeallocateFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-deallocate.html).
_BRANCH_DEFINE = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-deallocate"

_REPRESENTATIVE_ACTION = "deallocate_statement"
...
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "26000",
        "deallocate_declared_failure_provisional",
    ),
    ("prepared_state", "missing"): (
        "26000",
        "deallocate_missing_statement_provisional",
    ),
    ("prepared_state", "deallocated"): (
        "26000",
        "deallocate_already_deallocated_provisional",
    ),
...
def _load_grammar_actions() -> tuple[DeallocateGrammarAction, ...]:
    """Freeze every DEALLOCATE synopsis target action."""
    ...
    rows: tuple[tuple[str, str, str, str], ...] = (
        (
            ...
            "synopsis-deallocate-statement",
        ),
    )
    actions = [
        DeallocateGrammarAction(
            action_id=action_id,
    ...
    if len(actions) != 1:
        raise DeallocateFactorLoopError(
            "deallocate action count drift"
        )
    return tuple(actions)


def _compile_grammar_obligations() -> (
    list[DeallocateFactorObligation]
):
    rows: list[DeallocateFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            DeallocateFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DEALLOC-GRM|{action.grammar_branch_id}|"
    ...
    for row in catalog_rows:
        ...
        rows.append(
            DeallocateFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DEALLOC-SFV|{row.row_id}|{consumer}"
                ),
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[DeallocateFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"deallocate-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
def _baseline_assignments(
    obligation: DeallocateFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_deallocate_factor_loop_plan(
    repository_root: Path,
) -> DeallocateFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_deallocate_factor_loop_obligations(root)
    cases: list[DeallocateFactorCase] = []
    delegated: list[DeallocateFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            DeallocateFactorCase(
                ordinal=ordinal,
                case_id=f"DEALLOCATE{ordinal:05d}",
                sql_filename=f"DEALLOCATE{ordinal:05d}.sql",
                object_prefix=f"deallocate_{ordinal:05d}_",
...
def compile_deallocate_factor_loop_obligations(
    repository_root: Path,
) -> tuple[DeallocateFactorObligation, ...]:
...
__all__ = [
    ...
    "_SFV_FAILURE_VALUES",
    "compile_deallocate_factor_loop_obligations",
