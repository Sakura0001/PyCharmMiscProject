"""Actual-byte coverage validation for CLUSTER factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 55-case baseline
+ bounded 4864-case extension) and compares them exactly against the
...
class ClusterFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "cluster_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_cluster_factor_witness(case, repository_root)
    expected_sql = render_cluster_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_cluster(sql) != 1:
...
def validate_cluster_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> ClusterFactorProgramValidation:
    ...
    return ClusterFactorProgramValidation(
        passed=not issues,
