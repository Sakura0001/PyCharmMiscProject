"""Bounded PostgreSQL 18.4 runtime for CREATE COLLATION factor programs."""
...
from .create_collation_factor_loop import CreateCollationFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ccol-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_ccol"
...
class CreateCollationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateCollationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateCollationSuiteRun:
    run_ordinal: int
    cases: tuple[CreateCollationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateCollationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_collation_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateCollationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_collation_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_collation_runs(
    run_01: CreateCollationSuiteRun,
    run_02: CreateCollationSuiteRun,
) -> CreateCollationTwoRunComparison:
    ...
    return CreateCollationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateCollationRuntimeCase(Protocol):
...
def build_create_collation_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateCollationRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_collation_factor_loop_plan(root).cases
    extensions = build_create_collation_factor_extension_plan(root).cases
    cases: list[CreateCollationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateCollationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateCollationPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateCollationRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateCollationRuntimeCase,
        sql_path: Path,
    ) -> CreateCollationCaseRuntimeResult:
        ...
        return CreateCollationCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateCollationRuntimeCase],
        ...
        *,
        ...
        results: list[CreateCollationCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateCollationSuiteRun(
