"""Bounded PostgreSQL 18.4 runtime for SET factor programs."""
...
from .set_factor_loop import SetFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-set-sock-20260821")
PG18_PORT = 55622
PG18_DATABASE = "pgcf_set"
...
class SetSuiteRun:
    run_ordinal: int
    cases: tuple[SetCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SetRuntimeError(
...
class SetTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_pg18_two_run_comparison",
            "passed": self.passed,
...
def build_set_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SetRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_set_factor_loop_plan(root).cases
    )
    extensions = (
        build_set_factor_extension_plan(root).cases
    )
    cases: list[SetRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class SetPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise SetRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[SetCaseRuntimeResult] = []
        for case in cases:
