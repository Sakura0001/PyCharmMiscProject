"""Bounded post-coverage cross-factor extension expander for DROP AGGREGATE.

The marginal factor-value-loop (:mod:`drop_aggregate_factor_loop`) is the
...
extension phase allowed by ``drop_aggregate.yaml``
...
The T5 negative factors (``nonexistent_aggregate``, ``signature_mismatch``,
...
* ``branch_1`` — ``aggregate_name_shape`` x ``if_exists_clause`` x
...
class DropAggregateFactorExtensionError(ValueError):
    """Raised when a frozen DROP AGGREGATE extension input drifts."""


@dataclass(frozen=True)
class DropAggregateFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropAggregateFactorExtensionPlan:
    cases: tuple[DropAggregateFactorExtensionCase, ...]
...
# negative factors (nonexistent_aggregate, signature_mismatch,
# dependent_objects_exist, insufficient_privilege, expected_status,
# object_state=not_exists) are intentionally absent from the cross: they are
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    ...
    ``has_dependent_view`` / ``has_dependent_function`` surface
    ``dependent_objects_restricted`` (2BP01) only under a non-CASCADE drop
    policy.  Under CASCADE the dependent is removed along with the aggregate,
...
def _present_failure_pair(
    ...
    The privilege boundary (must be owner of aggregate, 42501) fires before
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-aggregate-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP AGGREGATE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_aggregate_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP AGGREGATE",
...
def build_drop_aggregate_factor_extension_plan(
    repository_root: Path,
) -> DropAggregateFactorExtensionPlan:
    """Build the bounded DROP AGGREGATE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_aggregate_factor_loop_plan(root)
    ...
    cases: list[DropAggregateFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropAggregateFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DAGG-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_aggregate_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropAggregateFactorExtensionPlan(
        cases=cases_tuple,
