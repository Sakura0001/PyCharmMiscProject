"""Actual-byte coverage validation for SELECT factor-loop SQL.
...
from .select_factor_render import (
...
class SelectFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "select_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_select_factor_witness(
        case, repository_root
    )
    expected_sql = render_select_factor_case(
        case, repository_root
    )
...
def validate_select_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return SelectFactorProgramValidation(
        passed=not issues,
