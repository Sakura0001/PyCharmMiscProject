"""Bounded PostgreSQL 18.4 runtime for DROP FOREIGN TABLE factor programs."""
...
from .drop_foreign_table_factor_loop import DropForeignTableFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dft-sock-20260820")
PG18_PORT = 55504
PG18_DATABASE = "pgcf_dft"
...
class DropForeignTableRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropForeignTableCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropForeignTableSuiteRun:
    run_ordinal: int
    cases: tuple[DropForeignTableCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropForeignTableRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_foreign_table_pg18_suite_run",
...
@dataclass(frozen=True)
class DropForeignTableTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_foreign_table_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_foreign_table_runs(
    run_01: DropForeignTableSuiteRun,
    run_02: DropForeignTableSuiteRun,
) -> DropForeignTableTwoRunComparison:
    ...
    return DropForeignTableTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropForeignTableRuntimeCase(Protocol):
...
def build_drop_foreign_table_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropForeignTableRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_foreign_table_factor_loop_plan(root).cases
    extensions = build_drop_foreign_table_factor_extension_plan(root).cases
    cases: list[DropForeignTableRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropForeignTableRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropForeignTablePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropForeignTableRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropForeignTableRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropForeignTableRuntimeCase,
        sql_path: Path,
    ) -> DropForeignTableCaseRuntimeResult:
        ...
        return DropForeignTableCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropForeignTableSuiteRun(
...
__all__ = [
    "DropForeignTableCaseRuntimeResult",
    "DropForeignTablePg18Runner",
    "DropForeignTableRuntimeCase",
    "DropForeignTableRuntimeError",
    "DropForeignTableSuiteRun",
    "DropForeignTableTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_foreign_table_runtime_case_set",
    "compare_drop_foreign_table_runs",
]
