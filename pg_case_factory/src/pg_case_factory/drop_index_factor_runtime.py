"""Bounded PostgreSQL 18.4 runtime for DROP INDEX factor programs."""
...
from .drop_index_factor_loop import DropIndexFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-di-sock-20260820")
PG18_PORT = 55507
PG18_DATABASE = "pgcf_di"
...
class DropIndexRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropIndexCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropIndexSuiteRun:
    run_ordinal: int
    cases: tuple[DropIndexCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropIndexRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_index_pg18_suite_run",
...
@dataclass(frozen=True)
class DropIndexTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_index_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_index_runs(
    run_01: DropIndexSuiteRun,
    run_02: DropIndexSuiteRun,
) -> DropIndexTwoRunComparison:
    ...
    return DropIndexTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropIndexRuntimeCase(Protocol):
...
def build_drop_index_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropIndexRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_index_factor_loop_plan(root).cases
    extensions = build_drop_index_factor_extension_plan(root).cases
    cases: list[DropIndexRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropIndexRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropIndexPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropIndexRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_class WHERE relname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_constraint WHERE conname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropIndexRuntimeCase,
        sql_path: Path,
    ) -> DropIndexCaseRuntimeResult:
        ...
        return DropIndexCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropIndexRuntimeCase],
        ...
        *,
        ...
        results: list[DropIndexCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropIndexCaseRuntimeResult",
    "DropIndexPg18Runner",
    "DropIndexRuntimeCase",
    "DropIndexRuntimeError",
    "DropIndexSuiteRun",
    "DropIndexTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_index_runtime_case_set",
    "compare_drop_index_runs",
]
