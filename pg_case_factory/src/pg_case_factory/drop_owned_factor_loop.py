"""Factor-value-loop obligation ledger for PostgreSQL 18.4 DROP OWNED.

This module compiles the marginal GRM/SFV/RISK obligation ledger for
``DROP OWNED``.  A ``DROP OWNED`` DDL has no ``INV`` block, so the canonical
SFV obligations are derived one-to-one from the shipped applicability matrix:
exactly 41 rows, one local obligation per row.

The official synopsis has a single branch
(``DROP OWNED BY { name | CURRENT_ROLE | CURRENT_USER | SESSION_USER } [, ...]
...
``DROP OWNED`` error that belongs to this statement.
"""
...
class DropOwnedFactorLoopError(ValueError):
    """Raised when a frozen DROP OWNED obligation input drifts."""


@dataclass(frozen=True)
class DropOwnedFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class DropOwnedFactorLoopPlan:
    obligations: tuple[DropOwnedFactorObligation, ...]
    cases: tuple[DropOwnedFactorCase, ...]
    delegated: tuple[DropOwnedFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-drop-owned.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-drop-owned"
...
_SFV_FACTOR_CONSUMER: dict[str, str] = {
    ...
    "multi_role": _REPRESENTATIVE_ACTION,
    ...
    "role_shape": _REPRESENTATIVE_ACTION,
    "verification_mode": _REPRESENTATIVE_ACTION,
}

_STATEMENT_BRANCH_CONSUMER: dict[str, str] = {
    "branch_drop_owned_by": "drop_owned",
    "branch_drop_owned_by_cascade": "drop_owned",
    "branch_drop_owned_by_restrict": "drop_owned",
}

# Canonical (factor, value) pairs that reach the PostgreSQL target check and
# are rejected (best-effort attribution against PG 18.4).  DROP OWNED requires
# superuser or CREATEROLE privilege, so executor_privilege=normal_user_no_privilege
# inherently fails (42501).  role_existence=role_not_exists inherently errors
# (42704) because the target role does not exist.  expected_status=failure is
# the declared failure marker.
...
def _canonical_consumer(row) -> str:
    if row.factor == "statement_branch":
        try:
            ...
            raise DropOwnedFactorLoopError(
...
def _compile_grammar_obligations() -> list[DropOwnedFactorObligation]:
    rows: list[DropOwnedFactorObligation] = []
    for action in _GRAMMAR_ACTIONS:
        rows.append(
            DropOwnedFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPOWNED-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise DropOwnedFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations(
    repository_root: Path,
) -> list[DropOwnedFactorObligation]:
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("drop_owned")
    if len(catalog_rows) != 41:
        raise DropOwnedFactorLoopError("canonical obligation count drift")
    rows: list[DropOwnedFactorObligation] = []
    for row in catalog_rows:
        ...
        rows.append(
            DropOwnedFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPOWNED-SFV|{row.row_id}|{consumer}"
                ),
                kind="SFV",
...
def _compile_risk_obligations() -> list[DropOwnedFactorObligation]:
    return [
        DropOwnedFactorObligation(
            ordinal=0,
            obligation_id=f"DROPOWNED-RISK|transaction|{value}",
            kind="RISK",
            ...
            disposition="covered",
            source_locator=(
                "postgresql-18.4-doc:sql-drop-owned:transactional-ddl"
            ),
        )
...
def _obligation_multiset_sha256(
    rows: tuple[DropOwnedFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"drop-owned-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
# executor_privilege, role_existence, and dependent_objects negatives whose
# SQLSTATEs are resolved here too.
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "42704",
        "drop_owned_declared_failure",
    ),
    ("role_existence", "role_not_exists"): (
        "42704",
        "undefined_role",
...
def _expected_failure_details(
    obligation: DropOwnedFactorObligation,
) -> tuple[str, str]:
    try:
...
_BASELINE_DEFAULTS: dict[str, str] = {
    ...
    "nonexistent_role": "role_does_not_exist",
    "owned_objects_state": "owns_no_objects",
    "privilege_insufficient": "non_superuser_dropping_other_role_objects",
...
def _baseline_assignments(
    ...
    if len(assignments) != len(set(assignments)):
        raise DropOwnedFactorLoopError("duplicate baseline factor key")
    return tuple(sorted(assignments.items()))


def build_drop_owned_factor_loop_plan(
    repository_root: Path,
) -> DropOwnedFactorLoopPlan:
    """Assign one stable local SQL program to every non-delegated obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_drop_owned_factor_loop_obligations(root)
    cases: list[DropOwnedFactorCase] = []
    delegated: list[DropOwnedFactorObligation] = []
    ...
    if len(plan.cases) != 44 or len(plan.delegated) != 0:
        raise DropOwnedFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 44
    ):
        raise DropOwnedFactorLoopError("local obligation mapping drift")
    if len({row.sql_filename for row in plan.cases}) != 44:
        raise DropOwnedFactorLoopError("duplicate SQL filename")
    return plan


_PLAN_CACHE: dict[Path, DropOwnedFactorLoopPlan] = {}


def _build_drop_owned_factor_plan_lazily(
    repository_root: Path,
) -> DropOwnedFactorLoopPlan:
    ...
    if root not in _PLAN_CACHE:
        _PLAN_CACHE[root] = build_drop_owned_factor_loop_plan(root)
    return _PLAN_CACHE[root]


def compile_drop_owned_factor_loop_obligations(
    repository_root: Path,
) -> tuple[DropOwnedFactorObligation, ...]:
    ...
    if len(rows) != 44:
        raise DropOwnedFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise DropOwnedFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 41, "RISK": 2}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise DropOwnedFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise DropOwnedFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 44:
        raise DropOwnedFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise DropOwnedFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "DropOwnedFactorLoopError",
    "DropOwnedFactorObligation",
    "DropOwnedFactorCase",
    "DropOwnedFactorLoopPlan",
    "compile_drop_owned_factor_loop_obligations",
    "build_drop_owned_factor_loop_plan",
    "_obligation_multiset_sha256",
    "_build_drop_owned_factor_plan_lazily",
