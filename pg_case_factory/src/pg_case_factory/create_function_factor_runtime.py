"""Bounded PostgreSQL 18.4 runtime for CREATE FUNCTION factor programs.

The runtime is not executed in the no-DB tickoff phase.  It is provided
here for the DB-phase doublerun, which executes each generated SQL
program against an isolated PostgreSQL 18.4 server (port 55601) and
compares two deterministic runs.
"""
...
from .create_function_factor_loop import CreateFunctionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cfunc-sock-20260820")
PG18_PORT = 55601
PG18_DATABASE = "pgcf_cfunc"
...
class CreateFunctionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateFunctionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateFunctionSuiteRun:
    run_ordinal: int
    cases: tuple[CreateFunctionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateFunctionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_function_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateFunctionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_function_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_function_runs(
    run_01: CreateFunctionSuiteRun,
    run_02: CreateFunctionSuiteRun,
) -> CreateFunctionTwoRunComparison:
    ...
    return CreateFunctionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateFunctionRuntimeCase(Protocol):
...
def build_create_function_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateFunctionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_function_factor_loop_plan(root).cases
    extensions = build_create_function_factor_extension_plan(root).cases
    cases: list[CreateFunctionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateFunctionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateFunctionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateFunctionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateFunctionRuntimeCase,
        sql_path: Path,
    ) -> CreateFunctionCaseRuntimeResult:
        ...
        return CreateFunctionCaseRuntimeResult(
...
__all__ = [
    "CreateFunctionCaseRuntimeResult",
    "CreateFunctionPg18Runner",
    "CreateFunctionRuntimeCase",
    "CreateFunctionRuntimeError",
    "CreateFunctionSuiteRun",
    "CreateFunctionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_function_runtime_case_set",
    "compare_create_function_runs",
]
