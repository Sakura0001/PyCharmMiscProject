"""Bounded PostgreSQL 18.4 runtime for CREATE USER MAPPING factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cum-sock-20260820")
PG18_PORT = 55615
PG18_DATABASE = "pgcf_cum"
...
class CreateUserMappingRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateUserMappingCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateUserMappingSuiteRun:
    run_ordinal: int
    cases: tuple[CreateUserMappingCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateUserMappingRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_user_mapping_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateUserMappingTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_user_mapping_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_user_mapping_runs(
    run_01: CreateUserMappingSuiteRun,
    run_02: CreateUserMappingSuiteRun,
) -> CreateUserMappingTwoRunComparison:
    ...
    return CreateUserMappingTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateUserMappingRuntimeCase(Protocol):
...
def build_create_user_mapping_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateUserMappingRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_user_mapping_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_user_mapping_factor_extension_plan(root).cases
    )
    cases: list[CreateUserMappingRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: CreateUserMappingRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateUserMappingPg18Runner:
    ...
    def _execute_case(
        ...
        case: CreateUserMappingRuntimeCase,
        sql_path: Path,
    ) -> CreateUserMappingCaseRuntimeResult:
        ...
        return CreateUserMappingCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateUserMappingRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> CreateUserMappingSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[CreateUserMappingCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateUserMappingSuiteRun(
...
__all__ = [
    "CreateUserMappingCaseRuntimeResult",
    "CreateUserMappingPg18Runner",
    "CreateUserMappingRuntimeCase",
    "CreateUserMappingRuntimeError",
    "CreateUserMappingSuiteRun",
    "CreateUserMappingTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_user_mapping_runtime_case_set",
    "compare_create_user_mapping_runs",
]
