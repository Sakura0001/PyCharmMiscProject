"""Actual-byte coverage validation for ROLLBACK PREPARED factor-loop SQL.
...
from .rollback_prepared_factor_render import (
...
class RollbackPreparedFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "rollback_prepared_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_rollback_prepared_factor_witness(
        case, repository_root
    )
    expected_sql = render_rollback_prepared_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_rollback_prepared(sql) != 1:
...
def validate_rollback_prepared_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return RollbackPreparedFactorProgramValidation(
        passed=not issues,
