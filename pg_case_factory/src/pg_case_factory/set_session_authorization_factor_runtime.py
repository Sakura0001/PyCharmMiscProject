"""Bounded PostgreSQL 18.4 runtime for SET SESSION AUTHORIZATION factor programs."""
...
PG18_SOCKET = Path(
    "/tmp/pgcf-pg18-set-session-authorization-sock-20260821"
)
PG18_PORT = 55623
PG18_DATABASE = "pgcf_set_session_authorization"
...
class SetSessionAuthorizationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SetSessionAuthorizationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SetSessionAuthorizationSuiteRun:
    run_ordinal: int
    cases: tuple[SetSessionAuthorizationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SetSessionAuthorizationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "set_session_authorization_pg18_suite_run"
            ),
...
@dataclass(frozen=True)
class SetSessionAuthorizationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "set_session_authorization_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_set_session_authorization_runs(
    run_01: SetSessionAuthorizationSuiteRun,
    run_02: SetSessionAuthorizationSuiteRun,
) -> SetSessionAuthorizationTwoRunComparison:
    ...
    return SetSessionAuthorizationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SetSessionAuthorizationRuntimeCase(Protocol):
...
def build_set_session_authorization_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SetSessionAuthorizationRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_set_session_authorization_factor_loop_plan(root).cases
    )
    extensions = (
        build_set_session_authorization_factor_extension_plan(root).cases
    )
    cases: list[SetSessionAuthorizationRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: SetSessionAuthorizationRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SetSessionAuthorizationPg18Runner:
    ...
    def _execute_case(
        ...
        case: SetSessionAuthorizationRuntimeCase,
        sql_path: Path,
    ) -> SetSessionAuthorizationCaseRuntimeResult:
        ...
        return SetSessionAuthorizationCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[SetSessionAuthorizationRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> SetSessionAuthorizationSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[SetSessionAuthorizationCaseRuntimeResult] = []
        for case in cases:
        ...
        return SetSessionAuthorizationSuiteRun(
...
__all__ = [
    "SetSessionAuthorizationCaseRuntimeResult",
    "SetSessionAuthorizationPg18Runner",
    "SetSessionAuthorizationRuntimeCase",
    "SetSessionAuthorizationRuntimeError",
    "SetSessionAuthorizationSuiteRun",
    "SetSessionAuthorizationTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_set_session_authorization_runtime_case_set",
    "compare_set_session_authorization_runs",
]
