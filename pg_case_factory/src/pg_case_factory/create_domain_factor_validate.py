"""Actual-byte coverage validation for CREATE DOMAIN factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 97-case baseline +
bounded 2880-case extension) and compares them exactly against the
...
class CreateDomainFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_domain_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_domain_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_domain_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_domain(sql) != 1:
...
def validate_create_domain_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateDomainFactorProgramValidation:
    ...
    return CreateDomainFactorProgramValidation(
        passed=not issues,
