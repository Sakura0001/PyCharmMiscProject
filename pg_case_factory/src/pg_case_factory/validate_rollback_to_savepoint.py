"""Actual-byte coverage validation for ROLLBACK TO SAVEPOINT factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 44-case baseline +
...
from .rollback_to_savepoint_factor_render import (
...
class RollbackToSavepointFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "rollback_to_savepoint_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_rollback_to_savepoint_factor_witness(
        case, repository_root
    )
    expected_sql = render_rollback_to_savepoint_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_rollback_to_savepoint(sql) != 1
    ):
...
def validate_rollback_to_savepoint_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return RollbackToSavepointFactorProgramValidation(
        passed=not issues,
