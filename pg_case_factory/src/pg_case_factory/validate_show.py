"""Actual-byte coverage validation for SHOW factor-loop SQL.
...
from .show_factor_render import (
    count_primary_show,
...
class ShowFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "show_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_show_factor_witness(
        case, repository_root
    )
    expected_sql = render_show_factor_case(
        case, repository_root
    )
...
def validate_show_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ShowFactorProgramValidation(
        passed=not issues,
