"""Actual-byte coverage validation for DROP TEXT SEARCH TEMPLATE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 36-case baseline +
...
from .drop_text_search_template_factor_render import (
...
class DropTextSearchTemplateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "drop_text_search_template_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_text_search_template_factor_witness(
        case, repository_root
    )
    expected_sql = render_drop_text_search_template_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or (
        count_primary_drop_text_search_template(sql) != 1
    ):
...
def validate_drop_text_search_template_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropTextSearchTemplateFactorProgramValidation(
        passed=not issues,
