"""Actual-byte coverage validation for INSERT factor-loop SQL.
...
from .insert_factor_render import (
...
class InsertFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "insert_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_insert_factor_witness(
        case, repository_root
    )
    expected_sql = render_insert_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_insert(sql) != 1:
...
def validate_insert_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return InsertFactorProgramValidation(
        passed=not issues,
