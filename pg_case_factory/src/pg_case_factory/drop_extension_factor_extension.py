"""Bounded post-coverage cross-factor extension expander for DROP EXTENSION.

The marginal factor-value-loop (:mod:`drop_extension_factor_loop`) is the
...
``nonexistent_extension=extension_missing_no_if_exists`` when
``if_exists_clause=absent``, ``external_dependency_state=has_external_deps``
...
* ``branch_drop_extension`` — ``privilege_level`` x
  ``nonexistent_extension`` x ``if_exists_clause`` x
  ``cascade_restrict`` x ``external_dependency_state``.
"""
...
class DropExtensionFactorExtensionError(ValueError):
    """Raised when a frozen DROP EXTENSION extension input drifts."""


@dataclass(frozen=True)
class DropExtensionFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropExtensionFactorExtensionPlan:
    cases: tuple[DropExtensionFactorExtensionCase, ...]
...
# external_dependency_state=has_external_deps) are crossed here with
...
_BRANCH_GRAMMAR: dict[str, str] = {
    "branch_drop_extension": "branch_1",
}
...
# boundary (42704) fires when the extension is absent and IF EXISTS is absent.
# The dependency boundary (2BP01) fires when external deps exist under
# RESTRICT.
...
_DEPENDENCY_NEGATIVE = ("external_dependency_state", "has_external_deps")
_RESTRICT_VALUES = frozenset({"restrict_default"})
...
def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    """extension_missing fires only when IF EXISTS is absent."""

    return (
        assignment.get("nonexistent_extension")
        == "extension_missing_no_if_exists"
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    """has_external_deps fires only under a non-CASCADE drop policy."""

    return (
        assignment.get("external_dependency_state") == "has_external_deps"
...
def _present_failure_pair(
    ...
    extension is absent and IF EXISTS is absent.  The dependency boundary
    (2BP01) fires last under RESTRICT with external dependencies.
    """
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-extension-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP EXTENSION extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_extension_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP EXTENSION",
...
def build_drop_extension_factor_extension_plan(
    repository_root: Path,
) -> DropExtensionFactorExtensionPlan:
    """Build the bounded DROP EXTENSION post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_extension_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropExtensionFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropExtensionFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPEXTENSION-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_extension_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropExtensionFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "DropExtensionFactorExtensionError",
    "DropExtensionFactorExtensionCase",
    "DropExtensionFactorExtensionPlan",
    "build_drop_extension_factor_extension_plan",
