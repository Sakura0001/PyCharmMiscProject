"""Bounded PostgreSQL 18.4 runtime for DROP TEXT SEARCH CONFIGURATION factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-dtsc-sock-20260821")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dtsc"
...
class DropTextSearchConfigurationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTextSearchConfigurationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTextSearchConfigurationSuiteRun:
    run_ordinal: int
    cases: tuple[DropTextSearchConfigurationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTextSearchConfigurationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_configuration_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTextSearchConfigurationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_configuration_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_text_search_configuration_runs(
    run_01: DropTextSearchConfigurationSuiteRun,
    run_02: DropTextSearchConfigurationSuiteRun,
) -> DropTextSearchConfigurationTwoRunComparison:
    ...
    return DropTextSearchConfigurationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTextSearchConfigurationRuntimeCase(Protocol):
...
def build_drop_text_search_configuration_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[DropTextSearchConfigurationRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_drop_text_search_configuration_factor_loop_plan(root).cases
    extensions = (
        build_drop_text_search_configuration_factor_extension_plan(root).cases
    )
    cases: list[DropTextSearchConfigurationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTextSearchConfigurationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTextSearchConfigurationPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTextSearchConfigurationRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_ts_config WHERE cfgname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTextSearchConfigurationRuntimeCase,
        sql_path: Path,
    ) -> DropTextSearchConfigurationCaseRuntimeResult:
        ...
        return DropTextSearchConfigurationCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropTextSearchConfigurationRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> DropTextSearchConfigurationSuiteRun:
        self.verify_server()
        selected = (
            set(selected_case_ids) if selected_case_ids is not None else None
        )
        results: list[DropTextSearchConfigurationCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropTextSearchConfigurationSuiteRun(
...
__all__ = [
    "DropTextSearchConfigurationCaseRuntimeResult",
    "DropTextSearchConfigurationPg18Runner",
    "DropTextSearchConfigurationRuntimeCase",
    "DropTextSearchConfigurationRuntimeError",
    "DropTextSearchConfigurationSuiteRun",
    "DropTextSearchConfigurationTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_text_search_configuration_runtime_case_set",
    "compare_drop_text_search_configuration_runs",
]
