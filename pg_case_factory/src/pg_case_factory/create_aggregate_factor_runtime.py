"""Bounded PostgreSQL 18.4 runtime for CREATE AGGREGATE factor programs."""
...
from .create_aggregate_factor_loop import CreateAggregateFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cagg-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_cagg"
...
class CreateAggregateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateAggregateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateAggregateSuiteRun:
    run_ordinal: int
    cases: tuple[CreateAggregateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateAggregateRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_aggregate_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateAggregateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_aggregate_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_aggregate_runs(
    run_01: CreateAggregateSuiteRun,
    run_02: CreateAggregateSuiteRun,
    ...
    return CreateAggregateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateAggregateRuntimeCase(Protocol):
...
def build_create_aggregate_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateAggregateRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_create_aggregate_factor_loop_plan(root).cases
    extensions = build_create_aggregate_factor_extension_plan(root).cases
    cases: list[CreateAggregateRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateAggregateRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateAggregatePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateAggregateRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_aggregate a "
    ...
    def _execute_case(
        self,
        case: CreateAggregateRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateAggregateRuntimeCase],
        ...
        *,
        ...
        results: list[CreateAggregateCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateAggregateSuiteRun(
