"""Bounded PostgreSQL 18.4 runtime for DROP DOMAIN factor programs."""
...
from .drop_domain_factor_loop import DropDomainFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ddom-sock-20260820")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_ddom"
...
class DropDomainRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropDomainCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropDomainSuiteRun:
    run_ordinal: int
    cases: tuple[DropDomainCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropDomainRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_domain_pg18_suite_run",
...
@dataclass(frozen=True)
class DropDomainTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_domain_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_domain_runs(
    run_01: DropDomainSuiteRun,
    run_02: DropDomainSuiteRun,
    ...
    return DropDomainTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropDomainRuntimeCase(Protocol):
...
def build_drop_domain_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropDomainRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_domain_factor_loop_plan(root).cases
    extensions = build_drop_domain_factor_extension_plan(root).cases
    cases: list[DropDomainRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropDomainRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropDomainPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropDomainRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[DropDomainRuntimeCase],
        ...
        *,
        ...
        results: list[DropDomainCaseRuntimeResult] = []
        for case in cases:
