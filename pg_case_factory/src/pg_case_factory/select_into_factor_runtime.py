"""Bounded PostgreSQL 18.4 runtime for SELECT INTO factor programs.

Combines the frozen baseline (57 cases) and bounded extension cases into a
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-select-into-sock-20260821")
)
PG18_PORT = 55622
PG18_DATABASE = "pgcf_select_into"
...
class SelectIntoRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SelectIntoCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SelectIntoSuiteRun:
    run_ordinal: int
    cases: tuple[SelectIntoCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SelectIntoRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "select_into_pg18_suite_run",
...
@dataclass(frozen=True)
class SelectIntoTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "select_into_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_select_into_runs(
    run_01: SelectIntoSuiteRun,
    run_02: SelectIntoSuiteRun,
    ...
    return SelectIntoTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SelectIntoRuntimeCase(Protocol):
...
def build_select_into_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SelectIntoRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_select_into_factor_loop_plan(root).cases
    extensions = (
        build_select_into_factor_extension_plan(root).cases
    )
    cases: list[SelectIntoRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: SelectIntoRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SelectIntoPg18Runner:
    ...
    def _execute_case(
        ...
        return SelectIntoCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[SelectIntoRuntimeCase],
        ...
        *,
        ...
        results: list[SelectIntoCaseRuntimeResult] = []
        for case in cases:
