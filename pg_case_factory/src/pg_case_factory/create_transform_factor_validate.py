"""Actual-byte coverage validation for CREATE TRANSFORM factor-loop SQL.
...
class CreateTransformFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_transform_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_transform_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_transform(sql) != 1
    ):
...
def validate_create_transform_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateTransformFactorProgramValidation:
    ...
    return CreateTransformFactorProgramValidation(
        passed=not issues,
