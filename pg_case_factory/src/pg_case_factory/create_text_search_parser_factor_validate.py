"""Actual-byte coverage validation for CREATE TEXT SEARCH PARSER factor-loop SQL.
...
from .create_text_search_parser_factor_render import (
...
class CreateTextSearchParserFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_text_search_parser_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_text_search_parser_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_text_search_parser_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_text_search_parser(sql) != 1
    ):
...
def validate_create_text_search_parser_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateTextSearchParserFactorProgramValidation:
    ...
    return CreateTextSearchParserFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_create_text_search_parser_factor_programs",
]
