"""Factor-value-loop obligation ledger for PostgreSQL 18.4 DROP OPERATOR CLASS.

This module compiles the marginal GRM/SFV/RISK obligation ledger for
``DROP OPERATOR CLASS``.  An ``DROP OPERATOR CLASS`` DDL has no ``INV`` block,
...
``DROP OPERATOR CLASS`` error that belongs to this statement.
"""
...
class DropOperatorClassFactorLoopError(ValueError):
    """Raised when a frozen DROP OPERATOR CLASS obligation input drifts."""


@dataclass(frozen=True)
class DropOperatorClassFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class DropOperatorClassFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropOperatorClassFactorLoopPlan:
    obligations: tuple[DropOperatorClassFactorObligation, ...]
    cases: tuple[DropOperatorClassFactorCase, ...]
    delegated: tuple[DropOperatorClassFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-dropopclass.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-dropopclass"
...
_GRAMMAR_ACTIONS: tuple[_GrammarAction, ...] = (
    _GrammarAction(
        action_id="drop_operator_class",
        grammar_branch_id=_BRANCH_1,
        source_locator=f"{_DOC_SOURCE}:synopsis",
    ),
)

_REPRESENTATIVE_ACTION = "drop_operator_class"
...
_SFV_FACTOR_CONSUMER: dict[str, str] = {
    ...
    "default_opclass_state": _REPRESENTATIVE_ACTION,
    ...
    "auto_family_cleanup": _REPRESENTATIVE_ACTION,
...
# are rejected (best-effort attribution against PG 18.4).  Operator classes
# require ownership, so privilege_context=non_owner and
# privilege_context=insufficient_privilege inherently fail (42501).
# target_object_state=missing inherently errors (42704) because the baseline
# default for if_exists_clause is "absent".  expected_status=failure is the
# declared failure marker.  Kept minimal (Option-A marginal); cross-product
...
def _canonical_consumer(row) -> str:
    if row.factor == "statement_branch":
        try:
            ...
            raise DropOperatorClassFactorLoopError(
...
def _compile_grammar_obligations() -> list[DropOperatorClassFactorObligation]:
    rows: list[DropOperatorClassFactorObligation] = []
    for action in _GRAMMAR_ACTIONS:
        rows.append(
            DropOperatorClassFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPOPERATORCLASS-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise DropOperatorClassFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations(
    repository_root: Path,
) -> list[DropOperatorClassFactorObligation]:
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("drop_operator_class")
    if len(catalog_rows) != 43:
        raise DropOperatorClassFactorLoopError("canonical obligation count drift")
    rows: list[DropOperatorClassFactorObligation] = []
    for row in catalog_rows:
        ...
        rows.append(
            DropOperatorClassFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPOPERATORCLASS-SFV|{row.row_id}|{consumer}"
                ),
                kind="SFV",
...
def _compile_risk_obligations() -> list[DropOperatorClassFactorObligation]:
    return [
        DropOperatorClassFactorObligation(
            ordinal=0,
            obligation_id=f"DROPOPERATORCLASS-RISK|transaction|{value}",
            kind="RISK",
            ...
            disposition="covered",
            source_locator=(
                "postgresql-18.4-doc:sql-dropopclass:transactional-ddl"
            ),
        )
...
def _obligation_multiset_sha256(
    rows: tuple[DropOperatorClassFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"drop-operator-class-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
# privilege_context, target_object_state, and dependency_state negatives whose
# SQLSTATEs are resolved here too.
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "42704",
        "drop_operator_class_declared_failure",
    ),
    ("target_object_state", "missing"): (
        "42704",
        "undefined_operator_class_no_if_exists",
    ),
    ("privilege_context", "non_owner"): (
        "42501",
...
def _expected_failure_details(
    obligation: DropOperatorClassFactorObligation,
) -> tuple[str, str]:
    try:
...
def _baseline_assignments(
    ...
    if len(assignments) != len(set(assignments)):
        raise DropOperatorClassFactorLoopError("duplicate baseline factor key")
    return tuple(sorted(assignments.items()))


def build_drop_operator_class_factor_loop_plan(
    repository_root: Path,
) -> DropOperatorClassFactorLoopPlan:
    """Assign one stable local SQL program to every non-delegated obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_drop_operator_class_factor_loop_obligations(root)
    cases: list[DropOperatorClassFactorCase] = []
    delegated: list[DropOperatorClassFactorObligation] = []
    ...
    plan = DropOperatorClassFactorLoopPlan(
    ...
    if len(plan.cases) != 46 or len(plan.delegated) != 0:
        raise DropOperatorClassFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 46
    ):
        raise DropOperatorClassFactorLoopError("local obligation mapping drift")
    if len({row.sql_filename for row in plan.cases}) != 46:
        raise DropOperatorClassFactorLoopError("duplicate SQL filename")
    return plan


_PLAN_CACHE: dict[Path, DropOperatorClassFactorLoopPlan] = {}


def _build_drop_operator_class_factor_plan_lazily(
    repository_root: Path,
) -> DropOperatorClassFactorLoopPlan:
    ...
    if root not in _PLAN_CACHE:
        _PLAN_CACHE[root] = build_drop_operator_class_factor_loop_plan(root)
    return _PLAN_CACHE[root]


def compile_drop_operator_class_factor_loop_obligations(
    repository_root: Path,
) -> tuple[DropOperatorClassFactorObligation, ...]:
    ...
    if len(rows) != 46:
        raise DropOperatorClassFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise DropOperatorClassFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 43, "RISK": 2}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise DropOperatorClassFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise DropOperatorClassFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 46:
        raise DropOperatorClassFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise DropOperatorClassFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "DropOperatorClassFactorLoopError",
    "DropOperatorClassFactorObligation",
    "DropOperatorClassFactorCase",
    "DropOperatorClassFactorLoopPlan",
    "compile_drop_operator_class_factor_loop_obligations",
    "build_drop_operator_class_factor_loop_plan",
    "_obligation_multiset_sha256",
    "_build_drop_operator_class_factor_plan_lazily",
