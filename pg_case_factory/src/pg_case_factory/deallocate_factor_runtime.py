"""Bounded PostgreSQL 18.4 runtime for DEALLOCATE factor programs."""
...
from .deallocate_factor_loop import DeallocateFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dealloc-sock-20260820")
PG18_PORT = 55617
PG18_DATABASE = "pgcf_dealloc"
...
class DeallocateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DeallocateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DeallocateSuiteRun:
    run_ordinal: int
    cases: tuple[DeallocateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DeallocateRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "deallocate_pg18_suite_run",
...
@dataclass(frozen=True)
class DeallocateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "deallocate_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_deallocate_runs(
    run_01: DeallocateSuiteRun,
    run_02: DeallocateSuiteRun,
) -> DeallocateTwoRunComparison:
    ...
    return DeallocateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DeallocateRuntimeCase(Protocol):
...
def build_deallocate_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[DeallocateRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_deallocate_factor_loop_plan(root).cases
    extensions = (
        build_deallocate_factor_extension_plan(root).cases
    )
    cases: list[DeallocateRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: DeallocateRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DeallocatePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_prepared_statements WHERE name LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: DeallocateRuntimeCase,
        sql_path: Path,
    ) -> DeallocateCaseRuntimeResult:
        ...
        return DeallocateCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DeallocateRuntimeCase],
        ...
        *,
        ...
        results: list[DeallocateCaseRuntimeResult] = []
        for case in cases:
