"""Factor-value-loop obligation ledger for PostgreSQL 18.4 LISTEN.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``LISTEN`` — register the current session to receive notifications on a
channel.  The official synopsis (``LISTEN channel``) has exactly one
...
matrix ``listen.yaml`` (the package binds directly to the matrix, which is
the source of truth); ``factor_value_count == 41``.

LISTEN is a session-scoped statement with no privilege restriction and
creates no tables.  It is permissive: it accepts any channel name string
(even non-existent) and simply registers the session to listen on that
channel.  Two canonical factor values reach a failure surface: the declared
meta-failure ``expected_status=failure`` (malformed syntax — a ``LISTEN``
with no channel name, SQLSTATE ``42601``) and
``payload_shape=boundary_length`` (a channel name exceeding ``NAMEDATALEN``
minus one, SQLSTATE ``08P01``).  The remaining 39 ``SFV`` values plus the
single ``GRM`` form are success-covered.
...
class ListenFactorLoopError(ValueError):
    """Raised when a frozen LISTEN obligation input drifts."""


@dataclass(frozen=True)
class ListenGrammarAction:
    """One official target action form of the LISTEN synopsis."""
...
@dataclass(frozen=True)
class ListenFactorLoopPlan:
    obligations: tuple[ListenFactorObligation, ...]
    cases: tuple[ListenFactorCase, ...]
    delegated: tuple[ListenFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-listen.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-listen"

# The single representative action for all canonical factors.
_REPRESENTATIVE_ACTION = "listen"

_STATEMENT_KEY = "listen"
...
_REFERENCE_PATH = (
    "references/statements/session/notification/listen.md"
)
...
# LISTEN has only one synopsis form, every factor observes on the single
# ``listen`` action.
...
# Canonical (factor, value) pairs hardcoded verbatim from listen.yaml.
...
# LISTEN is permissive: the only reachable failure surfaces are a malformed
# ``LISTEN`` with no channel name (SQLSTATE 42601, syntax_error) and a
...
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "42601",
        "listen_syntax_malformed",
    ),
    ("payload_shape", "boundary_length"): (
        "08P01",
        "listen_channel_too_long",
    ),
}


def _load_grammar_actions() -> tuple[ListenGrammarAction, ...]:
    """Freeze every LISTEN synopsis target action."""
    ...
    if len(actions) != 1:
        raise ListenFactorLoopError("listen action count drift")
...
def _compile_grammar_obligations() -> list[ListenFactorObligation]:
    rows: list[ListenFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            ListenFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"LISTEN-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise ListenFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations() -> list[ListenFactorObligation]:
    """Hardcoded SFV obligations verbatim from listen.yaml."""

    if _FACTOR_VALUE_COUNT != 41:
        raise ListenFactorLoopError("canonical factor value count drift")
    rows: list[ListenFactorObligation] = []
    for factor, value in _CANONICAL_FACTOR_VALUES:
        ...
        rows.append(
            ListenFactorObligation(
                ordinal=0,
                obligation_id=f"LISTEN-SFV|{row_id}|{consumer}",
                kind="SFV",
                factor_key=factor,
                value=value,
...
def _obligation_multiset_sha256(
    rows: tuple[ListenFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"listen-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    LISTEN is permissive inside transactions (no SQLSTATE failure), so
...
def _baseline_assignments(
    obligation: ListenFactorObligation,
) -> tuple[tuple[str, str], ...]:
    ...
    if len(assignments) != len(set(assignments)):
        raise ListenFactorLoopError("duplicate baseline factor key")
...
def build_listen_factor_loop_plan(
    repository_root: Path,
) -> ListenFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_listen_factor_loop_obligations(root)
    cases: list[ListenFactorCase] = []
    delegated: list[ListenFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            ListenFactorCase(
                ...
                sql_filename=f"LISTEN{ordinal:05d}.sql",
                object_prefix=f"listen_{ordinal:05d}_",
    ...
    if len(plan.cases) != 42 or len(plan.delegated) != 0:
        raise ListenFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 42
    ):
        raise ListenFactorLoopError("local obligation mapping drift")
    if (
        len({row.sql_filename for row in plan.cases}) != 42
    ):
        raise ListenFactorLoopError("duplicate SQL filename")
    return plan


def compile_listen_factor_loop_obligations(
    repository_root: Path,
) -> tuple[ListenFactorObligation, ...]:
    """Compile the exact marginal obligation ledger in frozen source order."""

    root = Path(repository_root).resolve(strict=True)
    if not (root / _MATRIX_PATH).is_file():
        raise ListenFactorLoopError(
            f"canonical matrix is missing for listen: {_MATRIX_PATH}"
        )
    unordered_rows = (
    ...
    if len(rows) != 42:
        raise ListenFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise ListenFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 41}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise ListenFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise ListenFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 42:
        raise ListenFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise ListenFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "ListenFactorLoopError",
    "ListenGrammarAction",
    "ListenFactorObligation",
    "ListenFactorCase",
    "ListenFactorLoopPlan",
    ...
    "compile_listen_factor_loop_obligations",
