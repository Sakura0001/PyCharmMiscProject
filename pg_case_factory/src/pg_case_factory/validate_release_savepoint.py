"""Actual-byte coverage validation for RELEASE SAVEPOINT factor-loop SQL.
...
from .release_savepoint_factor_render import (
...
class ReleaseSavepointFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "release_savepoint_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_release_savepoint_factor_witness(
        case, repository_root
    )
    expected_sql = render_release_savepoint_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_release_savepoint(sql) != 1
    ):
...
def validate_release_savepoint_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ReleaseSavepointFactorProgramValidation(
        passed=not issues,
