"""Bounded PostgreSQL 18.4 runtime for DROP CAST factor programs.

This module provides the serial psql executor and two-run comparison
harness for DROP CAST factor-loop SQL.  The runner is never invoked during
the no-DB phase; it is provided so the DB phase can execute the same frozen
SQL files and verify the provisional SQLSTATEs.

DROP CAST removes ``pg_catalog.pg_cast`` rows, not ``pg_class`` relations,
so the clean-state probe inspects ``pg_cast``, ``pg_type``, ``pg_proc`` and
``pg_roles``.
"""
...
from .drop_cast_factor_loop import DropCastFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dcast-sock-20260819")
PG18_PORT = 55490
PG18_DATABASE = "pgcf_dcast"
...
class DropCastRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropCastCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropCastSuiteRun:
    run_ordinal: int
    cases: tuple[DropCastCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropCastRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_cast_pg18_suite_run",
...
@dataclass(frozen=True)
class DropCastTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_cast_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_cast_runs(
    run_01: DropCastSuiteRun,
    run_02: DropCastSuiteRun,
) -> DropCastTwoRunComparison:
    ...
    return DropCastTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropCastRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`DropCastFactorCase` and the
    bounded extension case; the runner only reads the identity + expected-state
    attributes, which both carry.
    """
...
def build_drop_cast_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropCastRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0044) precedes the
    extension (0045-0140) in a stable, human-auditable order.
    """

    from .drop_cast_factor_extension import (
        build_drop_cast_factor_extension_plan,
    )
    from .drop_cast_factor_loop import build_drop_cast_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_drop_cast_factor_loop_plan(root).cases
    extensions = build_drop_cast_factor_extension_plan(root).cases
    cases: list[DropCastRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropCastRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropCastPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropCastRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_type WHERE typname LIKE '"
            + literal
            + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise DropCastRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: DropCastRuntimeCase,
        sql_path: Path,
    ) -> DropCastCaseRuntimeResult:
        ...
        return DropCastCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropCastRuntimeCase],
        ...
        *,
        ...
        results: list[DropCastCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_cast_runtime_case_set",
    "compare_drop_cast_runs",
]
