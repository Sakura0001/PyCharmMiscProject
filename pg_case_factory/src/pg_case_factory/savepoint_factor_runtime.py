"""Bounded PostgreSQL 18.4 runtime for SAVEPOINT factor programs."""
...
class SavepointRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SavepointCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SavepointSuiteRun:
    run_ordinal: int
    cases: tuple[SavepointCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SavepointRuntimeError(
...
@dataclass(frozen=True)
class SavepointTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "savepoint_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_savepoint_runs(
    run_01: SavepointSuiteRun,
    run_02: SavepointSuiteRun,
    ...
    return SavepointTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SavepointRuntimeCase(Protocol):
...
def build_savepoint_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SavepointRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_savepoint_factor_loop_plan(root).cases
    extensions = (
        build_savepoint_factor_extension_plan(root).cases
    )
    cases: list[SavepointRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: SavepointRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SavepointPg18Runner:
    ...
    def run_cases(
        self,
        cases: Sequence[SavepointRuntimeCase],
        ...
        *,
        ...
        results: list[SavepointCaseRuntimeResult] = []
        for case in cases:
