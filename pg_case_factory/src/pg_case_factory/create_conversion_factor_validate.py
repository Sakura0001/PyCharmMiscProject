"""Actual-byte coverage validation for CREATE CONVERSION factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 66-case baseline +
...
required factor value is still witnessed across
...
class CreateConversionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_conversion_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_conversion_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_conversion_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_conversion(sql) != 1:
...
def validate_create_conversion_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateConversionFactorProgramValidation(
        passed=not issues,
