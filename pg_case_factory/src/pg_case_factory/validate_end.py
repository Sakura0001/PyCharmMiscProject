"""Actual-byte coverage validation for END factor-loop SQL.
...
class EndFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "end_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_end_factor_witness(
        case, repository_root
    )
    expected_sql = render_end_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_end(sql) != 1:
...
def validate_end_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return EndFactorProgramValidation(
        passed=not issues,
