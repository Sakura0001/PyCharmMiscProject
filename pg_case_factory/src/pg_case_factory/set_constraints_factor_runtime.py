"""Bounded PostgreSQL 18.4 runtime for SET CONSTRAINTS factor programs."""
...
from .set_constraints_factor_loop import SetConstraintsFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-setconstraints-sock-20260821")
PG18_PORT = 55622
PG18_DATABASE = "pgcf_setconstraints"
...
class SetConstraintsRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SetConstraintsCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SetConstraintsSuiteRun:
    run_ordinal: int
    cases: tuple[SetConstraintsCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SetConstraintsRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_constraints_pg18_suite_run",
...
@dataclass(frozen=True)
class SetConstraintsTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_constraints_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_set_constraints_runs(
    run_01: SetConstraintsSuiteRun,
    run_02: SetConstraintsSuiteRun,
) -> SetConstraintsTwoRunComparison:
    ...
    return SetConstraintsTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SetConstraintsRuntimeCase(Protocol):
...
def build_set_constraints_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SetConstraintsRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_set_constraints_factor_loop_plan(root).cases
    )
    extensions = (
        build_set_constraints_factor_extension_plan(root).cases
    )
    cases: list[SetConstraintsRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: SetConstraintsRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SetConstraintsPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise SetConstraintsRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        case: SetConstraintsRuntimeCase,
        sql_path: Path,
    ) -> SetConstraintsCaseRuntimeResult:
        ...
        return SetConstraintsCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[SetConstraintsCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "SetConstraintsCaseRuntimeResult",
    "SetConstraintsPg18Runner",
    "SetConstraintsRuntimeCase",
    "SetConstraintsRuntimeError",
    "SetConstraintsSuiteRun",
    "SetConstraintsTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_set_constraints_runtime_case_set",
    "compare_set_constraints_runs",
]
