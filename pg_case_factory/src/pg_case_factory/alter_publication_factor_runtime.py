"""Bounded PostgreSQL 18.4 runtime for ALTER PUBLICATION factor programs."""
...
from .alter_publication_factor_loop import AlterPublicationFactorCase
...
class AlterPublicationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterPublicationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterPublicationSuiteRun:
    run_ordinal: int
    cases: tuple[AlterPublicationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterPublicationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_publication_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterPublicationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_publication_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_publication_runs(
    run_01: AlterPublicationSuiteRun,
    run_02: AlterPublicationSuiteRun,
) -> AlterPublicationTwoRunComparison:
    ...
    return AlterPublicationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterPublicationRuntimeCase(Protocol):
...
def build_alter_publication_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterPublicationRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_publication_factor_loop_plan(root).cases
    extensions = build_alter_publication_factor_extension_plan(root).cases
    cases: list[AlterPublicationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterPublicationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterPublicationPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterPublicationRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            "UNION ALL SELECT 1 FROM pg_catalog.pg_publication WHERE pubname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _execute_case(
        self, case: AlterPublicationRuntimeCase, sql_path: Path
    ...
    def run_cases(
        self, cases: Sequence[AlterPublicationRuntimeCase],
        ...
        results: list[AlterPublicationCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterPublicationSuiteRun(
...
__all__ = [
    ...
    "AlterPublicationRuntimeCase",
    "AlterPublicationRuntimeError",
    "AlterPublicationSuiteRun",
    "AlterPublicationTwoRunComparison",
    "MAX_PARALLELISM",
