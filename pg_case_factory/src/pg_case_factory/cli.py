...
def _cmd_run_execute(arguments: argparse.Namespace) -> int:
    ...
    validate_basic_endpoint_identity(post_identity)
...
def _resolve_formal_differential_settings(
    ...
    if profile is not None:
        expected = {
            "reference_service": profile.reference.service,
            "reference_database": profile.reference.database,
            "dut_service": profile.dut.service,
            "dut_database": profile.dut.database,
            "psql": profile.runner.executable,
            "timeout": profile.runner.timeout_seconds,
            "expected_reference_system_identifier": (
                profile.reference.expected_system_identifier
            ),
            "expected_dut_system_identifier": profile.dut.expected_system_identifier,
    ...
    psql = arguments.psql if arguments.psql is not None else "psql"
    if not isinstance(psql, str) or not psql.strip():
        raise ValueError("--psql must be a non-empty executable name or path")
    return {
        "reference_service": arguments.reference_service,
        "reference_database": arguments.reference_database,
        "dut_service": arguments.dut_service,
        "dut_database": arguments.dut_database,
        "psql": psql,
        "timeout": timeout,
        "expected_reference_system_identifier": None,
        "expected_dut_system_identifier": None,
...
def _cmd_run_differential(arguments: argparse.Namespace) -> int:
    ...
    with reserve_differential_artifacts(
        ...
        reference_target = PsqlTarget(
            name="reference",
            service=settings["reference_service"],
            database=settings["reference_database"],
        )
        dut_target = PsqlTarget(
            name="dut",
            service=settings["dut_service"],
        ...
        execution_arguments = {
            ...
            "expected_reference_system_identifier": locked_settings[
                "expected_reference_system_identifier"
            ],
            "expected_dut_system_identifier": locked_settings[
...
def _cmd_doctor(arguments: argparse.Namespace) -> int:
    from .audits import audit_repository

    report = audit_repository(Path(arguments.root))
...
def _cmd_applicability_scaffold(arguments: argparse.Namespace) -> int:
    ...
    universe = load_shipped_applicability_universe(arguments.repository_root)
    index = scaffold_feature_applicability(
        universe,
        arguments.output,
        feature_id=arguments.feature_id,
        universe_path=DEFAULT_LEDGER_PATH.as_posix(),
    )
    _emit(
        {
            "status": "scaffolded",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pg-case",
        description="PostgreSQL 18.4 feature coverage and differential test control plane",
    )
    ...
    applicability = commands.add_parser(
        "applicability",
        help="scaffold, review, validate, and compile the PostgreSQL 18.4 factor universe",
    )
    ...
    execute.add_argument("--service", required=True)
    execute.add_argument("--database", required=True)
    execute.add_argument("--psql", default="psql")
    ...
    differential = run_commands.add_parser(
        "differential",
        help="execute one SQL file on PostgreSQL 18.4 and the DUT, then compare",
    )
    ...
    differential.add_argument(
        "--psql",
        help="legacy/direct mode; defaults to psql when no run profile is bound",
    )
    differential.add_argument(
        "--timeout",
        type=int,
