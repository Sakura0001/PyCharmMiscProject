"""Bounded PostgreSQL 18.4 runtime for CREATE OPERATOR FAMILY factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cofam-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_cofam"
...
class CreateOperatorFamilyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateOperatorFamilyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateOperatorFamilySuiteRun:
    run_ordinal: int
    cases: tuple[CreateOperatorFamilyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateOperatorFamilyRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_family_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateOperatorFamilyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_family_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_operator_family_runs(
    run_01: CreateOperatorFamilySuiteRun,
    run_02: CreateOperatorFamilySuiteRun,
) -> CreateOperatorFamilyTwoRunComparison:
    ...
    return CreateOperatorFamilyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateOperatorFamilyRuntimeCase(Protocol):
...
def build_create_operator_family_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateOperatorFamilyRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_create_operator_family_factor_loop_plan(root).cases
    extensions = (
        build_create_operator_family_factor_extension_plan(root).cases
    )
    cases: list[CreateOperatorFamilyRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateOperatorFamilyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateOperatorFamilyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateOperatorFamilyRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_opfamily WHERE opfname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_opclass WHERE opcname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_am WHERE amname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateOperatorFamilyRuntimeCase,
        sql_path: Path,
    ) -> CreateOperatorFamilyCaseRuntimeResult:
        ...
        return CreateOperatorFamilyCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return CreateOperatorFamilySuiteRun(
...
__all__ = [
    "CreateOperatorFamilyCaseRuntimeResult",
    "CreateOperatorFamilyPg18Runner",
    "CreateOperatorFamilyRuntimeCase",
    "CreateOperatorFamilyRuntimeError",
    "CreateOperatorFamilySuiteRun",
    "CreateOperatorFamilyTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_operator_family_runtime_case_set",
    "compare_create_operator_family_runs",
]
