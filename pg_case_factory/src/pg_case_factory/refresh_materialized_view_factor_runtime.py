"""Bounded PostgreSQL 18.4 runtime for REFRESH MATERIALIZED VIEW programs.

The no-DB ledger phase does NOT execute against a live server.  This module
is present so that a later DB phase can drive the same case set through an
isolated, cleanable PostgreSQL 18.4 instance: construction and comparison
logic are fully tested here, while live execution is gated behind
:meth:`RefreshMaterializedViewPg18Runner.verify_server`.

REFRESH replaces the contents of a materialized view; the runner uses an
isolated socket/port/database so it never collides with the ``CLUSTER``
runner.
"""
...
PG18_SOCKET = Path(
    "/tmp/pgcf-pg18-refreshmaterializedview-sock-20260821"
)
PG18_PORT = 55557
PG18_DATABASE = "pgcf_refreshmaterializedview"
PG18_SUPERUSER = "pgcf_refreshmaterializedview_superuser"
...
class RefreshMaterializedViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class RefreshMaterializedViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class RefreshMaterializedViewSuiteRun:
    run_ordinal: int
    cases: tuple[RefreshMaterializedViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise RefreshMaterializedViewRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "refresh_materialized_view_pg18_suite_run",
...
@dataclass(frozen=True)
class RefreshMaterializedViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "refresh_materialized_view_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_refresh_materialized_view_runs(
    run_01: RefreshMaterializedViewSuiteRun,
    run_02: RefreshMaterializedViewSuiteRun,
    ...
    return RefreshMaterializedViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class RefreshMaterializedViewRuntimeCase(Protocol):
...
def build_refresh_materialized_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[RefreshMaterializedViewRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = (
        build_refresh_materialized_view_factor_loop_plan(root).cases
    )
    extensions = (
        build_refresh_materialized_view_factor_extension_plan(root).cases
    )
    cases: list[RefreshMaterializedViewRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: RefreshMaterializedViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class RefreshMaterializedViewPg18Runner:
    ...
    def _execute_case(
        self,
        case: RefreshMaterializedViewRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[RefreshMaterializedViewRuntimeCase],
        ...
        *,
        ...
        results: list[RefreshMaterializedViewCaseRuntimeResult] = []
        for case in cases:
        ...
        return RefreshMaterializedViewSuiteRun(
...
__all__ = [
    ...
    "RefreshMaterializedViewRuntimeCase",
