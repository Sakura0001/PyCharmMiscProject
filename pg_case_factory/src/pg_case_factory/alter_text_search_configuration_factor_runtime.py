"""Bounded PostgreSQL 18.4 runtime for ALTER TEXT SEARCH CONFIGURATION factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-atscfg-sock-20260820")
PG18_PORT = 55507
PG18_DATABASE = "pgcf_atscfg"
...
class AlterTextSearchConfigurationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTextSearchConfigurationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTextSearchConfigurationSuiteRun:
    run_ordinal: int
    cases: tuple[
    ...
    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTextSearchConfigurationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_text_search_configuration_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTextSearchConfigurationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_text_search_configuration_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_text_search_configuration_runs(
    run_01: AlterTextSearchConfigurationSuiteRun,
    run_02: AlterTextSearchConfigurationSuiteRun,
    ...
    return AlterTextSearchConfigurationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTextSearchConfigurationRuntimeCase(Protocol):
...
def build_alter_text_search_configuration_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[AlterTextSearchConfigurationRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_alter_text_search_configuration_factor_loop_plan(
        root
    ).cases
    extensions = (
        build_alter_text_search_configuration_factor_extension_plan(
    ...
    cases: list[AlterTextSearchConfigurationRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: AlterTextSearchConfigurationRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTextSearchConfigurationPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_ts_config WHERE cfgname LIKE '"
            + literal
            + "' "
    ...
    def run_cases(
        self,
        cases: Sequence[AlterTextSearchConfigurationRuntimeCase],
        ...
        *,
        ...
        results: list[AlterTextSearchConfigurationCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterTextSearchConfigurationSuiteRun(
...
__all__ = [
    ...
    "AlterTextSearchConfigurationRuntimeCase",
