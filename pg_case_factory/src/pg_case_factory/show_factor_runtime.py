"""Bounded PostgreSQL 18.4 runtime for SHOW factor programs."""
...
from .show_factor_loop import ShowFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-show-sock-20260821")
PG18_PORT = 55621
PG18_DATABASE = "pgcf_show"
...
class ShowSuiteRun:
    run_ordinal: int
    cases: tuple[ShowCaseRuntimeResult, ...]
...
class ShowTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "show_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_show_runs(
    run_01: ShowSuiteRun,
    run_02: ShowSuiteRun,
) -> ShowTwoRunComparison:
...
class ShowRuntimeCase(Protocol):
...
def build_show_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ShowRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_show_factor_loop_plan(root).cases
    )
    extensions = (
        build_show_factor_extension_plan(root).cases
    )
    cases: list[ShowRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class ShowPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ShowRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[ShowRuntimeCase],
        ...
        *,
        ...
        results: list[ShowCaseRuntimeResult] = []
        for case in cases:
