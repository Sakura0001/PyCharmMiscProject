"""Actual-byte coverage validation for EXECUTE factor-loop SQL.
...
from .execute_factor_render import (
...
class ExecuteFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "execute_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_execute_factor_witness(
        case, repository_root
    )
    expected_sql = render_execute_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_execute(sql) != 1
    ):
...
def validate_execute_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> ExecuteFactorProgramValidation:
    ...
    return ExecuteFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_execute_factor_programs",
]
