"""Bounded PostgreSQL 18.4 runtime for DROP MATERIALIZED VIEW factor programs."""
...
from .drop_materialized_view_factor_loop import DropMaterializedViewFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dmv-sock-20260820")
PG18_PORT = 55509
PG18_DATABASE = "pgcf_dmv"
...
class DropMaterializedViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropMaterializedViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropMaterializedViewSuiteRun:
    run_ordinal: int
    cases: tuple[DropMaterializedViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropMaterializedViewRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_materialized_view_pg18_suite_run",
...
@dataclass(frozen=True)
class DropMaterializedViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_materialized_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_materialized_view_runs(
    run_01: DropMaterializedViewSuiteRun,
    run_02: DropMaterializedViewSuiteRun,
    ...
    return DropMaterializedViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropMaterializedViewRuntimeCase(Protocol):
...
def build_drop_materialized_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropMaterializedViewRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_materialized_view_factor_loop_plan(root).cases
    extensions = build_drop_materialized_view_factor_extension_plan(root).cases
    cases: list[DropMaterializedViewRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropMaterializedViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropMaterializedViewPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropMaterializedViewRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropMaterializedViewRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropMaterializedViewRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[DropMaterializedViewRuntimeCase],
        ...
        *,
        ...
        results: list[DropMaterializedViewCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropMaterializedViewSuiteRun(
...
__all__ = [
    ...
    "DropMaterializedViewRuntimeCase",
