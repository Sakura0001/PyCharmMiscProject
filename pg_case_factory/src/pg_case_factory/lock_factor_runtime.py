"""Bounded PostgreSQL 18.4 runtime for LOCK TABLE factor programs.
...
gated behind :meth:`LockPg18Runner.verify_server`.

LOCK TABLE obtains a table-level lock on a relation; the runner uses an
isolated socket/port/database so it never collides with the ``CLUSTER``
or ``ALTER SUBSCRIPTION`` runners.
"""
...
from .lock_factor_loop import LockFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-lock-sock-20260821")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_lock"
PG18_SUPERUSER = "pgcf_lock_superuser"
...
class LockSuiteRun:
    run_ordinal: int
    cases: tuple[LockCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise LockRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "lock_pg18_suite_run",
...
class LockTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "lock_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_lock_runs(
    run_01: LockSuiteRun,
    run_02: LockSuiteRun,
) -> LockTwoRunComparison:
...
class LockRuntimeCase(Protocol):
...
def build_lock_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[LockRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_lock_factor_loop_plan(root).cases
    extensions = build_lock_factor_extension_plan(root).cases
    cases: list[LockRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: LockRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class LockPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise LockRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise LockRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: LockRuntimeCase,
        sql_path: Path,
    ) -> LockCaseRuntimeResult:
    ...
    def run_cases(
        self,
        cases: Sequence[LockRuntimeCase],
        ...
        *,
        ...
        results: list[LockCaseRuntimeResult] = []
        for case in cases:
