"""Render complete PostgreSQL 18.4 COMMIT factor-loop programs.
...
COMMIT is a TCL transaction-control statement: the target is the
``COMMIT [WORK|TRANSACTION] [AND [NO] CHAIN]`` statement itself, not a
...
COMMIT *ends* a transaction, so the setup typically ``BEGIN``s a
...
reflected in the setup ``BEGIN`` statement; COMMIT itself only accepts
...
class CommitFactorRenderError(ValueError):
    """Raised when a COMMIT case cannot be rendered."""


@dataclass(frozen=True)
class CommitFactorWitness:
...
def _synthetic_case(
    ...
    if pair is not None:
        ...
        factor_value = assignment.get(
            "target_action", "commit_transaction"
        )
    return CommitFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: CommitFactorCase | CommitFactorExtensionCase,
) -> CommitFactorCase:
    if isinstance(case, CommitFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: CommitFactorCase) -> dict[str, str]:
...
def _begin_with_mode(a: dict[str, str]) -> str:
    ...
    COMMIT itself only accepts WORK / TRANSACTION / AND [NO] CHAIN, so
...
def _probe_select(
    ...
    if mode == "returned_rows":
        return "SELECT 1 AS commit_completed;"
    return None
...
def _tables_to_drop(
    ...
    COMMIT is a TCL statement: it never creates a TABLE.  The bookend
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary COMMIT statement for the active mode."""
    ...
    if ic == "syntax_valid_semantic_error":
        return "COMMIT WORK TRANSACTION;"
    if ic == "object_type_mismatch":
        return "COMMIT;"

    # Chain behavior (valid syntax for COMMIT).
    if cb == "and_chain":
        return "COMMIT AND CHAIN;"
    if cb == "and_no_chain":
        return "COMMIT AND NO CHAIN;"

    # Expected status failure (meta -- when no other failure is set).
    # The failure is via transaction state (outside_transaction), so the
    # target is a plain COMMIT that warns 25P01.
    if es == "failure":
        return "COMMIT;"

    # Transaction mode: reflected in the setup BEGIN; the COMMIT keyword
...
def _resolve_case(
    ...
    # COMMIT produces the no-active-transaction warning (25P01).
    if (
...
def resolve_commit_factor_witness(
    case: CommitFactorCase | CommitFactorExtensionCase,
    repository_root: Path,
) -> CommitFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return CommitFactorWitness(
...
def count_primary_commit(sql: str) -> int:
    ...
    return len(re.findall(r"(?im)^\s*COMMIT\b", region))


def _header(case: CommitFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : COMMIT {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_commit_factor_case(
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 COMMIT。")
...
def _write_program(
    ...
    text = render_commit_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_commit_factor_programs(
    baseline_plan: CommitFactorLoopPlan,
...
__all__ = [
    "CommitFactorRenderError",
    "CommitFactorWitness",
    "count_primary_commit",
    "generate_commit_factor_programs",
    "render_commit_factor_case",
    "resolve_commit_factor_witness",
]
