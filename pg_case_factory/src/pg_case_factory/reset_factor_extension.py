"""Bounded post-coverage extension plan for RESET factor regress.

The baseline :mod:`reset_factor_loop` assigns one local SQL program to
...
RESET is a session-scoped statement that creates no tables, so the
bookend (``DROP TABLE IF EXISTS``) is never emitted.  The session runtime
state (parameters) is the semantic witness target.  ``RESET`` inside a
transaction block is the only reachable failure surface (SQLSTATE
``25001``); no RESET variant is privilege-restricted.
...
class ResetFactorExtensionError(ValueError):
    """Raised when RESET extension input drifts."""


@dataclass(frozen=True)
class ResetFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class ResetFactorExtensionPlan:
    cases: tuple[ResetFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "reset_required_factor_value_matrix"

_CONSUMER_ACTION = "reset"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"reset-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_reset_factor_extension_plan(
    repository_root: Path,
) -> ResetFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_reset_factor_loop_plan(root)
    ...
    cases: list[ResetFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise ResetFactorExtensionError(
                    ...
                    derivation_id = (
                        f"RESET-EXT|{ordinal:05d}|"
                        f"{verification}|{cleanup}|{group_name}"
                    )
                    cases.append(
                        ResetFactorExtensionCase(
                            ordinal=ordinal,
                            case_id=f"RESET{ordinal:05d}",
                            sql_filename=(
                                f"RESET{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"reset_{ordinal:05d}_"
                            ),
    ...
    if ordinals != expected_ordinals:
        raise ResetFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise ResetFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("RESET-EXT|")
        for case in plan.cases
    ):
        raise ResetFactorExtensionError(
