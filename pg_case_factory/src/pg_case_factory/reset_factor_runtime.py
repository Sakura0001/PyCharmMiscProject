"""Bounded PostgreSQL 18.4 runtime for RESET factor programs."""
...
from .reset_factor_loop import ResetFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-reset-sock-20260821")
PG18_PORT = 55620
PG18_DATABASE = "pgcf_reset"
...
class ResetSuiteRun:
    run_ordinal: int
    cases: tuple[ResetCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ResetRuntimeError(
...
class ResetTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reset_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_reset_runs(
    run_01: ResetSuiteRun,
    run_02: ResetSuiteRun,
) -> ResetTwoRunComparison:
...
class ResetRuntimeCase(Protocol):
...
def build_reset_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ResetRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_reset_factor_loop_plan(root).cases
    )
    extensions = (
        build_reset_factor_extension_plan(root).cases
    )
    cases: list[ResetRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class ResetPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ResetRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[ResetRuntimeCase],
        ...
        *,
        ...
        results: list[ResetCaseRuntimeResult] = []
        for case in cases:
