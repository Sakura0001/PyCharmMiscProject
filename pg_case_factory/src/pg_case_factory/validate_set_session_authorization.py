"""Actual-byte coverage validation for SET SESSION AUTHORIZATION factor-loop SQL.
...
from .set_session_authorization_factor_render import (
...
class SetSessionAuthorizationFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_set_session_authorization_factor_witness(
        case, repository_root
    )
    expected_sql = render_set_session_authorization_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_set_session_authorization(sql) != 1
    ):
...
def validate_set_session_authorization_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> SetSessionAuthorizationFactorProgramValidation:
    ...
    return SetSessionAuthorizationFactorProgramValidation(
        passed=not issues,
