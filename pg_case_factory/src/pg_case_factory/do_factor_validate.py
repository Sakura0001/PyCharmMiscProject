"""Actual-byte coverage validation for DO factor-loop SQL.
...
class DoFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "do_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_do_factor_witness(
        case, repository_root
    )
    expected_sql = render_do_factor_case(
        case, repository_root
    )
...
def validate_do_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DoFactorProgramValidation(
        passed=not issues,
