"""Bounded PostgreSQL 18.4 runtime for CREATE STATISTICS factor programs.

The runtime is NOT executed in the no-DB phase.  It provides the
construction and two-run comparison contract that the DB phase will
use once a local PostgreSQL 18.4 server is available.  All SQLSTATEs
captured by the no-DB validator are provisional and must be confirmed
by this runner on a live PG 18.4 server.
"""
...
from .create_statistics_factor_loop import CreateStatisticsFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cstat-sock-20260820")
PG18_PORT = 55613
PG18_DATABASE = "pgcf_cstat"
...
class CreateStatisticsRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateStatisticsCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateStatisticsSuiteRun:
    run_ordinal: int
    cases: tuple[CreateStatisticsCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateStatisticsRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_statistics_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateStatisticsTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_statistics_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_statistics_runs(
    run_01: CreateStatisticsSuiteRun,
    run_02: CreateStatisticsSuiteRun,
) -> CreateStatisticsTwoRunComparison:
    ...
    return CreateStatisticsTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateStatisticsRuntimeCase(Protocol):
...
def build_create_statistics_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateStatisticsRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_statistics_factor_loop_plan(root).cases
    extensions = build_create_statistics_factor_extension_plan(root).cases
    cases: list[CreateStatisticsRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateStatisticsRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateStatisticsPg18Runner:
    """Serial, bounded psql executor for an isolated PG 18.4 server."""
    ...
    def _base_command(self) -> list[str]:
        return [
            str(self.psql), "-X", "-h", str(self.socket_dir),
            "-p", str(self.port), "-d", self.database, "-U", self.role,
            "-A", "-t", "-q", "-v", "VERBOSITY=sqlstate",
            "-v", "ON_ERROR_STOP=1",
        ]
    ...
    def verify_server(self) -> int:
        ...
        result = subprocess.run(
            ...
            timeout=min(self.timeout_seconds, 10), check=False,
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateStatisticsRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_statistic_ext WHERE "
            "stxname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_tablespace WHERE spcname LIKE '"
            + literal + "');"
        )
        try:
            result = subprocess.run(
    ...
    def _normalize(payload: bytes, sql_path: Path) -> bytes:
        ...
        normalized = normalized.replace(
            str(sql_path.resolve()).encode("utf-8"), b"<SQL_PATH>"
        )
        normalized = re.sub(
            rb"(?m)^psql \([^\n]+\)\n", b"", normalized
        )
    ...
    def _execute_case(
        self,
        case: CreateStatisticsRuntimeCase,
        sql_path: Path,
        ...
        target_sqlstate = (
            ...
            if len(sqlstate_matches) == 1 else None
        )
        false_lines = sum(
            line.strip() == b"f" for line in stdout.splitlines()
        )
        return CreateStatisticsCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateStatisticsRuntimeCase],
        ...
        *,
        ...
        results: list[CreateStatisticsCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateStatisticsSuiteRun(
