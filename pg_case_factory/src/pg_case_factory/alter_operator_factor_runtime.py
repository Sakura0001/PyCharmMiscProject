"""Serial PG 18.4 runtime executor and two-run comparison for ALTER OPERATOR.
...
class AlterOperatorRuntimeError(RuntimeError):
    """Raised when the ALTER OPERATOR runtime detects a hard failure."""


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ao-sock-20260820")
PG18_PORT = 55492
PG18_DATABASE = "pgcf_ao"
...
class AlterOperatorCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterOperatorSuiteRun:
    run_ordinal: int
    results: tuple[AlterOperatorCaseRuntimeResult, ...]


@dataclass(frozen=True)
class AlterOperatorTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
...
class AlterOperatorRuntimeCase(Protocol):
    case_id: str
    sql_filename: str
    object_prefix: str
    expected_sqlstate: str
...
def build_alter_operator_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
):
    """Build the ordered runtime case set from committed SQL files."""

    from .alter_operator_factor_loop import build_alter_operator_factor_loop_plan
    from .alter_operator_factor_extension import (
        build_alter_operator_factor_extension_plan,
    )

    del repository_root
    baseline_plan = build_alter_operator_factor_loop_plan(Path("."))
    extension_plan = build_alter_operator_factor_extension_plan(Path("."))
...
class AlterOperatorPg18Runner:
    def __init__(
        ...
        *,
        ...
        if not 1 <= timeout_seconds <= 300:
            raise AlterOperatorRuntimeError("timeout out of range")
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        probe = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_operator WHERE "
            f"oprname LIKE '{object_prefix}%'"
            ") AND NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_proc WHERE "
            f"proname LIKE '{object_prefix}%'"
            ") AND NOT EXISTS ("
    ...
    def _execute_case(
        ...
        case,
        sql_path: Path,
    ) -> AlterOperatorCaseRuntimeResult:
        ...
        return AlterOperatorCaseRuntimeResult(
    ...
    def run_cases(
        ...
        return AlterOperatorSuiteRun(run_ordinal=run_ordinal, results=tuple(results))


def compare_alter_operator_runs(
    run_01: AlterOperatorSuiteRun,
    run_02: AlterOperatorSuiteRun,
) -> AlterOperatorTwoRunComparison:
    issues: list[str] = []
    ...
    return AlterOperatorTwoRunComparison(
        passed=not issues,
...
def run_alter_operator_two_run_comparison(
    cases,
    sql_paths,
    *,
    timeout_seconds: int = PER_FILE_TIMEOUT_SECONDS,
) -> AlterOperatorTwoRunComparison:
    runner = AlterOperatorPg18Runner(timeout_seconds=timeout_seconds)
    if not runner.verify_server():
        raise AlterOperatorRuntimeError("PG 18.4 cluster not reachable")
    run_01 = runner.run_cases(cases, sql_paths, run_ordinal=1)
    time.sleep(0)
    run_02 = runner.run_cases(cases, sql_paths, run_ordinal=2)
    return compare_alter_operator_runs(run_01, run_02)
...
__all__ = [
    ...
    "AlterOperatorSuiteRun",
    "AlterOperatorTwoRunComparison",
    "AlterOperatorRuntimeCase",
    "build_alter_operator_runtime_case_set",
    "AlterOperatorPg18Runner",
    "compare_alter_operator_runs",
    "run_alter_operator_two_run_comparison",
]
