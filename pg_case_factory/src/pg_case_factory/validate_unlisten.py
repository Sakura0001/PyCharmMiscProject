"""Actual-byte coverage validation for UNLISTEN factor-loop SQL.
...
from .unlisten_factor_render import (
...
class UnlistenFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "unlisten_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_unlisten_factor_witness(
        case, repository_root
    )
    expected_sql = render_unlisten_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_unlisten(sql) != 1
    ):
...
def validate_unlisten_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> UnlistenFactorProgramValidation:
    ...
    return UnlistenFactorProgramValidation(
        passed=not issues,
