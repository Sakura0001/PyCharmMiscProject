"""Bounded PostgreSQL 18.4 runtime for ALTER TEXT SEARCH TEMPLATE factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-atst-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_atst"
...
class AlterTextSearchTemplateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTextSearchTemplateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTextSearchTemplateSuiteRun:
    run_ordinal: int
    cases: tuple[AlterTextSearchTemplateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTextSearchTemplateRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_text_search_template_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTextSearchTemplateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_template_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_alter_text_search_template_runs(
    run_01: AlterTextSearchTemplateSuiteRun,
    run_02: AlterTextSearchTemplateSuiteRun,
) -> AlterTextSearchTemplateTwoRunComparison:
    ...
    return AlterTextSearchTemplateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTextSearchTemplateRuntimeCase(Protocol):
...
def build_alter_text_search_template_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[AlterTextSearchTemplateRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_alter_text_search_template_factor_loop_plan(
        root
    ).cases
    extensions = (
        build_alter_text_search_template_factor_extension_plan(
    ...
    cases: list[AlterTextSearchTemplateRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: AlterTextSearchTemplateRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTextSearchTemplatePg18Runner:
    ...
    def _execute_case(
        ...
        case: AlterTextSearchTemplateRuntimeCase,
        sql_path: Path,
    ) -> AlterTextSearchTemplateCaseRuntimeResult:
        ...
        return AlterTextSearchTemplateCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[AlterTextSearchTemplateRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> AlterTextSearchTemplateSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[AlterTextSearchTemplateCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterTextSearchTemplateSuiteRun(
...
__all__ = [
    "AlterTextSearchTemplateCaseRuntimeResult",
    "AlterTextSearchTemplatePg18Runner",
    "AlterTextSearchTemplateRuntimeCase",
    "AlterTextSearchTemplateRuntimeError",
    "AlterTextSearchTemplateSuiteRun",
    "AlterTextSearchTemplateTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_text_search_template_runtime_case_set",
    "compare_alter_text_search_template_runs",
]
