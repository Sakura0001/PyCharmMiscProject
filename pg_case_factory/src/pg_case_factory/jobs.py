...
def _execution_record(
    ...
    memberships = identity_document["dangerous_role_memberships"]
    if not isinstance(memberships, list) or any(not isinstance(item, str) for item in memberships):
        raise ValueError(
            f"{relative}.endpoint_identity.dangerous_role_memberships must be a string list"
        )
    privileged_memberships = identity_document["privileged_role_memberships"]
    if not isinstance(privileged_memberships, list) or any(
        not isinstance(item, str) for item in privileged_memberships
    ):
        raise ValueError(
            f"{relative}.endpoint_identity.privileged_role_memberships must be a string list"
        )
    ...
    if (
        ...
        or any(
            not isinstance(identity_document[key], bool)
            for key in (
                "is_superuser",
                "can_createdb",
                "can_createrole",
    ...
    try:
        identity = EndpointIdentity(
            ...
            service=identity_document["service"],
            database=identity_document["database"],
            server_version_num=identity_document["server_version_num"],
            system_identifier=identity_document["system_identifier"],
            server_address=identity_document["server_address"],
            server_port=identity_document["server_port"],
            current_user=identity_document["current_user"],
            is_superuser=identity_document["is_superuser"],
            can_createdb=identity_document["can_createdb"],
            can_createrole=identity_document["can_createrole"],
            can_replication=identity_document["can_replication"],
            can_bypassrls=identity_document["can_bypassrls"],
            dangerous_role_memberships=tuple(memberships),
            privileged_role_memberships=tuple(privileged_memberships),
        )
    ...
    if context.execution_profile is not None:
        ...
        if (
            identity.service != expected_endpoint.service
            or identity.database != expected_endpoint.database
            or identity.system_identifier
            != expected_endpoint.expected_system_identifier
            or identity.current_user != expected_endpoint.expected_current_user
        ):
            raise ValueError(
                f"{relative} endpoint service/database/system_identifier/current_user "
        ...
        EndpointIdentity(
            **{
                **reference.endpoint_identity,
                "dangerous_role_memberships": tuple(
                    reference.endpoint_identity["dangerous_role_memberships"]
                ),
                "privileged_role_memberships": tuple(
                    reference.endpoint_identity["privileged_role_memberships"]
                ),
            }
        ),
        EndpointIdentity(
            **{
                **dut.endpoint_identity,
                "dangerous_role_memberships": tuple(
                    dut.endpoint_identity["dangerous_role_memberships"]
                ),
                "privileged_role_memberships": tuple(
                    dut.endpoint_identity["privileged_role_memberships"]
                ),
            }
        ),
        require_basic_privileges=case.execution_profile == "basic_psql",
    )
    if document["execution_profile_sha256"] != context.execution_profile_sha256:
        raise ValueError(
            f"{relative}.execution_profile_sha256 does not match the immutable run profile"
...
def _validate_required_harnesses(
    ...
    for harness_id in sorted(harness_ids):
        ...
        if (
            ...
            or document["compatibility_target"] != "postgresql-18.4"
