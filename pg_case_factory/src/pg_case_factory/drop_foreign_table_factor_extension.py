"""Bounded post-coverage cross-factor extension expander for DROP FOREIGN TABLE.

The marginal factor-value-loop (:mod:`drop_foreign_table_factor_loop`) is the
...
``nonexistent_table=table_missing_no_if_exists`` when
``if_exists_clause=absent``, ``dependency_state=has_view_dependency`` under
...
* ``branch_drop_foreign_table`` — ``privilege_level`` x
  ``nonexistent_table`` x ``if_exists_clause`` x
  ``cascade_restrict`` x ``dependency_state``.
"""
...
class DropForeignTableFactorExtensionError(ValueError):
    """Raised when a frozen DROP FOREIGN TABLE extension input drifts."""


@dataclass(frozen=True)
class DropForeignTableFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropForeignTableFactorExtensionPlan:
    cases: tuple[DropForeignTableFactorExtensionCase, ...]
...
# (privilege_level=non_owner, nonexistent_table=table_missing_no_if_exists,
# dependency_state=has_view_dependency) are crossed here with
...
_BRANCH_GRAMMAR: dict[str, str] = {
    "branch_drop_foreign_table": "branch_1",
}
...
_BRANCH_AXES: dict[str, dict[str, tuple[str, ...]]] = {
    "branch_drop_foreign_table": {
        ...
        "dependency_state": ("has_view_dependency", "no_dependencies"),
    },
}
...
# boundary (42704) fires when the table is absent and IF EXISTS is absent.
...
_DEPENDENCY_NEGATIVE = ("dependency_state", "has_view_dependency")
...
def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    """table_missing fires only when IF EXISTS is absent."""

    return (
        assignment.get("nonexistent_table") == "table_missing_no_if_exists"
        and assignment.get("if_exists_clause") == _IF_EXISTS_OMITTED
    )


def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    """has_view_dependency fires only under a non-CASCADE drop policy."""

    return (
        assignment.get("dependency_state") == "has_view_dependency"
...
def _present_failure_pair(
    ...
    table is absent and IF EXISTS is absent.  The dependency boundary
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-foreign-table-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP FOREIGN TABLE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_foreign_table_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP FOREIGN TABLE",
...
def build_drop_foreign_table_factor_extension_plan(
    repository_root: Path,
) -> DropForeignTableFactorExtensionPlan:
    """Build the bounded DROP FOREIGN TABLE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_foreign_table_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropForeignTableFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropForeignTableFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPFOREIGNTABLE-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_foreign_table_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropForeignTableFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "DropForeignTableFactorExtensionError",
    "DropForeignTableFactorExtensionCase",
    "DropForeignTableFactorExtensionPlan",
    "build_drop_foreign_table_factor_extension_plan",
