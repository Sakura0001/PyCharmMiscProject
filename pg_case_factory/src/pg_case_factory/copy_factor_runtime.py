"""Bounded PostgreSQL 18.4 runtime for COPY factor programs.
...
from .copy_factor_loop import CopyFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-copy-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_copy"
...
class CopySuiteRun:
    run_ordinal: int
    cases: tuple[CopyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CopyRuntimeError(
...
class CopyTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "copy_pg18_two_run_comparison",
            "passed": self.passed,
...
class CopyRuntimeCase(Protocol):
...
def build_copy_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CopyRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_copy_factor_loop_plan(root).cases
    extensions = build_copy_factor_extension_plan(root).cases
    cases: list[CopyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CopyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CopyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CopyRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[CopyRuntimeCase],
        ...
        *,
        ...
        results: list[CopyCaseRuntimeResult] = []
        for case in cases:
