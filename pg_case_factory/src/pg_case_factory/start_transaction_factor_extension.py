"""Bounded post-coverage cross-factor extension expander for START TRANSACTION.

The marginal factor-value-loop (:mod:`start_transaction_factor_loop`) is the
required baseline: one program per factor value, 44 local cases (GRM 1 +
SFV 43).  This module adds the bounded post-coverage extension phase allowed
by ``start_transaction.yaml`` ``post_coverage_extension_policy.enabled:
true``: cross-factor combinations of the positive T1-T2 behaviour axes across
the single START TRANSACTION synopsis branch, with at most one
failure-causing value per case so attribution stays clean, and
``verification_mode`` crossed so every declared T6 value is exercised.

START TRANSACTION is a TCL transaction-control statement with one synopsis
branch.  The T5 factors (invalid_combination, state_boundary) are derived
from their T1-T2 counterparts, not crossed as axes.  The T3/T4
rotate-attach factors (transaction_id_shape, savepoint_name_shape,
environment_context) are covered in the baseline and set to defaults in the
extension.
...
Unlike COMMIT, START TRANSACTION's synopsis has no ``AND [NO] CHAIN``
clause: ``and_chain`` / ``and_no_chain`` are crossed negatives (42601)
here, and ``inside_transaction`` / ``savepoint_exists`` (25001) are the
"already in a transaction" negatives (COMMIT's negative was
``outside_transaction``).
"""
...
class StartTransactionFactorExtensionError(ValueError):
    """Raised when a frozen START TRANSACTION extension input drifts."""


@dataclass(frozen=True)
class StartTransactionFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class StartTransactionFactorExtensionPlan:
    cases: tuple[StartTransactionFactorExtensionCase, ...]
...
# General axes crossed for the single START TRANSACTION branch.
...
# target_action are overridden per branch.  START TRANSACTION's success
# default for ``transaction_state`` is ``outside_transaction`` (it *starts*
# a block, so the normal case is no active transaction).
...
# attribution.
#
# START TRANSACTION: ``and_chain`` / ``and_no_chain`` are SYNTAX ERRORS
# (42601) here (no AND CHAIN clause in the synopsis).  The
# "already in a transaction" negatives are ``inside_transaction`` /
# ``savepoint_exists`` (25001).  ``outside_transaction`` is the success
# default (COMMIT's negative) and ``missing_required_state`` /
# ``prepared_transaction_exists`` are success states.
    ...
    for values in itertools.product(
        *[_GENERAL_AXES[n] for n in names]
        ...
        if len(assignment) != len(set(assignment)):
            raise StartTransactionFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"start-transaction-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"START TRANSACTION extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": (
                        "start_transaction_factor_extension"
                    ),
...
def build_start_transaction_factor_extension_plan(
    repository_root: Path,
) -> StartTransactionFactorExtensionPlan:
    """Build the bounded START TRANSACTION post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_start_transaction_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[StartTransactionFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    StartTransactionFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"STARTTRANSACTION{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"starttransaction_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"STARTTRANSACTION-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "start_transaction_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return StartTransactionFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "StartTransactionFactorExtensionError",
    "StartTransactionFactorExtensionCase",
    "StartTransactionFactorExtensionPlan",
    "build_start_transaction_factor_extension_plan",
]
