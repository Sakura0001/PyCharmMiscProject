"""Bounded PostgreSQL 18.4 runtime for CREATE GROUP factor programs.

This module is present and tested for construction/comparison logic but is
NOT executed against a live PostgreSQL server in the no-DB phase.  The
frozen runtime port constant is 55602 (kept distinct from every other
statement's runtime port); it is only used when a later DB phase wires the
runner to an isolated PG 18.4 server.
"""
...
from .create_group_factor_loop import CreateGroupFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cgrp-sock-20260820")
PG18_PORT = 55602
PG18_DATABASE = "pgcf_cgrp"
...
class CreateGroupRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateGroupCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateGroupSuiteRun:
    run_ordinal: int
    cases: tuple[CreateGroupCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateGroupRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_group_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateGroupTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_group_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_group_runs(
    run_01: CreateGroupSuiteRun,
    run_02: CreateGroupSuiteRun,
) -> CreateGroupTwoRunComparison:
    ...
    return CreateGroupTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateGroupRuntimeCase(Protocol):
...
def build_create_group_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateGroupRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_group_factor_loop_plan(root).cases
    extensions = build_create_group_factor_extension_plan(root).cases
    cases: list[CreateGroupRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateGroupRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateGroupPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateGroupRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_roles WHERE rolname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_authid WHERE rolname LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: CreateGroupRuntimeCase,
        sql_path: Path,
    ) -> CreateGroupCaseRuntimeResult:
        ...
        return CreateGroupCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateGroupRuntimeCase],
        ...
        *,
        ...
        results: list[CreateGroupCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateGroupCaseRuntimeResult",
    "CreateGroupPg18Runner",
    "CreateGroupRuntimeCase",
    "CreateGroupRuntimeError",
    "CreateGroupSuiteRun",
    "CreateGroupTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_group_runtime_case_set",
    "compare_create_group_runs",
]
