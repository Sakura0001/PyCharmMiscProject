"""Actual-byte coverage validation for ALTER TEXT SEARCH DICTIONARY factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 64-case baseline +
...
class AlterTextSearchDictionaryFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_dictionary_actual_factor_"
                "witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    if (
        ...
        or count_primary_alter_text_search_dictionary(sql) != 1
    ):
...
def validate_alter_text_search_dictionary_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterTextSearchDictionaryFactorProgramValidation(
        passed=not issues,
