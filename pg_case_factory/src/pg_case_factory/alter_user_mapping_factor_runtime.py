"""Bounded PostgreSQL 18.4 runtime for ALTER USER MAPPING factor programs."""
...
from .alter_user_mapping_factor_loop import AlterUserMappingFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-aum-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_aum"
...
class AlterUserMappingRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterUserMappingCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterUserMappingSuiteRun:
    run_ordinal: int
    cases: tuple[AlterUserMappingCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterUserMappingRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_user_mapping_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterUserMappingTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_user_mapping_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_user_mapping_runs(
    run_01: AlterUserMappingSuiteRun,
    run_02: AlterUserMappingSuiteRun,
    ...
    return AlterUserMappingTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterUserMappingRuntimeCase(Protocol):
...
def build_alter_user_mapping_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterUserMappingRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_user_mapping_factor_loop_plan(root).cases
    extensions = build_alter_user_mapping_factor_extension_plan(root).cases
    cases: list[AlterUserMappingRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterUserMappingRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterUserMappingPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterUserMappingRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_user_mapping WHERE umoptions::text LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: AlterUserMappingRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[AlterUserMappingRuntimeCase],
        ...
        *,
        ...
        results: list[AlterUserMappingCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterUserMappingSuiteRun(
