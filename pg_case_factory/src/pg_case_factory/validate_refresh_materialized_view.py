"""Actual-byte coverage validation for REFRESH MATERIALIZED VIEW factor SQL.

Re-renders the canonical bytes for every case (frozen 38-case baseline +
...
factor value is still witnessed across ``baseline union extension``.
"""
...
from .refresh_materialized_view_factor_render import (
...
class RefreshMaterializedViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "refresh_materialized_view_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_refresh_materialized_view_factor_witness(
        case, repository_root
    )
    expected_sql = render_refresh_materialized_view_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_refresh_materialized_view(sql) != 1
    ):
...
def validate_refresh_materialized_view_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return RefreshMaterializedViewFactorProgramValidation(
        passed=not issues,
