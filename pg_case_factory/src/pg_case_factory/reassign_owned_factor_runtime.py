"""Bounded PostgreSQL 18.4 runtime for REASSIGN OWNED factor programs."""
...
from .reassign_owned_factor_loop import ReassignOwnedFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ro-sock-20260820")
PG18_PORT = 55514
PG18_DATABASE = "pgcf_ro"
...
class ReassignOwnedRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class ReassignOwnedCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class ReassignOwnedSuiteRun:
    run_ordinal: int
    cases: tuple[ReassignOwnedCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ReassignOwnedRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reassign_owned_pg18_suite_run",
...
@dataclass(frozen=True)
class ReassignOwnedTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reassign_owned_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_reassign_owned_runs(
    run_01: ReassignOwnedSuiteRun,
    run_02: ReassignOwnedSuiteRun,
) -> ReassignOwnedTwoRunComparison:
    ...
    return ReassignOwnedTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ReassignOwnedRuntimeCase(Protocol):
...
def build_reassign_owned_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[ReassignOwnedRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_reassign_owned_factor_loop_plan(root).cases
    extensions = build_reassign_owned_factor_extension_plan(root).cases
    cases: list[ReassignOwnedRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: ReassignOwnedRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ReassignOwnedPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise ReassignOwnedRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ReassignOwnedRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: ReassignOwnedRuntimeCase,
        sql_path: Path,
    ) -> ReassignOwnedCaseRuntimeResult:
        ...
        return ReassignOwnedCaseRuntimeResult(
...
__all__ = [
    "ReassignOwnedCaseRuntimeResult",
    "ReassignOwnedPg18Runner",
    "ReassignOwnedRuntimeCase",
    "ReassignOwnedRuntimeError",
    "ReassignOwnedSuiteRun",
    "ReassignOwnedTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_reassign_owned_runtime_case_set",
    "compare_reassign_owned_runs",
]
