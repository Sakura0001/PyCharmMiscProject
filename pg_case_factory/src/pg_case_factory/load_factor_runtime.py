"""Bounded PostgreSQL 18.4 runtime for LOAD factor programs."""
...
from .load_factor_loop import LoadFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-load-sock-20260821")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_load"
...
class LoadSuiteRun:
    run_ordinal: int
    cases: tuple[LoadCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise LoadRuntimeError(
...
class LoadTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "load_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_load_runs(
    run_01: LoadSuiteRun,
    run_02: LoadSuiteRun,
) -> LoadTwoRunComparison:
...
class LoadRuntimeCase(Protocol):
...
def build_load_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[LoadRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_load_factor_loop_plan(root).cases
    extensions = build_load_factor_extension_plan(root).cases
    cases: list[LoadRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: LoadRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class LoadPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise LoadRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: LoadRuntimeCase,
        sql_path: Path,
    ) -> LoadCaseRuntimeResult:
    ...
    def run_cases(
        self,
        cases: Sequence[LoadRuntimeCase],
        ...
        *,
        ...
        results: list[LoadCaseRuntimeResult] = []
        for case in cases:
