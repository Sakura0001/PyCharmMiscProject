"""Actual-byte coverage validation for ALTER SCHEMA factor-loop SQL.
...
class AlterSchemaFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_schema_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_schema_factor_witness(case, repository_root)
    expected_sql = render_alter_schema_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_schema(sql) != 1:
...
def validate_alter_schema_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterSchemaFactorProgramValidation(
        passed=not issues,
