"""Actual-byte coverage validation for CREATE ACCESS METHOD factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 31-case baseline +
bounded ~1008-case extension) and compares them exactly against the
...
class CreateAccessMethodFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_access_method_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_access_method_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_access_method_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_access_method(sql) != 1
    ):
...
def validate_create_access_method_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateAccessMethodFactorProgramValidation:
    ...
    return CreateAccessMethodFactorProgramValidation(
        passed=not issues,
