"""Bounded PostgreSQL 18.4 runtime for COMMIT factor programs."""
...
from .commit_factor_loop import CommitFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-commit-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_commit"
...
@dataclass(frozen=True)
class CommitSuiteRun:
    run_ordinal: int
    cases: tuple[CommitCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CommitRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "commit_pg18_suite_run",
...
class CommitTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "commit_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_commit_runs(
    run_01: CommitSuiteRun,
    run_02: CommitSuiteRun,
) -> CommitTwoRunComparison:
...
class CommitRuntimeCase(Protocol):
...
def build_commit_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CommitRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_commit_factor_loop_plan(root).cases
    extensions = build_commit_factor_extension_plan(root).cases
    cases: list[CommitRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CommitRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CommitPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CommitRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CommitRuntimeCase,
        sql_path: Path,
    ) -> CommitCaseRuntimeResult:
        ...
        return CommitCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CommitRuntimeCase],
        ...
        *,
        ...
        results: list[CommitCaseRuntimeResult] = []
        for case in cases:
