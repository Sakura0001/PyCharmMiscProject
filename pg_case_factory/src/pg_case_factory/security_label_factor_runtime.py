"""Bounded PostgreSQL 18.4 runtime for SECURITY LABEL factor programs."""
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-security-label-sock-20260821")
)
PG18_PORT = 55622
PG18_DATABASE = "pgcf_security_label"
...
class SecurityLabelRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SecurityLabelCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SecurityLabelSuiteRun:
    run_ordinal: int
    cases: tuple[SecurityLabelCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SecurityLabelRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "security_label_pg18_suite_run",
...
@dataclass(frozen=True)
class SecurityLabelTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "security_label_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_security_label_runs(
    run_01: SecurityLabelSuiteRun,
    run_02: SecurityLabelSuiteRun,
) -> SecurityLabelTwoRunComparison:
    ...
    return SecurityLabelTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SecurityLabelRuntimeCase(Protocol):
...
def build_security_label_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SecurityLabelRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_security_label_factor_loop_plan(root).cases
    extensions = (
        build_security_label_factor_extension_plan(root).cases
    )
    cases: list[SecurityLabelRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: SecurityLabelRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SecurityLabelPg18Runner:
    ...
    def _execute_case(
        self,
        case: SecurityLabelRuntimeCase,
        sql_path: Path,
    ) -> SecurityLabelCaseRuntimeResult:
        ...
        return SecurityLabelCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[SecurityLabelCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "SecurityLabelCaseRuntimeResult",
    "SecurityLabelPg18Runner",
    "SecurityLabelRuntimeCase",
    "SecurityLabelRuntimeError",
    "SecurityLabelSuiteRun",
    "SecurityLabelTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_security_label_runtime_case_set",
    "compare_security_label_runs",
]
