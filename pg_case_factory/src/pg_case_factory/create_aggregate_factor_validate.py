"""Actual-byte coverage validation for CREATE AGGREGATE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 69-case baseline +
bounded 18720-case extension) and compares them exactly against the
...
class CreateAggregateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_aggregate_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_aggregate_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_aggregate_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_aggregate(sql) != 1:
...
def validate_create_aggregate_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateAggregateFactorProgramValidation(
        passed=not issues,
