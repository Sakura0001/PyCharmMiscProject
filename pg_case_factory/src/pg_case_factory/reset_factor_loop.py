"""Factor-value-loop obligation ledger for PostgreSQL 18.4 RESET.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``RESET`` — reset a run-time parameter to its default.  The official
synopsis (``RESET configuration_parameter`` / ``RESET ALL``) has exactly
one branch, so there is a single ``GRM`` obligation.  The 41 canonical
``SFV`` factor-value pairs are hardcoded verbatim from the canonical
combination matrix ``reset.yaml`` (the package binds directly to the
matrix, which is the source of truth); ``factor_value_count == 41``.

RESET is a session-scoped statement with no privilege restriction: any
role may execute any variant.  The only reachable failure surface is
``RESET`` executed inside a transaction block, which PostgreSQL rejects
...
class ResetFactorLoopError(ValueError):
    """Raised when a frozen RESET obligation input drifts."""


@dataclass(frozen=True)
class ResetGrammarAction:
    """One official target action form of the RESET synopsis."""
...
@dataclass(frozen=True)
class ResetFactorLoopPlan:
    obligations: tuple[ResetFactorObligation, ...]
    cases: tuple[ResetFactorCase, ...]
    delegated: tuple[ResetFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-reset.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-reset"

# The single representative action for all canonical factors.
_REPRESENTATIVE_ACTION = "reset"

_STATEMENT_KEY = "reset"

_MATRIX_PATH = (
    "skills/pg-sql-generation/references/combinations/session/"
    "runtime_parameter/reset.yaml"
)
_REFERENCE_PATH = (
    "references/statements/session/runtime_parameter/reset.md"
)
...
# RESET has only one synopsis form, every factor observes on the single
# ``reset`` action.
...
# Canonical (factor, value) pairs hardcoded verbatim from reset.yaml.
...
# RESET's only reachable failure surface is RESET inside a transaction
...
def _load_grammar_actions() -> tuple[ResetGrammarAction, ...]:
    """Freeze every RESET synopsis target action."""
    ...
    if len(actions) != 1:
        raise ResetFactorLoopError("reset action count drift")
...
def _compile_grammar_obligations() -> list[ResetFactorObligation]:
    rows: list[ResetFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            ResetFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"RESET-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise ResetFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations() -> list[ResetFactorObligation]:
    """Hardcoded SFV obligations verbatim from reset.yaml."""

    if _FACTOR_VALUE_COUNT != 41:
        raise ResetFactorLoopError("canonical factor value count drift")
    rows: list[ResetFactorObligation] = []
    for factor, value in _CANONICAL_FACTOR_VALUES:
        ...
        rows.append(
            ResetFactorObligation(
                ordinal=0,
                obligation_id=f"RESET-SFV|{row_id}|{consumer}",
                kind="SFV",
                factor_key=factor,
                value=value,
...
def _obligation_multiset_sha256(
    rows: tuple[ResetFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"reset-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
def _baseline_assignments(
    obligation: ResetFactorObligation,
) -> tuple[tuple[str, str], ...]:
    ...
    if len(assignments) != len(set(assignments)):
        raise ResetFactorLoopError("duplicate baseline factor key")
...
def build_reset_factor_loop_plan(
    repository_root: Path,
) -> ResetFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_reset_factor_loop_obligations(root)
    cases: list[ResetFactorCase] = []
    delegated: list[ResetFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            ResetFactorCase(
                ordinal=ordinal,
                case_id=f"RESET{ordinal:05d}",
                sql_filename=f"RESET{ordinal:05d}.sql",
                object_prefix=f"reset_{ordinal:05d}_",
    ...
    if len(plan.cases) != 42 or len(plan.delegated) != 0:
        raise ResetFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 42
    ):
        raise ResetFactorLoopError("local obligation mapping drift")
    if (
        len({row.sql_filename for row in plan.cases}) != 42
    ):
        raise ResetFactorLoopError("duplicate SQL filename")
    return plan


def compile_reset_factor_loop_obligations(
    repository_root: Path,
) -> tuple[ResetFactorObligation, ...]:
    """Compile the exact marginal obligation ledger in frozen source order."""

    root = Path(repository_root).resolve(strict=True)
    if not (root / _MATRIX_PATH).is_file():
        raise ResetFactorLoopError(
            f"canonical matrix is missing for reset: {_MATRIX_PATH}"
        )
    unordered_rows = (
    ...
    if len(rows) != 42:
        raise ResetFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise ResetFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 41}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise ResetFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise ResetFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 42:
        raise ResetFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise ResetFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "ResetFactorLoopError",
    "ResetGrammarAction",
    "ResetFactorObligation",
    "ResetFactorCase",
    "ResetFactorLoopPlan",
    ...
    "compile_reset_factor_loop_obligations",
