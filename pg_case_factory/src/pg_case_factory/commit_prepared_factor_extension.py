"""Bounded post-coverage cross-factor extension expander for COMMIT PREPARED.

The marginal factor-value-loop (:mod:`commit_prepared_factor_loop`) is the
...
phase allowed by ``commit_prepared.yaml``
...
COMMIT PREPARED reaches four failure surfaces: the no-prepared-transaction
...
class CommitPreparedFactorExtensionError(ValueError):
    """Raised when a frozen COMMIT PREPARED extension input drifts."""


@dataclass(frozen=True)
class CommitPreparedFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CommitPreparedFactorExtensionPlan:
    cases: tuple[CommitPreparedFactorExtensionCase, ...]
...
# surfaces for COMMIT PREPARED that appear in the crossed axes.  The
...
def _behavior_combinations() -> list[dict[str, str]]:
    ...
    for values in itertools.product(
        *[list(_GENERAL_AXES[n]) for n in names]
        ...
        assignment["target_action"] = "commit_prepared"
        ...
        if len(assignment) != len(set(assignment)):
            raise CommitPreparedFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"commit-prepared-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "compatibility": {
                    "resolver": (
                        "commit_prepared_factor_extension"
                    ),
...
def build_commit_prepared_factor_extension_plan(
    repository_root: Path,
) -> CommitPreparedFactorExtensionPlan:
    """Build the bounded COMMIT PREPARED post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_commit_prepared_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[CommitPreparedFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    CommitPreparedFactorExtensionCase(
                        ordinal=ordinal,
                        case_id=(
                            f"COMMITPREPARED{ordinal:05d}"
                        ),
                        sql_filename=(
                            f"COMMITPREPARED{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"commitprepared_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"CPREP-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "commit_prepared_required_factor_"
                            "value_matrix"
                        ),
                        derivation_reason=(
    ...
    return CommitPreparedFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "CommitPreparedFactorExtensionError",
    "CommitPreparedFactorExtensionCase",
    "CommitPreparedFactorExtensionPlan",
