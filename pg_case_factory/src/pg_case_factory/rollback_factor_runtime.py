"""Bounded PostgreSQL 18.4 runtime for ROLLBACK factor programs."""
...
from .rollback_factor_loop import RollbackFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-rollback-sock-20260821")
PG18_PORT = 55622
PG18_DATABASE = "pgcf_rollback"
...
class RollbackCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class RollbackSuiteRun:
    run_ordinal: int
    cases: tuple[RollbackCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise RollbackRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_pg18_suite_run",
...
@dataclass(frozen=True)
class RollbackTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_rollback_runs(
    run_01: RollbackSuiteRun,
    run_02: RollbackSuiteRun,
    ...
    return RollbackTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class RollbackRuntimeCase(Protocol):
...
def build_rollback_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[RollbackRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_rollback_factor_loop_plan(root).cases
    extensions = build_rollback_factor_extension_plan(root).cases
    cases: list[RollbackRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: RollbackRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class RollbackPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise RollbackRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[RollbackRuntimeCase],
        ...
        *,
        ...
        results: list[RollbackCaseRuntimeResult] = []
        for case in cases:
