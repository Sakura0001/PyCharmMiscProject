"""Actual-byte coverage validation for ALTER POLICY factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 72-case baseline +
bounded 13032-case extension) and compares them exactly against the
...
class AlterPolicyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_policy_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_policy_factor_witness(case, repository_root)
    expected_sql = render_alter_policy_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_policy(sql) != 1:
...
def validate_alter_policy_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterPolicyFactorProgramValidation(
        passed=not issues,
