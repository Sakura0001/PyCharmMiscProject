"""Actual-byte coverage validation for ALTER TABLESPACE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 72-case baseline +
...
class AlterTablespaceFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_tablespace_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_tablespace_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_tablespace_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_tablespace(sql) != 1:
...
def validate_alter_tablespace_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterTablespaceFactorProgramValidation(
        passed=not issues,
