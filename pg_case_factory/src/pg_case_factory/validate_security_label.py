"""Actual-byte coverage validation for SECURITY LABEL factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 86-case baseline +
...
from .security_label_factor_render import (
...
class SecurityLabelFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "security_label_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_security_label_factor_witness(
        case, repository_root
    )
    expected_sql = render_security_label_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_security_label(sql) != 1
    ):
...
def validate_security_label_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return SecurityLabelFactorProgramValidation(
        passed=not issues,
