"""Actual-byte coverage validation for SET CONSTRAINTS factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 42-case baseline +
...
from .set_constraints_factor_render import (
...
class SetConstraintsFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_constraints_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_set_constraints_factor_witness(
        case, repository_root
    )
    expected_sql = render_set_constraints_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_set_constraints(sql) != 1
    ):
...
def validate_set_constraints_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> SetConstraintsFactorProgramValidation:
    ...
    return SetConstraintsFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_set_constraints_factor_programs",
]
