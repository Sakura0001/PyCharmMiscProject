"""Actual-byte coverage validation for ALTER USER MAPPING factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 55-case baseline +
bounded 6048-case extension) and compares them exactly against the
...
class AlterUserMappingFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_user_mapping_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_user_mapping_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_user_mapping_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_user_mapping(sql) != 1:
...
def validate_alter_user_mapping_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterUserMappingFactorProgramValidation(
        passed=not issues,
