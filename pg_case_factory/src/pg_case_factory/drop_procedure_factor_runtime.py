"""Bounded PostgreSQL 18.4 runtime for DROP PROCEDURE factor programs."""
...
from .drop_procedure_factor_loop import DropProcedureFactorCase
...
class DropProcedureRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropProcedureCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropProcedureSuiteRun:
    run_ordinal: int
    cases: tuple[DropProcedureCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropProcedureRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_procedure_pg18_suite_run",
...
@dataclass(frozen=True)
class DropProcedureTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_procedure_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_procedure_runs(
    run_01: DropProcedureSuiteRun,
    run_02: DropProcedureSuiteRun,
) -> DropProcedureTwoRunComparison:
    ...
    return DropProcedureTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropProcedureRuntimeCase(Protocol):
...
def build_drop_procedure_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropProcedureRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_procedure_factor_loop_plan(root).cases
    extensions = build_drop_procedure_factor_extension_plan(root).cases
    cases: list[DropProcedureRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropProcedureRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropProcedurePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropProcedureRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropProcedureRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_proc WHERE proname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropProcedureRuntimeCase,
        sql_path: Path,
    ) -> DropProcedureCaseRuntimeResult:
        ...
        return DropProcedureCaseRuntimeResult(
...
__all__ = [
    "DropProcedureCaseRuntimeResult",
    "DropProcedurePg18Runner",
    "DropProcedureRuntimeCase",
    "DropProcedureRuntimeError",
    "DropProcedureSuiteRun",
    "DropProcedureTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_procedure_runtime_case_set",
    "compare_drop_procedure_runs",
]
