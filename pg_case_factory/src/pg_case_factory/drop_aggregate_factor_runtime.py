"""Bounded PostgreSQL 18.4 runtime for DROP AGGREGATE factor programs."""
...
from .drop_aggregate_factor_loop import DropAggregateFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-da-sock-20260820")
PG18_PORT = 55490
PG18_DATABASE = "pgcf_da"
...
class DropAggregateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropAggregateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropAggregateSuiteRun:
    run_ordinal: int
    cases: tuple[DropAggregateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropAggregateRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_aggregate_pg18_suite_run",
...
@dataclass(frozen=True)
class DropAggregateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_aggregate_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_aggregate_runs(
    run_01: DropAggregateSuiteRun,
    run_02: DropAggregateSuiteRun,
) -> DropAggregateTwoRunComparison:
    ...
    return DropAggregateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropAggregateRuntimeCase(Protocol):
...
def build_drop_aggregate_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropAggregateRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0040) precedes the
    extension (0041-0376) in a stable, human-auditable order.
    """

    from .drop_aggregate_factor_extension import (
        build_drop_aggregate_factor_extension_plan,
    )
    from .drop_aggregate_factor_loop import build_drop_aggregate_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_drop_aggregate_factor_loop_plan(root).cases
    extensions = build_drop_aggregate_factor_extension_plan(root).cases
    cases: list[DropAggregateRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropAggregateRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropAggregatePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropAggregateRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropAggregateRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_aggregate a "
            "JOIN pg_catalog.pg_proc p ON a.aggfnoid = p.oid "
            "WHERE p.proname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_class "
            "WHERE relname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_namespace "
            "WHERE nspname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_type "
            "WHERE typname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_proc "
            "WHERE proname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_roles "
            "WHERE rolname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _execute_case(
        self,
        case: DropAggregateRuntimeCase,
        sql_path: Path,
    ) -> DropAggregateCaseRuntimeResult:
        ...
        return DropAggregateCaseRuntimeResult(
...
__all__ = [
    "DropAggregateCaseRuntimeResult",
    "DropAggregatePg18Runner",
    "DropAggregateRuntimeCase",
    "DropAggregateRuntimeError",
    "DropAggregateSuiteRun",
    "DropAggregateTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_aggregate_runtime_case_set",
    "compare_drop_aggregate_runs",
]
