"""Actual-byte coverage validation for EXPLAIN factor-loop SQL.
...
from .explain_factor_render import (
...
class ExplainFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "explain_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_explain_factor_witness(
        case, repository_root
    )
    expected_sql = render_explain_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_explain(sql) != 1:
...
def validate_explain_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> ExplainFactorProgramValidation:
    ...
    return ExplainFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_explain_factor_programs",
]
