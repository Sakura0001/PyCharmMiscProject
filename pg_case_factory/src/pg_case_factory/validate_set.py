"""Actual-byte coverage validation for SET factor-loop SQL.
...
class SetFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_set_factor_witness(
        case, repository_root
    )
    expected_sql = render_set_factor_case(
        case, repository_root
    )
...
def validate_set_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return SetFactorProgramValidation(
        passed=not issues,
