"""Actual-byte coverage validation for DROP AGGREGATE factor-loop SQL.
...
from .drop_aggregate_factor_render import (
...
class DropAggregateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_aggregate_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_aggregate_factor_witness(case, repository_root)
    expected_sql = render_drop_aggregate_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_drop_aggregate(sql) != 1:
...
def validate_drop_aggregate_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropAggregateFactorProgramValidation(
        passed=not issues,
