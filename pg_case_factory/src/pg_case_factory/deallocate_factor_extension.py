"""Bounded post-coverage extension plan for DEALLOCATE factor regress.

The baseline :mod:`deallocate_factor_loop` assigns one local SQL program
...
class DeallocateFactorExtensionError(ValueError):
    """Raised when DEALLOCATE extension input drifts."""


@dataclass(frozen=True)
class DeallocateFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DeallocateFactorExtensionPlan:
    cases: tuple[DeallocateFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "deallocate_declared_factor_baseline"

_CONSUMER_ACTION = "deallocate_statement"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"deallocate-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
...
def build_deallocate_factor_extension_plan(
    repository_root: Path,
) -> DeallocateFactorExtensionPlan:
    ...
    if len(catalog_rows) != 41:
        raise DeallocateFactorExtensionError(
            "catalog row count drift"
        )

    cases: list[DeallocateFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise (
                            DeallocateFactorExtensionError(
                    ...
                    cases.append(
                        DeallocateFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"DEALLOCATE{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"deallocate_{ordinal:05d}_"
                            ),
    ...
    plan = DeallocateFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if not all(
        case.derivation_id.startswith("DEALLOC-EXT|")
        for case in plan.cases
    ):
        raise DeallocateFactorExtensionError(
