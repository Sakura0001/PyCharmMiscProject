...
def _resolve_case(case: DropOperatorFactorCase) -> _CasePlan:
    ...
    operator_drop = (
        f"DROP OPERATOR IF EXISTS {op_ref} ({left}, {right}) CASCADE;"
    )
    func_drop = f"DROP FUNCTION IF EXISTS {func};"
    type_drop = f"DROP TYPE IF EXISTS {p}customtype;" if needs_custom else ""
    dep_drops = (
    ...
    pre_cleanup.append(operator_drop)
    pre_cleanup.append(func_drop)
    if type_drop:
        pre_cleanup.append(type_drop)
    ...
    cleanup.append(operator_drop)
    cleanup.append(func_drop)
    if type_drop:
        cleanup.append(type_drop)
