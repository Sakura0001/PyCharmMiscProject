"""Actual-byte coverage validation for VACUUM factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 47-case baseline
...
from .vacuum_factor_render import (
...
class VacuumFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "vacuum_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_vacuum_factor_witness(case, repository_root)
    expected_sql = render_vacuum_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_vacuum(sql) != 1:
...
def validate_vacuum_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return VacuumFactorProgramValidation(
        passed=not issues,
