"""Actual-byte coverage validation for SET ROLE factor-loop SQL.
...
from .set_role_factor_render import (
...
class SetRoleFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_role_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_set_role_factor_witness(
        case, repository_root
    )
    expected_sql = render_set_role_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_set_role(sql) != 1
    ):
...
def validate_set_role_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> SetRoleFactorProgramValidation:
    ...
    return SetRoleFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_set_role_factor_programs",
]
