...
def _resolve_case(case: DropSequenceFactorCase) -> _CasePlan:
    ...
    if multi:
        seq_drop = (
            f"DROP SEQUENCE IF EXISTS {seq_ref}, {_seq2_name(p)} CASCADE;"
        )
    else:
        seq_drop = f"DROP SEQUENCE IF EXISTS {seq_ref} CASCADE;"
    dep_drops = (
    ...
    # Pre-cleanup: DROP TABLE first (bookend gate), then sequence, then roles.
    pre_cleanup: list[str] = []
    pre_cleanup.extend(dep_drops)
    pre_cleanup.append(seq_drop)
    pre_cleanup.extend(role_drops)
    if not pre_cleanup:
        pre_cleanup.append("SELECT 1 AS residual_check_no_objects;")

    # Cleanup: RESET ROLE, then sequence/role drops, then DROP
    # TABLE last (bookend gate).
    cleanup: list[str] = []
    if effective:
        cleanup.append("RESET ROLE;")
    cleanup.append(seq_drop)
