"""Actual-byte coverage validation for CREATE PROCEDURE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 92-case baseline +
...
class CreateProcedureFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_procedure_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_procedure_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_procedure_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_procedure(sql) != 1:
...
def validate_create_procedure_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateProcedureFactorProgramValidation:
    ...
    return CreateProcedureFactorProgramValidation(
        passed=not issues,
