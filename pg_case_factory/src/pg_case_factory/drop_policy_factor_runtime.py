"""Bounded PostgreSQL 18.4 runtime for DROP POLICY factor programs."""
...
from .drop_policy_factor_loop import DropPolicyFactorCase
...
class DropPolicyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropPolicyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropPolicySuiteRun:
    run_ordinal: int
    cases: tuple[DropPolicyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropPolicyRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_policy_pg18_suite_run",
...
@dataclass(frozen=True)
class DropPolicyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_policy_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_policy_runs(
    run_01: DropPolicySuiteRun,
    run_02: DropPolicySuiteRun,
) -> DropPolicyTwoRunComparison:
    ...
    return DropPolicyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropPolicyRuntimeCase(Protocol):
...
def build_drop_policy_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropPolicyRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_policy_factor_loop_plan(root).cases
    extensions = build_drop_policy_factor_extension_plan(root).cases
    cases: list[DropPolicyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropPolicyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropPolicyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropPolicyRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropPolicyRuntimeCase,
        sql_path: Path,
    ) -> DropPolicyCaseRuntimeResult:
        ...
        return DropPolicyCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropPolicyRuntimeCase],
        ...
        *,
        ...
        results: list[DropPolicyCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropPolicyCaseRuntimeResult",
    "DropPolicyPg18Runner",
    "DropPolicyRuntimeCase",
    "DropPolicyRuntimeError",
    "DropPolicySuiteRun",
    "DropPolicyTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_policy_runtime_case_set",
    "compare_drop_policy_runs",
]
