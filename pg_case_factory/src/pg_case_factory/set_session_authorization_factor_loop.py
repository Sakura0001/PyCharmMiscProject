"""Factor-value-loop obligation ledger for PostgreSQL 18.4 SET SESSION AUTHORIZATION.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``SET SESSION AUTHORIZATION`` -- set the session user identifier of the
current session.  The official synopsis
(``SET [ SESSION | LOCAL ] SESSION AUTHORIZATION { role_name | DEFAULT }``)
has exactly one branch, so there is a single ``GRM`` obligation.  The 41
canonical ``SFV`` factor-value pairs are hardcoded verbatim from the
canonical combination matrix ``set_session_authorization.yaml`` (the package
binds directly to the matrix, which is the source of truth);
``factor_value_count == 41``.

SET SESSION AUTHORIZATION is a session-scoped statement with no privilege
restriction in the no-DB tickoff phase.  The only reachable failure surface
is ``SET SESSION AUTHORIZATION`` executed inside a transaction block, which
PostgreSQL rejects with SQLSTATE ``25001`` (``active_sql_transaction``).
Three canonical factor values reach this surface: the declared meta-failure
...
class SetSessionAuthorizationFactorLoopError(ValueError):
    """Raised when a frozen SET SESSION AUTHORIZATION obligation input drifts."""


@dataclass(frozen=True)
class SetSessionAuthorizationGrammarAction:
    """One official target action form of the SET SESSION AUTHORIZATION synopsis."""
...
@dataclass(frozen=True)
class SetSessionAuthorizationFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class SetSessionAuthorizationFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SetSessionAuthorizationFactorLoopPlan:
    obligations: tuple[SetSessionAuthorizationFactorObligation, ...]
    cases: tuple[SetSessionAuthorizationFactorCase, ...]
    delegated: tuple[SetSessionAuthorizationFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-set-session-authorization.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-set-session-authorization"

# The single representative action for all canonical factors.
_REPRESENTATIVE_ACTION = "set_session_authorization"

_STATEMENT_KEY = "set_session_authorization"
...
_REFERENCE_PATH = (
    "references/statements/session/authorization/"
    "set_session_authorization.md"
)
...
# SET SESSION AUTHORIZATION has only one synopsis form, every factor
# observes on the single ``set_session_authorization`` action.
...
# set_session_authorization.yaml.
...
# SET SESSION AUTHORIZATION's only reachable failure surface is
# SET SESSION AUTHORIZATION inside a transaction block
...
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "25001",
        "set_session_authorization_declared_failure",
    ),
    ("transaction_visibility", "inside_committed_transaction"): (
        "25001",
        "set_session_authorization_declared_failure",
...
def _load_grammar_actions() -> (
    tuple[SetSessionAuthorizationGrammarAction, ...]
):
    """Freeze every SET SESSION AUTHORIZATION synopsis target action."""
    ...
    rows: tuple[tuple[str, str, str, str], ...] = (
        (
            ...
            "{ role_name | DEFAULT }",
            "synopsis-set-session-authorization",
        ),
    )
    actions = [
        SetSessionAuthorizationGrammarAction(
            action_id=action_id,
    ...
    if len(actions) != 1:
        raise SetSessionAuthorizationFactorLoopError(
            "set_session_authorization action count drift"
        )
...
def _compile_grammar_obligations() -> (
    list[SetSessionAuthorizationFactorObligation]
):
    rows: list[SetSessionAuthorizationFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            SetSessionAuthorizationFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SETSESSIONAUTHORIZATION-GRM|"
                    f"{action.grammar_branch_id}|"
...
def _compile_canonical_obligations() -> (
    ...
    """Hardcoded SFV obligations verbatim from set_session_authorization.yaml."""
    ...
    for factor, value in _CANONICAL_FACTOR_VALUES:
        ...
        rows.append(
            SetSessionAuthorizationFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SETSESSIONAUTHORIZATION-SFV|{row_id}|{consumer}"
                ),
                kind="SFV",
                factor_key=factor,
                value=value,
...
def _obligation_multiset_sha256(
    rows: tuple[SetSessionAuthorizationFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"set-session-authorization-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
_ACTION_BRANCH = {
    "set_session_authorization": _BRANCH_1,
}
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    SET SESSION AUTHORIZATION's only reachable failure surface is
    SET SESSION AUTHORIZATION inside a transaction block
    (SQLSTATE 25001).  When ``expected_status=failure`` or a
...
def _baseline_assignments(
    obligation: SetSessionAuthorizationFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_set_session_authorization_factor_loop_plan(
    ...
    obligations = (
        compile_set_session_authorization_factor_loop_obligations(root)
    )
    cases: list[SetSessionAuthorizationFactorCase] = []
    delegated: list[SetSessionAuthorizationFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            SetSessionAuthorizationFactorCase(
                ordinal=ordinal,
                case_id=f"SETSESSIONAUTHORIZATION{ordinal:05d}",
                sql_filename=f"SETSESSIONAUTHORIZATION{ordinal:05d}.sql",
                object_prefix=f"setsessionauthorization_{ordinal:05d}_",
    ...
    plan = SetSessionAuthorizationFactorLoopPlan(
...
def compile_set_session_authorization_factor_loop_obligations(
    repository_root: Path,
) -> tuple[SetSessionAuthorizationFactorObligation, ...]:
    """Compile the exact marginal obligation ledger in frozen source order."""

    root = Path(repository_root).resolve(strict=True)
    if not (root / _MATRIX_PATH).is_file():
        raise SetSessionAuthorizationFactorLoopError(
            f"canonical matrix is missing for set_session_authorization: "
            f"{_MATRIX_PATH}"
        )
    unordered_rows = (
...
__all__ = [
    "SetSessionAuthorizationFactorLoopError",
    "SetSessionAuthorizationGrammarAction",
    "SetSessionAuthorizationFactorObligation",
    "SetSessionAuthorizationFactorCase",
    "SetSessionAuthorizationFactorLoopPlan",
