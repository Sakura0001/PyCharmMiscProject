"""Actual-byte coverage validation for ANALYZE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 48-case baseline +
bounded 7104-case extension) and compares them exactly against the
...
class AnalyzeFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "analyze_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_analyze_factor_witness(
        case, repository_root
    )
    expected_sql = render_analyze_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_analyze(sql) != 1:
...
def validate_analyze_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AnalyzeFactorProgramValidation:
    ...
    return AnalyzeFactorProgramValidation(
        passed=not issues,
