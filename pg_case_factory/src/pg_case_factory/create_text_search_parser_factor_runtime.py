"""Bounded PostgreSQL 18.4 runtime for CREATE TEXT SEARCH PARSER factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ctsp-sock-20260820")
PG18_PORT = 55614
PG18_DATABASE = "pgcf_ctsp"
...
class CreateTextSearchParserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTextSearchParserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTextSearchParserSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTextSearchParserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTextSearchParserRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_text_search_parser_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTextSearchParserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_text_search_parser_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_text_search_parser_runs(
    run_01: CreateTextSearchParserSuiteRun,
    run_02: CreateTextSearchParserSuiteRun,
) -> CreateTextSearchParserTwoRunComparison:
    ...
    return CreateTextSearchParserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTextSearchParserRuntimeCase(Protocol):
...
def build_create_text_search_parser_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateTextSearchParserRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_text_search_parser_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_text_search_parser_factor_extension_plan(root).cases
    )
    cases: list[CreateTextSearchParserRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: CreateTextSearchParserRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTextSearchParserPg18Runner:
    ...
    def _execute_case(
        ...
        case: CreateTextSearchParserRuntimeCase,
        sql_path: Path,
    ) -> CreateTextSearchParserCaseRuntimeResult:
        ...
        return CreateTextSearchParserCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTextSearchParserRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> CreateTextSearchParserSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[CreateTextSearchParserCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateTextSearchParserSuiteRun(
...
__all__ = [
    "CreateTextSearchParserCaseRuntimeResult",
    "CreateTextSearchParserPg18Runner",
    "CreateTextSearchParserRuntimeCase",
    "CreateTextSearchParserRuntimeError",
    "CreateTextSearchParserSuiteRun",
    "CreateTextSearchParserTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_text_search_parser_runtime_case_set",
    "compare_create_text_search_parser_runs",
]
