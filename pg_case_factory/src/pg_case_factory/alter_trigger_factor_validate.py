"""Actual-byte coverage validation for ALTER TRIGGER factor-loop SQL.
...
class AlterTriggerFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_trigger_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_trigger_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_trigger_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_trigger(sql) != 1:
...
def validate_alter_trigger_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterTriggerFactorProgramValidation(
        passed=not issues,
