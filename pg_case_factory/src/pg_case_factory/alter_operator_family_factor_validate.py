"""Actual-byte coverage validation for ALTER OPERATOR FAMILY factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 59-case baseline +
...
cardinality != 1, placeholder leakage, missing/duplicate/unknown cases, or an
extension case lacking a derivation record.  Also gates that every required
factor value is still witnessed across ``baseline ∪ extension``.
"""
...
class AlterOperatorFamilyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_operator_family_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_operator_family_factor_witness(case, repository_root)
    expected_sql = render_alter_operator_family_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_operator_family(sql) != 1:
...
def validate_alter_operator_family_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterOperatorFamilyFactorProgramValidation:
    ...
    for filename in sorted(programs):
        ...
        try:
            ...
            issues.append(f"{filename}: SQL is not UTF-8")
            continue
        passed, reasons, record = _semantic_program_matches(case, sql, Path(repository_root))
    ...
    return AlterOperatorFamilyFactorProgramValidation(
        passed=not issues,
