"""Bounded PostgreSQL 18.4 runtime for ALTER INDEX factor programs.

The runtime is DB-bound and serial (``MAX_PARALLELISM = 1``): one isolated
PostgreSQL 18.4 cluster per statement, two sequential runs for the two-run
self-consistency gate.  Fork B ships the module code + a skip-gated test; the
live double-run, calibration, and publish are the downstream Fork-C phase.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ai-sock-20260819")
PG18_PORT = 55487
PG18_DATABASE = "pgcf_ai"
...
class AlterIndexCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterIndexSuiteRun:
    run_ordinal: int
    cases: tuple[AlterIndexCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterIndexRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_index_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterIndexTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_index_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_index_runs(
    run_01: AlterIndexSuiteRun,
    run_02: AlterIndexSuiteRun,
) -> AlterIndexTwoRunComparison:
    ...
    return AlterIndexTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterIndexRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`AlterIndexFactorCase` and
    the bounded extension case; the runner only reads the identity +
...
def build_alter_index_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterIndexRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0772) precedes the
    extension (0773-07558) in a stable, human-auditable order.
    """

    from .alter_index_factor_extension import (
        build_alter_index_factor_extension_plan,
    )
    from .alter_index_factor_loop import build_alter_index_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_index_factor_loop_plan(root).cases
    extensions = build_alter_index_factor_extension_plan(root).cases
    cases: list[AlterIndexRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterIndexRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterIndexPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        """True when no residual objects share the case's object prefix.

        ALTER INDEX objects span relations (indexes/tables), namespaces,
        tablespaces, and roles; each is probed so a residue from a prior run
        is caught before section 1 begins.
        """
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterIndexRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            "UNION ALL SELECT 1 FROM pg_catalog.pg_tablespace WHERE spcname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterIndexRuntimeError("cleanup phase marker is not unique")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterIndexRuntimeCase],
        ...
        *,
        ...
        results: list[AlterIndexCaseRuntimeResult] = []
        for case in cases:
