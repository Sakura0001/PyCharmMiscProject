"""Bounded post-coverage cross-factor extension expander for ROLLBACK.

The marginal factor-value-loop (:mod:`rollback_factor_loop`) is the required
...
``rollback.yaml`` ``post_coverage_extension_policy.enabled: true``:
cross-factor combinations of the positive T1-T2 behaviour axes across
the single ROLLBACK synopsis branch, with at most one failure-causing value
per case so attribution stays clean, and ``verification_mode`` crossed so
every declared T6 value is exercised.

ROLLBACK is a TCL transaction-control statement with one synopsis branch.
...
class RollbackFactorExtensionError(ValueError):
    """Raised when a frozen ROLLBACK extension input drifts."""


@dataclass(frozen=True)
class RollbackFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class RollbackFactorExtensionPlan:
    cases: tuple[RollbackFactorExtensionCase, ...]
...
# General axes crossed for the single ROLLBACK branch.
...
# target_action are overridden per branch.  ROLLBACK's success default for
# ``transaction_state`` is ``inside_transaction`` (an active transaction to
# roll back).
...
# attribution.
#
# ROLLBACK: ``and_chain`` / ``and_no_chain`` are VALID syntax (not negatives).
# The failure-causing transaction_state values ``outside_transaction``
# (warning 25P01) and ``missing_required_state`` are the crossed negatives.
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    ``chain_behavior`` never implies an ``invalid_combination``.
    """
    ...
    for values in itertools.product(
        *[_GENERAL_AXES[n] for n in names]
        ...
        if len(assignment) != len(set(assignment)):
            raise RollbackFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"rollback-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"ROLLBACK extension {case.ordinal:05d}: "
...
def build_rollback_factor_extension_plan(
    repository_root: Path,
) -> RollbackFactorExtensionPlan:
    """Build the bounded ROLLBACK post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_rollback_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[RollbackFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    RollbackFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"ROLLBACK{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"rollback_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"ROLLBACK-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "rollback_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return RollbackFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "RollbackFactorExtensionError",
    "RollbackFactorExtensionCase",
    "RollbackFactorExtensionPlan",
    "build_rollback_factor_extension_plan",
]
