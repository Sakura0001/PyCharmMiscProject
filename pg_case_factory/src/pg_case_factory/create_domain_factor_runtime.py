"""Bounded PostgreSQL 18.4 runtime for CREATE DOMAIN factor programs.

The runtime is a serial, bounded psql executor for an isolated
PostgreSQL 18.4 server.  It combines the baseline + extension case sets
and, for each case, (1) probes a clean pre-run state, (2) executes the
SQL file, (3) extracts the target SQLSTATE, (4) runs the cleanup phase
in isolation, and (5) probes a clean post-run state.  Two runs are
compared byte-for-byte to guarantee reproducibility.

CREATE DOMAIN objects live in ``pg_catalog.pg_type`` (``typtype = 'd'``),
``pg_catalog.pg_roles`` (privilege fixture), and
``pg_catalog.pg_collation`` (collation fixture).  The clean-state probe
checks every catalog the render can touch.  No ``pg_class`` relations
are ever created (CREATE DOMAIN is a type-defining DDL statement), but
the probe checks ``pg_class`` too so any stray table is caught.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cdom-sock-20260820")
PG18_PORT = 55540
PG18_DATABASE = "pgcf_cdom"
...
class CreateDomainRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateDomainCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateDomainSuiteRun:
    run_ordinal: int
    cases: tuple[CreateDomainCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateDomainRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_domain_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateDomainTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_domain_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_domain_runs(
    run_01: CreateDomainSuiteRun,
    run_02: CreateDomainSuiteRun,
) -> CreateDomainTwoRunComparison:
    ...
    return CreateDomainTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateDomainRuntimeCase(Protocol):
...
def build_create_domain_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateDomainRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_domain_factor_loop_plan(root).cases
    extensions = build_create_domain_factor_extension_plan(root).cases
    cases: list[CreateDomainRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateDomainRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateDomainPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateDomainRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateDomainRuntimeCase,
        sql_path: Path,
    ) -> CreateDomainCaseRuntimeResult:
        ...
        return CreateDomainCaseRuntimeResult(
...
__all__ = [
    "CreateDomainCaseRuntimeResult",
    "CreateDomainPg18Runner",
    "CreateDomainRuntimeCase",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_domain_runtime_case_set",
    "compare_create_domain_runs",
]
