"""Actual-byte coverage validation for CREATE FOREIGN TABLE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 96-case baseline +
bounded extension) and compares them exactly against the on-disk SQL.
Fails closed on any drift: byte mismatch, header mismatch,
primary-target cardinality != 1, placeholder leakage, missing/duplicate/
unknown cases, or an extension case lacking a derivation record.  Also
gates that every required factor value is still witnessed across
``baseline union extension``.
"""
...
class CreateForeignTableFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_foreign_table_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_foreign_table_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_foreign_table_factor_case(
        case, repository_root
    )
    ...
    if (
        _single_header_value(sql, "expected_sqlstate")
        != expected_sqlstate
    ):
        reasons.append("expected_sqlstate_header_mismatch")
    if (
        actual_target is None
        or count_primary_create_foreign_table(sql) != 1
    ):
...
def _coverage_gaps(
    ...
    gaps = sorted(
        f"{f}={v}" for f, v in (required - witnessed)
    )
    return tuple(gaps)


def validate_create_foreign_table_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateForeignTableFactorProgramValidation:
    ...
    if selected_case_ids is None:
        ...
        validate_all = False
    case_by_filename = {
        case.sql_filename: case for case in selected
    }
        ...
        if case is None:
            if validate_all:
                try:
                    ...
                    issues.append(
                        f"{filename}: SQL is not UTF-8"
                    )
                issues.append(
                    f"{filename}: unexpected SQL program"
                )
            continue
        try:
            sql = raw.decode("utf-8")
    ...
    duplicate = tuple(
        sorted(
            cid
            for cid, count in actual_counter.items()
            if count > 1
        )
    )
    ...
    if unknown_tuple:
        issues.append(
            f"unknown {len(unknown_tuple)} obligations"
        )
    ...
    return CreateForeignTableFactorProgramValidation(
        passed=not issues,
