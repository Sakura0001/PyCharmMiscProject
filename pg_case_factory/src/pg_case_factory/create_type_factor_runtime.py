"""Bounded PostgreSQL 18.4 runtime for CREATE TYPE factor programs."""
...
from .create_type_factor_loop import CreateTypeFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ct-sock-20260820")
PG18_PORT = 55615
PG18_DATABASE = "pgcf_ct"
...
class CreateTypeRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTypeCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTypeSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTypeCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTypeRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_type_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTypeTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_type_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_type_runs(
    run_01: CreateTypeSuiteRun,
    run_02: CreateTypeSuiteRun,
) -> CreateTypeTwoRunComparison:
    ...
    return CreateTypeTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTypeRuntimeCase(Protocol):
...
def build_create_type_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateTypeRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_create_type_factor_loop_plan(root).cases
    extensions = (
        build_create_type_factor_extension_plan(root).cases
    )
    cases: list[CreateTypeRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateTypeRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTypePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateTypeRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_enum WHERE enumlabel LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_range WHERE rngtypid::regtype::text LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateTypeRuntimeCase,
        sql_path: Path,
    ) -> CreateTypeCaseRuntimeResult:
        ...
        return CreateTypeCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTypeRuntimeCase],
        ...
        *,
        ...
        results: list[CreateTypeCaseRuntimeResult] = []
        for case in cases:
