"""Render complete PostgreSQL 18.4 ROLLBACK TO SAVEPOINT factor-loop programs.
...
ROLLBACK TO SAVEPOINT is a TCL transaction-control statement: the target is
the ``ROLLBACK [ TO ] SAVEPOINT savepoint_name`` statement itself, not a
...
ROLLBACK TO SAVEPOINT *rolls back to a savepoint* within an active
transaction, so the setup typically ``BEGIN``s a transaction and creates a
``SAVEPOINT`` first (the normal success case is ``inside_transaction`` +
``savepoint_exists``).  The ``transaction_mode`` factor (isolation level /
read-only / etc.) is reflected in the setup ``BEGIN`` statement; ROLLBACK
TO SAVEPOINT itself only accepts ``SAVEPOINT`` / ``savepoint_name``.
"""
...
from .rollback_to_savepoint_factor_extension import (
    RollbackToSavepointFactorExtensionCase,
    _present_failure_pair,
)
from .rollback_to_savepoint_factor_loop import (
...
class RollbackToSavepointFactorRenderError(ValueError):
    """Raised when a ROLLBACK TO SAVEPOINT case cannot be rendered."""


@dataclass(frozen=True)
class RollbackToSavepointFactorWitness:
...
def _synthetic_case(
    ext: RollbackToSavepointFactorExtensionCase,
) -> RollbackToSavepointFactorCase:
    ...
    if pair is not None:
        ...
        factor_value = assignment.get(
            "target_action", "rollback_to_savepoint"
        )
    return RollbackToSavepointFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    ...
    if isinstance(case, RollbackToSavepointFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: RollbackToSavepointFactorCase) -> dict[str, str]:
    return dict(case.baseline_assignments)


def _savepoint_name(a: dict[str, str], p: str) -> str | None:
    """The savepoint identifier in SAVEPOINT / ROLLBACK TO SAVEPOINT."""
...
def _rollback_target(name: str) -> str:
    """Build a ROLLBACK TO SAVEPOINT target statement from a name."""
...
def _begin_with_mode(a: dict[str, str]) -> str:
    ...
    ROLLBACK TO SAVEPOINT itself only accepts SAVEPOINT / savepoint_name, so
    the isolation-level / read-only / deferrable mode is applied to the
    ``BEGIN`` that starts the transaction holding the savepoint.
    """
...
def _probe_select(
    case: RollbackToSavepointFactorCase,
    a: dict[str, str],
    p: str,
) -> str | None:
    ...
    if mode == "returned_rows":
        return "SELECT 1 AS rollback_to_savepoint_completed;"
    return None
...
def _tables_to_drop(
    ...
    ROLLBACK TO SAVEPOINT is a TCL statement: it never creates a TABLE.  The
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary ROLLBACK TO SAVEPOINT statement for the active mode."""
    ...
    if ic == "object_type_mismatch":
        return _rollback_target(name)

    # Chain behavior (mirror COMMIT: treated as success, provisional).
    if cb == "and_chain":
        return (
            f"ROLLBACK TO SAVEPOINT {name} AND CHAIN;"
            if name
            else "ROLLBACK TO SAVEPOINT AND CHAIN;"
        )
    if cb == "and_no_chain":
        return (
            f"ROLLBACK TO SAVEPOINT {name} AND NO CHAIN;"
            if name
            else "ROLLBACK TO SAVEPOINT AND NO CHAIN;"
        )

    # Expected status failure (meta -- when no other failure is set).
    # The failure is via transaction state (outside_transaction), so the
    # target is a plain ROLLBACK TO SAVEPOINT that warns 25P01.
    if es == "failure":
        return _rollback_target(name)

    # Default: plain ROLLBACK TO SAVEPOINT with the name.
    return _rollback_target(name)
...
def _resolve_case(
    ...
    # ROLLBACK TO SAVEPOINT produces the no-active-transaction warning (25P01).
    if (
    ...
    setup: list[str] = []
    locus = "target.rollback_to_savepoint"
        ...
        if ts == "savepoint_exists":
            ...
            if sp is not None:
                ...
                if sns == "released_name":
                    ...
                    # ``^ROLLBACK\s+TO\s+SAVEPOINT`` does not match the
...
def resolve_rollback_to_savepoint_factor_witness(
    case: (
        RollbackToSavepointFactorCase
        | RollbackToSavepointFactorExtensionCase
    ),
    repository_root: Path,
) -> RollbackToSavepointFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return RollbackToSavepointFactorWitness(
...
def count_primary_rollback_to_savepoint(sql: str) -> int:
    ...
    return len(re.findall(r"(?im)^\s*ROLLBACK\s+TO\s+SAVEPOINT\b", region))


def _header(case: RollbackToSavepointFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : ROLLBACK TO SAVEPOINT {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_rollback_to_savepoint_factor_case(
    case: (
        RollbackToSavepointFactorCase
        | RollbackToSavepointFactorExtensionCase
    ),
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 ROLLBACK TO SAVEPOINT。")
...
def _write_program(
    ...
    text = render_rollback_to_savepoint_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_rollback_to_savepoint_factor_programs(
    baseline_plan: RollbackToSavepointFactorLoopPlan,
...
__all__ = [
    "RollbackToSavepointFactorRenderError",
    "RollbackToSavepointFactorWitness",
    "count_primary_rollback_to_savepoint",
    "generate_rollback_to_savepoint_factor_programs",
    "render_rollback_to_savepoint_factor_case",
    "resolve_rollback_to_savepoint_factor_witness",
]
