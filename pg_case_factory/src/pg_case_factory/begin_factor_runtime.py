"""Bounded PostgreSQL 18.4 runtime for BEGIN factor programs."""
...
from .begin_factor_loop import BeginFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-begin-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_begin"
...
class BeginSuiteRun:
    run_ordinal: int
    cases: tuple[BeginCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise BeginRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "begin_pg18_suite_run",
...
class BeginTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "begin_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_begin_runs(
    run_01: BeginSuiteRun,
    run_02: BeginSuiteRun,
) -> BeginTwoRunComparison:
...
class BeginRuntimeCase(Protocol):
...
def build_begin_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[BeginRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_begin_factor_loop_plan(root).cases
    extensions = build_begin_factor_extension_plan(root).cases
    cases: list[BeginRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: BeginRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class BeginPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise BeginRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: BeginRuntimeCase,
        sql_path: Path,
    ) -> BeginCaseRuntimeResult:
    ...
    def run_cases(
        self,
        cases: Sequence[BeginRuntimeCase],
        ...
        *,
        ...
        results: list[BeginCaseRuntimeResult] = []
        for case in cases:
