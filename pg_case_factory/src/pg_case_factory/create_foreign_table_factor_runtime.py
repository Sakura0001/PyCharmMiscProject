"""Bounded PostgreSQL 18.4 runtime for CREATE FOREIGN TABLE factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cft-sock-20260820")
PG18_PORT = 55543
PG18_DATABASE = "pgcf_cft"
...
class CreateForeignTableRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime drifts."""


@dataclass(frozen=True)
class CreateForeignTableCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateForeignTableSuiteRun:
    run_ordinal: int
    cases: tuple[CreateForeignTableCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateForeignTableRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_foreign_table_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateForeignTableTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_foreign_table_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_foreign_table_runs(
    run_01: CreateForeignTableSuiteRun,
    run_02: CreateForeignTableSuiteRun,
) -> CreateForeignTableTwoRunComparison:
    """Require identical transcripts, structure, SQLSTATEs, and state."""
    ...
    cleanup: list[str] = []
    execution: list[str] = []
    ids_01 = tuple(c.case_id for c in run_01.cases)
    ids_02 = tuple(c.case_id for c in run_02.cases)
    if ids_01 != ids_02:
        issues.append("run case order or identity differs")
    by_id_02 = {c.case_id: c for c in run_02.cases}
    ...
    t_ids = tuple(dict.fromkeys(transcripts))
    s_ids = tuple(dict.fromkeys(structured))
    q_ids = tuple(dict.fromkeys(sqlstates))
    c_ids = tuple(dict.fromkeys(cleanup))
    e_ids = tuple(dict.fromkeys(execution))
    if t_ids:
        issues.append(f"{len(t_ids)} transcript mismatches")
    if s_ids:
        issues.append(
            f"{len(s_ids)} structured result mismatches"
        )
    if q_ids:
        issues.append(f"{len(q_ids)} SQLSTATE mismatches")
    if c_ids:
        issues.append(f"{len(c_ids)} cleanup failures")
    if e_ids:
        issues.append(f"{len(e_ids)} execution failures")
    return CreateForeignTableTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
        execution_count=len(run_01.cases) + len(run_02.cases),
        transcript_mismatch_case_ids=t_ids,
        structured_mismatch_case_ids=s_ids,
        sqlstate_mismatch_case_ids=q_ids,
        cleanup_failure_case_ids=c_ids,
        execution_failure_case_ids=e_ids,
    )


class CreateForeignTableRuntimeCase(Protocol):
...
def _case_ordinal(case: CreateForeignTableRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


def build_create_foreign_table_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateForeignTableRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_create_foreign_table_factor_loop_plan(root).cases
    extensions = (
        build_create_foreign_table_factor_extension_plan(root).cases
    )
    cases: list[CreateForeignTableRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class CreateForeignTablePg18Runner:
    ...
    def _execute_case(
        self,
        case: CreateForeignTableRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateForeignTableRuntimeCase],
        ...
        *,
        ...
        results: list[CreateForeignTableCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateForeignTableSuiteRun(
