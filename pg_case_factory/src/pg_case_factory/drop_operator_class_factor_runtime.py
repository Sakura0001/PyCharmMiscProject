"""Bounded PostgreSQL 18.4 runtime for DROP OPERATOR CLASS factor programs."""
...
from .drop_operator_class_factor_loop import DropOperatorClassFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dopcls-sock-20260820")
PG18_PORT = 55511
PG18_DATABASE = "pgcf_dopcls"
...
class DropOperatorClassRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropOperatorClassCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropOperatorClassSuiteRun:
    run_ordinal: int
    cases: tuple[DropOperatorClassCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropOperatorClassRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_class_pg18_suite_run",
...
@dataclass(frozen=True)
class DropOperatorClassTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_class_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_operator_class_runs(
    run_01: DropOperatorClassSuiteRun,
    run_02: DropOperatorClassSuiteRun,
) -> DropOperatorClassTwoRunComparison:
    ...
    return DropOperatorClassTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropOperatorClassRuntimeCase(Protocol):
...
def build_drop_operator_class_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropOperatorClassRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_operator_class_factor_loop_plan(root).cases
    extensions = build_drop_operator_class_factor_extension_plan(root).cases
    cases: list[DropOperatorClassRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropOperatorClassRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropOperatorClassPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropOperatorClassRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropOperatorClassRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_opclass WHERE opcname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropOperatorClassRuntimeCase,
        sql_path: Path,
    ) -> DropOperatorClassCaseRuntimeResult:
        ...
        return DropOperatorClassCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropOperatorClassSuiteRun(
...
__all__ = [
    "DropOperatorClassCaseRuntimeResult",
    "DropOperatorClassPg18Runner",
    "DropOperatorClassRuntimeCase",
    "DropOperatorClassRuntimeError",
    "DropOperatorClassSuiteRun",
    "DropOperatorClassTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_operator_class_runtime_case_set",
    "compare_drop_operator_class_runs",
]
