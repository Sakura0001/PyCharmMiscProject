"""Bounded post-coverage cross-factor extension expander for ALTER TEXT SEARCH PARSER.

The marginal factor-value-loop (:mod:`alter_text_search_parser_factor_loop`) is
the required baseline: one program per factor value, 35 local cases
(GRM 2 + SFV 33).  This module adds the bounded post-coverage extension
phase allowed by ``alter_text_search_parser.yaml``
``post_coverage_extension_policy.enabled: true``: cross-factor
combinations of the positive T1-T4 behaviour axes across both 2 grammar
...
ALTER TEXT SEARCH PARSER requires superuser privilege on every branch, so
...
duplicate_new_name, nonexistent_target_schema) are derived from their T1-T4
...
class AlterTextSearchParserFactorExtensionError(ValueError):
    """Raised when a frozen ALTER TEXT SEARCH PARSER extension input drifts."""


@dataclass(frozen=True)
class AlterTextSearchParserFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class AlterTextSearchParserFactorExtensionPlan:
    cases: tuple[AlterTextSearchParserFactorExtensionCase, ...]
...
def _is_valid_combination(assignment: dict[str, str]) -> bool:
    ...
    pns = assignment.get("parser_name_shape", "simple_id")
    if os_ == "not_exists" and pns != "nonexistent_name":
        return False
    if os_ != "not_exists" and pns == "nonexistent_name":
        return False

    dnn = assignment.get("duplicate_new_name")
    nns = assignment.get("new_name_shape")
    if dnn is not None and nns is not None:
        if dnn == "same_name_conflict" and nns != "duplicate_name":
            return False
        if dnn != "same_name_conflict" and nns == "duplicate_name":
            return False

    nts = assignment.get("nonexistent_target_schema")
    sns = assignment.get("schema_name_shape")
    if nts is not None and sns is not None:
        if nts == "schema_not_exists" and sns != "nonexistent_schema":
            return False
        if nts != "schema_not_exists" and sns == "nonexistent_schema":
            return False
...
def _derive_t5_factors(a: dict[str, str]) -> None:
    ...
    if os_ == "not_exists" or pns == "nonexistent_name":
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise AlterTextSearchParserFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"alter-text-search-parser-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"ALTER TEXT SEARCH PARSER extension "
                    f"{case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": (
                        "alter_text_search_parser_factor_extension"
                    ),
                ...
                "sql_shape": {
                    "target": "ALTER TEXT SEARCH PARSER",
                    "primary_fence": (
...
def build_alter_text_search_parser_factor_extension_plan(
    repository_root: Path,
) -> AlterTextSearchParserFactorExtensionPlan:
    """Build the bounded ALTER TEXT SEARCH PARSER post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_alter_text_search_parser_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[AlterTextSearchParserFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    AlterTextSearchParserFactorExtensionCase(
                        ordinal=ordinal,
                        case_id=(
                            f"ALTERTEXTSEARCHPARSER{ordinal:05d}"
                        ),
                        sql_filename=(
                            f"ALTERTEXTSEARCHPARSER{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                        ...
                        derived_from_combination_group=(
                            "alter_text_search_parser_required_factor_"
                            "value_matrix"
                        ),
                        derivation_reason=(
    ...
    return AlterTextSearchParserFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "AlterTextSearchParserFactorExtensionError",
    "AlterTextSearchParserFactorExtensionCase",
    "AlterTextSearchParserFactorExtensionPlan",
