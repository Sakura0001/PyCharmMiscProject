"""Bounded post-coverage extension plan for SET factor regress.

The baseline :mod:`set_factor_loop` assigns one local SQL program to
...
SET is a session-scoped statement that creates no tables, so the
bookend (``DROP TABLE IF EXISTS``) is never emitted.  The session runtime
state (parameters) is the semantic witness target.  ``SET`` inside a
transaction block is the only reachable failure surface (SQLSTATE
``25001``); no SET variant is privilege-restricted.
...
class SetFactorExtensionError(ValueError):
    """Raised when SET extension input drifts."""


@dataclass(frozen=True)
class SetFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SetFactorExtensionPlan:
    cases: tuple[SetFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "set_required_factor_value_matrix"

_CONSUMER_ACTION = "set"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"set-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_set_factor_extension_plan(
    repository_root: Path,
) -> SetFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_set_factor_loop_plan(root)
    ...
    cases: list[SetFactorExtensionCase] = []
    raw_count = 0
                ...
                for cleanup in _CLEANUP_MODES:
                    ...
                    cases.append(
                        SetFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"SET{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"set_{ordinal:05d}_"
                            ),
    ...
    if ordinals != expected_ordinals:
        raise SetFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise SetFactorExtensionError("unknown extension outcome")
    if not all(
