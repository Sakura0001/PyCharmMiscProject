"""Actual-byte coverage validation for LOCK TABLE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 46-case baseline
+ bounded 2268-case extension) and compares them exactly against the
...
This module mirrors :mod:`cluster_factor_validate` for the LOCK TABLE
utility statement.  The function :func:`validate_lock_factor_programs`
is the entry point imported by the statement-regress registration glue.
"""
...
from .lock_factor_render import (
    count_primary_lock,
...
class LockFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "lock_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_lock_factor_witness(case, repository_root)
    expected_sql = render_lock_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_lock(sql) != 1:
...
def validate_lock_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return LockFactorProgramValidation(
        passed=not issues,
