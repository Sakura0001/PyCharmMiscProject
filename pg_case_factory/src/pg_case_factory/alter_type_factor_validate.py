"""Actual-byte coverage validation for ALTER TYPE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 94-case baseline +
bounded 10428-case extension) and compares them exactly against the
...
class AlterTypeFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_type_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_type_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_type_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_type(sql) != 1:
...
def validate_alter_type_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterTypeFactorProgramValidation:
    ...
    return AlterTypeFactorProgramValidation(
        passed=not issues,
