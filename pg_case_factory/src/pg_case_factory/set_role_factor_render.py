"""Render complete PostgreSQL 18.4 SET ROLE factor-loop programs.
...
SET ROLE is a session-scoped statement (``SET [ SESSION | LOCAL ] ROLE
role_name`` / ``RESET ROLE``) that sets the current user identifier of the
current session.  It creates no TABLE, so the bookend (``DROP TABLE IF
EXISTS``) is never emitted (table-less; bookend gate exempt).  Each case
creates a witness role and grants it to the current user so ``SET ROLE``
has an observable effect; the oracle observes the role via
``pg_catalog.pg_roles`` (column ``rolname``) or ``current_user``.  Every
catalog ``SELECT`` carries a top-level ``ORDER BY`` so the
catalog-observability gate passes.  ``SET ROLE`` has no optional leading
keywords, so the col-0 keyword regex is ``(?m)^SET\\s+ROLE``.
"""
...
from .set_role_factor_extension import (
    SetRoleFactorExtensionCase,
    _present_failure_pair,
)
from .set_role_factor_loop import (
    SetRoleFactorCase,
...
class SetRoleFactorRenderError(ValueError):
    """Raised when a SET ROLE case cannot be rendered."""


@dataclass(frozen=True)
class SetRoleFactorWitness:
...
def _synthetic_case(
    ext: SetRoleFactorExtensionCase,
) -> SetRoleFactorCase:
    ...
    if pair is not None:
        ...
        factor_value = assignment.get("target_action", "set_role")
    return SetRoleFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: SetRoleFactorCase | SetRoleFactorExtensionCase,
) -> SetRoleFactorCase:
    if isinstance(case, SetRoleFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: SetRoleFactorCase) -> dict[str, str]:
...
def _effective_role(a: dict[str, str], p: str) -> str:
    """The session role under which the target SET ROLE runs."""
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary SET ROLE statement."""
...
def _header(case: SetRoleFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : SET ROLE {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_set_role_factor_case(
    case: SetRoleFactorCase | SetRoleFactorExtensionCase,
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 SET ROLE。")
...
def _write_program(
    ...
    text = render_set_role_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_set_role_factor_programs(
    baseline_plan: SetRoleFactorLoopPlan,
...
def count_primary_set_role(sql: str) -> int:
    ...
    return len(
        re.findall(r"(?im)^\s*SET\s+ROLE\b", region)
    )


def resolve_set_role_factor_witness(
    case: SetRoleFactorCase | SetRoleFactorExtensionCase,
    repository_root: Path,
) -> SetRoleFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return SetRoleFactorWitness(
