"""Bounded PostgreSQL 18.4 runtime for DELETE factor programs.
...
from .delete_factor_loop import DeleteFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-delete-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_delete"
...
@dataclass(frozen=True)
class DeleteSuiteRun:
    run_ordinal: int
    cases: tuple[DeleteCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DeleteRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "delete_pg18_suite_run",
...
class DeleteTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "delete_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_delete_runs(
    run_01: DeleteSuiteRun,
    run_02: DeleteSuiteRun,
) -> DeleteTwoRunComparison:
...
class DeleteRuntimeCase(Protocol):
...
def build_delete_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DeleteRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .delete_factor_extension import (
        build_delete_factor_extension_plan,
    )
    ...
    baseline = build_delete_factor_loop_plan(root).cases
    extensions = build_delete_factor_extension_plan(root).cases
    cases: list[DeleteRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DeleteRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DeletePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DeleteRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DeleteRuntimeCase,
        sql_path: Path,
    ) -> DeleteCaseRuntimeResult:
        ...
        return DeleteCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DeleteRuntimeCase],
        ...
        *,
        ...
        results: list[DeleteCaseRuntimeResult] = []
        for case in cases:
