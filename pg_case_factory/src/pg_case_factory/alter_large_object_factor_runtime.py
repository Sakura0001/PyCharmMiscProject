"""Bounded PostgreSQL 18.4 runtime for ALTER LARGE OBJECT factor programs."""
...
from .alter_large_object_factor_loop import AlterLargeObjectFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-alo-sock-20260819")
PG18_PORT = 55490
PG18_DATABASE = "pgcf_alo"
...
class AlterLargeObjectRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterLargeObjectCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterLargeObjectSuiteRun:
    run_ordinal: int
    cases: tuple[AlterLargeObjectCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterLargeObjectRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_large_object_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterLargeObjectTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_large_object_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_large_object_runs(
    run_01: AlterLargeObjectSuiteRun,
    run_02: AlterLargeObjectSuiteRun,
) -> AlterLargeObjectTwoRunComparison:
    ...
    return AlterLargeObjectTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterLargeObjectRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`AlterLargeObjectFactorCase`
...
def build_alter_large_object_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterLargeObjectRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_large_object_factor_loop_plan(root).cases
    extensions = build_alter_large_object_factor_extension_plan(root).cases
    cases: list[AlterLargeObjectRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterLargeObjectRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterLargeObjectPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterLargeObjectRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterLargeObjectRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_largeobject_metadata WHERE oid::text LIKE '" + literal + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterLargeObjectRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterLargeObjectRuntimeCase,
        sql_path: Path,
    ) -> AlterLargeObjectCaseRuntimeResult:
        ...
        return AlterLargeObjectCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return AlterLargeObjectSuiteRun(
...
__all__ = [
    "AlterLargeObjectCaseRuntimeResult",
    "AlterLargeObjectPg18Runner",
    "AlterLargeObjectRuntimeCase",
    "AlterLargeObjectRuntimeError",
    "AlterLargeObjectSuiteRun",
    "AlterLargeObjectTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_large_object_runtime_case_set",
    "compare_alter_large_object_runs",
]
