"""Render complete PostgreSQL 18.4 DISCARD factor-loop programs.
...
DISCARD is a session-scoped statement (``DISCARD { ALL | PLANS |
SEQUENCES | TEMPORARY | TEMP }``) that resets session state.  It creates
no TABLE, so the bookend (``DROP TABLE IF EXISTS``) is never emitted
(table-less; bookend gate exempt).  Each case sets a session runtime
parameter (``work_mem``) to a non-default value so ``DISCARD ALL`` has a
resettable witness; the oracle observes the restored default via
``pg_catalog.pg_settings`` or ``current_setting``.  Every catalog
``SELECT`` carries a top-level ``ORDER BY`` so the catalog-observability
gate passes.  ``DISCARD`` has no optional leading keywords, so the col-0
keyword regex is ``(?m)^DISCARD(?:\\s|;|$)``.
"""
...
from .discard_factor_extension import (
    DiscardFactorExtensionCase,
    _present_failure_pair,
)
from .discard_factor_loop import (
    DiscardFactorCase,
...
class DiscardFactorRenderError(ValueError):
    """Raised when a DISCARD case cannot be rendered."""


@dataclass(frozen=True)
class DiscardFactorWitness:
...
def _synthetic_case(
    ext: DiscardFactorExtensionCase,
) -> DiscardFactorCase:
    ...
    if pair is not None:
        ...
        factor_value = assignment.get("target_action", "discard")
    return DiscardFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: DiscardFactorCase | DiscardFactorExtensionCase,
) -> DiscardFactorCase:
    if isinstance(case, DiscardFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: DiscardFactorCase) -> dict[str, str]:
...
def _effective_role(a: dict[str, str], p: str) -> str:
    """The session role under which the target DISCARD runs."""
...
def _probe_select(
    ...
    if mode == "returned_rows":
        return (
            "SELECT reset_val AS work_mem_default "
            f"FROM pg_catalog.pg_settings "
            f"WHERE name = '{_WORK_MEM_SETTING}' "
            "ORDER BY reset_val;"
        )
    return (
        "SELECT count(*) > 0 AS session_reset "
        f"FROM pg_catalog.pg_settings "
        f"WHERE name = '{_WORK_MEM_SETTING}' "
        "ORDER BY count(*)"
        + ";"
...
def _header(case: DiscardFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : DISCARD {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_discard_factor_case(
    case: DiscardFactorCase | DiscardFactorExtensionCase,
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 DISCARD。")
...
def _write_program(
    ...
    text = render_discard_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_discard_factor_programs(
    baseline_plan: DiscardFactorLoopPlan,
...
def count_primary_discard(sql: str) -> int:
    ...
    return len(
        re.findall(r"(?im)^\s*DISCARD\b", region)
    )


def resolve_discard_factor_witness(
    case: DiscardFactorCase | DiscardFactorExtensionCase,
    repository_root: Path,
) -> DiscardFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return DiscardFactorWitness(
...
__all__ = [
    "DiscardFactorRenderError",
    "DiscardFactorWitness",
    "count_primary_discard",
    "generate_discard_factor_programs",
    "render_discard_factor_case",
    "resolve_discard_factor_witness",
]
