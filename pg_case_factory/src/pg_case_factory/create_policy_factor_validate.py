"""Actual-byte coverage validation for CREATE POLICY factor-loop SQL.
...
class CreatePolicyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_policy_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_policy_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_policy(sql) != 1:
...
def validate_create_policy_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreatePolicyFactorProgramValidation:
    ...
    return CreatePolicyFactorProgramValidation(
        passed=not issues,
