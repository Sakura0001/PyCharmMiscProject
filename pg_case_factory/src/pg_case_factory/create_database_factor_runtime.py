"""Bounded PostgreSQL 18.4 runtime for CREATE DATABASE factor programs."""
...
from .create_database_factor_loop import CreateDatabaseFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-crdb-sock-20260820")
PG18_PORT = 55530
PG18_DATABASE = "pgcf_crdb"
...
class CreateDatabaseRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateDatabaseCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateDatabaseSuiteRun:
    run_ordinal: int
    cases: tuple[CreateDatabaseCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateDatabaseRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_database_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateDatabaseTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_database_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_database_runs(
    run_01: CreateDatabaseSuiteRun,
    run_02: CreateDatabaseSuiteRun,
    ...
    return CreateDatabaseTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateDatabaseRuntimeCase(Protocol):
...
def build_create_database_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateDatabaseRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_database_factor_loop_plan(root).cases
    extensions = build_create_database_factor_extension_plan(
        root
    ).cases
    cases: list[CreateDatabaseRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateDatabaseRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateDatabasePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateDatabaseRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_database "
            "WHERE datname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_roles "
            "WHERE rolname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_tablespace "
            "WHERE spcname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: CreateDatabaseRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateDatabaseRuntimeCase],
        ...
        *,
        ...
        results: list[CreateDatabaseCaseRuntimeResult] = []
        for case in cases:
