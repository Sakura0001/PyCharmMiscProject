"""Bounded PostgreSQL 18.4 runtime for ALTER SEQUENCE factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-altseq-sock")
PG18_PORT = 55494
...
class AlterSequenceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterSequenceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterSequenceSuiteRun:
    run_ordinal: int
    cases: tuple[AlterSequenceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterSequenceRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_sequence_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterSequenceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_sequence_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_sequence_runs(
    run_01: AlterSequenceSuiteRun,
    run_02: AlterSequenceSuiteRun,
) -> AlterSequenceTwoRunComparison:
    ...
    return AlterSequenceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterSequenceRuntimeCase(Protocol):
...
def build_alter_sequence_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterSequenceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_sequence_factor_loop_plan(root).cases
    extensions = build_alter_sequence_factor_extension_plan(root).cases
    cases: list[AlterSequenceRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterSequenceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterSequencePg18Runner:
    ...
    def _base_command(self) -> list[str]:
        return [
            str(self.psql),
            "-X", "-h", str(self.socket_dir), "-p", str(self.port),
            "-d", self.database, "-U", self.role,
            "-A", "-t", "-q",
            "-v", "VERBOSITY=sqlstate", "-v", "ON_ERROR_STOP=1",
        ]
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterSequenceRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            "UNION ALL SELECT 1 FROM pg_catalog.pg_sequence s JOIN pg_catalog.pg_class c ON s.seqrelid = c.oid WHERE c.relname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _execute_case(
        self, case: AlterSequenceRuntimeCase, sql_path: Path
    ) -> AlterSequenceCaseRuntimeResult:
        ...
        return AlterSequenceCaseRuntimeResult(
    ...
    def run_cases(
        self, cases: Sequence[AlterSequenceRuntimeCase],
        sql_paths: Mapping[str, Path], *, run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> AlterSequenceSuiteRun:
        self.verify_server()
        selected = (
            set(selected_case_ids) if selected_case_ids is not None else None
        )
        results: list[AlterSequenceCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "AlterSequenceCaseRuntimeResult",
    "AlterSequencePg18Runner",
    "AlterSequenceRuntimeCase",
    "AlterSequenceRuntimeError",
    "AlterSequenceSuiteRun",
    "AlterSequenceTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_sequence_runtime_case_set",
    "compare_alter_sequence_runs",
]
