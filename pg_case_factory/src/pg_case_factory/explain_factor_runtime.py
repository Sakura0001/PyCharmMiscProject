"""Bounded PostgreSQL 18.4 runtime for EXPLAIN factor programs.
...
from .explain_factor_loop import ExplainFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-explain-sock-20260821")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_explain"
...
class ExplainCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class ExplainSuiteRun:
    run_ordinal: int
    cases: tuple[ExplainCaseRuntimeResult, ...]
...
class ExplainTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "explain_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_explain_runs(
    run_01: ExplainSuiteRun,
    run_02: ExplainSuiteRun,
    ...
    return ExplainTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ExplainRuntimeCase(Protocol):
...
def build_explain_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[ExplainRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_explain_factor_loop_plan(root).cases
    extensions = build_explain_factor_extension_plan(root).cases
    cases: list[ExplainRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: ExplainRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ExplainPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ExplainRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[ExplainRuntimeCase],
        ...
        *,
        ...
        results: list[ExplainCaseRuntimeResult] = []
        for case in cases:
