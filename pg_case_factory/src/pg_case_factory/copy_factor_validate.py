"""Actual-byte coverage validation for COPY factor-loop SQL.
...
class CopyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "copy_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_copy_factor_witness(
        case, repository_root
    )
    expected_sql = render_copy_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_copy(sql) != 1:
...
def validate_copy_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CopyFactorProgramValidation(
        passed=not issues,
