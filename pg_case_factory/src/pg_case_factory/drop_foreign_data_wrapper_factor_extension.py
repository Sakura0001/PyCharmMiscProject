"""Bounded post-coverage cross-factor extension expander for DROP FOREIGN DATA WRAPPER.

The marginal factor-value-loop (:mod:`drop_foreign_data_wrapper_factor_loop`) is
the required baseline: one program per factor value, 45 local cases
...
``nonexistent_fdw=fdw_missing_no_if_exists`` when ``if_exists_clause=absent``,
``dependent_objects`` non-no-deps under RESTRICT) are crossed here with
...
* ``branch_drop_fdw`` — ``privilege_level`` x
  ``nonexistent_fdw`` x ``if_exists_clause`` x
  ``cascade_restrict`` x ``dependent_objects``.
"""
...
class DropForeignDataWrapperFactorExtensionError(ValueError):
    """Raised when a frozen DROP FOREIGN DATA WRAPPER extension input drifts."""


@dataclass(frozen=True)
class DropForeignDataWrapperFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropForeignDataWrapperFactorExtensionPlan:
    cases: tuple[DropForeignDataWrapperFactorExtensionCase, ...]
...
# (privilege_level=non_owner, nonexistent_fdw=fdw_missing_no_if_exists,
# dependent_objects non-no-deps under RESTRICT) are crossed here with
...
# boundary (42704) fires when the FDW is absent and IF EXISTS is omitted.
...
def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    """fdw_missing fires only when IF EXISTS is omitted."""
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    """non-no-deps fires only under a non-CASCADE drop policy."""
...
def _present_failure_pair(
    ...
    FDW is absent and IF EXISTS is omitted.  The dependency boundary
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-foreign-data-wrapper-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP FOREIGN DATA WRAPPER extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_foreign_data_wrapper_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP FOREIGN DATA WRAPPER",
...
def build_drop_foreign_data_wrapper_factor_extension_plan(
    repository_root: Path,
) -> DropForeignDataWrapperFactorExtensionPlan:
    """Build the bounded DROP FOREIGN DATA WRAPPER post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_foreign_data_wrapper_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropForeignDataWrapperFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropForeignDataWrapperFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPFOREIGNDATAWRAPPER-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_foreign_data_wrapper_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropForeignDataWrapperFactorExtensionPlan(
        cases=cases_tuple,
