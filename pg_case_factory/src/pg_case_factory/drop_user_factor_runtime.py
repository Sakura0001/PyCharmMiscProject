"""Bounded PostgreSQL 18.4 runtime for DROP USER factor programs."""
...
from .drop_user_factor_loop import DropUserFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-du-sock-20260821")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_du"
...
class DropUserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropUserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropUserSuiteRun:
    run_ordinal: int
    cases: tuple[DropUserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropUserRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_user_pg18_suite_run",
...
@dataclass(frozen=True)
class DropUserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_user_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_user_runs(
    run_01: DropUserSuiteRun,
    run_02: DropUserSuiteRun,
    ...
    return DropUserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropUserRuntimeCase(Protocol):
...
def build_drop_user_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropUserRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_user_factor_loop_plan(root).cases
    extensions = build_drop_user_factor_extension_plan(root).cases
    cases: list[DropUserRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropUserRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropUserPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropUserRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[DropUserRuntimeCase],
        ...
        *,
        ...
        results: list[DropUserCaseRuntimeResult] = []
        for case in cases:
