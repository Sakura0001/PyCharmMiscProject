"""Bounded PostgreSQL 18.4 runtime for VACUUM factor programs.

The no-DB ledger phase does NOT execute against a live server.  This
module is present so that a later DB phase can drive the same case set
through an isolated, cleanable PostgreSQL 18.4 instance: construction
and comparison logic are fully tested here, while live execution is
gated behind :meth:`VacuumPg18Runner.verify_server`.

VACUUM garbage-collects and optionally analyzes a database; the runner
uses an isolated socket/port/database so it never collides with the
``CLUSTER`` runner.
"""
...
from .vacuum_factor_loop import VacuumFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-vacuum-sock-20260821")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_vacuum"
PG18_SUPERUSER = "pgcf_vacuum_superuser"
...
@dataclass(frozen=True)
class VacuumSuiteRun:
    run_ordinal: int
    cases: tuple[VacuumCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise VacuumRuntimeError(
...
class VacuumTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "vacuum_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_vacuum_runs(
    run_01: VacuumSuiteRun,
    run_02: VacuumSuiteRun,
) -> VacuumTwoRunComparison:
...
class VacuumRuntimeCase(Protocol):
...
def build_vacuum_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[VacuumRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_vacuum_factor_loop_plan(root).cases
    extensions = build_vacuum_factor_extension_plan(root).cases
    cases: list[VacuumRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: VacuumRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class VacuumPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise VacuumRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: VacuumRuntimeCase,
        sql_path: Path,
    ) -> VacuumCaseRuntimeResult:
        ...
        return VacuumCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[VacuumRuntimeCase],
        ...
        *,
        ...
        results: list[VacuumCaseRuntimeResult] = []
        for case in cases:
