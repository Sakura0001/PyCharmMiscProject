"""Bounded PostgreSQL 18.4 runtime for ALTER VIEW factor programs.

Construction + comparison logic only; NOT executed against a live server in
the no-DB ledger phase.  A later DB phase wires this runner to an isolated
PG 18.4 server (distinct port/socket/database from every sibling statement).
"""
...
from .alter_view_factor_loop import AlterViewFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-aview-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_aview"
...
class AlterViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterViewSuiteRun:
    run_ordinal: int
    cases: tuple[AlterViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterViewRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_view_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_view_runs(
    run_01: AlterViewSuiteRun,
    run_02: AlterViewSuiteRun,
    ...
    return AlterViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterViewRuntimeCase(Protocol):
...
def build_alter_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterViewRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_view_factor_loop_plan(root).cases
    extensions = build_alter_view_factor_extension_plan(root).cases
    cases: list[AlterViewRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterViewPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterViewRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterViewRuntimeCase],
        ...
        *,
        ...
        results: list[AlterViewCaseRuntimeResult] = []
        for case in cases:
