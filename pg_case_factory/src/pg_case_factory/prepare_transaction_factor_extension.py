"""Bounded post-coverage cross-factor extension expander for PREPARE TRANSACTION.

The marginal factor-value-loop (:mod:`prepare_transaction_factor_loop`) is the
...
phase allowed by ``prepare_transaction.yaml``
...
PREPARE TRANSACTION reaches four failure surfaces: the no-prepared-transaction
cluster (``transaction_state=missing_required_state``, 42704), the
...
class PrepareTransactionFactorExtensionError(ValueError):
    """Raised when a frozen PREPARE TRANSACTION extension input drifts."""


@dataclass(frozen=True)
class PrepareTransactionFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class PrepareTransactionFactorExtensionPlan:
    cases: tuple[PrepareTransactionFactorExtensionCase, ...]
...
# surfaces for PREPARE TRANSACTION that appear in the crossed axes.  The
...
def _behavior_combinations() -> list[dict[str, str]]:
    ...
    for values in itertools.product(
        *[list(_GENERAL_AXES[n]) for n in names]
        ...
        assignment["target_action"] = "prepare_transaction"
        ...
        if len(assignment) != len(set(assignment)):
            raise PrepareTransactionFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"prepare-transaction-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"PREPARE TRANSACTION extension "
                    f"{case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": (
                        "prepare_transaction_factor_extension"
                    ),
...
def build_prepare_transaction_factor_extension_plan(
    repository_root: Path,
) -> PrepareTransactionFactorExtensionPlan:
    """Build the bounded PREPARE TRANSACTION post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_prepare_transaction_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[PrepareTransactionFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    PrepareTransactionFactorExtensionCase(
                        ordinal=ordinal,
                        case_id=(
                            f"PREPARETRANSACTION{ordinal:05d}"
                        ),
                        sql_filename=(
                            f"PREPARETRANSACTION{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"preparetransaction_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"PREPARETRANSACTION-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "prepare_transaction_required_factor_"
                            "value_matrix"
                        ),
                        derivation_reason=(
    ...
    return PrepareTransactionFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "PrepareTransactionFactorExtensionError",
    "PrepareTransactionFactorExtensionCase",
    "PrepareTransactionFactorExtensionPlan",
