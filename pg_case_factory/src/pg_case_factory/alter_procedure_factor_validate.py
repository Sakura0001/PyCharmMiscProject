"""Actual-byte coverage validation for ALTER PROCEDURE factor-loop SQL.
...
class AlterProcedureFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_procedure_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_procedure_factor_witness(case, repository_root)
    expected_sql = render_alter_procedure_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_procedure(sql) != 1:
...
def validate_alter_procedure_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterProcedureFactorProgramValidation(
        passed=not issues,
