"""Bounded post-coverage cross-factor extension expander for MERGE.

The marginal factor-value-loop (:mod:`merge_factor_loop`) is the
...
allowed by ``merge.yaml`` ``post_coverage_extension_policy.enabled:
...
MERGE mutates a ``pg_class`` base-table relation; the failure-causing
...
class MergeFactorExtensionError(ValueError):
    """Raised when a frozen MERGE extension input drifts."""


@dataclass(frozen=True)
class MergeFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class MergeFactorExtensionPlan:
    cases: tuple[MergeFactorExtensionCase, ...]
...
# behaviour axes used for DELETE (MERGE shares the DML 15-factor
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise MergeFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"merge-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"MERGE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "merge_factor_extension",
...
def build_merge_factor_extension_plan(
    repository_root: Path,
) -> MergeFactorExtensionPlan:
    """Build the bounded MERGE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_merge_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[MergeFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    MergeFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"MERGE-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "merge_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return MergeFactorExtensionPlan(
        cases=cases_tuple,
