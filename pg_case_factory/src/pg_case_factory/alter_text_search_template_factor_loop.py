"""Factor-value-loop obligation ledger for PostgreSQL 18.4 ALTER TEXT SEARCH TEMPLATE.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``ALTER TEXT SEARCH TEMPLATE``.  ALTER TEXT SEARCH TEMPLATE is a PostgreSQL
full-text-search DDL statement with 2 official synopsis branches: RENAME TO
and SET SCHEMA.  The statement requires superuser privilege and touches the
``pg_catalog.pg_ts_template`` catalog row (not a ``pg_class`` relation), so
column/table/relation coverage is ``not_applicable`` and there is no ``INV``
block.
...
inline.  The 33 canonical ``SFV`` rows are loaded from the shipped
...
class AlterTextSearchTemplateFactorLoopError(ValueError):
    """Raised when a frozen ALTER TEXT SEARCH TEMPLATE obligation input drifts."""


@dataclass(frozen=True)
class AlterTextSearchTemplateGrammarAction:
    """One official target action form of the ALTER TEXT SEARCH TEMPLATE synopsis."""
...
@dataclass(frozen=True)
class AlterTextSearchTemplateFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class AlterTextSearchTemplateFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class AlterTextSearchTemplateFactorLoopPlan:
    obligations: tuple[AlterTextSearchTemplateFactorObligation, ...]
    cases: tuple[AlterTextSearchTemplateFactorCase, ...]
    delegated: tuple[AlterTextSearchTemplateFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branches (PostgreSQL 18 sql-altertstemplate.html).
_BRANCH_RENAME = "branch_rename"
_BRANCH_SET_SCHEMA = "branch_set_schema"

_DOC_SOURCE = "postgresql-18.4-doc:sql-altertstemplate"
...
def _load_grammar_actions() -> (
    tuple[AlterTextSearchTemplateGrammarAction, ...]
):
    """Freeze every ALTER TEXT SEARCH TEMPLATE synopsis target action."""
    ...
    actions = [
        AlterTextSearchTemplateGrammarAction(
            action_id=action_id,
    ...
    if len(actions) != 2:
        raise AlterTextSearchTemplateFactorLoopError(
            "alter text search template action count drift"
        )
...
def _compile_grammar_obligations() -> (
    list[AlterTextSearchTemplateFactorObligation]
):
    rows: list[AlterTextSearchTemplateFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            AlterTextSearchTemplateFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"ATST-GRM|{action.grammar_branch_id}|"
...
def _compile_canonical_obligations(
    ...
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("alter_text_search_template")
    ...
    for row in catalog_rows:
        ...
        rows.append(
            AlterTextSearchTemplateFactorObligation(
                ordinal=0,
                obligation_id=f"ATST-SFV|{row.row_id}|{consumer}",
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[AlterTextSearchTemplateFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"alter-text-search-template-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
# nonexistent_target_schema, non_superuser_attempt) are NOT baselined as
# independent success values: every declared value is either a failure mode
# (the first four) or a branch-specific boundary, so they are set only
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    """Derive overlapping T5/boundary factors and expected_status from T1-T4 values.
    ...
    aa = a.get("alter_action", "")
    sb = a.get("statement_branch", "")

    # nonexistent template cluster: object_state /
    # template_name_shape / nonexistent_template
    if (
        os_ == "not_exists"
    ...
    # missing-schema cluster: nonexistent_target_schema /
    # schema_name_shape / schema_existence
    if (
    ...
    # alter_action <-> statement_branch
    if aa == "set_schema" or sb == _BRANCH_SET_SCHEMA:
        a["alter_action"] = "set_schema"
        a["statement_branch"] = _BRANCH_SET_SCHEMA
        a["grammar_branch"] = _BRANCH_SET_SCHEMA
        a["target_action"] = "set_schema"
...
def _baseline_assignments(
    obligation: AlterTextSearchTemplateFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_alter_text_search_template_factor_loop_plan(
    repository_root: Path,
    ...
    obligations = (
        compile_alter_text_search_template_factor_loop_obligations(root)
    )
    cases: list[AlterTextSearchTemplateFactorCase] = []
    delegated: list[AlterTextSearchTemplateFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            AlterTextSearchTemplateFactorCase(
                ordinal=ordinal,
                case_id=f"ALTERTEXTSEARCHTEMPLATE{ordinal:05d}",
                sql_filename=(
                    f"ALTERTEXTSEARCHTEMPLATE{ordinal:05d}.sql"
                ),
                object_prefix=(
                    f"altertextsearchtemplate_{ordinal:05d}_"
                ),
    ...
    plan = AlterTextSearchTemplateFactorLoopPlan(
...
def compile_alter_text_search_template_factor_loop_obligations(
    repository_root: Path,
) -> tuple[AlterTextSearchTemplateFactorObligation, ...]:
...
__all__ = [
    "AlterTextSearchTemplateFactorLoopError",
    "AlterTextSearchTemplateGrammarAction",
    "AlterTextSearchTemplateFactorObligation",
    "AlterTextSearchTemplateFactorCase",
    "AlterTextSearchTemplateFactorLoopPlan",
    ...
    "_SFV_FAILURE_VALUES",
    "compile_alter_text_search_template_factor_loop_obligations",
