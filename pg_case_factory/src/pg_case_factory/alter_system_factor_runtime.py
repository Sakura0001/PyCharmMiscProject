"""Bounded PostgreSQL 18.4 runtime for ALTER SYSTEM factor programs."""
...
from .alter_system_factor_loop import AlterSystemFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-asystem-sock-20260820")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_asystem"
...
class AlterSystemRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterSystemCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterSystemSuiteRun:
    run_ordinal: int
    cases: tuple[AlterSystemCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterSystemRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_system_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterSystemTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_system_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_system_runs(
    run_01: AlterSystemSuiteRun,
    run_02: AlterSystemSuiteRun,
    ...
    return AlterSystemTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterSystemRuntimeCase(Protocol):
...
def build_alter_system_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterSystemRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_system_factor_loop_plan(root).cases
    extensions = build_alter_system_factor_extension_plan(root).cases
    cases: list[AlterSystemRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterSystemRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterSystemPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterSystemRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterSystemRuntimeCase],
        ...
        *,
        ...
        results: list[AlterSystemCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "AlterSystemRuntimeCase",
    "AlterSystemRuntimeError",
    "AlterSystemSuiteRun",
    "AlterSystemTwoRunComparison",
    "MAX_PARALLELISM",
