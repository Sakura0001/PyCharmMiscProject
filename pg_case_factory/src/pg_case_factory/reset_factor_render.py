"""Render complete PostgreSQL 18.4 RESET factor-loop programs.
...
RESET is a session-scoped statement (``RESET configuration_parameter`` /
``RESET ALL``) that resets a run-time parameter to its default.  It
...
runtime parameter (``work_mem``) to a non-default value so ``RESET`` has a
...
gate passes.  ``RESET`` has no optional leading keywords, so the col-0
keyword regex is ``(?m)^RESET(?:\\s|;|$)``.
"""
...
from .reset_factor_extension import (
...
class ResetFactorRenderError(ValueError):
    """Raised when a RESET case cannot be rendered."""


@dataclass(frozen=True)
class ResetFactorWitness:
...
def _synthetic_case(
    ...
    if pair is not None:
        ...
        factor_value = assignment.get("target_action", "reset")
    return ResetFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: ResetFactorCase | ResetFactorExtensionCase,
) -> ResetFactorCase:
    if isinstance(case, ResetFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: ResetFactorCase) -> dict[str, str]:
...
def _effective_role(a: dict[str, str], p: str) -> str:
    """The session role under which the target RESET runs."""
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary RESET statement."""

    return f"RESET {_WORK_MEM_SETTING};"
...
def _header(case: ResetFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : RESET {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_reset_factor_case(
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 RESET。")
...
def _write_program(
    ...
    text = render_reset_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_reset_factor_programs(
    baseline_plan: ResetFactorLoopPlan,
...
def count_primary_reset(sql: str) -> int:
    ...
    return len(
        re.findall(r"(?im)^\s*RESET\b", region)
    )


def resolve_reset_factor_witness(
    case: ResetFactorCase | ResetFactorExtensionCase,
    repository_root: Path,
) -> ResetFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return ResetFactorWitness(
