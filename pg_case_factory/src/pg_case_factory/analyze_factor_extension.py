"""Bounded post-coverage cross-factor extension expander for ANALYZE.

The marginal factor-value-loop (:mod:`analyze_factor_loop`) is the
...
ANALYZE has a single synopsis branch (``branch_1``), so every crossed
axis is observable through the ``analyze`` action.  The T5 boundary
factors (``invalid_combination``, ``resource_boundary``) are derived
from their T1-T4 counterparts, not crossed as axes.
...
Consistency rules:
  ...
  2. ``privilege_context == insufficient_privilege`` is invalid when
     ``target_state == database_wide`` (DB-wide ANALYZE with insufficient
...
class AnalyzeFactorExtensionError(ValueError):
    """Raised when a frozen ANALYZE extension input drifts."""


@dataclass(frozen=True)
class AnalyzeFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class AnalyzeFactorExtensionPlan:
    cases: tuple[AnalyzeFactorExtensionCase, ...]
...
# Main-axis factors crossed for the single ANALYZE branch.
        ...
        for values in itertools.product(
            *[axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise AnalyzeFactorExtensionError(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"ANALYZE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "analyze_factor_extension",
...
def build_analyze_factor_extension_plan(
    repository_root: Path,
) -> AnalyzeFactorExtensionPlan:
    """Build the bounded ANALYZE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_analyze_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[AnalyzeFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    AnalyzeFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"ANALYZE{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"analyze_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"ANALYZE-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "analyze_declared_factor_baseline"
                        ),
                        derivation_reason=(
    ...
    return AnalyzeFactorExtensionPlan(
        cases=cases_tuple,
