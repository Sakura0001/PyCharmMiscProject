"""Actual-byte coverage validation for CREATE COLLATION factor-loop SQL.
...
class CreateCollationFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_collation_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_collation_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_collation_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_collation(sql) != 1:
...
def validate_create_collation_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateCollationFactorProgramValidation:
    ...
    return CreateCollationFactorProgramValidation(
        passed=not issues,
