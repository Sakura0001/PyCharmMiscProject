"""Bounded PostgreSQL 18.4 runtime for ALTER FUNCTION factor programs."""
...
class AlterFunctionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterFunctionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterFunctionSuiteRun:
    run_ordinal: int
    cases: tuple[AlterFunctionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterFunctionRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_function_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterFunctionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_function_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_function_runs(
    run_01: AlterFunctionSuiteRun,
    run_02: AlterFunctionSuiteRun,
    ...
    return AlterFunctionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterFunctionPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterFunctionRuntimeError(
    ...
    def _cleanup_command(self) -> list[str]:
        ...
        The in-file section 5 already executed once with ON_ERROR_STOP=on.
        Re-running it encounters objects that are now absent, which the bare
        and CASCADE ``cleanup_mode`` factor forms reject with 42883.  This
        best-effort re-run uses ON_ERROR_STOP=off so those rejections are
        tolerated; ``post_clean`` remains the ground-truth cleanliness gate.
        """
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterFunctionRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterFunctionRuntimeError("cleanup phase marker is not unique")
