"""Actual-byte coverage validation for CREATE EXTENSION factor-loop SQL.
...
class CreateExtensionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    expected_sql = render_create_extension_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_extension(sql) != 1
    ):
...
def validate_create_extension_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateExtensionFactorProgramValidation(
        passed=not issues,
