"""Bounded PostgreSQL 18.4 runtime for ALTER USER factor programs."""
...
from .alter_user_factor_loop import AlterUserFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ausr-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_ausr"
...
class AlterUserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterUserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterUserSuiteRun:
    run_ordinal: int
    cases: tuple[AlterUserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterUserRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_user_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterUserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_user_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_user_runs(
    run_01: AlterUserSuiteRun,
    run_02: AlterUserSuiteRun,
    ...
    return AlterUserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterUserRuntimeCase(Protocol):
...
def build_alter_user_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterUserRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_user_factor_loop_plan(root).cases
    extensions = build_alter_user_factor_extension_plan(root).cases
    cases: list[AlterUserRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterUserRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterUserPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterUserRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterUserRuntimeCase],
        ...
        *,
        ...
        results: list[AlterUserCaseRuntimeResult] = []
        for case in cases:
