"""Actual-byte coverage validation for SAVEPOINT factor-loop SQL.
...
from .savepoint_factor_render import (
...
class SavepointFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "savepoint_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_savepoint_factor_witness(
        case, repository_root
    )
    expected_sql = render_savepoint_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_savepoint(sql) != 1
    ):
...
def validate_savepoint_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return SavepointFactorProgramValidation(
        passed=not issues,
