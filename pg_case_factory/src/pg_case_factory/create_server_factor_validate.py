"""Actual-byte coverage validation for CREATE SERVER factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 43-case baseline +
...
from .create_server_factor_render import (
...
class CreateServerFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_server_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_server_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_server_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_server(sql) != 1
    ):
...
def validate_create_server_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateServerFactorProgramValidation:
    ...
    return CreateServerFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_create_server_factor_programs",
]
