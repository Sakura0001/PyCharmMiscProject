"""Actual-byte coverage validation for CREATE OPERATOR FAMILY factor-loop SQL.
...
class CreateOperatorFamilyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_operator_family_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_operator_family_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_operator_family(sql) != 1
    ):
...
def validate_create_operator_family_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateOperatorFamilyFactorProgramValidation:
    ...
    return CreateOperatorFamilyFactorProgramValidation(
        passed=not issues,
