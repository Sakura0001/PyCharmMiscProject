"""Actual-byte coverage validation for VALUES factor-loop SQL.
...
from .values_factor_render import (
...
class ValuesFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "values_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_values_factor_witness(
        case, repository_root
    )
    expected_sql = render_values_factor_case(
        case, repository_root
    )
...
def validate_values_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ValuesFactorProgramValidation(
        passed=not issues,
...
def remove_primary_semantic_locus_but_keep_comments(
    ...
    return sql.replace(
        target,
        "SELECT 1 AS removed_primary_semantic_locus;",
        1,
    )
...
__all__ = [
    ...
    "validate_values_factor_programs",
]
