"""Bounded PostgreSQL 18.4 runtime for DROP COLLATION factor programs."""
...
from .drop_collation_factor_loop import DropCollationFactorCase
...
class DropCollationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropCollationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropCollationSuiteRun:
    run_ordinal: int
    cases: tuple[DropCollationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropCollationRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_collation_pg18_suite_run",
...
@dataclass(frozen=True)
class DropCollationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_collation_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_collation_runs(
    run_01: DropCollationSuiteRun,
    run_02: DropCollationSuiteRun,
) -> DropCollationTwoRunComparison:
    ...
    return DropCollationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropCollationRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`DropCollationFactorCase`
...
def build_drop_collation_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropCollationRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0030) precedes the
    extension (0031-0366) in a stable, human-auditable order.
    """

    from .drop_collation_factor_extension import (
        build_drop_collation_factor_extension_plan,
    )
    from .drop_collation_factor_loop import build_drop_collation_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_drop_collation_factor_loop_plan(root).cases
    extensions = build_drop_collation_factor_extension_plan(root).cases
    cases: list[DropCollationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropCollationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropCollationPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropCollationRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropCollationRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_collation WHERE collname LIKE '" + literal + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise DropCollationRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: DropCollationRuntimeCase,
        sql_path: Path,
    ) -> DropCollationCaseRuntimeResult:
        ...
        return DropCollationCaseRuntimeResult(
...
__all__ = [
    "DropCollationCaseRuntimeResult",
    "DropCollationPg18Runner",
    "DropCollationRuntimeCase",
    "DropCollationRuntimeError",
    "DropCollationSuiteRun",
    "DropCollationTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_collation_runtime_case_set",
    "compare_drop_collation_runs",
]
