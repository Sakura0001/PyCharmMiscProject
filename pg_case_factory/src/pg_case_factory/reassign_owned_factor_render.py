...
def _resolve_case(case: ReassignOwnedFactorCase) -> _CasePlan:
    ...
    # DROP OWNED BY is unreachable in pre-cleanup: the fixture roles are
    ...
    if fixture_kind != "none":
        role_list.append(old_fixture)
        role_list.append(new_fixture)
        if multi == "multiple_old_roles":
            role_list.append(f"{p}old2")
    needs_reset = bool(effective or _needs_role_switch(case, a, effective))
