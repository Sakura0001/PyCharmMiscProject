"""Bounded PostgreSQL 18.4 runtime for CREATE PUBLICATION factor programs."""
...
from .create_publication_factor_loop import CreatePublicationFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cpub-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_cpub"
...
class CreatePublicationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreatePublicationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreatePublicationSuiteRun:
    run_ordinal: int
    cases: tuple[CreatePublicationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreatePublicationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_publication_pg18_suite_run",
...
@dataclass(frozen=True)
class CreatePublicationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_publication_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_publication_runs(
    run_01: CreatePublicationSuiteRun,
    run_02: CreatePublicationSuiteRun,
) -> CreatePublicationTwoRunComparison:
    ...
    return CreatePublicationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreatePublicationRuntimeCase(Protocol):
...
def build_create_publication_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreatePublicationRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_publication_factor_loop_plan(root).cases
    extensions = build_create_publication_factor_extension_plan(root).cases
    cases: list[CreatePublicationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreatePublicationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreatePublicationPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreatePublicationRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_publication WHERE pubname LIKE '"
            + literal
            + "' "
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_subscription WHERE subname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreatePublicationRuntimeCase,
        sql_path: Path,
    ) -> CreatePublicationCaseRuntimeResult:
        ...
        return CreatePublicationCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return CreatePublicationSuiteRun(
...
__all__ = [
    "CreatePublicationCaseRuntimeResult",
    "CreatePublicationPg18Runner",
    "CreatePublicationRuntimeCase",
    "CreatePublicationRuntimeError",
    "CreatePublicationSuiteRun",
    "CreatePublicationTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_publication_runtime_case_set",
    "compare_create_publication_runs",
]
