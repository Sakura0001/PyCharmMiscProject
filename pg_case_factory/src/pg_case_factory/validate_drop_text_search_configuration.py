"""Actual-byte coverage validation for DROP TEXT SEARCH CONFIGURATION factor-loop SQL.
...
from .drop_text_search_configuration_factor_render import (
...
class DropTextSearchConfigurationFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "drop_text_search_configuration_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_text_search_configuration_factor_witness(
        case, repository_root
    )
    expected_sql = render_drop_text_search_configuration_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_drop_text_search_configuration(sql) != 1
    ):
...
def validate_drop_text_search_configuration_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropTextSearchConfigurationFactorProgramValidation(
        passed=not issues,
