"""Bounded PostgreSQL 18.4 runtime for ALTER ROUTINE factor programs."""
...
from .alter_routine_factor_loop import AlterRoutineFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-arout-sock-20260820")
PG18_PORT = 55496
PG18_DATABASE = "pgcf_arout"
...
class AlterRoutineRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterRoutineCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterRoutineSuiteRun:
    run_ordinal: int
    cases: tuple[AlterRoutineCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterRoutineRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_routine_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterRoutineTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_routine_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_routine_runs(
    run_01: AlterRoutineSuiteRun,
    run_02: AlterRoutineSuiteRun,
) -> AlterRoutineTwoRunComparison:
    ...
    return AlterRoutineTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterRoutineRuntimeCase(Protocol):
...
def build_alter_routine_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterRoutineRuntimeCase, ...], dict[str, Path]]:
    ...
    from .alter_routine_factor_loop import build_alter_routine_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_routine_factor_loop_plan(root).cases
    extensions = build_alter_routine_factor_extension_plan(root).cases
    cases: list[AlterRoutineRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterRoutineRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterRoutinePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterRoutineRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterRoutineRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterRoutineRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterRoutineRuntimeCase,
        sql_path: Path,
    ) -> AlterRoutineCaseRuntimeResult:
        ...
        return AlterRoutineCaseRuntimeResult(
...
__all__ = [
    "AlterRoutineCaseRuntimeResult",
    "AlterRoutinePg18Runner",
    "AlterRoutineRuntimeCase",
    "AlterRoutineRuntimeError",
    "AlterRoutineSuiteRun",
    "AlterRoutineTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_routine_runtime_case_set",
    "compare_alter_routine_runs",
]
