"""Bounded post-coverage cross-factor extension expander for CALL.

The marginal factor-value-loop (:mod:`call_factor_loop`) is the required
baseline: one program per factor value, 49 local cases (GRM 1 + SFV 48).
This module adds the bounded post-coverage extension phase allowed by
``call.yaml`` ``post_coverage_extension_policy.enabled: true``:
cross-factor combinations of the positive T1-T4 behaviour axes across
the single ``branch_1`` grammar branch, with at most one
...
CALL invokes a PROCEDURE; the failure-causing values are:
``target_relation_state=missing``, ``target_relation_state=
wrong_object_type``, ``privilege_context=insufficient_privilege``, and
...
class CallFactorExtensionError(ValueError):
    """Raised when a frozen CALL extension input drifts."""


@dataclass(frozen=True)
class CallFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CallFactorExtensionPlan:
    cases: tuple[CallFactorExtensionCase, ...]
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise CallFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"call-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"CALL extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "call_factor_extension",
...
def build_call_factor_extension_plan(
    repository_root: Path,
) -> CallFactorExtensionPlan:
    """Build the bounded CALL post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_call_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[CallFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    CallFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"CALL-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "call_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return CallFactorExtensionPlan(
        cases=cases_tuple,
