"""Bounded PostgreSQL 18.4 runtime for UNLISTEN factor programs."""
...
from .unlisten_factor_loop import UnlistenFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-unlisten-sock-20260821")
PG18_PORT = 55620
PG18_DATABASE = "pgcf_unlisten"
...
class UnlistenRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class UnlistenCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class UnlistenSuiteRun:
    run_ordinal: int
    cases: tuple[UnlistenCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise UnlistenRuntimeError(
...
@dataclass(frozen=True)
class UnlistenTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "unlisten_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_unlisten_runs(
    run_01: UnlistenSuiteRun,
    run_02: UnlistenSuiteRun,
) -> UnlistenTwoRunComparison:
    ...
    return UnlistenTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class UnlistenRuntimeCase(Protocol):
...
def build_unlisten_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[UnlistenRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_unlisten_factor_loop_plan(root).cases
    )
    extensions = (
        build_unlisten_factor_extension_plan(root).cases
    )
    cases: list[UnlistenRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class UnlistenPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise UnlistenRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return UnlistenCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[UnlistenRuntimeCase],
        ...
        *,
        ...
        results: list[UnlistenCaseRuntimeResult] = []
        for case in cases:
