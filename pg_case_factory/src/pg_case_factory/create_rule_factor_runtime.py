"""Bounded PostgreSQL 18.4 runtime for CREATE RULE factor programs.
...
from .create_rule_factor_loop import CreateRuleFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-crule-sock-20260820")
PG18_PORT = 55602
PG18_DATABASE = "pgcf_crule"
...
class CreateRuleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateRuleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateRuleSuiteRun:
    run_ordinal: int
    cases: tuple[CreateRuleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateRuleRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_rule_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateRuleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_rule_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_rule_runs(
    run_01: CreateRuleSuiteRun,
    run_02: CreateRuleSuiteRun,
) -> CreateRuleTwoRunComparison:
    ...
    return CreateRuleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateRuleRuntimeCase(Protocol):
...
def build_create_rule_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateRuleRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_rule_factor_loop_plan(root).cases
    extensions = build_create_rule_factor_extension_plan(root).cases
    cases: list[CreateRuleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateRuleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateRulePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateRuleRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateRuleRuntimeCase,
        sql_path: Path,
    ) -> CreateRuleCaseRuntimeResult:
        ...
        return CreateRuleCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateRuleRuntimeCase],
        ...
        *,
        ...
        results: list[CreateRuleCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateRuleCaseRuntimeResult",
    "CreateRulePg18Runner",
    "CreateRuleRuntimeCase",
    "CreateRuleRuntimeError",
    "CreateRuleSuiteRun",
    "CreateRuleTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_rule_runtime_case_set",
    "compare_create_rule_runs",
]
