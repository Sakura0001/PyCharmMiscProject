"""Bounded PostgreSQL 18.4 runtime for DROP SEQUENCE factor programs."""
...
from .drop_sequence_factor_loop import DropSequenceFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ds-sock-20260820")
PG18_PORT = 55502
...
class DropSequenceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropSequenceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropSequenceSuiteRun:
    run_ordinal: int
    cases: tuple[DropSequenceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropSequenceRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_sequence_pg18_suite_run",
...
@dataclass(frozen=True)
class DropSequenceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_sequence_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_sequence_runs(
    run_01: DropSequenceSuiteRun,
    run_02: DropSequenceSuiteRun,
) -> DropSequenceTwoRunComparison:
    ...
    return DropSequenceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropSequenceRuntimeCase(Protocol):
...
def build_drop_sequence_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropSequenceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_sequence_factor_loop_plan(root).cases
    extensions = build_drop_sequence_factor_extension_plan(root).cases
    cases: list[DropSequenceRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropSequenceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropSequencePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropSequenceRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropSequenceRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropSequenceRuntimeCase,
        sql_path: Path,
    ) -> DropSequenceCaseRuntimeResult:
        ...
        return DropSequenceCaseRuntimeResult(
...
__all__ = [
    "DropSequenceCaseRuntimeResult",
    "DropSequencePg18Runner",
    "DropSequenceRuntimeCase",
    "DropSequenceRuntimeError",
    "DropSequenceSuiteRun",
    "DropSequenceTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_sequence_runtime_case_set",
    "compare_drop_sequence_runs",
]
