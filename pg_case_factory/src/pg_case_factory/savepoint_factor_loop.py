"""Factor-value-loop obligation ledger for PostgreSQL 18.4 SAVEPOINT.

SAVEPOINT is a PostgreSQL TCL transaction-control statement with a
single official synopsis branch: ``SAVEPOINT savepoint_name``.  The
statement establishes a savepoint within the current transaction.  No
TABLE is created, so the bookend (DROP TABLE IF EXISTS) is never
emitted.  The obligation ledger has 1 GRM obligation (the single
...
SAVEPOINT is a no-DB tickoff statement: validation is via a static
...
class SavepointFactorLoopError(ValueError):
    """Raised when a frozen SAVEPOINT obligation input drifts."""


@dataclass(frozen=True)
class SavepointGrammarAction:
    """One official target action form of the SAVEPOINT synopsis."""
...
@dataclass(frozen=True)
class SavepointFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class SavepointFactorLoopPlan:
    obligations: tuple[SavepointFactorObligation, ...]
    cases: tuple[SavepointFactorCase, ...]
    delegated: tuple[SavepointFactorObligation, ...]
...
_DOC_SOURCE = "postgresql-18.4-doc:sql-savepoint"

# The single SAVEPOINT synopsis action.
_REPRESENTATIVE_ACTION = "savepoint"
...
# Canonical factor -> the action where the value is observable.  SAVEPOINT
# has only one synopsis, so every factor is observed through
# ``savepoint``.
...
#
# SAVEPOINT mirrors COMMIT's failure structure: it is a session-level
# TCL statement.  ``transaction_state=outside_transaction`` is a failure
# (warning 25P01 -- no transaction in progress).  The
...
def _load_grammar_actions() -> (
    tuple[SavepointGrammarAction, ...]
):
    """Freeze the single SAVEPOINT synopsis target action."""
    ...
    if len(actions) != 1:
        raise SavepointFactorLoopError(
            "savepoint action count drift"
        )
...
def _compile_grammar_obligations() -> (
    list[SavepointFactorObligation]
):
    rows: list[SavepointFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            SavepointFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SAVEPOINT-GRM|{action.grammar_branch_id}|"
    ...
    for row in catalog_rows:
        ...
        rows.append(
            SavepointFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SAVEPOINT-SFV|{row.row_id}|{consumer}"
                ),
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[SavepointFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"savepoint-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
#
# SAVEPOINT's normal success case is establishing a savepoint within an
# *active* transaction, so the default ``transaction_state`` is
# ``inside_transaction``.
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    SAVEPOINT mirrors COMMIT's derivation: only the
...
def _baseline_assignments(
    obligation: SavepointFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_savepoint_factor_loop_plan(
    repository_root: Path,
) -> SavepointFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_savepoint_factor_loop_obligations(root)
    cases: list[SavepointFactorCase] = []
    delegated: list[SavepointFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            SavepointFactorCase(
                ordinal=ordinal,
                case_id=f"SAVEPOINT{ordinal:05d}",
                sql_filename=f"SAVEPOINT{ordinal:05d}.sql",
                object_prefix=f"savepoint_{ordinal:05d}_",
...
def compile_savepoint_factor_loop_obligations(
    repository_root: Path,
) -> tuple[SavepointFactorObligation, ...]:
...
__all__ = [
    ...
    "_SFV_FAILURE_VALUES",
    "compile_savepoint_factor_loop_obligations",
    "build_savepoint_factor_loop_plan",
