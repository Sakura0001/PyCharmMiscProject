"""Actual-byte coverage validation for TRUNCATE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 53-case baseline
...
from .truncate_factor_render import (
...
class TruncateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "truncate_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_truncate_factor_witness(
        case, repository_root
    )
    expected_sql = render_truncate_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_truncate(sql) != 1:
...
def validate_truncate_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> TruncateFactorProgramValidation:
    ...
    return TruncateFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_truncate_factor_programs",
]
