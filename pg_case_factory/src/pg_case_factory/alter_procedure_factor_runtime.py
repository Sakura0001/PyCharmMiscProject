"""Bounded PostgreSQL 18.4 runtime for ALTER PROCEDURE factor programs."""
...
from .alter_procedure_factor_loop import AlterProcedureFactorCase
...
class AlterProcedureRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterProcedureCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterProcedureSuiteRun:
    run_ordinal: int
    cases: tuple[AlterProcedureCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterProcedureRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_procedure_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterProcedureTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_procedure_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_procedure_runs(
    run_01: AlterProcedureSuiteRun,
    run_02: AlterProcedureSuiteRun,
) -> AlterProcedureTwoRunComparison:
    ...
    return AlterProcedureTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterProcedureRuntimeCase(Protocol):
...
def build_alter_procedure_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterProcedureRuntimeCase, ...], dict[str, Path]]:
    ...
    from .alter_procedure_factor_loop import build_alter_procedure_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_procedure_factor_loop_plan(root).cases
    extensions = build_alter_procedure_factor_extension_plan(root).cases
    cases: list[AlterProcedureRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterProcedureRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterProcedurePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterProcedureRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterProcedureRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            "UNION ALL SELECT 1 FROM pg_catalog.pg_collation WHERE collname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_data_wrapper WHERE fdwname LIKE '" + literal + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterProcedureRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterProcedureRuntimeCase,
        sql_path: Path,
    ) -> AlterProcedureCaseRuntimeResult:
        ...
        return AlterProcedureCaseRuntimeResult(
...
__all__ = [
    "AlterProcedureCaseRuntimeResult",
    "AlterProcedurePg18Runner",
    "AlterProcedureRuntimeCase",
    "AlterProcedureRuntimeError",
    "AlterProcedureSuiteRun",
    "AlterProcedureTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_procedure_runtime_case_set",
    "compare_alter_procedure_runs",
]
