"""Bounded PostgreSQL 18.4 runtime for CREATE INDEX factor programs.
...
from .create_index_factor_loop import CreateIndexFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cinx-sock-20260820")
PG18_PORT = 55603
PG18_DATABASE = "pgcf_cinx"
...
class CreateIndexRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateIndexCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateIndexSuiteRun:
    run_ordinal: int
    cases: tuple[CreateIndexCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateIndexRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_index_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateIndexTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_index_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_index_runs(
    run_01: CreateIndexSuiteRun,
    run_02: CreateIndexSuiteRun,
) -> CreateIndexTwoRunComparison:
    ...
    return CreateIndexTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateIndexRuntimeCase(Protocol):
...
def build_create_index_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateIndexRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_index_factor_loop_plan(root).cases
    extensions = build_create_index_factor_extension_plan(root).cases
    cases: list[CreateIndexRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateIndexRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateIndexPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateIndexRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_index WHERE "
            "indexrelid::regclass::text LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: CreateIndexRuntimeCase,
        sql_path: Path,
    ) -> CreateIndexCaseRuntimeResult:
        ...
        return CreateIndexCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateIndexRuntimeCase],
        ...
        *,
        ...
        results: list[CreateIndexCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateIndexCaseRuntimeResult",
    "CreateIndexPg18Runner",
    "CreateIndexRuntimeCase",
    "CreateIndexRuntimeError",
    "CreateIndexSuiteRun",
    "CreateIndexTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_index_runtime_case_set",
    "compare_create_index_runs",
]
