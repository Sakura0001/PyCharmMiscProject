"""Bounded PostgreSQL 18.4 runtime for DROP TEXT SEARCH TEMPLATE factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-dtst-sock-20260821")
PG18_PORT = 55512
PG18_DATABASE = "pgcf_dtst"
...
class DropTextSearchTemplateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTextSearchTemplateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTextSearchTemplateSuiteRun:
    run_ordinal: int
    cases: tuple[DropTextSearchTemplateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTextSearchTemplateRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_template_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTextSearchTemplateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_template_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_text_search_template_runs(
    run_01: DropTextSearchTemplateSuiteRun,
    run_02: DropTextSearchTemplateSuiteRun,
    ...
    return DropTextSearchTemplateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTextSearchTemplateRuntimeCase(Protocol):
...
def build_drop_text_search_template_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[DropTextSearchTemplateRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_drop_text_search_template_factor_loop_plan(root).cases
    extensions = build_drop_text_search_template_factor_extension_plan(
        root
    ).cases
    cases: list[DropTextSearchTemplateRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: DropTextSearchTemplateRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTextSearchTemplatePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTextSearchTemplateRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_ts_template WHERE tmplname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_ts_dict WHERE dictname LIKE '"
            + literal
            + "' "
    ...
    def run_cases(
        self,
        cases: Sequence[DropTextSearchTemplateRuntimeCase],
        ...
        *,
        ...
        results: list[DropTextSearchTemplateCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropTextSearchTemplateSuiteRun(
...
__all__ = [
    ...
    "DropTextSearchTemplateRuntimeCase",
