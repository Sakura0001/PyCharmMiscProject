"""Bounded PostgreSQL 18.4 runtime for EXECUTE factor programs."""
...
from .execute_factor_loop import ExecuteFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-exec-sock-20260821")
PG18_PORT = 55620
PG18_DATABASE = "pgcf_exec"
...
class ExecuteCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class ExecuteSuiteRun:
    run_ordinal: int
    cases: tuple[ExecuteCaseRuntimeResult, ...]
...
class ExecuteTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "execute_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_execute_runs(
    run_01: ExecuteSuiteRun,
    run_02: ExecuteSuiteRun,
) -> ExecuteTwoRunComparison:
    ...
    return ExecuteTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ExecuteRuntimeCase(Protocol):
...
def build_execute_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ExecuteRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_execute_factor_loop_plan(root).cases
    extensions = (
        build_execute_factor_extension_plan(root).cases
    )
    cases: list[ExecuteRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: ExecuteRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ExecutePg18Runner:
    ...
    def _execute_case(
        self,
        case: ExecuteRuntimeCase,
        sql_path: Path,
    ) -> ExecuteCaseRuntimeResult:
        ...
        return ExecuteCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ExecuteRuntimeCase],
        ...
        *,
        ...
        results: list[ExecuteCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_execute_runtime_case_set",
    "compare_execute_runs",
]
