"""Bounded post-coverage cross-factor extension expander for DELETE.

The marginal factor-value-loop (:mod:`delete_factor_loop`) is the
...
allowed by ``delete.yaml`` ``post_coverage_extension_policy.enabled:
...
DELETE mutates a ``pg_class`` base-table relation; the failure-causing
...
class DeleteFactorExtensionError(ValueError):
    """Raised when a frozen DELETE extension input drifts."""


@dataclass(frozen=True)
class DeleteFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DeleteFactorExtensionPlan:
    cases: tuple[DeleteFactorExtensionCase, ...]
...
# behaviour axes used for CALL (DELETE shares the DML 15-factor schema),
# so the combinatorics are identical.
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise DeleteFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"delete-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DELETE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "delete_factor_extension",
...
def build_delete_factor_extension_plan(
    repository_root: Path,
) -> DeleteFactorExtensionPlan:
    """Build the bounded DELETE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_delete_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DeleteFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DeleteFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DELETE-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "delete_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return DeleteFactorExtensionPlan(
        cases=cases_tuple,
