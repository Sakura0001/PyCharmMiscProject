"""Bounded PostgreSQL 18.4 runtime for CREATE SERVER factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-csrv-sock-20260820")
PG18_PORT = 55545
PG18_DATABASE = "pgcf_csrv"
...
class CreateServerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateServerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateServerSuiteRun:
    run_ordinal: int
    cases: tuple[CreateServerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateServerRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_server_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateServerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_server_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_server_runs(
    run_01: CreateServerSuiteRun,
    run_02: CreateServerSuiteRun,
) -> CreateServerTwoRunComparison:
    ...
    return CreateServerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateServerRuntimeCase(Protocol):
...
def build_create_server_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateServerRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_server_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_server_factor_extension_plan(root).cases
    )
    cases: list[CreateServerRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateServerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateServerPg18Runner:
    ...
    def _execute_case(
        self,
        case: CreateServerRuntimeCase,
        sql_path: Path,
    ) -> CreateServerCaseRuntimeResult:
        ...
        return CreateServerCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[CreateServerCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateServerCaseRuntimeResult",
    "CreateServerPg18Runner",
    "CreateServerRuntimeCase",
    "CreateServerRuntimeError",
    "CreateServerSuiteRun",
    "CreateServerTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_server_runtime_case_set",
    "compare_create_server_runs",
]
