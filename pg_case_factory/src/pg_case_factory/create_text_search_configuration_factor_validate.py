"""Actual-byte coverage validation for CREATE TEXT SEARCH CONFIGURATION factor-loop SQL.
...
from .create_text_search_configuration_factor_render import (
...
class CreateTextSearchConfigurationFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    if (
        ...
        or count_primary_create_text_search_configuration(sql) != 1
    ):
...
def validate_create_text_search_configuration_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateTextSearchConfigurationFactorProgramValidation(
        passed=not issues,
