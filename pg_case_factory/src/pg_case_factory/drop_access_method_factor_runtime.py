"""Bounded PostgreSQL 18.4 runtime for DROP ACCESS METHOD factor programs."""
...
from .drop_access_method_factor_loop import DropAccessMethodFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dam-sock-20260820")
PG18_PORT = 55490
PG18_DATABASE = "pgcf_dam"
...
class DropAccessMethodRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropAccessMethodCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropAccessMethodSuiteRun:
    run_ordinal: int
    cases: tuple[DropAccessMethodCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropAccessMethodRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_access_method_pg18_suite_run",
...
@dataclass(frozen=True)
class DropAccessMethodTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_access_method_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_access_method_runs(
    run_01: DropAccessMethodSuiteRun,
    run_02: DropAccessMethodSuiteRun,
) -> DropAccessMethodTwoRunComparison:
    ...
    return DropAccessMethodTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropAccessMethodRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline
    :class:`DropAccessMethodFactorCase` and the bounded extension case; the
    runner only reads the identity + expected-state attributes, which both
    carry.
    """
...
def build_drop_access_method_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropAccessMethodRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0031) precedes the
    extension (0032-0367) in a stable, human-auditable order.
    """
    ...
    baseline = build_drop_access_method_factor_loop_plan(root).cases
    extensions = build_drop_access_method_factor_extension_plan(root).cases
    cases: list[DropAccessMethodRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropAccessMethodRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropAccessMethodPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropAccessMethodRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropAccessMethodRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_am WHERE amname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropAccessMethodRuntimeCase,
        sql_path: Path,
    ) -> DropAccessMethodCaseRuntimeResult:
        ...
        return DropAccessMethodCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropAccessMethodSuiteRun(
...
__all__ = [
    "DropAccessMethodCaseRuntimeResult",
    "DropAccessMethodPg18Runner",
    "DropAccessMethodRuntimeCase",
    "DropAccessMethodRuntimeError",
    "DropAccessMethodSuiteRun",
    "DropAccessMethodTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_access_method_runtime_case_set",
    "compare_drop_access_method_runs",
]
