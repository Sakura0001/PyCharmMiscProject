"""Bounded PostgreSQL 18.4 runtime for DROP TEXT SEARCH PARSER factor programs."""
...
from .drop_text_search_parser_factor_loop import DropTsParserFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dtsp-sock-20260821")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dtsp"
...
class DropTsParserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTsParserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTsParserSuiteRun:
    run_ordinal: int
    cases: tuple[DropTsParserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTsParserRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_parser_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTsParserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_parser_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_text_search_parser_runs(
    run_01: DropTsParserSuiteRun,
    run_02: DropTsParserSuiteRun,
) -> DropTsParserTwoRunComparison:
    ...
    return DropTsParserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTsParserRuntimeCase(Protocol):
...
def build_drop_text_search_parser_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTsParserRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_text_search_parser_factor_loop_plan(root).cases
    extensions = build_drop_text_search_parser_factor_extension_plan(root).cases
    cases: list[DropTsParserRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTsParserRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTsParserPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropTsParserRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTsParserRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_ts_parser WHERE prsname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTsParserRuntimeCase,
        sql_path: Path,
    ) -> DropTsParserCaseRuntimeResult:
        ...
        return DropTsParserCaseRuntimeResult(
...
__all__ = [
    "DropTsParserCaseRuntimeResult",
    "DropTsParserPg18Runner",
    "DropTsParserRuntimeCase",
    "DropTsParserRuntimeError",
    "DropTsParserSuiteRun",
    "DropTsParserTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_text_search_parser_runtime_case_set",
    "compare_drop_text_search_parser_runs",
]
