"""Bounded PostgreSQL 18.4 runtime for SET ROLE factor programs."""
...
from .set_role_factor_loop import SetRoleFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-set-role-sock-20260821")
PG18_PORT = 55622
PG18_DATABASE = "pgcf_set_role"
...
@dataclass(frozen=True)
class SetRoleSuiteRun:
    run_ordinal: int
    cases: tuple[SetRoleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SetRoleRuntimeError(
...
class SetRoleTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_role_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_set_role_runs(
    run_01: SetRoleSuiteRun,
    run_02: SetRoleSuiteRun,
) -> SetRoleTwoRunComparison:
    ...
    return SetRoleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SetRoleRuntimeCase(Protocol):
...
def build_set_role_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SetRoleRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_set_role_factor_loop_plan(root).cases
    )
    extensions = (
        build_set_role_factor_extension_plan(root).cases
    )
    cases: list[SetRoleRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class SetRolePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise SetRoleRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return SetRoleCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[SetRoleRuntimeCase],
        ...
        *,
        ...
        results: list[SetRoleCaseRuntimeResult] = []
        for case in cases:
