"""Bounded PostgreSQL 18.4 runtime for MERGE factor programs.
...
from .merge_factor_loop import MergeFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-merge-sock-20260821")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_merge"
...
class MergeSuiteRun:
    run_ordinal: int
    cases: tuple[MergeCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise MergeRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "merge_pg18_suite_run",
...
class MergeTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "merge_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_merge_runs(
    run_01: MergeSuiteRun,
    run_02: MergeSuiteRun,
) -> MergeTwoRunComparison:
...
class MergeRuntimeCase(Protocol):
...
def build_merge_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[MergeRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .merge_factor_extension import (
        build_merge_factor_extension_plan,
    )
    ...
    baseline = build_merge_factor_loop_plan(root).cases
    extensions = build_merge_factor_extension_plan(root).cases
    cases: list[MergeRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: MergeRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class MergePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise MergeRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: MergeRuntimeCase,
        sql_path: Path,
    ) -> MergeCaseRuntimeResult:
    ...
    def run_cases(
        self,
        cases: Sequence[MergeRuntimeCase],
        ...
        *,
        ...
        results: list[MergeCaseRuntimeResult] = []
        for case in cases:
