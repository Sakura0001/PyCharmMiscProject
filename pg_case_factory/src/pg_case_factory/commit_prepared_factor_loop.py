"""Factor-value-loop obligation ledger for PostgreSQL 18.4 COMMIT PREPARED.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``COMMIT PREPARED``.  COMMIT PREPARED is a PostgreSQL two-phase transaction
control statement that commits a transaction earlier prepared for
two-phase commit.  The official synopsis has exactly one branch
(``COMMIT PREPARED transaction_id``), so there is a single ``GRM``
obligation.  The 43 canonical ``SFV`` rows are loaded from the shipped
applicability universe (``postgresql_18_4_factor_audit.tsv``).

COMMIT PREPARED reaches several PostgreSQL error surfaces.  Six canonical
...
* ``transaction_state=inside_transaction`` -- COMMIT PREPARED cannot run
  inside a transaction block (25001).
* ``transaction_state=savepoint_exists`` -- a savepoint implies an open
  transaction block, which blocks COMMIT PREPARED (25001).
...
success-covered.  There are no ``RISK`` obligations (COMMIT PREPARED has
...
class CommitPreparedFactorLoopError(ValueError):
    """Raised when a frozen COMMIT PREPARED obligation input drifts."""


@dataclass(frozen=True)
class CommitPreparedGrammarAction:
    """One official target action form of the COMMIT PREPARED synopsis."""
...
@dataclass(frozen=True)
class CommitPreparedFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class CommitPreparedFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CommitPreparedFactorLoopPlan:
    obligations: tuple[CommitPreparedFactorObligation, ...]
    cases: tuple[CommitPreparedFactorCase, ...]
    delegated: tuple[CommitPreparedFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-commit-prepared.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-commit-prepared"

# The single representative action for all canonical factors.  COMMIT
# PREPARED has only one synopsis form, so every factor observes on the
# single ``commit_prepared`` action.
_REPRESENTATIVE_ACTION = "commit_prepared"

# statement_branch canonical value -> target action.
_STATEMENT_BRANCH_CONSUMER = {
    _BRANCH_1: "commit_prepared",
}

# Canonical factor -> the action where the value is observable.  Since
# COMMIT PREPARED has only one synopsis form, every factor observes on the
# single ``commit_prepared`` action.
...
def _load_grammar_actions() -> tuple[CommitPreparedGrammarAction, ...]:
    """Freeze every COMMIT PREPARED synopsis target action."""
    ...
    rows: tuple[tuple[str, str, str, str], ...] = (
        (
            ...
            "synopsis-commit-prepared",
        ),
    )
    actions = [
        CommitPreparedGrammarAction(
            action_id=action_id,
    ...
    if len(actions) != 1:
        raise CommitPreparedFactorLoopError(
            "commit prepared action count drift"
        )
...
def _compile_grammar_obligations() -> list[CommitPreparedFactorObligation]:
    rows: list[CommitPreparedFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            CommitPreparedFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"CPREP-GRM|{action.grammar_branch_id}|"
...
def _compile_canonical_obligations(
...
) -> list[CommitPreparedFactorObligation]:
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("commit_prepared")
    ...
    for row in catalog_rows:
        ...
        rows.append(
            CommitPreparedFactorObligation(
                ordinal=0,
                obligation_id=f"CPREP-SFV|{row.row_id}|{consumer}",
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[CommitPreparedFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"commit-prepared-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
# are always branch_1 / commit_prepared.
_BASELINE_DEFAULTS: dict[str, str] = {
    "statement_branch": _BRANCH_1,
    "grammar_branch": _BRANCH_1,
    "target_action": "commit_prepared",
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    COMMIT PREPARED's reachable failure surfaces cluster around three
...
def _baseline_assignments(
    obligation: CommitPreparedFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_commit_prepared_factor_loop_plan(
    repository_root: Path,
) -> CommitPreparedFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_commit_prepared_factor_loop_obligations(root)
    cases: list[CommitPreparedFactorCase] = []
    delegated: list[CommitPreparedFactorObligation] = []
    for obligation in obligations:
        ...
        if obligation.disposition == "expected_failure":
            ...
            failure_reason = None
        cases.append(
            CommitPreparedFactorCase(
                ordinal=ordinal,
                case_id=f"COMMITPREPARED{ordinal:05d}",
                sql_filename=f"COMMITPREPARED{ordinal:05d}.sql",
                object_prefix=(
                    f"commitprepared_{ordinal:05d}_"
                ),
    ...
    plan = CommitPreparedFactorLoopPlan(
    ...
    if len({row.sql_filename for row in plan.cases}) != 44:
        raise CommitPreparedFactorLoopError("duplicate SQL filename")
    return plan


def compile_commit_prepared_factor_loop_obligations(
    repository_root: Path,
) -> tuple[CommitPreparedFactorObligation, ...]:
    ...
    if len(rows) != 44:
        raise CommitPreparedFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise CommitPreparedFactorLoopError("duplicate obligation id")
...
__all__ = [
    ...
    "_SFV_FAILURE_VALUES",
    "compile_commit_prepared_factor_loop_obligations",
