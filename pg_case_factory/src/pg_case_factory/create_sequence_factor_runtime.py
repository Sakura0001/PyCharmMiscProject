"""Bounded PostgreSQL 18.4 runtime for CREATE SEQUENCE factor programs."""
...
from .create_sequence_factor_loop import CreateSequenceFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-csq-sock-20260820")
PG18_PORT = 55607
PG18_DATABASE = "pgcf_csq"
...
class CreateSequenceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateSequenceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateSequenceSuiteRun:
    run_ordinal: int
    cases: tuple[CreateSequenceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateSequenceRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_sequence_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateSequenceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_sequence_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_sequence_runs(
    run_01: CreateSequenceSuiteRun,
    run_02: CreateSequenceSuiteRun,
) -> CreateSequenceTwoRunComparison:
    ...
    return CreateSequenceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateSequenceRuntimeCase(Protocol):
...
def build_create_sequence_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateSequenceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_sequence_factor_loop_plan(root).cases
    extensions = build_create_sequence_factor_extension_plan(root).cases
    cases: list[CreateSequenceRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateSequenceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateSequencePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateSequenceRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateSequenceRuntimeCase,
        sql_path: Path,
    ) -> CreateSequenceCaseRuntimeResult:
        ...
        return CreateSequenceCaseRuntimeResult(
...
__all__ = [
    "CreateSequenceCaseRuntimeResult",
    "CreateSequencePg18Runner",
    "CreateSequenceRuntimeCase",
    "CreateSequenceRuntimeError",
    "CreateSequenceSuiteRun",
    "CreateSequenceTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_sequence_runtime_case_set",
    "compare_create_sequence_runs",
]
