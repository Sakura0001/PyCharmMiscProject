"""Bounded post-coverage cross-factor extension expander for SAVEPOINT.

The marginal factor-value-loop (:mod:`savepoint_factor_loop`) is the
...
by ``savepoint.yaml`` ``post_coverage_extension_policy.enabled:
true``: cross-factor combinations of the positive T1-T2 behaviour axes across
the single SAVEPOINT synopsis branch, with at most one
...
SAVEPOINT is a TCL transaction-control statement with one synopsis
...
class SavepointFactorExtensionError(ValueError):
    """Raised when a frozen SAVEPOINT extension input drifts."""


@dataclass(frozen=True)
class SavepointFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SavepointFactorExtensionPlan:
    cases: tuple[SavepointFactorExtensionCase, ...]
...
# General axes crossed for the single SAVEPOINT branch.
...
# target_action are overridden per branch.  SAVEPOINT's success
# default for ``transaction_state`` is ``inside_transaction`` (an active
# transaction in which to establish a savepoint).
...
# attribution.
#
# SAVEPOINT: the failure-causing transaction_state values
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    SAVEPOINT mirrors COMMIT's derivation.
    """
    ...
    for values in itertools.product(
        *[_GENERAL_AXES[n] for n in names]
        ...
        if len(assignment) != len(set(assignment)):
            raise SavepointFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"savepoint-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"SAVEPOINT extension {case.ordinal:05d}: "
...
def build_savepoint_factor_extension_plan(
    repository_root: Path,
) -> SavepointFactorExtensionPlan:
    """Build the bounded SAVEPOINT post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_savepoint_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[SavepointFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    SavepointFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"SAVEPOINT{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"savepoint_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"SAVEPOINT-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "savepoint_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return SavepointFactorExtensionPlan(
        cases=cases_tuple,
