...
def _resolve_case(
    ...
    if not missing and not wrong:
        proc_drops.append(f"DROP PROCEDURE IF EXISTS {proc};")
    elif wrong:
        proc_drops.append(f"DROP FUNCTION IF EXISTS {proc};")
    ...
    table_drops = [f"DROP TABLE IF EXISTS {tbl};"]

    # pre-cleanup: tables first (bookend), then procedures, then roles
    pre_cleanup: list[str] = []
    pre_cleanup.extend(table_drops)
    pre_cleanup.extend(proc_drops)
    pre_cleanup.extend(role_drops)

    # cleanup: RESET ROLE, procedures, roles, tables last (bookend)
    cleanup: list[str] = []
    if effective:
