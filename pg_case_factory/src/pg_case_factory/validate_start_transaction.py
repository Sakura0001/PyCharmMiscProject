"""Actual-byte coverage validation for START TRANSACTION factor-loop SQL.
...
from .start_transaction_factor_render import (
...
class StartTransactionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "start_transaction_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_start_transaction_factor_witness(
        case, repository_root
    )
    expected_sql = render_start_transaction_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_start_transaction(sql) != 1:
...
def validate_start_transaction_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return StartTransactionFactorProgramValidation(
        passed=not issues,
