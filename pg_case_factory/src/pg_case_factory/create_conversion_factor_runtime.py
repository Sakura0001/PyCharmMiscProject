"""Bounded PostgreSQL 18.4 runtime for CREATE CONVERSION factor programs."""
...
from .create_conversion_factor_loop import CreateConversionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cconv-sock-20260820")
PG18_PORT = 55520
PG18_DATABASE = "pgcf_cconv"
...
class CreateConversionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateConversionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateConversionSuiteRun:
    run_ordinal: int
    cases: tuple[CreateConversionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateConversionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_conversion_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateConversionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_conversion_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_conversion_runs(
    run_01: CreateConversionSuiteRun,
    run_02: CreateConversionSuiteRun,
) -> CreateConversionTwoRunComparison:
    ...
    return CreateConversionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateConversionRuntimeCase(Protocol):
...
def build_create_conversion_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateConversionRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_create_conversion_factor_loop_plan(root).cases
    extensions = (
        build_create_conversion_factor_extension_plan(root).cases
    )
    cases: list[CreateConversionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateConversionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateConversionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateConversionRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_conversion WHERE conname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateConversionRuntimeCase,
        sql_path: Path,
    ) -> CreateConversionCaseRuntimeResult:
        ...
        return CreateConversionCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return CreateConversionSuiteRun(
...
__all__ = [
    "CreateConversionCaseRuntimeResult",
    "CreateConversionPg18Runner",
    "CreateConversionRuntimeCase",
    "CreateConversionRuntimeError",
    "CreateConversionSuiteRun",
    "CreateConversionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_conversion_runtime_case_set",
    "compare_create_conversion_runs",
]
