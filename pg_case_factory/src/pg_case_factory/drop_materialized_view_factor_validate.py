"""Actual-byte coverage validation for DROP MATERIALIZED VIEW factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 37-case baseline +
...
from .drop_materialized_view_factor_render import (
...
class DropMaterializedViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_materialized_view_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_materialized_view_factor_witness(
        case, repository_root
    )
    expected_sql = render_drop_materialized_view_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_drop_materialized_view(sql) != 1:
...
def validate_drop_materialized_view_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropMaterializedViewFactorProgramValidation(
        passed=not issues,
