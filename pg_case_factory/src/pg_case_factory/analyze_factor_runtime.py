"""Bounded PostgreSQL 18.4 runtime for ANALYZE factor programs.

Construction and comparison logic for a serial, bounded psql executor
against an isolated PostgreSQL 18.4 server.  In the no-DB phase this
module is present and tested for construction/comparison logic but does
not execute against a live server.
"""
...
from .analyze_factor_loop import AnalyzeFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-analyze-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_analyze"
...
@dataclass(frozen=True)
class AnalyzeSuiteRun:
    run_ordinal: int
    cases: tuple[AnalyzeCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AnalyzeRuntimeError(
...
class AnalyzeTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "analyze_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_analyze_runs(
    run_01: AnalyzeSuiteRun,
    run_02: AnalyzeSuiteRun,
) -> AnalyzeTwoRunComparison:
    ...
    return AnalyzeTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AnalyzeRuntimeCase(Protocol):
...
def build_analyze_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AnalyzeRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_analyze_factor_loop_plan(root).cases
    extensions = build_analyze_factor_extension_plan(root).cases
    cases: list[AnalyzeRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AnalyzeRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AnalyzePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AnalyzeRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: AnalyzeRuntimeCase,
        sql_path: Path,
    ) -> AnalyzeCaseRuntimeResult:
        ...
        return AnalyzeCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[AnalyzeRuntimeCase],
        ...
        *,
        ...
        results: list[AnalyzeCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_analyze_runtime_case_set",
    "compare_analyze_runs",
]
