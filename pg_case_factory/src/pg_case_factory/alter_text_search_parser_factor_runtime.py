"""Bounded PostgreSQL 18.4 runtime for ALTER TEXT SEARCH PARSER factor programs."""
...
from .alter_text_search_parser_factor_loop import AlterTextSearchParserFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-atsp-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_atsp"
...
class AlterTextSearchParserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTextSearchParserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTextSearchParserSuiteRun:
    run_ordinal: int
    cases: tuple[AlterTextSearchParserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTextSearchParserRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_text_search_parser_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTextSearchParserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_parser_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_alter_text_search_parser_runs(
    run_01: AlterTextSearchParserSuiteRun,
    run_02: AlterTextSearchParserSuiteRun,
) -> AlterTextSearchParserTwoRunComparison:
    ...
    return AlterTextSearchParserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTextSearchParserRuntimeCase(Protocol):
...
def build_alter_text_search_parser_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterTextSearchParserRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_text_search_parser_factor_loop_plan(root).cases
    extensions = build_alter_text_search_parser_factor_extension_plan(root).cases
    cases: list[AlterTextSearchParserRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterTextSearchParserRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTextSearchParserPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterTextSearchParserRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_ts_parser WHERE prsname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: AlterTextSearchParserRuntimeCase,
        sql_path: Path,
    ) -> AlterTextSearchParserCaseRuntimeResult:
        ...
        return AlterTextSearchParserCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return AlterTextSearchParserSuiteRun(
...
__all__ = [
    "AlterTextSearchParserCaseRuntimeResult",
    "AlterTextSearchParserPg18Runner",
    "AlterTextSearchParserRuntimeCase",
    "AlterTextSearchParserRuntimeError",
    "AlterTextSearchParserSuiteRun",
    "AlterTextSearchParserTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_text_search_parser_runtime_case_set",
    "compare_alter_text_search_parser_runs",
]
