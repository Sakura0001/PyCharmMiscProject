"""Bounded PostgreSQL 18.4 runtime for INSERT factor programs.

This module provides the runtime case-set combiner and psql executor
for the no-DB factor-regress phase.  In the no-DB phase, the runtime is
present and tested for construction/comparison logic but does NOT
execute against a live PostgreSQL server.  SQLSTATE expectations are
provisional and verified later in a DB phase on PG 18.4.
"""
...
from .insert_factor_loop import InsertFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-insert-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_insert"
...
@dataclass(frozen=True)
class InsertSuiteRun:
    run_ordinal: int
    cases: tuple[InsertCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise InsertRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "insert_pg18_suite_run",
...
class InsertTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "insert_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_insert_runs(
    run_01: InsertSuiteRun,
    run_02: InsertSuiteRun,
) -> InsertTwoRunComparison:
...
class InsertRuntimeCase(Protocol):
...
def build_insert_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[InsertRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .insert_factor_extension import (
        build_insert_factor_extension_plan,
    )
    ...
    baseline = build_insert_factor_loop_plan(root).cases
    extensions = build_insert_factor_extension_plan(root).cases
    cases: list[InsertRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: InsertRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class InsertPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise InsertRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: InsertRuntimeCase,
        sql_path: Path,
    ) -> InsertCaseRuntimeResult:
        ...
        return InsertCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[InsertRuntimeCase],
        ...
        *,
        ...
        results: list[InsertCaseRuntimeResult] = []
        for case in cases:
