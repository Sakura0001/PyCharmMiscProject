"""Actual-byte coverage validation for CREATE SUBSCRIPTION factor-loop SQL.
...
from .create_subscription_factor_render import (
...
class CreateSubscriptionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_subscription_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_subscription_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_subscription(sql) != 1:
...
def validate_create_subscription_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateSubscriptionFactorProgramValidation:
    ...
    return CreateSubscriptionFactorProgramValidation(
        passed=not issues,
