"""Actual-byte coverage validation for CREATE STATISTICS factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 56-case baseline
+ bounded extension) and compares them exactly against the on-disk SQL.
Fails closed on any drift: byte mismatch, header mismatch, primary-
target cardinality != 1, placeholder leakage, missing/duplicate/unknown
...
from .create_statistics_factor_render import (
...
class CreateStatisticsFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_statistics_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_statistics_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_statistics_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_statistics(sql) != 1:
...
def validate_create_statistics_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateStatisticsFactorProgramValidation:
    ...
    return CreateStatisticsFactorProgramValidation(
        passed=not issues,
