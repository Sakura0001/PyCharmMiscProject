"""Bounded PostgreSQL 18.4 runtime for DROP ROLE factor programs."""
...
from .drop_role_factor_loop import DropRoleFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-role-sock-20260820")
PG18_PORT = 55124
PG18_DATABASE = "pgcf_role"
...
class DropRoleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropRoleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropRoleSuiteRun:
    run_ordinal: int
    cases: tuple[DropRoleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropRoleRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_role_pg18_suite_run",
...
@dataclass(frozen=True)
class DropRoleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_role_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_role_runs(
    run_01: DropRoleSuiteRun,
    run_02: DropRoleSuiteRun,
    ...
    return DropRoleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropRoleRuntimeCase(Protocol):
...
def build_drop_role_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropRoleRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_role_factor_loop_plan(root).cases
    extensions = build_drop_role_factor_extension_plan(root).cases
    cases: list[DropRoleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropRoleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropRolePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropRoleRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[DropRoleRuntimeCase],
        ...
        *,
        ...
        results: list[DropRoleCaseRuntimeResult] = []
        for case in cases:
