"""Bounded PostgreSQL 18.4 runtime for CREATE VIEW factor programs."""
...
from .create_view_factor_loop import CreateViewFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cv-sock-20260820")
PG18_PORT = 55616
PG18_DATABASE = "pgcf_cv"
...
class CreateViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateViewSuiteRun:
    run_ordinal: int
    cases: tuple[CreateViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateViewRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_view_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_view_runs(
    run_01: CreateViewSuiteRun,
    run_02: CreateViewSuiteRun,
) -> CreateViewTwoRunComparison:
    ...
    return CreateViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateViewRuntimeCase(Protocol):
...
def build_create_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateViewRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_view_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_view_factor_extension_plan(root).cases
    )
    cases: list[CreateViewRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateViewPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateViewRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateViewRuntimeCase,
        sql_path: Path,
    ) -> CreateViewCaseRuntimeResult:
        ...
        return CreateViewCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateViewRuntimeCase],
        ...
        *,
        ...
        results: list[CreateViewCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateViewCaseRuntimeResult",
    "CreateViewPg18Runner",
    "CreateViewRuntimeCase",
    "CreateViewRuntimeError",
    "CreateViewSuiteRun",
    "CreateViewTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_view_runtime_case_set",
    "compare_create_view_runs",
]
