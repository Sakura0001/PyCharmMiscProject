"""Bounded PostgreSQL 18.4 runtime for VALUES factor programs."""
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-values-sock-20260821")
)
PG18_PORT = 55623
PG18_DATABASE = "pgcf_values"
...
@dataclass(frozen=True)
class ValuesSuiteRun:
    run_ordinal: int
    cases: tuple[ValuesCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ValuesRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "values_pg18_suite_run",
...
class ValuesTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "values_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_values_runs(
    run_01: ValuesSuiteRun,
    run_02: ValuesSuiteRun,
) -> ValuesTwoRunComparison:
...
class ValuesRuntimeCase(Protocol):
...
def build_values_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ValuesRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_values_factor_loop_plan(root).cases
    extensions = (
        build_values_factor_extension_plan(root).cases
    )
    cases: list[ValuesRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: ValuesRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ValuesPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ValuesRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: ValuesRuntimeCase,
        sql_path: Path,
    ) -> ValuesCaseRuntimeResult:
        ...
        return ValuesCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ValuesRuntimeCase],
        ...
        *,
        ...
        results: list[ValuesCaseRuntimeResult] = []
        for case in cases:
