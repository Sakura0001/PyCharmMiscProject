"""Bounded PostgreSQL 18.4 runtime for DROP ROUTINE factor programs."""
...
from .drop_routine_factor_loop import DropRoutineFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dr-sock-20260820")
PG18_PORT = 55502
PG18_DATABASE = "pgcf_dr"
...
class DropRoutineRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropRoutineCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropRoutineSuiteRun:
    run_ordinal: int
    cases: tuple[DropRoutineCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropRoutineRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_routine_pg18_suite_run",
...
@dataclass(frozen=True)
class DropRoutineTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_routine_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_routine_runs(
    run_01: DropRoutineSuiteRun,
    run_02: DropRoutineSuiteRun,
) -> DropRoutineTwoRunComparison:
    ...
    return DropRoutineTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropRoutineRuntimeCase(Protocol):
...
def build_drop_routine_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropRoutineRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_routine_factor_loop_plan(root).cases
    extensions = build_drop_routine_factor_extension_plan(root).cases
    cases: list[DropRoutineRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropRoutineRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropRoutinePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropRoutineRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise DropRoutineRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: DropRoutineRuntimeCase,
        sql_path: Path,
    ) -> DropRoutineCaseRuntimeResult:
        ...
        return DropRoutineCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropRoutineRuntimeCase],
        ...
        *,
        ...
        results: list[DropRoutineCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropRoutineCaseRuntimeResult",
    "DropRoutinePg18Runner",
    "DropRoutineRuntimeCase",
    "DropRoutineRuntimeError",
    "DropRoutineSuiteRun",
    "DropRoutineTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_routine_runtime_case_set",
    "compare_drop_routine_runs",
]
