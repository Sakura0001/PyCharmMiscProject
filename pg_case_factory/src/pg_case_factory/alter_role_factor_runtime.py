"""Bounded PostgreSQL 18.4 runtime for ALTER ROLE factor programs.

Serial, bounded ``psql`` executor for an isolated PostgreSQL 18.4 server.
The no-DB phase freezes the case set and the deterministic plan; the DB
phase fork runs every case twice (``run_ordinal`` 1 and 2) and compares
transcripts, structure, SQLSTATEs, and clean state via
:func:`compare_alter_role_runs`.  The runner probes the clean state via
``pg_catalog.pg_roles`` (ALTER ROLE touches role catalog objects, not
relations), so the pre/post clean probe checks for residual role names
matching the case ``object_prefix``.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ar-sock-20260820")
PG18_PORT = 55487
PG18_DATABASE = "pgcf_ar"
...
class AlterRoleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterRoleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterRoleSuiteRun:
    run_ordinal: int
    cases: tuple[AlterRoleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterRoleRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_role_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterRoleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_role_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_role_runs(
    run_01: AlterRoleSuiteRun,
    run_02: AlterRoleSuiteRun,
) -> AlterRoleTwoRunComparison:
    ...
    return AlterRoleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterRoleRuntimeCase(Protocol):
...
def build_alter_role_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterRoleRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_role_factor_loop_plan(root).cases
    extensions = build_alter_role_factor_extension_plan(root).cases
    cases: list[AlterRoleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterRoleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterRolePg18Runner:
    """Serial, bounded psql executor for an isolated PostgreSQL 18.4 server."""
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterRoleRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_roles WHERE rolname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_database WHERE datname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterRoleRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterRoleRuntimeCase,
        sql_path: Path,
    ) -> AlterRoleCaseRuntimeResult:
        ...
        return AlterRoleCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[AlterRoleRuntimeCase],
        ...
        *,
        ...
        results: list[AlterRoleCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "AlterRoleCaseRuntimeResult",
    "AlterRolePg18Runner",
    "AlterRoleRuntimeCase",
    "AlterRoleRuntimeError",
    "AlterRoleSuiteRun",
    "AlterRoleTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_role_runtime_case_set",
    "compare_alter_role_runs",
]
