"""Bounded PostgreSQL 18.4 runtime for ALTER STATISTICS factor programs.

This module mirrors :mod:`alter_schema_factor_runtime`.  The NO-DB ledger
is static-gate only (no doublerun needed): the test-suite exercises the
comparison and case-set helpers without an actual PostgreSQL server, and
the runtime is provided for parity with the doublerun contract used by
DB-backed ledgers.
"""
...
from .alter_statistics_factor_loop import AlterStatisticsFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-astat-sock-20260820")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_astat"
...
class AlterStatisticsRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterStatisticsCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterStatisticsSuiteRun:
    run_ordinal: int
    cases: tuple[AlterStatisticsCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterStatisticsRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_statistics_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterStatisticsTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_statistics_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_statistics_runs(
    run_01: AlterStatisticsSuiteRun,
    run_02: AlterStatisticsSuiteRun,
    ...
    return AlterStatisticsTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterStatisticsRuntimeCase(Protocol):
...
def build_alter_statistics_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterStatisticsRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_statistics_factor_loop_plan(root).cases
    extensions = build_alter_statistics_factor_extension_plan(root).cases
    cases: list[AlterStatisticsRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterStatisticsRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterStatisticsPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterStatisticsRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterStatisticsRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_statistic_ext WHERE stxname LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: AlterStatisticsRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[AlterStatisticsRuntimeCase],
        ...
        *,
        ...
        results: list[AlterStatisticsCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterStatisticsSuiteRun(
...
__all__ = [
    ...
    "AlterStatisticsRuntimeCase",
    "AlterStatisticsRuntimeError",
    "AlterStatisticsSuiteRun",
    "AlterStatisticsTwoRunComparison",
    "MAX_PARALLELISM",
