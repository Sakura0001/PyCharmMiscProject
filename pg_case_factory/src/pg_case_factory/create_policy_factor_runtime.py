"""Bounded PostgreSQL 18.4 runtime for CREATE POLICY factor programs."""
...
from .create_policy_factor_loop import CreatePolicyFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cpol-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_cpol"
...
class CreatePolicyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreatePolicyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreatePolicySuiteRun:
    run_ordinal: int
    cases: tuple[CreatePolicyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreatePolicyRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_policy_pg18_suite_run",
...
@dataclass(frozen=True)
class CreatePolicyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_policy_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_policy_runs(
    run_01: CreatePolicySuiteRun,
    run_02: CreatePolicySuiteRun,
) -> CreatePolicyTwoRunComparison:
    ...
    return CreatePolicyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreatePolicyRuntimeCase(Protocol):
...
def build_create_policy_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreatePolicyRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_policy_factor_loop_plan(root).cases
    extensions = build_create_policy_factor_extension_plan(root).cases
    cases: list[CreatePolicyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreatePolicyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreatePolicyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreatePolicyRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_policy WHERE polname LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: CreatePolicyRuntimeCase,
        sql_path: Path,
    ) -> CreatePolicyCaseRuntimeResult:
        ...
        return CreatePolicyCaseRuntimeResult(
...
__all__ = [
    "CreatePolicyCaseRuntimeResult",
    "CreatePolicyPg18Runner",
    "CreatePolicyRuntimeCase",
    "CreatePolicyRuntimeError",
    "CreatePolicySuiteRun",
    "CreatePolicyTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_policy_runtime_case_set",
    "compare_create_policy_runs",
]
