"""Bounded PostgreSQL 18.4 runtime for DROP SERVER factor programs."""
...
from .drop_server_factor_loop import DropServerFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dsr-sock-20260820")
PG18_PORT = 55502
PG18_DATABASE = "pgcf_dsr"
...
class DropServerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropServerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropServerSuiteRun:
    run_ordinal: int
    cases: tuple[DropServerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropServerRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_server_pg18_suite_run",
...
@dataclass(frozen=True)
class DropServerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_server_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_server_runs(
    run_01: DropServerSuiteRun,
    run_02: DropServerSuiteRun,
    ...
    return DropServerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropServerRuntimeCase(Protocol):
...
def build_drop_server_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropServerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_server_factor_loop_plan(root).cases
    extensions = build_drop_server_factor_extension_plan(root).cases
    cases: list[DropServerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropServerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropServerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropServerRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_foreign_server WHERE srvname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_data_wrapper WHERE fdwname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_user_mapping WHERE umserver IN (SELECT oid FROM pg_catalog.pg_foreign_server WHERE srvname LIKE '" + literal + "') "
    ...
    def run_cases(
        self,
        cases: Sequence[DropServerRuntimeCase],
        ...
        *,
        ...
        results: list[DropServerCaseRuntimeResult] = []
        for case in cases:
