"""Bounded post-coverage cross-factor extension expander for UPDATE.

The marginal factor-value-loop (:mod:`update_factor_loop`) is the
...
allowed by ``update.yaml`` ``post_coverage_extension_policy.enabled:
...
UPDATE mutates a ``pg_class`` base-table relation; the failure-causing
...
class UpdateFactorExtensionError(ValueError):
    """Raised when a frozen UPDATE extension input drifts."""


@dataclass(frozen=True)
class UpdateFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class UpdateFactorExtensionPlan:
    cases: tuple[UpdateFactorExtensionCase, ...]
...
# behaviour axes used for CALL (UPDATE shares the DML 15-factor schema),
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise UpdateFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"update-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"UPDATE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "update_factor_extension",
...
def build_update_factor_extension_plan(
    repository_root: Path,
) -> UpdateFactorExtensionPlan:
    """Build the bounded UPDATE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_update_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[UpdateFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    UpdateFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"UPDATE-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "update_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return UpdateFactorExtensionPlan(
        cases=cases_tuple,
