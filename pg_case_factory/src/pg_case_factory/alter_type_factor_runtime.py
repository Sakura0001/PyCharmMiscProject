"""Bounded PostgreSQL 18.4 runtime for ALTER TYPE factor programs.

This module is present and tested for construction/comparison logic in the
no-DB ledger phase.  It is NOT executed against a live PG 18.4 server
here; a later DB phase drives :class:`AlterTypePg18Runner` against an
isolated server.  The runtime constants are distinct from the
``alter_schema`` (port 55497) and ``alter_subscription`` (port 55498)
runtimes so the three statements never share a server instance.
"""
...
from .alter_type_factor_loop import AlterTypeFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-atype-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_atype"
...
class AlterTypeRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTypeCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTypeSuiteRun:
    run_ordinal: int
    cases: tuple[AlterTypeCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTypeRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_type_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTypeTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_type_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_type_runs(
    run_01: AlterTypeSuiteRun,
    run_02: AlterTypeSuiteRun,
    ...
    return AlterTypeTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTypeRuntimeCase(Protocol):
...
def build_alter_type_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterTypeRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_type_factor_loop_plan(root).cases
    extensions = build_alter_type_factor_extension_plan(root).cases
    cases: list[AlterTypeRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterTypeRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTypePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterTypeRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterTypeRuntimeCase],
        ...
        *,
        ...
        results: list[AlterTypeCaseRuntimeResult] = []
        for case in cases:
