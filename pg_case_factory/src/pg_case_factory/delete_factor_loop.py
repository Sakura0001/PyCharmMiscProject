"""Factor-value-loop obligation ledger for PostgreSQL 18.4 DELETE.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``DELETE``.  DELETE is a PostgreSQL DML row-mutation statement with a
...
(exists / missing / wrong_object_type) refers to the deleted relation.

The 15 canonical factors and their values are frozen in the shipped
combination matrix ``delete.yaml`` (the source of truth);
...
DELETE is a transactional DML statement and the inventory declares no
...
class DeleteFactorLoopError(ValueError):
    """Raised when a frozen DELETE obligation input drifts."""


@dataclass(frozen=True)
class DeleteGrammarAction:
    """One official target action form of the DELETE synopsis."""
...
@dataclass(frozen=True)
class DeleteFactorLoopPlan:
    obligations: tuple[DeleteFactorObligation, ...]
    cases: tuple[DeleteFactorCase, ...]
    delegated: tuple[DeleteFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-delete.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-delete"
_STATEMENT_REF = "references/statements/dml/table/delete.md"

# The single consumer action for all DELETE factors.  Since DELETE has
# only one branch, every factor's consumer is "delete".
_REPRESENTATIVE_ACTION = "delete"
...
# value.  DELETE targets a ``pg_class`` relation, so a missing target
...
# delete.yaml ``factor_contract.factors``.  The sum of len(values) is
...
def _load_grammar_actions() -> tuple[DeleteGrammarAction, ...]:
    """Freeze every DELETE synopsis target action."""
    ...
    if len(actions) != 1:
        raise DeleteFactorLoopError("delete action count drift")
...
def _canonical_consumer(row) -> str:
    try:
        ...
        raise DeleteFactorLoopError(
            f"canonical factor has no consumer action: {row.factor}"
        ) from exc


def _compile_grammar_obligations() -> list[DeleteFactorObligation]:
    rows: list[DeleteFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            DeleteFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DELETE-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise DeleteFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations(
    repository_root,  # noqa: ANN001  (Path)
) -> list[DeleteFactorObligation]:
    """Compile the 49 SFV obligations from the shipped applicability catalog."""

    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("delete")
    if len(catalog_rows) != 49:
        raise DeleteFactorLoopError("canonical obligation count drift")
    rows: list[DeleteFactorObligation] = []
    for row in catalog_rows:
        ...
        rows.append(
            DeleteFactorObligation(
                ordinal=0,
                obligation_id=f"DELETE-SFV|{row.row_id}|{consumer}",
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[DeleteFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"delete-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
# predicate, mirroring how CALL defaulted data_or_query_shape to
...
def _baseline_assignments(
    obligation: DeleteFactorObligation,
) -> tuple[tuple[str, str], ...]:
    ...
    if len(assignments) != len(set(assignments)):
        raise DeleteFactorLoopError("duplicate baseline factor key")
...
def build_delete_factor_loop_plan(
    repository_root,  # noqa: ANN001  (Path; kept for API parity)
) -> DeleteFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    obligations = compile_delete_factor_loop_obligations(repository_root)
    cases: list[DeleteFactorCase] = []
    delegated: list[DeleteFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            DeleteFactorCase(
                ordinal=ordinal,
                case_id=f"DELETE{ordinal:05d}",
                sql_filename=f"DELETE{ordinal:05d}.sql",
                object_prefix=f"delete_{ordinal:05d}_",
    ...
    if len(plan.cases) != 50 or len(plan.delegated) != 0:
        raise DeleteFactorLoopError("factor loop plan count drift")
    if len({row.primary_obligation_id for row in plan.cases}) != 50:
        raise DeleteFactorLoopError("local obligation mapping drift")
    if len({row.sql_filename for row in plan.cases}) != 50:
        raise DeleteFactorLoopError("duplicate SQL filename")
    return plan


def compile_delete_factor_loop_obligations(
    repository_root,  # noqa: ANN001  (Path; kept for API parity)
) -> tuple[DeleteFactorObligation, ...]:
    ...
    if len(rows) != 50:
        raise DeleteFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise DeleteFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 49}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise DeleteFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise DeleteFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 50:
        raise DeleteFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise DeleteFactorLoopError("unknown obligation disposition")
    return rows
