"""Render complete PostgreSQL 18.4 RELEASE SAVEPOINT factor-loop programs.
...
RELEASE SAVEPOINT is a TCL transaction-control statement: the target is
the ``RELEASE [SAVEPOINT] savepoint_name`` statement itself, not a
...
RELEASE SAVEPOINT *releases a savepoint* within an active transaction, so
the setup typically ``BEGIN``s a transaction and creates a ``SAVEPOINT``
first (the normal success case is ``inside_transaction`` +
``savepoint_exists``).  The ``transaction_mode`` factor (isolation level /
read-only / etc.) is reflected in the setup ``BEGIN`` statement; RELEASE
SAVEPOINT itself only accepts ``SAVEPOINT`` / ``savepoint_name``.
"""
...
from .release_savepoint_factor_extension import (
    ReleaseSavepointFactorExtensionCase,
    _present_failure_pair,
)
from .release_savepoint_factor_loop import (
...
class ReleaseSavepointFactorRenderError(ValueError):
    """Raised when a RELEASE SAVEPOINT case cannot be rendered."""


@dataclass(frozen=True)
class ReleaseSavepointFactorWitness:
...
def _synthetic_case(
    ext: ReleaseSavepointFactorExtensionCase,
) -> ReleaseSavepointFactorCase:
    ...
    if pair is not None:
        ...
        factor_value = assignment.get(
            "target_action", "release_savepoint"
        )
    return ReleaseSavepointFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    ...
    if isinstance(case, ReleaseSavepointFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: ReleaseSavepointFactorCase) -> dict[str, str]:
...
def _release_target(name: str) -> str:
    """Build a RELEASE SAVEPOINT target statement from a name."""
...
def _begin_with_mode(a: dict[str, str]) -> str:
    ...
    RELEASE SAVEPOINT itself only accepts SAVEPOINT / savepoint_name, so
...
def _probe_select(
    case: ReleaseSavepointFactorCase,
    a: dict[str, str],
    p: str,
) -> str | None:
    ...
    if mode == "returned_rows":
        return "SELECT 1 AS release_savepoint_completed;"
    return None
...
def _tables_to_drop(
    ...
    RELEASE SAVEPOINT is a TCL statement: it never creates a TABLE.  The
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary RELEASE SAVEPOINT statement for the active mode."""
    ...
    if ic == "object_type_mismatch":
        return _release_target(name)

    # Chain behavior (mirror COMMIT: treated as success, provisional).
    if cb == "and_chain":
        return (
            f"RELEASE SAVEPOINT {name} AND CHAIN;"
            if name
            else "RELEASE SAVEPOINT AND CHAIN;"
        )
    if cb == "and_no_chain":
        return (
            f"RELEASE SAVEPOINT {name} AND NO CHAIN;"
            if name
            else "RELEASE SAVEPOINT AND NO CHAIN;"
        )

    # Expected status failure (meta -- when no other failure is set).
    # The failure is via transaction state (outside_transaction), so the
    # target is a plain RELEASE SAVEPOINT that warns 25P01.
    if es == "failure":
        return _release_target(name)

    # Default: plain RELEASE SAVEPOINT with the name.
    return _release_target(name)
...
def _resolve_case(
    ...
    # RELEASE SAVEPOINT produces the no-active-transaction warning (25P01).
    if (
    ...
    setup: list[str] = []
    locus = "target.release_savepoint"
        ...
        if ts == "savepoint_exists":
            ...
            if sp is not None:
                ...
                if sns == "released_name":
                    ...
                    # ``^RELEASE\s+SAVEPOINT`` does not match the
...
def resolve_release_savepoint_factor_witness(
    case: (
        ReleaseSavepointFactorCase
        | ReleaseSavepointFactorExtensionCase
    ),
    repository_root: Path,
) -> ReleaseSavepointFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return ReleaseSavepointFactorWitness(
...
def count_primary_release_savepoint(sql: str) -> int:
    ...
    return len(re.findall(r"(?im)^\s*RELEASE\s+SAVEPOINT\b", region))


def _header(case: ReleaseSavepointFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : RELEASE SAVEPOINT {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_release_savepoint_factor_case(
    case: (
        ReleaseSavepointFactorCase
        | ReleaseSavepointFactorExtensionCase
    ),
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 RELEASE SAVEPOINT。")
...
def _write_program(
    ...
    text = render_release_savepoint_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_release_savepoint_factor_programs(
    baseline_plan: ReleaseSavepointFactorLoopPlan,
...
__all__ = [
    "ReleaseSavepointFactorRenderError",
    "ReleaseSavepointFactorWitness",
    "count_primary_release_savepoint",
    "generate_release_savepoint_factor_programs",
    "render_release_savepoint_factor_case",
    "resolve_release_savepoint_factor_witness",
]
