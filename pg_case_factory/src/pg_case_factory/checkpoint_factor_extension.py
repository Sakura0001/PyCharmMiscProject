"""Bounded post-coverage cross-factor extension expander for CHECKPOINT.

The marginal factor-value-loop (:mod:`checkpoint_factor_loop`) is the
...
phase allowed by ``checkpoint.yaml``
...
CHECKPOINT requires superuser privilege, so
``privilege_context=insufficient_privilege`` is an unconditional failure
(SQLSTATE 42501).  No other factor value causes a failure: CHECKPOINT's
only reachable error surface is privilege denial.
...
class CheckpointFactorExtensionError(ValueError):
    """Raised when a frozen CHECKPOINT extension input drifts."""


@dataclass(frozen=True)
class CheckpointFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CheckpointFactorExtensionPlan:
    cases: tuple[CheckpointFactorExtensionCase, ...]
...
# surface for CHECKPOINT is privilege denial.
...
def _derive_factors(a: dict[str, str]) -> None:
    """Derive expected_status from privilege_context (and vice versa)."""
...
def _behavior_combinations() -> list[dict[str, str]]:
    ...
    for values in itertools.product(
        *[list(_GENERAL_AXES[n]) for n in names]
        ...
        assignment["target_action"] = "checkpoint"
        ...
        if len(assignment) != len(set(assignment)):
            raise CheckpointFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"checkpoint-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "compatibility": {
                    "resolver": "checkpoint_factor_extension",
...
def build_checkpoint_factor_extension_plan(
    repository_root: Path,
) -> CheckpointFactorExtensionPlan:
    """Build the bounded CHECKPOINT post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_checkpoint_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[CheckpointFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    CheckpointFactorExtensionCase(
                        ordinal=ordinal,
                        case_id=(
                            f"CHECKPOINT{ordinal:05d}"
                        ),
                        sql_filename=(
                            f"CHECKPOINT{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"checkpoint_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"CP-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "checkpoint_required_factor_"
                            "value_matrix"
                        ),
                        derivation_reason=(
    ...
    return CheckpointFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "CheckpointFactorExtensionError",
    "CheckpointFactorExtensionCase",
    "CheckpointFactorExtensionPlan",
