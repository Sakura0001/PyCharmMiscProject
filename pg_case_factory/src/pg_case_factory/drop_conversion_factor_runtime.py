"""Bounded PostgreSQL 18.4 runtime for DROP CONVERSION factor programs."""
...
from .drop_conversion_factor_loop import DropConversionFactorCase
...
class DropConversionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropConversionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropConversionSuiteRun:
    run_ordinal: int
    cases: tuple[DropConversionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropConversionRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_conversion_pg18_suite_run",
...
@dataclass(frozen=True)
class DropConversionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_conversion_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_conversion_runs(
    run_01: DropConversionSuiteRun,
    run_02: DropConversionSuiteRun,
) -> DropConversionTwoRunComparison:
    ...
    return DropConversionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropConversionRuntimeCase(Protocol):
...
def build_drop_conversion_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropConversionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_conversion_factor_loop_plan(root).cases
    extensions = build_drop_conversion_factor_extension_plan(root).cases
    cases: list[DropConversionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropConversionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropConversionPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropConversionRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropConversionRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_conversion WHERE conname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropConversionRuntimeCase,
        sql_path: Path,
    ) -> DropConversionCaseRuntimeResult:
        ...
        return DropConversionCaseRuntimeResult(
...
__all__ = [
    "DropConversionCaseRuntimeResult",
    "DropConversionPg18Runner",
    "DropConversionRuntimeCase",
    "DropConversionRuntimeError",
    "DropConversionSuiteRun",
    "DropConversionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_conversion_runtime_case_set",
    "compare_drop_conversion_runs",
]
