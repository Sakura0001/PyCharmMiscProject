"""Bounded PostgreSQL 18.4 runtime for CREATE EXTENSION factor programs.

This module provides the runtime case-set combiner and psql executor for
CREATE EXTENSION factor-loop SQL.  In the no-DB phase the construction
and comparison logic is present and tested, but the executor is not run
against a live PostgreSQL server.  SQLSTATE expectations are provisional
(a later DB phase verifies on PG 18.4).
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ce-sock-20260820")
PG18_PORT = 55542
PG18_DATABASE = "pgcf_ce"
...
class CreateExtensionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateExtensionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateExtensionSuiteRun:
    run_ordinal: int
    cases: tuple[CreateExtensionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateExtensionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_extension_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateExtensionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_extension_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_extension_runs(
    run_01: CreateExtensionSuiteRun,
    run_02: CreateExtensionSuiteRun,
) -> CreateExtensionTwoRunComparison:
    ...
    return CreateExtensionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateExtensionRuntimeCase(Protocol):
...
def build_create_extension_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateExtensionRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_create_extension_factor_loop_plan(root).cases
    extensions = (
        build_create_extension_factor_extension_plan(root).cases
    )
    cases: list[CreateExtensionRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateExtensionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateExtensionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_extension WHERE extname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateExtensionRuntimeCase,
        sql_path: Path,
    ) -> CreateExtensionCaseRuntimeResult:
        ...
        return CreateExtensionCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return CreateExtensionSuiteRun(
...
__all__ = [
    "CreateExtensionCaseRuntimeResult",
    "CreateExtensionPg18Runner",
    "CreateExtensionRuntimeCase",
    "CreateExtensionRuntimeError",
    "CreateExtensionSuiteRun",
    "CreateExtensionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_extension_runtime_case_set",
    "compare_create_extension_runs",
]
