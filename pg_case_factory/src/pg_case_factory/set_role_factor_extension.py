"""Bounded post-coverage extension plan for SET ROLE factor regress.

The baseline :mod:`set_role_factor_loop` assigns one local SQL program to
every ``SFV``/``GRM`` obligation (marginal 1:1, 43 local cases).  This
...
SET ROLE is a session-scoped statement that creates no tables, so the
bookend (``DROP TABLE IF EXISTS``) is never emitted.  The session runtime
state (current role) is the semantic witness target.  ``SET ROLE`` inside a
transaction block is the only reachable failure surface (SQLSTATE
``25001``).
...
class SetRoleFactorExtensionError(ValueError):
    """Raised when SET ROLE extension input drifts."""


@dataclass(frozen=True)
class SetRoleFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SetRoleFactorExtensionPlan:
    cases: tuple[SetRoleFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "set_role_required_factor_value_matrix"

_CONSUMER_ACTION = "set_role"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"set-role-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_set_role_factor_extension_plan(
    repository_root: Path,
) -> SetRoleFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_set_role_factor_loop_plan(root)
    ...
    cases: list[SetRoleFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise SetRoleFactorExtensionError(
                    ...
                    cases.append(
                        SetRoleFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"SETROLE{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"setrole_{ordinal:05d}_"
                            ),
    ...
    plan = SetRoleFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if ordinals != expected_ordinals:
        raise SetRoleFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise SetRoleFactorExtensionError("unknown extension outcome")
    if not all(
