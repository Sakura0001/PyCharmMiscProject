"""Actual-byte coverage validation for CREATE SCHEMA factor-loop SQL.
...
class CreateSchemaFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_schema_factor_program_validation",
            "passed": self.passed,
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_schema_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_schema_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_schema(sql) != 1:
...
def validate_create_schema_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateSchemaFactorProgramValidation(
        passed=not issues,
