"""Bounded PostgreSQL 18.4 runtime for CREATE TEXT SEARCH CONFIGURATION factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ctscfg-sock-20260820")
PG18_PORT = 55508
PG18_DATABASE = "pgcf_ctscfg"
...
class CreateTextSearchConfigurationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTextSearchConfigurationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTextSearchConfigurationSuiteRun:
    run_ordinal: int
    cases: tuple[
    ...
    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTextSearchConfigurationRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_text_search_configuration_pg18_suite_run"
            ),
...
@dataclass(frozen=True)
class CreateTextSearchConfigurationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
...
def compare_create_text_search_configuration_runs(
    run_01: CreateTextSearchConfigurationSuiteRun,
    run_02: CreateTextSearchConfigurationSuiteRun,
    ...
    return CreateTextSearchConfigurationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTextSearchConfigurationRuntimeCase(Protocol):
...
def build_create_text_search_configuration_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateTextSearchConfigurationRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_text_search_configuration_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_text_search_configuration_factor_extension_plan(
            root
    ...
    cases: list[CreateTextSearchConfigurationRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: CreateTextSearchConfigurationRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTextSearchConfigurationPg18Runner:
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTextSearchConfigurationRuntimeCase],
        ...
        *,
        ...
        results: list[CreateTextSearchConfigurationCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateTextSearchConfigurationSuiteRun(
...
__all__ = [
    ...
    "CreateTextSearchConfigurationRuntimeCase",
