"""Actual-byte coverage validation for CREATE VIEW factor-loop SQL.
...
from .create_view_factor_render import (
...
class CreateViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_view_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_view_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_view_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_view(sql) != 1
    ):
...
def validate_create_view_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateViewFactorProgramValidation:
    ...
    return CreateViewFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_create_view_factor_programs",
]
