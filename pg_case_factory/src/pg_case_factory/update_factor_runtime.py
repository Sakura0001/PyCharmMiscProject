"""Bounded PostgreSQL 18.4 runtime for UPDATE factor programs.
...
from .update_factor_loop import UpdateFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-update-sock-20260820")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_update"
...
@dataclass(frozen=True)
class UpdateSuiteRun:
    run_ordinal: int
    cases: tuple[UpdateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise UpdateRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "update_pg18_suite_run",
...
class UpdateTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "update_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_update_runs(
    run_01: UpdateSuiteRun,
    run_02: UpdateSuiteRun,
) -> UpdateTwoRunComparison:
...
class UpdateRuntimeCase(Protocol):
...
def build_update_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[UpdateRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .update_factor_extension import (
        build_update_factor_extension_plan,
    )
    ...
    baseline = build_update_factor_loop_plan(root).cases
    extensions = build_update_factor_extension_plan(root).cases
    cases: list[UpdateRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: UpdateRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class UpdatePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise UpdateRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: UpdateRuntimeCase,
        sql_path: Path,
    ) -> UpdateCaseRuntimeResult:
        ...
        return UpdateCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[UpdateRuntimeCase],
        ...
        *,
        ...
        results: list[UpdateCaseRuntimeResult] = []
        for case in cases:
