"""Bounded PostgreSQL 18.4 runtime for CREATE USER factor programs.
...
frozen runtime port constant is 55608 (kept distinct from every other
...
from .create_user_factor_loop import CreateUserFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cuser-sock-20260820")
PG18_PORT = 55608
PG18_DATABASE = "pgcf_cuser"
...
class CreateUserRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateUserCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateUserSuiteRun:
    run_ordinal: int
    cases: tuple[CreateUserCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateUserRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_user_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateUserTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_user_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_user_runs(
    run_01: CreateUserSuiteRun,
    run_02: CreateUserSuiteRun,
) -> CreateUserTwoRunComparison:
    ...
    return CreateUserTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateUserRuntimeCase(Protocol):
...
def build_create_user_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateUserRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_user_factor_loop_plan(root).cases
    extensions = build_create_user_factor_extension_plan(root).cases
    cases: list[CreateUserRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateUserRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateUserPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateUserRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateUserRuntimeCase,
        sql_path: Path,
    ) -> CreateUserCaseRuntimeResult:
        ...
        return CreateUserCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateUserRuntimeCase],
        ...
        *,
        ...
        results: list[CreateUserCaseRuntimeResult] = []
        for case in cases:
