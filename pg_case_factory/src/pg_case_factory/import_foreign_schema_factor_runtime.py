"""Bounded PostgreSQL 18.4 runtime for IMPORT FOREIGN SCHEMA factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-importfs-sock-20260820")
PG18_PORT = 55618
PG18_DATABASE = "pgcf_importfs"
...
class ImportForeignSchemaRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class ImportForeignSchemaCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class ImportForeignSchemaSuiteRun:
    run_ordinal: int
    cases: tuple[ImportForeignSchemaCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ImportForeignSchemaRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "import_foreign_schema_pg18_suite_run",
...
@dataclass(frozen=True)
class ImportForeignSchemaTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "import_foreign_schema_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_import_foreign_schema_runs(
    run_01: ImportForeignSchemaSuiteRun,
    run_02: ImportForeignSchemaSuiteRun,
) -> ImportForeignSchemaTwoRunComparison:
    ...
    return ImportForeignSchemaTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ImportForeignSchemaRuntimeCase(Protocol):
...
def build_import_foreign_schema_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ImportForeignSchemaRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_import_foreign_schema_factor_loop_plan(root).cases
    extensions = (
        build_import_foreign_schema_factor_extension_plan(root).cases
    )
    cases: list[ImportForeignSchemaRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: ImportForeignSchemaRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ImportForeignSchemaPg18Runner:
    ...
    def _execute_case(
        self,
        case: ImportForeignSchemaRuntimeCase,
        sql_path: Path,
    ) -> ImportForeignSchemaCaseRuntimeResult:
        ...
        return ImportForeignSchemaCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ImportForeignSchemaRuntimeCase],
        ...
        *,
        ...
        results: list[ImportForeignSchemaCaseRuntimeResult] = []
        for case in cases:
        ...
        return ImportForeignSchemaSuiteRun(
...
__all__ = [
    ...
    "ImportForeignSchemaRuntimeCase",
