"""Actual-byte coverage validation for PREPARE TRANSACTION factor-loop SQL.
...
from .prepare_transaction_factor_render import (
...
class PrepareTransactionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "prepare_transaction_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_prepare_transaction_factor_witness(
        case, repository_root
    )
    expected_sql = render_prepare_transaction_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_prepare_transaction(sql) != 1:
...
def validate_prepare_transaction_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return PrepareTransactionFactorProgramValidation(
        passed=not issues,
