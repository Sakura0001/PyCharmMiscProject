"""Bounded PostgreSQL 18.4 runtime for COMMIT PREPARED factor programs.
...
:mod:`commit_prepared_factor_validate`) is the gate that actually runs.
...
from .commit_prepared_factor_loop import CommitPreparedFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cprep-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_cprep"
...
class CommitPreparedRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CommitPreparedCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CommitPreparedSuiteRun:
    run_ordinal: int
    cases: tuple[CommitPreparedCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CommitPreparedRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "commit_prepared_pg18_suite_run",
...
@dataclass(frozen=True)
class CommitPreparedTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "commit_prepared_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_commit_prepared_runs(
    run_01: CommitPreparedSuiteRun,
    run_02: CommitPreparedSuiteRun,
    ...
    return CommitPreparedTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CommitPreparedRuntimeCase(Protocol):
...
def build_commit_prepared_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CommitPreparedRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_commit_prepared_factor_loop_plan(root).cases
    extensions = build_commit_prepared_factor_extension_plan(root).cases
    cases: list[CommitPreparedRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CommitPreparedRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CommitPreparedPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CommitPreparedRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CommitPreparedRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CommitPreparedRuntimeCase],
        ...
        *,
        ...
        results: list[CommitPreparedCaseRuntimeResult] = []
        for case in cases:
