"""Bounded PostgreSQL 18.4 runtime for DROP TRANSFORM factor programs."""
...
from .drop_transform_factor_loop import DropTransformFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dt-sock-20260820")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dt"
...
class DropTransformRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTransformCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTransformSuiteRun:
    run_ordinal: int
    cases: tuple[DropTransformCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTransformRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_transform_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTransformTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_transform_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_transform_runs(
    run_01: DropTransformSuiteRun,
    run_02: DropTransformSuiteRun,
) -> DropTransformTwoRunComparison:
    ...
    return DropTransformTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTransformRuntimeCase(Protocol):
...
def build_drop_transform_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTransformRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_transform_factor_loop_plan(root).cases
    extensions = build_drop_transform_factor_extension_plan(root).cases
    cases: list[DropTransformRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTransformRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTransformPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropTransformRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTransformRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_namespace WHERE nspname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTransformRuntimeCase,
        sql_path: Path,
    ) -> DropTransformCaseRuntimeResult:
        ...
        return DropTransformCaseRuntimeResult(
...
__all__ = [
    "DropTransformCaseRuntimeResult",
    "DropTransformPg18Runner",
    "DropTransformRuntimeCase",
    "DropTransformRuntimeError",
    "DropTransformSuiteRun",
    "DropTransformTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_transform_runtime_case_set",
    "compare_drop_transform_runs",
]
