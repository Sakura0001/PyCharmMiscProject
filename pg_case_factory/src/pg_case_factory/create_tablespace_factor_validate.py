"""Actual-byte coverage validation for CREATE TABLESPACE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 67-case baseline +
...
from .create_tablespace_factor_render import (
...
class CreateTablespaceFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_tablespace_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_tablespace_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_tablespace(sql) != 1:
...
def validate_create_tablespace_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CreateTablespaceFactorProgramValidation(
        passed=not issues,
