"""Bounded PostgreSQL 18.4 runtime for ALTER TEXT SEARCH DICTIONARY factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-atsd-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_atsd"
...
class AlterTextSearchDictionaryRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTextSearchDictionaryCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTextSearchDictionarySuiteRun:
    run_ordinal: int
    cases: tuple[
        AlterTextSearchDictionaryCaseRuntimeResult, ...
    ]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTextSearchDictionaryRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_text_search_dictionary_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTextSearchDictionaryTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_dictionary_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_alter_text_search_dictionary_runs(
    run_01: AlterTextSearchDictionarySuiteRun,
    run_02: AlterTextSearchDictionarySuiteRun,
) -> AlterTextSearchDictionaryTwoRunComparison:
    ...
    return AlterTextSearchDictionaryTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTextSearchDictionaryRuntimeCase(Protocol):
...
def build_alter_text_search_dictionary_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[AlterTextSearchDictionaryRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_alter_text_search_dictionary_factor_loop_plan(root).cases
    )
    extensions = (
        build_alter_text_search_dictionary_factor_extension_plan(
            root
    ...
    cases: list[AlterTextSearchDictionaryRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: AlterTextSearchDictionaryRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTextSearchDictionaryPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_ts_config WHERE cfgname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        ...
        return AlterTextSearchDictionaryCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[AlterTextSearchDictionaryRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> AlterTextSearchDictionarySuiteRun:
        self.verify_server()
        selected = (
        ...
        return AlterTextSearchDictionarySuiteRun(
...
__all__ = [
    "AlterTextSearchDictionaryCaseRuntimeResult",
    "AlterTextSearchDictionaryPg18Runner",
    "AlterTextSearchDictionaryRuntimeCase",
    "AlterTextSearchDictionaryRuntimeError",
    "AlterTextSearchDictionarySuiteRun",
    "AlterTextSearchDictionaryTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_text_search_dictionary_runtime_case_set",
    "compare_alter_text_search_dictionary_runs",
]
