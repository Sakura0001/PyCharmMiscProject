"""Actual-byte coverage validation for CREATE GROUP factor-loop SQL.
...
class CreateGroupFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_group_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_group_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_group_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_group(sql) != 1:
...
def validate_create_group_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateGroupFactorProgramValidation:
    ...
    return CreateGroupFactorProgramValidation(
        passed=not issues,
