"""Bounded PostgreSQL 18.4 runtime for DROP STATISTICS factor programs."""
...
from .drop_statistics_factor_loop import DropStatisticsFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ds-sock-20260821")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_ds"
...
class DropStatisticsRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropStatisticsCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropStatisticsSuiteRun:
    run_ordinal: int
    cases: tuple[DropStatisticsCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropStatisticsRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_statistics_pg18_suite_run",
...
@dataclass(frozen=True)
class DropStatisticsTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_statistics_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_statistics_runs(
    run_01: DropStatisticsSuiteRun,
    run_02: DropStatisticsSuiteRun,
) -> DropStatisticsTwoRunComparison:
    ...
    return DropStatisticsTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropStatisticsRuntimeCase(Protocol):
...
def build_drop_statistics_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropStatisticsRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_statistics_factor_loop_plan(root).cases
    extensions = build_drop_statistics_factor_extension_plan(root).cases
    cases: list[DropStatisticsRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropStatisticsRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropStatisticsPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropStatisticsRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropStatisticsRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_statistic_ext WHERE stxname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropStatisticsRuntimeCase,
        sql_path: Path,
    ) -> DropStatisticsCaseRuntimeResult:
        ...
        return DropStatisticsCaseRuntimeResult(
...
__all__ = [
    "DropStatisticsCaseRuntimeResult",
    "DropStatisticsPg18Runner",
    "DropStatisticsRuntimeCase",
    "DropStatisticsRuntimeError",
    "DropStatisticsSuiteRun",
    "DropStatisticsTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_statistics_runtime_case_set",
    "compare_drop_statistics_runs",
]
