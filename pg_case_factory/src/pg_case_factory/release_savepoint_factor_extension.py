"""Bounded post-coverage cross-factor extension expander for RELEASE SAVEPOINT.

The marginal factor-value-loop (:mod:`release_savepoint_factor_loop`) is the
...
by ``release_savepoint.yaml`` ``post_coverage_extension_policy.enabled:
true``: cross-factor combinations of the positive T1-T2 behaviour axes across
the single RELEASE SAVEPOINT synopsis branch, with at most one
...
RELEASE SAVEPOINT is a TCL transaction-control statement with one synopsis
...
class ReleaseSavepointFactorExtensionError(ValueError):
    """Raised when a frozen RELEASE SAVEPOINT extension input drifts."""


@dataclass(frozen=True)
class ReleaseSavepointFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class ReleaseSavepointFactorExtensionPlan:
    cases: tuple[ReleaseSavepointFactorExtensionCase, ...]
...
# General axes crossed for the single RELEASE SAVEPOINT branch.
...
# target_action are overridden per branch.  RELEASE SAVEPOINT's success
# default for ``transaction_state`` is ``inside_transaction`` (an active
# transaction with a savepoint to release).
...
# attribution.
#
# RELEASE SAVEPOINT: the failure-causing transaction_state values
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    RELEASE SAVEPOINT mirrors COMMIT's derivation.
    """
    ...
    for values in itertools.product(
        *[_GENERAL_AXES[n] for n in names]
        ...
        if len(assignment) != len(set(assignment)):
            raise ReleaseSavepointFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"release-savepoint-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"RELEASE SAVEPOINT extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": (
                        "release_savepoint_factor_extension"
                    ),
...
def build_release_savepoint_factor_extension_plan(
    repository_root: Path,
) -> ReleaseSavepointFactorExtensionPlan:
    """Build the bounded RELEASE SAVEPOINT post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_release_savepoint_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[ReleaseSavepointFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    ReleaseSavepointFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"RELEASESAVEPOINT{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"releasesavepoint_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"RELEASESAVEPOINT-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "release_savepoint_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return ReleaseSavepointFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "ReleaseSavepointFactorExtensionError",
    "ReleaseSavepointFactorExtensionCase",
    "ReleaseSavepointFactorExtensionPlan",
    "build_release_savepoint_factor_extension_plan",
]
