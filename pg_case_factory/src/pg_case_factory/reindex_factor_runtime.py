"""Bounded PostgreSQL 18.4 runtime for REINDEX factor programs."""
...
from .reindex_factor_loop import ReindexFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-reindex-sock-20260821")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_reindex"
PG18_SUPERUSER = "pgcf_reindex_superuser"
...
@dataclass(frozen=True)
class ReindexSuiteRun:
    run_ordinal: int
    cases: tuple[ReindexCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ReindexRuntimeError(
...
class ReindexTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reindex_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_reindex_runs(
    run_01: ReindexSuiteRun,
    run_02: ReindexSuiteRun,
    ...
    return ReindexTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ReindexRuntimeCase(Protocol):
    case_id: str
    sql_filename: str
    object_prefix: str
    expected_sqlstate: str
...
def build_reindex_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[ReindexRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_reindex_factor_loop_plan(root).cases
    extensions = build_reindex_factor_extension_plan(root).cases
    cases: list[ReindexRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: ReindexRuntimeCase) -> int:
    return getattr(case, "ordinal")


class ReindexPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ReindexRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return ReindexCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ReindexRuntimeCase],
        ...
        *,
        ...
        results: list[ReindexCaseRuntimeResult] = []
        for case in cases:
