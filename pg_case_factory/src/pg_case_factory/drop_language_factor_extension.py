"""Bounded post-coverage cross-factor extension expander for DROP LANGUAGE.

The marginal factor-value-loop (:mod:`drop_language_factor_loop`) is the
required baseline: one program per factor value, 35 local cases
(GRM 1 + SFV 32 + Risk 2).  This module adds the bounded post-coverage
...
The negative factors (``privilege_context=non_owner``,
``target_object_state=missing`` when ``if_exists_clause=absent``, and the
dependency boundary under a non-CASCADE drop policy) are crossed here with
...
* ``branch_1`` — ``privilege_context`` x ``target_object_state`` x
  ``if_exists_clause`` x ``cascade_clause`` x ``dependency_state``.
"""
...
class DropLanguageFactorExtensionError(ValueError):
    """Raised when a frozen DROP LANGUAGE extension input drifts."""


@dataclass(frozen=True)
class DropLanguageFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropLanguageFactorExtensionPlan:
    cases: tuple[DropLanguageFactorExtensionCase, ...]
...
# (privilege_context=non_owner, target_object_state=missing under absent
# IF EXISTS, dependency_state=has_dependents under RESTRICT) are crossed here
...
# boundary (42704) fires when the language is absent and IF EXISTS is omitted.
# The dependency boundary (2BP01) fires when dependents exist under a
...
def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    """missing fires only when IF EXISTS is omitted."""
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    """dependents fire only under a non-CASCADE drop policy."""
...
def _present_failure_pair(
    ...
    language is absent and IF EXISTS is omitted.  The dependency boundary
    (2BP01) fires last under RESTRICT with dependents; it is attributed to
    ``dependency_state`` when that axis says has_dependents, else to
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-language-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP LANGUAGE extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_language_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP LANGUAGE",
...
def build_drop_language_factor_extension_plan(
    repository_root: Path,
) -> DropLanguageFactorExtensionPlan:
    """Build the bounded DROP LANGUAGE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_language_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropLanguageFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropLanguageFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPLANGUAGE-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_language_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropLanguageFactorExtensionPlan(
        cases=cases_tuple,
