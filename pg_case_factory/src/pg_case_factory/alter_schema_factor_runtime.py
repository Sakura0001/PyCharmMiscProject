"""Bounded PostgreSQL 18.4 runtime for ALTER SCHEMA factor programs."""
...
from .alter_schema_factor_loop import AlterSchemaFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-aschema-sock-20260820")
PG18_PORT = 55497
...
class AlterSchemaRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterSchemaCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterSchemaSuiteRun:
    run_ordinal: int
    cases: tuple[AlterSchemaCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterSchemaRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_schema_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterSchemaTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_schema_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_schema_runs(
    run_01: AlterSchemaSuiteRun,
    run_02: AlterSchemaSuiteRun,
    ...
    return AlterSchemaTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterSchemaRuntimeCase(Protocol):
...
def build_alter_schema_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterSchemaRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_schema_factor_loop_plan(root).cases
    extensions = build_alter_schema_factor_extension_plan(root).cases
    cases: list[AlterSchemaRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterSchemaRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterSchemaPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterSchemaRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterSchemaRuntimeCase],
        ...
        *,
        ...
        results: list[AlterSchemaCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "AlterSchemaRuntimeCase",
    "AlterSchemaRuntimeError",
    "AlterSchemaSuiteRun",
    "AlterSchemaTwoRunComparison",
    "MAX_PARALLELISM",
