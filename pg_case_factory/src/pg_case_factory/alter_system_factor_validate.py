"""Actual-byte coverage validation for ALTER SYSTEM factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 55-case baseline +
...
derivation record.  Also gates that every required factor value is still
witnessed across ``baseline ∪ extension``.
"""
...
class AlterSystemFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_system_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_system_factor_witness(case, repository_root)
    expected_sql = render_alter_system_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_system(sql) != 1:
...
def validate_alter_system_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterSystemFactorProgramValidation:
    ...
    return AlterSystemFactorProgramValidation(
        passed=not issues,
