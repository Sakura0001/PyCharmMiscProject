"""Actual-byte coverage validation for DELETE factor-loop SQL.
...
from .delete_factor_render import (
...
class DeleteFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "delete_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_delete_factor_witness(
        case, repository_root
    )
    expected_sql = render_delete_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_delete(sql) != 1:
...
def validate_delete_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DeleteFactorProgramValidation(
        passed=not issues,
