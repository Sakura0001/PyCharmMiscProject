"""Bounded PostgreSQL 18.4 runtime for PREPARE TRANSACTION factor programs.

Present and tested for construction/comparison logic only.  The runner is
NOT executed against a live PostgreSQL server in the no-DB ledger phase;
the byte-level witness validator (see
:mod:`validate_prepare_transaction`) is the gate that actually runs.
The two-run comparison harness here is the contract a later DB phase will
honour when verifying the provisional SQLSTATEs on a real PG 18.4 server.
"""
...
from .prepare_transaction_factor_loop import PrepareTransactionFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-preptran-sock-20260821")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_preptran"
...
class PrepareTransactionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class PrepareTransactionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class PrepareTransactionSuiteRun:
    run_ordinal: int
    cases: tuple[PrepareTransactionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise PrepareTransactionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "prepare_transaction_pg18_suite_run",
...
@dataclass(frozen=True)
class PrepareTransactionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "prepare_transaction_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_prepare_transaction_runs(
    run_01: PrepareTransactionSuiteRun,
    run_02: PrepareTransactionSuiteRun,
) -> PrepareTransactionTwoRunComparison:
    ...
    return PrepareTransactionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class PrepareTransactionRuntimeCase(Protocol):
...
def build_prepare_transaction_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[PrepareTransactionRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .prepare_transaction_factor_extension import (
        build_prepare_transaction_factor_extension_plan,
    )
    ...
    baseline = build_prepare_transaction_factor_loop_plan(root).cases
    extensions = build_prepare_transaction_factor_extension_plan(root).cases
    cases: list[PrepareTransactionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: PrepareTransactionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class PrepareTransactionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise PrepareTransactionRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_collation WHERE collname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_data_wrapper WHERE fdwname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_server WHERE srvname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: PrepareTransactionRuntimeCase,
        sql_path: Path,
    ) -> PrepareTransactionCaseRuntimeResult:
        ...
        return PrepareTransactionCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return PrepareTransactionSuiteRun(
...
__all__ = [
    "PrepareTransactionCaseRuntimeResult",
    "PrepareTransactionPg18Runner",
    "PrepareTransactionRuntimeCase",
    "PrepareTransactionRuntimeError",
    "PrepareTransactionSuiteRun",
    "PrepareTransactionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_prepare_transaction_runtime_case_set",
    "compare_prepare_transaction_runs",
]
