"""Bounded PostgreSQL 18.4 runtime for CREATE EVENT TRIGGER factor programs.

Construction/comparison logic only; NOT executed live in the no-DB
phase.  The runtime is present + tested for the combiner and comparison
contract; a later DB phase verifies provisional SQLSTATEs on PG 18.4.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-evttrig-sock-20260820")
PG18_PORT = 55541
PG18_DATABASE = "pgcf_evttrig"
...
class CreateEventTriggerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateEventTriggerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateEventTriggerSuiteRun:
    run_ordinal: int
    cases: tuple[CreateEventTriggerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateEventTriggerRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_event_trigger_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateEventTriggerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_event_trigger_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_event_trigger_runs(
    run_01: CreateEventTriggerSuiteRun,
    run_02: CreateEventTriggerSuiteRun,
) -> CreateEventTriggerTwoRunComparison:
    """Require identical transcripts, structure, SQLSTATEs, clean state."""
    ...
    return CreateEventTriggerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateEventTriggerRuntimeCase(Protocol):
...
def build_create_event_trigger_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateEventTriggerRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_create_event_trigger_factor_loop_plan(root).cases
    extensions = (
        build_create_event_trigger_factor_extension_plan(root).cases
    )
    cases: list[CreateEventTriggerRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateEventTriggerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateEventTriggerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_event_trigger WHERE evtname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateEventTriggerRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateEventTriggerRuntimeCase],
        ...
        *,
        ...
        results: list[CreateEventTriggerCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateEventTriggerSuiteRun(
