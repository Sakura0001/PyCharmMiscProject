"""Bounded PostgreSQL 18.4 runtime for ALTER RULE factor programs."""
...
from .alter_rule_factor_loop import AlterRuleFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-arule-sock-20260820")
PG18_PORT = 55496
PG18_DATABASE = "pgcf_arule"
...
class AlterRuleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterRuleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterRuleSuiteRun:
    run_ordinal: int
    cases: tuple[AlterRuleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterRuleRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_rule_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterRuleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_rule_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_rule_runs(
    run_01: AlterRuleSuiteRun,
    run_02: AlterRuleSuiteRun,
    ...
    return AlterRuleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterRuleRuntimeCase(Protocol):
...
def build_alter_rule_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterRuleRuntimeCase, ...], dict[str, Path]]:
    ...
    from .alter_rule_factor_loop import build_alter_rule_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_rule_factor_loop_plan(root).cases
    extensions = build_alter_rule_factor_extension_plan(root).cases
    cases: list[AlterRuleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterRuleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterRulePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterRuleRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterRuleRuntimeError("cleanup phase marker is not unique")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterRuleRuntimeCase],
        ...
        *,
        ...
        results: list[AlterRuleCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "AlterRuleCaseRuntimeResult",
    "AlterRulePg18Runner",
    "AlterRuleRuntimeCase",
    "AlterRuleRuntimeError",
    "AlterRuleSuiteRun",
    "AlterRuleTwoRunComparison",
    "MAX_PARALLELISM",
