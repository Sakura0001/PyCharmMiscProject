"""Bounded post-coverage cross-factor extension expander for DROP TRANSFORM.

The marginal factor-value-loop (:mod:`drop_transform_factor_loop`) is the
required baseline: one program per factor value, 48 local cases (GRM 1 +
SFV 45 + Risk 2).  This module adds the bounded post-coverage extension
...
The negative factors (``privilege_requirement=missing_*``,
``type_existence=type_not_exists`` / ``type_name_shape=nonexistent_name``,
``language_existence=language_not_exists`` /
``language_name_shape=nonexistent_name``, ``object_state=absent`` when
...
privilege boundary fires first (42501), then the type-lookup boundary
(42704), then the language-lookup boundary (42704), then the
transform-lookup boundary (42704), then the dependency boundary (2BP01).

The single official synopsis branch is crossed:

* ``branch_drop_transform`` — ``privilege_requirement`` x ``type_existence``
  x ``object_state`` x ``if_exists_clause`` x ``cascade_restrict`` x
  ``dependency_context`` x ``type_name_shape`` x ``language_name_shape`` x
  ``language_existence``.
"""
...
class DropTransformFactorExtensionError(ValueError):
    """Raised when a frozen DROP TRANSFORM extension input drifts."""


@dataclass(frozen=True)
class DropTransformFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropTransformFactorExtensionPlan:
    cases: tuple[DropTransformFactorExtensionCase, ...]
...
# (privilege_requirement=missing_*, type_existence=type_not_exists /
# type_name_shape=nonexistent_name, language_existence=language_not_exists /
# language_name_shape=nonexistent_name, object_state=absent,
# dependency_context=has_dependent_objects) are crossed here with
...
_BRANCH_GRAMMAR: dict[str, str] = {
    "branch_drop_transform": "branch_1",
}
...
# Failure boundaries (in PostgreSQL execution order: privilege -> type
# lookup -> language lookup -> transform lookup -> dependency).  The
# privilege boundary (42501) fires first.  The type-lookup boundary (42704)
# fires when the bound type is absent.  The language-lookup boundary (42704)
# fires when the bound language is absent.  The transform-lookup boundary
# (42704) fires when the transform is absent (object_state=absent) and IF
...
_PRIVILEGE_NEGATIVE_VALUES = frozenset(
    {"missing_type_ownership", "missing_language_ownership"}
)
_TYPE_MISSING_NEGATIVE = ("type_existence", "type_not_exists")
_TYPE_MISSING_NAME = ("type_name_shape", "nonexistent_name")
_LANGUAGE_MISSING_NEGATIVE = (
    "language_existence",
    "language_not_exists",
)
_LANGUAGE_MISSING_NAME = ("language_name_shape", "nonexistent_name")
_TRANSFORM_MISSING_OBJECT = ("object_state", "absent")
...
def _privilege_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("privilege_requirement")
        in _PRIVILEGE_NEGATIVE_VALUES
    )


def _type_missing_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("type_existence") == "type_not_exists"
        or assignment.get("type_name_shape") == "nonexistent_name"
    )


def _language_missing_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("language_existence") == "language_not_exists"
        or assignment.get("language_name_shape") == "nonexistent_name"
    )


def _transform_missing_failure_fires(assignment: dict[str, str]) -> bool:
    """transform-missing fires only when IF EXISTS is omitted."""
...
def _failure_unit_count(assignment: dict[str, str]) -> int:
    ...
    if _privilege_failure_fires(assignment):
        count += 1
    if _type_missing_failure_fires(assignment):
        count += 1
    if _language_missing_failure_fires(assignment):
        count += 1
    if _transform_missing_failure_fires(assignment):
        count += 1
...
def _present_failure_pair(
    ...
    attributed first.  The type-lookup boundary (42704) fires next when the
    bound type is absent.  The language-lookup boundary (42704) fires next
    when the bound language is absent.  The transform-lookup boundary
    (42704) fires next when the transform is absent and IF EXISTS is
    omitted.  The dependency boundary (2BP01) fires last under RESTRICT with
    dependencies.
    """

    if _privilege_failure_fires(assignment):
        pair = _PRIVILEGE_NEGATIVE
        if assignment.get("privilege_requirement") == "missing_language_ownership":
            pair = (
                "privilege_requirement",
                "missing_language_ownership",
            )
        return pair
    if _type_missing_failure_fires(assignment):
        if assignment.get("type_existence") == "type_not_exists":
            return _TYPE_MISSING_NEGATIVE
        return _TYPE_MISSING_NAME
    if _language_missing_failure_fires(assignment):
        if assignment.get("language_existence") == "language_not_exists":
            return _LANGUAGE_MISSING_NEGATIVE
        return _LANGUAGE_MISSING_NAME
    if _transform_missing_failure_fires(assignment):
        return _TRANSFORM_MISSING_OBJECT
    if _dependency_failure_fires(assignment):
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-transform-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP TRANSFORM extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_transform_factor_extension",
...
def build_drop_transform_factor_extension_plan(
    repository_root: Path,
) -> DropTransformFactorExtensionPlan:
    """Build the bounded DROP TRANSFORM post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_transform_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropTransformFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropTransformFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPTRANSFORM-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_transform_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropTransformFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "DropTransformFactorExtensionError",
    "DropTransformFactorExtensionCase",
    "DropTransformFactorExtensionPlan",
