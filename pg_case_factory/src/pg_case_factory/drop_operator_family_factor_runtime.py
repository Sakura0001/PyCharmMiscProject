"""Bounded PostgreSQL 18.4 runtime for DROP OPERATOR FAMILY factor programs."""
...
from .drop_operator_family_factor_loop import DropOperatorFamilyFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dof-sock-20260820")
PG18_PORT = 55506
PG18_DATABASE = "pgcf_dof"
...
class DropOperatorFamilyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropOperatorFamilyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropOperatorFamilySuiteRun:
    run_ordinal: int
    cases: tuple[DropOperatorFamilyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropOperatorFamilyRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_family_pg18_suite_run",
...
@dataclass(frozen=True)
class DropOperatorFamilyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_family_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_operator_family_runs(
    run_01: DropOperatorFamilySuiteRun,
    run_02: DropOperatorFamilySuiteRun,
) -> DropOperatorFamilyTwoRunComparison:
    ...
    return DropOperatorFamilyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropOperatorFamilyRuntimeCase(Protocol):
...
def build_drop_operator_family_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropOperatorFamilyRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_operator_family_factor_loop_plan(root).cases
    extensions = build_drop_operator_family_factor_extension_plan(root).cases
    cases: list[DropOperatorFamilyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropOperatorFamilyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropOperatorFamilyPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropOperatorFamilyRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropOperatorFamilyRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_opfamily WHERE opfname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_opclass WHERE opcname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropOperatorFamilyRuntimeCase,
        sql_path: Path,
    ) -> DropOperatorFamilyCaseRuntimeResult:
        ...
        return DropOperatorFamilyCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropOperatorFamilySuiteRun(
...
__all__ = [
    "DropOperatorFamilyCaseRuntimeResult",
    "DropOperatorFamilyPg18Runner",
    "DropOperatorFamilyRuntimeCase",
    "DropOperatorFamilyRuntimeError",
    "DropOperatorFamilySuiteRun",
    "DropOperatorFamilyTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_operator_family_runtime_case_set",
    "compare_drop_operator_family_runs",
]
