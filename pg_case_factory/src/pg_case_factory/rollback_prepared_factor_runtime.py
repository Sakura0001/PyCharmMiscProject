"""Bounded PostgreSQL 18.4 runtime for ROLLBACK PREPARED factor programs.
...
:mod:`validate_rollback_prepared`) is the gate that actually runs.
...
from .rollback_prepared_factor_loop import RollbackPreparedFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-rprep-sock-20260821")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_rprep"
...
class RollbackPreparedRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class RollbackPreparedCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class RollbackPreparedSuiteRun:
    run_ordinal: int
    cases: tuple[RollbackPreparedCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise RollbackPreparedRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_prepared_pg18_suite_run",
...
@dataclass(frozen=True)
class RollbackPreparedTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_prepared_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_rollback_prepared_runs(
    run_01: RollbackPreparedSuiteRun,
    run_02: RollbackPreparedSuiteRun,
) -> RollbackPreparedTwoRunComparison:
    ...
    return RollbackPreparedTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class RollbackPreparedRuntimeCase(Protocol):
...
def build_rollback_prepared_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[RollbackPreparedRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_rollback_prepared_factor_loop_plan(root).cases
    extensions = build_rollback_prepared_factor_extension_plan(root).cases
    cases: list[RollbackPreparedRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: RollbackPreparedRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class RollbackPreparedPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise RollbackPreparedRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: RollbackPreparedRuntimeCase,
        sql_path: Path,
    ) -> RollbackPreparedCaseRuntimeResult:
        ...
        return RollbackPreparedCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return RollbackPreparedSuiteRun(
...
__all__ = [
    "RollbackPreparedCaseRuntimeResult",
    "RollbackPreparedPg18Runner",
    "RollbackPreparedRuntimeCase",
    "RollbackPreparedRuntimeError",
    "RollbackPreparedSuiteRun",
    "RollbackPreparedTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_rollback_prepared_runtime_case_set",
    "compare_rollback_prepared_runs",
]
