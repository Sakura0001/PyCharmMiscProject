"""Bounded PostgreSQL 18.4 runtime for DROP OWNED factor programs."""
...
from .drop_owned_factor_loop import DropOwnedFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dow-sock-20260820")
PG18_PORT = 55513
PG18_DATABASE = "pgcf_dow"
...
class DropOwnedRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropOwnedCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropOwnedSuiteRun:
    run_ordinal: int
    cases: tuple[DropOwnedCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropOwnedRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_owned_pg18_suite_run",
...
@dataclass(frozen=True)
class DropOwnedTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_owned_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_owned_runs(
    run_01: DropOwnedSuiteRun,
    run_02: DropOwnedSuiteRun,
) -> DropOwnedTwoRunComparison:
    ...
    return DropOwnedTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropOwnedRuntimeCase(Protocol):
...
def build_drop_owned_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropOwnedRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_owned_factor_loop_plan(root).cases
    extensions = build_drop_owned_factor_extension_plan(root).cases
    cases: list[DropOwnedRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropOwnedRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropOwnedPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropOwnedRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropOwnedRuntimeCase,
        sql_path: Path,
    ) -> DropOwnedCaseRuntimeResult:
        ...
        return DropOwnedCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropOwnedRuntimeCase],
        ...
        *,
        ...
        results: list[DropOwnedCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropOwnedCaseRuntimeResult",
    "DropOwnedPg18Runner",
    "DropOwnedRuntimeCase",
    "DropOwnedRuntimeError",
    "DropOwnedSuiteRun",
    "DropOwnedTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_owned_runtime_case_set",
    "compare_drop_owned_runs",
]
