"""Bounded PostgreSQL 18.4 runtime for END factor programs."""
...
from .end_factor_loop import EndFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-end-sock-20260820")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_end"
...
class EndSuiteRun:
    run_ordinal: int
    cases: tuple[EndCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise EndRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "end_pg18_suite_run",
...
class EndTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "end_pg18_two_run_comparison",
            "passed": self.passed,
...
def build_end_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[EndRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_end_factor_loop_plan(root).cases
    extensions = build_end_factor_extension_plan(root).cases
    cases: list[EndRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: EndRuntimeCase) -> int:
...
class EndPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise EndRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[EndCaseRuntimeResult] = []
        for case in cases:
