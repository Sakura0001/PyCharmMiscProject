"""Actual-byte coverage validation for CHECKPOINT factor-loop SQL.
...
class CheckpointFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "checkpoint_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_checkpoint_factor_witness(
        case, repository_root
    )
    expected_sql = render_checkpoint_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_checkpoint(sql) != 1:
...
def validate_checkpoint_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CheckpointFactorProgramValidation(
        passed=not issues,
