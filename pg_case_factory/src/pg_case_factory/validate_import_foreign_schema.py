"""Actual-byte coverage validation for IMPORT FOREIGN SCHEMA factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 57-case baseline +
...
from .import_foreign_schema_factor_render import (
...
class ImportForeignSchemaFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "import_foreign_schema_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_import_foreign_schema_factor_witness(
        case, repository_root
    )
    expected_sql = render_import_foreign_schema_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_import_foreign_schema(sql) != 1
    ):
...
def validate_import_foreign_schema_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> ImportForeignSchemaFactorProgramValidation:
    ...
    return ImportForeignSchemaFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_import_foreign_schema_factor_programs",
]
