"""Bounded PostgreSQL 18.4 runtime for CREATE SCHEMA factor programs.

The runtime is present and tested for construction/comparison logic but is
NOT executed against a live server in the no-DB phase (tickoff is via
static byte-level validation).  A later DB phase will run these programs
against PG18.4 to verify the provisional SQLSTATEs.
"""
...
from .create_schema_factor_loop import CreateSchemaFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-csch-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_csch"
...
class CreateSchemaRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateSchemaCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateSchemaSuiteRun:
    run_ordinal: int
    cases: tuple[CreateSchemaCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateSchemaRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_schema_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateSchemaTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_schema_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_schema_runs(
    run_01: CreateSchemaSuiteRun,
    run_02: CreateSchemaSuiteRun,
    ...
    return CreateSchemaTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateSchemaRuntimeCase(Protocol):
...
def build_create_schema_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateSchemaRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_schema_factor_loop_plan(root).cases
    extensions = build_create_schema_factor_extension_plan(root).cases
    cases: list[CreateSchemaRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateSchemaRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateSchemaPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateSchemaRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_namespace WHERE nspname LIKE '"
            + literal
            + "' "
    ...
    def run_cases(
        self,
        cases: Sequence[CreateSchemaRuntimeCase],
        ...
        *,
        ...
        results: list[CreateSchemaCaseRuntimeResult] = []
        for case in cases:
