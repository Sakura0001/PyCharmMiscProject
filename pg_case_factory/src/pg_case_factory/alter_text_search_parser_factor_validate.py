"""Actual-byte coverage validation for ALTER TEXT SEARCH PARSER factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 35-case baseline +
bounded 108-case extension) and compares them exactly against the
...
class AlterTextSearchParserFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_parser_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_text_search_parser_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_text_search_parser_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_alter_text_search_parser(sql) != 1
    ):
...
def validate_alter_text_search_parser_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterTextSearchParserFactorProgramValidation(
        passed=not issues,
