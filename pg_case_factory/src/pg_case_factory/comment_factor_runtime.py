"""Bounded PostgreSQL 18.4 runtime for COMMENT ON factor programs."""
...
from .comment_factor_loop import CommentFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-comment-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_comment"
...
@dataclass(frozen=True)
class CommentSuiteRun:
    run_ordinal: int
    cases: tuple[CommentCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CommentRuntimeError(
...
class CommentTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "comment_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_comment_runs(
    run_01: CommentSuiteRun,
    run_02: CommentSuiteRun,
) -> CommentTwoRunComparison:
    ...
    return CommentTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CommentRuntimeCase(Protocol):
...
def build_comment_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CommentRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_comment_factor_loop_plan(root).cases
    extensions = build_comment_factor_extension_plan(root).cases
    cases: list[CommentRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CommentRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CommentPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CommentRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CommentRuntimeCase,
        sql_path: Path,
    ) -> CommentCaseRuntimeResult:
        ...
        return CommentCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CommentRuntimeCase],
        ...
        *,
        ...
        results: list[CommentCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_comment_runtime_case_set",
    "compare_comment_runs",
]
