"""Bounded post-coverage cross-factor extension expander for INSERT.

The marginal factor-value-loop (:mod:`insert_factor_loop`) is the
...
allowed by ``insert.yaml`` ``post_coverage_extension_policy.enabled:
...
INSERT mutates a ``pg_class`` base-table relation; the failure-causing
...
class InsertFactorExtensionError(ValueError):
    """Raised when a frozen INSERT extension input drifts."""


@dataclass(frozen=True)
class InsertFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class InsertFactorExtensionPlan:
    cases: tuple[InsertFactorExtensionCase, ...]
...
# General axes crossed for the single branch.  These are the same six
# behaviour axes used for CALL/DELETE (INSERT shares the DML 15-factor
# schema), so the combinatorics are identical.
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise InsertFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"insert-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"INSERT extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "insert_factor_extension",
...
def build_insert_factor_extension_plan(
    repository_root: Path,
) -> InsertFactorExtensionPlan:
    """Build the bounded INSERT post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_insert_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[InsertFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    InsertFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"INSERT-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "insert_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return InsertFactorExtensionPlan(
        cases=cases_tuple,
