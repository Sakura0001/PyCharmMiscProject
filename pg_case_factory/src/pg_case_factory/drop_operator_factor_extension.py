"""Bounded post-coverage cross-factor extension expander for DROP OPERATOR.

The marginal factor-value-loop (:mod:`drop_operator_factor_loop`) is the
...
The negative factors (``privilege_context`` in [non_owner,
insufficient_privilege] or ``ownership_boundary`` = non_owner,
``target_object_state`` = missing when ``if_exists_clause`` = absent,
``target_object_state`` = exists_with_dependents under RESTRICT) are crossed
here with at-most-one-failure attribution; the privilege boundary fires first
(42501), then the not-exist boundary (42704), then the dependency boundary
(2BP01).

The single official synopsis branch is crossed:

* ``branch_1`` — ``privilege_context`` x ``ownership_boundary`` x
  ``target_object_state`` x ``if_exists_clause`` x ``cascade_clause`` x
  ``dependency_state``.
"""
...
class DropOperatorFactorExtensionError(ValueError):
    """Raised when a frozen DROP OPERATOR extension input drifts."""


@dataclass(frozen=True)
class DropOperatorFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropOperatorFactorExtensionPlan:
    cases: tuple[DropOperatorFactorExtensionCase, ...]
...
# (privilege_context in [non_owner, insufficient_privilege] or
# ownership_boundary=non_owner, target_object_state=missing under
# if_exists_clause=absent, target_object_state=exists_with_dependents
# under RESTRICT) are crossed here with at-most-one-failure attribution.
...
_BRANCH_AXES: dict[str, dict[str, tuple[str, ...]]] = {
    "branch_1": {
        ...
        "dependency_state": ("no_dependents", "has_dependents"),
    },
}
...
# boundary (42704) fires when the operator is absent and IF EXISTS is
# omitted.  The dependency boundary (2BP01) fires when the operator has
# dependents and RESTRICT is used.
_PRIVILEGE_NEGATIVE = ("privilege_context", "non_owner")
_NOT_EXIST_NEGATIVE = ("target_object_state", "missing")
_DEPENDENCY_NEGATIVE = ("target_object_state", "exists_with_dependents")
...
def _privilege_failure_fires(assignment: dict[str, str]) -> bool:
    return (
        assignment.get("privilege_context") in ("non_owner", "insufficient_privilege")
        or assignment.get("ownership_boundary") == "non_owner"
    )


def _not_exist_failure_fires(assignment: dict[str, str]) -> bool:
    """not-exist fires only when IF EXISTS is absent."""
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    """exists_with_dependents fires only under a non-CASCADE drop policy."""

    return (
        assignment.get("target_object_state") == "exists_with_dependents"
...
def _present_failure_pair(
    ...
    operator is absent and IF EXISTS is absent.  The dependency boundary
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-operator-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP OPERATOR extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_operator_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP OPERATOR",
...
def build_drop_operator_factor_extension_plan(
    repository_root: Path,
) -> DropOperatorFactorExtensionPlan:
    """Build the bounded DROP OPERATOR post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_operator_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[DropOperatorFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropOperatorFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DROPOPERATOR-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_operator_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropOperatorFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "DropOperatorFactorExtensionError",
    "DropOperatorFactorExtensionCase",
    "DropOperatorFactorExtensionPlan",
    "build_drop_operator_factor_extension_plan",
