"""Render complete PostgreSQL 18.4 UPDATE factor-loop programs.
...
UPDATE is a DML row-mutation statement: the target is a ``pg_class``
...
optional FROM source ``{p}src`` and an FK referencing table
...
from .update_factor_extension import (
    UpdateFactorExtensionCase,
    _present_failure_pair,
)
from .update_factor_loop import (
    UpdateFactorCase,
...
class UpdateFactorRenderError(ValueError):
    """Raised when an UPDATE case cannot be rendered."""


@dataclass(frozen=True)
class UpdateFactorWitness:
...
def _synthetic_case(
    ...
    if pair is not None:
        ...
        factor_value = assignment.get("target_action", "update")
    return UpdateFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: UpdateFactorCase | UpdateFactorExtensionCase,
) -> UpdateFactorCase:
    if isinstance(case, UpdateFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: UpdateFactorCase) -> dict[str, str]:
...
def _update_target(a: dict[str, str], p: str) -> str:
    """The UPDATE target identifier (with optional alias)."""
...
def _with_prefix(a: dict[str, str], p: str) -> str:
    """The WITH clause prefix for the UPDATE statement."""
...
def _effective_role(a: dict[str, str], p: str) -> str:
    """The session role under which the target UPDATE runs."""
...
def _set_clause(a: dict[str, str], p: str) -> str:
    """The SET clause mapping data_or_query_shape + expression_shape."""

    cb = a.get("constraint_boundary", "none")
    # constraint_violation: change the referenced PK to orphan the FK.
    ...
    if doqs == "query_source":
        return f"SET val = (SELECT val FROM {src})"
    if doqs == "cte_source":
        return f"SET val = (SELECT val FROM {p}cte)"
    ...
    if es == "column_reference":
        return "SET val = id"
    if es == "function_call":
        return "SET val = abs(1)"
    if es == "subquery_expression":
        return "SET val = (SELECT 1)"
    return "SET val = 1"
...
def _from_clause(a: dict[str, str], p: str) -> str:
    cs = a.get("condition_shape", "simple_predicate")
    if cs == "join_or_match_condition":
        return f"FROM {_source_table_name(p)}"
    return ""
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary UPDATE statement."""

    with_prefix = _with_prefix(a, p)
    target = _update_target(a, p)
    set_clause = _set_clause(a, p)
    from_clause = _from_clause(a, p)
    where = _where_clause(a, p)
    returning = _returning_clause(a)
    parts: list[str] = [f"{with_prefix}UPDATE {target} {set_clause}"]
    if from_clause:
...
def _probe_select(
    ...
    # effect_query and returned_rows both probe post-update state.
    if not present:
        cmp_op = ">" if present else "="
        return (
...
def _resolve_case(case: UpdateFactorCase) -> _CasePlan:
    a = _baseline(case)
    ...
    # --- FROM source table (join / query_source) -------------------
...
def resolve_update_factor_witness(
    case: UpdateFactorCase | UpdateFactorExtensionCase,
    repository_root: Path,
) -> UpdateFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return UpdateFactorWitness(
...
def count_primary_update(sql: str) -> int:
    ...
    # terminated, so the primary UPDATE always starts at line start
    # (col-0), matching the content-gate regex ``^UPDATE``.  UPDATE has
    # NO optional leading keywords.
    return len(re.findall(r"(?m)^UPDATE\b", region))


def _header(case: UpdateFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : UPDATE {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_update_factor_case(
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 UPDATE。")
...
def _write_program(
    ...
    text = render_update_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_update_factor_programs(
    baseline_plan: UpdateFactorLoopPlan,
...
__all__ = [
    "UpdateFactorRenderError",
    "UpdateFactorWitness",
    "count_primary_update",
    "generate_update_factor_programs",
    "render_update_factor_case",
    "resolve_update_factor_witness",
]
