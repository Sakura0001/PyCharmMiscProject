"""Bounded PostgreSQL 18.4 runtime for PREPARE factor programs."""
...
from .prepare_factor_loop import PrepareFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-prep-sock-20260821")
PG18_PORT = 55621
PG18_DATABASE = "pgcf_prep"
...
class PrepareCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class PrepareSuiteRun:
    run_ordinal: int
    cases: tuple[PrepareCaseRuntimeResult, ...]
...
class PrepareTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "prepare_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_prepare_runs(
    run_01: PrepareSuiteRun,
    run_02: PrepareSuiteRun,
) -> PrepareTwoRunComparison:
    ...
    return PrepareTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class PrepareRuntimeCase(Protocol):
...
def build_prepare_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[PrepareRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_prepare_factor_loop_plan(root).cases
    extensions = (
        build_prepare_factor_extension_plan(root).cases
    )
    cases: list[PrepareRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: PrepareRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class PreparePg18Runner:
    ...
    def _execute_case(
        self,
        case: PrepareRuntimeCase,
        sql_path: Path,
    ) -> PrepareCaseRuntimeResult:
        ...
        return PrepareCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[PrepareRuntimeCase],
        ...
        *,
        ...
        results: list[PrepareCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_prepare_runtime_case_set",
    "compare_prepare_runs",
]
