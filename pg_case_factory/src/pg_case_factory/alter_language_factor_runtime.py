"""Bounded PostgreSQL 18.4 runtime for ALTER LANGUAGE factor programs."""
...
from .alter_language_factor_loop import AlterLanguageFactorCase
...
class AlterLanguageRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterLanguageCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterLanguageSuiteRun:
    run_ordinal: int
    cases: tuple[AlterLanguageCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterLanguageRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_language_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterLanguageTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_language_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_language_runs(
    run_01: AlterLanguageSuiteRun,
    run_02: AlterLanguageSuiteRun,
) -> AlterLanguageTwoRunComparison:
    ...
    return AlterLanguageTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterLanguageRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`AlterLanguageFactorCase`
...
def build_alter_language_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterLanguageRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0042) precedes the
    extension (0043-0642) in a stable, human-auditable order.
    """

    from .alter_language_factor_extension import (
        build_alter_language_factor_extension_plan,
    )
    from .alter_language_factor_loop import build_alter_language_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_language_factor_loop_plan(root).cases
    extensions = build_alter_language_factor_extension_plan(root).cases
    cases: list[AlterLanguageRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterLanguageRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterLanguagePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterLanguageRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterLanguageRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_language WHERE lanname LIKE '" + literal + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterLanguageRuntimeError("cleanup phase marker is not unique")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterLanguageRuntimeCase],
        ...
        *,
        ...
        results: list[AlterLanguageCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "AlterLanguageRuntimeCase",
    "AlterLanguageRuntimeError",
    "AlterLanguageSuiteRun",
    "AlterLanguageTwoRunComparison",
    "MAX_PARALLELISM",
