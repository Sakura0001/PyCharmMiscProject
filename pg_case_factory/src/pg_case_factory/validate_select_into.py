"""Actual-byte coverage validation for SELECT INTO factor-loop SQL.
...
from .select_into_factor_render import (
...
class SelectIntoFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "select_into_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_select_into_factor_witness(
        case, repository_root
    )
    expected_sql = render_select_into_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_select_into(sql) != 1
    ):
...
def validate_select_into_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return SelectIntoFactorProgramValidation(
        passed=not issues,
