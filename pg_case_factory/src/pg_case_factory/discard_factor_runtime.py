"""Bounded PostgreSQL 18.4 runtime for DISCARD factor programs."""
...
from .discard_factor_loop import DiscardFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-discard-sock-20260820")
PG18_PORT = 55618
PG18_DATABASE = "pgcf_discard"
...
@dataclass(frozen=True)
class DiscardSuiteRun:
    run_ordinal: int
    cases: tuple[DiscardCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DiscardRuntimeError(
...
class DiscardTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "discard_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_discard_runs(
    run_01: DiscardSuiteRun,
    run_02: DiscardSuiteRun,
) -> DiscardTwoRunComparison:
    ...
    return DiscardTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DiscardRuntimeCase(Protocol):
...
def build_discard_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[DiscardRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_discard_factor_loop_plan(root).cases
    )
    extensions = (
        build_discard_factor_extension_plan(root).cases
    )
    cases: list[DiscardRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class DiscardPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DiscardRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return DiscardCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DiscardRuntimeCase],
        ...
        *,
        ...
        results: list[DiscardCaseRuntimeResult] = []
        for case in cases:
