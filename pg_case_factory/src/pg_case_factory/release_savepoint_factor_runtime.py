"""Bounded PostgreSQL 18.4 runtime for RELEASE SAVEPOINT factor programs."""
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-release-savepoint-sock-20260821")
)
PG18_PORT = 55622
PG18_DATABASE = "pgcf_rsp"
...
class ReleaseSavepointRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class ReleaseSavepointCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class ReleaseSavepointSuiteRun:
    run_ordinal: int
    cases: tuple[ReleaseSavepointCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ReleaseSavepointRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "release_savepoint_pg18_suite_run",
...
@dataclass(frozen=True)
class ReleaseSavepointTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "release_savepoint_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_release_savepoint_runs(
    run_01: ReleaseSavepointSuiteRun,
    run_02: ReleaseSavepointSuiteRun,
) -> ReleaseSavepointTwoRunComparison:
    ...
    for first in run_01.cases:
        ...
        for result in pairs:
            ...
            if (
                result.cleanup_exit_code != 0
                or not result.post_clean
            ):
    ...
    return ReleaseSavepointTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ReleaseSavepointRuntimeCase(Protocol):
...
def build_release_savepoint_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ReleaseSavepointRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_release_savepoint_factor_loop_plan(root).cases
    extensions = (
        build_release_savepoint_factor_extension_plan(root).cases
    )
    cases: list[ReleaseSavepointRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: ReleaseSavepointRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ReleaseSavepointPg18Runner:
    ...
    def _execute_case(
        self,
        case: ReleaseSavepointRuntimeCase,
        sql_path: Path,
    ) -> ReleaseSavepointCaseRuntimeResult:
        ...
        return ReleaseSavepointCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ReleaseSavepointRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> ReleaseSavepointSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[ReleaseSavepointCaseRuntimeResult] = []
        for case in cases:
        ...
        return ReleaseSavepointSuiteRun(
...
__all__ = [
    "ReleaseSavepointCaseRuntimeResult",
    "ReleaseSavepointPg18Runner",
    "ReleaseSavepointRuntimeCase",
    "ReleaseSavepointRuntimeError",
    "ReleaseSavepointSuiteRun",
    "ReleaseSavepointTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_release_savepoint_runtime_case_set",
    "compare_release_savepoint_runs",
]
