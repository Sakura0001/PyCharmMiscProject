"""Bounded PostgreSQL 18.4 runtime for CREATE CAST factor programs.

This module provides the serial psql executor and two-run comparison
harness for CREATE CAST factor-loop SQL.  The runner is never invoked
during the no-DB phase; it is provided so the DB phase can execute
the same frozen SQL files and verify the provisional SQLSTATEs.

CREATE CAST creates ``pg_catalog.pg_cast`` rows, not ``pg_class``
relations, so the clean-state probe inspects ``pg_cast``,
``pg_proc``, ``pg_type``, and ``pg_roles``.
"""
...
from .create_cast_factor_loop import CreateCastFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ccast-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_ccast"
...
class CreateCastRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateCastCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateCastSuiteRun:
    run_ordinal: int
    cases: tuple[CreateCastCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateCastRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_cast_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateCastTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_cast_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_cast_runs(
    run_01: CreateCastSuiteRun,
    run_02: CreateCastSuiteRun,
) -> CreateCastTwoRunComparison:
    ...
    return CreateCastTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateCastRuntimeCase(Protocol):
...
def build_create_cast_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateCastRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_cast_factor_loop_plan(root).cases
    extensions = build_create_cast_factor_extension_plan(root).cases
    cases: list[CreateCastRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateCastRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateCastPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateCastRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_cast WHERE castsource::text LIKE '"
            + literal
            + "' OR casttarget::text LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: CreateCastRuntimeCase,
        sql_path: Path,
    ) -> CreateCastCaseRuntimeResult:
        ...
        return CreateCastCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateCastRuntimeCase],
        ...
        *,
        ...
        results: list[CreateCastCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateCastCaseRuntimeResult",
    "CreateCastPg18Runner",
    "CreateCastRuntimeCase",
    "CreateCastRuntimeError",
    "CreateCastSuiteRun",
    "CreateCastTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_cast_runtime_case_set",
    "compare_create_cast_runs",
]
