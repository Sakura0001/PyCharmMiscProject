"""Actual-byte coverage validation for CREATE EVENT TRIGGER factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 62-case baseline
...
primary-target cardinality != 1, placeholder leakage,
missing/duplicate/unknown cases, or an extension case lacking a
derivation record.  Also gates that every required factor value is
still witnessed across ``baseline union extension``.
"""
...
class CreateEventTriggerFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_event_trigger_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_event_trigger_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_event_trigger_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_event_trigger(sql) != 1
    ):
...
def validate_create_event_trigger_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateEventTriggerFactorProgramValidation:
    ...
    return CreateEventTriggerFactorProgramValidation(
        passed=not issues,
