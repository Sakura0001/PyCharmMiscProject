"""Actual-byte coverage validation for COMMENT ON factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 117-case baseline +
...
class CommentFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "comment_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_comment_factor_witness(
        case, repository_root
    )
    expected_sql = render_comment_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_comment(sql) != 1:
...
def validate_comment_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CommentFactorProgramValidation(
        passed=not issues,
