"""Actual-byte coverage validation for RESET factor-loop SQL.
...
from .reset_factor_render import (
    count_primary_reset,
...
class ResetFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reset_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_reset_factor_witness(
        case, repository_root
    )
    expected_sql = render_reset_factor_case(
        case, repository_root
    )
...
def validate_reset_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ResetFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_reset_factor_programs",
]
