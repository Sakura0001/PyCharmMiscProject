"""Bounded PostgreSQL 18.4 runtime for DROP USER MAPPING factor programs."""
...
from .drop_user_mapping_factor_loop import DropUserMappingFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dum-sock-20260820")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dum"
...
class DropUserMappingRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropUserMappingCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropUserMappingSuiteRun:
    run_ordinal: int
    cases: tuple[DropUserMappingCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropUserMappingRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_user_mapping_pg18_suite_run",
...
@dataclass(frozen=True)
class DropUserMappingTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_user_mapping_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_user_mapping_runs(
    run_01: DropUserMappingSuiteRun,
    run_02: DropUserMappingSuiteRun,
) -> DropUserMappingTwoRunComparison:
    ...
    return DropUserMappingTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropUserMappingRuntimeCase(Protocol):
...
def build_drop_user_mapping_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropUserMappingRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_user_mapping_factor_loop_plan(root).cases
    extensions = build_drop_user_mapping_factor_extension_plan(root).cases
    cases: list[DropUserMappingRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropUserMappingRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropUserMappingPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropUserMappingRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropUserMappingRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_user_mapping um "
            "JOIN pg_catalog.pg_authid a ON um.umuser = a.oid "
            "WHERE a.rolname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_server "
            "WHERE srvname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropUserMappingRuntimeCase,
        sql_path: Path,
    ) -> DropUserMappingCaseRuntimeResult:
        ...
        return DropUserMappingCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropUserMappingSuiteRun(
...
__all__ = [
    "DropUserMappingCaseRuntimeResult",
    "DropUserMappingPg18Runner",
    "DropUserMappingRuntimeCase",
    "DropUserMappingRuntimeError",
    "DropUserMappingSuiteRun",
    "DropUserMappingTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_user_mapping_runtime_case_set",
    "compare_drop_user_mapping_runs",
]
