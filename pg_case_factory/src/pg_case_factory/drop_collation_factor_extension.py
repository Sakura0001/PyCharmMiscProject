"""Bounded post-coverage cross-factor extension expander for DROP COLLATION.

The marginal factor-value-loop (:mod:`drop_collation_factor_loop`) is the
required baseline: one program per factor value, 30 local cases
(GRM 1 + SFV 27 + RISK 2).  This module adds the bounded post-coverage
extension phase allowed by ``drop_collation.yaml``
...
The T5 negative factors (``nonexistent_collation``,
...
* ``branch_1`` — ``collation_name_shape`` x ``if_exists_clause`` x
...
class DropCollationFactorExtensionError(ValueError):
    """Raised when a frozen DROP COLLATION extension input drifts."""


@dataclass(frozen=True)
class DropCollationFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropCollationFactorExtensionPlan:
    cases: tuple[DropCollationFactorExtensionCase, ...]
...
# negative factors (nonexistent_collation, dependent_objects_exist,
...
# (privilege_level=non_owner) is counted as a single unit.  The dependency
...
def _dependency_failure_fires(assignment: dict[str, str]) -> bool:
    ...
    ``referenced_by_table_column`` / ``referenced_by_index`` surface
    ``dependent_objects_restricted`` (2BPQ1) only under a non-CASCADE drop
    policy.  Under CASCADE the dependent is removed along with the collation,
    so the drop succeeds and the negative never fires.
    """
...
def _present_failure_pair(
    ...
    The privilege boundary (must be owner of collation, 42501) fires before
    the dependency check, so it is attributed first when present (it shadows
    any co-occurring dependency negative, which the at-most-one rule has
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"drop-collation-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
        ...
        entries.append(
            {
                ...
                "title": (
                    f"DROP COLLATION extension {case.ordinal:05d}: "
                ...
                "compatibility": {
                    "resolver": "drop_collation_factor_extension",
                    "attribution_pair": pair,
                    "success_when": case.outcome == "success",
                    "failure_when": case.outcome == "expected_failure",
                },
                "sql_shape": {
                    "target": "DROP COLLATION",
...
def build_drop_collation_factor_extension_plan(
    repository_root: Path,
) -> DropCollationFactorExtensionPlan:
    """Build the bounded DROP COLLATION post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_drop_collation_factor_loop_plan(root)
    ...
    cases: list[DropCollationFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    DropCollationFactorExtensionCase(
                        ...
                        derivation_id=(
                            f"DC-EXT|{ordinal:05d}|{action}"
                            f"|{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "drop_collation_required_baseline_factor_space"
                        ),
                        derivation_reason=(
    ...
    return DropCollationFactorExtensionPlan(
        cases=cases_tuple,
