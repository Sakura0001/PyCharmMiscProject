"""Bounded PostgreSQL 18.4 runtime for ALTER SUBSCRIPTION factor programs."""
...
from .alter_subscription_factor_loop import AlterSubscriptionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-asub-sock-20260820")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_asub"
...
class AlterSubscriptionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterSubscriptionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterSubscriptionSuiteRun:
    run_ordinal: int
    cases: tuple[AlterSubscriptionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterSubscriptionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_subscription_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterSubscriptionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_subscription_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_subscription_runs(
    run_01: AlterSubscriptionSuiteRun,
    run_02: AlterSubscriptionSuiteRun,
    ...
    return AlterSubscriptionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterSubscriptionRuntimeCase(Protocol):
...
def build_alter_subscription_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterSubscriptionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_subscription_factor_loop_plan(root).cases
    extensions = build_alter_subscription_factor_extension_plan(root).cases
    cases: list[AlterSubscriptionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterSubscriptionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterSubscriptionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterSubscriptionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: AlterSubscriptionRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[AlterSubscriptionRuntimeCase],
        ...
        *,
        ...
        results: list[AlterSubscriptionCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterSubscriptionSuiteRun(
