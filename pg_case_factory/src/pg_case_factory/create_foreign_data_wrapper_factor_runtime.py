"""Bounded PostgreSQL 18.4 runtime for CREATE FOREIGN DATA WRAPPER factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cfdw-sock-20260820")
PG18_PORT = 55544
PG18_DATABASE = "pgcf_cfdw"
...
class CreateForeignDataWrapperRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateForeignDataWrapperCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateForeignDataWrapperSuiteRun:
    run_ordinal: int
    cases: tuple[CreateForeignDataWrapperCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateForeignDataWrapperRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_foreign_data_wrapper_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateForeignDataWrapperTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_foreign_data_wrapper_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_foreign_data_wrapper_runs(
    run_01: CreateForeignDataWrapperSuiteRun,
    run_02: CreateForeignDataWrapperSuiteRun,
    ...
    return CreateForeignDataWrapperTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateForeignDataWrapperRuntimeCase(Protocol):
...
def build_create_foreign_data_wrapper_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateForeignDataWrapperRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_foreign_data_wrapper_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_foreign_data_wrapper_factor_extension_plan(root).cases
    )
    cases: list[CreateForeignDataWrapperRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: CreateForeignDataWrapperRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateForeignDataWrapperPg18Runner:
    ...
    def run_cases(
        self,
        cases: Sequence[CreateForeignDataWrapperRuntimeCase],
        ...
        *,
        ...
        results: list[CreateForeignDataWrapperCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateForeignDataWrapperSuiteRun(
...
__all__ = [
    ...
    "CreateForeignDataWrapperRuntimeCase",
