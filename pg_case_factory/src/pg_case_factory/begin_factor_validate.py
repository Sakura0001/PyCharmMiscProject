"""Actual-byte coverage validation for BEGIN factor-loop SQL.
...
class BeginFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "begin_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_begin_factor_witness(
        case, repository_root
    )
    expected_sql = render_begin_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_begin(sql) != 1:
...
def validate_begin_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return BeginFactorProgramValidation(
        passed=not issues,
