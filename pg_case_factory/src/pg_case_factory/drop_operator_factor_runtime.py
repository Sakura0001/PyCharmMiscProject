"""Bounded PostgreSQL 18.4 runtime for DROP OPERATOR factor programs."""
...
from .drop_operator_factor_loop import DropOperatorFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-do-sock-20260820")
PG18_PORT = 55510
PG18_DATABASE = "pgcf_do"
...
class DropOperatorRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropOperatorCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropOperatorSuiteRun:
    run_ordinal: int
    cases: tuple[DropOperatorCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropOperatorRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_pg18_suite_run",
...
@dataclass(frozen=True)
class DropOperatorTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_operator_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_operator_runs(
    run_01: DropOperatorSuiteRun,
    run_02: DropOperatorSuiteRun,
    ...
    return DropOperatorTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropOperatorRuntimeCase(Protocol):
...
def build_drop_operator_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropOperatorRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_operator_factor_loop_plan(root).cases
    extensions = build_drop_operator_factor_extension_plan(root).cases
    cases: list[DropOperatorRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropOperatorRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropOperatorPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropOperatorRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropOperatorRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_operator WHERE oprname LIKE '" + literal + "' "
    ...
    def run_cases(
        self,
        cases: Sequence[DropOperatorRuntimeCase],
        ...
        *,
        ...
        results: list[DropOperatorCaseRuntimeResult] = []
        for case in cases:
