"""Bounded PostgreSQL 18.4 runtime for DROP TABLESPACE factor programs."""
...
from .drop_tablespace_factor_loop import DropTablespaceFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dts-sock-20260821")
PG18_PORT = 55513
PG18_DATABASE = "pgcf_dts"
...
class DropTablespaceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTablespaceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTablespaceSuiteRun:
    run_ordinal: int
    cases: tuple[DropTablespaceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTablespaceRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_tablespace_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTablespaceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_tablespace_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_tablespace_runs(
    run_01: DropTablespaceSuiteRun,
    run_02: DropTablespaceSuiteRun,
) -> DropTablespaceTwoRunComparison:
    ...
    return DropTablespaceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTablespaceRuntimeCase(Protocol):
...
def build_drop_tablespace_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTablespaceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_tablespace_factor_loop_plan(root).cases
    extensions = build_drop_tablespace_factor_extension_plan(root).cases
    cases: list[DropTablespaceRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTablespaceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTablespacePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropTablespaceRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTablespaceRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_tablespace WHERE spcname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTablespaceRuntimeCase,
        sql_path: Path,
    ) -> DropTablespaceCaseRuntimeResult:
        ...
        return DropTablespaceCaseRuntimeResult(
...
__all__ = [
    "DropTablespaceCaseRuntimeResult",
    "DropTablespacePg18Runner",
    "DropTablespaceRuntimeCase",
    "DropTablespaceRuntimeError",
    "DropTablespaceSuiteRun",
    "DropTablespaceTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_tablespace_runtime_case_set",
    "compare_drop_tablespace_runs",
]
