"""Actual-byte coverage validation for LISTEN factor-loop SQL.
...
from .listen_factor_render import (
...
class ListenFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "listen_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_listen_factor_witness(
        case, repository_root
    )
    expected_sql = render_listen_factor_case(
        case, repository_root
    )
...
def validate_listen_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ListenFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_listen_factor_programs",
]
