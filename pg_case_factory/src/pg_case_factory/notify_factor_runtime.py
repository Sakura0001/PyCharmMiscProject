"""Bounded PostgreSQL 18.4 runtime for NOTIFY factor programs."""
...
from .notify_factor_loop import NotifyFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-notify-sock-20260821")
PG18_PORT = 55620
PG18_DATABASE = "pgcf_notify"
...
@dataclass(frozen=True)
class NotifySuiteRun:
    run_ordinal: int
    cases: tuple[NotifyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise NotifyRuntimeError(
...
class NotifyTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "notify_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_notify_runs(
    run_01: NotifySuiteRun,
    run_02: NotifySuiteRun,
) -> NotifyTwoRunComparison:
...
class NotifyRuntimeCase(Protocol):
...
def build_notify_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[NotifyRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_notify_factor_loop_plan(root).cases
    )
    extensions = (
        build_notify_factor_extension_plan(root).cases
    )
    cases: list[NotifyRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class NotifyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise NotifyRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return NotifyCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[NotifyRuntimeCase],
        ...
        *,
        ...
        results: list[NotifyCaseRuntimeResult] = []
        for case in cases:
