"""Bounded PostgreSQL 18.4 runtime for DROP VIEW factor programs."""
...
from .drop_view_factor_loop import DropViewFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dv-sock-20260821")
PG18_PORT = 55504
PG18_DATABASE = "pgcf_dv"
...
class DropViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropViewSuiteRun:
    run_ordinal: int
    cases: tuple[DropViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropViewRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_view_pg18_suite_run",
...
@dataclass(frozen=True)
class DropViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_view_runs(
    run_01: DropViewSuiteRun,
    run_02: DropViewSuiteRun,
) -> DropViewTwoRunComparison:
    ...
    return DropViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropViewRuntimeCase(Protocol):
...
def build_drop_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropViewRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_view_factor_loop_plan(root).cases
    extensions = build_drop_view_factor_extension_plan(root).cases
    cases: list[DropViewRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropViewPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropViewRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropViewRuntimeCase,
        sql_path: Path,
    ) -> DropViewCaseRuntimeResult:
        ...
        return DropViewCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropViewRuntimeCase],
        ...
        *,
        ...
        results: list[DropViewCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_view_runtime_case_set",
    "compare_drop_view_runs",
]
