"""Bounded PostgreSQL 18.4 runtime for CREATE TRIGGER factor programs."""
...
from .create_trigger_factor_loop import CreateTriggerFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ctrg-sock-20260820")
PG18_PORT = 55502
PG18_DATABASE = "pgcf_ctrg"
...
class CreateTriggerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTriggerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTriggerSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTriggerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTriggerRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_trigger_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTriggerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_trigger_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_trigger_runs(
    run_01: CreateTriggerSuiteRun,
    run_02: CreateTriggerSuiteRun,
    ...
    return CreateTriggerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTriggerRuntimeCase(Protocol):
...
def build_create_trigger_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateTriggerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_trigger_factor_loop_plan(root).cases
    extensions = build_create_trigger_factor_extension_plan(root).cases
    cases: list[CreateTriggerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateTriggerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTriggerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateTriggerRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_trigger WHERE tgname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTriggerRuntimeCase],
        ...
        *,
        ...
        results: list[CreateTriggerCaseRuntimeResult] = []
        for case in cases:
