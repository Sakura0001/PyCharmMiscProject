"""Bounded post-coverage extension plan for SHOW factor regress.

The baseline :mod:`show_factor_loop` assigns one local SQL program to
every ``SFV``/``GRM`` obligation (marginal 1:1, 44 local cases).  This
...
SHOW is a session-scoped, read-only display statement that creates no
tables, so the bookend (``DROP TABLE IF EXISTS``) is never emitted.  The
session runtime state (parameters) is the semantic witness target.
``pg_catalog.pg_settings`` (``name``/``setting``) is the canonical GUC
repository SHOW displays.  Mirroring the GUC-family precedent, the
in-transaction ``transaction_visibility`` values are the failure surface
(provisional SQLSTATE ``25001``); no SHOW variant is privilege-restricted.
...
class ShowFactorExtensionError(ValueError):
    """Raised when SHOW extension input drifts."""


@dataclass(frozen=True)
class ShowFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class ShowFactorExtensionPlan:
    cases: tuple[ShowFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "show_required_factor_value_matrix"

_CONSUMER_ACTION = "show"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"show-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_show_factor_extension_plan(
    repository_root: Path,
) -> ShowFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_show_factor_loop_plan(root)
    ...
    cases: list[ShowFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise ShowFactorExtensionError(
                    ...
                    cases.append(
                        ShowFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"SHOW{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"show_{ordinal:05d}_"
                            ),
    ...
    if ordinals != expected_ordinals:
        raise ShowFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise ShowFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("SHOW-EXT|")
        for case in plan.cases
    ):
        raise ShowFactorExtensionError(
...
__all__ = [
    "ShowFactorExtensionError",
    "ShowFactorExtensionCase",
    "ShowFactorExtensionPlan",
    "build_show_factor_extension_plan",
    "_BASELINE_COUNT",
    "_CAP",
    "_VERIFICATION_MODES",
    "_CLEANUP_MODES",
