"""Bounded PostgreSQL 18.4 runtime for CREATE TABLE factor programs.
...
from .create_table_factor_loop import CreateTableFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ctable-sock-20260820")
PG18_PORT = 55610
PG18_DATABASE = "pgcf_ctable"
...
class CreateTableRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTableCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTableSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTableCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTableRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_table_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTableTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_table_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_table_runs(
    run_01: CreateTableSuiteRun,
    run_02: CreateTableSuiteRun,
) -> CreateTableTwoRunComparison:
    ...
    return CreateTableTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTableRuntimeCase(Protocol):
...
def build_create_table_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateTableRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_table_factor_loop_plan(root).cases
    extensions = build_create_table_factor_extension_plan(root).cases
    cases: list[CreateTableRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateTableRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTablePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateTableRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateTableRuntimeCase,
        sql_path: Path,
    ) -> CreateTableCaseRuntimeResult:
        ...
        return CreateTableCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateTableRuntimeCase],
        ...
        *,
        ...
        results: list[CreateTableCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CreateTableCaseRuntimeResult",
    "CreateTablePg18Runner",
    "CreateTableRuntimeCase",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_table_runtime_case_set",
    "compare_create_table_runs",
]
