"""Render complete PostgreSQL 18.4 NOTIFY factor-loop programs.
...
NOTIFY is a session-scoped statement (``NOTIFY channel [ , payload ]``)
that generates a notification on a channel.  It creates no TABLE, so the
bookend (``DROP TABLE IF EXISTS``) is never emitted (table-less; bookend
gate exempt).  The credited target ``NOTIFY <channel>;`` is itself the
byte-observable syntax witness.  The oracle observes the session
listening set via the built-in ``pg_listening_channels()``
...
gate passes.  ``NOTIFY`` has no optional leading keywords, so the col-0
keyword regex is ``(?m)^NOTIFY(?:\\s|;|$)``.
"""
...
from .notify_factor_extension import (
    NotifyFactorExtensionCase,
    _present_failure_pair,
)
from .notify_factor_loop import (
    NotifyFactorCase,
...
class NotifyFactorRenderError(ValueError):
    """Raised when a NOTIFY case cannot be rendered."""


@dataclass(frozen=True)
class NotifyFactorWitness:
...
def _synthetic_case(
    ...
    if pair is not None:
        ...
        factor_value = assignment.get("target_action", "notify")
    return NotifyFactorCase(
        ordinal=ext.ordinal,
        case_id=ext.case_id,
...
def _as_render_case(
    case: NotifyFactorCase | NotifyFactorExtensionCase,
) -> NotifyFactorCase:
    if isinstance(case, NotifyFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: NotifyFactorCase) -> dict[str, str]:
    return dict(case.baseline_assignments)


def _is_failure(case: NotifyFactorCase) -> bool:
...
def _effective_role(a: dict[str, str], p: str) -> str:
    """The session role under which the target NOTIFY runs."""
...
def _target_channel(a: dict[str, str], p: str) -> str:
    """The channel token for the credited NOTIFY target per name_shape."""
    ...
    # plain_identifier, alias_used (NOTIFY has no alias; covered abstractly)
    return f"{p}ch"


def _probe_channel(
    case: NotifyFactorCase, a: dict[str, str], p: str
) -> str:
...
def _build_setup(
    ...
    NOTIFY is idempotent -- the credited ``NOTIFY <channel>;`` target is
    itself the byte-observable syntax witness.  Only a non-superuser
...
def _build_target(
    ...
    """The primary NOTIFY statement."""

    if _is_failure(case):
        if case.expected_sqlstate == "42601":
            return "NOTIFY ;"
        if case.expected_sqlstate == "08P01":
            return f"NOTIFY {_TOO_LONG_CHANNEL};"
...
def _header(case: NotifyFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
        ...
        f"-- version      : {_VERSION}",
        f"-- description  : NOTIFY {case.factor_key}="
        f"{case.factor_value}",
        f"-- FE           : {_FE}",
        "-- ++",
        "-- --------------------------------------------------------",
...
def render_notify_factor_case(
    ...
    lines.append("-- 3. 执行唯一获得覆盖信用的 NOTIFY。")
...
def _write_program(
    ...
    text = render_notify_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_notify_factor_programs(
    baseline_plan: NotifyFactorLoopPlan,
...
def count_primary_notify(sql: str) -> int:
    ...
    return len(
        re.findall(r"(?im)^\s*NOTIFY\b", region)
    )


def resolve_notify_factor_witness(
    case: NotifyFactorCase | NotifyFactorExtensionCase,
    repository_root: Path,
) -> NotifyFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return NotifyFactorWitness(
...
__all__ = [
    "NotifyFactorRenderError",
    "NotifyFactorWitness",
    "count_primary_notify",
    "generate_notify_factor_programs",
    "render_notify_factor_case",
    "resolve_notify_factor_witness",
]
