"""Actual-byte coverage validation for PREPARE factor-loop SQL.
...
from .prepare_factor_render import (
...
class PrepareFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "prepare_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_prepare_factor_witness(
        case, repository_root
    )
    expected_sql = render_prepare_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_prepare(sql) != 1
    ):
...
def validate_prepare_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> PrepareFactorProgramValidation:
    ...
    return PrepareFactorProgramValidation(
        passed=not issues,
