"""Bounded post-coverage extension plan for UNLISTEN factor regress.

The baseline :mod:`unlisten_factor_loop` assigns one local SQL program to
...
UNLISTEN is a session-scoped statement that creates no tables, so the
...
target.  UNLISTEN is permissive inside transactions, so the only crossed
...
class UnlistenFactorExtensionError(ValueError):
    """Raised when UNLISTEN extension input drifts."""


@dataclass(frozen=True)
class UnlistenFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class UnlistenFactorExtensionPlan:
    cases: tuple[UnlistenFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "unlisten_required_factor_value_matrix"

_CONSUMER_ACTION = "unlisten"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"unlisten-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_unlisten_factor_extension_plan(
    repository_root: Path,
) -> UnlistenFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_unlisten_factor_loop_plan(root)
    ...
    cases: list[UnlistenFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise UnlistenFactorExtensionError(
                    ...
                    cases.append(
                        UnlistenFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"UNLISTEN{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"unlisten_{ordinal:05d}_"
                            ),
    ...
    plan = UnlistenFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if ordinals != expected_ordinals:
        raise UnlistenFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise UnlistenFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("UNLISTEN-EXT|")
        for case in plan.cases
    ):
        raise UnlistenFactorExtensionError(
...
__all__ = [
    "UnlistenFactorExtensionError",
    "UnlistenFactorExtensionCase",
    "UnlistenFactorExtensionPlan",
    "build_unlisten_factor_extension_plan",
    "_BASELINE_COUNT",
    "_CAP",
    "_VERIFICATION_MODES",
    "_CLEANUP_MODES",
