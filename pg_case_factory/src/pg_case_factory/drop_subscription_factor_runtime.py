"""Bounded PostgreSQL 18.4 runtime for DROP SUBSCRIPTION factor programs."""
...
from .drop_subscription_factor_loop import DropSubscriptionFactorCase
...
class DropSubscriptionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropSubscriptionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropSubscriptionSuiteRun:
    run_ordinal: int
    cases: tuple[DropSubscriptionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropSubscriptionRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_subscription_pg18_suite_run",
...
@dataclass(frozen=True)
class DropSubscriptionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_subscription_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_subscription_runs(
    run_01: DropSubscriptionSuiteRun,
    run_02: DropSubscriptionSuiteRun,
    ...
    return DropSubscriptionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropSubscriptionRuntimeCase(Protocol):
...
def build_drop_subscription_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropSubscriptionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_subscription_factor_loop_plan(root).cases
    extensions = build_drop_subscription_factor_extension_plan(root).cases
    cases: list[DropSubscriptionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropSubscriptionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropSubscriptionPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropSubscriptionRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropSubscriptionRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_subscription WHERE subname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropSubscriptionRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[DropSubscriptionRuntimeCase],
        ...
        *,
        ...
        results: list[DropSubscriptionCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropSubscriptionSuiteRun(
