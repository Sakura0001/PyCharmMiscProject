"""Bounded PostgreSQL 18.4 runtime for DROP FUNCTION factor programs."""
...
from .drop_function_factor_loop import DropFunctionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-df-sock-20260820")
PG18_PORT = 55505
PG18_DATABASE = "pgcf_df"
...
class DropFunctionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropFunctionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropFunctionSuiteRun:
    run_ordinal: int
    cases: tuple[DropFunctionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropFunctionRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_function_pg18_suite_run",
...
@dataclass(frozen=True)
class DropFunctionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_function_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_function_runs(
    run_01: DropFunctionSuiteRun,
    run_02: DropFunctionSuiteRun,
) -> DropFunctionTwoRunComparison:
    ...
    return DropFunctionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropFunctionRuntimeCase(Protocol):
...
def build_drop_function_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropFunctionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_function_factor_loop_plan(root).cases
    extensions = build_drop_function_factor_extension_plan(root).cases
    cases: list[DropFunctionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropFunctionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropFunctionPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropFunctionRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropFunctionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropFunctionRuntimeCase,
        sql_path: Path,
    ) -> DropFunctionCaseRuntimeResult:
        ...
        return DropFunctionCaseRuntimeResult(
...
__all__ = [
    "DropFunctionCaseRuntimeResult",
    "DropFunctionPg18Runner",
    "DropFunctionRuntimeCase",
    "DropFunctionRuntimeError",
    "DropFunctionSuiteRun",
    "DropFunctionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_function_runtime_case_set",
    "compare_drop_function_runs",
]
