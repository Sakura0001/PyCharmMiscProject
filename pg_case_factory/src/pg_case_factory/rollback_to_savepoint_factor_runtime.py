"""Bounded PostgreSQL 18.4 runtime for ROLLBACK TO SAVEPOINT factor programs."""
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-rollback-to-savepoint-sock-20260821")
)
PG18_PORT = 55622
PG18_DATABASE = "pgcf_rollback_to_savepoint"
...
class RollbackToSavepointRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class RollbackToSavepointCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class RollbackToSavepointSuiteRun:
    run_ordinal: int
    cases: tuple[RollbackToSavepointCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise RollbackToSavepointRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_to_savepoint_pg18_suite_run",
...
@dataclass(frozen=True)
class RollbackToSavepointTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "rollback_to_savepoint_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_rollback_to_savepoint_runs(
    run_01: RollbackToSavepointSuiteRun,
    run_02: RollbackToSavepointSuiteRun,
    ...
    return RollbackToSavepointTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class RollbackToSavepointRuntimeCase(Protocol):
...
def build_rollback_to_savepoint_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[RollbackToSavepointRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_rollback_to_savepoint_factor_loop_plan(root).cases
    extensions = (
        build_rollback_to_savepoint_factor_extension_plan(root).cases
    )
    cases: list[RollbackToSavepointRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: RollbackToSavepointRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class RollbackToSavepointPg18Runner:
    ...
    def _execute_case(
        self,
        case: RollbackToSavepointRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[RollbackToSavepointRuntimeCase],
        ...
        *,
        ...
        results: list[RollbackToSavepointCaseRuntimeResult] = []
        for case in cases:
        ...
        return RollbackToSavepointSuiteRun(
...
__all__ = [
    ...
    "RollbackToSavepointRuntimeCase",
