"""Bounded PostgreSQL 18.4 runtime for DROP TABLE factor programs."""
...
from .drop_table_factor_loop import DropTableFactorCase
...
class DropTableRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTableCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTableSuiteRun:
    run_ordinal: int
    cases: tuple[DropTableCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTableRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_table_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTableTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_table_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_table_runs(
    run_01: DropTableSuiteRun,
    run_02: DropTableSuiteRun,
    ...
    return DropTableTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTableRuntimeCase(Protocol):
...
def build_drop_table_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTableRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_table_factor_loop_plan(root).cases
    extensions = build_drop_table_factor_extension_plan(root).cases
    cases: list[DropTableRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTableRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTablePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTableRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[DropTableRuntimeCase],
        ...
        *,
        ...
        results: list[DropTableCaseRuntimeResult] = []
        for case in cases:
