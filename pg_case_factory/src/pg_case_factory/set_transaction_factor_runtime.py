"""Bounded PostgreSQL 18.4 runtime for SET TRANSACTION factor programs.

This module is the no-DB static tickoff runtime: programs are validated
by actual-byte re-render, not by execution against a live PostgreSQL
server.  ``runtime_status`` is ``not_run_static_sql_only`` and all
SQLSTATEs remain provisional until the DB phase verifies on PG 18.4.
"""
...
from .set_transaction_factor_loop import SetTransactionFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-settransaction-sock-20260821")
PG18_PORT = 55621
PG18_DATABASE = "pgcf_settransaction"
...
class SetTransactionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class SetTransactionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class SetTransactionSuiteRun:
    run_ordinal: int
    cases: tuple[SetTransactionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SetTransactionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_transaction_pg18_suite_run",
...
@dataclass(frozen=True)
class SetTransactionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "set_transaction_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_set_transaction_runs(
    run_01: SetTransactionSuiteRun,
    run_02: SetTransactionSuiteRun,
) -> SetTransactionTwoRunComparison:
    ...
    return SetTransactionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class SetTransactionRuntimeCase(Protocol):
...
def build_set_transaction_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SetTransactionRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_set_transaction_factor_loop_plan(root).cases
    )
    extensions = (
        build_set_transaction_factor_extension_plan(root).cases
    )
    cases: list[SetTransactionRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(
    case: SetTransactionRuntimeCase,
) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SetTransactionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise SetTransactionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        case: SetTransactionRuntimeCase,
        sql_path: Path,
    ) -> SetTransactionCaseRuntimeResult:
        ...
        return SetTransactionCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[SetTransactionRuntimeCase],
        ...
        *,
        ...
        results: list[SetTransactionCaseRuntimeResult] = []
        for case in cases:
