"""Bounded PostgreSQL 18.4 runtime for DROP GROUP factor programs."""
...
from .drop_group_factor_loop import DropGroupFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dg-sock-20260820")
PG18_PORT = 55506
PG18_DATABASE = "pgcf_dg"
...
class DropGroupRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropGroupCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropGroupSuiteRun:
    run_ordinal: int
    cases: tuple[DropGroupCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropGroupRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_group_pg18_suite_run",
...
@dataclass(frozen=True)
class DropGroupTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_group_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_group_runs(
    run_01: DropGroupSuiteRun,
    run_02: DropGroupSuiteRun,
) -> DropGroupTwoRunComparison:
    ...
    return DropGroupTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropGroupRuntimeCase(Protocol):
...
def build_drop_group_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropGroupRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_group_factor_loop_plan(root).cases
    extensions = build_drop_group_factor_extension_plan(root).cases
    cases: list[DropGroupRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropGroupRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropGroupPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropGroupRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropGroupRuntimeCase,
        sql_path: Path,
    ) -> DropGroupCaseRuntimeResult:
        ...
        return DropGroupCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropGroupRuntimeCase],
        ...
        *,
        ...
        results: list[DropGroupCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropGroupCaseRuntimeResult",
    "DropGroupPg18Runner",
    "DropGroupRuntimeCase",
    "DropGroupRuntimeError",
    "DropGroupSuiteRun",
    "DropGroupTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_group_runtime_case_set",
    "compare_drop_group_runs",
]
