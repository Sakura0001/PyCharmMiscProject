...
CASE_EXECUTION_PROFILES = ("basic_psql", "external_isolated")
...
CANONICAL_SCOPE_SELECTORS = {
    ...
    "relation": "#relation_kinds.all_pg18_relkinds",
...
_REPO_TYPE_CATALOG = "skills/pg-sql-generation/references/common/pg18_type_catalog.md"
_SKILL_TYPE_CATALOG = "references/common/pg18_type_catalog.md"
...
CANONICAL_SCOPE_SOURCE_GROUPS = {
    ...
    "relation": (
        (f"{_REPO_INVENTORY}#relation_kinds.all_pg18_relkinds",),
        (f"{_SKILL_INVENTORY}#relation_kinds.all_pg18_relkinds",),
    ),
    "table": (
...
class FeatureManifest:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "FeatureManifest":
        ...
        if compatibility_target.lower() != "postgresql-18.4":
            raise ContractValidationError(
                "feature_manifest.compatibility_target must be postgresql-18.4"
            )
...
class ExecutionEndpoint:
    """One formal differential endpoint addressed only through libpq service."""
    ...
    def from_dict(
        ...
        service = _required_string(document, "service", location)
        if not _LIBPQ_SERVICE_PATTERN.fullmatch(service):
            raise ContractValidationError(
                f"{location}.service must be a bare libpq service name"
            )
        ...
        if (
            ...
            or "=" in database
            or lowered_database.startswith("postgresql://")
            or lowered_database.startswith("postgres://")
        ...
        expected_system_identifier = document["expected_system_identifier"]
        if (
            not isinstance(expected_system_identifier, str)
            or re.fullmatch(r"[1-9][0-9]*", expected_system_identifier) is None
        ):
            raise ContractValidationError(
                f"{location}.expected_system_identifier must be a positive "
                "decimal string"
            )
...
class ExecutionSecurityPolicy:
    credential_source: str = "external-libpq-service"
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "ExecutionSecurityPolicy":
        ...
        if document["credential_source"] != "external-libpq-service":
            raise ContractValidationError(
                f"{location}.credential_source must be external-libpq-service"
            )
...
class ExecutionProfile:
    """Immutable configuration for a formal PostgreSQL 18.4 differential run."""
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "ExecutionProfile":
        ...
        if document["compatibility_target"] != "postgresql-18.4":
            raise ContractValidationError(
                "execution_profile.compatibility_target must be postgresql-18.4"
            )
        ...
        if reference.service == dut.service:
            raise ContractValidationError(
                "execution_profile reference and DUT must use different libpq services"
            )
        ...
        if (
            reference.expected_system_identifier
            == dut.expected_system_identifier
        ):
            raise ContractValidationError(
...
def _validate_case_execution_route(
    ...
    if execution_profile == "basic_psql" and execution_harness is not None:
        raise ContractValidationError(
            f"{location} basic_psql route must not declare execution_harness"
        )
...
class TestPoint:
    ...
    description: Optional[str] = None
    default_execution_profile: str = "basic_psql"
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.default_execution_profile != "basic_psql":
...
class CaseManifest:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "CaseManifest":
        ...
        if oracle != "upstream-postgresql-18.4":
            raise ContractValidationError(
                "case_manifest.comparison.oracle must be upstream-postgresql-18.4"
            )
