"""Actual-byte coverage validation for CREATE TABLE AS factor-loop SQL.
...
from .create_table_as_factor_render import (
...
class CreateTableAsFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_table_as_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_table_as_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_table_as(sql) != 1:
...
def validate_create_table_as_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateTableAsFactorProgramValidation:
    ...
    return CreateTableAsFactorProgramValidation(
        passed=not issues,
