"""Actual-byte coverage validation for DEALLOCATE factor-loop SQL.
...
from .deallocate_factor_render import (
...
class DeallocateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "deallocate_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_deallocate_factor_witness(
        case, repository_root
    )
    expected_sql = render_deallocate_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_deallocate(sql) != 1
    ):
...
def validate_deallocate_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> DeallocateFactorProgramValidation:
    ...
    return DeallocateFactorProgramValidation(
        passed=not issues,
...
__all__ = [
    ...
    "validate_deallocate_factor_programs",
]
