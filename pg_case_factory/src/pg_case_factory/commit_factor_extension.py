"""Bounded post-coverage cross-factor extension expander for COMMIT.

The marginal factor-value-loop (:mod:`commit_factor_loop`) is the required
...
``commit.yaml`` ``post_coverage_extension_policy.enabled: true``:
cross-factor combinations of the positive T1-T2 behaviour axes across
the single COMMIT synopsis branch, with at most one failure-causing value
per case so attribution stays clean, and ``verification_mode`` crossed so
every declared T6 value is exercised.

COMMIT is a TCL transaction-control statement with one synopsis branch.
...
class CommitFactorExtensionError(ValueError):
    """Raised when a frozen COMMIT extension input drifts."""


@dataclass(frozen=True)
class CommitFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CommitFactorExtensionPlan:
    cases: tuple[CommitFactorExtensionCase, ...]
...
# General axes crossed for the single COMMIT branch.
...
# target_action are overridden per branch.  COMMIT's success default for
...
# attribution.
#
# COMMIT: ``and_chain`` / ``and_no_chain`` are VALID syntax (not negatives).
    ...
    for values in itertools.product(
        *[_GENERAL_AXES[n] for n in names]
        ...
        if len(assignment) != len(set(assignment)):
            raise CommitFactorExtensionError(
        ...
        entries.append(
            {
                ...
                "title": (
                    f"COMMIT extension {case.ordinal:05d}: "
...
def build_commit_factor_extension_plan(
    repository_root: Path,
) -> CommitFactorExtensionPlan:
    """Build the bounded COMMIT post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_commit_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[CommitFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    CommitFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"COMMIT{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"commit_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"COMMIT-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "commit_required_factor_value_matrix"
                        ),
                        derivation_reason=(
    ...
    return CommitFactorExtensionPlan(
        cases=cases_tuple,
