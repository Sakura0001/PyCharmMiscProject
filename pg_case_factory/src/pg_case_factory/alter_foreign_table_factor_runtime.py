"""Bounded PostgreSQL 18.4 runtime for ALTER FOREIGN TABLE factor programs."""
...
class AlterForeignTableRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterForeignTableCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterForeignTableSuiteRun:
    run_ordinal: int
    cases: tuple[AlterForeignTableCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterForeignTableRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_foreign_table_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterForeignTableTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_foreign_table_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_foreign_table_runs(
    run_01: AlterForeignTableSuiteRun,
    run_02: AlterForeignTableSuiteRun,
) -> AlterForeignTableTwoRunComparison:
    ...
    return AlterForeignTableTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterForeignTablePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterForeignTableRuntimeError("unsafe object prefix")
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterForeignTableRuntimeError("cleanup phase marker is not unique")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterForeignTableFactorCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
    ) -> AlterForeignTableSuiteRun:
        self.verify_server()
        results: list[AlterForeignTableCaseRuntimeResult] = []
        for case in cases:
            try:
        ...
        return AlterForeignTableSuiteRun(
