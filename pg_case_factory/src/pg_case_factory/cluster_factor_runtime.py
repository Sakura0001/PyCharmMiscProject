"""Bounded PostgreSQL 18.4 runtime for CLUSTER factor programs.
...
gated behind :meth:`ClusterPg18Runner.verify_server`.

CLUSTER physically reorders a table according to an index; the runner
uses an isolated socket/port/database so it never collides with the
``ALTER SUBSCRIPTION`` runner.
"""
...
from .cluster_factor_loop import ClusterFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cluster-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_cluster"
PG18_SUPERUSER = "pgcf_cluster_superuser"
...
@dataclass(frozen=True)
class ClusterSuiteRun:
    run_ordinal: int
    cases: tuple[ClusterCaseRuntimeResult, ...]
...
class ClusterTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "cluster_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_cluster_runs(
    run_01: ClusterSuiteRun,
    run_02: ClusterSuiteRun,
    ...
    return ClusterTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class ClusterRuntimeCase(Protocol):
...
def build_cluster_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[ClusterRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_cluster_factor_loop_plan(root).cases
    extensions = build_cluster_factor_extension_plan(root).cases
    cases: list[ClusterRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: ClusterRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class ClusterPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ClusterRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[ClusterRuntimeCase],
        ...
        *,
        ...
        results: list[ClusterCaseRuntimeResult] = []
        for case in cases:
