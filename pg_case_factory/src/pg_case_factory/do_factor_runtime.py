"""Bounded PostgreSQL 18.4 runtime for DO factor programs."""
...
from .do_factor_loop import DoFactorCase
...
class DoSuiteRun:
    run_ordinal: int
    cases: tuple[DoCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DoRuntimeError(
...
class DoTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "do_pg18_two_run_comparison",
            "passed": self.passed,
...
def build_do_runtime_case_set(
    ...
    baseline = (
        build_do_factor_loop_plan(root).cases
    )
    extensions = (
        build_do_factor_extension_plan(root).cases
    )
    cases: list[DoRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: DoRuntimeCase) -> int:
...
class DoPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DoRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[DoCaseRuntimeResult] = []
        for case in cases:
