"""Bounded PostgreSQL 18.4 runtime for ALTER TRIGGER factor programs."""
...
from .alter_trigger_factor_loop import AlterTriggerFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-atrig-sock-20260820")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_atrig"
...
class AlterTriggerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTriggerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTriggerSuiteRun:
    run_ordinal: int
    cases: tuple[AlterTriggerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTriggerRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_trigger_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTriggerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_trigger_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_trigger_runs(
    run_01: AlterTriggerSuiteRun,
    run_02: AlterTriggerSuiteRun,
) -> AlterTriggerTwoRunComparison:
    ...
    return AlterTriggerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTriggerRuntimeCase(Protocol):
...
def build_alter_trigger_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterTriggerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_trigger_factor_loop_plan(root).cases
    extensions = build_alter_trigger_factor_extension_plan(root).cases
    cases: list[AlterTriggerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterTriggerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTriggerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterTriggerRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: AlterTriggerRuntimeCase,
        sql_path: Path,
    ) -> AlterTriggerCaseRuntimeResult:
        ...
        return AlterTriggerCaseRuntimeResult(
...
__all__ = [
    "AlterTriggerCaseRuntimeResult",
    "AlterTriggerPg18Runner",
    "AlterTriggerRuntimeCase",
    "AlterTriggerRuntimeError",
    "AlterTriggerSuiteRun",
    "AlterTriggerTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_trigger_runtime_case_set",
    "compare_alter_trigger_runs",
]
