"""Actual-byte coverage validation for MERGE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 52-case baseline +
...
from .merge_factor_render import (
    count_primary_merge,
...
class MergeFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "merge_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_merge_factor_witness(
        case, repository_root
    )
    expected_sql = render_merge_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_merge(sql) != 1:
...
def validate_merge_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return MergeFactorProgramValidation(
        passed=not issues,
