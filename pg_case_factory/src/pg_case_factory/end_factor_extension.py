"""Bounded post-coverage cross-factor extension expander for END.

The marginal factor-value-loop (:mod:`end_factor_loop`) is the required
...
``end.yaml`` ``post_coverage_extension_policy.enabled: true``:
cross-factor combinations of the positive T1-T2 behaviour axes across
the single END synopsis branch, with at most one failure-causing value
per case so attribution stays clean, and ``verification_mode`` crossed so
every declared T6 value is exercised.

END is a TCL transaction-control statement with one synopsis branch.
...
class EndFactorExtensionError(ValueError):
    """Raised when a frozen END extension input drifts."""


@dataclass(frozen=True)
class EndFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class EndFactorExtensionPlan:
    cases: tuple[EndFactorExtensionCase, ...]
...
# General axes crossed for the single END branch.
...
# target_action are overridden per branch.  END's success default for
...
# attribution.
#
# END: ``and_chain`` / ``and_no_chain`` are VALID syntax (not negatives).
        ...
        entries.append(
            {
                ...
                "title": (
                    f"END extension {case.ordinal:05d}: "
...
def build_end_factor_extension_plan(
    repository_root: Path,
) -> EndFactorExtensionPlan:
    """Build the bounded END post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_end_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[EndFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    EndFactorExtensionCase(
                        ...
                        sql_filename=(
                            f"END{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"end_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"END-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "end_required_factor_value_matrix"
                        ),
                        derivation_reason=(
...
__all__ = [
    "EndFactorExtensionError",
    "EndFactorExtensionCase",
    "EndFactorExtensionPlan",
    "build_end_factor_extension_plan",
]
