"""Bounded PostgreSQL 18.4 runtime for DROP SCHEMA factor programs."""
...
from .drop_schema_factor_loop import DropSchemaFactorCase
...
class DropSchemaRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropSchemaCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropSchemaSuiteRun:
    run_ordinal: int
    cases: tuple[DropSchemaCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropSchemaRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_schema_pg18_suite_run",
...
@dataclass(frozen=True)
class DropSchemaTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_schema_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_schema_runs(
    run_01: DropSchemaSuiteRun,
    run_02: DropSchemaSuiteRun,
) -> DropSchemaTwoRunComparison:
    ...
    return DropSchemaTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropSchemaRuntimeCase(Protocol):
...
def build_drop_schema_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropSchemaRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_schema_factor_loop_plan(root).cases
    extensions = build_drop_schema_factor_extension_plan(root).cases
    cases: list[DropSchemaRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropSchemaRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropSchemaPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropSchemaRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: DropSchemaRuntimeCase,
        sql_path: Path,
    ) -> DropSchemaCaseRuntimeResult:
        ...
        return DropSchemaCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropSchemaRuntimeCase],
        ...
        *,
        ...
        results: list[DropSchemaCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropSchemaCaseRuntimeResult",
    "DropSchemaPg18Runner",
    "DropSchemaRuntimeCase",
    "DropSchemaRuntimeError",
    "DropSchemaSuiteRun",
    "DropSchemaTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_schema_runtime_case_set",
    "compare_drop_schema_runs",
]
