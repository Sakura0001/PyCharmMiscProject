"""Actual-byte coverage validation for REINDEX factor-loop SQL."""
...
from .reindex_factor_render import (
...
class ReindexFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "reindex_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_reindex_factor_witness(case, repository_root)
    expected_sql = render_reindex_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_reindex(sql) != 1:
...
def validate_reindex_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return ReindexFactorProgramValidation(
        passed=not issues,
