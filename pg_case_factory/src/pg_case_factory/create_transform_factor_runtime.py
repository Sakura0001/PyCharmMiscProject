"""Bounded PostgreSQL 18.4 runtime for CREATE TRANSFORM factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-ctr-sock-20260820")
PG18_PORT = 55616
PG18_DATABASE = "pgcf_ctr"
...
class CreateTransformRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateTransformCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateTransformSuiteRun:
    run_ordinal: int
    cases: tuple[CreateTransformCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateTransformRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_transform_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateTransformTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_transform_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_create_transform_runs(
    run_01: CreateTransformSuiteRun,
    run_02: CreateTransformSuiteRun,
) -> CreateTransformTwoRunComparison:
    ...
    return CreateTransformTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateTransformRuntimeCase(Protocol):
...
def build_create_transform_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateTransformRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_transform_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_transform_factor_extension_plan(root).cases
    )
    cases: list[CreateTransformRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateTransformRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateTransformPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_transform t "
            "JOIN pg_catalog.pg_type ty ON t.trftype = ty.oid "
            "WHERE ty.typname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateTransformRuntimeCase,
        sql_path: Path,
    ) -> CreateTransformCaseRuntimeResult:
        ...
        return CreateTransformCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[CreateTransformCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateTransformSuiteRun(
...
__all__ = [
    "CreateTransformCaseRuntimeResult",
    "CreateTransformPg18Runner",
    "CreateTransformRuntimeCase",
    "CreateTransformRuntimeError",
    "CreateTransformSuiteRun",
    "CreateTransformTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_transform_runtime_case_set",
    "compare_create_transform_runs",
]
