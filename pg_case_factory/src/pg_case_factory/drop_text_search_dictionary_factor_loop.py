"""Factor-value-loop obligation ledger for PostgreSQL 18.4 DROP TEXT SEARCH DICTIONARY.

This module compiles the marginal GRM/SFV/RISK obligation ledger for
``DROP TEXT SEARCH DICTIONARY``.  The statement has no ``INV`` block, so the
canonical SFV obligations are derived one-to-one from the shipped applicability
matrix: exactly 35 rows, one local obligation per row.
...
``DROP TEXT SEARCH DICTIONARY`` error that belongs to this statement.
"""
...
class DropTextSearchDictionaryFactorLoopError(ValueError):
    """Raised when a frozen DROP TEXT SEARCH DICTIONARY obligation input drifts."""


@dataclass(frozen=True)
class DropTextSearchDictionaryFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class DropTextSearchDictionaryFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DropTextSearchDictionaryFactorLoopPlan:
    obligations: tuple[DropTextSearchDictionaryFactorObligation, ...]
    cases: tuple[DropTextSearchDictionaryFactorCase, ...]
    delegated: tuple[DropTextSearchDictionaryFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-droptsdictionary.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-droptsdictionary"
...
_GRAMMAR_ACTIONS: tuple[_GrammarAction, ...] = (
    _GrammarAction(
        action_id="drop_text_search_dictionary",
        grammar_branch_id=_BRANCH_1,
        source_locator=f"{_DOC_SOURCE}:synopsis",
    ),
)

_REPRESENTATIVE_ACTION = "drop_text_search_dictionary"

# Every canonical factor maps to the single DROP TEXT SEARCH DICTIONARY consumer.
...
_STATEMENT_BRANCH_CONSUMER: dict[str, str] = {
    ...
    "branch_drop_ts_dict_if_exists": "drop_text_search_dictionary",
}

# Canonical (factor, value) pairs that reach a PostgreSQL target check and
# are rejected (best-effort attribution against PG 18.4).  Kept minimal
...
def _canonical_consumer(row) -> str:
    if row.factor == "statement_branch":
        try:
            ...
            raise DropTextSearchDictionaryFactorLoopError(
...
def _compile_grammar_obligations() -> list[DropTextSearchDictionaryFactorObligation]:
    rows: list[DropTextSearchDictionaryFactorObligation] = []
    for action in _GRAMMAR_ACTIONS:
        rows.append(
            DropTextSearchDictionaryFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPTEXTSEARCHDICTIONARY-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise DropTextSearchDictionaryFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations(
    repository_root: Path,
) -> list[DropTextSearchDictionaryFactorObligation]:
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("drop_text_search_dictionary")
    if len(catalog_rows) != 35:
        raise DropTextSearchDictionaryFactorLoopError("canonical obligation count drift")
    rows: list[DropTextSearchDictionaryFactorObligation] = []
    for row in catalog_rows:
        ...
        rows.append(
            DropTextSearchDictionaryFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"DROPTEXTSEARCHDICTIONARY-SFV|{row.row_id}|{consumer}"
                ),
                kind="SFV",
...
def _compile_risk_obligations() -> list[DropTextSearchDictionaryFactorObligation]:
    return [
        DropTextSearchDictionaryFactorObligation(
            ordinal=0,
            obligation_id=f"DROPTEXTSEARCHDICTIONARY-RISK|transaction|{value}",
            kind="RISK",
            ...
            disposition="covered",
            source_locator=(
                "postgresql-18.4-doc:sql-droptsdictionary:transactional-ddl"
            ),
        )
...
def _obligation_multiset_sha256(
    rows: tuple[DropTextSearchDictionaryFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"drop-text-search-dictionary-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
_SFV_FAILURE_SQLSTATE: dict[tuple[str, str], tuple[str, str]] = {
    ("expected_status", "failure"): (
        "42704",
        "drop_ts_dict_declared_failure",
    ),
    ("authorization_path", "non_owner"): (
        "42501",
        "insufficient_dict_privilege",
    ),
    ("privilege_context", "non_owner_session"): (
        "42501",
        "insufficient_dict_privilege",
    ...
    ("object_state", "absent"): (
        "42704",
        "undefined_ts_dict_absent",
    ),
    ("dict_name_shape", "non_existent_name"): (
        "42704",
        "undefined_ts_dict_name",
    ),
    ("dependency_context", "config_using_dict"): (
        "2BP01",
    ...
    ("error_type", "non_existent_without_if_exists"): (
        "42704",
        "undefined_ts_dict_no_if_exists",
    ),
    ("error_type", "dependent_object_exists"): (
        "2BP01",
        "dependent_objects_restricted",
...
def _expected_failure_details(
    obligation: DropTextSearchDictionaryFactorObligation,
) -> tuple[str, str]:
    try:
...
# Baseline primaries whose target dictionary is intentionally absent, so the
...
# dictionary surfaces a hard error rather than an IF EXISTS notice.
...
def _baseline_assignments(
    ...
    if (obligation.factor_key, obligation.value) in _ABSENT_DICT_PRIMARIES:
        assignments["if_exists_clause"] = "absent"
    if obligation.factor_key == "statement_branch":
        if obligation.value == "branch_drop_ts_dict_if_exists":
            assignments["if_exists_clause"] = "present"
    if obligation.factor_key == "dependency_context":
        if obligation.value == "config_using_dict":
            assignments["cascade_restrict"] = "explicit_restrict"
            assignments["dependency_status"] = "has_config_dependencies"
    if obligation.factor_key == "dependency_status":
        if obligation.value == "has_config_dependencies":
            assignments["dependency_context"] = "config_using_dict"
            assignments["cascade_restrict"] = "explicit_restrict"
    if obligation.factor_key == "privilege_context":
        if obligation.value == "non_owner_session":
    ...
    if obligation.factor_key == "error_type":
        if obligation.value == "non_existent_without_if_exists":
            ...
            assignments["if_exists_clause"] = "absent"
            assignments["dict_name_shape"] = "non_existent_name"
        elif obligation.value == "dependent_object_exists":
            assignments["dependency_context"] = "config_using_dict"
            assignments["dependency_status"] = "has_config_dependencies"
            assignments["cascade_restrict"] = "explicit_restrict"
    ...
    if len(assignments) != len(set(assignments)):
        raise DropTextSearchDictionaryFactorLoopError("duplicate baseline factor key")
    return tuple(sorted(assignments.items()))


def build_drop_text_search_dictionary_factor_loop_plan(
    repository_root: Path,
) -> DropTextSearchDictionaryFactorLoopPlan:
    """Assign one stable local SQL program to every non-delegated obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_drop_text_search_dictionary_factor_loop_obligations(root)
    cases: list[DropTextSearchDictionaryFactorCase] = []
    delegated: list[DropTextSearchDictionaryFactorObligation] = []
    ...
    plan = DropTextSearchDictionaryFactorLoopPlan(
    ...
    if len(plan.cases) != 38 or len(plan.delegated) != 0:
        raise DropTextSearchDictionaryFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 38
    ):
        raise DropTextSearchDictionaryFactorLoopError("local obligation mapping drift")
    if len({row.sql_filename for row in plan.cases}) != 38:
        raise DropTextSearchDictionaryFactorLoopError("duplicate SQL filename")
    return plan


_PLAN_CACHE: dict[Path, DropTextSearchDictionaryFactorLoopPlan] = {}


def _build_drop_text_search_dictionary_factor_plan_lazily(
    repository_root: Path,
) -> DropTextSearchDictionaryFactorLoopPlan:
    ...
    if root not in _PLAN_CACHE:
        _PLAN_CACHE[root] = build_drop_text_search_dictionary_factor_loop_plan(root)
    return _PLAN_CACHE[root]


def compile_drop_text_search_dictionary_factor_loop_obligations(
    repository_root: Path,
) -> tuple[DropTextSearchDictionaryFactorObligation, ...]:
    ...
    if len(rows) != 38:
        raise DropTextSearchDictionaryFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise DropTextSearchDictionaryFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 35, "RISK": 2}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise DropTextSearchDictionaryFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise DropTextSearchDictionaryFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 38:
        raise DropTextSearchDictionaryFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise DropTextSearchDictionaryFactorLoopError("unknown obligation disposition")
    return rows


__all__ = [
    "DropTextSearchDictionaryFactorLoopError",
    "DropTextSearchDictionaryFactorObligation",
    "DropTextSearchDictionaryFactorCase",
    "DropTextSearchDictionaryFactorLoopPlan",
    "compile_drop_text_search_dictionary_factor_loop_obligations",
    "build_drop_text_search_dictionary_factor_loop_plan",
    "_obligation_multiset_sha256",
    "_build_drop_text_search_dictionary_factor_plan_lazily",
