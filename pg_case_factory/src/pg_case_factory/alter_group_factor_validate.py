"""Actual-byte coverage validation for ALTER GROUP factor-loop SQL.

Re-renders the canonical bytes for every case (frozen baseline +
...
cardinality != 1, ``{placeholder}`` leakage, missing/duplicate/unknown
cases, or an extension case lacking a derivation record.  Also gates that
every required factor value is still witnessed across
``baseline ∪ extension``.
"""
...
from .alter_group_factor_render import (
...
class AlterGroupFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_group_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    rc = AlterGroupRenderCase.from_case(case)
    reasons: list[str] = []
    witness = resolve_alter_group_factor_witness(case, repository_root)
    expected_sql = render_alter_group_factor_case(case, repository_root)
    expected_target = _target_region(expected_sql)
    actual_target = _target_region(sql)
    if "{" in sql or "}" in sql:
        reasons.append("placeholder_leakage")
    if _single_header_value(sql, "case_id") != rc.case_id:
        reasons.append("case_id_header_mismatch")
    if _single_header_value(sql, "primary_obligation_id") != rc.primary_identifier:
        reasons.append("primary_obligation_header_mismatch")
    if _single_header_value(sql, "expected_outcome") != rc.outcome:
        reasons.append("expected_outcome_header_mismatch")
    if _single_header_value(sql, "expected_sqlstate") != rc.expected_sqlstate:
        reasons.append("expected_sqlstate_header_mismatch")
    if actual_target is None or count_primary_alter_group(sql) != 1:
    ...
    if rc.is_extension and not _has_derivation_record(case):
        reasons.append("extension_derivation_record_missing")
    unique = tuple(dict.fromkeys(reasons))
    record = {
        "case_id": rc.case_id,
        "sql_filename": rc.sql_filename,
        "primary_obligation_id": rc.primary_identifier,
        "is_extension": rc.is_extension,
        "consumer_action_id": rc.consumer_action_id,
        "semantic_locus": witness.semantic_locus,
        "expected_outcome": rc.outcome,
...
def validate_alter_group_factor_programs(
    ...
    *,
    ...
    missing = tuple(
        cid for cid in expected_ids if actual_counter[cid] == 0
    )
    duplicate = tuple(
    ...
    return AlterGroupFactorProgramValidation(
        passed=not issues,
