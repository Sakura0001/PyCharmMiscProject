"""Bounded PostgreSQL 18.4 runtime for CREATE MATERIALIZED VIEW factor programs.

This module provides the serial psql executor and two-run comparison
harness for CREATE MATERIALIZED VIEW factor-loop SQL.  The runner is
never invoked during the no-DB phase; it is provided so the DB phase can
execute the same frozen SQL files and verify the provisional SQLSTATEs.

CREATE MATERIALIZED VIEW creates ``pg_class`` rows (``relkind='m'``),
also visible in ``pg_catalog.pg_matviews``, so the clean-state probe
inspects ``pg_matviews``, ``pg_class``, ``pg_proc``, and ``pg_roles``.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-cmatview-sock-20260820")
PG18_PORT = 55605
PG18_DATABASE = "pgcf_cmatview"
...
class CreateMaterializedViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateMaterializedViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateMaterializedViewSuiteRun:
    run_ordinal: int
    cases: tuple[CreateMaterializedViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateMaterializedViewRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_materialized_view_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateMaterializedViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_materialized_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_materialized_view_runs(
    run_01: CreateMaterializedViewSuiteRun,
    run_02: CreateMaterializedViewSuiteRun,
    ...
    return CreateMaterializedViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateMaterializedViewRuntimeCase(Protocol):
...
def build_create_materialized_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateMaterializedViewRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = build_create_materialized_view_factor_loop_plan(
        root
    ).cases
    extensions = (
        build_create_materialized_view_factor_extension_plan(root).cases
    )
    cases: list[CreateMaterializedViewRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateMaterializedViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateMaterializedViewPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_matviews WHERE matviewname LIKE '"
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_class WHERE relname LIKE '"
            + literal
            + "' "
    ...
    def _execute_case(
        self,
        case: CreateMaterializedViewRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[CreateMaterializedViewRuntimeCase],
        ...
        *,
        ...
        results: list[CreateMaterializedViewCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateMaterializedViewSuiteRun(
...
__all__ = [
    ...
    "CreateMaterializedViewRuntimeCase",
