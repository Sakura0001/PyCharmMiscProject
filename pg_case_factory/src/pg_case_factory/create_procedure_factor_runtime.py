"""Bounded PostgreSQL 18.4 runtime for CREATE PROCEDURE factor programs.
...
program against an isolated PostgreSQL 18.4 server (port 55602) and
...
from .create_procedure_factor_loop import CreateProcedureFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-cproc-sock-20260820")
PG18_PORT = 55602
PG18_DATABASE = "pgcf_cproc"
...
class CreateProcedureRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateProcedureCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateProcedureSuiteRun:
    run_ordinal: int
    cases: tuple[CreateProcedureCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateProcedureRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_procedure_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateProcedureTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_procedure_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_procedure_runs(
    run_01: CreateProcedureSuiteRun,
    run_02: CreateProcedureSuiteRun,
) -> CreateProcedureTwoRunComparison:
    ...
    return CreateProcedureTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateProcedureRuntimeCase(Protocol):
...
def build_create_procedure_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateProcedureRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_procedure_factor_loop_plan(root).cases
    extensions = build_create_procedure_factor_extension_plan(root).cases
    cases: list[CreateProcedureRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateProcedureRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateProcedurePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateProcedureRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateProcedureRuntimeCase,
        sql_path: Path,
    ) -> CreateProcedureCaseRuntimeResult:
        ...
        return CreateProcedureCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateProcedureRuntimeCase],
        ...
        *,
        ...
        results: list[CreateProcedureCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateProcedureSuiteRun(
