"""Actual-byte coverage validation for ALTER MATERIALIZED VIEW factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 61-case baseline +
bounded 8424-case extension) and compares them exactly against the on-disk
...
unknown cases, or an extension case lacking a derivation record.  Also gates
that every required factor value is still witnessed across
``baseline ∪ extension``.
"""
...
class AlterMaterializedViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_materialized_view_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_materialized_view_factor_witness(case, repository_root)
    expected_sql = render_alter_materialized_view_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_materialized_view(sql) != 1:
...
def validate_alter_materialized_view_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> AlterMaterializedViewFactorProgramValidation:
    ...
    return AlterMaterializedViewFactorProgramValidation(
        passed=not issues,
