"""Bounded PostgreSQL 18.4 runtime for ALTER POLICY factor programs.

Serial, one-file-at-a-time ``psql`` executor against an isolated PG 18.4
cluster (port 55494, database ``pgcf_ap``).  Each case runs twice; the
...
``pg_catalog`` relations an ALTER POLICY case can leave behind
(``pg_policy`` for policies, ``pg_class`` for tables, ``pg_roles`` for the
actor/owner/role-target roles, plus the shared namespace/proc/type/
extension safety net).

The runtime test is skip-gated: when the cluster is down the suite reports
a skip rather than failing, mirroring the sibling statement runtimes.
"""
...
from .alter_policy_factor_loop import AlterPolicyFactorCase
...
class AlterPolicyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterPolicyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterPolicySuiteRun:
    run_ordinal: int
    cases: tuple[AlterPolicyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterPolicyRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_policy_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterPolicyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_policy_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_policy_runs(
    run_01: AlterPolicySuiteRun,
    run_02: AlterPolicySuiteRun,
) -> AlterPolicyTwoRunComparison:
    ...
    return AlterPolicyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterPolicyRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`AlterPolicyFactorCase`
...
def build_alter_policy_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterPolicyRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0072) precedes the
    ...
    from .alter_policy_factor_loop import build_alter_policy_factor_loop_plan

    root = Path(repository_root).resolve(strict=True)
    sql_dir = Path(sql_dir)
    baseline = build_alter_policy_factor_loop_plan(root).cases
    extensions = build_alter_policy_factor_extension_plan(root).cases
    cases: list[AlterPolicyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterPolicyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterPolicyPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterPolicyRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_policy WHERE polname LIKE '" + literal + "' "
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterPolicyRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterPolicyRuntimeCase,
        sql_path: Path,
    ) -> AlterPolicyCaseRuntimeResult:
        ...
        return AlterPolicyCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[AlterPolicyRuntimeCase],
        ...
        *,
        ...
        results: list[AlterPolicyCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "AlterPolicyCaseRuntimeResult",
    "AlterPolicyPg18Runner",
    "AlterPolicyRuntimeCase",
    "AlterPolicyRuntimeError",
    "AlterPolicySuiteRun",
    "AlterPolicyTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_policy_runtime_case_set",
    "compare_alter_policy_runs",
]
