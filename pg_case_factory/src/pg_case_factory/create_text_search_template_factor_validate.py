"""Actual-byte coverage validation for CREATE TEXT SEARCH TEMPLATE factor-loop SQL.
...
from .create_text_search_template_factor_render import (
...
class CreateTextSearchTemplateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_text_search_template_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_text_search_template_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_text_search_template(sql) != 1
    ):
...
def validate_create_text_search_template_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateTextSearchTemplateFactorProgramValidation:
    ...
    return CreateTextSearchTemplateFactorProgramValidation(
        passed=not issues,
