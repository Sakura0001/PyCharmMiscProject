...
def _resolve_case(case: DropTriggerFactorCase) -> _CasePlan:
    ...
    trigger_drop = (
        [f"DROP TRIGGER IF EXISTS {trigger_ref} ON {base_table};"]
        if host_kind != "none"
        else []
    )
    function_drop = (
    ...
    # Pre-cleanup: DROP TABLE first (bookend gate), then function/dep/role.
    pre_cleanup: list[str] = []
    pre_cleanup.extend(table_drop)
    pre_cleanup.extend(function_drop)
    pre_cleanup.extend(dep_drop)
    pre_cleanup.extend(role_drops)
    if not pre_cleanup:
        pre_cleanup.append("SELECT 1 AS residual_check_no_objects;")

    # Cleanup: RESET ROLE, then trigger/function/dep/role drops, then DROP
    # TABLE last (bookend gate).
    cleanup: list[str] = []
    if effective:
        cleanup.append("RESET ROLE;")
    cleanup.extend(trigger_drop)
    cleanup.extend(function_drop)
