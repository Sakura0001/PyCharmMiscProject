"""Bounded PostgreSQL 18.4 runtime for CALL factor programs.
...
from .call_factor_loop import CallFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-call-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_call"
...
class CallSuiteRun:
    run_ordinal: int
    cases: tuple[CallCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CallRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "call_pg18_suite_run",
...
class CallTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "call_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_call_runs(
    run_01: CallSuiteRun,
    run_02: CallSuiteRun,
) -> CallTwoRunComparison:
...
class CallRuntimeCase(Protocol):
...
def build_call_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CallRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_call_factor_loop_plan(root).cases
    extensions = build_call_factor_extension_plan(root).cases
    cases: list[CallRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CallRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CallPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CallRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CallRuntimeCase,
        sql_path: Path,
    ) -> CallCaseRuntimeResult:
    ...
    def run_cases(
        self,
        cases: Sequence[CallRuntimeCase],
        ...
        *,
        ...
        results: list[CallCaseRuntimeResult] = []
        for case in cases:
