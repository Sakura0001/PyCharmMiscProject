"""Bounded PostgreSQL 18.4 runtime for DROP TEXT SEARCH DICTIONARY factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-dtsd-sock-20260821")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dtsd"
...
class DropTextSearchDictionaryRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTextSearchDictionaryCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTextSearchDictionarySuiteRun:
    run_ordinal: int
    cases: tuple[DropTextSearchDictionaryCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTextSearchDictionaryRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_dictionary_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTextSearchDictionaryTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_dictionary_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_text_search_dictionary_runs(
    run_01: DropTextSearchDictionarySuiteRun,
    run_02: DropTextSearchDictionarySuiteRun,
) -> DropTextSearchDictionaryTwoRunComparison:
    ...
    return DropTextSearchDictionaryTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTextSearchDictionaryRuntimeCase(Protocol):
...
def build_drop_text_search_dictionary_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTextSearchDictionaryRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_text_search_dictionary_factor_loop_plan(root).cases
    extensions = build_drop_text_search_dictionary_factor_extension_plan(root).cases
    cases: list[DropTextSearchDictionaryRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTextSearchDictionaryRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTextSearchDictionaryPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTextSearchDictionaryRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_ts_dict WHERE dictname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_ts_config WHERE cfgname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTextSearchDictionaryRuntimeCase,
        sql_path: Path,
    ) -> DropTextSearchDictionaryCaseRuntimeResult:
        ...
        return DropTextSearchDictionaryCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropTextSearchDictionaryRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> DropTextSearchDictionarySuiteRun:
        self.verify_server()
        selected = (
            set(selected_case_ids) if selected_case_ids is not None else None
        )
        results: list[DropTextSearchDictionaryCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropTextSearchDictionarySuiteRun(
...
__all__ = [
    "DropTextSearchDictionaryCaseRuntimeResult",
    "DropTextSearchDictionaryPg18Runner",
    "DropTextSearchDictionaryRuntimeCase",
    "DropTextSearchDictionaryRuntimeError",
    "DropTextSearchDictionarySuiteRun",
    "DropTextSearchDictionaryTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_text_search_dictionary_runtime_case_set",
    "compare_drop_text_search_dictionary_runs",
]
