"""Bounded PostgreSQL 18.4 runtime for ALTER OPERATOR CLASS factor programs.

Serial, deterministic two-run executor mirroring the alter_large_object runtime
contract.  Each generated ``.sql`` program is run twice on the isolated
``alter_operator_class`` cluster (port 55493) and the structured projections are
...
class AlterOperatorClassSuiteRun:
    run_ordinal: int
    cases: tuple[AlterOperatorClassCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterOperatorClassRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_operator_class_pg18_suite_run",
...
class AlterOperatorClassTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_operator_class_pg18_two_run_comparison",
            "passed": self.passed,
...
class AlterOperatorClassRuntimeCase(Protocol):
    ...
    Satisfied by both the frozen baseline case and the bounded extension case;
    the runner only reads the identity + expected-state attributes.
    """
...
def build_alter_operator_class_runtime_case_set(
    ...
    sql_dir: Path,
) -> tuple[tuple[AlterOperatorClassRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline precedes the extension in a
    stable, human-auditable order.
    """
    ...
    baseline = build_alter_operator_class_factor_loop_plan(root).cases
    extensions = build_alter_operator_class_factor_extension_plan(root).cases
    cases: list[AlterOperatorClassRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterOperatorClassRuntimeCase) -> int:
...
class AlterOperatorClassPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterOperatorClassRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterOperatorClassRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: AlterOperatorClassRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[AlterOperatorClassRuntimeCase],
        ...
        *,
        ...
        results: list[AlterOperatorClassCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterOperatorClassSuiteRun(
