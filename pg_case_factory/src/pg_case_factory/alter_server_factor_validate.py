"""Actual-byte coverage validation for ALTER SERVER factor-loop SQL.
...
class AlterServerFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_server_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_server_factor_witness(case, repository_root)
    expected_sql = render_alter_server_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_server(sql) != 1:
...
def validate_alter_server_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterServerFactorProgramValidation(
        passed=not issues,
