"""Bounded post-coverage extension plan for CREATE ROLE factor regress.

The baseline :mod:`create_role_factor_loop` assigns one local SQL program
...
class CreateRoleFactorExtensionError(ValueError):
    """Raised when CREATE ROLE extension input drifts."""


@dataclass(frozen=True)
class CreateRoleFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class CreateRoleFactorExtensionPlan:
    cases: tuple[CreateRoleFactorExtensionCase, ...]
...
_GENERAL_AXES: dict[str, tuple[str, ...]] = {
    "role_identity": ("not_exists", "exists"),
    "executor_privilege": (
        "superuser",
...
_BEHAVIOR_AXES: dict[str, tuple[str, ...]] = {
    ...
    "with_keyword": ("present", "absent"),
    "connection_limit_clause": ("omitted", "positive_limit"),
}
...
_COMBINATION_GROUP = "create_role_required_factor_value_matrix"

_CONSUMER_ACTION = "create_role_branch_1"
...
def _derive_t5_factors(a: dict[str, str]) -> None:
    ...
    if a.get("role_identity") == "exists":
        ...
        a["duplicate_role_name"] = "none"

    if a.get("executor_privilege") == "normal_user_no_createrole":
        a["privilege_insufficient"] = "no_createrole_privilege"
    else:
        a["privilege_insufficient"] = "none"

    if a.get("executor_privilege") == "superuser":
        if a.get("role_attribute") in (
            "superuser",
            "replication",
            "bypassrls",
        ):
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"create-role-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
...
def _present_failure_pair(
    ...
    if a.get("role_identity") == "exists":
        return ("role_identity", "exists")
    if a.get("executor_privilege") == "normal_user_no_createrole":
        return ("executor_privilege", "normal_user_no_createrole")
    return None


def build_create_role_factor_extension_plan(
    repository_root: Path,
) -> CreateRoleFactorExtensionPlan:
    ...
    if len(catalog_rows) != 101:
        raise CreateRoleFactorExtensionError("catalog row count drift")

    cases: list[CreateRoleFactorExtensionCase] = []
    raw_count = 0
    ...
    plan = CreateRoleFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if len(plan.cases) < 1:
        raise CreateRoleFactorExtensionError(
    ...
    if ordinals != list(
        ...
        raise CreateRoleFactorExtensionError("extension ordinal gap")
    ...
    if not all(
        ...
        raise CreateRoleFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("CRP-EXT|") for case in plan.cases
    ):
        raise CreateRoleFactorExtensionError(
...
__all__ = [
    "CreateRoleFactorExtensionError",
    "CreateRoleFactorExtensionCase",
    "CreateRoleFactorExtensionPlan",
