"""Bounded PostgreSQL 18.4 runtime for DROP TRIGGER factor programs."""
...
from .drop_trigger_factor_loop import DropTriggerFactorCase
...
class DropTriggerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropTriggerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropTriggerSuiteRun:
    run_ordinal: int
    cases: tuple[DropTriggerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropTriggerRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_trigger_pg18_suite_run",
...
@dataclass(frozen=True)
class DropTriggerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_trigger_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_trigger_runs(
    run_01: DropTriggerSuiteRun,
    run_02: DropTriggerSuiteRun,
) -> DropTriggerTwoRunComparison:
    ...
    return DropTriggerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropTriggerRuntimeCase(Protocol):
...
def build_drop_trigger_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropTriggerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_trigger_factor_loop_plan(root).cases
    extensions = build_drop_trigger_factor_extension_plan(root).cases
    cases: list[DropTriggerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropTriggerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropTriggerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropTriggerRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_trigger WHERE tgname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropTriggerRuntimeCase,
        sql_path: Path,
    ) -> DropTriggerCaseRuntimeResult:
        ...
        return DropTriggerCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropTriggerRuntimeCase],
        ...
        *,
        ...
        results: list[DropTriggerCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "DropTriggerCaseRuntimeResult",
    "DropTriggerPg18Runner",
    "DropTriggerRuntimeCase",
    "DropTriggerRuntimeError",
    "DropTriggerSuiteRun",
    "DropTriggerTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_trigger_runtime_case_set",
    "compare_drop_trigger_runs",
]
