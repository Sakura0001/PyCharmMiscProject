"""Actual-byte coverage validation for CREATE MATERIALIZED VIEW factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 48-case baseline +
bounded extension cases) and compares them exactly against the on-disk
SQL.  Fails closed on any drift: byte mismatch, header mismatch,
...
class CreateMaterializedViewFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_materialized_view_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_materialized_view_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_materialized_view_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_materialized_view(sql) != 1
    ):
...
def validate_create_materialized_view_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateMaterializedViewFactorProgramValidation:
    ...
    duplicate = tuple(
        sorted(
            cid for cid, count in actual_counter.items() if count > 1
        )
    )
    ...
    return CreateMaterializedViewFactorProgramValidation(
        passed=not issues,
