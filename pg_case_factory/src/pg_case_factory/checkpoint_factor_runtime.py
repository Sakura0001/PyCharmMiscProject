"""Bounded PostgreSQL 18.4 runtime for CHECKPOINT factor programs."""
...
from .checkpoint_factor_loop import CheckpointFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ckpt-sock-20260820")
PG18_PORT = 55499
PG18_DATABASE = "pgcf_ckpt"
...
class CheckpointCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CheckpointSuiteRun:
    run_ordinal: int
    cases: tuple[CheckpointCaseRuntimeResult, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "checkpoint_pg18_suite_run",
...
@dataclass(frozen=True)
class CheckpointTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "checkpoint_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_checkpoint_runs(
    run_01: CheckpointSuiteRun,
    run_02: CheckpointSuiteRun,
) -> CheckpointTwoRunComparison:
    ...
    return CheckpointTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CheckpointRuntimeCase(Protocol):
...
def build_checkpoint_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CheckpointRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_checkpoint_factor_loop_plan(root).cases
    extensions = build_checkpoint_factor_extension_plan(root).cases
    cases: list[CheckpointRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CheckpointRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CheckpointPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CheckpointRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CheckpointRuntimeCase,
        sql_path: Path,
    ) -> CheckpointCaseRuntimeResult:
        ...
        return CheckpointCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CheckpointRuntimeCase],
        ...
        *,
        ...
        results: list[CheckpointCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    "CheckpointCaseRuntimeResult",
    "CheckpointPg18Runner",
    "CheckpointRuntimeCase",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_checkpoint_runtime_case_set",
