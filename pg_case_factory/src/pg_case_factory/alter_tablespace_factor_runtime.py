"""Bounded PostgreSQL 18.4 runtime for ALTER TABLESPACE factor programs."""
...
from .alter_tablespace_factor_loop import AlterTablespaceFactorCase
...
class AlterTablespaceRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterTablespaceCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterTablespaceSuiteRun:
    run_ordinal: int
    cases: tuple[AlterTablespaceCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterTablespaceRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_tablespace_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterTablespaceTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_tablespace_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_tablespace_runs(
    run_01: AlterTablespaceSuiteRun,
    run_02: AlterTablespaceSuiteRun,
) -> AlterTablespaceTwoRunComparison:
    ...
    return AlterTablespaceTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterTablespaceRuntimeCase(Protocol):
...
def build_alter_tablespace_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterTablespaceRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_tablespace_factor_loop_plan(root).cases
    extensions = build_alter_tablespace_factor_extension_plan(root).cases
    cases: list[AlterTablespaceRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterTablespaceRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterTablespacePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterTablespaceRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: AlterTablespaceRuntimeCase,
        sql_path: Path,
    ) -> AlterTablespaceCaseRuntimeResult:
        ...
        return AlterTablespaceCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return AlterTablespaceSuiteRun(
...
__all__ = [
    "AlterTablespaceCaseRuntimeResult",
    "AlterTablespacePg18Runner",
    "AlterTablespaceRuntimeCase",
    "AlterTablespaceRuntimeError",
    "AlterTablespaceSuiteRun",
    "AlterTablespaceTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_tablespace_runtime_case_set",
    "compare_alter_tablespace_runs",
]
