"""Actual-byte coverage validation for DROP ROUTINE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen baseline + bounded
extension) and compares them exactly against the on-disk SQL.  Fails
...
from .drop_routine_factor_render import (
...
class DropRoutineFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_routine_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_routine_factor_witness(case, repository_root)
    expected_sql = render_drop_routine_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_drop_routine(sql) != 1:
...
def validate_drop_routine_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropRoutineFactorProgramValidation(
        passed=not issues,
