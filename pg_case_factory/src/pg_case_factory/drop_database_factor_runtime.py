"""Bounded PostgreSQL 18.4 runtime for DROP DATABASE factor programs."""
...
from .drop_database_factor_loop import DropDatabaseFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-ddb-sock-20260820")
PG18_PORT = 55497
PG18_DATABASE = "pgcf_ddb"
...
class DropDatabaseRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropDatabaseCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropDatabaseSuiteRun:
    run_ordinal: int
    cases: tuple[DropDatabaseCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropDatabaseRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_database_pg18_suite_run",
...
@dataclass(frozen=True)
class DropDatabaseTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_database_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_database_runs(
    run_01: DropDatabaseSuiteRun,
    run_02: DropDatabaseSuiteRun,
) -> DropDatabaseTwoRunComparison:
    ...
    return DropDatabaseTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropDatabaseRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline
    :class:`DropDatabaseFactorCase` and the bounded extension case; the
...
def build_drop_database_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropDatabaseRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0050) precedes the
    extension (0051-0482) in a stable, human-auditable order.
    """
    ...
    baseline = build_drop_database_factor_loop_plan(root).cases
    extensions = build_drop_database_factor_extension_plan(root).cases
    cases: list[DropDatabaseRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropDatabaseRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropDatabasePg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropDatabaseRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropDatabaseRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_database WHERE datname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropDatabaseRuntimeCase,
        sql_path: Path,
    ) -> DropDatabaseCaseRuntimeResult:
        ...
        return DropDatabaseCaseRuntimeResult(
...
__all__ = [
    "DropDatabaseCaseRuntimeResult",
    "DropDatabasePg18Runner",
    "DropDatabaseRuntimeCase",
    "DropDatabaseRuntimeError",
    "DropDatabaseSuiteRun",
    "DropDatabaseTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_database_runtime_case_set",
    "compare_drop_database_runs",
]
