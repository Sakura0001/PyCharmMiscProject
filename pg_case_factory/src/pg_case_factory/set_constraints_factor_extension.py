"""Bounded post-coverage extension plan for SET CONSTRAINTS factor regress.

The baseline :mod:`set_constraints_factor_loop` assigns one local SQL
program to every ``SFV``/``GRM`` obligation (marginal 1:1, 42 local
...
SET CONSTRAINTS is a session-scoped statement that creates no tables, so
the bookend (``DROP TABLE IF EXISTS``) is never emitted.  The
transaction-mode runtime state is the semantic witness target.  ``SET
CONSTRAINTS`` inside a transaction block is the only reachable failure
surface (SQLSTATE ``25001``); no SET CONSTRAINTS variant is
...
class SetConstraintsFactorExtensionError(ValueError):
    """Raised when SET CONSTRAINTS extension input drifts."""


@dataclass(frozen=True)
class SetConstraintsFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SetConstraintsFactorExtensionPlan:
    cases: tuple[SetConstraintsFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "set_constraints_required_factor_value_matrix"

_CONSUMER_ACTION = "set_constraints"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"set_constraints-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
...
def build_set_constraints_factor_extension_plan(
    repository_root: Path,
) -> SetConstraintsFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_set_constraints_factor_loop_plan(root)
    ...
    cases: list[SetConstraintsFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise SetConstraintsFactorExtensionError(
                    ...
                    derivation_id = (
                        f"SETCONSTRAINTS-EXT|{ordinal:05d}|"
                    ...
                    cases.append(
                        SetConstraintsFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"SETCONSTRAINTS{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"setconstraints_{ordinal:05d}_"
                            ),
    ...
    plan = SetConstraintsFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if not all(
        case.derivation_id.startswith("SETCONSTRAINTS-EXT|")
        for case in plan.cases
    ):
        raise SetConstraintsFactorExtensionError(
...
__all__ = [
    "SetConstraintsFactorExtensionError",
    "SetConstraintsFactorExtensionCase",
    "SetConstraintsFactorExtensionPlan",
    "build_set_constraints_factor_extension_plan",
    "_BASELINE_COUNT",
    "_CAP",
    "_VERIFICATION_MODES",
    "_CLEANUP_MODES",
