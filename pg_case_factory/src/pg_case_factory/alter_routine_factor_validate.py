"""Actual-byte coverage validation for ALTER ROUTINE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 115-case baseline +
...
class AlterRoutineFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_routine_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_routine_factor_witness(case, repository_root)
    expected_sql = render_alter_routine_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_routine(sql) != 1:
    ...
    sqlstate_oracle = (
        ...
        f"'{expected_sqlstate}' AS target_sqlstate_matches_expected;"
    )
...
def validate_alter_routine_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterRoutineFactorProgramValidation(
        passed=not issues,
