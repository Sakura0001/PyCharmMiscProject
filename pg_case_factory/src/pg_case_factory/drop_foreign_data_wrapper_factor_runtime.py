"""Bounded PostgreSQL 18.4 runtime for DROP FOREIGN DATA WRAPPER factor programs."""
...
from .drop_foreign_data_wrapper_factor_loop import DropForeignDataWrapperFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dfdw-sock-20260820")
PG18_PORT = 55503
PG18_DATABASE = "pgcf_dfdw"
...
class DropForeignDataWrapperRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropForeignDataWrapperCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropForeignDataWrapperSuiteRun:
    run_ordinal: int
    cases: tuple[DropForeignDataWrapperCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropForeignDataWrapperRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_foreign_data_wrapper_pg18_suite_run",
...
@dataclass(frozen=True)
class DropForeignDataWrapperTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_foreign_data_wrapper_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_foreign_data_wrapper_runs(
    run_01: DropForeignDataWrapperSuiteRun,
    run_02: DropForeignDataWrapperSuiteRun,
) -> DropForeignDataWrapperTwoRunComparison:
    ...
    return DropForeignDataWrapperTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropForeignDataWrapperRuntimeCase(Protocol):
...
def build_drop_foreign_data_wrapper_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropForeignDataWrapperRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file."""

    from .drop_foreign_data_wrapper_factor_extension import (
        build_drop_foreign_data_wrapper_factor_extension_plan,
    )
    ...
    baseline = build_drop_foreign_data_wrapper_factor_loop_plan(root).cases
    extensions = build_drop_foreign_data_wrapper_factor_extension_plan(root).cases
    cases: list[DropForeignDataWrapperRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropForeignDataWrapperRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropForeignDataWrapperPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropForeignDataWrapperRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropForeignDataWrapperRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_foreign_data_wrapper WHERE fdwname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_foreign_server WHERE srvname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropForeignDataWrapperRuntimeCase,
        sql_path: Path,
    ) -> DropForeignDataWrapperCaseRuntimeResult:
        ...
        return DropForeignDataWrapperCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropForeignDataWrapperRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> DropForeignDataWrapperSuiteRun:
        self.verify_server()
        selected = (
            set(selected_case_ids) if selected_case_ids is not None else None
        )
        results: list[DropForeignDataWrapperCaseRuntimeResult] = []
        for case in cases:
        ...
        return DropForeignDataWrapperSuiteRun(
...
__all__ = [
    "DropForeignDataWrapperCaseRuntimeResult",
    "DropForeignDataWrapperPg18Runner",
    "DropForeignDataWrapperRuntimeCase",
    "DropForeignDataWrapperRuntimeError",
    "DropForeignDataWrapperSuiteRun",
    "DropForeignDataWrapperTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_foreign_data_wrapper_runtime_case_set",
    "compare_drop_foreign_data_wrapper_runs",
]
