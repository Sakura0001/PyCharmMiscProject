"""Factor-value-loop obligation ledger for PostgreSQL 18.4 SHOW.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``SHOW`` — display the value of a run-time parameter.  The official
synopsis (``SHOW configuration_parameter`` / ``SHOW ALL``) has exactly
one branch, so there is a single ``GRM`` obligation.  The 43 canonical
``SFV`` factor-value pairs are hardcoded verbatim from the canonical
combination matrix ``show.yaml`` (the package binds directly to the
matrix, which is the source of truth); ``factor_value_count == 43``.

SHOW is a session-scoped, read-only display statement with no privilege
restriction: any role may execute any variant and SHOW is observable
inside a transaction block.  Mirroring the GUC-family precedent (RESET,
SET), the declared meta-failure ``expected_status=failure`` and the two
in-transaction ``transaction_visibility`` values are modelled as the
reachable failure surface (provisional SQLSTATE ``25001``; the DB phase
verifies on PG 18.4).  The remaining 40 ``SFV`` values plus the single
``GRM`` form are success-covered.  The two PG18-specific ``name_shape``
values ``lc_collate`` / ``lc_ctype`` render ``SHOW LC_COLLATE`` /
``SHOW LC_CTYPE`` and are witnessed via ``pg_catalog.pg_settings``.
...
class ShowFactorLoopError(ValueError):
    """Raised when a frozen SHOW obligation input drifts."""


@dataclass(frozen=True)
class ShowGrammarAction:
    """One official target action form of the SHOW synopsis."""
...
@dataclass(frozen=True)
class ShowFactorLoopPlan:
    obligations: tuple[ShowFactorObligation, ...]
    cases: tuple[ShowFactorCase, ...]
    delegated: tuple[ShowFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-show.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-show"

# The single representative action for all canonical factors.
_REPRESENTATIVE_ACTION = "show"

_STATEMENT_KEY = "show"

_MATRIX_PATH = (
    "skills/pg-sql-generation/references/combinations/session/"
    "runtime_parameter/show.yaml"
)
_REFERENCE_PATH = (
    "references/statements/session/runtime_parameter/show.md"
)
...
# SHOW has only one synopsis form, every factor observes on the single
# ``show`` action.
...
# Canonical (factor, value) pairs hardcoded verbatim from show.yaml.
...
# Mirrors the GUC-family (RESET/SET) transaction-visibility failure model.
...
def _load_grammar_actions() -> tuple[ShowGrammarAction, ...]:
    """Freeze every SHOW synopsis target action."""
    ...
    if len(actions) != 1:
        raise ShowFactorLoopError("show action count drift")
...
def _compile_grammar_obligations() -> list[ShowFactorObligation]:
    rows: list[ShowFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            ShowFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"SHOW-GRM|{action.grammar_branch_id}|"
    ...
    if len(rows) != 1:
        raise ShowFactorLoopError("grammar obligation count drift")
    return rows


def _compile_canonical_obligations() -> list[ShowFactorObligation]:
    """Hardcoded SFV obligations verbatim from show.yaml."""

    if _FACTOR_VALUE_COUNT != 43:
        raise ShowFactorLoopError("canonical factor value count drift")
    rows: list[ShowFactorObligation] = []
    for factor, value in _CANONICAL_FACTOR_VALUES:
        ...
        rows.append(
            ShowFactorObligation(
                ordinal=0,
                obligation_id=f"SHOW-SFV|{row_id}|{consumer}",
                kind="SFV",
                factor_key=factor,
                value=value,
...
def _obligation_multiset_sha256(
    rows: tuple[ShowFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(b"show-factor-obligations-v1\n")
    for row in rows:
        digest.update(
            json.dumps(
                {
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    Mirrors the GUC-family (RESET/SET) transaction-visibility failure
    model.  When ``expected_status=failure`` or a
...
def _baseline_assignments(
    obligation: ShowFactorObligation,
) -> tuple[tuple[str, str], ...]:
    ...
    if len(assignments) != len(set(assignments)):
        raise ShowFactorLoopError("duplicate baseline factor key")
...
def build_show_factor_loop_plan(
    repository_root: Path,
) -> ShowFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_show_factor_loop_obligations(root)
    cases: list[ShowFactorCase] = []
    delegated: list[ShowFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            ShowFactorCase(
                ordinal=ordinal,
                case_id=f"SHOW{ordinal:05d}",
                sql_filename=f"SHOW{ordinal:05d}.sql",
                object_prefix=f"show_{ordinal:05d}_",
    ...
    if len(plan.cases) != 44 or len(plan.delegated) != 0:
        raise ShowFactorLoopError("factor loop plan count drift")
    if (
        len({row.primary_obligation_id for row in plan.cases})
        != 44
    ):
        raise ShowFactorLoopError("local obligation mapping drift")
    if (
        len({row.sql_filename for row in plan.cases}) != 44
    ):
        raise ShowFactorLoopError("duplicate SQL filename")
    return plan


def compile_show_factor_loop_obligations(
    repository_root: Path,
) -> tuple[ShowFactorObligation, ...]:
    """Compile the exact marginal obligation ledger in frozen source order."""

    root = Path(repository_root).resolve(strict=True)
    if not (root / _MATRIX_PATH).is_file():
        raise ShowFactorLoopError(
            f"canonical matrix is missing for show: {_MATRIX_PATH}"
        )
    unordered_rows = (
    ...
    if len(rows) != 44:
        raise ShowFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise ShowFactorLoopError("duplicate obligation id")
    expected_kind_counts = {"GRM": 1, "SFV": 43}
    if Counter(row.kind for row in rows) != expected_kind_counts:
        raise ShowFactorLoopError("obligation kind count drift")
    if sum(row.disposition == "delegated" for row in rows) != 0:
        raise ShowFactorLoopError("delegated obligation count drift")
    if sum(
        row.disposition in {"covered", "expected_failure"}
        for row in rows
    ) != 44:
        raise ShowFactorLoopError("local obligation count drift")
    allowed_dispositions = {"covered", "expected_failure", "delegated"}
    if any(
        row.disposition not in allowed_dispositions for row in rows
    ):
        raise ShowFactorLoopError("unknown obligation disposition")
    return rows
