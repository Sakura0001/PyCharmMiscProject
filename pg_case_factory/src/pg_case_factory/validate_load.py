"""Actual-byte coverage validation for LOAD factor-loop SQL.
...
from .load_factor_render import (
    count_primary_load,
...
class LoadFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "load_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_load_factor_witness(
        case, repository_root
    )
    expected_sql = render_load_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_load(sql) != 1:
...
def validate_load_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return LoadFactorProgramValidation(
        passed=not issues,
