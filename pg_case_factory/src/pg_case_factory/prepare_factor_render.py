"""Render complete PostgreSQL 18.4 PREPARE factor-loop programs.
...
PREPARE fixtures ``CREATE TABLE`` (to ``PREPARE`` a
...
from .prepare_factor_extension import (
...
_DOC_SOURCE = (
    ...
    "prepared_statement/prepare.md"
)
_FACTOR_SOURCE = (
    "skills/pg-sql-generation/references/combinations/prepared/"
    "prepared_statement/prepare.yaml"
)
_AUTHOR = "codex"
...
class PrepareFactorRenderError(ValueError):
    """Raised when a PREPARE case cannot be rendered."""


@dataclass(frozen=True)
class PrepareFactorWitness:
...
def _synthetic_case(
    ext: PrepareFactorExtensionCase,
) -> PrepareFactorCase:
...
def _as_render_case(
    case: PrepareFactorCase | PrepareFactorExtensionCase,
) -> PrepareFactorCase:
    if isinstance(case, PrepareFactorExtensionCase):
        return _synthetic_case(case)
    return case


def _baseline(case: PrepareFactorCase) -> dict[str, str]:
...
def _stmt_name(a: dict[str, str], p: str) -> str:
    """The prepared statement name created by PREPARE."""
...
def _prepare_sql(a: dict[str, str], p: str) -> str:
    """The PREPARE target statement based on body and parameter_shape."""
...
def _build_target(a: dict[str, str], p: str) -> str:
    """The primary PREPARE statement (credited target)."""

    if _is_failure(a):
        return (
            f"PREPARE {p}nonexistent AS "
            f"SELECT * FROM {p}nonexistent_table;"
        )
    if _is_missing_name(a):
        return f"PREPARE  AS SELECT 1;"
    return _prepare_sql(a, p)
...
def _build_post_target_lifecycle(
    a: dict[str, str], p: str
) -> list[str]:
    """EXECUTE/DEALLOCATE after the PREPARE target for lifecycle tests."""
    ...
    if lifecycle == "execute_after_deallocate":
        if _is_all_form(a):
            return ["DEALLOCATE ALL;", f"EXECUTE {name};"]
        return [f"DEALLOCATE {name};", f"EXECUTE {name};"]
    if lifecycle == "deallocate_all":
        return ["DEALLOCATE ALL;"]
    return [f"EXECUTE {name};"]


def _probe_select(
    case: PrepareFactorCase,
    a: dict[str, str],
    p: str,
) -> str | None:
...
def _resolve_case(
    ...
    assert_lines.extend(_build_post_target_lifecycle(a, p))
...
def _header(case: PrepareFactorCase) -> list[str]:
    return [
        "-- --------------------------------------------------------",
...
def render_prepare_factor_case(
    case: PrepareFactorCase | PrepareFactorExtensionCase,
...
def _write_program(
    ...
    text = render_prepare_factor_case(case, Path("."))
    rc = _as_render_case(case)
    (out / rc.sql_filename).write_text(text, encoding="utf-8")


def generate_prepare_factor_programs(
    baseline_plan: PrepareFactorLoopPlan,
...
def count_primary_prepare(sql: str) -> int:
    ...
    return len(
        re.findall(r"(?im)^PREPARE(?:\s|;|$)", region)
    )


def resolve_prepare_factor_witness(
    case: PrepareFactorCase | PrepareFactorExtensionCase,
    repository_root: Path,
) -> PrepareFactorWitness:
    rc = _as_render_case(case)
    plan = _resolve_case(rc)
    return PrepareFactorWitness(
...
__all__ = [
    "PrepareFactorRenderError",
    "PrepareFactorWitness",
    "count_primary_prepare",
    "generate_prepare_factor_programs",
    "render_prepare_factor_case",
    "resolve_prepare_factor_witness",
]
