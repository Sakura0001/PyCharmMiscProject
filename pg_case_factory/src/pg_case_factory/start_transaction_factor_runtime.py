"""Bounded PostgreSQL 18.4 runtime for START TRANSACTION factor programs."""
...
from .start_transaction_factor_loop import StartTransactionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path(
    "/tmp/pgcf-pg18-start-transaction-sock-20260820"
)
PG18_PORT = 55502
PG18_DATABASE = "pgcf_start_transaction"
...
class StartTransactionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class StartTransactionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class StartTransactionSuiteRun:
    run_ordinal: int
    cases: tuple[StartTransactionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise StartTransactionRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "start_transaction_pg18_suite_run",
...
@dataclass(frozen=True)
class StartTransactionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "start_transaction_pg18_two_run_comparison"
            ),
            "passed": self.passed,
...
def compare_start_transaction_runs(
    run_01: StartTransactionSuiteRun,
    run_02: StartTransactionSuiteRun,
) -> StartTransactionTwoRunComparison:
    ...
    return StartTransactionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class StartTransactionRuntimeCase(Protocol):
...
def build_start_transaction_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[StartTransactionRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_start_transaction_factor_loop_plan(root).cases
    extensions = (
        build_start_transaction_factor_extension_plan(root).cases
    )
    cases: list[StartTransactionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: StartTransactionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class StartTransactionPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise StartTransactionRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: StartTransactionRuntimeCase,
        sql_path: Path,
    ) -> StartTransactionCaseRuntimeResult:
        ...
        return StartTransactionCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[StartTransactionRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> StartTransactionSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[StartTransactionCaseRuntimeResult] = []
        for case in cases:
        ...
        return StartTransactionSuiteRun(
...
__all__ = [
    "StartTransactionCaseRuntimeResult",
    "StartTransactionPg18Runner",
    "StartTransactionRuntimeCase",
    "StartTransactionRuntimeError",
    "StartTransactionSuiteRun",
    "StartTransactionTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_start_transaction_runtime_case_set",
    "compare_start_transaction_runs",
]
