"""Bounded PostgreSQL 18.4 runtime for ALTER SERVER factor programs."""
...
from .alter_server_factor_loop import AlterServerFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-aserver-sock-20260820")
PG18_PORT = 55498
PG18_DATABASE = "pgcf_aserver"
...
class AlterServerRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterServerCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterServerSuiteRun:
    run_ordinal: int
    cases: tuple[AlterServerCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterServerRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_server_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterServerTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_server_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_server_runs(
    run_01: AlterServerSuiteRun,
    run_02: AlterServerSuiteRun,
    ...
    return AlterServerTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterServerRuntimeCase(Protocol):
...
def build_alter_server_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterServerRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_server_factor_loop_plan(root).cases
    extensions = build_alter_server_factor_extension_plan(root).cases
    cases: list[AlterServerRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterServerRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterServerPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterServerRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[AlterServerRuntimeCase],
        ...
        *,
        ...
        results: list[AlterServerCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "AlterServerRuntimeCase",
    "AlterServerRuntimeError",
    "AlterServerSuiteRun",
    "AlterServerTwoRunComparison",
    "MAX_PARALLELISM",
