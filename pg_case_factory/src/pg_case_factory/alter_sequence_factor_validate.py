"""Actual-byte coverage validation for ALTER SEQUENCE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen baseline + bounded
extension) and compares them exactly against the on-disk SQL.  Fails closed
on any drift: byte mismatch, header mismatch, primary-target cardinality
!= 1, placeholder leakage, missing/duplicate/unknown cases, or an extension
case lacking a derivation record.  Also gates that every required factor
value is still witnessed across ``baseline ∪ extension``.
"""
...
class AlterSequenceFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_sequence_actual_factor_witness_report",
...
def _semantic_program_matches(
    case: object, sql: str, repository_root: Path,
) -> tuple[bool, tuple[str, ...], Mapping[str, Any]]:
    reasons: list[str] = []
    witness = resolve_alter_sequence_factor_witness(case, repository_root)
    expected_sql = render_alter_sequence_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_sequence(sql) != 1:
...
def _coverage_gaps(
    baseline_plan: object, all_cases: tuple[object, ...],
) -> tuple[str, ...]:
...
def validate_alter_sequence_factor_programs(
    ...
    *,
...
) -> AlterSequenceFactorProgramValidation:
    ...
    return AlterSequenceFactorProgramValidation(
        passed=not issues,
