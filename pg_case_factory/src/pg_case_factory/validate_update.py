"""Actual-byte coverage validation for UPDATE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 49-case baseline +
bounded extension cases) and compares them exactly against the
...
from .update_factor_render import (
...
class UpdateFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "update_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_update_factor_witness(
        case, repository_root
    )
    expected_sql = render_update_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_update(sql) != 1:
...
def validate_update_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return UpdateFactorProgramValidation(
        passed=not issues,
