"""Actual-byte coverage validation for NOTIFY factor-loop SQL.
...
from .notify_factor_render import (
...
class NotifyFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "notify_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_notify_factor_witness(
        case, repository_root
    )
    expected_sql = render_notify_factor_case(
        case, repository_root
    )
...
def validate_notify_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return NotifyFactorProgramValidation(
        passed=not issues,
