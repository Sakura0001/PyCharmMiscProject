"""Bounded PostgreSQL 18.4 runtime for ALTER MATERIALIZED VIEW factor programs.

Serial, one-file-at-a-time ``psql`` executor against an isolated PG 18.4
cluster (port 55491, database ``pgcf_amv``).  Each case runs twice; the
two-run comparison requires identical transcripts, structured projections,
SQLSTATEs, and a clean post-state.  The clean-state probe sweeps the
``pg_catalog`` relations an ALTER MATERIALIZED VIEW case can leave behind
(``pg_class`` for mviews/tables/indexes, ``pg_roles`` for owner/actor roles,
``pg_extension`` for the DEPENDS ON EXTENSION fixture, plus the shared
namespace/proc/type safety net).

The runtime test is skip-gated: when the cluster is down the suite reports a
skip rather than failing, mirroring the sibling statement runtimes.
"""
...
from .alter_materialized_view_factor_loop import AlterMaterializedViewFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-amv-sock-20260820")
PG18_PORT = 55491
PG18_DATABASE = "pgcf_amv"
...
class AlterMaterializedViewRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterMaterializedViewCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterMaterializedViewSuiteRun:
    run_ordinal: int
    cases: tuple[AlterMaterializedViewCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterMaterializedViewRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_materialized_view_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterMaterializedViewTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_materialized_view_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_materialized_view_runs(
    run_01: AlterMaterializedViewSuiteRun,
    run_02: AlterMaterializedViewSuiteRun,
) -> AlterMaterializedViewTwoRunComparison:
    ...
    return AlterMaterializedViewTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterMaterializedViewRuntimeCase(Protocol):
    """Structural contract for a case the runner can execute.

    Satisfied by both the frozen baseline :class:`AlterMaterializedViewFactorCase`
...
def build_alter_materialized_view_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterMaterializedViewRuntimeCase, ...], dict[str, Path]]:
    """Combine baseline + extension cases and map each to its SQL file.

    Cases are sorted by ordinal so the baseline (0001-0061) precedes the
    extension in a stable, human-auditable order.
    """
    ...
    baseline = build_alter_materialized_view_factor_loop_plan(root).cases
    extensions = build_alter_materialized_view_factor_extension_plan(root).cases
    cases: list[AlterMaterializedViewRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterMaterializedViewRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterMaterializedViewPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterMaterializedViewRuntimeError(
    ...
    def _cleanup_command(self) -> list[str]:
        """Best-effort safety-net command for re-running section 5."""
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterMaterializedViewRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_class WHERE relname LIKE '" + literal + "' "
            ...
            "UNION ALL SELECT 1 FROM pg_catalog.pg_roles WHERE rolname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_extension WHERE extname LIKE '" + literal + "'"
            ");"
        )
        try:
            result = subprocess.run(
    ...
    def _cleanup_sql(sql: str) -> str:
        if sql.count(_CLEANUP_MARKER) != 1:
            raise AlterMaterializedViewRuntimeError("cleanup phase marker is not unique")
    ...
    def _execute_case(
        self,
        case: AlterMaterializedViewRuntimeCase,
        sql_path: Path,
    ...
    def run_cases(
        self,
        cases: Sequence[AlterMaterializedViewRuntimeCase],
        ...
        *,
        ...
        results: list[AlterMaterializedViewCaseRuntimeResult] = []
        for case in cases:
        ...
        return AlterMaterializedViewSuiteRun(
...
__all__ = [
    ...
    "AlterMaterializedViewRuntimeCase",
    "AlterMaterializedViewRuntimeError",
    "AlterMaterializedViewSuiteRun",
    "AlterMaterializedViewTwoRunComparison",
    "MAX_PARALLELISM",
