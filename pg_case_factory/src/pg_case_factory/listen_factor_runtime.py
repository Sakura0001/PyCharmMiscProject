"""Bounded PostgreSQL 18.4 runtime for LISTEN factor programs."""
...
from .listen_factor_loop import ListenFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-listen-sock-20260821")
PG18_PORT = 55619
PG18_DATABASE = "pgcf_listen"
...
@dataclass(frozen=True)
class ListenSuiteRun:
    run_ordinal: int
    cases: tuple[ListenCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise ListenRuntimeError(
...
class ListenTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "listen_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_listen_runs(
    run_01: ListenSuiteRun,
    run_02: ListenSuiteRun,
) -> ListenTwoRunComparison:
...
class ListenRuntimeCase(Protocol):
...
def build_listen_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[ListenRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_listen_factor_loop_plan(root).cases
    )
    extensions = (
        build_listen_factor_extension_plan(root).cases
    )
    cases: list[ListenRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
class ListenPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise ListenRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        ...
        return ListenCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[ListenRuntimeCase],
        ...
        *,
        ...
        results: list[ListenCaseRuntimeResult] = []
        for case in cases:
