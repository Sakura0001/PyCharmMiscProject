"""Bounded PostgreSQL 18.4 runtime for CREATE ROLE factor programs.
...
frozen runtime port constant is 55603 (kept distinct from every other
...
from .create_role_factor_loop import CreateRoleFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-crole-sock-20260820")
PG18_PORT = 55603
PG18_DATABASE = "pgcf_crole"
...
class CreateRoleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateRoleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateRoleSuiteRun:
    run_ordinal: int
    cases: tuple[CreateRoleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateRoleRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_role_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateRoleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_role_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_role_runs(
    run_01: CreateRoleSuiteRun,
    run_02: CreateRoleSuiteRun,
) -> CreateRoleTwoRunComparison:
    ...
    return CreateRoleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateRoleRuntimeCase(Protocol):
...
def build_create_role_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateRoleRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_role_factor_loop_plan(root).cases
    extensions = build_create_role_factor_extension_plan(root).cases
    cases: list[CreateRoleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateRoleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateRolePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateRoleRuntimeError("unsafe object prefix")
    ...
    def _execute_case(
        self,
        case: CreateRoleRuntimeCase,
        sql_path: Path,
    ) -> CreateRoleCaseRuntimeResult:
        ...
        return CreateRoleCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateRoleRuntimeCase],
        ...
        *,
        ...
        results: list[CreateRoleCaseRuntimeResult] = []
        for case in cases:
