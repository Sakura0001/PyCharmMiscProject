"""Bounded PostgreSQL 18.4 runtime for DROP RULE factor programs."""
...
from .drop_rule_factor_loop import DropRuleFactorCase
...
class DropRuleRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropRuleCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropRuleSuiteRun:
    run_ordinal: int
    cases: tuple[DropRuleCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropRuleRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_rule_pg18_suite_run",
...
@dataclass(frozen=True)
class DropRuleTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_rule_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_rule_runs(
    run_01: DropRuleSuiteRun,
    run_02: DropRuleSuiteRun,
) -> DropRuleTwoRunComparison:
    ...
    return DropRuleTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropRuleRuntimeCase(Protocol):
...
def build_drop_rule_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropRuleRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_rule_factor_loop_plan(root).cases
    extensions = build_drop_rule_factor_extension_plan(root).cases
    cases: list[DropRuleRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropRuleRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropRulePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropRuleRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_rewrite WHERE rulename LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropRuleRuntimeCase,
        sql_path: Path,
    ) -> DropRuleCaseRuntimeResult:
        ...
        return DropRuleCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[DropRuleRuntimeCase],
        ...
        *,
        ...
        results: list[DropRuleCaseRuntimeResult] = []
        for case in cases:
...
__all__ = [
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_rule_runtime_case_set",
    "compare_drop_rule_runs",
]
