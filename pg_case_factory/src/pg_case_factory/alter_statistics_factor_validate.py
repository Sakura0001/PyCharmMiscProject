"""Actual-byte coverage validation for ALTER STATISTICS factor-loop SQL.
...
class AlterStatisticsFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_statistics_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    expected_sql = render_alter_statistics_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_statistics(sql) != 1:
...
def validate_alter_statistics_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterStatisticsFactorProgramValidation(
        passed=not issues,
