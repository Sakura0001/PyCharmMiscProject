"""Actual-byte coverage validation for COMMIT factor-loop SQL.
...
class CommitFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "commit_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_commit_factor_witness(
        case, repository_root
    )
    expected_sql = render_commit_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_commit(sql) != 1:
...
def validate_commit_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CommitFactorProgramValidation(
        passed=not issues,
