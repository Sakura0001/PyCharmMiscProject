"""Bounded post-coverage extension plan for DISCARD factor regress.

The baseline :mod:`discard_factor_loop` assigns one local SQL program to
...
DISCARD is a session-scoped statement that creates no tables, so the
bookend (``DROP TABLE IF EXISTS``) is never emitted.  The session runtime
state (parameters, prepared statements, listen channels) is the semantic
witness target.  ``DISCARD ALL`` inside a transaction block is the only
reachable failure surface (SQLSTATE ``25001``); no DISCARD variant is
...
class DiscardFactorExtensionError(ValueError):
    """Raised when DISCARD extension input drifts."""


@dataclass(frozen=True)
class DiscardFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class DiscardFactorExtensionPlan:
    cases: tuple[DiscardFactorExtensionCase, ...]
...
_COMBINATION_GROUP = "discard_required_factor_value_matrix"

_CONSUMER_ACTION = "discard"
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(b"discard-factor-extension-v1\n")
    for case in cases:
        digest.update(
            json.dumps(
                {
...
def build_discard_factor_extension_plan(
    repository_root: Path,
) -> DiscardFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_discard_factor_loop_plan(root)
    ...
    cases: list[DiscardFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise DiscardFactorExtensionError(
                    ...
                    cases.append(
                        DiscardFactorExtensionCase(
                            ...
                            sql_filename=(
                                f"DISCARD{ordinal:05d}.sql"
                            ),
                            object_prefix=(
                                f"discard_{ordinal:05d}_"
                            ),
    ...
    plan = DiscardFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if ordinals != expected_ordinals:
        raise DiscardFactorExtensionError("extension ordinal gap")
    if (
    ...
    if not all(
        ...
        raise DiscardFactorExtensionError("unknown extension outcome")
    if not all(
        case.derivation_id.startswith("DIS-EXT|")
        for case in plan.cases
    ):
        raise DiscardFactorExtensionError(
