"""Bounded PostgreSQL 18.4 runtime for TRUNCATE factor programs.
...
from .truncate_factor_loop import TruncateFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-truncate-sock-20260821")
PG18_PORT = 55501
PG18_DATABASE = "pgcf_truncate"
...
class TruncateRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class TruncateCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class TruncateSuiteRun:
    run_ordinal: int
    cases: tuple[TruncateCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise TruncateRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "truncate_pg18_suite_run",
...
@dataclass(frozen=True)
class TruncateTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "truncate_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_truncate_runs(
    run_01: TruncateSuiteRun,
    run_02: TruncateSuiteRun,
    ...
    return TruncateTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class TruncateRuntimeCase(Protocol):
...
def build_truncate_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[TruncateRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_truncate_factor_loop_plan(root).cases
    extensions = build_truncate_factor_extension_plan(root).cases
    cases: list[TruncateRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: TruncateRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class TruncatePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise TruncateRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[TruncateRuntimeCase],
        ...
        *,
        ...
        results: list[TruncateCaseRuntimeResult] = []
        for case in cases:
