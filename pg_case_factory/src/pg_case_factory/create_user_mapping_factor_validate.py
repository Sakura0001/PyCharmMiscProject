"""Actual-byte coverage validation for CREATE USER MAPPING factor-loop SQL.
...
from .create_user_mapping_factor_render import (
...
class CreateUserMappingFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_user_mapping_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_user_mapping_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_create_user_mapping(sql) != 1
    ):
...
def validate_create_user_mapping_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateUserMappingFactorProgramValidation:
    ...
    return CreateUserMappingFactorProgramValidation(
        passed=not issues,
