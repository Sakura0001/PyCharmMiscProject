"""Actual-byte coverage validation for DROP TEXT SEARCH PARSER factor-loop SQL.
...
from .drop_text_search_parser_factor_render import (
...
class DropTsParserFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_text_search_parser_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_text_search_parser_factor_witness(
        case, repository_root
    )
    expected_sql = render_drop_text_search_parser_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_drop_text_search_parser(sql) != 1:
...
def validate_drop_text_search_parser_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropTsParserFactorProgramValidation(
        passed=not issues,
