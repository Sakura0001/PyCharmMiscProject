"""Actual-byte coverage validation for CALL factor-loop SQL.
...
class CallFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "call_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_call_factor_witness(
        case, repository_root
    )
    expected_sql = render_call_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_call(sql) != 1:
...
def validate_call_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return CallFactorProgramValidation(
        passed=not issues,
