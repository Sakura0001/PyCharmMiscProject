"""Actual-byte coverage validation for DROP ACCESS METHOD factor-loop SQL.
...
from .drop_access_method_factor_render import (
...
class DropAccessMethodFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_access_method_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_drop_access_method_factor_witness(case, repository_root)
    expected_sql = render_drop_access_method_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_drop_access_method(sql) != 1:
...
def validate_drop_access_method_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return DropAccessMethodFactorProgramValidation(
        passed=not issues,
