"""Bounded PostgreSQL 18.4 runtime for CREATE LANGUAGE factor programs.

The runtime case-set combiner and psql runner are present for the DB
phase.  In the no-DB phase, the combiner is exercised by unit tests for
construction/comparison logic only; it is not executed against a live
...
from .create_language_factor_loop import CreateLanguageFactorCase

PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-clang-sock-20260820")
PG18_PORT = 55604
PG18_DATABASE = "pgcf_clang"
...
class CreateLanguageRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateLanguageCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateLanguageSuiteRun:
    run_ordinal: int
    cases: tuple[CreateLanguageCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateLanguageRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_language_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateLanguageTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_language_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_language_runs(
    run_01: CreateLanguageSuiteRun,
    run_02: CreateLanguageSuiteRun,
) -> CreateLanguageTwoRunComparison:
    ...
    return CreateLanguageTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateLanguageRuntimeCase(Protocol):
...
def build_create_language_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[CreateLanguageRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_create_language_factor_loop_plan(root).cases
    extensions = build_create_language_factor_extension_plan(root).cases
    cases: list[CreateLanguageRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: CreateLanguageRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateLanguagePg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise CreateLanguageRuntimeError("unsafe object prefix")
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_language WHERE lanname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateLanguageRuntimeCase,
        sql_path: Path,
    ) -> CreateLanguageCaseRuntimeResult:
        ...
        return CreateLanguageCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateLanguageRuntimeCase],
        ...
        *,
        ...
        results: list[CreateLanguageCaseRuntimeResult] = []
        for case in cases:
