"""Bounded PostgreSQL 18.4 runtime for CREATE OPERATOR CLASS factor programs."""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-copc-sock-20260820")
PG18_PORT = 55500
PG18_DATABASE = "pgcf_copc"
...
class CreateOperatorClassRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class CreateOperatorClassCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class CreateOperatorClassSuiteRun:
    run_ordinal: int
    cases: tuple[CreateOperatorClassCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise CreateOperatorClassRuntimeError(
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_class_pg18_suite_run",
...
@dataclass(frozen=True)
class CreateOperatorClassTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "create_operator_class_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_create_operator_class_runs(
    run_01: CreateOperatorClassSuiteRun,
    run_02: CreateOperatorClassSuiteRun,
) -> CreateOperatorClassTwoRunComparison:
    ...
    return CreateOperatorClassTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class CreateOperatorClassRuntimeCase(Protocol):
...
def build_create_operator_class_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[CreateOperatorClassRuntimeCase, ...],
    dict[str, Path],
]:
    ...
    baseline = (
        build_create_operator_class_factor_loop_plan(root).cases
    )
    extensions = (
        build_create_operator_class_factor_extension_plan(root).cases
    )
    cases: list[CreateOperatorClassRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: CreateOperatorClassRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class CreateOperatorClassPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        ...
        query = (
            ...
            + literal
            + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_operator WHERE oprname LIKE '"
            + literal
            + "');"
        )
        try:
    ...
    def _execute_case(
        self,
        case: CreateOperatorClassRuntimeCase,
        sql_path: Path,
    ) -> CreateOperatorClassCaseRuntimeResult:
        ...
        return CreateOperatorClassCaseRuntimeResult(
    ...
    def run_cases(
        self,
        cases: Sequence[CreateOperatorClassRuntimeCase],
        sql_paths: Mapping[str, Path],
        *,
        run_ordinal: int,
        selected_case_ids: Sequence[str] | None = None,
    ) -> CreateOperatorClassSuiteRun:
        self.verify_server()
        selected = (
        ...
        results: list[CreateOperatorClassCaseRuntimeResult] = []
        for case in cases:
        ...
        return CreateOperatorClassSuiteRun(
...
__all__ = [
    "CreateOperatorClassCaseRuntimeResult",
    "CreateOperatorClassPg18Runner",
    "CreateOperatorClassRuntimeCase",
    "CreateOperatorClassRuntimeError",
    "CreateOperatorClassSuiteRun",
    "CreateOperatorClassTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_create_operator_class_runtime_case_set",
    "compare_create_operator_class_runs",
]
