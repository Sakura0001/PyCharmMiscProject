"""Bounded PostgreSQL 18.4 runtime for SELECT factor programs."""
...
PG18_SOCKET = (
    Path("/tmp/pgcf-pg18-select-sock-20260821")
)
PG18_PORT = 55622
PG18_DATABASE = "pgcf_select"
...
@dataclass(frozen=True)
class SelectSuiteRun:
    run_ordinal: int
    cases: tuple[SelectCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise SelectRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "select_pg18_suite_run",
...
class SelectTwoRunComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "select_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_select_runs(
    run_01: SelectSuiteRun,
    run_02: SelectSuiteRun,
...
class SelectRuntimeCase(Protocol):
...
def build_select_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[
    tuple[SelectRuntimeCase, ...], dict[str, Path]
]:
    ...
    baseline = build_select_factor_loop_plan(root).cases
    extensions = (
        build_select_factor_extension_plan(root).cases
    )
    cases: list[SelectRuntimeCase] = [
        *baseline,
        *extensions,
    ]
...
def _case_ordinal(case: SelectRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class SelectPg18Runner:
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise SelectRuntimeError("unsafe object prefix")
    ...
    def run_cases(
        self,
        cases: Sequence[SelectRuntimeCase],
        ...
        *,
        ...
        results: list[SelectCaseRuntimeResult] = []
        for case in cases:
