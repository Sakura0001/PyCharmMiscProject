"""Bounded PostgreSQL 18.4 runtime for DROP EXTENSION factor programs."""
...
from .drop_extension_factor_loop import DropExtensionFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dext-sock-20260820")
PG18_PORT = 55502
PG18_DATABASE = "pgcf_dext"
...
class DropExtensionRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropExtensionCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropExtensionSuiteRun:
    run_ordinal: int
    cases: tuple[DropExtensionCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropExtensionRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_extension_pg18_suite_run",
...
@dataclass(frozen=True)
class DropExtensionTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_extension_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_extension_runs(
    run_01: DropExtensionSuiteRun,
    run_02: DropExtensionSuiteRun,
    ...
    return DropExtensionTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropExtensionRuntimeCase(Protocol):
...
def build_drop_extension_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropExtensionRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_extension_factor_loop_plan(root).cases
    extensions = build_drop_extension_factor_extension_plan(root).cases
    cases: list[DropExtensionRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropExtensionRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropExtensionPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropExtensionRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropExtensionRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_extension WHERE extname LIKE '" + literal + "' "
    ...
    def run_cases(
        self,
        cases: Sequence[DropExtensionRuntimeCase],
        ...
        *,
        ...
        results: list[DropExtensionCaseRuntimeResult] = []
        for case in cases:
