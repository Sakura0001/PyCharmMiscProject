"""Actual-byte coverage validation for ALTER INDEX factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 772-case baseline +
...
class AlterIndexFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_index_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_index_factor_witness(case, repository_root)
    expected_sql = render_alter_index_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_index(sql) != 1:
...
def validate_alter_index_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterIndexFactorProgramValidation(
        passed=not issues,
