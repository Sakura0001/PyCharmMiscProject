"""Bounded PostgreSQL 18.4 runtime for DROP PUBLICATION factor programs."""
...
from .drop_publication_factor_loop import DropPublicationFactorCase


PG18_BIN = Path("/tmp/pgcf-postgresql-18.4-install/bin")
PG18_SOCKET = Path("/tmp/pgcf-pg18-dp-sock-20260820")
PG18_PORT = 55502
PG18_DATABASE = "pgcf_dp"
...
class DropPublicationRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class DropPublicationCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class DropPublicationSuiteRun:
    run_ordinal: int
    cases: tuple[DropPublicationCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise DropPublicationRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_publication_pg18_suite_run",
...
@dataclass(frozen=True)
class DropPublicationTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "drop_publication_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_drop_publication_runs(
    run_01: DropPublicationSuiteRun,
    run_02: DropPublicationSuiteRun,
) -> DropPublicationTwoRunComparison:
    ...
    return DropPublicationTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class DropPublicationRuntimeCase(Protocol):
...
def build_drop_publication_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[DropPublicationRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_drop_publication_factor_loop_plan(root).cases
    extensions = build_drop_publication_factor_extension_plan(root).cases
    cases: list[DropPublicationRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: DropPublicationRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class DropPublicationPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise DropPublicationRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise DropPublicationRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_publication WHERE pubname LIKE '" + literal + "' "
            "UNION ALL SELECT 1 FROM pg_catalog.pg_subscription WHERE subname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: DropPublicationRuntimeCase,
        sql_path: Path,
    ) -> DropPublicationCaseRuntimeResult:
        ...
        return DropPublicationCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return DropPublicationSuiteRun(
...
__all__ = [
    "DropPublicationCaseRuntimeResult",
    "DropPublicationPg18Runner",
    "DropPublicationRuntimeCase",
    "DropPublicationRuntimeError",
    "DropPublicationSuiteRun",
    "DropPublicationTwoRunComparison",
    "MAX_PARALLELISM",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_drop_publication_runtime_case_set",
    "compare_drop_publication_runs",
]
