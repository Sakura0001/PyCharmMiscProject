"""Factor-value-loop obligation ledger for PostgreSQL 18.4 PREPARE TRANSACTION.

This module compiles the marginal ``GRM``/``SFV`` obligation ledger for
``PREPARE TRANSACTION``.  PREPARE TRANSACTION is a PostgreSQL two-phase
transaction control statement that prepares the current transaction for
two-phase commit.  The official synopsis has exactly one branch
(``PREPARE TRANSACTION transaction_id``), so there is a single ``GRM``
obligation.  The 43 canonical ``SFV`` rows are loaded from the shipped
applicability universe (``postgresql_18_4_factor_audit.tsv``).

PREPARE TRANSACTION reaches several PostgreSQL error surfaces.  Six canonical
factor values reach the target check and are rejected (provisional
sqlstates -- a later DB phase verifies on PG 18.4):

* ``expected_status=failure`` -- declared meta-failure mapped to the
  primary surface (prepared transaction does not exist, 42704).
* ``transaction_state=missing_required_state`` -- no prepared transaction
  exists for the supplied identifier (42704).
* ``transaction_state=inside_transaction`` -- PREPARE TRANSACTION cannot run
  inside a transaction block (25001).
* ``transaction_state=savepoint_exists`` -- a savepoint implies an open
  transaction block, which blocks PREPARE TRANSACTION (25001).
...
* ``invalid_combination=syntax_valid_semantic_error`` -- a well-formed
  statement rejected for a semantic reason (the prepared transaction does
  not exist, 42704).

All other 37 ``SFV`` values plus the single ``GRM`` branch are
success-covered.  There are no ``RISK`` obligations (PREPARE TRANSACTION has
no ``transaction_outcome`` factor).
"""
...
class PrepareTransactionFactorLoopError(ValueError):
    """Raised when a frozen PREPARE TRANSACTION obligation input drifts."""


@dataclass(frozen=True)
class PrepareTransactionGrammarAction:
    """One official target action form of the PREPARE TRANSACTION synopsis."""
...
@dataclass(frozen=True)
class PrepareTransactionFactorObligation:
    ordinal: int
    obligation_id: str
    kind: str
    factor_key: str
...
@dataclass(frozen=True)
class PrepareTransactionFactorCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class PrepareTransactionFactorLoopPlan:
    obligations: tuple[PrepareTransactionFactorObligation, ...]
    cases: tuple[PrepareTransactionFactorCase, ...]
    delegated: tuple[PrepareTransactionFactorObligation, ...]
    obligation_multiset_sha256: str


# Official synopsis branch (PostgreSQL 18 sql-prepare-transaction.html).
_BRANCH_1 = "branch_1"

_DOC_SOURCE = "postgresql-18.4-doc:sql-prepare-transaction"

# The single representative action for all canonical factors.  PREPARE
# TRANSACTION has only one synopsis form, so every factor observes on the
# single ``prepare_transaction`` action.
_REPRESENTATIVE_ACTION = "prepare_transaction"

# statement_branch canonical value -> target action.
_STATEMENT_BRANCH_CONSUMER = {
    _BRANCH_1: "prepare_transaction",
}

# Canonical factor -> the action where the value is observable.  Since
# PREPARE TRANSACTION has only one synopsis form, every factor observes on
# the single ``prepare_transaction`` action.
...
def _load_grammar_actions() -> tuple[PrepareTransactionGrammarAction, ...]:
    """Freeze every PREPARE TRANSACTION synopsis target action."""
    ...
    rows: tuple[tuple[str, str, str, str], ...] = (
        (
            ...
            "synopsis-prepare-transaction",
        ),
    )
    actions = [
        PrepareTransactionGrammarAction(
            action_id=action_id,
    ...
    if len(actions) != 1:
        raise PrepareTransactionFactorLoopError(
            "prepare transaction action count drift"
        )
...
def _compile_grammar_obligations() -> list[PrepareTransactionFactorObligation]:
    rows: list[PrepareTransactionFactorObligation] = []
    for action in _load_grammar_actions():
        rows.append(
            PrepareTransactionFactorObligation(
                ordinal=0,
                obligation_id=(
                    f"PREPARETRANSACTION-GRM|{action.grammar_branch_id}|"
...
def _compile_canonical_obligations(
...
) -> list[PrepareTransactionFactorObligation]:
    catalog_rows = load_shipped_applicability_universe(
        repository_root
    ).rows_for_statement("prepare_transaction")
    ...
    for row in catalog_rows:
        ...
        rows.append(
            PrepareTransactionFactorObligation(
                ordinal=0,
                obligation_id=f"PREPARETRANSACTION-SFV|{row.row_id}|{consumer}",
                kind="SFV",
...
def _obligation_multiset_sha256(
    rows: tuple[PrepareTransactionFactorObligation, ...],
) -> str:
    digest = hashlib.sha256(
        b"prepare-transaction-factor-obligations-v1\n"
    )
    for row in rows:
        digest.update(
            json.dumps(
...
# are always branch_1 / prepare_transaction.
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    ...
    PREPARE TRANSACTION's reachable failure surfaces cluster around three
    overlapping notions of the same "no committable prepared transaction"
...
def _baseline_assignments(
    obligation: PrepareTransactionFactorObligation,
) -> tuple[tuple[str, str], ...]:
...
def build_prepare_transaction_factor_loop_plan(
    repository_root: Path,
) -> PrepareTransactionFactorLoopPlan:
    """Assign one stable local SQL program to every obligation."""

    root = Path(repository_root).resolve(strict=True)
    obligations = compile_prepare_transaction_factor_loop_obligations(root)
    cases: list[PrepareTransactionFactorCase] = []
    delegated: list[PrepareTransactionFactorObligation] = []
    for obligation in obligations:
        ...
        cases.append(
            PrepareTransactionFactorCase(
                ordinal=ordinal,
                case_id=f"PREPARETRANSACTION{ordinal:05d}",
                sql_filename=f"PREPARETRANSACTION{ordinal:05d}.sql",
                object_prefix=(
                    f"preparetransaction_{ordinal:05d}_"
                ),
    ...
    plan = PrepareTransactionFactorLoopPlan(
    ...
    if len({row.sql_filename for row in plan.cases}) != 44:
        raise PrepareTransactionFactorLoopError("duplicate SQL filename")
    return plan


def compile_prepare_transaction_factor_loop_obligations(
    repository_root: Path,
) -> tuple[PrepareTransactionFactorObligation, ...]:
    ...
    if len(rows) != 44:
        raise PrepareTransactionFactorLoopError("obligation count drift")
    if len({row.obligation_id for row in rows}) != len(rows):
        raise PrepareTransactionFactorLoopError("duplicate obligation id")
...
__all__ = [
    "PrepareTransactionFactorLoopError",
    "PrepareTransactionGrammarAction",
    "PrepareTransactionFactorObligation",
    "PrepareTransactionFactorCase",
    "PrepareTransactionFactorLoopPlan",
    ...
    "_SFV_FAILURE_VALUES",
    "compile_prepare_transaction_factor_loop_obligations",
    "build_prepare_transaction_factor_loop_plan",
