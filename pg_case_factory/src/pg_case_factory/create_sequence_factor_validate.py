"""Actual-byte coverage validation for CREATE SEQUENCE factor-loop SQL.
...
from .create_sequence_factor_render import (
...
class CreateSequenceFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "create_sequence_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_create_sequence_factor_witness(
        case, repository_root
    )
    expected_sql = render_create_sequence_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_create_sequence(sql) != 1:
...
def validate_create_sequence_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> CreateSequenceFactorProgramValidation:
    ...
    return CreateSequenceFactorProgramValidation(
        passed=not issues,
