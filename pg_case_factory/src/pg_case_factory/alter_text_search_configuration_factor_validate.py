"""Actual-byte coverage validation for ALTER TEXT SEARCH CONFIGURATION factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 89-case baseline +
bounded 4890-case extension) and compares them exactly against the
on-disk SQL.  Fails closed on any drift: byte mismatch, header mismatch,
...
class AlterTextSearchConfigurationFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_text_search_configuration_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_text_search_configuration_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_text_search_configuration_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_alter_text_search_configuration(sql) != 1
    ):
...
def validate_alter_text_search_configuration_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterTextSearchConfigurationFactorProgramValidation(
        passed=not issues,
