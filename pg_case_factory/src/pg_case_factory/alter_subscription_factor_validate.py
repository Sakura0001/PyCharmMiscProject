"""Actual-byte coverage validation for ALTER SUBSCRIPTION factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 81-case baseline +
bounded 1038-case extension) and compares them exactly against the
...
class AlterSubscriptionFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": (
                "alter_subscription_actual_factor_witness_report"
            ),
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_subscription_factor_witness(
        case, repository_root
    )
    expected_sql = render_alter_subscription_factor_case(
        case, repository_root
    )
    ...
    if actual_target is None or count_primary_alter_subscription(sql) != 1:
...
def validate_alter_subscription_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterSubscriptionFactorProgramValidation(
        passed=not issues,
