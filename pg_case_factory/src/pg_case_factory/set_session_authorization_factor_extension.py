"""Bounded post-coverage extension plan for SET SESSION AUTHORIZATION factor regress.

The baseline :mod:`set_session_authorization_factor_loop` assigns one local
SQL program to every ``SFV``/``GRM`` obligation (marginal 1:1, 42 local
cases).  This module crosses the main-axis factor values with
verification/cleanup axes under an at-most-one-failure attribution policy,
producing a bounded set of additional regress programs that exercise
pairwise factor interactions.

SET SESSION AUTHORIZATION is a session-scoped statement that creates no
tables, so the bookend (``DROP TABLE IF EXISTS``) is never emitted.  The
session runtime state (session user) is the semantic witness target.
``SET SESSION AUTHORIZATION`` inside a transaction block is the only
reachable failure surface (SQLSTATE ``25001``).
...
class SetSessionAuthorizationFactorExtensionError(ValueError):
    """Raised when SET SESSION AUTHORIZATION extension input drifts."""


@dataclass(frozen=True)
class SetSessionAuthorizationFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class SetSessionAuthorizationFactorExtensionPlan:
    cases: tuple[SetSessionAuthorizationFactorExtensionCase, ...]
...
# expected_status is derived from transaction_visibility, not crossed, so it
# is NOT listed here (would double-count a single failure and break
# attribution).
...
_COMBINATION_GROUP = "set_session_authorization_required_factor_value_matrix"

_CONSUMER_ACTION = "set_session_authorization"
...
def _derive_overlapping_factors(a: dict[str, str]) -> None:
    """Derive expected_status from transaction_visibility (and vice versa)."""
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"set-session-authorization-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
...
def build_set_session_authorization_factor_extension_plan(
    repository_root: Path,
) -> SetSessionAuthorizationFactorExtensionPlan:
    """Build the bounded post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_set_session_authorization_factor_loop_plan(root)
    ...
    cases: list[SetSessionAuthorizationFactorExtensionCase] = []
    raw_count = 0
            ...
            for verification in _VERIFICATION_MODES:
                for cleanup in _CLEANUP_MODES:
                    ...
                    if len(full) != len(set(full)):
                        raise (
                            SetSessionAuthorizationFactorExtensionError(
                    ...
                    derivation_id = (
                        f"SETSESSIONAUTHORIZATION-EXT|{ordinal:05d}|"
                    ...
                    cases.append(
                        SetSessionAuthorizationFactorExtensionCase(
                            ordinal=ordinal,
                            case_id=(
                                f"SETSESSIONAUTHORIZATION{ordinal:05d}"
                            ),
                            sql_filename=(
                                f"SETSESSIONAUTHORIZATION{ordinal:05d}"
                                f".sql"
                            ...
                            object_prefix=(
                                f"setsessionauthorization_{ordinal:05d}_"
                            ),
                            ...
                            derivation_reason=(
                                "SET SESSION AUTHORIZATION extension: "
                                f"group={group_name}, "
    ...
    plan = SetSessionAuthorizationFactorExtensionPlan(
        cases=tuple(cases),
    ...
    if len(plan.cases) < expected_min:
        raise SetSessionAuthorizationFactorExtensionError(
    ...
    if not all(
        case.derivation_id.startswith("SETSESSIONAUTHORIZATION-EXT|")
        for case in plan.cases
    ):
        raise SetSessionAuthorizationFactorExtensionError(
