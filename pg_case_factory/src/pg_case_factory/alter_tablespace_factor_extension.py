"""Bounded post-coverage cross-factor extension expander for ALTER TABLESPACE.

The marginal factor-value-loop (:mod:`alter_tablespace_factor_loop`) is
the required baseline: one program per factor value, 72 local cases
(GRM 4 + SFV 68).  This module adds the bounded post-coverage extension
phase allowed by ``alter_tablespace.yaml``
...
ALTER TABLESPACE requires the executor to be the tablespace owner (or a
superuser); ``privilege_level=non_owner`` is an unconditional failure.  The
T5 single-value factors (nonexistent_tablespace, pg_reserved_new_name,
duplicate_new_name, non_owner_attempt, cannot_set_role,
nonexistent_owner_role, invalid_option_name) are derived from their T1-T4
counterparts, not crossed as axes.  ``invalid_option_value`` is a
standalone T5 factor with no T1-T4 counterpart; it is crossed as a
branch-specific axis in ``branch_set``.
...
class AlterTablespaceFactorExtensionError(ValueError):
    """Raised when a frozen ALTER TABLESPACE extension input drifts."""


@dataclass(frozen=True)
class AlterTablespaceFactorExtensionCase:
    ordinal: int
    case_id: str
    sql_filename: str
    object_prefix: str
...
@dataclass(frozen=True)
class AlterTablespaceFactorExtensionPlan:
    cases: tuple[AlterTablespaceFactorExtensionCase, ...]
...
_GENERAL_AXES: dict[str, tuple[str, ...]] = {
    ...
    "privilege_level": ("superuser", "owner", "non_owner"),
}
...
def _is_valid_combination(assignment: dict[str, str]) -> bool:
    ...
    re_ = assignment.get("role_existence", "role_exists")
    ons = assignment.get("owner_name_shape", "simple_id")
    if re_ == "role_not_exists" and ons != "nonexistent_role":
        return False
    if re_ != "role_not_exists" and ons == "nonexistent_role":
        return False
...
def _derive_t5_factors(a: dict[str, str]) -> None:
    ...
    if os_ == "not_exists" or tns == "nonexistent_name":
        ...
        a["nonexistent_tablespace"] = "tablespace_exists"

    nns = a.get("new_name_shape", "simple_id")
    if nns == "pg_prefix_reserved":
        a["pg_reserved_new_name"] = "pg_prefix_name"
    else:
        a["pg_reserved_new_name"] = "normal_name"
    ...
    if pl == "non_owner":
        ...
        a["non_owner_attempt"] = "superuser_execution"
    ...
    if opns == "invalid_option_name":
        ...
        a["invalid_option_name"] = "valid_option"

    # invalid_option_value is a standalone T5 with no T1-T4 counterpart.
    a.setdefault("invalid_option_value", "valid_value")
        ...
        for values in itertools.product(
            *[all_axes[n] for n in names]
            ...
            if len(assignment) != len(set(assignment)):
                raise AlterTablespaceFactorExtensionError(
...
def _extension_multiset_sha256(
    ...
    digest = hashlib.sha256(
        b"alter-tablespace-factor-extension-v1\n"
    )
    for case in cases:
        digest.update(
            json.dumps(
        ...
        entries.append(
            {
                ...
                "compatibility": {
                    "resolver": (
                        "alter_tablespace_factor_extension"
                    ),
...
def build_alter_tablespace_factor_extension_plan(
    repository_root: Path,
) -> AlterTablespaceFactorExtensionPlan:
    """Build the bounded ALTER TABLESPACE post-coverage extension plan."""

    root = Path(repository_root).resolve(strict=True)
    build_alter_tablespace_factor_loop_plan(root)
    behaviors = _behavior_combinations()
    behaviors.sort(key=lambda a: tuple(sorted(a.items())))
    cases: list[AlterTablespaceFactorExtensionCase] = []
            ...
            for cleanup in _CLEANUP_MODES:
                ...
                cases.append(
                    AlterTablespaceFactorExtensionCase(
                        ordinal=ordinal,
                        case_id=(
                            f"ALTERTABLESPACE{ordinal:05d}"
                        ),
                        sql_filename=(
                            f"ALTERTABLESPACE{ordinal:05d}.sql"
                        ),
                        object_prefix=(
                            f"alter_tablespace_{ordinal:05d}_"
                        ),
                        derivation_id=(
                            f"ATSP-EXT|{ordinal:05d}|"
                            f"{verification}|{cleanup}"
                        ),
                        derived_from_combination_group=(
                            "alter_tablespace_required_factor_"
                            "value_matrix"
                        ),
                        derivation_reason=(
    ...
    return AlterTablespaceFactorExtensionPlan(
        cases=cases_tuple,
...
__all__ = [
    "AlterTablespaceFactorExtensionError",
    "AlterTablespaceFactorExtensionCase",
    "AlterTablespaceFactorExtensionPlan",
