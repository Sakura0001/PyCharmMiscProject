"""Actual-byte coverage validation for ALTER ROLE factor-loop SQL.

Re-renders the canonical bytes for every case (frozen 108-case baseline +
...
from .alter_role_factor_render import (
...
class AlterRoleFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_role_actual_factor_witness_report",
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_alter_role_factor_witness(case, repository_root)
    expected_sql = render_alter_role_factor_case(case, repository_root)
    ...
    if actual_target is None or count_primary_alter_role(sql) != 1:
...
def validate_alter_role_factor_programs(
    baseline_plan: object,
    ...
    *,
    ...
    return AlterRoleFactorProgramValidation(
        passed=not issues,
