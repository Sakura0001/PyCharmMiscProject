"""Bounded PostgreSQL 18.4 runtime for ALTER OPERATOR FAMILY factor programs.

Serial, deterministic two-run executor mirroring the alter_operator_class runtime
contract.  Each generated ``.sql`` program is run twice on the isolated
``alter_operator_family`` cluster (port 55494) and the structured projections are
compared for idempotency.  When the cluster is not reachable (no-DB phase), the
suite skips rather than errors.
"""
...
PG18_SOCKET = Path("/tmp/pgcf-pg18-aof-sock-20260820")
PG18_PORT = 55494
PG18_DATABASE = "pgcf_aof"
...
class AlterOperatorFamilyRuntimeError(RuntimeError):
    """Raised when the frozen PostgreSQL runtime is unavailable or drifts."""


@dataclass(frozen=True)
class AlterOperatorFamilyCaseRuntimeResult:
    case_id: str
    sql_filename: str
...
@dataclass(frozen=True)
class AlterOperatorFamilySuiteRun:
    run_ordinal: int
    cases: tuple[AlterOperatorFamilyCaseRuntimeResult, ...]

    def __post_init__(self) -> None:
        if self.run_ordinal not in {1, 2}:
            raise AlterOperatorFamilyRuntimeError("run ordinal must be 1 or 2")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_operator_family_pg18_suite_run",
...
@dataclass(frozen=True)
class AlterOperatorFamilyTwoRunComparison:
    passed: bool
    issues: tuple[str, ...]
    execution_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "alter_operator_family_pg18_two_run_comparison",
            "passed": self.passed,
...
def compare_alter_operator_family_runs(
    run_01: AlterOperatorFamilySuiteRun,
    run_02: AlterOperatorFamilySuiteRun,
) -> AlterOperatorFamilyTwoRunComparison:
    ...
    return AlterOperatorFamilyTwoRunComparison(
        passed=not issues,
        issues=tuple(issues),
...
class AlterOperatorFamilyRuntimeCase(Protocol):
...
def build_alter_operator_family_runtime_case_set(
    repository_root: Path,
    sql_dir: Path,
) -> tuple[tuple[AlterOperatorFamilyRuntimeCase, ...], dict[str, Path]]:
    ...
    baseline = build_alter_operator_family_factor_loop_plan(root).cases
    extensions = build_alter_operator_family_factor_extension_plan(root).cases
    cases: list[AlterOperatorFamilyRuntimeCase] = [*baseline, *extensions]
...
def _case_ordinal(case: AlterOperatorFamilyRuntimeCase) -> int:
    return getattr(case, "ordinal")  # type: ignore[no-any-return]


class AlterOperatorFamilyPg18Runner:
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds <= 0 or timeout_seconds > 300:
            raise AlterOperatorFamilyRuntimeError(
    ...
    def _clean_probe(self, object_prefix: str) -> bool:
        if _PREFIX.fullmatch(object_prefix) is None:
            raise AlterOperatorFamilyRuntimeError("unsafe object prefix")
        literal = object_prefix.replace("'", "''") + "%"
        query = (
            "SELECT NOT EXISTS ("
            "SELECT 1 FROM pg_catalog.pg_opfamily WHERE opfname LIKE '" + literal + "' "
    ...
    def _execute_case(
        self,
        case: AlterOperatorFamilyRuntimeCase,
        sql_path: Path,
    ) -> AlterOperatorFamilyCaseRuntimeResult:
        ...
        return AlterOperatorFamilyCaseRuntimeResult(
    ...
    def run_cases(
        ...
        *,
        ...
        return AlterOperatorFamilySuiteRun(
...
__all__ = [
    ...
    "AlterOperatorFamilyRuntimeCase",
    ...
    "PG18_SOCKET",
    "PG18_SUPERUSER",
    "build_alter_operator_family_runtime_case_set",
    "compare_alter_operator_family_runs",
]
