"""Actual-byte coverage validation for DISCARD factor-loop SQL.
...
from .discard_factor_render import (
...
class DiscardFactorProgramValidation:
    passed: bool
    issues: tuple[str, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "discard_actual_factor_witness_report",
            "generation_mode": (
...
def _semantic_program_matches(
    ...
    reasons: list[str] = []
    witness = resolve_discard_factor_witness(
        case, repository_root
    )
    expected_sql = render_discard_factor_case(
        case, repository_root
    )
    ...
    if (
        ...
        or count_primary_discard(sql) != 1
    ):
...
def validate_discard_factor_programs(
    baseline_plan: object,
    ...
    *,
...
) -> DiscardFactorProgramValidation:
    ...
    return DiscardFactorProgramValidation(
        passed=not issues,
