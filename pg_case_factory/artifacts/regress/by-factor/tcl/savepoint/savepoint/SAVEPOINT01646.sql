...
-- primary_obligation_id: SAVEPOINT-EXT|01646|effect_query|reset_state
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'savepoint_01646_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN READ ONLY;
PREPARE TRANSACTION 'savepoint_01646_prep';
-- 3. 执行唯一获得覆盖信用的 SAVEPOINT。
-- primary-target-begin
SAVEPOINT savepoint_01646_sp;
-- primary-target-end
