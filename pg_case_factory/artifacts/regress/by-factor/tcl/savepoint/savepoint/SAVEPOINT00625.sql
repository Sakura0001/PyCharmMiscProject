...
-- primary_obligation_id: SAVEPOINT-EXT|00625|effect_query|drop_objects
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'savepoint_00625_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN READ ONLY;
PREPARE TRANSACTION 'savepoint_00625_prep';
-- 3. 执行唯一获得覆盖信用的 SAVEPOINT。
-- primary-target-begin
SAVEPOINT savepoint_00625_sp AND CHAIN;
-- primary-target-end
