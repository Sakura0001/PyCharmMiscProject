...
-- ++
-- --------------------------------------------------------
-- case_id: ROLLBACKTOSAVEPOINT00641
...
-- primary_obligation_id: ROLLBACKTOSAVEPOINT-EXT|00641|returned_rows|reset_state
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'rollbacktosavepoint_00641_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN READ WRITE;
PREPARE TRANSACTION 'rollbacktosavepoint_00641_prep';
-- 3. 执行唯一获得覆盖信用的 ROLLBACK TO SAVEPOINT。
-- primary-target-begin
ROLLBACK TO SAVEPOINT rollbacktosavepoint_00641_sp AND CHAIN;
-- primary-target-end
