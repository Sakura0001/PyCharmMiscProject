...
-- ++
-- --------------------------------------------------------
-- case_id: ROLLBACKTOSAVEPOINT01038
...
-- primary_obligation_id: ROLLBACKTOSAVEPOINT-EXT|01038|error_assertion|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'rollbacktosavepoint_01038_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN ISOLATION LEVEL SERIALIZABLE;
PREPARE TRANSACTION 'rollbacktosavepoint_01038_prep';
-- 3. 执行唯一获得覆盖信用的 ROLLBACK TO SAVEPOINT。
-- primary-target-begin
ROLLBACK TO SAVEPOINT rollbacktosavepoint_01038_sp AND NO CHAIN;
-- primary-target-end
