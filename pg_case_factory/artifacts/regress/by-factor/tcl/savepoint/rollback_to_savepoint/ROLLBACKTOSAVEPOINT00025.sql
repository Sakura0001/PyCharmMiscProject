...
-- version      : 1.0
-- description  : ROLLBACK TO SAVEPOINT state_boundary=prepared_transaction_leftover
-- FE           : PG18-STATEMENT-FACTOR-LOOP
-- ++
-- --------------------------------------------------------
-- case_id: ROLLBACKTOSAVEPOINT00025
...
-- primary_obligation_id: ROLLBACKTOSAVEPOINT-SFV|sfv-5aea7f88b1f614d7e816a529|rollback_to_savepoint
-- expected_outcome: expected_failure
-- expected_sqlstate: 55000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'rollbacktosavepoint_00025_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN;
PREPARE TRANSACTION 'rollbacktosavepoint_00025_prep';
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ROLLBACK TO SAVEPOINT。
-- primary-target-begin
ROLLBACK TO SAVEPOINT rollbacktosavepoint_00025_sp;
-- primary-target-end
