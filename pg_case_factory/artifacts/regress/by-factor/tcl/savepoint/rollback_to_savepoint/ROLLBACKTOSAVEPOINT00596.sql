...
-- ++
-- --------------------------------------------------------
-- case_id: ROLLBACKTOSAVEPOINT00596
...
-- primary_obligation_id: ROLLBACKTOSAVEPOINT-EXT|00596|error_assertion|reset_state
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'rollbacktosavepoint_00596_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN;
PREPARE TRANSACTION 'rollbacktosavepoint_00596_prep';
-- 3. 执行唯一获得覆盖信用的 ROLLBACK TO SAVEPOINT。
-- primary-target-begin
ROLLBACK TO SAVEPOINT rollbacktosavepoint_00596_sp AND CHAIN;
-- primary-target-end
