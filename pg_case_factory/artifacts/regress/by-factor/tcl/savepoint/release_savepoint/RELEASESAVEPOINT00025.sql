...
-- version      : 1.0
-- description  : RELEASE SAVEPOINT state_boundary=prepared_transaction_leftover
-- FE           : PG18-STATEMENT-FACTOR-LOOP
-- ++
-- --------------------------------------------------------
-- case_id: RELEASESAVEPOINT00025
...
-- primary_obligation_id: RELEASESAVEPOINT-SFV|sfv-d6b7606113b8a8dafb6502e0|release_savepoint
-- expected_outcome: expected_failure
-- expected_sqlstate: 55000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'releasesavepoint_00025_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN;
PREPARE TRANSACTION 'releasesavepoint_00025_prep';
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 RELEASE SAVEPOINT。
-- primary-target-begin
RELEASE SAVEPOINT releasesavepoint_00025_sp;
-- primary-target-end
