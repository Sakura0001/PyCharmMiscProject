...
-- primary_obligation_id: RELEASESAVEPOINT-EXT|00441|catalog_query|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'releasesavepoint_00441_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN READ ONLY;
PREPARE TRANSACTION 'releasesavepoint_00441_prep';
-- 3. 执行唯一获得覆盖信用的 RELEASE SAVEPOINT。
-- primary-target-begin
RELEASE SAVEPOINT releasesavepoint_00441_sp AND CHAIN;
-- primary-target-end
