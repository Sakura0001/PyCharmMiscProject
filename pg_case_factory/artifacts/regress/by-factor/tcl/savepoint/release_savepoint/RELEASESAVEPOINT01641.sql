...
-- primary_obligation_id: RELEASESAVEPOINT-EXT|01641|catalog_query|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'releasesavepoint_01641_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN READ ONLY;
PREPARE TRANSACTION 'releasesavepoint_01641_prep';
-- 3. 执行唯一获得覆盖信用的 RELEASE SAVEPOINT。
-- primary-target-begin
RELEASE SAVEPOINT releasesavepoint_01641_sp;
-- primary-target-end
