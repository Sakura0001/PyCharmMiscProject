...
-- primary_obligation_id: RELEASESAVEPOINT-EXT|01806|error_assertion|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
ROLLBACK PREPARED 'releasesavepoint_01806_prep';
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
BEGIN ISOLATION LEVEL SERIALIZABLE READ ONLY DEFERRABLE;
PREPARE TRANSACTION 'releasesavepoint_01806_prep';
-- 3. 执行唯一获得覆盖信用的 RELEASE SAVEPOINT。
-- primary-target-begin
RELEASE SAVEPOINT releasesavepoint_01806_sp;
-- primary-target-end
