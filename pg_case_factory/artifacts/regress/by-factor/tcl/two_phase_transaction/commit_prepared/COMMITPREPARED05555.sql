...
-- primary_obligation_id: CPREP-EXT|05555|catalog_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS commitprepared_05555_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE commitprepared_05555_data (id integer, payload text);
INSERT INTO commitprepared_05555_data VALUES (1, 'commit_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COMMIT PREPARED。
-- primary-target-begin
COMMIT PREPARED 'commitprepared_05555_Mixed Tx';
-- primary-target-end
