...
-- primary_obligation_id: CPREP-EXT|01373|returned_rows|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS commitprepared_01373_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE commitprepared_01373_data (id integer, payload text);
INSERT INTO commitprepared_01373_data VALUES (1, 'commit_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COMMIT PREPARED。
-- primary-target-begin
COMMIT PREPARED 'commitprepared_01373_Mixed Tx';
-- primary-target-end
...
SELECT id, payload FROM commitprepared_01373_data ORDER BY id;
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS commitprepared_01373_data;
