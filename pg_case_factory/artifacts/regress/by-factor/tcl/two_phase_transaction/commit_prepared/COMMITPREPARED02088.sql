...
-- primary_obligation_id: CPREP-EXT|02088|effect_query|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS commitprepared_02088_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE commitprepared_02088_data (id integer, payload text);
INSERT INTO commitprepared_02088_data VALUES (1, 'commit_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COMMIT PREPARED。
-- primary-target-begin
COMMIT PREPARED 'commitprepared_02088_duptx';
-- primary-target-end
...
SELECT count(*) AS committed_rows FROM commitprepared_02088_data ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS commitprepared_02088_data;
