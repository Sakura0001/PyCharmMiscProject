...
-- primary_obligation_id: ROLLBACKPREPARED-EXT|01309|effect_query|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42601
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS rollbackprepared_01309_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE rollbackprepared_01309_data (id integer, payload text);
INSERT INTO rollbackprepared_01309_data VALUES (1, 'rollback_prepared_fixture');
...
SELECT count(*) AS residual_rows FROM rollbackprepared_01309_data ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS rollbackprepared_01309_data;
