...
-- primary_obligation_id: ROLLBACKPREPARED-EXT|01553|returned_rows|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS rollbackprepared_01553_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE rollbackprepared_01553_data (id integer, payload text);
INSERT INTO rollbackprepared_01553_data VALUES (1, 'rollback_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ROLLBACK PREPARED。
-- primary-target-begin
ROLLBACK PREPARED 'rollbackprepared_01553_tx';
-- primary-target-end
...
SELECT id, payload FROM rollbackprepared_01553_data ORDER BY id;
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS rollbackprepared_01553_data;
