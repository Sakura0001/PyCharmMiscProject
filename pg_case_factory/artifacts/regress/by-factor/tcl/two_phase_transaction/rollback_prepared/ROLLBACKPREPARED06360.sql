...
-- primary_obligation_id: ROLLBACKPREPARED-EXT|06360|effect_query|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42601
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS rollbackprepared_06360_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE rollbackprepared_06360_data (id integer, payload text);
INSERT INTO rollbackprepared_06360_data VALUES (1, 'rollback_prepared_fixture');
...
SELECT count(*) AS residual_rows FROM rollbackprepared_06360_data ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS rollbackprepared_06360_data;
