...
-- primary_obligation_id: ROLLBACKPREPARED-EXT|11899|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS rollbackprepared_11899_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE rollbackprepared_11899_data (id integer, payload text);
INSERT INTO rollbackprepared_11899_data VALUES (1, 'rollback_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ROLLBACK PREPARED。
-- primary-target-begin
ROLLBACK PREPARED 'rollbackprepared_11899_tx';
-- primary-target-end
