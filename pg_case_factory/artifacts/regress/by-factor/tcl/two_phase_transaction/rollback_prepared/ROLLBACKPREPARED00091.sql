...
-- primary_obligation_id: ROLLBACKPREPARED-EXT|00091|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS rollbackprepared_00091_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE rollbackprepared_00091_data (id integer, payload text);
INSERT INTO rollbackprepared_00091_data VALUES (1, 'rollback_prepared_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ROLLBACK PREPARED。
-- primary-target-begin
ROLLBACK PREPARED 'rollbackprepared_00091_duptx';
-- primary-target-end
