...
-- ++
-- --------------------------------------------------------
-- case_id: PREPARETRANSACTION02167
...
-- primary_obligation_id: PREPARETRANSACTION-EXT|02167|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS preparetransaction_02167_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE preparetransaction_02167_data (id integer, payload text);
INSERT INTO preparetransaction_02167_data VALUES (1, 'prepare_transaction_fixture');
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 PREPARE TRANSACTION。
-- primary-target-begin
PREPARE TRANSACTION 'preparetransaction_02167_duptx';
-- primary-target-end
