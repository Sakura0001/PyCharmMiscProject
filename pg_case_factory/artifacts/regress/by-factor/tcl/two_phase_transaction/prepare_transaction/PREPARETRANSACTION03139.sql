...
-- ++
-- --------------------------------------------------------
-- case_id: PREPARETRANSACTION03139
...
-- primary_obligation_id: PREPARETRANSACTION-EXT|03139|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS preparetransaction_03139_data;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE preparetransaction_03139_data (id integer, payload text);
INSERT INTO preparetransaction_03139_data VALUES (1, 'prepare_transaction_fixture');
BEGIN;
SAVEPOINT preparetransaction_03139_sp;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 PREPARE TRANSACTION。
-- primary-target-begin
PREPARE TRANSACTION 'preparetransaction_03139_duptx';
-- primary-target-end
