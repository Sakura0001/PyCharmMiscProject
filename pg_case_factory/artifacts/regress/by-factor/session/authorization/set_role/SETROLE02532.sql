...
-- primary_obligation_id: SETROLE-EXT|02532|catalog_query|reset_state|transaction_shape
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
RESET ROLE;
DROP ROLE IF EXISTS setrole_02532_witness;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE setrole_02532_witness LOGIN;
GRANT setrole_02532_witness TO CURRENT_USER;
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 SET ROLE。
-- primary-target-begin
SET ROLE setrole_02532_witness;
-- primary-target-end
...
SELECT count(*) > 0 AS role_observed FROM pg_catalog.pg_roles WHERE rolname = 'setrole_02532_witness' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
RESET ROLE;
DROP ROLE IF EXISTS setrole_02532_witness;
