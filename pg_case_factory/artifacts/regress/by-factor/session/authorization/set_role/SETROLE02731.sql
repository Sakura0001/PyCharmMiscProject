...
-- primary_obligation_id: SETROLE-EXT|02731|error_assertion|rollback|scope_name
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
RESET ROLE;
DROP ROLE IF EXISTS setrole_02731_witness;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE setrole_02731_witness LOGIN;
GRANT setrole_02731_witness TO CURRENT_USER;
-- 3. 执行唯一获得覆盖信用的 SET ROLE。
-- primary-target-begin
SET ROLE setrole_02731_witness;
-- primary-target-end
