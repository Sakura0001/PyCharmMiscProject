...
-- ++
-- --------------------------------------------------------
-- case_id: SETSESSIONAUTHORIZATION01958
...
-- primary_obligation_id: SETSESSIONAUTHORIZATION-EXT|01958|error_assertion|rollback|transaction_shape
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
SET SESSION AUTHORIZATION DEFAULT;
DROP ROLE IF EXISTS setsessionauthorization_01958_witness;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE setsessionauthorization_01958_witness LOGIN;
GRANT setsessionauthorization_01958_witness TO CURRENT_USER;
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信心的 SET SESSION AUTHORIZATION。
-- primary-target-begin
SET SESSION AUTHORIZATION setsessionauthorization_01958_witness;
-- primary-target-end
