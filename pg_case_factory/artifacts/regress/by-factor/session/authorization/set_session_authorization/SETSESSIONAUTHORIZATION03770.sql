...
-- ++
-- --------------------------------------------------------
-- case_id: SETSESSIONAUTHORIZATION03770
...
-- primary_obligation_id: SETSESSIONAUTHORIZATION-EXT|03770|error_assertion|rollback|dependency_transaction
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
ROLLBACK;
SET SESSION AUTHORIZATION DEFAULT;
DROP ROLE IF EXISTS setsessionauthorization_03770_witness;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE setsessionauthorization_03770_witness LOGIN;
GRANT setsessionauthorization_03770_witness TO CURRENT_USER;
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信心的 SET SESSION AUTHORIZATION。
-- primary-target-begin
SET SESSION AUTHORIZATION setsessionauthorization_03770_witness;
-- primary-target-end
