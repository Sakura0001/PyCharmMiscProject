...
-- primary_obligation_id: NOTIFY-EXT|01807|catalog_query|reset_state|dependency_invalid
...
NOTIFY notify_01807_ch;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) > 0 AS listening FROM pg_catalog.pg_listening_channels() AS ch WHERE ch = 'notify_01807_ch' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
UNLISTEN *;
