...
-- primary_obligation_id: UNLISTEN-EXT|01332|catalog_query|rollback|name_payload
-- expected_outcome: expected_failure
-- expected_sqlstate: 08P01
-- 1. 清理本编号对象，保证脚本可重复执行。
UNLISTEN *;
RESET ROLE;
DROP ROLE IF EXISTS unlisten_01332_actor;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE unlisten_01332_actor LOGIN NOSUPERUSER;
SET ROLE unlisten_01332_actor;
...
UNLISTEN *;
RESET ROLE;
DROP OWNED BY unlisten_01332_actor CASCADE;
DROP ROLE IF EXISTS unlisten_01332_actor;
