...
-- primary_obligation_id: VACUUM-EXT|01287|effect_query|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS vacuum_01287_t CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE vacuum_01287_t (id integer);
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 VACUUM。
-- primary-target-begin
VACUUM "vacuum_01287_t";
-- primary-target-end
...
SELECT count(*) AS vacuum_stat_rows FROM pg_catalog.pg_class c WHERE c.relname = 'vacuum_01287_t' AND c.relpages >= 0 ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS vacuum_01287_t CASCADE;
