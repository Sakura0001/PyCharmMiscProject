...
-- primary_obligation_id: VACUUM-EXT|01304|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS vacuum_01304_t CASCADE;
DROP SCHEMA IF EXISTS vacuum_01304_sch CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE SCHEMA vacuum_01304_sch;
CREATE TABLE vacuum_01304_t (id integer);
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 VACUUM。
-- primary-target-begin
VACUUM vacuum_01304_sch.vacuum_01304_t;
-- primary-target-end
