...
-- primary_obligation_id: VACUUM-EXT|00919|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS vacuum_00919_t CASCADE;
DROP VIEW IF EXISTS vacuum_00919_v CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE vacuum_00919_t (id integer);
CREATE VIEW vacuum_00919_v AS SELECT id FROM vacuum_00919_t;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 VACUUM。
-- primary-target-begin
VACUUM FULL vacuum_00919_v;
-- primary-target-end
