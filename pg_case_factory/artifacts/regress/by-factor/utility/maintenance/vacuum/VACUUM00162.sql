...
-- primary_obligation_id: VACUUM-EXT|00162|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS vacuum_00162_t CASCADE;
DROP VIEW IF EXISTS vacuum_00162_v CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE vacuum_00162_t (id integer);
INSERT INTO vacuum_00162_t (id) VALUES (1), (2);
CREATE VIEW vacuum_00162_v AS SELECT id FROM vacuum_00162_t;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 VACUUM。
-- primary-target-begin
VACUUM FULL vacuum_00162_v;
-- primary-target-end
