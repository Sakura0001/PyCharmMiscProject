...
-- primary_obligation_id: ANALYZE-EXT|00212|returned_rows|drop_objects
...
ANALYZE (SKIP_LOCKED) analyze_00212_t * (analyze_00212_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_analyze FROM pg_catalog.pg_class WHERE relname = 'analyze_00212_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
