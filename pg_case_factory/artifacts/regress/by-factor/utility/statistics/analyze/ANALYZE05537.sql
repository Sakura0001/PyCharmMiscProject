...
-- primary_obligation_id: ANALYZE-EXT|05537|effect_query|drop_objects
...
ANALYZE (SKIP_LOCKED) analyze_05537_t * (analyze_05537_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_05537_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
