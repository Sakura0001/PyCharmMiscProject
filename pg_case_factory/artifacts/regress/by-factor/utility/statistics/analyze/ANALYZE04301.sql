...
-- primary_obligation_id: ANALYZE-EXT|04301|effect_query|drop_objects
...
ANALYZE (VERBOSE) analyze_04301_t * (analyze_04301_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_04301_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
