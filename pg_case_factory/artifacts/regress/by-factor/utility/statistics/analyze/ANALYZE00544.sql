...
-- primary_obligation_id: ANALYZE-EXT|00544|effect_query|rollback
...
ANALYZE (BUFFER_USAGE_LIMIT 100) analyze_00544_t;
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_00544_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
