...
-- primary_obligation_id: ANALYZE-EXT|01948|effect_query|rollback
...
ANALYZE (SKIP_LOCKED) ONLY analyze_01948_t (analyze_01948_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_01948_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
