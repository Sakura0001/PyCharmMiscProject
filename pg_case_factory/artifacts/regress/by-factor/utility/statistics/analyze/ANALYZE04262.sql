...
-- primary_obligation_id: ANALYZE-EXT|04262|catalog_query|drop_objects
...
ANALYZE (VERBOSE) ONLY analyze_04262_t (analyze_04262_c);
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'analyze_04262_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
