...
-- primary_obligation_id: ANALYZE-EXT|02977|catalog_query|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS analyze_02977_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE VIEW analyze_02977_t AS SELECT 1 AS analyze_02977_c;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ANALYZE。
-- primary-target-begin
ANALYZE (VERBOSE) analyze_02977_t;
-- primary-target-end
...
SELECT count(*) = 0 AS not_a_regular_table FROM pg_catalog.pg_class WHERE relname = 'analyze_02977_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
DROP VIEW IF EXISTS analyze_02977_t;
