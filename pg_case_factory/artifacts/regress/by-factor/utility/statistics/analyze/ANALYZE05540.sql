...
-- primary_obligation_id: ANALYZE-EXT|05540|returned_rows|drop_objects
...
ANALYZE (SKIP_LOCKED) analyze_05540_t * (analyze_05540_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_analyze FROM pg_catalog.pg_class WHERE relname = 'analyze_05540_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
