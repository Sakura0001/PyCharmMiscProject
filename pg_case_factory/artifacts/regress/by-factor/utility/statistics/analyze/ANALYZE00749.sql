...
-- primary_obligation_id: ANALYZE-EXT|00749|effect_query|drop_objects
...
ANALYZE (VERBOSE) analyze_00749_t * (analyze_00749_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_00749_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
