...
-- primary_obligation_id: ANALYZE-EXT|00175|returned_rows|rollback
...
ANALYZE (SKIP_LOCKED) ONLY analyze_00175_t (analyze_00175_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_analyze FROM pg_catalog.pg_class WHERE relname = 'analyze_00175_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
