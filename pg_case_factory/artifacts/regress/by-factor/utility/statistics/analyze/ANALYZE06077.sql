...
-- primary_obligation_id: ANALYZE-EXT|06077|effect_query|drop_objects
...
ANALYZE (VERBOSE) analyze_06077_t * (analyze_06077_c);
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_06077_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
