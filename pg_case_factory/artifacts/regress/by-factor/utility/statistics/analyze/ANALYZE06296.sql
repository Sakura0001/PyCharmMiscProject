...
-- primary_obligation_id: ANALYZE-EXT|06296|returned_rows|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS analyze_06296_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE VIEW analyze_06296_t AS SELECT 1 AS analyze_06296_c;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ANALYZE。
-- primary-target-begin
ANALYZE analyze_06296_t;
-- primary-target-end
...
SELECT count(*) = 0 AS no_analyze FROM pg_catalog.pg_class WHERE relname = 'analyze_06296_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP VIEW IF EXISTS analyze_06296_t;
