...
-- primary_obligation_id: ANALYZE-EXT|02501|effect_query|drop_objects
...
ANALYZE (VERBOSE) analyze_02501_t;
-- primary-target-end
...
SELECT count(*) = 0 AS no_stats FROM pg_catalog.pg_class WHERE relname = 'analyze_02501_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
