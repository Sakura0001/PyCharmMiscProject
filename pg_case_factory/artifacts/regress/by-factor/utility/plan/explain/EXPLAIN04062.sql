...
-- primary_obligation_id: EXPLAIN-EXT|04062|effect_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SEQUENCE IF EXISTS explain_04062_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE explain_04062_t;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 EXPLAIN。
-- primary-target-begin
EXPLAIN (ANALYZE) SELECT * FROM "explain_04062_t";
-- primary-target-end
...
SELECT count(*) = 0 AS no_plan FROM pg_catalog.pg_class WHERE relname = 'explain_04062_t' AND relkind = 'r' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
RESET search_path;
DROP SEQUENCE IF EXISTS explain_04062_t;
