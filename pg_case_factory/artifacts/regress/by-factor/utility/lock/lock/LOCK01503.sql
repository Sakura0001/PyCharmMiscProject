...
-- primary_obligation_id: LOCK-EXT|01503|effect_query|reset_state
...
LOCK TABLE lock_01503_wrong_type_target IN SHARE MODE;
-- primary-target-end
...
SELECT count(*) AS relation_effect_witness FROM pg_catalog.pg_class WHERE relname = 'lock_01503_t' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
