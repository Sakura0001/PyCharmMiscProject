...
-- primary_obligation_id: LOCK-EXT|00068|returned_rows|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS lock_00068_t CASCADE;
DROP ROLE IF EXISTS lock_00068_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE ROLE lock_00068_actor LOGIN NOSUPERUSER;
CREATE TABLE lock_00068_t (c integer);
SET ROLE lock_00068_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 LOCK TABLE。
-- primary-target-begin
LOCK TABLE "lock_00068_t" IN ACCESS EXCLUSIVE MODE NOWAIT;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY lock_00068_actor;
DROP ROLE IF EXISTS lock_00068_actor;
DROP TABLE IF EXISTS lock_00068_t CASCADE;
