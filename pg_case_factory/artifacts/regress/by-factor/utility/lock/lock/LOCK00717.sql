...
-- primary_obligation_id: LOCK-EXT|00717|returned_rows|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS lock_00717_t CASCADE;
DROP ROLE IF EXISTS lock_00717_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE ROLE lock_00717_actor LOGIN NOSUPERUSER;
CREATE TABLE lock_00717_t (c integer);
SET ROLE lock_00717_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 LOCK TABLE。
-- primary-target-begin
LOCK TABLE "lock_00717_t" IN SHARE MODE;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY lock_00717_actor;
DROP ROLE IF EXISTS lock_00717_actor;
DROP TABLE IF EXISTS lock_00717_t CASCADE;
