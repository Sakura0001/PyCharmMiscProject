...
-- primary_obligation_id: LOCK-EXT|00301|catalog_query|rollback
...
LOCK TABLE lock_00301_no_such_table IN ACCESS EXCLUSIVE MODE NOWAIT;
-- primary-target-end
...
SELECT count(*) = 0 AS relation_absent FROM pg_catalog.pg_class WHERE relname = 'lock_00301_t' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
