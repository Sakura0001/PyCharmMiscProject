...
-- primary_obligation_id: DO-EXT|05292|catalog_query|drop_objects|env_resource
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
RESET ROLE;
DROP ROLE IF EXISTS do_05292_actor;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE do_05292_actor LOGIN NOSUPERUSER;
SET ROLE do_05292_actor;
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY do_05292_actor CASCADE;
DROP ROLE IF EXISTS do_05292_actor;
