...
-- primary_obligation_id: DO-EXT|01766|effect_query|rollback|option_execution
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
RESET ROLE;
DROP ROLE IF EXISTS do_01766_actor;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE do_01766_actor LOGIN NOSUPERUSER;
SET ROLE do_01766_actor;
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY do_01766_actor CASCADE;
DROP ROLE IF EXISTS do_01766_actor;
