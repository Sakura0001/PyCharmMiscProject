...
-- primary_obligation_id: COPY-EXT|02537|catalog_query|drop_objects
...
COPY (SELECT count(*) AS copy_02537_c FROM copy_02537_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'copy_02537_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
