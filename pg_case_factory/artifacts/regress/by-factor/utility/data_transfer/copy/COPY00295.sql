...
-- primary_obligation_id: COPY-EXT|00295|effect_query|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS copy_00295_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE copy_00295_t (copy_00295_c int);
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY copy_00295_t FROM 'copy_00295_src.csv';
-- primary-target-end
...
SELECT count(*) > 0 AS database_wide_marker FROM pg_catalog.pg_class WHERE relname = 'copy_00295_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
DROP TABLE IF EXISTS copy_00295_t;
