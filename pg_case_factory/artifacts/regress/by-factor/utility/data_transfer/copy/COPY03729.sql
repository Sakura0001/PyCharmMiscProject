...
-- primary_obligation_id: COPY-EXT|03729|effect_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS copy_03729_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE copy_03729_t (copy_03729_c int);
INSERT INTO copy_03729_t VALUES (1), (2), (3);
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY copy_03729_t TO STDOUT;
-- primary-target-end
...
SELECT count(*) > 0 AS database_wide_marker FROM pg_catalog.pg_class WHERE relname = 'copy_03729_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
RESET search_path;
DROP TABLE IF EXISTS copy_03729_t;
