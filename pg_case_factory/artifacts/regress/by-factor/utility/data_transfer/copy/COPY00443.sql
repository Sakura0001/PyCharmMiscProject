...
-- primary_obligation_id: COPY-EXT|00443|returned_rows|drop_objects
...
COPY "copy_00443_t" FROM 'copy_00443_src.csv';
-- primary-target-end
...
SELECT count(*) = 0 AS no_copy FROM pg_catalog.pg_class WHERE relname = 'copy_00443_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
