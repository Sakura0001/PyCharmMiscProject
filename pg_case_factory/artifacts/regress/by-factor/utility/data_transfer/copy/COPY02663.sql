...
-- primary_obligation_id: COPY-EXT|02663|returned_rows|drop_objects
...
COPY (SELECT count(*) AS copy_02663_c FROM copy_02663_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS no_copy FROM pg_catalog.pg_class WHERE relname = 'copy_02663_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
