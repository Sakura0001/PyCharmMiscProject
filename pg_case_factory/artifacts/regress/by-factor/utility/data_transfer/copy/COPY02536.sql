...
-- primary_obligation_id: COPY-EXT|02536|catalog_query|rollback
...
COPY (SELECT count(*) AS copy_02536_c FROM copy_02536_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'copy_02536_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
