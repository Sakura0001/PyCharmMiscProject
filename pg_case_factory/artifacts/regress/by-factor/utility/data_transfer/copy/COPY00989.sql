...
-- primary_obligation_id: COPY-EXT|00989|catalog_query|drop_objects
...
COPY "copy_00989_t" FROM 'copy_00989_src.csv';
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'copy_00989_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
