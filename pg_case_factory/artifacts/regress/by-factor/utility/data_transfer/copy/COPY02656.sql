...
-- primary_obligation_id: COPY-EXT|02656|catalog_query|rollback
...
COPY (SELECT count(*) AS copy_02656_c FROM copy_02656_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'copy_02656_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
