...
-- primary_obligation_id: COPY-EXT|02833|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS copy_02833_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE copy_02833_actor LOGIN NOSUPERUSER;
SELECT 1 AS target_table_intentionally_absent;
GRANT SELECT, INSERT ON copy_02833_t TO copy_02833_actor;
SET ROLE copy_02833_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY copy_02833_t TO 'copy_02833_out.csv';
-- primary-target-end
