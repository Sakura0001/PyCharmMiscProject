...
-- primary_obligation_id: COPY-EXT|00054|catalog_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS copy_00054_t;
DROP ROLE IF EXISTS copy_00054_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE copy_00054_actor LOGIN NOSUPERUSER;
CREATE TABLE copy_00054_t (copy_00054_c int);
GRANT SELECT, INSERT ON copy_00054_t TO copy_00054_actor;
SET ROLE copy_00054_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY copy_00054_t FROM 'copy_00054_src.csv';
-- primary-target-end
