...
-- primary_obligation_id: COPY-EXT|02629|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS copy_02629_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE copy_02629_t (copy_02629_c int);
INSERT INTO copy_02629_t VALUES (1), (2), (3);
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY (SELECT count(*) AS copy_02629_c FROM copy_02629_t) TO STDOUT;
-- primary-target-end
