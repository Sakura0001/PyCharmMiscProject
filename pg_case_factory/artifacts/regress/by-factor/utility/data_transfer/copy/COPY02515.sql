...
-- primary_obligation_id: COPY-EXT|02515|effect_query|rollback
...
COPY (SELECT count(*) AS copy_02515_c FROM copy_02515_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS no_table FROM pg_catalog.pg_class WHERE relname = 'copy_02515_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
