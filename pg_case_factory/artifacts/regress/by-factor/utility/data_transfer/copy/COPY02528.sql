...
-- primary_obligation_id: COPY-EXT|02528|effect_query|drop_objects
...
COPY (SELECT count(*) AS copy_02528_c FROM copy_02528_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS no_table FROM pg_catalog.pg_class WHERE relname = 'copy_02528_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
