...
-- primary_obligation_id: COPY-EXT|03357|effect_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS copy_03357_t;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE VIEW copy_03357_t AS SELECT 1 AS copy_03357_c;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COPY。
-- primary-target-begin
COPY public.copy_03357_t TO 'copy_03357_out.csv';
-- primary-target-end
...
SELECT count(*) = 0 AS no_table FROM pg_catalog.pg_class WHERE relname = 'copy_03357_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
RESET search_path;
DROP VIEW IF EXISTS copy_03357_t;
