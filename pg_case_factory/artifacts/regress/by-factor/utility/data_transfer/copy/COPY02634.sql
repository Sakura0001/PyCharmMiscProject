...
-- primary_obligation_id: COPY-EXT|02634|catalog_query|reset_state
...
COPY (SELECT count(*) AS copy_02634_c FROM copy_02634_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS table_absent FROM pg_catalog.pg_class WHERE relname = 'copy_02634_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
RESET search_path;
