...
-- primary_obligation_id: COPY-EXT|02542|returned_rows|rollback
...
COPY (SELECT count(*) AS copy_02542_c FROM copy_02542_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS no_copy FROM pg_catalog.pg_class WHERE relname = 'copy_02542_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
ROLLBACK;
