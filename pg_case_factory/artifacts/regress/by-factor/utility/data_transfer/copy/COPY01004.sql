...
-- primary_obligation_id: COPY-EXT|01004|effect_query|drop_objects
...
COPY public.copy_01004_t FROM 'copy_01004_src.csv';
-- primary-target-end
...
SELECT count(*) = 0 AS no_table FROM pg_catalog.pg_class WHERE relname = 'copy_01004_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
