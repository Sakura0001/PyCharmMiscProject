...
-- primary_obligation_id: COPY-EXT|02541|effect_query|reset_state
...
COPY (SELECT count(*) AS copy_02541_c FROM copy_02541_t) TO STDOUT;
-- primary-target-end
...
SELECT count(*) = 0 AS no_table FROM pg_catalog.pg_class WHERE relname = 'copy_02541_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
RESET search_path;
