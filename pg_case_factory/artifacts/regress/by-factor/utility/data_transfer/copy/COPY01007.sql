...
-- primary_obligation_id: COPY-EXT|01007|returned_rows|drop_objects
...
COPY public.copy_01007_t FROM 'copy_01007_src.csv';
-- primary-target-end
...
SELECT count(*) = 0 AS no_copy FROM pg_catalog.pg_class WHERE relname = 'copy_01007_t' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
