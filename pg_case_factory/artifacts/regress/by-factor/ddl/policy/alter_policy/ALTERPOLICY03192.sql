...
-- primary_obligation_id: AP-EXT|03192|modify_roles|error_assertion|drop_table
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS alterpolicy_03192_no_such_table CASCADE;
DROP ROLE IF EXISTS alterpolicy_03192_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和策略以及因子专用夹具。
CREATE ROLE alterpolicy_03192_owner LOGIN NOSUPERUSER;
SELECT 1 AS target_table_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信心的 ALTER POLICY。
-- primary-target-begin
ALTER POLICY "alterpolicy_03192_pol" ON alterpolicy_03192_no_such_table TO CURRENT_USER;
-- primary-target-end
