...
-- primary_obligation_id: DROPSERVER-EXT|01534|drop_server|error_assertion|drop_server_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 2BP01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP USER MAPPING IF EXISTS FOR public SERVER "user";
DROP SERVER IF EXISTS "user" CASCADE;
DROP FOREIGN DATA WRAPPER IF EXISTS dropserver_01534_fdw CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地外部服务器和因子专用夹具。
CREATE FOREIGN DATA WRAPPER dropserver_01534_fdw;
CREATE SERVER "user" FOREIGN DATA WRAPPER dropserver_01534_fdw;
