...
-- primary_obligation_id: DROPSERVER-EXT|02106|drop_server|error_assertion|drop_server_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SERVER IF EXISTS "user" CASCADE;
DROP SERVER IF EXISTS "order" CASCADE;
DROP FOREIGN DATA WRAPPER IF EXISTS dropserver_02106_fdw CASCADE;
DROP OWNED BY dropserver_02106_actor;
DROP ROLE IF EXISTS dropserver_02106_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地外部服务器和因子专用夹具。
CREATE ROLE dropserver_02106_actor LOGIN NOSUPERUSER;
CREATE FOREIGN DATA WRAPPER dropserver_02106_fdw;
CREATE SERVER "user" FOREIGN DATA WRAPPER dropserver_02106_fdw;
SET ROLE dropserver_02106_actor;
