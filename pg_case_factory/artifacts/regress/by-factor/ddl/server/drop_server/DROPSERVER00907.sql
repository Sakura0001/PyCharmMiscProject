...
-- primary_obligation_id: DROPSERVER-EXT|00907|drop_server|error_assertion|drop_user_mapping_then_drop_server
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP USER MAPPING IF EXISTS FOR public SERVER "user";
DROP SERVER IF EXISTS "user" CASCADE;
DROP FOREIGN DATA WRAPPER IF EXISTS dropserver_00907_fdw CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地外部服务器和因子专用夹具。
CREATE FOREIGN DATA WRAPPER dropserver_00907_fdw;
CREATE SERVER "user" FOREIGN DATA WRAPPER dropserver_00907_fdw;
