...
-- primary_obligation_id: DROPSERVER-EXT|00219|drop_server|error_assertion|drop_user_mapping_then_drop_server
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SERVER IF EXISTS "user" CASCADE;
DROP FOREIGN DATA WRAPPER IF EXISTS dropserver_00219_fdw CASCADE;
DROP OWNED BY dropserver_00219_actor;
DROP ROLE IF EXISTS dropserver_00219_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地外部服务器和因子专用夹具。
CREATE ROLE dropserver_00219_actor LOGIN NOSUPERUSER;
CREATE FOREIGN DATA WRAPPER dropserver_00219_fdw;
SELECT 1 AS target_server_intentionally_absent;
SET ROLE dropserver_00219_actor;
