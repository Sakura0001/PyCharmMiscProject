...
-- primary_obligation_id: DROPSERVER-EXT|00420|drop_server|pg_foreign_server_catalog|drop_server_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SERVER IF EXISTS "user" CASCADE;
DROP SERVER IF EXISTS "order" CASCADE;
DROP FOREIGN DATA WRAPPER IF EXISTS dropserver_00420_fdw CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地外部服务器和因子专用夹具。
CREATE FOREIGN DATA WRAPPER dropserver_00420_fdw;
CREATE SERVER "user" FOREIGN DATA WRAPPER dropserver_00420_fdw;
