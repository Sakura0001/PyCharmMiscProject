...
-- ++
-- --------------------------------------------------------
-- case_id: ALTEROPERATORFAMILY01952
...
-- primary_obligation_id: AOF-EXT|01952|drop_elements|effect_query|drop_objects
...
DO $$ DECLARE r record; BEGIN
  FOR r IN SELECT n.nspname, o.opfname, a.amname
    ...
    WHERE o.opfname LIKE 'alteroperatorfamily_01952_%'
  LOOP
...
-- 2. 构造本编号对象与角色。
CREATE SCHEMA IF NOT EXISTS alteroperatorfamily_01952_sch;
CREATE SCHEMA IF NOT EXISTS alteroperatorfamily_01952_target_sch;
\set ON_ERROR_STOP off
-- primary-target-begin
ALTER OPERATOR FAMILY alteroperatorfamily_01952_opf USING btree DROP OPERATOR 1 (NONE, integer);
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=42601
-- 4. 断言与目录审计。
SELECT count(*) AS opf_present FROM pg_catalog.pg_opfamily WHERE opfname = 'alteroperatorfamily_01952_opf' AND opfmethod = (SELECT oid FROM pg_catalog.pg_am WHERE amname = 'btree') ORDER BY count(*) LIMIT 1;
...
DO $$ DECLARE r record; BEGIN
  ...
  END LOOP;
END $$;
DROP SCHEMA IF EXISTS alteroperatorfamily_01952_sch CASCADE;
DROP SCHEMA IF EXISTS alteroperatorfamily_01952_target_sch CASCADE;
