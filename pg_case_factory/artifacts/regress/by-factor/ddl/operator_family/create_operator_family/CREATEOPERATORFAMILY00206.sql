...
-- ++
-- --------------------------------------------------------
-- case_id: CREATEOPERATORFAMILY00206
...
-- primary_obligation_id: COF-EXT|00206|catalog_query|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 3F000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OPERATOR FAMILY IF EXISTS pgcf_noschema.createoperatorfamily_00206_opfam CASCADE;
RESET ROLE;
...
CREATE OPERATOR FAMILY pgcf_noschema.createoperatorfamily_00206_opfam USING gin;
-- primary-target-end
...
SELECT count(*) = 0 AS opfamily_state FROM pg_catalog.pg_opfamily WHERE opfname = 'createoperatorfamily_00206_opfam' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP OPERATOR FAMILY IF EXISTS pgcf_noschema.createoperatorfamily_00206_opfam CASCADE;
