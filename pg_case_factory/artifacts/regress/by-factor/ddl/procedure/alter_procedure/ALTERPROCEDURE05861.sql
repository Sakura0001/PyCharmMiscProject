...
-- primary_obligation_id: AP-EXT|05861|rename|pg_get_functiondef|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_05861_Mixed Procedure"(integer) RENAME TO "alterprocedure_05861_from";
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_05861_Mixed Procedure' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
