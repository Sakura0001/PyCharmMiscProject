...
-- primary_obligation_id: AP-EXT|06619|rename|pg_get_functiondef|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE "alterprocedure_06619_select"(integer) RENAME TO alterprocedure_06619_renamed;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_06619_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
