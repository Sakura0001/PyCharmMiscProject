...
-- primary_obligation_id: AP-EXT|12417|security_invoker|pg_proc_catalog_query|DROP_PROCEDURE_IF_EXISTS
...
ALTER PROCEDURE alterprocedure_12417_proc SECURITY INVOKER;
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_12417_proc' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
