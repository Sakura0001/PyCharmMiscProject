...
-- primary_obligation_id: AP-EXT|12262|reset_all|pg_get_functiondef|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE "alterprocedure_12262_select" RESET ALL RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_12262_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
