...
-- primary_obligation_id: AP-EXT|12119|reset_all|pg_proc_catalog_query|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_12119_Mixed Procedure" RESET ALL;
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_12119_Mixed Procedure' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
