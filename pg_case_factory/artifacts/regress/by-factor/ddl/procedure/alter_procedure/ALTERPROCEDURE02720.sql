...
-- primary_obligation_id: AP-EXT|02720|set_parameter|pg_get_functiondef|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_02720_select"(integer) SET work_mem TO 100 RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_02720_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
