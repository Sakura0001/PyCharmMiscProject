...
-- primary_obligation_id: AP-EXT|02648|reset_parameter|pg_get_functiondef|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_02648_select"(integer) RESET work_mem;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_02648_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
