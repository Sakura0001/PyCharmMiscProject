...
-- primary_obligation_id: AP-EXT|00724|depends_on_extension|pg_get_functiondef|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE "alterprocedure_00724_select"(integer) NO DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_00724_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
