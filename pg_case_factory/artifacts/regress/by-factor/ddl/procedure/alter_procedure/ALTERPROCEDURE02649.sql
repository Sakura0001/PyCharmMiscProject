...
-- primary_obligation_id: AP-EXT|02649|reset_parameter|pg_get_functiondef|DROP_PROCEDURE_IF_EXISTS
...
ALTER PROCEDURE "alterprocedure_02649_select"(integer) RESET work_mem;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_02649_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
