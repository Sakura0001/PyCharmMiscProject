...
-- primary_obligation_id: AP-EXT|00736|depends_on_extension|pg_proc_catalog_query|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE alterprocedure_00736_proc(integer) NO DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_00736_proc' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
