...
-- primary_obligation_id: AP-EXT|04241|owner|pg_get_functiondef|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_04241_Mixed Procedure"(integer) OWNER TO CURRENT_ROLE;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_04241_Mixed Procedure' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
