...
-- primary_obligation_id: AP-EXT|04485|owner|pg_get_functiondef|DROP_PROCEDURE_IF_EXISTS
...
ALTER PROCEDURE alterprocedure_04485_proc(integer) OWNER TO SESSION_USER;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_04485_proc' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
