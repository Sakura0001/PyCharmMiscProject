...
-- primary_obligation_id: AP-EXT|02624|set_parameter|pg_proc_catalog_query|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_02624_Mixed Procedure"(integer) SET work_mem TO 100 RESTRICT;
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_02624_Mixed Procedure' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
