...
-- primary_obligation_id: AP-EXT|05180|rename|pg_proc_catalog_query|DROP_PROCEDURE
...
ALTER PROCEDURE "alterprocedure_05180_select"(integer) RENAME TO "alterprocedure_05180_Mixed Name";
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_05180_select' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
