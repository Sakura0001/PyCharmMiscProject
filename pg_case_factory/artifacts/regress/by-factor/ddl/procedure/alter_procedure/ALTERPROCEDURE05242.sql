...
-- primary_obligation_id: AP-EXT|05242|rename|pg_get_functiondef|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE alterprocedure_05242_proc(integer) RENAME TO "alterprocedure_05242_Mixed Name";
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_05242_proc' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
