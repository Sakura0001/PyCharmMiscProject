...
-- primary_obligation_id: AP-EXT|06604|rename|pg_proc_catalog_query|DROP_PROCEDURE_CASCADE
...
ALTER PROCEDURE "alterprocedure_06604_select"(integer) RENAME TO alterprocedure_06604_renamed;
-- primary-target-end
...
SELECT p.prokind, p.prosecdef FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterprocedure_06604_select' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
