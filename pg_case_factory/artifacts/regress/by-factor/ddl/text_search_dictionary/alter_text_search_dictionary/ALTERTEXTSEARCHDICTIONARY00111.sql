...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHDICTIONARY00111
...
-- primary_obligation_id: ATSD-EXT|00111|catalog_query_pg_ts_dict|revert_rename
...
ALTER TEXT SEARCH DICTIONARY alter_text_search_dictionary_00111_no_such_dict (stopword);
-- primary-target-end
...
SELECT count(*) = 0 AS dict_state FROM pg_catalog.pg_ts_dict WHERE dictname = 'alter_text_search_dictionary_00111_no_such_dict' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
