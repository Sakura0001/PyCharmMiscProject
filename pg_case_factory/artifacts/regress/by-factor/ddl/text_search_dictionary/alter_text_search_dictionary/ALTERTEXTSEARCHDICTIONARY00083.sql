...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHDICTIONARY00083
...
-- primary_obligation_id: ATSD-EXT|00083|catalog_query_pg_ts_dict|drop_text_search_dictionary
...
ALTER TEXT SEARCH DICTIONARY alter_text_search_dictionary_00083_no_such_dict (dummy);
-- primary-target-end
...
SELECT count(*) = 0 AS dict_state FROM pg_catalog.pg_ts_dict WHERE dictname = 'alter_text_search_dictionary_00083_no_such_dict' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
