...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHDICTIONARY00145
...
-- primary_obligation_id: ATSD-EXT|00145|option_query|revert_option
...
ALTER TEXT SEARCH DICTIONARY alter_text_search_dictionary_00145_no_such_dict (stopword = 'english');
-- primary-target-end
...
SELECT count(*) = 0 AS option_state FROM pg_catalog.pg_ts_dict WHERE dictname = 'alter_text_search_dictionary_00145_no_such_dict' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
