...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHDICTIONARY00434
...
-- primary_obligation_id: ATSD-EXT|00434|option_query|role_cleanup
...
ALTER TEXT SEARCH DICTIONARY alter_text_search_dictionary_00434_no_such_dict OWNER TO CURRENT_ROLE;
-- primary-target-end
...
SELECT count(*) = 0 AS option_state FROM pg_catalog.pg_ts_dict WHERE dictname = 'alter_text_search_dictionary_00434_no_such_dict' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
