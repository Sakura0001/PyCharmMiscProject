...
-- primary_obligation_id: ASYS-EXT|00724|show_command|alter_system_reset_parameter
-- expected_outcome: expected_failure
-- expected_sqlstate: 22023
-- 1. 清理本编号对象，保证脚本可重复执行。
ALTER SYSTEM RESET alter_system_00724_custom.param;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER SYSTEM。
-- primary-target-begin
ALTER SYSTEM SET alter_system_00724_custom.param = 'not_a_number';
-- primary-target-end
...
SHOW alter_system_00724_custom.param;
-- 5. 清理全部本编号对象。
ALTER SYSTEM RESET alter_system_00724_custom.param;
