...
-- primary_obligation_id: CLANG-EXT|02110|catalog_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42804
-- 1. 清理本编号对象，保证脚本可重复执行。
SELECT 1 AS residual_check_no_objects;
DROP LANGUAGE IF EXISTS "createlanguage_02110_QLang" CASCADE;
RESET ROLE;
...
CREATE OR REPLACE LANGUAGE "createlanguage_02110_QLang" HANDLER createlanguage_02110_handler;
-- primary-target-end
...
SELECT count(*) = 0 AS language_state FROM pg_catalog.pg_language WHERE lanname = 'createlanguage_02110_QLang' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP LANGUAGE IF EXISTS "createlanguage_02110_QLang" CASCADE;
