...
-- ++
-- --------------------------------------------------------
-- case_id: CREATEFOREIGNDATAWRAPPER01706
...
-- primary_obligation_id: CFDW-EXT|01706|error_assertion|role_cleanup
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP FOREIGN DATA WRAPPER IF EXISTS createforeigndatawrapper_01706_fdw CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE FOREIGN DATA WRAPPER。
-- primary-target-begin
CREATE FOREIGN DATA WRAPPER createforeigndatawrapper_01706_fdw OPTIONS ('createforeigndatawrapper_01706_opt1' 'val1');
-- primary-target-end
