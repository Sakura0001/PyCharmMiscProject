...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHTEMPLATE00131
...
-- primary_obligation_id: ATST-EXT|00131|error_assertion|drop_text_search_template
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS altertextsearchtemplate_00131_sch CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA altertextsearchtemplate_00131_sch;
SELECT 1 AS target_template_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER TEXT SEARCH TEMPLATE。
-- primary-target-begin
ALTER TEXT SEARCH TEMPLATE altertextsearchtemplate_00131_no_such_tmpl SET SCHEMA altertextsearchtemplate_00131_sch;
-- primary-target-end
