...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHTEMPLATE00138
...
-- primary_obligation_id: ATST-EXT|00138|error_assertion|revert_rename
-- expected_outcome: expected_failure
-- expected_sqlstate: 3F000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH TEMPLATE IF EXISTS "altertextsearchtemplate_00138_Mixed Tmpl";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TEXT SEARCH TEMPLATE "altertextsearchtemplate_00138_Mixed Tmpl" (lexize = dsimple_lexize);
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER TEXT SEARCH TEMPLATE。
-- primary-target-begin
ALTER TEXT SEARCH TEMPLATE "altertextsearchtemplate_00138_Mixed Tmpl" SET SCHEMA altertextsearchtemplate_00138_no_such_sch;
-- primary-target-end
