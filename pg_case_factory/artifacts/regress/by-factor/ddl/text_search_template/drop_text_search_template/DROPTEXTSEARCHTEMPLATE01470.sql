...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHTEMPLATE01470
...
-- primary_obligation_id: DROPTEXTSEARCHTEMPLATE-EXT|01470|drop_text_search_template|notice_assertion|manual_dependency_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH DICTIONARY IF EXISTS droptextsearchtemplate_01470_dict CASCADE;
DROP TEXT SEARCH TEMPLATE IF EXISTS "select" CASCADE;
DROP OWNED BY droptextsearchtemplate_01470_actor;
DROP ROLE IF EXISTS droptextsearchtemplate_01470_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地模板和因子专用夹具。
CREATE ROLE droptextsearchtemplate_01470_actor LOGIN NOSUPERUSER;
CREATE TEXT SEARCH TEMPLATE "select" INIT (dsimple_init) LEXIZE (dsimple_lexize);
CREATE TEXT SEARCH DICTIONARY droptextsearchtemplate_01470_dict TEMPLATE "select";
SET ROLE droptextsearchtemplate_01470_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP TEXT SEARCH TEMPLATE。
-- primary-target-begin
DROP TEXT SEARCH TEMPLATE "select" CASCADE;
-- primary-target-end
