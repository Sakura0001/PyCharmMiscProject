...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHTEMPLATE02403
...
-- primary_obligation_id: DROPTEXTSEARCHTEMPLATE-EXT|02403|drop_text_search_template|error_assertion|cascade_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH TEMPLATE IF EXISTS "select" CASCADE;
DROP OWNED BY droptextsearchtemplate_02403_actor;
DROP ROLE IF EXISTS droptextsearchtemplate_02403_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地模板和因子专用夹具。
CREATE ROLE droptextsearchtemplate_02403_actor LOGIN NOSUPERUSER;
CREATE TEXT SEARCH TEMPLATE "select" INIT (dsimple_init) LEXIZE (dsimple_lexize);
SET ROLE droptextsearchtemplate_02403_actor;
