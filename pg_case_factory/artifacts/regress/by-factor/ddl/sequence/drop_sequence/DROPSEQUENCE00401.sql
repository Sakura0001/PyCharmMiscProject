...
-- primary_obligation_id: DROPSEQUENCE-EXT|00401|drop_sequence|pg_class_query|manual_cleanup
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SEQUENCE IF EXISTS dropsequence_00401_seq CASCADE;
...
SELECT count(*) = 0 AS sequence_absent FROM pg_catalog.pg_class WHERE relname = 'dropsequence_00401_seq' AND relkind = 'S' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP SEQUENCE IF EXISTS dropsequence_00401_seq CASCADE;
