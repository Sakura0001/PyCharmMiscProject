...
-- primary_obligation_id: CSQ-EXT|05656|nextval_call|DROP_SEQUENCE_CASCADE|naming_options
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SEQUENCE IF EXISTS "select" CASCADE;
RESET ROLE;
DROP ROLE IF EXISTS createsequence_05656_actor;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE createsequence_05656_actor LOGIN NOSUPERUSER;
SET ROLE createsequence_05656_actor;
...
DROP OWNED BY createsequence_05656_actor CASCADE;
DROP ROLE IF EXISTS createsequence_05656_actor;
