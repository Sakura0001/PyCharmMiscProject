...
-- primary_obligation_id: CSQ-EXT|06233|nextval_call|DROP_SEQUENCE|naming_options
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS createsequence_06233_tbl CASCADE;
DROP SEQUENCE IF EXISTS "select" CASCADE;
RESET ROLE;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE createsequence_06233_tbl (id bigint);
CREATE SEQUENCE "select";
SELECT 1 AS pre_target_boundary;
-- 3. 执行唯一获得覆盖信用的 CREATE SEQUENCE。
-- primary-target-begin
CREATE SEQUENCE IF NOT EXISTS "select" OWNED BY createsequence_06233_tbl.id;
-- primary-target-end
