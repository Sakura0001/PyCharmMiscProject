...
-- primary_obligation_id: CSQ-EXT|06204|nextval_call|DROP_SEQUENCE_IF_EXISTS|naming_options
-- expected_outcome: expected_failure
-- expected_sqlstate: 42710
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS createsequence_06204_tbl CASCADE;
DROP SEQUENCE IF EXISTS "select" CASCADE;
RESET ROLE;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE createsequence_06204_tbl (id bigint);
CREATE SEQUENCE "select";
\set ON_ERROR_STOP off
SELECT 1 AS pre_target_boundary;
-- 3. 执行唯一获得覆盖信用的 CREATE SEQUENCE。
-- primary-target-begin
CREATE SEQUENCE "select" OWNED BY createsequence_06204_tbl.id;
-- primary-target-end
