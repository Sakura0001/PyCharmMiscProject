...
-- primary_obligation_id: CSQ-EXT|06716|pg_class_catalog_query|DROP_SEQUENCE|naming_options
-- expected_outcome: expected_failure
-- expected_sqlstate: 42710
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SEQUENCE IF EXISTS "select" CASCADE;
DROP SCHEMA IF EXISTS createsequence_06716_schema CASCADE;
RESET ROLE;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA createsequence_06716_schema;
...
SELECT count(*) > 0 AS seq_state FROM pg_catalog.pg_class WHERE relname = 'select' AND relkind = 'S' AND relnamespace = 'createsequence_06716_schema'::regnamespace ORDER BY count(*);
-- 5. 清理全部本编号对象。
