...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00774
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00774|drop_text_search_parser|notice_assertion|manual_dependency_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 2BP01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH CONFIGURATION IF EXISTS droptextsearchparser_00774_config CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地解析器和因子专用夹具。
SELECT 1 AS setup_boundary;
SELECT 1 AS target_parser_intentionally_absent;
CREATE TEXT SEARCH CONFIGURATION droptextsearchparser_00774_config (PARSER = "user");
