...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00531
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00531|drop_text_search_parser|error_assertion|cascade_cleanup
...
DROP TEXT SEARCH PARSER droptextsearchparser_00531_noexist CASCADE;
-- primary-target-end
...
SELECT count(*) = 0 AS parser_absent FROM pg_catalog.pg_ts_parser WHERE prsname = 'droptextsearchparser_00531_noexist' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
