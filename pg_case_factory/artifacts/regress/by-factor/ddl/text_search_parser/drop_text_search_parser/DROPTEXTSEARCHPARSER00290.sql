...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00290
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00290|drop_text_search_parser|catalog_query|manual_dependency_cleanup
...
DROP TEXT SEARCH PARSER IF EXISTS droptextsearchparser_00290_parser;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS parser_absent FROM pg_catalog.pg_ts_parser WHERE prsname = 'droptextsearchparser_00290_parser' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
