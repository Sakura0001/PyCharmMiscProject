...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00821
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00821|drop_text_search_parser|notice_assertion|cascade_cleanup
...
DROP TEXT SEARCH PARSER droptextsearchparser_00821_noexist RESTRICT;
-- primary-target-end
...
SELECT count(*) = 0 AS parser_absent FROM pg_catalog.pg_ts_parser WHERE prsname = 'droptextsearchparser_00821_noexist' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
