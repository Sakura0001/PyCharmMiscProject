...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00602
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00602|drop_text_search_parser|catalog_query|manual_dependency_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OWNED BY droptextsearchparser_00602_actor;
DROP ROLE IF EXISTS droptextsearchparser_00602_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地解析器和因子专用夹具。
SELECT 1 AS setup_boundary;
CREATE ROLE droptextsearchparser_00602_actor LOGIN NOSUPERUSER;
SELECT 1 AS target_parser_intentionally_absent;
SET ROLE droptextsearchparser_00602_actor;
