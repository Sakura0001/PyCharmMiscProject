...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHPARSER00864
...
-- primary_obligation_id: DROPTEXTSEARCHPARSER-EXT|00864|drop_text_search_parser|notice_assertion|manual_dependency_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH PARSER IF EXISTS "user" CASCADE;
DROP OWNED BY droptextsearchparser_00864_actor;
DROP ROLE IF EXISTS droptextsearchparser_00864_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地解析器和因子专用夹具。
SELECT 1 AS setup_boundary;
CREATE ROLE droptextsearchparser_00864_actor LOGIN NOSUPERUSER;
...
SET ROLE droptextsearchparser_00864_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP TEXT SEARCH PARSER。
-- primary-target-begin
DROP TEXT SEARCH PARSER "user" RESTRICT;
-- primary-target-end
