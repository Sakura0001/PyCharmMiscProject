...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHPARSER00227
...
-- primary_obligation_id: ATSP-EXT|00227|error_assertion|drop_text_search_parser
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS alter_text_search_parser_00227_tgt_schema CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA alter_text_search_parser_00227_tgt_schema;
SELECT 1 AS target_parser_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER TEXT SEARCH PARSER。
-- primary-target-begin
ALTER TEXT SEARCH PARSER alter_text_search_parser_00227_no_such_parser SET SCHEMA alter_text_search_parser_00227_tgt_schema;
-- primary-target-end
