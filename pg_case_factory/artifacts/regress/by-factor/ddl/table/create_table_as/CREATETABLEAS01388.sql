...
-- primary_obligation_id: CTAS-EXT|01388|SELECT_count|DROP_TABLE_CASCADE|secondary_query_shape
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS createtableas_01388_t CASCADE;
...
CREATE TEMP TABLE createtableas_01388_t AS VALUES (1, 2), (3, 4);
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) AS row_count FROM createtableas_01388_t ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS createtableas_01388_t CASCADE;
