...
-- primary_obligation_id: CTAS-EXT|01159|information_schema_columns|DROP_TABLE_IF_EXISTS|secondary_query_shape
-- expected_outcome: expected_failure
-- expected_sqlstate: 42601
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS createtableas_01159_t CASCADE;
...
CREATE GLOBAL TEMPORARY TABLE createtableas_01159_t AS EXECUTE createtableas_01159_prep;
-- primary-target-end
...
SELECT count(*) AS target_created_count FROM pg_catalog.pg_class c WHERE c.relname = 'createtableas_01159_t' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS createtableas_01159_t CASCADE;
