...
-- primary_obligation_id: CLUSTER-EXT|03613|pg_stat_progress_cluster|none
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS cluster_03613_t CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE cluster_03613_t (id integer);
CREATE INDEX cluster_03613_i ON cluster_03613_t (id);
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CLUSTER。
-- primary-target-begin
CLUSTER (VERBOSE FALSE) cluster_03613_t;
-- primary-target-end
