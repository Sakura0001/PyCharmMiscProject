...
-- primary_obligation_id: CLUSTER-EXT|00149|pg_stat_progress_cluster|none
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS cluster_00149_t, cluster_00149_t_p1 CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE cluster_00149_t (id integer) PARTITION BY RANGE (id);
CREATE TABLE cluster_00149_t_p1 PARTITION OF cluster_00149_t FOR VALUES FROM (0) TO (1000);
CREATE INDEX cluster_00149_i ON cluster_00149_t (id);
CLUSTER cluster_00149_t USING cluster_00149_i;
BEGIN;
