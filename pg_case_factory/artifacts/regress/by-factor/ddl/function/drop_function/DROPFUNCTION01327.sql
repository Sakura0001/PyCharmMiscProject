...
-- primary_obligation_id: DROPFUNCTION-EXT|01327|drop_function|pg_proc_catalog_query|DROP_FUNCTION_cascade
...
DROP FUNCTION IF EXISTS dropfunction_01327_fn(integer);
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS function_absent FROM pg_catalog.pg_proc WHERE proname = 'dropfunction_01327_fn' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
