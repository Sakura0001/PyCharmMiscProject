...
-- primary_obligation_id: AF-EXT|03577|rename|pg_get_functiondef|DROP_FUNCTION
...
ALTER FUNCTION "alterfunction_03577_select"(integer) RENAME TO alterfunction_03577_renamed;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_03577_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
