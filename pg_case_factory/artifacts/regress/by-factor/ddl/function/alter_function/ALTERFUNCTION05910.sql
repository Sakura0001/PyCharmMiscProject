...
-- primary_obligation_id: AF-EXT|05910|immutable|pg_get_functiondef|DROP_FUNCTION_CASCADE
...
ALTER FUNCTION alterfunction_05910_fn(integer) IMMUTABLE RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_05910_fn' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
