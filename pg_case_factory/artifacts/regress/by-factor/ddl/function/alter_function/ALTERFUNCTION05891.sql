...
-- primary_obligation_id: AF-EXT|05891|volatile|pg_get_functiondef|DROP_FUNCTION_IF_EXISTS
...
ALTER FUNCTION alterfunction_05891_fn(integer) VOLATILE;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_05891_fn' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
