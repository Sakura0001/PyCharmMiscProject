...
-- primary_obligation_id: AF-EXT|12469|volatile|pg_get_functiondef|DROP_FUNCTION
...
ALTER FUNCTION alterfunction_12469_fn VOLATILE RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_12469_fn' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
