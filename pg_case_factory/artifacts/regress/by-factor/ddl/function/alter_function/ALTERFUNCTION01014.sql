...
-- primary_obligation_id: AF-EXT|01014|called_on_null_input|pg_get_functiondef|DROP_FUNCTION_CASCADE
...
ALTER FUNCTION "alterfunction_01014_Mixed Function"(integer) CALLED ON NULL INPUT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_01014_Mixed Function' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
