...
-- primary_obligation_id: AF-EXT|11185|security_definer|pg_proc_catalog_query|DROP_FUNCTION
...
ALTER FUNCTION "alterfunction_11185_select" SECURITY DEFINER RESTRICT;
-- primary-target-end
...
SELECT p.provolatile, p.prokind, p.prosecdef, p.proleakproof, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_11185_select' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
