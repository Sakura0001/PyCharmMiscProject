...
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterfunction_11056_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地函数和因子专用夹具。
CREATE ROLE alterfunction_11056_owner LOGIN;
GRANT CREATE ON SCHEMA public TO alterfunction_11056_owner;
SET ROLE alterfunction_11056_owner;
...
ALTER FUNCTION "alterfunction_11056_select" VOLATILE;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_11056_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY alterfunction_11056_owner;
DROP ROLE IF EXISTS alterfunction_11056_owner;
