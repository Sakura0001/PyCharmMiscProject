...
-- primary_obligation_id: AF-EXT|05904|immutable|pg_proc_catalog_query|DROP_FUNCTION_CASCADE
...
ALTER FUNCTION alterfunction_05904_fn(integer) IMMUTABLE RESTRICT;
-- primary-target-end
...
SELECT p.provolatile, p.prokind, p.prosecdef, p.proleakproof, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_05904_fn' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
