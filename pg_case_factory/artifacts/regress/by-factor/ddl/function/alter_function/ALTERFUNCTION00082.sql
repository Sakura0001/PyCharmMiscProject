...
-- primary_obligation_id: AF-SFV|sfv-2390babd35633e045e4699e0|volatile
...
ALTER FUNCTION alterfunction_00082_fn(integer) VOLATILE;
-- primary-target-end
...
SELECT p.provolatile, p.prokind, p.prosecdef, p.proleakproof, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_00082_fn' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
