...
-- primary_obligation_id: AF-EXT|02714|set_parameter|pg_get_functiondef|DROP_FUNCTION_IF_EXISTS
...
ALTER FUNCTION "alterfunction_02714_select"(integer) SET work_mem TO 100 RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_02714_select' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
