...
-- primary_obligation_id: AF-EXT|05936|set_parameter|pg_get_functiondef|DROP_FUNCTION_IF_EXISTS
...
ALTER FUNCTION alterfunction_05936_fn(integer) SET work_mem TO 100 RESTRICT;
-- primary-target-end
...
SELECT pg_get_functiondef(p.oid) FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterfunction_05936_fn' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
