...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW01817
...
-- primary_obligation_id: AMV-EXT|01817|owner_to|effect_query|reset_state
...
ALTER MATERIALIZED VIEW IF EXISTS public.altermaterializedview_01817_mv OWNER TO SESSION_USER;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS effect_not_verified FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_01817_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
