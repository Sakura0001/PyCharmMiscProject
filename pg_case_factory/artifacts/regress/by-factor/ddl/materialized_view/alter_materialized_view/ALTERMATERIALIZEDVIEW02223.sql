...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW02223
...
-- primary_obligation_id: AMV-EXT|02223|owner_to|catalog_query|reset_state
...
ALTER MATERIALIZED VIEW public.altermaterializedview_02223_mv OWNER TO SESSION_USER;
-- primary-target-end
...
SELECT count(*) = 0 AS mview_absent FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_02223_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
