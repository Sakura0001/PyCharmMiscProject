...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW00908
...
-- primary_obligation_id: AMV-EXT|00908|owner_to|catalog_query|drop_objects
...
ALTER MATERIALIZED VIEW public.altermaterializedview_00908_mv OWNER TO CURRENT_USER;
-- primary-target-end
...
SELECT count(*) = 0 AS mview_absent FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_00908_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
