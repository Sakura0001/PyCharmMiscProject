...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW02225
...
-- primary_obligation_id: AMV-EXT|02225|owner_to|effect_query|reset_state
...
ALTER MATERIALIZED VIEW public.altermaterializedview_02225_mv OWNER TO SESSION_USER;
-- primary-target-end
...
SELECT count(*) = 0 AS effect_not_verified FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_02225_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
