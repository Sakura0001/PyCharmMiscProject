...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW00604
...
-- primary_obligation_id: AMV-EXT|00604|owner_to|effect_query|drop_objects
...
ALTER MATERIALIZED VIEW altermaterializedview_00604_mv OWNER TO CURRENT_ROLE;
-- primary-target-end
...
SELECT count(*) = 0 AS effect_not_verified FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_00604_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
