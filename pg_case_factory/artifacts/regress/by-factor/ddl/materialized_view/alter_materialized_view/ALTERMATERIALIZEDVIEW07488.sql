...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW07488
...
-- primary_obligation_id: AMV-EXT|07488|set_tablespace|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altermaterializedview_07488_new_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地物化视图和因子专用夹具。
CREATE ROLE altermaterializedview_07488_new_owner LOGIN;
SELECT 1 AS target_materialized_view_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER MATERIALIZED VIEW。
-- primary-target-begin
ALTER MATERIALIZED VIEW public.altermaterializedview_07488_mv SET TABLESPACE pg_default;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP OWNED BY altermaterializedview_07488_new_owner CASCADE;
DROP ROLE IF EXISTS altermaterializedview_07488_new_owner;
