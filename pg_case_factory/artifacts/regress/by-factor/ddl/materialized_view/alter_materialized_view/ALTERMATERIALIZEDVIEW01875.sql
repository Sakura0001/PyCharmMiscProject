...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW01875
...
-- primary_obligation_id: AMV-EXT|01875|owner_to|catalog_query|reset_state
...
ALTER MATERIALIZED VIEW altermaterializedview_01875_mv OWNER TO CURRENT_USER;
-- primary-target-end
...
SELECT count(*) = 0 AS mview_absent FROM pg_catalog.pg_class WHERE relname = 'altermaterializedview_01875_mv' AND relkind = 'm' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
