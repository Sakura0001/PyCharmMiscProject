...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW04375
...
-- primary_obligation_id: AMV-EXT|04375|set_access_method|error_assertion|reset_state
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altermaterializedview_04375_new_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地物化视图和因子专用夹具。
CREATE ROLE altermaterializedview_04375_new_owner LOGIN;
SELECT 1 AS target_materialized_view_intentionally_absent;
-- 3. 执行唯一获得覆盖信用的 ALTER MATERIALIZED VIEW。
-- primary-target-begin
ALTER MATERIALIZED VIEW IF EXISTS altermaterializedview_04375_mv SET ACCESS METHOD heap;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP OWNED BY altermaterializedview_04375_new_owner;
DROP ROLE IF EXISTS altermaterializedview_04375_new_owner;
