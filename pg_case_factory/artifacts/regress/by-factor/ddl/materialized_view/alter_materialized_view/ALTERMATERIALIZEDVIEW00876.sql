...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW00876
...
-- primary_obligation_id: AMV-EXT|00876|owner_to|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS altermaterializedview_00876_tbl;
DROP MATERIALIZED VIEW IF EXISTS altermaterializedview_00876_mv;
\set ON_ERROR_STOP on
-- 2. 创建完整本地物化视图和因子专用夹具。
CREATE TABLE altermaterializedview_00876_tbl AS SELECT 1 AS c;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER MATERIALIZED VIEW。
-- primary-target-begin
ALTER MATERIALIZED VIEW "altermaterializedview_00876_tbl" OWNER TO SESSION_USER;
-- primary-target-end
