...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERMATERIALIZEDVIEW03961
...
-- primary_obligation_id: AMV-EXT|03961|reset_storage_parameter|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altermaterializedview_03961_new_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地物化视图和因子专用夹具。
CREATE ROLE altermaterializedview_03961_new_owner LOGIN;
SELECT 1 AS target_materialized_view_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER MATERIALIZED VIEW。
-- primary-target-begin
ALTER MATERIALIZED VIEW public.altermaterializedview_03961_mv RESET (autovacuum_enabled);
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP OWNED BY altermaterializedview_03961_new_owner;
DROP ROLE IF EXISTS altermaterializedview_03961_new_owner;
