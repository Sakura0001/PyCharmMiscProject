...
-- ++
-- --------------------------------------------------------
-- case_id: REFRESHMATERIALIZEDVIEW001423
...
-- primary_obligation_id: REFRESHMATERIALIZEDVIEW-EXT|001423|catalog_query|drop_objects
...
REFRESH MATERIALIZED VIEW CONCURRENTLY refreshmaterializedview_001423_no_such_mv WITH NO DATA;
-- primary-target-end
...
SELECT count(*) AS matview_relation_count FROM pg_catalog.pg_class c WHERE c.relname = 'refreshmaterializedview_001423_mv' AND c.relkind = 'm' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
