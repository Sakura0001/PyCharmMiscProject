...
-- ++
-- --------------------------------------------------------
-- case_id: REFRESHMATERIALIZEDVIEW000080
...
-- primary_obligation_id: REFRESHMATERIALIZEDVIEW-EXT|000080|catalog_query|reset_state
...
REFRESH MATERIALIZED VIEW refreshmaterializedview_000080_no_such_mv WITH DATA;
-- primary-target-end
...
SELECT count(*) AS matview_relation_count FROM pg_catalog.pg_class c WHERE c.relname = 'refreshmaterializedview_000080_mv' AND c.relkind = 'm' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
