...
-- ++
-- --------------------------------------------------------
-- case_id: REFRESHMATERIALIZEDVIEW001372
...
-- primary_obligation_id: REFRESHMATERIALIZEDVIEW-EXT|001372|returned_rows|reset_state
...
REFRESH MATERIALIZED VIEW CONCURRENTLY refreshmaterializedview_001372_no_such_mv WITH NO DATA;
-- primary-target-end
...
SELECT count(*) AS returned_row_count FROM refreshmaterializedview_001372_mv ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
