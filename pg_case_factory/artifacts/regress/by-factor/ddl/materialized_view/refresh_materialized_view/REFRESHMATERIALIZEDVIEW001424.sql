...
-- ++
-- --------------------------------------------------------
-- case_id: REFRESHMATERIALIZEDVIEW001424
...
-- primary_obligation_id: REFRESHMATERIALIZEDVIEW-EXT|001424|catalog_query|reset_state
...
REFRESH MATERIALIZED VIEW CONCURRENTLY refreshmaterializedview_001424_no_such_mv WITH NO DATA;
-- primary-target-end
...
SELECT count(*) AS matview_relation_count FROM pg_catalog.pg_class c WHERE c.relname = 'refreshmaterializedview_001424_mv' AND c.relkind = 'm' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
