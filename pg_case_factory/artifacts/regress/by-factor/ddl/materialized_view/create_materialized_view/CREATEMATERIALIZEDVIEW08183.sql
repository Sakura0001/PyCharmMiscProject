...
-- ++
-- --------------------------------------------------------
-- case_id: CREATEMATERIALIZEDVIEW08183
...
-- primary_obligation_id: CMATVIEW-EXT|08183|error_assertion|drop_objects
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP MATERIALIZED VIEW IF EXISTS public.creatematerializedview_08183_mv;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE MATERIALIZED VIEW。
-- primary-target-begin
CREATE MATERIALIZED VIEW IF NOT EXISTS public.creatematerializedview_08183_mv (c1) AS SELECT 1 WITH DATA;
-- primary-target-end
