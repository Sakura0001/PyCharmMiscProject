...
-- ++
-- --------------------------------------------------------
-- case_id: CREATEMATERIALIZEDVIEW03703
...
-- primary_obligation_id: CMATVIEW-EXT|03703|error_assertion|drop_objects
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP MATERIALIZED VIEW IF EXISTS creatematerializedview_03703_mv;
\set ON_ERROR_STOP on
SELECT 1 AS setup_boundary;
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE MATERIALIZED VIEW。
-- primary-target-begin
CREATE MATERIALIZED VIEW creatematerializedview_03703_mv WITH (autovacuum_enabled=true) AS SELECT 1::int AS c1, 'x'::text AS c2 WITH DATA;
-- primary-target-end
