...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHCONFIGURATION00658
...
-- primary_obligation_id: DROPTEXTSEARCHCONFIGURATION-EXT|00658|drop_text_search_configuration|catalog_query|manual_dependency_cleanup
...
DROP TEXT SEARCH CONFIGURATION IF EXISTS droptextsearchconfiguration_00658_cfg;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS config_absent FROM pg_catalog.pg_ts_config WHERE cfgname = 'droptextsearchconfiguration_00658_cfg' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
