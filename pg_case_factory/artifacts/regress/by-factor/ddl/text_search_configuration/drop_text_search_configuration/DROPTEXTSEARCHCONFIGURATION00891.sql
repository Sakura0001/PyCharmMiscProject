...
-- ++
-- --------------------------------------------------------
-- case_id: DROPTEXTSEARCHCONFIGURATION00891
...
-- primary_obligation_id: DROPTEXTSEARCHCONFIGURATION-EXT|00891|drop_text_search_configuration|catalog_query|cascade_cleanup
...
DROP TEXT SEARCH CONFIGURATION IF EXISTS droptextsearchconfiguration_00891_cfg CASCADE;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS config_absent FROM pg_catalog.pg_ts_config WHERE cfgname = 'droptextsearchconfiguration_00891_cfg' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
