...
-- ++
-- --------------------------------------------------------
-- case_id: ALTERTEXTSEARCHCONFIGURATION04751
...
-- primary_obligation_id: ATSC-EXT|04751|error_assertion|revert_rename
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS alter_text_search_configuration_04751_schema CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA alter_text_search_configuration_04751_schema;
SELECT 1 AS target_config_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信度的 ALTER TEXT SEARCH CONFIGURATION。
-- primary-target-begin
ALTER TEXT SEARCH CONFIGURATION alter_text_search_configuration_04751_nosuchcfg SET SCHEMA alter_text_search_configuration_04751_schema;
-- primary-target-end
