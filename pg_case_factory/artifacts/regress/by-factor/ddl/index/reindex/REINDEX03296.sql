...
-- primary_obligation_id: REINDEX-EXT|03296|search_path_sandbox|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_03296_t CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE reindex_03296_t (id integer);
CREATE INDEX reindex_03296_i ON reindex_03296_t (id);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX INDEX CONCURRENTLY reindex_03296_t;
-- primary-target-end
