...
-- primary_obligation_id: REINDEX-EXT|00572|search_path_sandbox|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS reindex_00572_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE ROLE reindex_00572_actor LOGIN NOSUPERUSER;
SET ROLE reindex_00572_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX SYSTEM reindex_00572_db;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY reindex_00572_actor;
DROP ROLE IF EXISTS reindex_00572_actor;
