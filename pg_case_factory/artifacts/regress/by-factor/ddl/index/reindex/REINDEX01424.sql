...
-- primary_obligation_id: REINDEX-EXT|01424|search_path_sandbox|rollback
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_01424_t, reindex_01424_t_p1 CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE reindex_01424_t (id integer) PARTITION BY RANGE (id);
CREATE TABLE reindex_01424_t_p1 PARTITION OF reindex_01424_t FOR VALUES FROM (0) TO (1000);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX TABLE "reindex_01424_t";
-- primary-target-end
