...
-- primary_obligation_id: REINDEX-EXT|03222|search_path_sandbox|drop_invalid_index
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_03222_t CASCADE;
DROP SCHEMA IF EXISTS reindex_03222_sch CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE SCHEMA reindex_03222_sch;
CREATE TABLE reindex_03222_t (id integer);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX TABLE CONCURRENTLY reindex_03222_sch.reindex_03222_t;
-- primary-target-end
