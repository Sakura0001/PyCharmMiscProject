...
-- primary_obligation_id: REINDEX-EXT|02790|search_path_sandbox|drop_invalid_index
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_02790_t CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE reindex_02790_t (id integer);
CREATE INDEX reindex_02790_i ON reindex_02790_t (id);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX INDEX CONCURRENTLY reindex_02790_t;
-- primary-target-end
