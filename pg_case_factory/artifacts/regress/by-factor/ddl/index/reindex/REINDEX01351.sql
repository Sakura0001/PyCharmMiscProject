...
-- primary_obligation_id: REINDEX-EXT|01351|search_path_sandbox|reindex_concurrently_fix
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_01351_t CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE TABLE reindex_01351_t (id integer);
CREATE INDEX reindex_01351_i ON reindex_01351_t (id);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX INDEX "reindex_01351_t";
-- primary-target-end
