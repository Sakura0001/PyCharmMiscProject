...
-- primary_obligation_id: REINDEX-EXT|01531|search_path_sandbox|reindex_concurrently_fix
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS reindex_01531_t CASCADE;
DROP SCHEMA IF EXISTS reindex_01531_sch CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表和因子专用夹具。
CREATE SCHEMA reindex_01531_sch;
CREATE TABLE reindex_01531_t (id integer);
-- 3. 执行唯一获得覆盖信用的 REINDEX。
-- primary-target-begin
REINDEX TABLE reindex_01531_sch.reindex_01531_t;
-- primary-target-end
