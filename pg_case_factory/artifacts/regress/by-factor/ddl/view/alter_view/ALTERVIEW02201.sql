...
-- primary_obligation_id: AVIEW-EXT|02201|error_assertion|revert_alter
-- expected_outcome: expected_failure
-- expected_sqlstate: 3F000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS alterview_02201_view;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE VIEW alterview_02201_view AS SELECT 1 AS alterview_02201_col, 2 AS alterview_02201_col2;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER VIEW。
-- primary-target-begin
ALTER VIEW IF EXISTS alterview_02201_view SET SCHEMA alterview_02201_nosuchschema;
-- primary-target-end
