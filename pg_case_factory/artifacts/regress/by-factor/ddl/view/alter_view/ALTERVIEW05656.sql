...
-- primary_obligation_id: AVIEW-EXT|05656|pg_views_query|drop_view_cascade
...
ALTER VIEW IF EXISTS alterview_05656_nosuchview SET (security_barrier = false);
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS view_state FROM pg_catalog.pg_views WHERE viewname = 'alterview_05656_nosuchview' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
