...
-- primary_obligation_id: AVIEW-EXT|05827|select_from_view|drop_view_cascade
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS alterview_05827_newschema CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA alterview_05827_newschema;
SELECT 1 AS target_view_intentionally_absent;
-- 3. 执行唯一获得覆盖信用的 ALTER VIEW。
-- primary-target-begin
ALTER VIEW IF EXISTS alterview_05827_nosuchview SET SCHEMA alterview_05827_newschema;
-- primary-target-end
