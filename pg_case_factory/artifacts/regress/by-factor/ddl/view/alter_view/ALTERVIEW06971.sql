...
-- primary_obligation_id: AVIEW-EXT|06971|error_assertion|revert_alter
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS alterview_06971_newschema CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA alterview_06971_newschema;
SELECT 1 AS target_view_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER VIEW。
-- primary-target-begin
ALTER VIEW alterview_06971_nosuchview SET SCHEMA alterview_06971_newschema;
-- primary-target-end
