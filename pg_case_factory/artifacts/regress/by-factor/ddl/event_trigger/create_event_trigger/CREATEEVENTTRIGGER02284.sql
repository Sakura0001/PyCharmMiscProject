...
-- ++
-- --------------------------------------------------------
-- case_id: CREATEEVENTTRIGGER02284
...
-- primary_obligation_id: CET-EXT|02284|trigger_firing_test|drop_function
-- expected_outcome: expected_failure
-- expected_sqlstate: 42883
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP EVENT TRIGGER IF EXISTS createeventtrigger_02284_trig;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE EVENT TRIGGER。
-- primary-target-begin
CREATE EVENT TRIGGER createeventtrigger_02284_trig ON ddl_command_start EXECUTE FUNCTION createeventtrigger_02284_nonexistfn();
-- primary-target-end
...
SELECT evtenabled FROM pg_catalog.pg_event_trigger WHERE evtname = 'createeventtrigger_02284_trig' ORDER BY evtenabled;
-- 5. 清理全部本编号对象。
DROP EVENT TRIGGER IF EXISTS createeventtrigger_02284_trig;
