...
-- primary_obligation_id: DROPTRIGGER-EXT|01124|drop_trigger|pg_trigger_catalog_query|DROP_TRIGGER_if_exists_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS droptrigger_01124_t CASCADE;
DROP FUNCTION IF EXISTS droptrigger_01124_fn() CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地触发器和因子专用夹具。
SELECT 1 AS setup_boundary;
CREATE TABLE droptrigger_01124_t (c integer);
SELECT 1 AS target_trigger_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP TRIGGER。
-- primary-target-begin
DROP TRIGGER "select" ON "droptrigger_01124_qt" CASCADE;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP TRIGGER IF EXISTS "select" ON droptrigger_01124_t;
DROP FUNCTION IF EXISTS droptrigger_01124_fn() CASCADE;
DROP TABLE IF EXISTS droptrigger_01124_t CASCADE;
