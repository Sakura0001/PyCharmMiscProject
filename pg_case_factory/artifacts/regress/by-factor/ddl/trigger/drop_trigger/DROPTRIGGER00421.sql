...
-- primary_obligation_id: DROPTRIGGER-EXT|00421|drop_trigger|pg_trigger_catalog_query|DROP_TRIGGER_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS droptrigger_00421_depv CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地触发器和因子专用夹具。
CREATE VIEW droptrigger_00421_depv AS SELECT * FROM ;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP TRIGGER。
-- primary-target-begin
DROP TRIGGER IF EXISTS "select" ON "droptrigger_00421_qt" CASCADE;
-- primary-target-end
