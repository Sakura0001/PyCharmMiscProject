...
-- ++
-- --------------------------------------------------------
-- case_id: CREATECAST00637
-- source_md: skills/pg-sql-generation/references/statements/ddl/cast/create_cast.md
-- factor_md: skills/pg-sql-generation/references/combinations/ddl/cast/create_cast.yaml
-- primary_obligation_id: CCAST-EXT|00637|pg_cast_catalog_query|DROP_CAST_IF_EXISTS
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP CAST IF EXISTS (integer AS bigint);
DROP CAST IF EXISTS (bigint AS integer);
DROP FUNCTION IF EXISTS createcast_00637_revcastfn;
DROP OWNED BY createcast_00637_actor CASCADE;
DROP ROLE IF EXISTS createcast_00637_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE CAST (bigint AS integer) WITHOUT FUNCTION;
CREATE ROLE createcast_00637_actor LOGIN NOSUPERUSER;
SET ROLE createcast_00637_actor;
