...
-- ++
-- --------------------------------------------------------
-- case_id: CREATECAST00696
-- source_md: skills/pg-sql-generation/references/statements/ddl/cast/create_cast.md
-- factor_md: skills/pg-sql-generation/references/combinations/ddl/cast/create_cast.yaml
-- primary_obligation_id: CCAST-EXT|00696|pg_cast_catalog_query|DROP_CAST
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP CAST IF EXISTS (text AS boolean);
DROP CAST IF EXISTS (boolean AS text);
DROP FUNCTION IF EXISTS createcast_00696_revcastfn;
DROP OWNED BY createcast_00696_actor CASCADE;
DROP ROLE IF EXISTS createcast_00696_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE CAST (boolean AS text) WITHOUT FUNCTION;
CREATE ROLE createcast_00696_actor LOGIN NOSUPERUSER;
SET ROLE createcast_00696_actor;
