...
-- primary_obligation_id: DROPOWNED-EXT|00420|drop_owned|error_assertion|drop_owned_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS dropowned_00420_v;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE VIEW dropowned_00420_v AS SELECT * FROM dropowned_00420_t;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP OWNED。
-- primary-target-begin
DROP OWNED BY CURRENT_USER, dropowned_00420_actor2 CASCADE;
-- primary-target-end
