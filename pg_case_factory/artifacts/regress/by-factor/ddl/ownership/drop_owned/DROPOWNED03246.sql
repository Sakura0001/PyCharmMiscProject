...
-- primary_obligation_id: DROPOWNED-EXT|03246|drop_owned|error_assertion|drop_owned_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS dropowned_03246_v;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE VIEW dropowned_03246_v AS SELECT * FROM dropowned_03246_t;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP OWNED。
-- primary-target-begin
DROP OWNED BY SESSION_USER, dropowned_03246_actor2 RESTRICT;
-- primary-target-end
