...
-- primary_obligation_id: SECURITYLABEL-EXT|00137|error_assertion|security_label_is_null
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS securitylabel_00137_tbl CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE securitylabel_00137_tbl (id int);
-- 3. 执行唯一获得覆盖信用的 SECURITY LABEL。
-- primary-target-begin
SECURITY LABEL FOR seclabel ON TABLE securitylabel_00137_tbl IS NULL;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
SECURITY LABEL FOR seclabel ON TABLE securitylabel_00137_tbl IS NULL;
DROP TABLE IF EXISTS securitylabel_00137_tbl;
