...
-- version      : 1.0
-- description  : SECURITY LABEL ON object_type=role
-- FE           : PG18-STATEMENT-FACTOR-LOOP
-- ++
-- --------------------------------------------------------
-- case_id: SECURITYLABEL00044
...
-- primary_obligation_id: SECURITYLABEL-SFV|sfv-0d01de91cfea769636e22ee9|role
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS securitylabel_00044_role;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE securitylabel_00044_role;
-- 3. 执行唯一获得覆盖信用的 SECURITY LABEL。
-- primary-target-begin
SECURITY LABEL ON ROLE securitylabel_00044_role IS 'classification';
-- primary-target-end
...
-- 5. 清理全部本编号对象。
SECURITY LABEL ON ROLE securitylabel_00044_role IS NULL;
DROP ROLE IF EXISTS securitylabel_00044_role;
