...
-- primary_obligation_id: DROPCONVERSION-SFV|sfv-82bb56f3d183c79d210e1126|drop_conversion
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP CONVERSION IF EXISTS dropconversion_00014_conv;
\set ON_ERROR_STOP on
-- 2. 创建完整本地转换和因子专用夹具。
SELECT 1 AS target_conversion_intentionally_absent;
-- 3. 执行唯一获得覆盖信用的 DROP CONVERSION。
-- primary-target-begin
DROP CONVERSION dropconversion_00014_conv;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT count(*) = 0 AS conversion_absent FROM pg_catalog.pg_conversion WHERE conname = 'dropconversion_00014_conv' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP CONVERSION IF EXISTS dropconversion_00014_conv;
