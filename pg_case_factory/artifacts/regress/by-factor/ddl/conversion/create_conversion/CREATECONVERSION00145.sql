...
-- primary_obligation_id: CCONV-EXT|00145|error_assertion|drop_conversion
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P13
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP FUNCTION IF EXISTS createconversion_00145_func CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE FUNCTION createconversion_00145_func(text) RETURNS text LANGUAGE plpgsql AS $$BEGIN RETURN 'x'; END$$;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE CONVERSION。
-- primary-target-begin
CREATE DEFAULT CONVERSION createconversion_00145_language FOR 'UTF8' TO 'LATIN1' FROM createconversion_00145_func;
-- primary-target-end
