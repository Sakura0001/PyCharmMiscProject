...
-- primary_obligation_id: CD-EXT|00540|error_assertion|drop_domain
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DOMAIN IF EXISTS createdomain_00540_dom;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE DOMAIN。
-- primary-target-begin
CREATE DOMAIN createdomain_00540_dom AS boolean DEFAULT TRUE CONSTRAINT createdomain_00540_chk CHECK (VALUE IS NOT NULL);
-- primary-target-end
