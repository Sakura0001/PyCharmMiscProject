...
-- primary_obligation_id: CD-EXT|00967|error_assertion|cascade_drop_domain
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DOMAIN IF EXISTS createdomain_00967_dom;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE DOMAIN。
-- primary-target-begin
CREATE DOMAIN createdomain_00967_dom AS integer DEFAULT (1+1) CHECK (VALUE > 0);
-- primary-target-end
