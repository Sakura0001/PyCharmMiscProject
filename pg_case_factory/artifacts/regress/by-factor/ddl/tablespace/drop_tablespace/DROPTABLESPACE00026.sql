...
-- primary_obligation_id: DROPTABLESPACE-SFV|sfv-7e955359b826f8a0455e9524|drop_tablespace
-- expected_outcome: expected_failure
-- expected_sqlstate: 55006
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS droptablespace_00026_ts;
\set ON_ERROR_STOP on
-- 2. 创建完整本地表空间和因子专用夹具。
SELECT 1 AS setup_boundary;
CREATE TABLESPACE droptablespace_00026_ts LOCATION '/tmp/droptablespace_00026_tsloc';
...
SELECT count(*) > 0 AS tablespace_present FROM pg_catalog.pg_tablespace WHERE spcname = 'droptablespace_00026_ts' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP TABLESPACE IF EXISTS droptablespace_00026_ts;
