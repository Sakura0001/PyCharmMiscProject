...
-- primary_obligation_id: ATSP-EXT|06144|error_assertion|revert_owner
-- expected_outcome: expected_failure
-- expected_sqlstate: 42601
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS alter_tablespace_06144_bad-ts;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLESPACE alter_tablespace_06144_bad-ts LOCATION '/tmp/pgcf_tablespace_location';
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER TABLESPACE。
-- primary-target-begin
ALTER TABLESPACE alter_tablespace_06144_bad-ts SET (seq_page_cost = 1.0);
-- primary-target-end
