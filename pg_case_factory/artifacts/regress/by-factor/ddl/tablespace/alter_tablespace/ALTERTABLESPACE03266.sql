...
-- primary_obligation_id: ATSP-EXT|03266|error_assertion|drop_tablespace
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS alter_tablespace_03266_ts;
DROP TABLESPACE IF EXISTS "select";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLESPACE alter_tablespace_03266_ts LOCATION '/tmp/pgcf_tablespace_location';
-- 3. 执行唯一获得覆盖信用的 ALTER TABLESPACE。
-- primary-target-begin
ALTER TABLESPACE alter_tablespace_03266_ts RENAME TO "select";
-- primary-target-end
