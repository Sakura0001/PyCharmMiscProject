...
-- primary_obligation_id: CTSP-EXT|01561|error_assertion|role_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42939
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OWNED BY createtablespace_01561_owner CASCADE;
DROP ROLE IF EXISTS createtablespace_01561_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE createtablespace_01561_owner LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE pg_createtablespace_01561_ts OWNER createtablespace_01561_owner LOCATION '/tmp/pgcf_ctsp/createtablespace_01561_dir';
-- primary-target-end
