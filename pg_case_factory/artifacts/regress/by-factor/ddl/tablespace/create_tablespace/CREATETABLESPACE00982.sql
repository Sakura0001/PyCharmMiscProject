...
-- primary_obligation_id: CTSP-EXT|00982|catalog_query_pg_tablespace|role_cleanup
...
CREATE TABLESPACE pg_createtablespace_00982_ts LOCATION '/tmp/pgcf_ctsp/createtablespace_00982_dir' WITH (seq_page_cost = 1.0, random_page_cost = 4.0);
-- primary-target-end
...
SELECT count(*) = 0 AS tablespace_state FROM pg_catalog.pg_tablespace WHERE spcname = 'pg_createtablespace_00982_ts' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
