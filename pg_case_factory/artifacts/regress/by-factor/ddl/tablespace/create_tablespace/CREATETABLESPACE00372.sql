...
-- primary_obligation_id: CTSP-EXT|00372|error_assertion|filesystem_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS "createtablespace_00372_Mixed Ts";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE "createtablespace_00372_Mixed Ts" OWNER SESSION_USER LOCATION '/tmp/pgcf_ctsp/createtablespace_00372_dir' WITH (effective_io_concurrency = 16);
-- primary-target-end
