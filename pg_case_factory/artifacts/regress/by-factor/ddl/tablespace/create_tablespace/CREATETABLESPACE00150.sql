...
-- primary_obligation_id: CTSP-EXT|00150|error_assertion|filesystem_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 0A000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS "createtablespace_00150_Mixed Ts";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE "createtablespace_00150_Mixed Ts" OWNER CURRENT_ROLE LOCATION '/tmp/pgcf_ctsp/createtablespace_00150_dir';
-- primary-target-end
