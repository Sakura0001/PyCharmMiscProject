...
-- primary_obligation_id: CTSP-EXT|00921|catalog_query_pg_tablespace|filesystem_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42602
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OWNED BY createtablespace_00921_owner CASCADE;
DROP ROLE IF EXISTS createtablespace_00921_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE createtablespace_00921_owner LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE "" OWNER createtablespace_00921_owner LOCATION '/tmp/pgcf_ctsp/createtablespace_00921_dir' WITH (random_page_cost = 4.0);
-- primary-target-end
