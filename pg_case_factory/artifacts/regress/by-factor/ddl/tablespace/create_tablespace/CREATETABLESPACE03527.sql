...
-- primary_obligation_id: CTSP-EXT|03527|error_assertion|drop_tablespace
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS createtablespace_03527_ts;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE createtablespace_03527_ts LOCATION '/tmp/pgcf_ctsp/createtablespace_03527_dir' WITH (seq_page_cost = 1.0, random_page_cost = 4.0);
-- primary-target-end
