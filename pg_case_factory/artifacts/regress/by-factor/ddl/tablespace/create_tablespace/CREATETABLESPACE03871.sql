...
-- primary_obligation_id: CTSP-EXT|03871|error_assertion|role_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 58P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS "createtablespace_03871_Mixed Ts";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE "createtablespace_03871_Mixed Ts" LOCATION '/tmp/pgcf_ctsp/createtablespace_03871_dir' WITH (maintenance_io_concurrency = 16);
-- primary-target-end
