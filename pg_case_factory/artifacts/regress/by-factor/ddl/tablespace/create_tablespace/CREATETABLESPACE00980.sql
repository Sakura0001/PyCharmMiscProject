...
-- primary_obligation_id: CTSP-EXT|00980|catalog_query_pg_tablespace|drop_tablespace
...
CREATE TABLESPACE pg_createtablespace_00980_ts LOCATION '/tmp/pgcf_ctsp/createtablespace_00980_dir' WITH (seq_page_cost = 1.0, random_page_cost = 4.0);
-- primary-target-end
...
SELECT count(*) = 0 AS tablespace_state FROM pg_catalog.pg_tablespace WHERE spcname = 'pg_createtablespace_00980_ts' ORDER BY count(*);
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
