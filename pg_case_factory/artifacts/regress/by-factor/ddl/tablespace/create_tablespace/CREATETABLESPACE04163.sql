...
-- primary_obligation_id: CTSP-EXT|04163|error_assertion|drop_tablespace
-- expected_outcome: expected_failure
-- expected_sqlstate: 58P01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLESPACE IF EXISTS "createtablespace_04163_Mixed Ts";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE TABLESPACE。
-- primary-target-begin
CREATE TABLESPACE "createtablespace_04163_Mixed Ts" OWNER SESSION_USER LOCATION '/tmp/pgcf_ctsp/createtablespace_04163_dir' WITH (random_page_cost = 4.0);
-- primary-target-end
