...
-- primary_obligation_id: DROPSUBSCRIPTION-EXT|00870|drop_subscription|error_assertion|drop_subscription_cascade
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SUBSCRIPTION IF EXISTS "user" CASCADE;
DROP OWNED BY dropsubscription_00870_actor;
DROP ROLE IF EXISTS dropsubscription_00870_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地订阅和因子专用夹具。
SELECT 1 AS setup_boundary;
CREATE ROLE dropsubscription_00870_actor LOGIN NOSUPERUSER;
SELECT 1 AS target_subscription_intentionally_absent;
SET ROLE dropsubscription_00870_actor;
