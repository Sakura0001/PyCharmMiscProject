...
-- primary_obligation_id: AG-EXT|0335|add_user|error_assertion|drop_role
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altergroup_0335_grp;
DROP ROLE IF EXISTS "altergroup_0335_Usr";
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE altergroup_0335_grp;
CREATE ROLE "altergroup_0335_Usr";
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP altergroup_0335_grp ADD USER "altergroup_0335_Usr";
-- primary-target-end
