...
-- primary_obligation_id: AG-EXT|0369|add_user|error_assertion|grant_membership
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altergroup_0369_grp;
DROP ROLE IF EXISTS "altergroup_0369_Usr";
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "altergroup_0369_Usr";
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP altergroup_0369_grp ADD USER "altergroup_0369_Usr";
-- primary-target-end
