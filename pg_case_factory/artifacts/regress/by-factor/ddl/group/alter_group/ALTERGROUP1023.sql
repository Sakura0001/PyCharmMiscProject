...
SELECT count(*) > 0 AS membership_present FROM pg_catalog.pg_auth_members m JOIN pg_catalog.pg_roles g ON g.oid = m.roleid JOIN pg_catalog.pg_roles u ON u.oid = m.member WHERE g.rolname = 'altergroup_1023_grp' AND u.rolname = 'altergroup_1023_usr' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP ROLE IF EXISTS altergroup_1023_grp;
DROP ROLE IF EXISTS altergroup_1023_usr;
DROP ROLE IF EXISTS altergroup_1023_usr2;
