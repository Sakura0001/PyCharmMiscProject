...
SELECT EXISTS(SELECT 1 FROM pg_catalog.pg_auth_members m JOIN pg_catalog.pg_roles g ON g.oid = m.roleid JOIN pg_catalog.pg_roles u ON u.oid = m.member WHERE g.rolname = 'altergroup_1177_grp' AND u.rolname = 'altergroup_1177_Usr') AS membership_present;
-- 5. 清理全部本编号对象。
RESET ROLE;
