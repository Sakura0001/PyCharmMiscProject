...
SELECT count(*) > 0 AS membership_present FROM pg_catalog.pg_auth_members m JOIN pg_catalog.pg_roles g ON g.oid = m.roleid JOIN pg_catalog.pg_roles u ON u.oid = m.member WHERE g.rolname = 'altergroup_0019_Grp' AND u.rolname = 'altergroup_0019_usr' ORDER BY count(*);
-- 5. 清理全部本编号对象。
RESET ROLE;
