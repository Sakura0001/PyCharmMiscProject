...
-- primary_obligation_id: AG-EXT|0479|add_user|error_assertion|drop_role
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "altergroup_0479_Grp";
DROP ROLE IF EXISTS altergroup_0479_usr;
DROP ROLE IF EXISTS altergroup_0479_admin;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE altergroup_0479_usr;
CREATE ROLE altergroup_0479_admin LOGIN;
SET ROLE altergroup_0479_admin;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP "altergroup_0479_Grp" ADD USER altergroup_0479_usr;
-- primary-target-end
