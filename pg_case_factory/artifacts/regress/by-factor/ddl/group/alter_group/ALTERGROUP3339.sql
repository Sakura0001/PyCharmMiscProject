...
SELECT count(*) = 0 AS membership_absent FROM pg_catalog.pg_auth_members m JOIN pg_catalog.pg_roles g ON g.oid = m.roleid JOIN pg_catalog.pg_roles u ON u.oid = m.member WHERE g.rolname = 'altergroup_3339_Grp' AND u.rolname = 'altergroup_3339_Usr' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP ROLE IF EXISTS "altergroup_3339_Grp";
DROP ROLE IF EXISTS "altergroup_3339_Usr";
DROP ROLE IF EXISTS "altergroup_3339_Usr2";
