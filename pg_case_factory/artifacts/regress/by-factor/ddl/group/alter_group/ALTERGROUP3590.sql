...
-- primary_obligation_id: AG-EXT|3590|rename|pg_auth_members_catalog|revert_rename
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "altergroup_3590_Grp";
DROP ROLE IF EXISTS altergroup_3590_renamed;
DROP ROLE IF EXISTS altergroup_3590_admin;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE altergroup_3590_admin LOGIN;
ALTER ROLE altergroup_3590_admin CREATEROLE;
SET ROLE altergroup_3590_admin;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP "altergroup_3590_Grp" RENAME TO altergroup_3590_renamed;
-- primary-target-end
