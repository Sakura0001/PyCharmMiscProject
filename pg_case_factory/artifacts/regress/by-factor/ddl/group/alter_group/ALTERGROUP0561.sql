...
-- primary_obligation_id: AG-EXT|0561|add_user|error_assertion|grant_membership
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "altergroup_0561_Grp";
DROP ROLE IF EXISTS altergroup_0561_usr;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "altergroup_0561_Grp";
CREATE ROLE altergroup_0561_usr;
GRANT "altergroup_0561_Grp" TO altergroup_0561_usr;
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP "altergroup_0561_Grp" ADD USER altergroup_0561_usr;
-- primary-target-end
