...
-- primary_obligation_id: AG-EXT|0345|add_user|error_assertion|grant_membership
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS altergroup_0345_grp;
DROP ROLE IF EXISTS "altergroup_0345_Usr";
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE altergroup_0345_grp;
CREATE ROLE "altergroup_0345_Usr";
GRANT altergroup_0345_grp TO "altergroup_0345_Usr";
-- 3. 执行唯一获得覆盖信用的 ALTER GROUP。
-- primary-target-begin
ALTER GROUP altergroup_0345_grp ADD USER "altergroup_0345_Usr";
-- primary-target-end
