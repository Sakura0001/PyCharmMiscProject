...
SELECT NOT EXISTS(SELECT 1 FROM pg_catalog.pg_auth_members m JOIN pg_catalog.pg_roles g ON g.oid = m.roleid JOIN pg_catalog.pg_roles u ON u.oid = m.member WHERE g.rolname = 'altergroup_2823_grp' AND u.rolname = 'altergroup_2823_Usr') AS membership_absent;
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP ROLE IF EXISTS altergroup_2823_grp;
DROP ROLE IF EXISTS "altergroup_2823_Usr";
