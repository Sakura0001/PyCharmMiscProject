...
-- primary_obligation_id: COMMENT-EXT|02952|catalog_query_pg_description|cascade_drop
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DOMAIN IF EXISTS comment_02952_dom;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE DOMAIN comment_02952_dom AS int CONSTRAINT comment_02952_con CHECK (VALUE > 0);
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON CONSTRAINT comment_02952_con ON DOMAIN comment_02952_dom IS '';
-- primary-target-end
