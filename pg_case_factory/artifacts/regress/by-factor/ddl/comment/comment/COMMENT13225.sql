...
-- primary_obligation_id: COMMENT-EXT|13225|catalog_query_pg_description|drop_prerequisite_object
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS comment_13225_tbl CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE comment_13225_tbl (id int);
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON TABLE comment_13225_tbl IS 'short comment';
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS comment_13225_tbl;
