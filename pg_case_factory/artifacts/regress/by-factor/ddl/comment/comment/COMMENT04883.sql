...
-- primary_obligation_id: COMMENT-EXT|04883|psql_dd_command|comment_is_null_cleanup
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
SELECT lo_unlink(16399);
DROP OWNED BY comment_04883_actor CASCADE;
DROP ROLE IF EXISTS comment_04883_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE comment_04883_actor LOGIN NOSUPERUSER;
SELECT lo_create(16399) AS large_object_created;
SET ROLE comment_04883_actor;
