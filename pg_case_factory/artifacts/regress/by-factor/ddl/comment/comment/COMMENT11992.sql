...
-- primary_obligation_id: COMMENT-EXT|11992|psql_dd_command|drop_prerequisite_object
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DOMAIN IF EXISTS comment_11992_dom;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE DOMAIN comment_11992_dom AS int CONSTRAINT comment_11992_con CHECK (VALUE > 0);
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON CONSTRAINT comment_11992_con ON DOMAIN comment_11992_dom IS 'short comment';
-- primary-target-end
