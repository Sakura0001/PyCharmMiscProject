...
-- primary_obligation_id: COMMENT-EXT|13503|col_description_query|cascade_drop
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TYPE IF EXISTS comment_13503_typ;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TYPE comment_13503_typ AS (x int);
...
SELECT col_description('comment_13503_tbl'::regclass, 1) AS comment ORDER BY comment;
-- 5. 清理全部本编号对象。
DROP TYPE IF EXISTS comment_13503_typ;
