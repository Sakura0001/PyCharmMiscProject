...
-- primary_obligation_id: COMMENT-EXT|12788|psql_dd_command|comment_is_null_cleanup
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP PROCEDURE IF EXISTS comment_12788_proc();
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE PROCEDURE comment_12788_proc() AS $$BEGIN NULL; END;$$ LANGUAGE plpgsql;
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON PROCEDURE comment_12788_proc() IS 'short comment';
-- primary-target-end
