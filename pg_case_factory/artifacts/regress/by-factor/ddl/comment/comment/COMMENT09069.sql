...
-- primary_obligation_id: COMMENT-EXT|09069|psql_dd_command|cascade_drop
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TYPE IF EXISTS comment_09069_typ;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TYPE comment_09069_typ AS (x int);
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON TYPE comment_09069_typ IS NULL;
-- primary-target-end
