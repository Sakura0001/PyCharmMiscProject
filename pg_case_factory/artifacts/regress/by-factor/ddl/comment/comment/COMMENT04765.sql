...
-- primary_obligation_id: COMMENT-EXT|04765|catalog_query_pg_description|drop_prerequisite_object
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OWNED BY comment_04765_actor CASCADE;
DROP ROLE IF EXISTS comment_04765_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE comment_04765_actor LOGIN NOSUPERUSER;
SET ROLE comment_04765_actor;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON DATABASE comment_04765_db IS NULL;
-- primary-target-end
