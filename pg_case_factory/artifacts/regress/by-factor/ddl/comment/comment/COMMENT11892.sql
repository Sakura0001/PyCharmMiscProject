...
-- primary_obligation_id: COMMENT-EXT|11892|catalog_query_pg_description|cascade_drop
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP COLLATION IF EXISTS comment_11892_col;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE COLLATION comment_11892_col (locale = 'C');
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON COLLATION comment_11892_col IS 'short comment';
-- primary-target-end
