...
-- primary_obligation_id: COMMENT-EXT|04878|col_description_query|cascade_drop
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
SELECT lo_unlink(16399);
DROP OWNED BY comment_04878_actor CASCADE;
DROP ROLE IF EXISTS comment_04878_actor;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE comment_04878_actor LOGIN NOSUPERUSER;
SELECT lo_create(16399) AS large_object_created;
SET ROLE comment_04878_actor;
...
SELECT col_description('comment_04878_tbl'::regclass, 1) AS comment ORDER BY comment;
-- 5. 清理全部本编号对象。
DROP OWNED BY comment_04878_actor CASCADE;
DROP ROLE IF EXISTS comment_04878_actor;
SELECT lo_unlink(16399);
