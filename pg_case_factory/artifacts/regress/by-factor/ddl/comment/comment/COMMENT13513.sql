...
-- primary_obligation_id: COMMENT-EXT|13513|obj_description_query|drop_prerequisite_object
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TYPE IF EXISTS comment_13513_typ;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TYPE comment_13513_typ AS (x int);
...
SELECT obj_description('comment_13513_tbl'::regclass) AS comment ORDER BY comment;
-- 5. 清理全部本编号对象。
DROP TYPE IF EXISTS comment_13513_typ;
