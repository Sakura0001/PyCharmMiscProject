...
-- primary_obligation_id: COMMENT-EXT|13330|catalog_query_pg_description|drop_prerequisite_object
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TEXT SEARCH CONFIGURATION IF EXISTS comment_13330_tsc;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TEXT SEARCH CONFIGURATION comment_13330_tsc (COPY = simple);
-- 3. 执行唯一获得覆盖信用的 COMMENT ON。
-- primary-target-begin
COMMENT ON TEXT SEARCH CONFIGURATION comment_13330_tsc IS 'short comment';
-- primary-target-end
