...
-- primary_obligation_id: DROPDATABASE-EXT|00408|drop_database|error_assertion|force_drop_database
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DATABASE IF EXISTS dropdatabase_00408_db WITH (FORCE);
\set ON_ERROR_STOP on
-- 2. 创建完整本地数据库和因子专用夹具。
CREATE DATABASE dropdatabase_00408_db;
BEGIN;
...
SELECT count(*) > 0 AS database_present FROM pg_catalog.pg_database WHERE datname = 'dropdatabase_00408_db' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP DATABASE IF EXISTS dropdatabase_00408_db WITH (FORCE);
