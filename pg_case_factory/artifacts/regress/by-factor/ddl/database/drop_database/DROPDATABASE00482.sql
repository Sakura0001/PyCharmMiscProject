...
-- primary_obligation_id: DROPDATABASE-EXT|00482|drop_database|notice_assertion_if_exists|force_drop_database
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DATABASE IF EXISTS dropdatabase_00482_db WITH (FORCE);
...
SELECT count(*) = 0 AS database_absent FROM pg_catalog.pg_database WHERE datname = 'dropdatabase_00482_db' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP DATABASE IF EXISTS dropdatabase_00482_db WITH (FORCE);
