...
-- primary_obligation_id: DROPDATABASE-SFV|sfv-635881bcb36b07d00cb58e95|drop_database_txn_error
-- expected_outcome: expected_failure
-- expected_sqlstate: 25001
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP DATABASE IF EXISTS dropdatabase_00029_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地数据库和因子专用夹具。
CREATE DATABASE dropdatabase_00029_db;
BEGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP DATABASE。
-- primary-target-begin
DROP DATABASE dropdatabase_00029_db;
-- primary-target-end
...
SELECT count(*) > 0 AS database_present FROM pg_catalog.pg_database WHERE datname = 'dropdatabase_00029_db' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP DATABASE IF EXISTS dropdatabase_00029_db;
