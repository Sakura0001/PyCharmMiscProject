...
-- primary_obligation_id: CD-SFV|sfv-d6ef12daab62ee63165ce79b|create_database
-- expected_outcome: expected_failure
-- expected_sqlstate: 42P04
-- 1. 清理本编号对象，保证脚本可重复执行。
SELECT 1 AS residual_check_no_objects;
DROP DATABASE IF EXISTS createdatabase_00035_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE DATABASE createdatabase_00035_db;
...
SELECT datname FROM pg_catalog.pg_database WHERE datname = 'createdatabase_00035_db' ORDER BY datname;
-- 5. 清理全部本编号对象。
DROP DATABASE IF EXISTS createdatabase_00035_db;
SELECT 1 AS residual_check_no_objects;
