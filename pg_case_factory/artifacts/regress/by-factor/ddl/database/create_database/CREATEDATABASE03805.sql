...
-- primary_obligation_id: CD-EXT|03805|error_assertion|drop_database
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
SELECT 1 AS residual_check_no_objects;
DROP DATABASE IF EXISTS createdatabase_03805_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE DATABASE。
-- primary-target-begin
CREATE DATABASE createdatabase_03805_db WITH TEMPLATE template0 ENCODING 'SQL_ASCII' LOCALE 'POSIX' STRATEGY FILE_COPY;
-- primary-target-end
