...
-- primary_obligation_id: AR-EXT|04224|owner|pg_aggregate_catalog|drop_aggregate
...
ALTER ROUTINE alterroutine_04224_routine OWNER TO SESSION_USER;
-- primary-target-end
...
SELECT p.proname, p.prokind FROM pg_catalog.pg_aggregate AS ag JOIN pg_catalog.pg_proc AS p ON p.oid = ag.aggfnoid WHERE p.proname = 'alterroutine_04224_routine' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
