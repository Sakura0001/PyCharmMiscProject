...
-- primary_obligation_id: AR-EXT|04087|owner|effect_query|drop_function
...
ALTER ROUTINE alterroutine_04087_routine OWNER TO CURRENT_USER;
-- primary-target-end
...
SELECT p.proname, p.prokind, p.proleakproof, p.prosecdef, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterroutine_04087_routine' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
