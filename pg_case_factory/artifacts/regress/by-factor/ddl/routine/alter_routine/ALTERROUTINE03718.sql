...
-- primary_obligation_id: AR-EXT|03718|security_invoker|pg_proc_catalog|drop_procedure
...
ALTER ROUTINE alterroutine_03718_routine SECURITY INVOKER;
-- primary-target-end
...
SELECT p.proname, p.prokind, p.proowner::regrole, p.proleakproof, p.prosecdef, p.proparallel FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterroutine_03718_routine' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
