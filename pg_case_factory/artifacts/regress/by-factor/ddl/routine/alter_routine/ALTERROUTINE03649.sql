...
-- primary_obligation_id: AR-EXT|03649|security_invoker|effect_query|drop_aggregate
...
ALTER ROUTINE alterroutine_03649_routine SECURITY INVOKER;
-- primary-target-end
...
SELECT p.proname, p.prokind, p.proleakproof, p.prosecdef, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterroutine_03649_routine' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
