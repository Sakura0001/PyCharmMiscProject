...
-- primary_obligation_id: AR-EXT|04132|owner|error_assertion|drop_function
-- expected_outcome: expected_failure
-- expected_sqlstate: 42883
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterroutine_04132_new_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地过程和因子专用夹具。
CREATE ROLE alterroutine_04132_new_owner LOGIN;
SELECT 1 AS target_routine_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER ROUTINE。
-- primary-target-begin
ALTER ROUTINE alterroutine_04132_routine OWNER TO alterroutine_04132_new_owner;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP OWNED BY alterroutine_04132_new_owner;
DROP ROLE IF EXISTS alterroutine_04132_new_owner;
