...
-- primary_obligation_id: AR-EXT|03936|depends_on_extension|pg_proc_catalog|drop_routine
...
ALTER ROUTINE alterroutine_03936_routine DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
SELECT p.proname, p.prokind, p.proowner::regrole, p.proleakproof, p.prosecdef, p.proparallel FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterroutine_03936_routine' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
