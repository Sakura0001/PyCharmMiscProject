...
-- primary_obligation_id: AR-EXT|04384|rename|pg_aggregate_catalog|drop_aggregate
...
ALTER ROUTINE alterroutine_04384_routine RENAME TO alterroutine_04384_renamed;
-- primary-target-end
...
SELECT p.proname, p.prokind FROM pg_catalog.pg_aggregate AS ag JOIN pg_catalog.pg_proc AS p ON p.oid = ag.aggfnoid WHERE p.proname = 'alterroutine_04384_routine' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
