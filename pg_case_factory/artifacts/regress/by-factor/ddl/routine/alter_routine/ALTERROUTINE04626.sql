...
-- primary_obligation_id: AR-EXT|04626|depends_on_extension|effect_query|drop_routine
...
ALTER ROUTINE alterroutine_04626_routine DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
SELECT p.proname, p.prokind, p.proleakproof, p.prosecdef, p.proparallel, p.procost, p.prorows FROM pg_catalog.pg_proc AS p WHERE p.proname = 'alterroutine_04626_routine' ORDER BY p.proname, p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
