...
-- primary_obligation_id: AR-EXT|03945|depends_on_extension|pg_aggregate_catalog|reset_config_parameter
...
ALTER ROUTINE alterroutine_03945_routine DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
SELECT p.proname, p.prokind FROM pg_catalog.pg_aggregate AS ag JOIN pg_catalog.pg_proc AS p ON p.oid = ag.aggfnoid WHERE p.proname = 'alterroutine_03945_routine' ORDER BY p.oid;
-- 5. 清理全部本编号对象。
SELECT 1 AS residual_check_no_objects;
