...
-- primary_obligation_id: AR-EXT|02253|depends_on_extension|error_assertion|drop_procedure
-- expected_outcome: expected_failure
-- expected_sqlstate: 42883
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterroutine_02253_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地过程和因子专用夹具。
CREATE ROLE alterroutine_02253_owner LOGIN;
GRANT CREATE ON SCHEMA public TO alterroutine_02253_owner;
SET ROLE alterroutine_02253_owner;
...
ALTER ROUTINE alterroutine_02253_routine DEPENDS ON EXTENSION plpgsql;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
RESET ROLE;
DROP OWNED BY alterroutine_02253_owner;
DROP ROLE IF EXISTS alterroutine_02253_owner;
