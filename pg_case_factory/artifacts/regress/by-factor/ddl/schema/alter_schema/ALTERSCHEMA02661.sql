...
-- primary_obligation_id: AS-EXT|02661|pg_namespace_catalog_query|DROP_SCHEMA_CASCADE
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS alterschema_02661_tab CASCADE;
DROP SCHEMA IF EXISTS "select" CASCADE;
DROP OWNED BY alterschema_02661_schowner CASCADE;
DROP ROLE IF EXISTS alterschema_02661_schowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE alterschema_02661_schowner LOGIN;
CREATE SCHEMA "select";
ALTER SCHEMA "select" OWNER TO alterschema_02661_schowner;
SET search_path TO "select";
CREATE TABLE alterschema_02661_tab (alterschema_02661_col integer);
RESET search_path;
SET ROLE alterschema_02661_schowner;
