...
-- primary_obligation_id: AS-EXT|01256|pg_namespace_catalog_query|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS "select" CASCADE;
DROP OWNED BY alterschema_01256_schowner CASCADE;
DROP ROLE IF EXISTS alterschema_01256_schowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE alterschema_01256_schowner LOGIN;
CREATE SCHEMA "select";
ALTER SCHEMA "select" OWNER TO alterschema_01256_schowner;
SET ROLE alterschema_01256_schowner;
