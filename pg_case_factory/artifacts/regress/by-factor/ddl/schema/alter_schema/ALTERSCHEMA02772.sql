...
-- primary_obligation_id: AS-EXT|02772|information_schema_schemata|DROP_SCHEMA_CASCADE
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS alterschema_02772_tab CASCADE;
DROP SCHEMA IF EXISTS "select" CASCADE;
DROP OWNED BY alterschema_02772_newowner CASCADE;
DROP ROLE IF EXISTS alterschema_02772_newowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA "select";
SET search_path TO "select";
CREATE TABLE alterschema_02772_tab (alterschema_02772_col integer);
RESET search_path;
-- 3. 执行唯一获得覆盖信用的 ALTER SCHEMA。
-- primary-target-begin
ALTER SCHEMA "select" OWNER TO alterschema_02772_newowner;
-- primary-target-end
