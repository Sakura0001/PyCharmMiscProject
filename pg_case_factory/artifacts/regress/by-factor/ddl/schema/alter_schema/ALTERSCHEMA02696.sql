...
-- primary_obligation_id: AS-EXT|02696|pg_namespace_catalog_query|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS alterschema_02696_tab CASCADE;
DROP SCHEMA IF EXISTS "select" CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA "select";
SET search_path TO "select";
CREATE TABLE alterschema_02696_tab (alterschema_02696_col integer);
RESET search_path;
