...
-- primary_obligation_id: AS-EXT|04175|information_schema_schemata|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS "select" CASCADE;
DROP OWNED BY alterschema_04175_newowner CASCADE;
DROP ROLE IF EXISTS alterschema_04175_newowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA "select";
CREATE VIEW "select".alterschema_04175_v AS SELECT 1 AS alterschema_04175_vcol;
-- 3. 执行唯一获得覆盖信用的 ALTER SCHEMA。
-- primary-target-begin
ALTER SCHEMA "select" OWNER TO alterschema_04175_newowner;
-- primary-target-end
