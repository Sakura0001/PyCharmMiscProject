...
-- primary_obligation_id: AS-EXT|00932|pg_namespace_catalog_query|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: expected_failure
-- expected_sqlstate: 3F000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP OWNED BY alterschema_00932_schowner CASCADE;
DROP ROLE IF EXISTS alterschema_00932_schowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE alterschema_00932_schowner LOGIN;
SELECT 1 AS target_schema_intentionally_absent;
SET ROLE alterschema_00932_schowner;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER SCHEMA。
-- primary-target-begin
ALTER SCHEMA "select" RENAME TO alterschema_00932_newsch;
-- primary-target-end
