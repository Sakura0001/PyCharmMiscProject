...
-- primary_obligation_id: AS-EXT|02556|information_schema_schemata|DROP_SCHEMA_CASCADE
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS alterschema_02556_tab CASCADE;
DROP SCHEMA IF EXISTS "select" CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SCHEMA "select";
SET search_path TO "select";
CREATE TABLE alterschema_02556_tab (alterschema_02556_col integer);
RESET search_path;
