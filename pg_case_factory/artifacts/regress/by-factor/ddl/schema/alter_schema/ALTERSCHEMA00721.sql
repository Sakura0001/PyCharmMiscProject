...
-- primary_obligation_id: AS-EXT|00721|current_schema_query|DROP_SCHEMA
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS "select" CASCADE;
DROP OWNED BY alterschema_00721_schowner CASCADE;
DROP ROLE IF EXISTS alterschema_00721_schowner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE alterschema_00721_schowner LOGIN;
CREATE SCHEMA "select";
ALTER SCHEMA "select" OWNER TO alterschema_00721_schowner;
SET ROLE alterschema_00721_schowner;
