...
-- primary_obligation_id: CSCHEMA-EXT|06623|current_schema_query|DROP_SCHEMA_CASCADE
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS "createschema_06623_Mixed Schema" CASCADE;
DROP OWNED BY createschema_06623_owner CASCADE;
DROP ROLE IF EXISTS createschema_06623_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE createschema_06623_owner LOGIN;
...
-- 5. 清理全部本编号对象。
DROP SCHEMA "createschema_06623_Mixed Schema" CASCADE;
DROP OWNED BY createschema_06623_owner CASCADE;
DROP ROLE IF EXISTS createschema_06623_owner;
