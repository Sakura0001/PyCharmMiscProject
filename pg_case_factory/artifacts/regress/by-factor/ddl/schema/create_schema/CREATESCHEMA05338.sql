...
-- primary_obligation_id: CSCHEMA-EXT|05338|pg_namespace_catalog_query|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: expected_failure
-- expected_sqlstate: 42939
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS pg_createschema_05338_bad CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE SCHEMA。
-- primary-target-begin
CREATE SCHEMA IF NOT EXISTS pg_createschema_05338_bad;
-- primary-target-end
...
SELECT count(*) = 0 AS schema_state FROM pg_catalog.pg_namespace WHERE nspname = 'pg_createschema_05338_bad' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP SCHEMA IF EXISTS pg_createschema_05338_bad CASCADE;
