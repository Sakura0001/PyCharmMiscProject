...
-- primary_obligation_id: CSCHEMA-EXT|06929|current_schema_query|DROP_SCHEMA_CASCADE
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS "createschema_06929_select" CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE SCHEMA。
-- primary-target-begin
CREATE SCHEMA "createschema_06929_select" CREATE VIEW createschema_06929_v AS SELECT 1 AS col;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP SCHEMA "createschema_06929_select" CASCADE;
