...
-- primary_obligation_id: CSCHEMA-EXT|04668|current_schema_query|DROP_SCHEMA
-- expected_outcome: expected_failure
-- expected_sqlstate: 42939
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS pg_createschema_04668_bad CASCADE;
DROP OWNED BY createschema_04668_owner CASCADE;
DROP ROLE IF EXISTS createschema_04668_owner;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE createschema_04668_owner LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CREATE SCHEMA。
-- primary-target-begin
CREATE SCHEMA AUTHORIZATION SESSION_USER CREATE VIEW createschema_04668_v AS SELECT 1 AS col;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
DROP SCHEMA pg_createschema_04668_bad CASCADE;
DROP OWNED BY createschema_04668_owner CASCADE;
DROP ROLE IF EXISTS createschema_04668_owner;
