...
-- primary_obligation_id: CSCHEMA-EXT|06802|current_schema_query|DROP_SCHEMA_IF_EXISTS
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP SCHEMA IF EXISTS createschema_06802_s CASCADE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
-- 3. 执行唯一获得覆盖信用的 CREATE SCHEMA。
-- primary-target-begin
CREATE SCHEMA createschema_06802_s CREATE TABLE createschema_06802_t (id integer);
-- primary-target-end
