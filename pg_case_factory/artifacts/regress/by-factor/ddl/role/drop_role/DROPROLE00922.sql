...
-- primary_obligation_id: DROPROLE-EXT|00922|drop_role|error_assertion|revoke_privileges_then_drop_role
-- expected_outcome: expected_failure
-- expected_sqlstate: 55006
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "droprole_00922_QuotedRole";
...
DROP ROLE "droprole_00922_QuotedRole";
-- primary-target-end
...
SELECT count(*) = 0 AS role_absent FROM pg_catalog.pg_roles WHERE rolname = 'droprole_00922_QuotedRole' ORDER BY count(*) LIMIT 1;
-- 5. 清理全部本编号对象。
DROP ROLE IF EXISTS "droprole_00922_QuotedRole";
