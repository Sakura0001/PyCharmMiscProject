...
-- primary_obligation_id: ALTERROLE-EXT|1269|set_value|pg_settings_catalog|revert_attribute_change
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_1269_target;
DROP DATABASE IF EXISTS alterrole_1269_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE DATABASE alterrole_1269_db;
CREATE ROLE alterrole_1269_target LOGIN;
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE alterrole_1269_target IN DATABASE alterrole_1269_db SET work_mem TO 100;
-- primary-target-end
