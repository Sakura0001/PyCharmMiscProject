...
-- primary_obligation_id: ALTERROLE-EXT|0603|set_value|error_assertion|revert_attribute_change
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_0603_target;
DROP DATABASE IF EXISTS alterrole_0603_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE DATABASE alterrole_0603_db;
CREATE ROLE alterrole_0603_target LOGIN;
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE alterrole_0603_target IN DATABASE alterrole_0603_db SET work_mem TO 100;
-- primary-target-end
