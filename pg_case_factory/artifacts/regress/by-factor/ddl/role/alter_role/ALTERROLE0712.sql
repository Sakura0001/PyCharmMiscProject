...
-- primary_obligation_id: ALTERROLE-EXT|0712|set_value|pg_settings_catalog|reset_config_parameter
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_0712_target;
DROP ROLE IF EXISTS alterrole_0712_admin;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE alterrole_0712_target LOGIN;
CREATE ROLE alterrole_0712_admin LOGIN CREATEROLE;
SET ROLE alterrole_0712_admin;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE alterrole_0712_target SET work_mem TO 100;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
RESET ROLE;
ALTER ROLE alterrole_0712_target RESET work_mem;
DROP ROLE IF EXISTS alterrole_0712_target;
DROP ROLE IF EXISTS alterrole_0712_admin;
