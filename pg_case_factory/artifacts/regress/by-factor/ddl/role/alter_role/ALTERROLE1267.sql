...
-- primary_obligation_id: ALTERROLE-EXT|1267|set_value|pg_settings_catalog|reset_config_parameter
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_1267_target;
DROP DATABASE IF EXISTS alterrole_1267_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE DATABASE alterrole_1267_db;
CREATE ROLE alterrole_1267_target LOGIN;
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE alterrole_1267_target IN DATABASE alterrole_1267_db SET work_mem TO 100;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
ALTER ROLE alterrole_1267_target RESET work_mem;
DROP ROLE IF EXISTS alterrole_1267_target;
DROP DATABASE IF EXISTS alterrole_1267_db;
