...
-- primary_obligation_id: ALTERROLE-EXT|0528|set_value|error_assertion|revert_attribute_change
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_0528_admin;
DROP DATABASE IF EXISTS alterrole_0528_db;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE DATABASE alterrole_0528_db;
CREATE ROLE alterrole_0528_admin LOGIN CREATEROLE;
SET ROLE alterrole_0528_admin;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE ALL IN DATABASE alterrole_0528_db SET work_mem TO 100;
-- primary-target-end
