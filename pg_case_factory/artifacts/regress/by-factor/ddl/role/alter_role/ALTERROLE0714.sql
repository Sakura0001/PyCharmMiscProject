...
-- primary_obligation_id: ALTERROLE-EXT|0714|set_value|pg_settings_catalog|revert_attribute_change
-- expected_outcome: expected_failure
-- expected_sqlstate: 42501
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alterrole_0714_target;
DROP ROLE IF EXISTS alterrole_0714_admin;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE alterrole_0714_target LOGIN;
CREATE ROLE alterrole_0714_admin LOGIN CREATEROLE;
SET ROLE alterrole_0714_admin;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE alterrole_0714_target SET work_mem TO 100;
-- primary-target-end
