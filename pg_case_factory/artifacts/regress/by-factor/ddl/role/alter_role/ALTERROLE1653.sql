...
-- primary_obligation_id: ALTERROLE-EXT|1653|with_option|error_assertion|revert_attribute_change
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alterrole_1653_select";
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "alterrole_1653_select" LOGIN;
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE "alterrole_1653_select" LOGIN;
-- primary-target-end
...
-- 5. 清理全部本编号对象。
ALTER ROLE "alterrole_1653_select" NOLOGIN;
DROP ROLE IF EXISTS "alterrole_1653_select";
