...
-- primary_obligation_id: ALTERROLE-EXT|1648|with_option|effect_query|reset_config_parameter
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alterrole_1648_select";
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "alterrole_1648_select" LOGIN;
-- 3. 执行唯一获得覆盖信用的 ALTER ROLE。
-- primary-target-begin
ALTER ROLE "alterrole_1648_select" LOGIN;
-- primary-target-end
\set target_sqlstate :SQLSTATE
\echo PGCF_TARGET_SQLSTATE=:target_sqlstate
-- 4. 验证 SQLSTATE、目录状态和数据行为。
SELECT :'target_sqlstate' = '00000' AS target_sqlstate_matches_expected;
SELECT rolname FROM pg_catalog.pg_roles WHERE rolname = 'alterrole_1648_select' ORDER BY oid;
-- 5. 清理全部本编号对象。
DROP ROLE IF EXISTS "alterrole_1648_select";
