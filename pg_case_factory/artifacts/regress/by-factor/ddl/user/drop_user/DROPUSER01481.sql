...
-- primary_obligation_id: DROPUSER-EXT|01481|drop_user|catalog_query|drop_role
-- expected_outcome: expected_failure
-- expected_sqlstate: 2BP01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP VIEW IF EXISTS dropuser_01481_ownv CASCADE;
DROP ROLE IF EXISTS "order";
DROP ROLE IF EXISTS dropuser_01481_role2;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE dropuser_01481_role2 LOGIN;
CREATE VIEW dropuser_01481_ownv AS SELECT 1 AS c;
ALTER VIEW dropuser_01481_ownv OWNER TO "order";
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP USER。
-- primary-target-begin
DROP USER IF EXISTS "order", dropuser_01481_role2;
-- primary-target-end
