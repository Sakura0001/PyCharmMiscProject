...
-- primary_obligation_id: DROPUSER-EXT|01289|drop_user|notice_assertion|drop_role
-- expected_outcome: expected_failure
-- expected_sqlstate: 2BP01
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "order";
DROP ROLE IF EXISTS dropuser_01289_role2;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE dropuser_01289_role2 LOGIN;
SELECT 1 AS active_session_boundary_provisional;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DROP USER。
-- primary-target-begin
DROP USER IF EXISTS "order", dropuser_01289_role2;
-- primary-target-end
