...
-- primary_obligation_id: DROPUSER-EXT|00142|drop_user|error_assertion|reassign_objects_then_drop
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "order";
DROP ROLE IF EXISTS dropuser_00142_role2;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "order" LOGIN;
CREATE ROLE dropuser_00142_role2 LOGIN;
-- 3. 执行唯一获得覆盖信用的 DROP USER。
-- primary-target-begin
DROP USER "order", dropuser_00142_role2;
-- primary-target-end
