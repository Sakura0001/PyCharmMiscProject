...
-- primary_obligation_id: DROPUSER-EXT|01125|drop_user|error_assertion|terminate_sessions
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "order";
DROP ROLE IF EXISTS dropuser_01125_role2;
\set ON_ERROR_STOP on
-- 2. 创建完整本地角色和因子专用夹具。
CREATE ROLE "order" LOGIN;
CREATE ROLE dropuser_01125_role2 LOGIN;
-- 3. 执行唯一获得覆盖信用的 DROP USER。
-- primary-target-begin
DROP USER IF EXISTS "order", dropuser_01125_role2;
-- primary-target-end
