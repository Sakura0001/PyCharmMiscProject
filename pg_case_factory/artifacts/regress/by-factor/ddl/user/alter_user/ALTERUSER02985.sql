...
-- primary_obligation_id: AUSR-EXT|02985|config_parameter_query|revert_rename
-- expected_outcome: success
-- expected_sqlstate: 00000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alteruser_02985_Mixed Role";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE "alteruser_02985_Mixed Role" LOGIN;
SET ROLE "alteruser_02985_Mixed Role";
-- 3. 执行唯一获得覆盖信用的 ALTER USER。
-- primary-target-begin
ALTER USER "alteruser_02985_Mixed Role" IN DATABASE pgcf SET work_mem = '4MB';
-- primary-target-end
