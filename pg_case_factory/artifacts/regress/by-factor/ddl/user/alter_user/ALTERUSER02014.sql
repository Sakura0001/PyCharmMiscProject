...
-- primary_obligation_id: AUSR-EXT|02014|config_parameter_query|reset_config
-- expected_outcome: expected_failure
-- expected_sqlstate: 42710
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS alteruser_02014_role;
DROP ROLE IF EXISTS alteruser_02014_conflict;
DROP ROLE IF EXISTS alteruser_02014_conflict;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE alteruser_02014_role LOGIN;
CREATE ROLE alteruser_02014_conflict LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER USER。
-- primary-target-begin
ALTER USER alteruser_02014_role RENAME TO alteruser_02014_conflict;
-- primary-target-end
