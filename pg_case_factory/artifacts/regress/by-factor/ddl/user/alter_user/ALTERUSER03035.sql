...
-- primary_obligation_id: AUSR-EXT|03035|config_parameter_query|drop_role
-- expected_outcome: expected_failure
-- expected_sqlstate: 3D000
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alteruser_03035_Mixed Role";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE "alteruser_03035_Mixed Role" LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER USER。
-- primary-target-begin
ALTER USER "alteruser_03035_Mixed Role" IN DATABASE alteruser_03035_nosuchdb SET work_mem = '4MB';
-- primary-target-end
