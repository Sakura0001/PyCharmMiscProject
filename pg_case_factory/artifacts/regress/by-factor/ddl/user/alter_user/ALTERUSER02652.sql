...
-- primary_obligation_id: AUSR-EXT|02652|error_assertion|revert_option
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alteruser_02652_Mixed Role";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE "alteruser_02652_Mixed Role" LOGIN;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER USER。
-- primary-target-begin
ALTER USER "alteruser_02652_Mixed Role" SET nosuchparam = '4MB';
-- primary-target-end
