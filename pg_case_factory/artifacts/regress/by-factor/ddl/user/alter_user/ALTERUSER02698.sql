...
-- primary_obligation_id: AUSR-EXT|02698|config_parameter_query|reset_config
-- expected_outcome: expected_failure
-- expected_sqlstate: 42704
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP ROLE IF EXISTS "alteruser_02698_Mixed Role";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE ROLE "alteruser_02698_Mixed Role" LOGIN;
SET ROLE "alteruser_02698_Mixed Role";
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 ALTER USER。
-- primary-target-begin
ALTER USER "alteruser_02698_Mixed Role" IN DATABASE pgcf SET nosuchparam = '4MB';
-- primary-target-end
