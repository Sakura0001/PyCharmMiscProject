...
-- primary_obligation_id: SELECT-EXT|14485|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS select_14485_tbl, select_14485_src, select_14485_ref CASCADE;
DROP SEQUENCE IF EXISTS select_14485_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE select_14485_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 SELECT。
-- primary-target-begin
SELECT 1 FROM public.select_14485_seq WHERE id = 1;
-- primary-target-end
