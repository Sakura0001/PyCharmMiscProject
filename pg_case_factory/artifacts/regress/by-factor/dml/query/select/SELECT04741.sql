...
-- primary_obligation_id: SELECT-EXT|04741|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS select_04741_tbl, select_04741_src, select_04741_ref CASCADE;
DROP SEQUENCE IF EXISTS select_04741_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE select_04741_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 SELECT。
-- primary-target-begin
SELECT 1 FROM public.select_04741_seq WHERE id IN (SELECT val FROM select_04741_src);
-- primary-target-end
