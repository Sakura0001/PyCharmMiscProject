...
-- primary_obligation_id: CALL-EXT|09241|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS call_09241_tbl;
DROP FUNCTION IF EXISTS "call_09241_Proc";
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE call_09241_tbl (val int);
CREATE FUNCTION "call_09241_Proc"(int) RETURNS int AS $$ SELECT $1 $$ LANGUAGE sql;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CALL。
-- primary-target-begin
CALL "call_09241_Proc"((SELECT 1));
-- primary-target-end
