...
-- primary_obligation_id: CALL-EXT|10417|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS call_10417_tbl;
DROP FUNCTION IF EXISTS call_10417_proc;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE call_10417_tbl (val int);
CREATE FUNCTION call_10417_proc(int) RETURNS int AS $$ SELECT $1 $$ LANGUAGE sql;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CALL。
-- primary-target-begin
CALL call_10417_proc();
-- primary-target-end
