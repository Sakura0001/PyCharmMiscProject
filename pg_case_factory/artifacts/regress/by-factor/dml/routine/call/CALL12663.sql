...
-- primary_obligation_id: CALL-EXT|12663|catalog_query|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42883
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS call_12663_tbl;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE call_12663_tbl (val int);
SELECT 1 AS target_procedure_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CALL。
-- primary-target-begin
WITH call_12663_cte AS (SELECT 42 AS val)
CALL call_12663_proc((SELECT val FROM call_12663_cte));
-- primary-target-end
...
SELECT count(*) = 0 AS procedure_state FROM pg_catalog.pg_proc WHERE proname = 'call_12663_proc' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS call_12663_tbl;
