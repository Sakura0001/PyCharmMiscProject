...
-- primary_obligation_id: CALL-EXT|15780|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS call_15780_tbl;
DROP FUNCTION IF EXISTS call_15780_proc;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE call_15780_tbl (val int);
CREATE FUNCTION call_15780_proc(int) RETURNS int AS $$ SELECT $1 $$ LANGUAGE sql;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CALL。
-- primary-target-begin
CALL call_15780_proc();
-- primary-target-end
