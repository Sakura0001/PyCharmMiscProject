...
-- primary_obligation_id: CALL-EXT|01582|returned_rows|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42883
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS call_01582_tbl;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE TABLE call_01582_tbl (val int);
SELECT 1 AS target_procedure_intentionally_absent;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 CALL。
-- primary-target-begin
CALL public.call_01582_proc(abs(1));
-- primary-target-end
...
SELECT count(*) = 0 AS returned_rows_state FROM call_01582_tbl ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP TABLE IF EXISTS call_01582_tbl;
