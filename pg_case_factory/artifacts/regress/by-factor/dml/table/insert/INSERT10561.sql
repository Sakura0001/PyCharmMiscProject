...
-- primary_obligation_id: INSERT-EXT|10561|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS insert_10561_tbl, insert_10561_src, insert_10561_ref CASCADE;
DROP SEQUENCE IF EXISTS insert_10561_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE insert_10561_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 INSERT。
-- primary-target-begin
INSERT INTO insert_10561_seq SELECT 1, 100 WHERE false;
-- primary-target-end
