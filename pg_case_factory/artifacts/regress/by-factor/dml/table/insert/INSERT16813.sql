...
-- primary_obligation_id: INSERT-EXT|16813|error_assertion|drop_objects
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS insert_16813_tbl, insert_16813_src, insert_16813_ref CASCADE;
DROP SEQUENCE IF EXISTS insert_16813_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE insert_16813_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 INSERT。
-- primary-target-begin
INSERT INTO insert_16813_seq AS i (id, val) SELECT 1, val FROM insert_16813_src;
-- primary-target-end
