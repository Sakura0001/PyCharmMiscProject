...
-- primary_obligation_id: INSERT-EXT|04730|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS insert_04730_tbl, insert_04730_src, insert_04730_ref CASCADE;
DROP SEQUENCE IF EXISTS insert_04730_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE insert_04730_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 INSERT。
-- primary-target-begin
INSERT INTO "insert_04730_seq" (id, val) SELECT 1, val FROM insert_04730_src;
-- primary-target-end
