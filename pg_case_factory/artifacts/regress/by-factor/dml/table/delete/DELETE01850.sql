...
-- primary_obligation_id: DELETE-EXT|01850|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS delete_01850_tbl, delete_01850_src, delete_01850_ref CASCADE;
DROP SEQUENCE IF EXISTS delete_01850_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE delete_01850_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DELETE。
-- primary-target-begin
DELETE FROM delete_01850_seq AS d WHERE id = id;
-- primary-target-end
