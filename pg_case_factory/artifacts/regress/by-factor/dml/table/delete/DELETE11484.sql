...
-- primary_obligation_id: DELETE-EXT|11484|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS delete_11484_tbl, delete_11484_src, delete_11484_ref CASCADE;
DROP SEQUENCE IF EXISTS delete_11484_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE delete_11484_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DELETE。
-- primary-target-begin
DELETE FROM "delete_11484_seq" WHERE id = 999;
-- primary-target-end
