...
-- primary_obligation_id: DELETE-EXT|07562|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS delete_07562_tbl, delete_07562_src, delete_07562_ref CASCADE;
DROP SEQUENCE IF EXISTS delete_07562_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE delete_07562_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DELETE。
-- primary-target-begin
WITH delete_07562_cte AS (SELECT 1 AS val)
DELETE FROM delete_07562_seq AS d WHERE id = 999;
-- primary-target-end
