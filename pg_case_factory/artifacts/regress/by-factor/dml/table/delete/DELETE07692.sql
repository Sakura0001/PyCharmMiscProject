...
-- primary_obligation_id: DELETE-EXT|07692|error_assertion|rollback
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS delete_07692_tbl, delete_07692_src, delete_07692_ref CASCADE;
DROP SEQUENCE IF EXISTS delete_07692_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE delete_07692_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 DELETE。
-- primary-target-begin
WITH delete_07692_cte AS (SELECT 1 AS val)
DELETE FROM delete_07692_seq AS d WHERE id = 999;
-- primary-target-end
