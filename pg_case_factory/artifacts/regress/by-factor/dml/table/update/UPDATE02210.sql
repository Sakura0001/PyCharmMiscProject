...
-- primary_obligation_id: UPDATE-EXT|02210|error_assertion|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS update_02210_tbl, update_02210_src, update_02210_ref CASCADE;
DROP SEQUENCE IF EXISTS update_02210_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE update_02210_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 UPDATE。
-- primary-target-begin
UPDATE public.update_02210_seq SET val = 1 WHERE id = 1;
-- primary-target-end
