...
-- primary_obligation_id: UPDATE-EXT|09212|effect_query|reset_state
-- expected_outcome: expected_failure
-- expected_sqlstate: 42809
-- 1. 清理本编号对象，保证脚本可重复执行。
DROP TABLE IF EXISTS update_09212_tbl, update_09212_src, update_09212_ref CASCADE;
DROP SEQUENCE IF EXISTS update_09212_seq;
RESET ROLE;
\set ON_ERROR_STOP on
-- 2. 创建完整本地规则和因子专用夹具。
CREATE SEQUENCE update_09212_seq;
\set ON_ERROR_STOP off
-- 3. 执行唯一获得覆盖信用的 UPDATE。
-- primary-target-begin
UPDATE update_09212_seq SET val = (SELECT 1) WHERE id = 999;
-- primary-target-end
...
SELECT count(*) = 0 AS effect_state FROM pg_catalog.pg_class WHERE relname = 'update_09212_seq' AND relkind = 'r' ORDER BY count(*);
-- 5. 清理全部本编号对象。
DROP SEQUENCE IF EXISTS update_09212_seq;
DROP TABLE IF EXISTS update_09212_tbl, update_09212_src, update_09212_ref CASCADE;
