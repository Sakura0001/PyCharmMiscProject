...
官方说明：SET CONSTRAINTS — set constraint check timing for the current transaction
...
structured_config:
  ...
  statement:
    ...
    - set constraints
    - SET CONSTRAINTS
    purpose: SET CONSTRAINTS — set constraint check timing for the current transaction
  syntax_templates:
  - SET CONSTRAINTS { ALL | name [, ...] } { DEFERRED | IMMEDIATE }
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - session_state
  rendering:
    statement_template: SET CONSTRAINTS { ALL | name [, ...] } { DEFERRED | IMMEDIATE }
