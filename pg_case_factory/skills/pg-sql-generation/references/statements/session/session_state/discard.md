...
来源：https://www.postgresql.org/docs/16/sql-discard.html
...
官方说明：DISCARD — discard session state
...
structured_config:
  ...
  skill_name: discard
  official_source: https://www.postgresql.org/docs/16/sql-discard.html
  statement:
    key: discard
    name: DISCARD
    aliases:
    ...
    - discard
    - DISCARD
    purpose: DISCARD — discard session state
  syntax_templates:
  - DISCARD { ALL | PLANS | SEQUENCES | TEMPORARY | TEMP }
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - session_state
  rendering:
    statement_template: DISCARD { ALL | PLANS | SEQUENCES | TEMPORARY | TEMP }
