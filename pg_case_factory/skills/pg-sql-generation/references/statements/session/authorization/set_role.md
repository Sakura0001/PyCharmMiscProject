...
官方说明：SET ROLE — set the current user identifier of the current session
...
structured_config:
  ...
  statement:
    ...
    - set role
    - SET ROLE
    purpose: SET ROLE — set the current user identifier of the current session
  syntax_templates:
  - 'SET [ SESSION | LOCAL ] ROLE role_name
  ...
  coverage_policy:
    ...
    - session_state
  rendering:
    statement_template: SET [ SESSION | LOCAL ] ROLE role_name
