...
官方说明：SET SESSION AUTHORIZATION — set the session user identifier and the current user identifier of the current session
...
- 切换会话授权会影响后续权限，清理阶段必须 RESET SESSION AUTHORIZATION。
...
structured_config:
  ...
  statement:
    ...
    - set session authorization
    - SET SESSION AUTHORIZATION
    purpose: SET SESSION AUTHORIZATION — set the session user identifier and the current user identifier of the current session
  syntax_templates:
  - 'SET [ SESSION | LOCAL ] SESSION AUTHORIZATION user_name
  ...
  coverage_policy:
    ...
    - session_state
  rendering:
    statement_template: SET [ SESSION | LOCAL ] SESSION AUTHORIZATION user_name
