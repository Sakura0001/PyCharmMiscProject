...
来源：https://www.postgresql.org/docs/16/sql-listen.html
...
官方说明：LISTEN — listen for a notification
...
- 完整通知接收验证需要第二连接；单连接样本只验证 LISTEN 成功与 UNLISTEN 清理。
...
structured_config:
  ...
  domain: notification
  skill_name: listen
  official_source: https://www.postgresql.org/docs/16/sql-listen.html
  statement:
    key: listen
    name: LISTEN
    aliases:
    ...
    - listen
    - LISTEN
    purpose: LISTEN — listen for a notification
  syntax_templates:
  - LISTEN channel
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - session_state
  rendering:
    statement_template: LISTEN channel
