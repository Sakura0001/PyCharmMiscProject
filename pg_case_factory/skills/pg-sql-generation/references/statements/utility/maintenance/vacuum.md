...
官方说明：VACUUM — garbage-collect and optionally analyze a database
...
- VACUUM FULL、FREEZE、DISABLE_PAGE_SKIPPING 等分支可能改变锁和运行时间，应与普通 VACUUM 分支分离。
...
structured_config:
  ...
  statement:
    ...
    - vacuum
    - VACUUM
    purpose: VACUUM — garbage-collect and optionally analyze a database
  syntax_templates:
  - "VACUUM [ ( option [, ...] ) ] [ table_and_columns [, ...] ]\nVACUUM [ FULL ] [ FREEZE ] [ VERBOSE ] [ ANALYZE ] [ table_and_columns\
    \ [, ...] ]\n\nwhere option can be one of:\n\n    FULL [ boolean ]\n    FREEZE [ boolean ]\n    VERBOSE [ boolean ]\n\
    \    ANALYZE [ boolean ]\n    DISABLE_PAGE_SKIPPING [ boolean ]\n    SKIP_LOCKED [ boolean ]\n    INDEX_CLEANUP { AUTO\
  ...
  coverage_policy:
    ...
    - target_state
  rendering:
    statement_template: VACUUM [ ( option [, ...] ) ] [ table_and_columns [, ...] ]
