...
官方说明：CHECKPOINT — force a write-ahead log checkpoint
...
- CHECKPOINT 需要检查点执行权限；无权限环境中只能生成标注为受限的失败样本。
...
structured_config:
  ...
  statement:
    ...
    - checkpoint
    - CHECKPOINT
    purpose: CHECKPOINT — force a write-ahead log checkpoint
  syntax_templates:
  - CHECKPOINT
  factor_layers:
  - tier: T1
