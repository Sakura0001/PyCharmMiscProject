...
- LOCK TABLE 只能在事务块中产生可验证锁状态，NOWAIT 失败路径需要单独会话或预持锁场景。
...
structured_config:
  ...
  statement:
    ...
    - lock
    - LOCK
    purpose: LOCK — lock a table
  syntax_templates:
  - "LOCK [ TABLE ] [ ONLY ] name [ * ] [, ...] [ IN lockmode MODE ] [ NOWAIT ]\n\nwhere lockmode is one of:\n\n    ACCESS\
    \ SHARE | ROW SHARE | ROW EXCLUSIVE | SHARE UPDATE EXCLUSIVE\n    | SHARE | SHARE ROW EXCLUSIVE | EXCLUSIVE | ACCESS EXCLUSIVE"
  factor_layers:
  - tier: T1
    name: 核心语义因子
    factors:
  ...
  coverage_policy:
    ...
    - target_state
  rendering:
    statement_template: LOCK [ TABLE ] [ ONLY ] name [ * ] [, ...] [ IN lockmode MODE ] [ NOWAIT ]
