...
官方说明：EXPLAIN — show the execution plan of a statement
...
- EXPLAIN ANALYZE 会执行目标 statement，数据变更语句必须使用隔离对象并清理副作用。
...
structured_config:
  ...
  statement:
    ...
    - explain
    - EXPLAIN
    purpose: EXPLAIN — show the execution plan of a statement
  syntax_templates:
  - "EXPLAIN [ ( option [, ...] ) ] statement\nEXPLAIN [ ANALYZE ] [ VERBOSE ] statement\n\nwhere option can be one of:\n\n\
    \    ANALYZE [ boolean ]\n    VERBOSE [ boolean ]\n    COSTS [ boolean ]\n    SETTINGS [ boolean ]\n    GENERIC_PLAN [\
    \ boolean ]\n    BUFFERS [ boolean ]\n    WAL [ boolean ]\n    TIMING [ boolean ]\n    SUMMARY [ boolean ]\n    FORMAT\
  ...
  coverage_policy:
    ...
    - target_state
  rendering:
    statement_template: EXPLAIN [ ( option [, ...] ) ] statement
