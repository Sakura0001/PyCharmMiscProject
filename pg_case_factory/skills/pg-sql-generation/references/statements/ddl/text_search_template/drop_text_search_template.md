...
- IF EXISTS：如果 template 不存在，不报错而是发出通知
- 该语句不涉及列类型，不需要挂靠基表列类型

## 语句作用

官方说明：DROP TEXT SEARCH TEMPLATE — remove a text search template

该 reference 关注全文搜索模板删除操作的 superuser 权限边界、依赖对象驻留和 IF EXISTS 行为，不涉及表/列/索引组合。

## 测试因子分级

### T1：核心语义因子
- statement_branch：官方语法分支（DROP TEXT SEARCH TEMPLATE / DROP TEXT SEARCH TEMPLATE IF EXISTS）
...
- dependency_status：依赖对象状态（无依赖 / 有 dictionary 依赖）

### T3：对象名与输入形态因子
- template_name_shape：text search template 标识符形态
...
- 覆盖 DROP TEXT SEARCH TEMPLATE 全部语法分支。
...
- DROP TEXT SEARCH TEMPLATE 要求 SUPERUSER 权限；非 superuser 执行路径属于失败路径，必须在生成样本中显式标注。
- 必须覆盖目标 template 存在时的成功删除路径，以及目标 template 不存在时的失败路径。
- 支持 IF EXISTS 时，必须覆盖不存在对象的代表性 no-op 路径。
- 支持 CASCADE | RESTRICT 时，必须覆盖存在依赖对象下的 RESTRICT 失败与 CASCADE 成功路径。
- DROP TEXT SEARCH TEMPLATE 不涉及 table / column 组合，不需要挂靠基表列类型。
- 每个样本必须包含明确的前置对象准备、目标 DROP TEXT SEARCH TEMPLATE 语句、验证语句与清理语句。
...
structured_config:
  ...
  statement:
    ...
    purpose: DROP TEXT SEARCH TEMPLATE — remove a text search template
  syntax_templates:
  - "DROP TEXT SEARCH TEMPLATE [ IF EXISTS ] name [ CASCADE | RESTRICT ]"
  factor_layers:
  - tier: T1
  ...
  factors:
    statement_branch:
      ...
      values:
      - key: branch_drop_ts_template
        label: DROP TEXT SEARCH TEMPLATE name
      - key: branch_drop_ts_template_if_exists
        label: DROP TEXT SEARCH TEMPLATE IF EXISTS name
    object_state:
    ...
    template_name_shape:
      label: text search template 标识符形态
      importance: non_important
      values:
      - simple_id
      - schema_qualified_id
  ...
  coverage_policy:
    ...
    - object_state
  rendering:
    statement_template: "DROP TEXT SEARCH TEMPLATE [ IF EXISTS ] {template_name} [ CASCADE | RESTRICT ]"
