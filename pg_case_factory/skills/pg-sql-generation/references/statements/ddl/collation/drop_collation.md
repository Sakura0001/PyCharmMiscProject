...
来源：https://www.postgresql.org/docs/16/sql-dropcollation.html
...
**重要行为说明**：
- DROP COLLATION 删除指定的校对规则。
- `IF EXISTS` 在校对规则不存在时不抛出错误，仅发出通知。
- `CASCADE` 自动删除依赖该校对规则的对象（使用该 collation 的表字段、索引等），以及依赖于这些对象的所有对象。
- `RESTRICT` 拒绝删除有依赖对象的校对规则，这是默认行为。
- 必须拥有该校对规则才能删除它。
- name 可以是 schema 限定名。
- DROP COLLATION 不涉及表/列/索引类型组合，但 CASCADE 会删除引用该 collation 的列定义。

## 语句作用

官方说明：DROP COLLATION — remove a collation

该 reference 关注校对规则删除语句的语法分支、IF EXISTS 行为、CASCADE/RESTRICT 依赖处理与权限边界，不负责覆盖表/列/索引类型组合。

DROP COLLATION **不涉及列类型组合**，具体表现为：
- 语句仅删除校对规则及其依赖对象
- CASCADE 行为涉及引用该 collation 的列/索引，但不需要按列类型展开

## 测试因子分级

### T1：核心语义因子
- statement_branch：官方 synopsis 语法分支
- object_state：目标 Collation 对象存在性（不存在、已存在_no_deps、已存在_with_deps）
...
- dependency_state：依赖对象状态（no_dependencies、referenced_by_table_column、referenced_by_index）

### T3：对象名与输入形态因子
- collation_name_shape：校对规则名称形态（plain_identifier、quoted_identifier、schema_qualified）

### T4：依赖对象与环境因子
- privilege_level：权限级别（collation_owner、non_owner）

### T5：异常与边界因子
- nonexistent_collation：校对规则不存在（无 IF EXISTS → error）
- dependent_objects_exist：有依赖对象且 RESTRICT → error
- insufficient_privilege：非 Owner 删除校对规则 → error

### T6：验证与清理因子
- verification_mode：验证方式（pg_collation_catalog_query、pg_collation_removed_assertion）
- cleanup_mode：清理方式（DROP_COLLATION_CASCADE、DROP_DEPENDENT_OBJECTS_FIRST）

## 覆盖策略

- 必须覆盖 DROP COLLATION 的唯一语法分支。
- 必须覆盖 IF EXISTS / CASCADE / RESTRICT 的组合行为。
- 不需要覆盖所有基表列类型；仅在依赖对象场景中准备最小依赖对象。
- T1 因子做笛卡尔积覆盖。
...
- 每个样本必须包含明确的前置对象准备、目标 DROP COLLATION 语句、验证语句与清理语句。
...
structured_config:
  ...
  category: ddl
  domain: collation
  skill_name: drop_collation
  official_source: https://www.postgresql.org/docs/16/sql-dropcollation.html
  statement:
    key: drop_collation
    name: DROP COLLATION
    aliases:
  ...
  factors:
    ...
    object_state:
      ...
      - key: not_exists
        label: 校对规则不存在
      - key: already_exists_no_deps
        label: 校对规则已存在且无依赖
      - key: already_exists_with_deps
        label: 校对规则已存在且有依赖对象
    expected_status:
      label: 预期结果
      importance: important
    ...
    nonexistent_collation:
      ...
      - key: without_if_exists
        label: 不使用IF EXISTS且collation不存在 → error
      - key: with_if_exists
        label: 使用IF EXISTS且collation不存在 → notice (no-op)
    dependent_objects_exist:
    ...
    insufficient_privilege:
      ...
      values:
      - key: non_owner_drop
        label: 非 Owner 删除校对规则 → error
    verification_mode:
      label: 验证方式
      importance: non_important
      values:
      - key: pg_collation_catalog_query
        label: pg_collation 系统目录查询确认删除
      - key: pg_collation_removed_assertion
        label: 确认校对规则不再存在于系统目录
    cleanup_mode:
      label: 清理方式
      importance: non_important
      values:
      ...
      - key: DROP_DEPENDENT_OBJECTS_FIRST
        label: 先删除依赖对象再删除校对规则
  notes:
    restrict_default: RESTRICT 是默认行为，有依赖对象时拒绝删除。
    cascade_drops_dependencies: CASCADE 自动删除使用该 collation 的表字段和索引等依赖对象。
    owner_required: 必须拥有该校对规则才能删除它。
    if_exists_noop: IF EXISTS 在校对规则不存在时发出 notice 而非 error。
    no_table_column_index_types: DROP COLLATION 不涉及表/列/索引类型组合。
  defaults:
  ...
  coverage_policy:
    ...
    - object_state
  rendering:
    statement_template: "DROP COLLATION {if_exists} {collation_name} {cascade_restrict}"
