# 技能：DROP TEXT SEARCH DICTIONARY
...
- IF EXISTS：如果字典不存在，不报错而是发出通知
- 该语句不涉及列类型，不需要挂靠基表列类型

## 语句作用

官方说明：DROP TEXT SEARCH DICTIONARY — remove a text search dictionary

该 reference 关注全文搜索字典删除操作的权限边界、依赖对象驻留和 IF EXISTS 行为，不涉及表/列/索引组合。

## 测试因子分级

### T1：核心语义因子
- statement_branch：官方语法分支（DROP TEXT SEARCH DICTIONARY / DROP TEXT SEARCH DICTIONARY IF EXISTS）
...
- dependency_status：依赖对象状态（无依赖 / 有 text search configuration 依赖）

### T3：对象名与输入形态因子
- dict_name_shape：text search dictionary 标识符形态
...
- 覆盖 DROP TEXT SEARCH DICTIONARY 全部语法分支。
...
- 必须覆盖目标字典存在时的成功删除路径，以及目标字典不存在时的失败路径。
- 支持 IF EXISTS 时，必须覆盖不存在对象的代表性 no-op 路径。
- 支持 CASCADE | RESTRICT 时，必须覆盖存在依赖对象下的 RESTRICT 失败与 CASCADE 成功路径。
- DROP TEXT SEARCH DICTIONARY 要求执行者是 owner；非 owner 执行路径属于失败路径。
- DROP TEXT SEARCH DICTIONARY 不涉及 table / column 组合，不需要挂靠基表列类型。
- 每个样本必须包含明确的前置对象准备、目标 DROP TEXT SEARCH DICTIONARY 语句、验证语句与清理语句。
...
structured_config:
  ...
  statement:
    ...
    purpose: DROP TEXT SEARCH DICTIONARY — remove a text search dictionary
  syntax_templates:
  - "DROP TEXT SEARCH DICTIONARY [ IF EXISTS ] name [ CASCADE | RESTRICT ]"
  factor_layers:
  - tier: T1
  ...
  factors:
    statement_branch:
      ...
      values:
      - key: branch_drop_ts_dict
        label: DROP TEXT SEARCH DICTIONARY name
      - key: branch_drop_ts_dict_if_exists
        label: DROP TEXT SEARCH DICTIONARY IF EXISTS name
    object_state:
    ...
    dict_name_shape:
      label: text search dictionary 标识符形态
      importance: non_important
      values:
      - simple_id
      - schema_qualified_id
  ...
  coverage_policy:
    ...
    - object_state
  rendering:
    statement_template: "DROP TEXT SEARCH DICTIONARY [ IF EXISTS ] {dict_name} [ CASCADE | RESTRICT ]"
