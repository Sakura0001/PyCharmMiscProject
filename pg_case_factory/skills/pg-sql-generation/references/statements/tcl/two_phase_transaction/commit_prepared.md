...
官方说明：COMMIT PREPARED — commit a transaction that was earlier prepared for two-phase commit
...
structured_config:
  ...
  statement:
    ...
    - commit prepared
    - COMMIT PREPARED
    purpose: COMMIT PREPARED — commit a transaction that was earlier prepared for two-phase commit
  syntax_templates:
  - COMMIT PREPARED transaction_id
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - transaction_state
  rendering:
    statement_template: COMMIT PREPARED transaction_id
