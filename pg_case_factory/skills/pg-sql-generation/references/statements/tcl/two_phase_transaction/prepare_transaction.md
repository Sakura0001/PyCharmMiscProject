...
官方说明：PREPARE TRANSACTION — prepare the current transaction for two-phase commit
...
- 该语句要求 max_prepared_transactions 大于 0，生成器必须提供跳过或环境标注。
...
structured_config:
  ...
  statement:
    ...
    - prepare transaction
    - PREPARE TRANSACTION
    purpose: PREPARE TRANSACTION — prepare the current transaction for two-phase commit
  syntax_templates:
  - PREPARE TRANSACTION transaction_id
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - transaction_state
  rendering:
    statement_template: PREPARE TRANSACTION transaction_id
