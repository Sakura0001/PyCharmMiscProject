...
官方说明：ROLLBACK PREPARED — cancel a transaction that was earlier prepared for two-phase commit
...
- 该语句要求存在已准备事务，清理策略必须避免 prepared transaction 泄漏。
...
structured_config:
  ...
  statement:
    ...
    - rollback prepared
    - ROLLBACK PREPARED
    purpose: ROLLBACK PREPARED — cancel a transaction that was earlier prepared for two-phase commit
  syntax_templates:
  - ROLLBACK PREPARED transaction_id
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - transaction_state
  rendering:
    statement_template: ROLLBACK PREPARED transaction_id
