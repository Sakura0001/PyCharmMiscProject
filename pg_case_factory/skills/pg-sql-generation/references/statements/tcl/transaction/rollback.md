...
来源：https://www.postgresql.org/docs/16/sql-rollback.html
...
官方说明：ROLLBACK — abort the current transaction
...
structured_config:
  ...
  domain: transaction
  skill_name: rollback
  official_source: https://www.postgresql.org/docs/16/sql-rollback.html
  statement:
    key: rollback
    name: ROLLBACK
    aliases:
    ...
    - rollback
    - ROLLBACK
    purpose: ROLLBACK — abort the current transaction
  syntax_templates:
  - ROLLBACK [ WORK | TRANSACTION ] [ AND [ NO ] CHAIN ]
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - transaction_state
  rendering:
    statement_template: ROLLBACK [ WORK | TRANSACTION ] [ AND [ NO ] CHAIN ]
