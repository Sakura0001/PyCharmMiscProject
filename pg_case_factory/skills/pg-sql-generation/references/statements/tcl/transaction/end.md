...
来源：https://www.postgresql.org/docs/16/sql-end.html
...
官方说明：END — commit the current transaction
...
structured_config:
  ...
  domain: transaction
  skill_name: end
  official_source: https://www.postgresql.org/docs/16/sql-end.html
  statement:
    key: end
    name: END
    aliases:
    ...
    - end
    - END
    purpose: END — commit the current transaction
  syntax_templates:
  - END [ WORK | TRANSACTION ] [ AND [ NO ] CHAIN ]
  factor_layers:
  - tier: T1
  ...
  coverage_policy:
    ...
    - transaction_state
  rendering:
    statement_template: END [ WORK | TRANSACTION ] [ AND [ NO ] CHAIN ]
