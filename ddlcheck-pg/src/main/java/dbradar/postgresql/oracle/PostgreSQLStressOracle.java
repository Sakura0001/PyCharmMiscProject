...
public final class PostgreSQLStressOracle implements TestOracle {
    ...
    private void createTableForDqlIfNeeded() throws Exception {
        if (hasDqlCandidateTable()) {
            return;
        }
        for (int attempt = 0; attempt < MAX_GENERATION_ATTEMPTS && !hasDqlCandidateTable(); attempt++) {
            executeDdl(true, true, true);
        }
    }

    private boolean hasDqlCandidateTable() {
        return state.getSchema().getDatabaseTablesWithoutViews().stream()
                .anyMatch(table -> !table.getColumns().isEmpty());
    }
