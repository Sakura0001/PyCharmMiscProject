...
public abstract class GlobalState {
    ...
    public void updateSchema() throws Exception {
        AbstractSchema<?, ?, ?> previousSchema = schema;
        Exception lastTransientError = null;
        int maxAttempts = Math.max(1, getSchemaRefreshRetryCount());
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                setSchema(readSchema());
                return;
            } catch (Exception e) {
                if (!isTransientSchemaRefreshError(e)) {
                    throw e;
                }
                lastTransientError = e;
                if (attempt < maxAttempts) {
                    sleepBeforeSchemaRefreshRetry(e);
                }
            }
        }
    ...
    protected boolean isTransientSchemaRefreshError(Exception e) {
        return false;
    }

    protected int getSchemaRefreshRetryCount() {
        return 1;
    }
    ...
    private void sleepBeforeSchemaRefreshRetry(Exception schemaRefreshError) throws Exception {
        long delayMillis = getSchemaRefreshRetryDelayMillis();
        if (delayMillis <= 0) {
            return;
        }
        try {
            ...
            schemaRefreshError.addSuppressed(e);
            throw schemaRefreshError;
        }
    }
