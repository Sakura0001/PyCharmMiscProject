...
public final class PostgreSQLDMLCoverageTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        verifyBulkInsertSwitchEmitsLargeBatch();
        verifyForeignKeyInsertUsesReferencedValues();
        verifyGeneratedInsertHandlesComplexPostgreSQLValues();
    }
    ...
    private static void verifyBulkInsertSwitchEmitsLargeBatch() throws Exception {
        PostgreSQLGlobalState state = createState("dml_bulk_insert_state", true);
        try {
            execute(state, "CREATE TABLE dml_bulk_insert (c1 INT NOT NULL, c2 TEXT)");
            state.updateSchema();

            state.setRandomly(new Randomly(9001));
            SQLQueryAdapter insert = PostgreSQLProvider.PostgreSQLQueryProvider.INSERT.getQuery(state);
            String sql = insert.getQueryString();
            if (countValuesRows(sql) < 100) {
                throw new AssertionError("Expected --bulk-insert to generate at least 100 VALUES rows: " + sql);
            }
            execute(state, sql);
            requireAtLeast(state, "SELECT count(*) FROM dml_bulk_insert", 100);
        } finally {
            closeQuietly(state);
        }
    }

    private static void verifyForeignKeyInsertUsesReferencedValues() throws Exception {
        PostgreSQLGlobalState state = createState("dml_fk_pool_state");
        try {
            execute(state, "CREATE TABLE dml_fk_parent (id INT PRIMARY KEY)");
            execute(state, "CREATE TABLE dml_fk_child (id INT, parent_id INT NOT NULL REFERENCES dml_fk_parent(id))");
            execute(state, "INSERT INTO dml_fk_parent VALUES (10), (20), (30)");
            state.updateSchema();
            ...
            for (long seed = 1; seed <= 1000; seed++) {
                ...
                SQLQueryAdapter insert = PostgreSQLProvider.PostgreSQLQueryProvider.INSERT.getQuery(state);
                String sql = insert.getQueryString();
                if (!normalize(sql).contains("DML_FK_CHILD")) {
                    continue;
                }
                execute(state, sql);
                requireAtLeast(state, "SELECT count(*) FROM dml_fk_child c JOIN dml_fk_parent p ON p.id = c.parent_id", 1);
                return;
            }
            throw new AssertionError("Expected generated INSERT to target the foreign-key child table");
        } finally {
            closeQuietly(state);
        }
    }

    private static void verifyGeneratedInsertHandlesComplexPostgreSQLValues() throws Exception {
        PostgreSQLGlobalState state = createState("dml_complex_value_state");
        try {
            execute(state, "CREATE TABLE dml_complex_values (c1 INTEGER[] NOT NULL, c2 JSONB NOT NULL, "
                    + "c3 INT4RANGE NOT NULL, c4 INT4MULTIRANGE NOT NULL, c5 TIMETZ NOT NULL, "
                    + "c6 TIMESTAMPTZ NOT NULL)");
            state.updateSchema();

            state.setRandomly(new Randomly(7001));
            SQLQueryAdapter insert = PostgreSQLProvider.PostgreSQLQueryProvider.INSERT.getQuery(state);
            String sql = insert.getQueryString();
            if (!normalize(sql).contains("DML_COMPLEX_VALUES")) {
                throw new AssertionError("Expected generated INSERT to target complex value table: " + sql);
            }
            execute(state, sql);
            requireAtLeast(state, "SELECT count(*) FROM dml_complex_values", 1);
        } finally {
            closeQuietly(state);
        }
    }

    private static int countValuesRows(String sql) {
        ...
        while (index >= 0) {
            rowCount++;
            index = normalized.indexOf("),(", index + 3);
        }
        index = normalized.indexOf("), (");
        while (index >= 0) {
            rowCount++;
            index = normalized.indexOf("), (", index + 4);
        }
        return rowCount;
    }
    ...
    private static PostgreSQLGlobalState createState(String databaseName, boolean allowBulkInsert) throws Exception {
        MainOptions options = new MainOptions();
        Randomly.initialize(options);

        PostgreSQLOptions dbOptions = new PostgreSQLOptions();
        dbOptions.allowBulkInsert = allowBulkInsert;
