...
public final class PostgreSQLSelectQueryGenerationTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        verifyGeneratedCoverageGapSelectsExecute();
    }
    ...
    private static void verifyGeneratedCoverageGapSelectsExecute() throws Exception {
        PostgreSQLGlobalState state = createState("select_query_gap_state");
        try {
            createCoverageTables(state);
            execute(state, "SET SESSION statement_timeout = 1000");
            state.updateSchema();
            ...
            for (long seed = 1; seed <= 5000 && observedKinds.size() < 8; seed++) {
                state.setRandomly(new Randomly(seed));
                SQLQueryAdapter query;
                try {
                    query = PostgreSQLProvider.PostgreSQLQueryProvider.SELECT.getQuery(state);
                ...
                String kind = classifyCoverageGapSelect(normalized);
                if (kind == null || observedKinds.contains(kind)) {
                    continue;
                }
                try (Statement statement = state.getConnection().createStatement()) {
                    statement.executeQuery(sql);
                } catch (Exception e) {
                    throw new AssertionError("Generated " + kind + " SELECT failed: " + sql, e);
                }
                observedKinds.add(kind);
            }
            Set<String> expectedKinds = Set.of("WINDOW", "JSON", "RANGE", "ARRAY", "REGEX",
                    "CROSS_JOIN", "LOCKING", "FUNCTIONS");
            if (!observedKinds.containsAll(expectedKinds)) {
                throw new AssertionError("Expected SELECT generation to cover " + expectedKinds
                        + " but observed " + observedKinds);
            }
        } finally {
            closeQuietly(state);
        }
    ...
    private static String classifyCoverageGapSelect(String normalizedSql) {
        if (normalizedSql.contains(" OVER (")) {
            return "WINDOW";
        }
        if (normalizedSql.contains("JSONB_TYPEOF(") || normalizedSql.contains(" ->> ")
                || normalizedSql.contains(" #> ") || normalizedSql.contains(" ?| ")) {
            return "JSON";
        }
        if (normalizedSql.contains("ISEMPTY(") || normalizedSql.contains("RANGE_MERGE(")
                || normalizedSql.contains("LOWER_INF(")) {
            return "RANGE";
        }
        if (normalizedSql.contains("ARRAY_LENGTH(") || normalizedSql.contains("CARDINALITY(")
                || normalizedSql.contains("ARRAY_CAT(")) {
            return "ARRAY";
        }
        ...
        if (normalizedSql.contains(" CROSS JOIN ")) {
            return "CROSS_JOIN";
        }
        if (normalizedSql.contains(" FOR SHARE") || normalizedSql.contains(" FOR KEY SHARE")
                || normalizedSql.contains(" FOR UPDATE") || normalizedSql.contains(" FOR NO KEY UPDATE")) {
            return "LOCKING";
        }
        if (normalizedSql.contains("CURRENT_DATABASE()") || normalizedSql.contains("MASKLEN(")
                || normalizedSql.contains("ASCII(") || normalizedSql.contains("PG_JIT_AVAILABLE(")
                || normalizedSql.contains("BROADCAST(")) {
            return "FUNCTIONS";
        }
        return null;
    }
    ...
    private static void createCoverageTables(PostgreSQLGlobalState state) throws Exception {
        execute(state, "CREATE TABLE select_safety_a (c1 INT, c2 INT, c3 TEXT, c4 BOOLEAN, c5 DATE, "
                + "c6 JSONB, c7 INT4RANGE, c8 INTEGER[], c9 INET, "
                + "c10 TIME WITH TIME ZONE, c11 TIMESTAMP WITH TIME ZONE)");
        ...
        for (int i = 0; i < 100; i++) {
            execute(state, String.format(Locale.ROOT,
                    "INSERT INTO select_safety_a VALUES (%d, %d, 'a_%d', %s, DATE '2024-01-01' + %d, "
                            + "'{\"value\":%d}'::jsonb, int4range(%d, %d), ARRAY[%d,%d], "
                            + "'10.0.0.%d'::inet, TIME WITH TIME ZONE '12:00:00+08', "
                            + "TIMESTAMP WITH TIME ZONE '2024-01-01 12:00:00+08')",
                    i, i % 17, i, i % 2 == 0 ? "TRUE" : "FALSE", i,
                    i, i, i + 10, i, i + 1, (i % 200) + 1));
            execute(state, String.format(Locale.ROOT,
                    "INSERT INTO select_safety_b VALUES (%d, %d, 'b_%d', %s, DATE '2024-02-01' + %d, "
                            + "'{\"value\":%d}'::jsonb, int4range(%d, %d), ARRAY[%d,%d], "
                            + "'10.0.1.%d'::inet, TIME WITH TIME ZONE '13:00:00+08', "
                            + "TIMESTAMP WITH TIME ZONE '2024-02-01 13:00:00+08')",
