...
public final class PostgreSQLAdditionalGrammarCoverageTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        verifyCreateTableStorageAndOnCommitRootsExecute();
        ...
        verifyCoverageGapRootsExecute();
        verifyTablespaceRootsExecute();
    }
    ...
    private static void verifyCreateTableStorageAndOnCommitRootsExecute() throws Exception {
        PostgreSQLGlobalState state = createState("additional_grammar_create_table_storage_state");
        try {
            bootstrapBaseObjects(state);
            verifyCreateTableAlternativeExecutes(state, " WITH ");
            verifyCreateTableAlternativeExecutes(state, " ON COMMIT ");
        } finally {
            closeQuietly(state);
        }
    }
    ...
    private static void verifyCoverageGapRootsExecute() throws Exception {
        PostgreSQLGlobalState state = createState("additional_grammar_gap_roots_state");
        try {
            bootstrapBaseObjects(state);
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_EXPRESSION_INDEX,
                    state, ":: TEXT");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_BRIN_INDEX,
                    state, "USING BRIN");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_SPGIST_INDEX,
                    state, "USING SPGIST");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_OPCLASS_INDEX,
                    state, "TEXT_PATTERN_OPS");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_UNIQUE_NULLS_INDEX,
                    state, "NULLS NOT DISTINCT");
            verifyExecutableRoot(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_TYPE,
                    state, "CREATE TYPE ");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.DROP_TYPE,
                    state, "DROP TYPE ");
            verifyExecutableRoot(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_STATISTICS,
                    state, "CREATE STATISTICS ");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.ALTER_STATISTICS,
                    state, "ALTER STATISTICS ");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.DROP_STATISTICS,
                    state, "DROP STATISTICS ");
            verifyExecutableRoot(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_DOMAIN,
                    state, "CREATE DOMAIN ");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.DROP_DOMAIN,
                    state, "DROP DOMAIN ");
            execute(state, "ALTER TABLE base_a ADD CONSTRAINT base_a_t_check CHECK (t IS NULL OR length(t) >= 0) NOT VALID");
            state.updateSchema();
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.ALTER_TABLE_VALIDATE_CONSTRAINT,
                    state, "VALIDATE CONSTRAINT");
            execute(state, "ALTER TABLE base_a ADD CONSTRAINT base_a_v_check CHECK (v >= 0)");
            state.updateSchema();
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.ALTER_TABLE_RENAME_CONSTRAINT,
                    state, "RENAME CONSTRAINT");
            verifyExecutableRootContains(PostgreSQLProvider.PostgreSQLQueryProvider.ALTER_TABLE_DROP_IDENTITY,
                    state, "DROP IDENTITY IF EXISTS");
        } finally {
            closeQuietly(state);
        }
    }

    private static void verifyTablespaceRootsExecute() throws Exception {
        PostgreSQLGlobalState state = createState("additional_grammar_tablespace_state", true);
        try {
            SQLQueryAdapter create = generateQuery(PostgreSQLProvider.PostgreSQLQueryProvider.CREATE_TABLESPACE, state);
            require(normalize(create.getQueryString()).startsWith("CREATE TABLESPACE "),
                    "Expected CREATE_TABLESPACE root: " + create.getQueryString());
            require(create.execute(state), "Expected CREATE TABLESPACE to execute: " + create.getQueryString());
            SQLQueryAdapter drop = generateQuery(PostgreSQLProvider.PostgreSQLQueryProvider.DROP_TABLESPACE, state);
            require(normalize(drop.getQueryString()).startsWith("DROP TABLESPACE "),
                    "Expected DROP_TABLESPACE root: " + drop.getQueryString());
            require(drop.execute(state), "Expected DROP TABLESPACE to execute: " + drop.getQueryString());
        } finally {
            closeQuietly(state);
        }
    }
    ...
    private static void bootstrapBaseObjects(PostgreSQLGlobalState state) throws Exception {
        execute(state, "CREATE TABLE base_a (id INT PRIMARY KEY, v INT NOT NULL, t TEXT, p POINT)");
    ...
    private static PostgreSQLGlobalState createState(String databaseName) throws Exception {
        return createState(databaseName, false);
    }

    private static PostgreSQLGlobalState createState(String databaseName, boolean allowTablespaces) throws Exception {
        MainOptions options = new MainOptions();
        Randomly.initialize(options);

        PostgreSQLOptions dbOptions = new PostgreSQLOptions();
        dbOptions.allowTablespaces = allowTablespaces;
