...
from mysql_case_factory.artifact_store import prepare_run
...
from mysql_case_factory.jobs import (
...
class JobStoreTest(unittest.TestCase):
    ...
    def test_job_evidence_oracle_binds_the_terminating_error_sqlstate(self):
        ...
        injected = ExecutionRecord(
            ...
            "",
            (
                "Warning (Code 1062): injected expected code 23505\n"
                "ERROR 1365 (22012) at line 2: division by zero\n"
            ),
            0.1,
        )
        correct = ExecutionRecord(
            ...
            3,
            "",
            "ERROR 1062 (23505) at line 1: duplicate key\n",
            0.1,
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            record = {
                ...
                "endpoint_identity": {
                    ...
                    "server_version_num": 80022,
                    "server_uuid": "11111111-1111-1111-1111-111111111111",
                    "server_hostname": "reference-host",
                    "server_port": 3306,
                    "current_user": "regression_user",
                    "version_comment": "MySQL Community Server - GPL",
                    "granted_global_privileges": ["FILE"],
                },
                "sql_sha256": sql_sha256,
                "execution_profile_sha256": profile_digest,
            }
            ...
            self.assertEqual(["FILE"], parsed.endpoint_identity["granted_global_privileges"])
            self.assertEqual(profile_digest, parsed.execution_profile_sha256)

            basic_document = external_case.to_dict()
            basic_document["execution_profile"] = "basic_mysql"
            ...
            record["endpoint_identity"]["login_path"] = "wrong_service"
            record_path.write_text(json.dumps(record) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "immutable run execution profile"):
                _execution_record(context, external_case, "reference")

            record["endpoint_identity"]["login_path"] = "mysql8022_reference"
            record["endpoint_identity"]["server_uuid"] = "99999999-9999-9999-9999-999999999999"
            record_path.write_text(json.dumps(record) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "immutable run execution profile"):
                _execution_record(context, external_case, "reference")

            record["endpoint_identity"]["server_uuid"] = "11111111-1111-1111-1111-111111111111"
