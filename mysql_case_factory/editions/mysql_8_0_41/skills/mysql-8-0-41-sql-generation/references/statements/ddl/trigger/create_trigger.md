...
structured_config:
  ...
  statement:
    ...
    purpose: Create a MySQL row trigger, including IF NOT EXISTS available since 8.0.29.
  syntax_templates:
    - "CREATE [DEFINER = user] TRIGGER [IF NOT EXISTS] trigger_name trigger_time trigger_event ON tbl_name FOR EACH ROW [trigger_order] trigger_body"
  factor_layers:
    - tier: T1
      factors: [if_not_exists, trigger_time, trigger_event, body_shape, expected_status]
    - tier: T2
  ...
  coverage_policy:
    main_combination_axes: [if_not_exists, trigger_time, trigger_event, body_shape, expected_status]
