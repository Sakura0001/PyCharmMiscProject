...
Official source: https://dev.mysql.com/doc/refman/8.0/en/update.html
...
structured_config:
  ...
  domain: table
  skill_name: update
  official_source: https://dev.mysql.com/doc/refman/8.0/en/update.html
  statement:
    key: update
    name: UPDATE
    aliases: [update]
    purpose: Generate MySQL 8.0.22 single-table and multiple-table UPDATE cases.
  syntax_templates:
  ...
  factor_layers:
    ...
    - tier: T2
      factors: [priority_modifier, ignore_modifier, order_limit_shape, partition_clause]
  factors:
    statement_branch:
      label: UPDATE branch
      importance: important
      values: [single_table, multiple_table]
    condition_shape:
  ...
  coverage_policy:
    ...
    non_main_factors: [priority_modifier, ignore_modifier, order_limit_shape, partition_clause]
    python_expand_threshold: 160
  rendering:
    statement_template: "UPDATE {ignore_sql}{table_name} SET varchar_col = 'updated' WHERE id_col = 1"
    verification_query_template: "SELECT varchar_col FROM {table_name} WHERE id_col = 1"
