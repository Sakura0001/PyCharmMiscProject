"""Fail-closed feature applicability accounting for a frozen MySQL edition.
...
DEFAULT_LEDGER_PATH = Path(
    "editions/mysql_8_0_22/skills/mysql-8-0-22-sql-generation/"
    "references/common/mysql_8_0_22_factor_audit.tsv"
)
SHIPPED_STATEMENT_COUNT = 112
SHIPPED_FACTOR_PAIR_COUNT = 505
SHIPPED_VALUE_ROW_COUNT = 1365
SHIPPED_UNIVERSE_SEMANTIC_SHA256 = (
    "bd53fa6560473a8c0501d37561b30a1e71f6004270806392bf5ec9a6ef0eddd4"
)
...
COMPATIBILITY_TARGETS = frozenset(
    {"mysql-community-8.0.22", "mysql-community-8.0.41"}
)
DECISION_STATUSES = frozenset({"pending", "covered", "justified_exclusion"})
EXECUTABLE_OUTCOMES = frozenset({"success", "expected_failure"})
EXECUTION_PROFILES = frozenset({"basic_mysql", "external_isolated"})
...
MYSQL_LOCATOR_PATTERN = re.compile(r"^mysql(?:8022|8041):[a-z0-9][a-z0-9._-]*$")
...
def applicability_axis_id(statement_key: str) -> str:
    ...
    if not isinstance(statement_key, str) or not STATEMENT_KEY_PATTERN.fullmatch(statement_key):
        raise ApplicabilityValidationError(
            "statement key cannot form an applicability axis ID"
        )
...
def _universe_semantic_sha256(rows: Sequence[CatalogRow]) -> str:
    digest = hashlib.sha256(b"mysql-case-feature-applicability-universe-v1\n")
    for row in rows:
        payload = json.dumps(
            row.ledger_dict(),
            ensure_ascii=False,
...
def load_shipped_applicability_universe(
    ...
    if universe.semantic_sha256 != SHIPPED_UNIVERSE_SEMANTIC_SHA256:
        raise ApplicabilityValidationError(
            ...
            "pinned MySQL 8.0.22 edition snapshot"
        )
    return universe


def load_edition_applicability_universe(
    edition_root: str | Path,
) -> ApplicabilityUniverse:
    """Load the factor/value ledger frozen inside one edition snapshot."""

    root = Path(edition_root).resolve()
    try:
        manifest = _load_yaml(root / "edition.yaml", "edition manifest")
        skill = _mapping(manifest.get("skill"), "edition.skill")
        skill_root = (root / _required_string(skill, "root", "edition.skill")).resolve()
        if root not in skill_root.parents:
            raise ApplicabilityValidationError("edition skill root escapes edition")
        support = _load_yaml(
            skill_root / "references/common/statement_support_inventory.yaml",
            "statement support inventory",
        )
        factor_audit = _mapping(support.get("factor_audit"), "statement_support.factor_audit")
        ledger = (skill_root / _required_string(factor_audit, "path", "statement_support.factor_audit")).resolve()
        if skill_root not in ledger.parents:
            raise ApplicabilityValidationError("edition factor audit escapes skill root")
        expected = UniverseCounts(
            statements=_positive_int(factor_audit, "statement_count", "statement_support.factor_audit"),
            statement_factor_pairs=_positive_int(factor_audit, "factor_pair_count", "statement_support.factor_audit"),
            statement_factor_values=_positive_int(factor_audit, "factor_value_count", "statement_support.factor_audit"),
        )
    except OSError as exc:
        raise ApplicabilityValidationError(f"cannot load edition applicability metadata: {exc}") from exc
    universe = load_applicability_universe(ledger, expected_counts=expected)
    expected_sha = _sha256_string(
        factor_audit,
        "sha256",
        "statement_support.factor_audit",
    )
    if universe.source_sha256 != expected_sha:
        raise ApplicabilityValidationError("edition applicability ledger SHA-256 mismatch")
    return universe
...
class MatrixWitnessCoverageAudit:
    ...
    edition_compatibility_point_rows: int
    ...
    def complete(self) -> bool:
        return (
            ...
            == self.combination_group_rows + self.edition_compatibility_point_rows
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "edition_compatibility_point_rows": self.edition_compatibility_point_rows,
...
class _MatrixEvidence:
    ...
    edition_points_by_id: Mapping[str, Mapping[str, Any]]
...
def _validate_locator_evidence(
    ...
    for locator in locators:
        ...
        if feature_match:
            ...
            continue
        if MYSQL_LOCATOR_PATTERN.fullmatch(locator):
            has_mysql8022 = True
            continue
        raise ApplicabilityValidationError(
    ...
    if not has_mysql8022:
        raise ApplicabilityValidationError(
            f"{location} requires a mysql8022:<topic> or mysql8041:<topic> locator"
        )
...
def _validate_matrix_witness(
    ...
    if evidence is None:
        ...
        edition_points_by_id: dict[str, Mapping[str, Any]] = {}
        compatibility = matrix.get("edition_compatibility")
        if compatibility is not None:
            if not isinstance(compatibility, Mapping):
                raise ApplicabilityValidationError(
                    f"{location} matrix edition_compatibility must be a mapping"
                )
            points = compatibility.get("test_points")
            if not isinstance(points, list):
                raise ApplicabilityValidationError(
                    f"{location} matrix edition_compatibility.test_points must be a list"
                )
            if points and compatibility.get("target_version") not in {"8.0.22", "8.0.41"}:
                raise ApplicabilityValidationError(
                    f"{location} edition compatibility points require an exact supported target_version"
                )
            for point in points:
                if not isinstance(point, Mapping):
                    raise ApplicabilityValidationError(
                        f"{location} matrix edition compatibility point must be a mapping"
                    )
                ...
                if (
                    ...
                    raise ApplicabilityValidationError(
                        f"{location} matrix edition compatibility point has no stable id"
                    )
                if point_id in groups_by_id or point_id in edition_points_by_id:
                    raise ApplicabilityValidationError(
                        f"{location} matrix coverage witness ID is not unique: {point_id}"
                    )
                if point.get("oracle") != "reference_parity":
                    raise ApplicabilityValidationError(
                        f"{location} edition point {point_id} must use oracle=reference_parity"
                    )
                sql = point.get("sql")
                if not isinstance(sql, str) or not sql.strip():
                    raise ApplicabilityValidationError(
                        f"{location} edition point {point_id} must declare non-empty SQL"
                    )
                ...
                if (
                    ...
                    raise ApplicabilityValidationError(
                        f"{location} edition point {point_id} must declare unique affected_factors"
                    )
                if not isinstance(affected_values, Mapping) or set(
                    affected_values
                ) != set(affected_factors):
                    raise ApplicabilityValidationError(
                        f"{location} edition point {point_id} affected_values must exactly "
                ...
                for factor_name, values in affected_values.items():
                    if (
                        ...
                        raise ApplicabilityValidationError(
                            f"{location} edition point {point_id} affected_values.{factor_name} "
                            "must be a non-empty unique string list"
                        )
                edition_points_by_id[point_id] = point
        evidence = _MatrixEvidence(
            ...
            edition_points_by_id=edition_points_by_id,
        )
    ...
    point = evidence.edition_points_by_id.get(witness.combination_group_id)
    if group is None and point is None:
        raise ApplicabilityValidationError(
            f"{location}.combination_group_id must identify exactly one coverage "
            "witness (combination group or edition compatibility point)"
        )
    ...
    if point is not None:
        ...
        if (
            ...
            raise ApplicabilityValidationError(
                f"{location} edition coverage witness {witness.combination_group_id} does not "
...
def audit_universe_matrix_witness_coverage(
    ...
    *,
    ...
    """Prove every canonical row has a matrix group or edition point witness."""
    ...
    return MatrixWitnessCoverageAudit(
        ...
        edition_compatibility_point_rows=edition_rows,
...
def scaffold_feature_applicability(
    ...
    *,
    ...
    """Create a deterministic per-statement skeleton with every value pending.
    ...
    if compatibility_target not in COMPATIBILITY_TARGETS:
        raise ApplicabilityValidationError("unsupported MySQL compatibility target")
...
def load_feature_applicability_index(
    ...
    *,
    ...
    expected_counts: UniverseCounts | None = None,
    expected_compatibility_target: str | None = None,
    draft: bool = False,
    ...
    if compatibility_target not in COMPATIBILITY_TARGETS:
        raise ApplicabilityValidationError(
            f"{location}.compatibility_target must be an exact supported MySQL edition"
        )
    if (
        expected_compatibility_target is not None
        and compatibility_target != expected_compatibility_target
    ):
        raise ApplicabilityValidationError(
            f"{location}.compatibility_target does not match the requested edition"
        )
    ...
    if (
        ...
        raise ApplicabilityValidationError(
            f"{location}.universe does not match the pinned MySQL 8.0.22 semantic snapshot"
        )
...
def refresh_feature_applicability_index(
    ...
    *,
    ...
    feature_id = _required_string(document, "feature_id", location)
    if document.get("compatibility_target") not in COMPATIBILITY_TARGETS:
        raise ApplicabilityValidationError(
            f"{location}.compatibility_target must be an exact supported MySQL edition"
        )
    compatibility_target = str(document["compatibility_target"])
...
def _build_compiled_plan_document(
    ...
    for statement_key, covered in covered_statements.items():
        ...
        generated_axes[axis_id] = {
            ...
            "description": (
                f"Every feature-reviewed covered MySQL edition factor value for "
