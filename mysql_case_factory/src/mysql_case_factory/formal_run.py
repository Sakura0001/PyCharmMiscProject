"""Immutable, fail-closed lifecycle for formal MySQL compatibility runs.
...
def _metadata(inputs: FormalRunInputs) -> tuple[dict[str, Any], bytes, bytes, bytes]:
    ...
    metadata = {
        "formal_run": True,
        "compatibility_target": inputs.manifest.compatibility_target,
...
def validate_formal_run(
    ...
    *,
    ...
    if metadata["compatibility_target"] not in {
        "mysql-community-8.0.22",
        "mysql-community-8.0.41",
    }:
        raise ValueError("formal run compatibility_target must be an exact supported MySQL edition")
    ...
    if manifest.compatibility_target != metadata["compatibility_target"]:
        raise ValueError("persisted feature manifest target differs from run.json")
    _require_resolved_questions(manifest)
