...
REFERENCES_ROOT = Path("skills/mysql-sql-generation/references")
...
TYPE_CATALOG = REFERENCES_ROOT / "common" / "mysql80_type_catalog.md"
...
def _relative_reference(root: Path, path: Path) -> str:
    try:
        ...
        if rel.parts[:4] == ("skills", "mysql-sql-generation", "references", "statements"):
...
def _validate_column_type_catalog(result: AuditResult, root: Path, path: Path, matrix: Mapping[str, Any]) -> None:
    ...
    if "mysql80_type_catalog" not in inventory_source or not type_catalog_path.exists():
        result.errors.append(f"{path}: column_type_coverage requires mysql80_type_catalog")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit MySQL 8.0.22 statement combination matrices.")
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="mysql_case_factory project root")
