...
Official source: https://dev.mysql.com/doc/refman/8.0/en/alter-database.html
...
structured_config:
  ...
  statement:
    key: alter_schema
    name: ALTER SCHEMA
    aliases: [alter schema]
    purpose: MySQL ALTER SCHEMA synonym for ALTER DATABASE.
  syntax_templates:
    - "ALTER SCHEMA [db_name] alter_option ..."
  factor_layers:
    - tier: T1
      factors: [schema_name_shape, expected_status]
    - tier: T2
  ...
  coverage_policy:
    main_combination_axes: [schema_name_shape, expected_status]
    non_main_factors: [charset_option, collation_option, encryption_option, read_only_option]
    python_expand_threshold: 80
  rendering:
    statement_template: "ALTER SCHEMA {database_name} CHARACTER SET utf8mb4"
