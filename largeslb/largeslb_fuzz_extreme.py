...
TWO_MB = 2 * 1024 * 1024
DEFAULT_SUB2M_TARGET_BYTES = 1835008
SUB2M_BURST_KIND = "sub2m_concurrent_burst"
...
def burst_row_offset(plan: OperationPlan, bucket_row_count: int, worker_count: int) -> int:
    if bucket_row_count < plan.rows_per_bucket:
        raise ValueError("bucket_row_count must be >= plan.rows_per_bucket")
    if worker_count < 1:
        raise ValueError("worker_count must be positive")
    required_rows = worker_count * plan.rows_per_bucket
    if bucket_row_count < required_rows:
        raise ValueError("bucket does not have enough rows for disjoint burst ranges")
    window_count = max(1, bucket_row_count // required_rows)
    window_index = (plan.sequence - 1) % window_count
    return window_index * required_rows + plan.worker_id * plan.rows_per_bucket
...
class ScenarioGenerator:
    def __init__(
        ...
        sub2m_concurrent_burst: bool = False,
        sub2m_target_bytes: int = DEFAULT_SUB2M_TARGET_BYTES,
    ):
        if bucket_count < 1:
            raise ValueError("bucket_count must be positive")
        if sub2m_target_bytes >= TWO_MB:
            raise ValueError("sub2m_target_bytes must be less than 2MB")
        self.seed = seed
        ...
        self.sub2m_concurrent_burst = sub2m_concurrent_burst
        self.sub2m_target_bytes = sub2m_target_bytes
        if sub2m_concurrent_burst:
            burst_fields = [
                field
            ...
            if not burst_fields:
                raise ValueError("sub2m burst mode requires at least one non-binary target field >= 8KB")
            self.weighted_kinds = [(SUB2M_BURST_KIND, 100)]
            return
        # 极致触发模式：只保留能触发 SLB > 2MB 分拆的场景
        # same_page_hot: 同 page-id 累积，不受 Slice 分散影响，最可靠
        # large_single_bucket: 单桶大 payload，集中一个 Slice
        # multi_batch_large: 多批次同桶累积，依赖延迟刷盘
        # boundary_2m: 精确 2MB 边界测试
        # 去掉: small_fast_path(不触发), multi_bucket_large(分散),
        #        char_varchar_boundary(小payload), blob/text_family(权重不够高)
        self.weighted_kinds = [
            ("same_page_hot", 40),
            ("large_single_bucket", 30),
    ...
    def next_plan(self, worker_id: int, sequence: int) -> OperationPlan:
        ...
        if kind == SUB2M_BURST_KIND:
            target_field = self._choose_target_field(kind)
            payload_len = self._choose_payload_len(target_field, kind)
            rows_per_bucket = max(1, self.sub2m_target_bytes // payload_len)
            while rows_per_bucket * payload_len >= TWO_MB:
                rows_per_bucket -= 1
        ...
        if kind == "large_single_bucket":
            ...
            # 极致触发：提高到 256-512 行，确保单桶 redo 远超 2MB
            # 512 × 256KB(longtext safe_length) = 128MB redo 集中在 1-2 Slice
            # 384 × 128KB = 48MB redo
            # 即使最小的 256 × 64KB = 16MB redo 也远超 2MB
            return OperationPlan(
                op_id=op_id,
                kind=kind,
                worker_id=worker_id,
        ...
        if kind == "same_page_hot":
            bucket = self.rng.randrange(self.bucket_count)
            # 极致触发：提高到 128/192/256 次重复更新，更大 payload
            # 128 × 256KB = 32MB redo 到同 page-id，必定触发分拆
            # 192 × 128KB = 24MB redo 到同 page-id
            # 256 × 64KB = 16MB redo 到同 page-id
            return OperationPlan(
                op_id=op_id,
                kind=kind,
                worker_id=worker_id,
    ...
    def _choose_target_field(self, kind: str) -> str:
        ...
        if kind in ("text_family_large", "large_single_bucket", "multi_batch_large", "multi_bucket_large", "same_page_hot", SUB2M_BURST_KIND):
            candidates = [
                field
                for field in self.target_fields
                if not FIELD_SPECS[field].is_binary and FIELD_SPECS[field].safe_max_len >= 8192
    ...
    def _choose_payload_len(self, target_field: str, kind: str) -> int:
        ...
        if kind == SUB2M_BURST_KIND:
            candidates = [
                length
                for length in spec.safe_lengths
                if 8192 <= length <= 65536 and length < TWO_MB
            ]
            return self.rng.choice(candidates or [min(spec.safe_max_len, 32768)])
...
class FuzzRunner:
    def __init__(self, args: argparse.Namespace):
        ...
        self.burst_commit_barrier: Optional[threading.Barrier] = None
        if args.sub2m_concurrent_burst and args.workers > 1:
            self.burst_commit_barrier = threading.Barrier(args.workers)
    ...
    def worker_loop(self, worker_id: int) -> None:
        ...
        generator = ScenarioGenerator(
            ...
            sub2m_concurrent_burst=self.args.sub2m_concurrent_burst,
            sub2m_target_bytes=self.args.sub2m_target_bytes,
        )
    ...
    def commit_plan(
        ...
        try:
            with primary.transaction() as conn:
                ...
                self.wait_for_burst_commit(plan)
    ...
    def wait_for_burst_commit(self, plan: OperationPlan) -> None:
        if plan.kind != SUB2M_BURST_KIND or self.burst_commit_barrier is None:
            return
        try:
            self.burst_commit_barrier.wait(timeout=self.args.burst_barrier_timeout)
        except threading.BrokenBarrierError as exc:
            raise FuzzFailure(
                "burst_barrier_broken",
                "sub2m burst workers did not reach commit barrier together",
                {
                    "op_id": plan.op_id,
                    "worker_id": plan.worker_id,
                    "sequence": plan.sequence,
                    "timeout": self.args.burst_barrier_timeout,
                },
            ) from exc
    ...
    def select_rows_for_update(
        ...
        with conn.cursor() as cur:
            for bucket in plan.buckets:
                ...
                if plan.kind == SUB2M_BURST_KIND:
                    try:
                        offset = burst_row_offset(plan, count, self.args.workers)
                    except ValueError as exc:
                        raise FuzzFailure(
                            "insufficient_burst_rows",
                            "bucket does not contain enough rows for disjoint concurrent burst ranges",
                            {
                                "bucket": bucket,
                                "count": count,
                                "needed": self.args.workers * plan.rows_per_bucket,
                                "workers": self.args.workers,
                                "rows_per_worker": plan.rows_per_bucket,
                            },
                        ) from exc
                    cur.execute(
                        """
                        ...
                        LIMIT 1 OFFSET %s
                        """,
                        (self.run_id, bucket, offset),
                    )
                    start_row = cur.fetchone()
                    if not start_row:
                        raise FuzzFailure(
                            "missing_burst_start_row",
                            "burst start row was not found for computed offset",
                            {"bucket": bucket, "offset": offset, "count": count},
                        )
                    cur.execute(
                        """
                        SELECT id, bucket, version
                        FROM lslb_fuzz_rows
                        WHERE run_id=%s AND bucket=%s AND id >= %s
                        ORDER BY id
                        LIMIT %s
                        FOR UPDATE
                        """,
                        (self.run_id, bucket, int(start_row["id"]), plan.rows_per_bucket),
                    )
                else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Run only concurrent burst plans: each worker writes a large transaction below 2MB and waits at a commit barrier.",
    )
    parser.add_argument(
        "--sub2m-target-bytes",
        type=int,
        default=DEFAULT_SUB2M_TARGET_BYTES,
        help="Approximate payload bytes per worker transaction in sub2m burst mode; must be < 2097152.",
    )
    parser.add_argument(
        "--burst-barrier-timeout",
        type=float,
        default=30.0,
        help="Seconds a burst worker waits for peers before aborting the barrier.",
    )
    parser.add_argument(
