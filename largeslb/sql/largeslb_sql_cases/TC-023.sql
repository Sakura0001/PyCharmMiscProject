...
--   可动态或重启配置相关参数
-- 测试步骤:
--   1. 分别设置 slice_tlb_size 为 4KB、64KB、2MB。
--   2. 分别设置 slice_flush_size_threshold 为 4KB、64KB、2MB。
--   3. 分别设置 slice_tlb_size_max 为 64KB、2MB。
--   4. 尝试非法组合：slice_tlb_size > slice_tlb_size_max、slice_flush_size_threshold > slice_tlb_size_max。
-- 预期结果:
--   合法范围内行为正确；非法组合被拒绝或有明确错误；不会进入不可诊断状态。
-- 验证点:
--   参数约束、错误提示、重启/动态变更行为。
--
-- LargeSLB 执行前推荐参数:
