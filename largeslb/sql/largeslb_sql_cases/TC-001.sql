...
--   回归
-- 前置条件:
--   enable_large_slb=1，业务写入正常，构造每个 slice 的 SLB 小于 2MB
-- 测试步骤:
--   1. 使用 sysbench 或自定义脚本持续写入普通小事务。
--   2. 观察 GroupFlushLogHarvester 未设置 split 标志。
--   3. 校验每个 GFB 到目标 slice 仍最多一个 TLB。
--   4. 校验主库、只读实例数据一致。
-- 预期结果:
--   fast path 生效；无 split TLB；large_mtr 不增长；QPS/TPS 与基线无明显退化。
-- 验证点:
--   无拆分路径性能、旧逻辑兼容、指标不误报。
--
-- LargeSLB 执行前推荐参数:
