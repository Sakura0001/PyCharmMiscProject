...
--   组件
-- 前置条件:
--   当前 TLB 已含前一个 SLB 的部分数据，剩余空间不足以容纳下一条 log record
-- 测试步骤:
--   1. 构造当前 TLB 非空且空间不足。
--   2. 调用 fillSliceFragment。
--   3. 观察先 flush 当前 TLB，再分配/填充新 TLB。
-- 预期结果:
--   不覆盖旧数据；不产生超限 TLB；flush 顺序与 LSN 顺序一致。
-- 验证点:
--   TLB 空间判断、flush 顺序、动态临时 TLB 分配。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
