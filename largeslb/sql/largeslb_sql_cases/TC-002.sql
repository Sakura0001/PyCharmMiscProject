...
--   正向
-- 前置条件:
--   enable_large_slb=1；innodb_log_write_max_size 最大只有 512KB，因此单次 redo write 不会达到 2MB；使用单会话大事务产生多批 redo/GFB，并使其在同一 slice flush session 中汇聚
-- 测试步骤:
--   1. 设置 innodb_log_write_max_size=524288、innodb_log_write_min_time_interval=1000000、innodb_log_write_min_size=131072。
--   2. 构造单会话大事务，使多批 redo 映射到同一 slice 后累计超过当前 TLB/SLB 阈值。
--   3. 等待 flush 完成并观察 split segment/TLB 日志。
--   4. 校验业务事务提交成功并可查询。
-- 预期结果:
--   多批 GFB 在同一 slice 汇聚后触发 LargeSLB，被拆成多个 split TLB，全部 flush 成功；不 crash；业务结果正确。
-- 验证点:
--   多批 redo/GFB 汇聚、split 触发、TLB 数正确、flush 完整、数据正确。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
