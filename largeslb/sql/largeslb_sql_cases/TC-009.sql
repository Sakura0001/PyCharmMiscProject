...
--   UT / 组件
-- 前置条件:
--   准备无拆分、单 slice 拆分、多 slice 拆分、多 GFB 汇聚四类输入
-- 测试步骤:
--   1. 分别输入四类 page_ref_map。
--   2. 计算期望 TLB 总数。
--   3. 对比 GroupFlushLogHarvester 维护的 GFB TLB 个数。
-- 预期结果:
--   无拆分时仍走快速路径；拆分时总数精确，不少算导致提前 SYNC，不多算导致卡住。
-- 验证点:
--   fast path、split 标志、计数准确性。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
