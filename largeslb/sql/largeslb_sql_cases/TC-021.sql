...
--   兼容 / 可靠性
-- 前置条件:
--   sysbench 标准模型表已导入数据；写负载运行；旧版本或旧配置可触发 LargeSLB 异常
-- 测试步骤:
--   1. 配置 innodb_log_write_max_size、innodb_log_write_min_time_interval、innodb_log_write_min_size 为大值。
--   2. 触发历史异常场景。
--   3. 替换到支持 LargeSLB 的版本。
--   4. 重启拉起实例并校验数据。
-- 预期结果:
--   版本替换后实例正常拉起，不再反复 crash；数据一致。
-- 验证点:
--   历史问题修复、恢复兼容、数据完整性。
--
-- LargeSLB 执行前推荐参数:
