...
--   正向
-- 前置条件:
--   多个并发写入线程命中同一目标 slice，使多个 GFB 汇聚后 SLB 超阈值
-- 测试步骤:
--   1. 启动多连接写入热点 page/slice。
--   2. 控制 flush 周期，使多个 GFB 被组织到同一 slice flush session。
--   3. 观察 split TLB 与 GFB_info 依赖关系。
--   4. 校验所有 GFB 的 endLSN 最终安全推进。
-- 预期结果:
--   多 GFB 场景下 TLB 总数计算正确；每个 GFB 所依赖的 split TLB 都完成后才推进。
-- 验证点:
--   跨 GFB 计数、依赖关系、SYNC_MSG_SLICE 时机。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
