...
--   正向
-- 前置条件:
--   构造写入命中多个 slice，至少两个 slice 产生 split TLB
-- 测试步骤:
--   1. 并发写入不同表/页使多个 slice 触发 LargeSLB。
--   2. 观察每个 slice 的 TLB 计数和 flush completion。
--   3. 校验 GFB 总完成条件。
-- 预期结果:
--   每个 slice 的 split 独立正确；GFB 总 TLB 完成计数等于各 slice split TLB 之和；所有完成后才推进 CV-LSN。
-- 验证点:
--   多 slice 计数、并发完成顺序、全局推进条件。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
