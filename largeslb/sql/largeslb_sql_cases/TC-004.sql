...
--   正向 / UT
-- 前置条件:
--   可通过 UT 或 debug 注入构造同一 page-id 的多条 COMPACT redo，总大小超过 TLB 上限
-- 测试步骤:
--   1. 调用 sal_scan_redo_log_buffer 构建 page_map_t。
--   2. 逐条加入同 page-id log record。
--   3. 在超过上限时检查旧 segment 被 seal，并创建新 segment。
-- 预期结果:
--   space_page_key::compSeg 从 0 递增；同一 page-id 存在多个 map entry；head segment 的 sealed 非 0 时表示当前活跃 segment；非 head segment 的 sealed 与自身 segment 编号一致。
-- 验证点:
--   segment 编号、sealed 语义、head/current segment 查找。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
