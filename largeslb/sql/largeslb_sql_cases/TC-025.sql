...
--   异常
-- 前置条件:
--   可注入 Slice Store 单个 split TLB flush 失败
-- 测试步骤:
--   1. 触发 LargeSLB，注入第 N 个 split TLB 首次 flush 失败。
--   2. 观察 completion、guard、SYNC 消息。
--   3. 解除故障后重试 flush。
-- 预期结果:
--   失败期间不推进 safe LSN，不发送 SYNC_MSG_SLICE；重试成功后完整推进；无重复写入导致的数据错误。
-- 验证点:
--   失败隔离、重试幂等、LSN 安全。
--
-- LargeSLB 执行前推荐参数:
