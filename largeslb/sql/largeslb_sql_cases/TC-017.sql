...
--   集成
-- 前置条件:
--   LargeSLB 写入持续运行，开启 recycle/release 相关日志或指标
-- 测试步骤:
--   1. 触发多个 LargeSLB。
--   2. 记录 CV-LSN、recycle LSN、release LSN。
--   3. 注入部分 TLB flush 延迟。
-- 预期结果:
--   recycle LSN 和 release LSN 依赖 safe CV-LSN，不越过未完成 LargeSLB 的安全边界。
-- 验证点:
--   日志回收安全、buffer release 安全。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
