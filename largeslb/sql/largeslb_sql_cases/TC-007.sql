...
--   UT / 组件
-- 前置条件:
--   构造乱序 page_map_t，包含多个 page-id、多个 compSeg、多个 LSN 范围
-- 测试步骤:
--   1. 执行 mapPagesToSlices。
--   2. 读取每个 slice 的 page_ref_map_t。
--   3. 检查 log record 的 LSN 顺序。
-- 预期结果:
--   每个 slice 中的记录按 LSN 递增；split-SLB LSN 范围无重叠、无倒序。
-- 验证点:
--   排序稳定性、跨 page-id 顺序、跨 segment 顺序。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
