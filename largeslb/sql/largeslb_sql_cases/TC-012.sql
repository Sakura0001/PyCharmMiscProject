...
--   组件 / 可靠性
-- 前置条件:
--   构造大型 SLB，需要多个 split TLB 才能完整复制
-- 测试步骤:
--   1. 在第一个 split TLB 复制后暂停。
--   2. 检查 redo log buffer 是否仍被持有。
--   3. 所有 split TLB 复制完成后再次检查。
-- 预期结果:
--   所有日志被消费并复制到 split TLB 之前，redo log buffer 不释放回 SQL 层；全部复制后正常释放。
-- 验证点:
--   buffer 生命周期、并发安全、无 use-after-free。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
