...
--   组件 / 正向
-- 前置条件:
--   一个 LargeSLB 拆为 3 个 TLB，前 2 个为非尾部 split TLB
-- 测试步骤:
--   1. flush 第 1 个非尾部 TLB。
--   2. 尝试推进 persistLSN 到该 TLB endLSN。
--   3. flush 第 2 个非尾部 TLB 并再次尝试。
--   4. flush 尾部 TLB。
-- 预期结果:
--   非尾部 endLSN 被加入 m_lsnGuard，persistLSN 不得推进到 unsafe LSN；尾部完成后推进到 GFB 安全边界，并清理小于等于 safe persist LSN 的 unsafe LSN。
-- 验证点:
--   unsafe add/check/remove、safe boundary、guard 清理。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
