...
--   集成
-- 前置条件:
--   一个 GFB 拆成多个 TLB，可注入部分 TLB flush 延迟
-- 测试步骤:
--   1. 触发 LargeSLB。
--   2. 延迟最后一个 split TLB 的 async-flush completion。
--   3. 观察 SyncMsgSliceManager 状态和消息发送。
--   4. 恢复最后一个 completion。
-- 预期结果:
--   延迟期间不发送该 GFB 的 SYNC_MSG_SLICE，不推进 SQL Replica CV-LSN；最后一个 TLB 完成后才发送并推进。
-- 验证点:
--   TLB completion 计数、SYNC 发送时机、CV-LSN 安全性。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
