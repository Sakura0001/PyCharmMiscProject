...
--   异常 / 负向 / 不支持
-- 前置条件:
--   隔离可丢弃实例；优先用 debug/UT 构造单条 redo record 含元数据后超过 2MB。SQL 脚本只能作为尝试性压力输入，不保证一定产生单条 redo record
-- 测试步骤:
--   1. 输入单条超大 redo record。
--   2. 执行 parsing/flush 路径。
--   3. 观察实例 crash、错误栈和最后 LSN。
--   4. 清理或重建隔离实例。
-- 预期结果:
--   实例直接 crash；该场景明确不属于 LargeSLB 支持范围；不得将 crash 误判为缺陷修复失败；不得在共享环境批量执行。
-- 验证点:
--   out-of-scope 场景隔离、crash 行为符合预期、不会被测试报告误标为支持。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
...
--   警告: TC-006 是不支持场景，单条 redo >2MB 预期直接 crash；SQL 不保证一定生成单条 redo，最终以 debug/UT 注入确认为准。
