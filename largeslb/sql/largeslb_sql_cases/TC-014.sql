...
--   组件 / 回归
-- 前置条件:
--   支持控制 flush completion handler 或 LSN-Watcher 分别成为推进 persistLSN 的触发方
-- 测试步骤:
--   1. 场景 A：由 completion handler 推进 persistLSN。
--   2. 场景 B：阻塞 completion 推进，由 LSN-Watcher 发现并推进。
--   3. 对比 unsafe guard 行为。
-- 预期结果:
--   两条路径都检查 m_lsnGuard；都只推进 safe persist LSN；unsafe LSN 清理一致。
-- 验证点:
--   watchPersistLsn 历史问题回归、双路径一致性。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
