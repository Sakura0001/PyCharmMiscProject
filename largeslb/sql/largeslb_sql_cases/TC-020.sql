...
--   可靠性 / 补洞
-- 前置条件:
--   Slice Store 支持模拟缺失某个 split-SLB
-- 测试步骤:
--   1. 触发 LargeSLB 并成功生成多个 split TLB。
--   2. 人为删除或屏蔽其中一个非尾部或尾部 split-SLB。
--   3. 触发 gossip/补洞。
--   4. 校验 served LSN 和数据。
-- 预期结果:
--   Slice Store 可按 split-SLB 粒度识别空洞并填充；补洞后 served LSN 连续；数据可读。
-- 验证点:
--   gap 检测、gossip 填充、尾部/非尾部差异。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
