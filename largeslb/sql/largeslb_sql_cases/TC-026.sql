...
--   可靠性
-- 前置条件:
--   LargeSLB redo 已写 PWAL，可在不同阶段 kill 进程
-- 测试步骤:
--   1. 在 parsing 后、部分 TLB flush 后、全部 TLB flush 前、全部 TLB flush 后四个阶段分别 kill -9。
--   2. 重启实例。
--   3. 观察 redo replay、slice flush 和数据校验。
-- 预期结果:
--   不发生反复 crash；replay 后要么回到上一个 safe LSN，要么完成 LargeSLB；数据一致。
-- 验证点:
--   crash recovery、原子性、幂等。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
