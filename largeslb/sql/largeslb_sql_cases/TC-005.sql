...
--   边界
-- 前置条件:
--   可精确控制 log payload 与元数据大小
-- 测试步骤:
--   1. UT/debug 注入精确构造含 RECORD_LSN_SIZE、SLICE_LOG_RECORD_FOOTER_SIZE、PACK_HEADER_SIZE、SLB_HEADER_SIZE、SLB_FOOTER_SIZE 和 Intel Disk padding 后总大小为 2MB-1。
--   2. UT/debug 注入精确构造总大小等于 2MB。
--   3. UT/debug 注入精确构造总大小为 2MB+1。
--   4. SQL 辅助脚本使用 511/512/513 行 x 4KB 作为近似压力流量，最终边界判定以 UT/debug size 统计为准。
-- 预期结果:
--   <=2MB 不拆分或不超过单 TLB 上限；>2MB 必须拆分；size 计算包含所有元数据和 padding。
-- 验证点:
--   size 计算精确性、off-by-one、padding 影响。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
