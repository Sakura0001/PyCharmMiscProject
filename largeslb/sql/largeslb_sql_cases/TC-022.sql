...
--   参数 / 兼容
-- 前置条件:
--   sysbench 标准模型表；sal_tlb_max_size 设置为小值便于触发
-- 测试步骤:
--   1. 设置 enable_large_slb=0，触发 LargeSLB 对照场景并记录行为。
--   2. 设置 enable_large_slb=1。
--   3. 使用相同负载再次触发。
--   4. 对比日志、指标和恢复结果。
-- 预期结果:
--   开启后 LargeSLB 正常处理并能拉起；关闭时行为符合旧版本/旧配置预期且不得误报为通过。
-- 验证点:
--   开关有效性、对照实验、观测指标。
--
-- LargeSLB 执行前推荐参数:
