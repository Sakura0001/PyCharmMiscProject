...
--   组件
-- 前置条件:
--   一个 PMP 上挂载的 log record 需要跨多个 TLB 拆分
-- 测试步骤:
--   1. 调用 fillSliceFragment 消费部分 log record。
--   2. 触发 TLB 满并 flush。
--   3. 再次调用 fillSliceFragment。
-- 预期结果:
--   PMP 不会被过早从 flush session 列表删除；已消费偏移正确记录；后续调用从正确位置继续。
-- 验证点:
--   PMP 生命周期、remaining bytes、无重复/遗漏复制。
--
-- LargeSLB 执行前推荐参数:
--   当前内核查询范围:
